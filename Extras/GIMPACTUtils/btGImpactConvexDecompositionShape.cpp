...
class GIM_ConvexDecomposition
 ...
 public:
  ...
  GIM_ConvexDecomposition(btGImpactConvexDecompositionShape* compoundShape,
                          bool transformSubShapes) {
    mBaseCount = 0;
    mHullCount = 0;
    ...
    if (m_transformSubShapes) {
      ...
      for (unsigned int i = 0; i < result.mHullVcount; i++) {
        btVector3 vertex(result.mHullVertices[i * 3],
                         result.mHullVertices[i * 3 + 1],
                         result.mHullVertices[i * 3 + 2]);
...
void btGImpactConvexDecompositionShape::buildConvexDecomposition(
    bool transformSubShapes) {
...
void btGImpactConvexDecompositionShape::processAllTriangles(
