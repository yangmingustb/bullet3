...
vec3 randomInertiaPrincipal() {
  ...
  } while (inertia(0) + inertia(1) < inertia(2) ||
...
mat33 randomInertiaMatrix() {
  ...
  mat33 rot(transformX(randomFloat(-BT_ID_PI, BT_ID_PI)) *
            transformY(randomFloat(-BT_ID_PI, BT_ID_PI)) *
...
vec3 randomAxis() {
  ...
  do {
    ...
    length = BT_ID_SQRT(BT_ID_POW(axis(0), 2) + BT_ID_POW(axis(1), 2) +
                        BT_ID_POW(axis(2), 2));
  } while (length < 0.01);
