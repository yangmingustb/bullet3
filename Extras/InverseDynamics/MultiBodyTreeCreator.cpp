...
MultiBodyTree* CreateMultiBodyTree(const MultiBodyTreeCreator& creator) {
  ...
  for (int index = 0; index < num_bodies; index++) {
    ...
    if (-1 ==
        creator.getBody(index, &parent_index, &joint_type,
                        &body_r_parent_body_ref, &body_R_parent_ref,
    ...
    if (-1 ==
        tree->addBody(index, parent_index, joint_type, body_r_parent_body_ref,
                      body_R_parent_ref, body_axis_of_motion, mass,
                      body_r_body_com, body_I_body, user_int, user_ptr)) {
