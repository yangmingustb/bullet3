...
int compareInverseAndForwardDynamics(vecx &q, vecx &u, vecx &dot_u,
                                     btVector3 &gravity, bool verbose,
                                     btMultiBody *btmb, MultiBodyTree *id_tree,
                                     double *pos_error, double *acc_error) {
...
#define RETURN_ON_FAILURE(x)                   \
    ...
    for (int i = 0; i < 3; i++) {
      if (verbose) {
        printf("bt::base_dot_omega(%d)= %e dot_u[%d]= %e, diff= %e\n", i,
               base_dot_omega(i), i, dot_u[i], base_dot_omega(i) - dot_u[i]);
      }
    ...
    for (int i = 0; i < 3; i++) {
      if (verbose) {
        printf("bt::base_ddot_com(%d)= %e dot_u[%d]= %e, diff= %e\n", i,
               base_ddot_com(i), i, dot_u[i + 3],
               base_ddot_com(i) - dot_u[i + 3]);
      }
  ...
  *pos_error = 0.0;

  {
    ...
    if (verbose) {
      ...
      printf(
          ...
          "              %f %f %f\n"
          "              %f %f %f\n",
          world_T_body(0, 0), world_T_body(0, 1), world_T_body(0, 2),
          world_T_body(1, 0), world_T_body(1, 1), world_T_body(1, 2),
          world_T_body(2, 0), world_T_body(2, 1), world_T_body(2, 2));
    }
  }
  ...
  for (int l = 0; l < btmb->getNumLinks(); l++) {
    ...
    if (verbose) {
      printf("------------- link %d\n", l + 1);
      printf("bt:com:       %f %f %f\n", bt_origin(0), bt_origin(1),
             bt_origin(2));
      printf(
          "bt:transform: %f %f %f\n"
          "              %f %f %f\n"
          "              %f %f %f\n",
          bt_basis(0, 0), bt_basis(0, 1), bt_basis(0, 2), bt_basis(1, 0),
          bt_basis(1, 1), bt_basis(1, 2), bt_basis(2, 0), bt_basis(2, 1),
          bt_basis(2, 2));
    }
    ...
    if (verbose) {
      ...
      printf("diff-transform: %e %e %e %e %e %e %e %e %e\n", diff_basis(0, 0),
             diff_basis(0, 1), diff_basis(0, 2), diff_basis(1, 0),
             diff_basis(1, 1), diff_basis(1, 2), diff_basis(2, 0),
             diff_basis(2, 1), diff_basis(2, 2));
    }
    double total_pos_err = BT_ID_SQRT(
        BT_ID_POW(diff_com(0), 2) + BT_ID_POW(diff_com(1), 2) +
