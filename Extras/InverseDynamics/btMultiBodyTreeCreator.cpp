...
int btMultiBodyTreeCreator::createFromBtMultiBody(const btMultiBody *btmb,
                                                  const bool verbose) {
  if (0x0 == btmb) {
  ...
  // add base link data
  {
    ...
    // BulletDynamics uses body-fixed frame in the cog, aligned with principal
    // axes
    ...
    if (verbose) {
      id_printf(
          ...
          btmb->getBaseInertia()[2], link.body_I_body(0, 0),
          link.body_I_body(0, 1), link.body_I_body(0, 2),
          link.body_I_body(1, 0), link.body_I_body(1, 1),
          link.body_I_body(1, 2), link.body_I_body(2, 0),
          link.body_I_body(2, 1), link.body_I_body(2, 2));
    }
  }
  ...
  for (int bt_index = 0; bt_index < btmb->getNumLinks(); bt_index++) {
    ...
    if (verbose) {
      id_printf("com= %f %f %f\n", link.body_r_body_com[0],
                link.body_r_body_com[1], link.body_r_body_com[2]);
    }
    // BulletDynamics uses a body-fixed frame in the CoM, aligned with principal
    // axes
    ...
    if (verbose) {
      id_printf(
          ...
          "    %f %f %f;\n"
          "    %f %f %f]\n",
          bt_index, link.mass, bt_link.m_inertiaLocal[0],
          bt_link.m_inertiaLocal[1], bt_link.m_inertiaLocal[2],
          link.body_I_body(0, 0), link.body_I_body(0, 1),
          link.body_I_body(0, 2), link.body_I_body(1, 0),
          link.body_I_body(1, 1), link.body_I_body(1, 2),
          link.body_I_body(2, 0), link.body_I_body(2, 1),
          link.body_I_body(2, 2));
    }
    // transform for vectors written in parent frame to this link's body-fixed
    // frame
    ...
    if (verbose) {
      id_printf(
          ...
          "                   %f %f %f\n"
          "                   %f %f %f\n",
          basis[0][0], basis[0][1], basis[0][2], basis[1][0], basis[1][1],
          basis[1][2], basis[2][0], basis[2][1], basis[2][2]);
    }
    switch (bt_link.m_jointType) {
      ...
      case btMultibodyLink::ePrismatic:
        ...
        //                                but seems to be: from parent's com to
        //                                parent's
        //                                pivot ??
        //                       m_dVector = thisPivotToThisComOffset
        link.parent_r_parent_body_ref(0) = bt_link.m_eVector[0];
    ...
    if (link.parent_index > 0) {  // parent body isn't the root
      const btMultibodyLink &bt_parent_link =
          btmb->getLink(link.parent_index - 1);
    ...
    if (verbose) {
      id_printf("parent_r_parent_body_ref= %f %f %f\n",
                ...
                link.parent_r_parent_body_ref[1],
                link.parent_r_parent_body_ref[2]);
    }
  }
...
int btMultiBodyTreeCreator::getBody(const int body_index, int *parent_index,
                                    ...
                                    vec3 *body_r_body_com, mat33 *body_I_body,
                                    int *user_int, void **user_ptr) const {
  if (false == m_initialized) {
