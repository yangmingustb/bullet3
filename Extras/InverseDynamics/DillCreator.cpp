...
DillCreator::DillCreator(int level)
    : m_level(level), m_num_bodies(BT_ID_POW(2, level)) {
...
int DillCreator::getBody(const int body_index, int* parent_index,
                         ...
                         mat33* body_T_parent_ref, vec3* body_axis_of_motion,
                         idScalar* mass, vec3* body_r_body_com,
                         mat33* body_I_body, int* user_int,
                         void** user_ptr) const {
...
int DillCreator::recurseDill(const int level, const int parent,
                             const idScalar d_DH_in, const idScalar a_DH_in,
                             const idScalar alpha_DH_in) {
  if (level < 0) {
