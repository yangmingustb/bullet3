/// Current makesdna.cpp is a from Blender, but we will completely rewrite it in
/// C++ under a ZLib license
/// The Original version is at
/// https://svn.blender.org/svnroot/bf-blender/trunk/blender/source/blender/makesdna/intern/makesdna.c
...
char *includefiles[] = {
    ...
    "../makesdna/DNA_rigidbody.h", "../../../src/LinearMath/btVector3.h",
...
short
    *alphalens; /* contains sizes as they are calculated on the DEC Alpha (64
                   bits) */
short **structs,
    *structdata; /* at sp= structs[a] is the first adress of a struct definition
                                                      sp[0] is type number
                                                      sp[1] is amount of
                    elements
                                                      sp[2] sp[3] is typenr,
...
const char *skipStructTypes[] = {"btContactSolverInfoData",
...
int convert_include(char *filename) {
  ...
  */
  ...
  overslaan = 0;
  while (count < filelen) {
    /* code for skipping a struct: two hashes on 2 lines. (preprocess added a
     * space) */
    if (md[0] == '#' && md[1] == ' ' && md[2] == '#') {
      overslaan = 1;
    }
...
static int calculate_structlens(int firststruct) {
  int a, b, len, alphalen, unknown = nr_structs, lastunknown, structtype, type,
                           mul, namelen;
  short *sp, *structpoin;
  char *cp;
    ...
    if (0) {
      ...
      if (fp == NULL)
        ...
        for (a = firststruct; a < nr_types; a++, sp++) {
          if (*sp) {
            fprintf(fp, "\tif(sizeof(struct %s) - %d) printf(\"ALIGN ERROR:",
                    types[a], *sp);
