...
static int get_int_attribute_by_name(const XMLElement* pElement,
                                     const char* attribName, int* value) {
  if (!pElement) return 0;

  const XMLAttribute* pAttrib = pElement->FirstAttribute();
  while (pAttrib) {
    if (!strcmp(pAttrib->Name(), attribName))
      if (pAttrib->QueryIntValue(value) == XML_SUCCESS) return 1;
...
void stringToFloatArray(const std::string& string,
...
void btBulletXmlWorldImporter::deSerializeVector3FloatData(
    XMLNode* pParent, btAlignedObjectArray<btVector3FloatData>& vectors) {
...
#define SET_INT_VALUE(xmlnode, targetdata, argname)              \
  btAssert((xmlnode)->FirstChildElement(#argname) &&             \
           (xmlnode)->FirstChildElement(#argname)->ToElement()); \
  if ((xmlnode)->FirstChildElement(#argname) &&                  \
      (xmlnode)->FirstChildElement(#argname)->ToElement())       \
    (targetdata)->argname = (int)atof(                           \
        xmlnode->FirstChildElement(#argname)->ToElement()->GetText());
  ...
  {                                                                       \
    ...
    if (xmlnode) {                                                        \
      ...
      if (eleNode && eleNode->FirstChildElement()) {                      \
        const char* txt =                                                 \
            eleNode->FirstChildElement()->ToElement()->GetText();         \
        btVector3FloatData vec4 = TextToVector3Data(txt);                 \
...
void btBulletXmlWorldImporter::deSerializeCollisionShapeData(
    XMLNode* pParent, btCollisionShapeData* colShapeData) {
  SET_INT_VALUE(pParent, colShapeData, m_shapeType)
  colShapeData->m_name = 0;
}

void btBulletXmlWorldImporter::deSerializeConvexHullShapeData(
    XMLNode* pParent) {
  MyLocalCaster cast;
  get_int_attribute_by_name(pParent->ToElement(), "pointer", &cast.m_int);

  btConvexHullShapeData* convexHullData =
      (btConvexHullShapeData*)btAlignedAlloc(sizeof(btConvexHullShapeData), 16);

  XMLNode* xmlConvexInt =
      pParent->FirstChildElement("m_convexInternalShapeData");
  btAssert(xmlConvexInt);

  XMLNode* xmlColShape =
      xmlConvexInt->FirstChildElement("m_collisionShapeData");
  btAssert(xmlColShape);

  deSerializeCollisionShapeData(
      xmlColShape,
      &convexHullData->m_convexInternalShapeData.m_collisionShapeData);

  SET_FLOAT_VALUE(xmlConvexInt, &convexHullData->m_convexInternalShapeData,
                  m_collisionMargin)
  SET_VECTOR4_VALUE(xmlConvexInt, &convexHullData->m_convexInternalShapeData,
                    m_localScaling)
  SET_VECTOR4_VALUE(xmlConvexInt, &convexHullData->m_convexInternalShapeData,
...

  {
    ...
    if (node) {
      ...
      cast.m_int = (int)atof(txt);
      (*convexHullData).m_unscaledPointsFloatPtr =
          (btVector3FloatData*)cast.m_ptr;
    }
  }

  SET_POINTER_VALUE(pParent, *convexHullData, m_unscaledPointsFloatPtr,
                    btVector3FloatData*);
  SET_POINTER_VALUE(pParent, *convexHullData, m_unscaledPointsDoublePtr,
                    btVector3DoubleData*);
...
void btBulletXmlWorldImporter::deSerializeCompoundShapeChildData(
    XMLNode* pParent) {
  MyLocalCaster cast;
  get_int_attribute_by_name(pParent->ToElement(), "pointer", &cast.m_int);

  int numChildren = 0;
  btAlignedObjectArray<btCompoundShapeChildData>* compoundChildArrayPtr =
      new btAlignedObjectArray<btCompoundShapeChildData>;
  {
    ...
    while (transNode && colShapeNode && marginNode && childTypeNode) {
      ...
      SET_VECTOR4_VALUE(transNode, &compoundChildArrayPtr->at(i).m_transform,
                        m_origin)
      SET_MATRIX33_VALUE(transNode, &compoundChildArrayPtr->at(i).m_transform,
                         m_basis)

      const char* txt = (colShapeNode)->ToElement()->GetText();
      MyLocalCaster cast;
      cast.m_int = (int)atof(txt);
      compoundChildArrayPtr->at(i).m_childShape =
          (btCollisionShapeData*)cast.m_ptr;

      btAssert(childTypeNode->ToElement());
      if (childTypeNode->ToElement()) {
        compoundChildArrayPtr->at(i).m_childShapeType =
            (int)atof(childTypeNode->ToElement()->GetText());
      }

      btAssert(marginNode->ToElement());
      if (marginNode->ToElement()) {
        compoundChildArrayPtr->at(i).m_childMargin =
            (float)atof(marginNode->ToElement()->GetText());
      }
...
void btBulletXmlWorldImporter::deSerializeCompoundShapeData(XMLNode* pParent) {
  ...
  btCompoundShapeData* compoundData =
      (btCompoundShapeData*)btAlignedAlloc(sizeof(btCompoundShapeData), 16);

  XMLNode* xmlColShape = pParent->FirstChildElement("m_collisionShapeData");
  btAssert(xmlColShape);
  deSerializeCollisionShapeData(xmlColShape,
                                &compoundData->m_collisionShapeData);
...

  {
    ...
    // SET_POINTER_VALUE(xmlColShape,
    // *compoundData,m_childShapePtr,btCompoundShapeChildData*);
  }
...
void btBulletXmlWorldImporter::deSerializeStaticPlaneShapeData(
    XMLNode* pParent) {
  MyLocalCaster cast;
  get_int_attribute_by_name(pParent->ToElement(), "pointer", &cast.m_int);

  btStaticPlaneShapeData* planeData = (btStaticPlaneShapeData*)btAlignedAlloc(
      sizeof(btStaticPlaneShapeData), 16);
...
void btBulletXmlWorldImporter::deSerializeConvexInternalShapeData(
    XMLNode* pParent) {
  MyLocalCaster cast;
  get_int_attribute_by_name(pParent->ToElement(), "pointer", &cast.m_int);

  btConvexInternalShapeData* convexShape =
      (btConvexInternalShapeData*)btAlignedAlloc(
          sizeof(btConvexInternalShapeData), 16);
  memset(convexShape, 0, sizeof(btConvexInternalShapeData));

  XMLNode* xmlShapeData = pParent->FirstChildElement("m_collisionShapeData");
  btAssert(xmlShapeData);

  deSerializeCollisionShapeData(xmlShapeData,
                                &convexShape->m_collisionShapeData);
...
void btBulletXmlWorldImporter::deSerializeGeneric6DofConstraintData(
    XMLNode* pParent) {
  MyLocalCaster cast;
  get_int_attribute_by_name(pParent->ToElement(), "pointer", &cast.m_int);

  btGeneric6DofConstraintData2* dof6Data =
      (btGeneric6DofConstraintData2*)btAlignedAlloc(
          sizeof(btGeneric6DofConstraintData2), 16);

  XMLNode* n = pParent->FirstChildElement("m_typeConstraintData");
  if (n) {
    SET_POINTER_VALUE(n, dof6Data->m_typeConstraintData, m_rbA,
                      btRigidBodyData*);
    SET_POINTER_VALUE(n, dof6Data->m_typeConstraintData, m_rbB,
                      btRigidBodyData*);
    ...
    SET_INT_VALUE(n, &dof6Data->m_typeConstraintData,
                  m_disableCollisionsBetweenLinkedBodies);
    SET_INT_VALUE(n, &dof6Data->m_typeConstraintData,
                  m_overrideNumSolverIterations);
    SET_FLOAT_VALUE(n, &dof6Data->m_typeConstraintData,
...
void btBulletXmlWorldImporter::deSerializeRigidBodyFloatData(XMLNode* pParent) {
  ...
  if (!get_int_attribute_by_name(pParent->ToElement(), "pointer",
                                 &cast.m_int)) {
    m_fileOk = false;
    return;
  }
  ...
  btRigidBodyData* rbData =
      (btRigidBodyData*)btAlignedAlloc(sizeof(btRigidBodyData), 16);

  XMLNode* n = pParent->FirstChildElement("m_collisionObjectData");

  if (n) {
    SET_POINTER_VALUE(n, rbData->m_collisionObjectData, m_collisionShape,
                      void*);
    SET_TRANSFORM_VALUE(n, &rbData->m_collisionObjectData, m_worldTransform);
    SET_TRANSFORM_VALUE(n, &rbData->m_collisionObjectData,
                        m_interpolationWorldTransform);
    SET_VECTOR4_VALUE(n, &rbData->m_collisionObjectData,
    ...
    SET_FLOAT_VALUE(n, &rbData->m_collisionObjectData,
...
void btBulletXmlWorldImporter::fixupConstraintData(
    btTypedConstraintData2* tcd) {
  if (tcd->m_rbA) {
    btRigidBodyData** ptrptr =
        (btRigidBodyData**)m_pointerLookup.find(tcd->m_rbA);
    btAssert(ptrptr);
    tcd->m_rbA = ptrptr ? *ptrptr : 0;
  }
  if (tcd->m_rbB) {
    btRigidBodyData** ptrptr =
        (btRigidBodyData**)m_pointerLookup.find(tcd->m_rbB);
    btAssert(ptrptr);
    tcd->m_rbB = ptrptr ? *ptrptr : 0;
  }
}

void btBulletXmlWorldImporter::fixupCollisionDataPointers(
  ...
  switch (shapeData->m_shapeType) {
    ...
    case CONVEX_HULL_SHAPE_PROXYTYPE: {
      ...
      btVector3FloatData** ptrptr = (btVector3FloatData**)m_pointerLookup.find(
          (void*)convexData->m_unscaledPointsFloatPtr);
      btAssert(ptrptr);
      if (ptrptr) {
...
void btBulletXmlWorldImporter::auto_serialize_root_level_children(
    XMLNode* pParent) {
  int numChildren = 0;
  btAssert(pParent);
  if (pParent) {
    XMLNode* pChild;
    for (pChild = pParent->FirstChildElement(); pChild != 0;
         pChild = pChild->NextSibling(), numChildren++) {
      //			printf("child Name=%s\n", pChild->Value());
      if (!strcmp(pChild->Value(), "btVector3FloatData")) {
        ...
        btVector3FloatData* vectors = (btVector3FloatData*)btAlignedAlloc(
            sizeof(btVector3FloatData) * numVectors, 16);
        for (int i = 0; i < numVectors; i++) vectors[i] = v[i];
  ...
  for (int i = 0; i < m_compoundShapeChildDataArrays.size(); i++) {
    btAlignedObjectArray<btCompoundShapeChildData>* childDataArray =
        m_compoundShapeChildDataArrays[i];
    for (int c = 0; c < childDataArray->size(); c++) {
      btCompoundShapeChildData* childData = &childDataArray->at(c);
      btCollisionShapeData** ptrptr =
          (btCollisionShapeData**)m_pointerLookup[childData->m_childShape];
      btAssert(ptrptr);
      if (ptrptr) {
  ...
  for (int i = 0; i < m_rigidBodyData.size(); i++) {
    ...
    void** ptrptr =
        m_pointerLookup.find(rbData->m_collisionObjectData.m_collisionShape);
    // btAssert(ptrptr);
...
bool btBulletXmlWorldImporter::loadFile(const char* fileName) {
  ...
  if (loadOkay == XML_SUCCESS) {
    if (get_int_attribute_by_name(doc.FirstChildElement()->ToElement(),
                                  "version", &m_fileVersion)) {
      if (m_fileVersion == 281) {
        m_fileOk = true;
        int itemcount;
        get_int_attribute_by_name(doc.FirstChildElement()->ToElement(),
                                  "itemcount", &itemcount);
