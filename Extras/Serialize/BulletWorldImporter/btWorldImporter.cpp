...
btWorldImporter::btWorldImporter(btDynamicsWorld* world)
    : m_dynamicsWorld(world), m_verboseMode(0), m_importerFlags(0) {}
...
void btWorldImporter::deleteAllData() {
  ...
  for (i = 0; i < m_allocatedRigidBodies.size(); i++) {
    if (m_dynamicsWorld)
      m_dynamicsWorld->removeRigidBody(
          btRigidBody::upcast(m_allocatedRigidBodies[i]));
  ...
  for (i = 0; i < m_allocatedbtStridingMeshInterfaceDatas.size(); i++) {
    btStridingMeshInterfaceData* curData =
        m_allocatedbtStridingMeshInterfaceDatas[i];

    for (int a = 0; a < curData->m_numMeshParts; a++) {
      btMeshPartData* curPart = &curData->m_meshPartsPtr[a];
      if (curPart->m_vertices3f) delete[] curPart->m_vertices3f;

      if (curPart->m_vertices3d) delete[] curPart->m_vertices3d;

      if (curPart->m_indices32) delete[] curPart->m_indices32;

      if (curPart->m_3indices16) delete[] curPart->m_3indices16;

      if (curPart->m_indices16) delete[] curPart->m_indices16;

      if (curPart->m_3indices8) delete[] curPart->m_3indices8;
    }
  ...
  switch (shapeData->m_shapeType) {
    ...
    case SCALED_TRIANGLE_MESH_SHAPE_PROXYTYPE: {
      btScaledTriangleMeshShapeData* scaledMesh =
          (btScaledTriangleMeshShapeData*)shapeData;
      btCollisionShapeData* colShapeData =
          (btCollisionShapeData*)&scaledMesh->m_trimeshShapeData;
    ...
    case GIMPACT_SHAPE_PROXYTYPE: {
      ...
      if (gimpactData->m_gimpactSubType == CONST_GIMPACT_TRIMESH_SHAPE) {
        btStridingMeshInterfaceData* interfaceData =
            createStridingMeshInterfaceData(&gimpactData->m_meshInterface);
        btTriangleIndexVertexArray* meshInterface =
            createMeshInterface(*interfaceData);
    ...
    // The btCapsuleShape* API has issue passing the margin/scaling/halfextents
    // unmodified through the API
    // so deal with this
    case CAPSULE_SHAPE_PROXYTYPE: {
      ...
      switch (capData->m_upAxis) {
        ...
        default: { printf("error: wrong up axis for btCapsuleShape\n"); }
      };
      if (shape) {
    ...
    case CONVEX_HULL_SHAPE_PROXYTYPE: {
      ...
      btVector3 margin(bsd->m_collisionMargin, bsd->m_collisionMargin,
                       bsd->m_collisionMargin);
      switch (shapeData->m_shapeType) {
        case BOX_SHAPE_PROXYTYPE: {
          btBoxShape* box = (btBoxShape*)createBoxShape(
              implicitShapeDimensions / localScaling + margin);
        ...
        case CYLINDER_SHAPE_PROXYTYPE: {
          ...
          switch (cylData->m_upAxis) {
            case 0: {
              shape =
                  createCylinderShapeX(halfExtents.getY(), halfExtents.getX());
              break;
            }
            case 1: {
              shape =
                  createCylinderShapeY(halfExtents.getX(), halfExtents.getY());
              break;
            }
            case 2: {
              shape =
                  createCylinderShapeZ(halfExtents.getX(), halfExtents.getZ());
              break;
            }
            default: { printf("unknown Cylinder up axis\n"); }
          };
        ...
        case CONE_SHAPE_PROXYTYPE: {
          ...
          switch (conData->m_upIndex) {
            ...
            default: { printf("unknown Cone up axis\n"); }
          };
        ...
        case CONVEX_HULL_SHAPE_PROXYTYPE: {
          ...
          for (i = 0; i < numPoints; i++) {
            ...
            if (convexData->m_unscaledPointsDoublePtr)
              tmpPoints[i].deSerialize(
                  convexData->m_unscaledPointsDoublePtr[i]);
            if (convexData->m_unscaledPointsFloatPtr)
              tmpPoints[i].deSerializeFloat(
                  convexData->m_unscaledPointsFloatPtr[i]);
#else
            if (convexData->m_unscaledPointsFloatPtr)
              tmpPoints[i].deSerialize(convexData->m_unscaledPointsFloatPtr[i]);
            if (convexData->m_unscaledPointsDoublePtr)
              tmpPoints[i].deSerializeDouble(
                  convexData->m_unscaledPointsDoublePtr[i]);
    ...
    case TRIANGLE_MESH_SHAPE_PROXYTYPE: {
      ...
      btStridingMeshInterfaceData* interfaceData =
          createStridingMeshInterfaceData(&trimesh->m_meshInterface);
      btTriangleIndexVertexArray* meshInterface =
          createMeshInterface(*interfaceData);
      ...
      btBvhTriangleMeshShape* trimeshShape =
          createBvhTriangleMeshShape(meshInterface, bvh);
    ...
    case COMPOUND_SHAPE_PROXYTYPE: {
      ...
      for (int i = 0; i < compoundData->m_numChildShapes; i++) {
        btCollisionShapeData* cd =
            compoundData->m_childShapePtr[i].m_childShape;

        btCollisionShape* childShape = convertCollisionShape(cd);
        if (childShape) {
          btTransform localTransform;
          localTransform.deSerializeFloat(
              compoundData->m_childShapePtr[i].m_transform);
...
void btWorldImporter::convertConstraintBackwardsCompatible281(
    btTypedConstraintData* constraintData, btRigidBody* rbA, btRigidBody* rbB,
    int fileVersion) {
  btTypedConstraint* constraint = 0;

  switch (constraintData->m_objectType) {
    case POINT2POINT_CONSTRAINT_TYPE: {
      btPoint2PointConstraintDoubleData* p2pData =
          (btPoint2PointConstraintDoubleData*)constraintData;
      if (rbA && rbB) {
        btVector3 pivotInA, pivotInB;
        pivotInA.deSerializeDouble(p2pData->m_pivotInA);
        pivotInB.deSerializeDouble(p2pData->m_pivotInB);
        constraint =
            createPoint2PointConstraint(*rbA, *rbB, pivotInA, pivotInB);
      } else {
        btVector3 pivotInA;
        pivotInA.deSerializeDouble(p2pData->m_pivotInA);
    ...
    case HINGE_CONSTRAINT_TYPE: {
      ...
      btHingeConstraintDoubleData* hingeData =
          (btHingeConstraintDoubleData*)constraintData;
      if (rbA && rbB) {
        ...
        hinge = createHingeConstraint(*rbA, *rbB, rbAFrame, rbBFrame,
                                      hingeData->m_useReferenceFrameA != 0);
      } else {
        btTransform rbAFrame;
        rbAFrame.deSerializeDouble(hingeData->m_rbAFrame);
        hinge = createHingeConstraint(*rbA, rbAFrame,
                                      hingeData->m_useReferenceFrameA != 0);
      }
      if (hingeData->m_enableAngularMotor) {
        hinge->enableAngularMotor(true,
                                  (btScalar)hingeData->m_motorTargetVelocity,
                                  (btScalar)hingeData->m_maxMotorImpulse);
      }
      hinge->setAngularOnly(hingeData->m_angularOnly != 0);
      hinge->setLimit(btScalar(hingeData->m_lowerLimit),
    ...
    case CONETWIST_CONSTRAINT_TYPE: {
      btConeTwistConstraintData* coneData =
          (btConeTwistConstraintData*)constraintData;
      ...
      coneTwist->setLimit(
          (btScalar)coneData->m_swingSpan1, (btScalar)coneData->m_swingSpan2,
          (btScalar)coneData->m_twistSpan, (btScalar)coneData->m_limitSoftness,
          (btScalar)coneData->m_biasFactor,
          (btScalar)coneData->m_relaxationFactor);
    ...
    case D6_SPRING_CONSTRAINT_TYPE: {
      btGeneric6DofSpringConstraintData* dofData =
          (btGeneric6DofSpringConstraintData*)constraintData;
      ...
      if (rbA && rbB) {
        ...
        dof = createGeneric6DofSpringConstraint(
            *rbA, *rbB, rbAFrame, rbBFrame,
            dofData->m_6dofData.m_useLinearReferenceFrameA != 0);
      } else {
        printf(
            "Error in btWorldImporter::createGeneric6DofSpringConstraint: "
            "requires rbA && rbB\n");
      }
      ...
      if (dof) {
        ...
        if (fileVersion > 280) {
          for (i = 0; i < 6; i++) {
            ...
            dof->setEquilibriumPoint(i,
                                     (btScalar)dofData->m_equilibriumPoint[i]);
    ...
    case D6_CONSTRAINT_TYPE: {
      btGeneric6DofConstraintData* dofData =
          (btGeneric6DofConstraintData*)constraintData;
      ...
      if (rbA && rbB) {
        ...
        dof = createGeneric6DofConstraint(
            *rbA, *rbB, rbAFrame, rbBFrame,
            dofData->m_useLinearReferenceFrameA != 0);
      } else {
        if (rbB) {
          ...
          printf(
              "Error in btWorldImporter::createGeneric6DofConstraint: missing "
              "rbB\n");
        }
      }
    ...
    case SLIDER_CONSTRAINT_TYPE: {
      btSliderConstraintData* sliderData =
          (btSliderConstraintData*)constraintData;
      ...
      if (rbA && rbB) {
        ...
        slider =
            createSliderConstraint(*rbA, *rbB, rbAFrame, rbBFrame,
                                   sliderData->m_useLinearReferenceFrameA != 0);
      } else {
        btTransform rbBFrame;
        rbBFrame.deSerializeFloat(sliderData->m_rbBFrame);
        slider = createSliderConstraint(
            *rbB, rbBFrame, sliderData->m_useLinearReferenceFrameA != 0);
      }
    ...
    default: { printf("unknown constraint type\n"); }
  };

  if (constraint) {
    constraint->setDbgDrawSize((btScalar)constraintData->m_dbgDrawSize);
    /// those fields didn't exist and set to zero for pre-280 versions, so do a
    /// check here
    if (fileVersion >= 280) {
      constraint->setBreakingImpulseThreshold(
          (btScalar)constraintData->m_breakingImpulseThreshold);
      constraint->setEnabled(constraintData->m_isEnabled != 0);
      constraint->setOverrideNumSolverIterations(
          constraintData->m_overrideNumSolverIterations);
    }
    ...
    if (m_dynamicsWorld)
      m_dynamicsWorld->addConstraint(
          constraint,
          constraintData->m_disableCollisionsBetweenLinkedBodies != 0);
  }
}

void btWorldImporter::convertConstraintFloat(
    btTypedConstraintFloatData* constraintData, btRigidBody* rbA,
    btRigidBody* rbB, int fileVersion) {
  btTypedConstraint* constraint = 0;

  switch (constraintData->m_objectType) {
    case POINT2POINT_CONSTRAINT_TYPE: {
      btPoint2PointConstraintFloatData* p2pData =
          (btPoint2PointConstraintFloatData*)constraintData;
      if (rbA && rbB) {
        btVector3 pivotInA, pivotInB;
    ...
    case HINGE_CONSTRAINT_TYPE: {
      ...
      btHingeConstraintFloatData* hingeData =
          (btHingeConstraintFloatData*)constraintData;
      if (rbA && rbB) {
        btTransform rbAFrame, rbBFrame;
      ...
      if (hingeData->m_enableAngularMotor) {
        hinge->enableAngularMotor(true, hingeData->m_motorTargetVelocity,
                                  hingeData->m_maxMotorImpulse);
      }
      hinge->setAngularOnly(hingeData->m_angularOnly != 0);
      hinge->setLimit(btScalar(hingeData->m_lowerLimit),
                      btScalar(hingeData->m_upperLimit),
    ...
    case CONETWIST_CONSTRAINT_TYPE: {
      ...
      coneTwist->setLimit(coneData->m_swingSpan1, coneData->m_swingSpan2,
                          coneData->m_twistSpan, coneData->m_limitSoftness,
                          coneData->m_biasFactor, coneData->m_relaxationFactor);
    ...
    case GEAR_CONSTRAINT_TYPE: {
      btGearConstraintFloatData* gearData =
          (btGearConstraintFloatData*)constraintData;
      ...
      if (rbA && rbB) {
        ...
        gear = createGearConstraint(*rbA, *rbB, axisInA, axisInB,
                                    gearData->m_ratio);
      } else {
        btAssert(0);
        // perhaps a gear against a 'fixed' body, while the 'fixed' body is not
        // serialized?
        // btGearConstraint(btRigidBody& rbA, btRigidBody& rbB, const btVector3&
        // axisInA,const btVector3& axisInB, btScalar ratio=1.f);
      }
      constraint = gear;
      break;
    }
    case D6_SPRING_2_CONSTRAINT_TYPE: {
      btGeneric6DofSpring2ConstraintData* dofData =
          (btGeneric6DofSpring2ConstraintData*)constraintData;
      ...
      if (rbA && rbB) {
        ...
        rbAFrame.deSerializeFloat(dofData->m_rbAFrame);
        rbBFrame.deSerializeFloat(dofData->m_rbBFrame);
        dof = createGeneric6DofSpring2Constraint(*rbA, *rbB, rbAFrame, rbBFrame,
                                                 dofData->m_rotateOrder);
      } else {
        printf(
            "Error in btWorldImporter::createGeneric6DofSpring2Constraint: "
            "requires rbA && rbB\n");
      }
        ...
        if (fileVersion > 280) {
          ...
          for (i = 0; i < 3; i++) {
            dof->setStiffness(i, dofData->m_linearSpringStiffness.m_floats[i],
                              dofData->m_linearSpringStiffnessLimited[i] != 0);
            dof->setEquilibriumPoint(
                i, dofData->m_linearEquilibriumPoint.m_floats[i]);
            dof->enableSpring(i, dofData->m_linearEnableSpring[i] != 0);
            dof->setDamping(i, dofData->m_linearSpringDamping.m_floats[i],
                            (dofData->m_linearSpringDampingLimited[i] != 0));
          }
          for (i = 0; i < 3; i++) {
            dof->setStiffness(
                i + 3, dofData->m_angularSpringStiffness.m_floats[i],
                (dofData->m_angularSpringStiffnessLimited[i] != 0));
            dof->setEquilibriumPoint(
                i + 3, dofData->m_angularEquilibriumPoint.m_floats[i]);
            dof->enableSpring(i + 3, dofData->m_angularEnableSpring[i] != 0);
            dof->setDamping(i + 3, dofData->m_angularSpringDamping.m_floats[i],
                            dofData->m_angularSpringDampingLimited[i]);
          }
        }
      }
    ...
    case FIXED_CONSTRAINT_TYPE: {
      ...
      if (rbA && rbB) {
        btTransform rbAFrame, rbBFrame;
        // compute a shared world frame, and compute frameInA, frameInB relative
        // to this
        btTransform sharedFrame;
        sharedFrame.setIdentity();
        btVector3 centerPos =
            btScalar(0.5) * (rbA->getWorldTransform().getOrigin() +
  ...
  if (constraint) {
    ...
    if (fileVersion >= 280) {
      constraint->setBreakingImpulseThreshold(
          constraintData->m_breakingImpulseThreshold);
      constraint->setEnabled(constraintData->m_isEnabled != 0);
      constraint->setOverrideNumSolverIterations(
          constraintData->m_overrideNumSolverIterations);
    }
...
void btWorldImporter::convertConstraintDouble(
    btTypedConstraintDoubleData* constraintData, btRigidBody* rbA,
    btRigidBody* rbB, int fileVersion) {
  btTypedConstraint* constraint = 0;

  switch (constraintData->m_objectType) {
    case POINT2POINT_CONSTRAINT_TYPE: {
      btPoint2PointConstraintDoubleData2* p2pData =
          (btPoint2PointConstraintDoubleData2*)constraintData;
      if (rbA && rbB) {
        btVector3 pivotInA, pivotInB;
        pivotInA.deSerializeDouble(p2pData->m_pivotInA);
        pivotInB.deSerializeDouble(p2pData->m_pivotInB);
    ...
    case HINGE_CONSTRAINT_TYPE: {
      ...
      btHingeConstraintDoubleData2* hingeData =
          (btHingeConstraintDoubleData2*)constraintData;
      if (rbA && rbB) {
        btTransform rbAFrame, rbBFrame;
        rbAFrame.deSerializeDouble(hingeData->m_rbAFrame);
        rbBFrame.deSerializeDouble(hingeData->m_rbBFrame);
    ...
    case CONETWIST_CONSTRAINT_TYPE: {
      btConeTwistConstraintDoubleData* coneData =
          (btConeTwistConstraintDoubleData*)constraintData;
      btConeTwistConstraint* coneTwist = 0;
    ...
    case D6_SPRING_CONSTRAINT_TYPE: {
      btGeneric6DofSpringConstraintDoubleData2* dofData =
          (btGeneric6DofSpringConstraintDoubleData2*)constraintData;
      //	int sz = sizeof(btGeneric6DofSpringConstraintData);
      btGeneric6DofSpringConstraint* dof = 0;
      ...
      if (dof) {
        btVector3 angLowerLimit, angUpperLimit, linLowerLimit, linUpperlimit;
        angLowerLimit.deSerializeDouble(
            dofData->m_6dofData.m_angularLowerLimit);
        angUpperLimit.deSerializeDouble(
            dofData->m_6dofData.m_angularUpperLimit);
    ...
    case D6_CONSTRAINT_TYPE: {
      btGeneric6DofConstraintDoubleData2* dofData =
          (btGeneric6DofConstraintDoubleData2*)constraintData;
      btGeneric6DofConstraint* dof = 0;
    ...
    case SLIDER_CONSTRAINT_TYPE: {
      btSliderConstraintDoubleData* sliderData =
          (btSliderConstraintDoubleData*)constraintData;
      btSliderConstraint* slider = 0;
      if (rbA && rbB) {
        btTransform rbAFrame, rbBFrame;
    ...
    case GEAR_CONSTRAINT_TYPE: {
      btGearConstraintDoubleData* gearData =
          (btGearConstraintDoubleData*)constraintData;
      btGearConstraint* gear = 0;
      if (rbA && rbB) {
        btVector3 axisInA, axisInB;
    ...
    case D6_SPRING_2_CONSTRAINT_TYPE: {
      btGeneric6DofSpring2ConstraintDoubleData2* dofData =
          (btGeneric6DofSpring2ConstraintDoubleData2*)constraintData;
      ...
      if (dof) {
        ...
        if (fileVersion > 280) {
          ...
          for (i = 0; i < 3; i++) {
            dof->setStiffness(i, dofData->m_linearSpringStiffness.m_floats[i],
                              dofData->m_linearSpringStiffnessLimited[i]);
            dof->setEquilibriumPoint(
                i, dofData->m_linearEquilibriumPoint.m_floats[i]);
            dof->enableSpring(i, dofData->m_linearEnableSpring[i] != 0);
            dof->setDamping(i, dofData->m_linearSpringDamping.m_floats[i],
          ...
          for (i = 0; i < 3; i++) {
            ...
            dof->enableSpring(i + 3, dofData->m_angularEnableSpring[i] != 0);
            dof->setDamping(i + 3, dofData->m_angularSpringDamping.m_floats[i],
                            (dofData->m_angularSpringDampingLimited[i] != 0));
          }
        }
      }
...
btTriangleIndexVertexArray* btWorldImporter::createMeshInterface(
  ...
  for (int i = 0; i < meshData.m_numMeshParts; i++) {
    ...
    if (meshData.m_meshPartsPtr[i].m_indices32) {
      ...
      int* indexArray =
          (int*)btAlignedAlloc(sizeof(int) * 3 * meshPart.m_numTriangles, 16);
      ...
      if (meshData.m_meshPartsPtr[i].m_3indices16) {
        ...
        meshPart.m_triangleIndexStride =
            sizeof(short int) * 3;  // sizeof(btShortIntIndexTripletData);

        short int* indexArray = (short int*)btAlignedAlloc(
            sizeof(short int) * 3 * meshPart.m_numTriangles, 16);
        m_shortIndexArrays.push_back(indexArray);

        for (int j = 0; j < meshPart.m_numTriangles; j++) {
          indexArray[3 * j] =
              meshData.m_meshPartsPtr[i].m_3indices16[j].m_values[0];
          indexArray[3 * j + 1] =
              meshData.m_meshPartsPtr[i].m_3indices16[j].m_values[1];
          indexArray[3 * j + 2] =
              meshData.m_meshPartsPtr[i].m_3indices16[j].m_values[2];
        }
      ...
      if (meshData.m_meshPartsPtr[i].m_3indices8) {
        ...
        unsigned char* indexArray = (unsigned char*)btAlignedAlloc(
            sizeof(unsigned char) * 3 * meshPart.m_numTriangles, 16);
        m_charIndexArrays.push_back(indexArray);

        for (int j = 0; j < meshPart.m_numTriangles; j++) {
          indexArray[3 * j] =
              meshData.m_meshPartsPtr[i].m_3indices8[j].m_values[0];
          indexArray[3 * j + 1] =
              meshData.m_meshPartsPtr[i].m_3indices8[j].m_values[1];
          indexArray[3 * j + 2] =
              meshData.m_meshPartsPtr[i].m_3indices8[j].m_values[2];
        }
    ...
    if (meshData.m_meshPartsPtr[i].m_vertices3f) {
      ...
      btVector3FloatData* vertices = (btVector3FloatData*)btAlignedAlloc(
      ...
      for (int j = 0; j < meshPart.m_numVertices; j++) {
        vertices[j].m_floats[0] =
            meshData.m_meshPartsPtr[i].m_vertices3f[j].m_floats[0];
        vertices[j].m_floats[1] =
            meshData.m_meshPartsPtr[i].m_vertices3f[j].m_floats[1];
        vertices[j].m_floats[2] =
            meshData.m_meshPartsPtr[i].m_vertices3f[j].m_floats[2];
        vertices[j].m_floats[3] =
            meshData.m_meshPartsPtr[i].m_vertices3f[j].m_floats[3];
      }
      ...
      btVector3DoubleData* vertices = (btVector3DoubleData*)btAlignedAlloc(
          sizeof(btVector3DoubleData) * meshPart.m_numVertices, 16);
      m_doubleVertexArrays.push_back(vertices);

      for (int j = 0; j < meshPart.m_numVertices; j++) {
        vertices[j].m_floats[0] =
            meshData.m_meshPartsPtr[i].m_vertices3d[j].m_floats[0];
        vertices[j].m_floats[1] =
            meshData.m_meshPartsPtr[i].m_vertices3d[j].m_floats[1];
        vertices[j].m_floats[2] =
            meshData.m_meshPartsPtr[i].m_vertices3d[j].m_floats[2];
        vertices[j].m_floats[3] =
            meshData.m_meshPartsPtr[i].m_vertices3d[j].m_floats[3];
      }
      meshPart.m_vertexBase = (const unsigned char*)vertices;
    }
...
btStridingMeshInterfaceData* btWorldImporter::createStridingMeshInterfaceData(
    btStridingMeshInterfaceData* interfaceData) {
  // create a new btStridingMeshInterfaceData that is an exact copy of shapedata
  // and store it in the WorldImporter
  ...
  for (int i = 0; i < newData->m_numMeshParts; i++) {
    ...
    if (curPart->m_vertices3f) {
      curNewPart->m_vertices3f =
          new btVector3FloatData[curNewPart->m_numVertices];
      memcpy(curNewPart->m_vertices3f, curPart->m_vertices3f,
             sizeof(btVector3FloatData) * curNewPart->m_numVertices);
    } else
      curNewPart->m_vertices3f = NULL;

    if (curPart->m_vertices3d) {
      curNewPart->m_vertices3d =
          new btVector3DoubleData[curNewPart->m_numVertices];
      memcpy(curNewPart->m_vertices3d, curPart->m_vertices3d,
             sizeof(btVector3DoubleData) * curNewPart->m_numVertices);
    } else
      curNewPart->m_vertices3d = NULL;

    int numIndices = curNewPart->m_numTriangles * 3;
    /// the m_3indices8 was not initialized in some Bullet versions, this can
    /// cause crashes at loading time
    /// we catch it by only dealing with m_3indices8 if none of the other
    ...
    if (curPart->m_indices32) {
      ...
      memcpy(curNewPart->m_indices32, curPart->m_indices32,
             sizeof(btIntIndexData) * numIndices);
    } else
      curNewPart->m_indices32 = NULL;

    if (curPart->m_3indices16) {
      uninitialized3indices8Workaround = true;
      curNewPart->m_3indices16 =
          new btShortIntIndexTripletData[curNewPart->m_numTriangles];
      memcpy(curNewPart->m_3indices16, curPart->m_3indices16,
             sizeof(btShortIntIndexTripletData) * curNewPart->m_numTriangles);
    } else
    ...
    if (curPart->m_indices16) {
      ...
      memcpy(curNewPart->m_indices16, curPart->m_indices16,
             sizeof(btShortIntIndexData) * numIndices);
    } else
      curNewPart->m_indices16 = NULL;

    if (!uninitialized3indices8Workaround && curPart->m_3indices8) {
      curNewPart->m_3indices8 =
          new btCharIndexTripletData[curNewPart->m_numTriangles];
      memcpy(curNewPart->m_3indices8, curPart->m_3indices8,
             sizeof(btCharIndexTripletData) * curNewPart->m_numTriangles);
    } else
...
static bool btAdjustInternalEdgeContactsCallback(
    btManifoldPoint& cp, const btCollisionObject* colObj0, int partId0,
    int index0, const btCollisionObject* colObj1, int partId1, int index1) {
  btAdjustInternalEdgeContacts(cp, colObj1, colObj0, partId1, index1);
  // btAdjustInternalEdgeContacts(cp,colObj1,colObj0, partId1,index1,
  // BT_TRIANGLE_CONVEX_BACKFACE_MODE);
  // btAdjustInternalEdgeContacts(cp,colObj1,colObj0, partId1,index1,
  // BT_TRIANGLE_CONVEX_DOUBLE_SIDED+BT_TRIANGLE_CONCAVE_DOUBLE_SIDED);
  return true;
}
#endif  // USE_INTERNAL_EDGE_UTILITY

btCollisionObject* btWorldImporter::createCollisionObject(
    const btTransform& startTransform, btCollisionShape* shape,
    const char* bodyName) {
  return createRigidBody(false, 0, startTransform, shape, bodyName);
}

void btWorldImporter::setDynamicsWorldInfo(
    const btVector3& gravity, const btContactSolverInfo& solverInfo) {
  if (m_dynamicsWorld) {
...
btRigidBody* btWorldImporter::createRigidBody(bool isDynamic, btScalar mass,
...
btCollisionShape* btWorldImporter::createPlaneShape(
...
btCollisionShape* btWorldImporter::createBoxShape(
...
btCollisionShape* btWorldImporter::createCylinderShapeX(btScalar radius,
  ...
  btCylinderShapeX* shape =
      new btCylinderShapeX(btVector3(height, radius, radius));
  m_allocatedCollisionShapes.push_back(shape);
  return shape;
}
...
btCollisionShape* btWorldImporter::createCylinderShapeY(btScalar radius,
  ...
  btCylinderShape* shape =
      new btCylinderShape(btVector3(radius, height, radius));
  m_allocatedCollisionShapes.push_back(shape);
  return shape;
}
...
btCollisionShape* btWorldImporter::createCylinderShapeZ(btScalar radius,
  ...
  btCylinderShapeZ* shape =
      new btCylinderShapeZ(btVector3(radius, radius, height));
  m_allocatedCollisionShapes.push_back(shape);
  return shape;
}
...
btBvhTriangleMeshShape* btWorldImporter::createBvhTriangleMeshShape(
  ...
  if (bvh) {
    btBvhTriangleMeshShape* bvhTriMesh =
        new btBvhTriangleMeshShape(trimesh, bvh->isQuantized(), false);
...
btCollisionShape* btWorldImporter::createConvexTriangleMeshShape(
    btStridingMeshInterface* trimesh) {
  return 0;
}
btGImpactMeshShape* btWorldImporter::createGimpactShape(
    btStridingMeshInterface* trimesh) {
#ifdef USE_GIMPACT
...
btScaledBvhTriangleMeshShape* btWorldImporter::createScaledTrangleMeshShape(
    btBvhTriangleMeshShape* meshShape, const btVector3& localScaling) {
  btScaledBvhTriangleMeshShape* shape =
      new btScaledBvhTriangleMeshShape(meshShape, localScaling);
  m_allocatedCollisionShapes.push_back(shape);
  return shape;
}

btMultiSphereShape* btWorldImporter::createMultiSphereShape(
    const btVector3* positions, const btScalar* radi, int numSpheres) {
  btMultiSphereShape* shape =
      new btMultiSphereShape(positions, radi, numSpheres);
  m_allocatedCollisionShapes.push_back(shape);
  return shape;
}

class btHeightfieldTerrainShape* btWorldImporter::createHeightfieldShape(
    int heightStickWidth, int heightStickLength, const void* heightfieldData,
    btScalar heightScale, btScalar minHeight, btScalar maxHeight, int upAxis,
    int heightDataType, bool flipQuadEdges) {
  btHeightfieldTerrainShape* shape = new btHeightfieldTerrainShape(
      heightStickWidth, heightStickLength, heightfieldData, heightScale,
      minHeight, maxHeight, upAxis, PHY_ScalarType(heightDataType),
      flipQuadEdges);
  m_allocatedCollisionShapes.push_back(shape);
  return shape;
}
...
btRigidBody& btWorldImporter::getFixedBody() {
  ...
  s_fixed.setMassProps(btScalar(0.),
                       btVector3(btScalar(0.), btScalar(0.), btScalar(0.)));
  return s_fixed;
}

btPoint2PointConstraint* btWorldImporter::createPoint2PointConstraint(
...
btHingeConstraint* btWorldImporter::createHingeConstraint(
...
btHingeConstraint* btWorldImporter::createHingeConstraint(
    btRigidBody& rbA, const btTransform& rbAFrame, bool useReferenceFrameA) {
  btHingeConstraint* hinge =
...
btConeTwistConstraint* btWorldImporter::createConeTwistConstraint(
    btRigidBody& rbA, btRigidBody& rbB, const btTransform& rbAFrame,
    const btTransform& rbBFrame) {
  btConeTwistConstraint* cone =
...
btGeneric6DofConstraint* btWorldImporter::createGeneric6DofConstraint(
  ...
  btGeneric6DofConstraint* dof = new btGeneric6DofConstraint(
      rbA, rbB, frameInA, frameInB, useLinearReferenceFrameA);
  m_allocatedConstraints.push_back(dof);
  return dof;
}

btGeneric6DofConstraint* btWorldImporter::createGeneric6DofConstraint(
    btRigidBody& rbB, const btTransform& frameInB,
    bool useLinearReferenceFrameB) {
  btGeneric6DofConstraint* dof =
      new btGeneric6DofConstraint(rbB, frameInB, useLinearReferenceFrameB);
  m_allocatedConstraints.push_back(dof);
  return dof;
}

btGeneric6DofSpring2Constraint*
btWorldImporter::createGeneric6DofSpring2Constraint(btRigidBody& rbA,
                                                    btRigidBody& rbB,
                                                    const btTransform& frameInA,
                                                    const btTransform& frameInB,
                                                    int rotateOrder) {
  btGeneric6DofSpring2Constraint* dof = new btGeneric6DofSpring2Constraint(
      rbA, rbB, frameInA, frameInB, (RotateOrder)rotateOrder);
  m_allocatedConstraints.push_back(dof);
  return dof;
}
...
btWorldImporter::createGeneric6DofSpringConstraint(
    btRigidBody& rbA, btRigidBody& rbB, const btTransform& frameInA,
    const btTransform& frameInB, bool useLinearReferenceFrameA) {
  btGeneric6DofSpringConstraint* dof = new btGeneric6DofSpringConstraint(
      rbA, rbB, frameInA, frameInB, useLinearReferenceFrameA);
  m_allocatedConstraints.push_back(dof);
  return dof;
}

btSliderConstraint* btWorldImporter::createSliderConstraint(
    btRigidBody& rbA, btRigidBody& rbB, const btTransform& frameInA,
    const btTransform& frameInB, bool useLinearReferenceFrameA) {
...
btGearConstraint* btWorldImporter::createGearConstraint(
    btRigidBody& rbA, btRigidBody& rbB, const btVector3& axisInA,
    const btVector3& axisInB, btScalar ratio) {
  btGearConstraint* gear =
      new btGearConstraint(rbA, rbB, axisInA, axisInB, ratio);
...
const char* btWorldImporter::getNameForPointer(const void* ptr) const {
  ...
  if (namePtr && *namePtr) return *namePtr;
  return 0;
}
...
int btWorldImporter::getNumBvhs() const { return m_allocatedBvhs.size(); }
...
void btWorldImporter::convertRigidBodyFloat(btRigidBodyFloatData* colObjData) {
  btScalar mass = btScalar(
      colObjData->m_inverseMass ? 1.f / colObjData->m_inverseMass : 0.f);
  btVector3 localInertia;
  localInertia.setZero();
  btCollisionShape** shapePtr =
      m_shapeMap.find(colObjData->m_collisionObjectData.m_collisionShape);
  if (shapePtr && *shapePtr) {
    btTransform startTransform;
    colObjData->m_collisionObjectData.m_worldTransform.m_origin.m_floats[3] =
        0.f;
    startTransform.deSerializeFloat(
        colObjData->m_collisionObjectData.m_worldTransform);
...
void btWorldImporter::convertRigidBodyDouble(
    btRigidBodyDoubleData* colObjData) {
  btScalar mass = btScalar(
      colObjData->m_inverseMass ? 1.f / colObjData->m_inverseMass : 0.f);
  btVector3 localInertia;
  localInertia.setZero();
  ...
  if (shapePtr && *shapePtr) {
    ...
    colObjData->m_collisionObjectData.m_worldTransform.m_origin.m_floats[3] =
        0.f;
    startTransform.deSerializeDouble(
        colObjData->m_collisionObjectData.m_worldTransform);
    ...
    body->setRestitution(
        btScalar(colObjData->m_collisionObjectData.m_restitution));
    btVector3 linearFactor, angularFactor;
