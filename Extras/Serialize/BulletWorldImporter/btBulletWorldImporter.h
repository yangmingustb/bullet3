...
/// note that not all data is converted yet. You are expected to override or
/// modify this class.
/// See Bullet/Demos/SerializeDemo for a derived class that extract btSoftBody
/// objects too.
class btBulletWorldImporter : public btWorldImporter {
 public:
  ...
  /// if you pass a valid preSwapFilenameOut, it will save a new file with a
  /// different endianness
  /// this pre-swapped file can be loaded without swapping on a target platform
  /// of different endianness
  bool loadFile(const char* fileName, const char* preSwapFilenameOut = 0);

  /// the memoryBuffer might be modified (for example if endian swaps are
  /// necessary)
  bool loadFileFromMemory(char* memoryBuffer, int len);

  bool loadFileFromMemory(bParse::btBulletFile* file);

  // call make sure bulletFile2 has been parsed, either using
  // btBulletFile::parse or btBulletWorldImporter::loadFileFromMemory
