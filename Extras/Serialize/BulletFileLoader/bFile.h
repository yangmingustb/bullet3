...
class bFile {
 protected:
  ...
  int resolvePointersStructRecursive(char* strcPtr, int old_dna,
  ...
  void parseStruct(char* strcPtr, char* dtPtr, int old_dna, int new_dna,
                   bool fixupPointers);
  void getMatchingFileDNA(short* old, const char* lookupName,
                          const char* lookupType, char* strcData, char* data,
                          bool fixupPointers);
  char* getFileElement(short* firstStruct, char* lookupName, char* lookupType,
                       char* data, short** foundPos);
 ...
 public:
  ...
  bDNA* getFileDNA() { return mFileDNA; }

  virtual void addDataBlock(char* dataBlock) = 0;

  int getFlags() const { return mFlags; }

  void setFileDNAisMemoryDNA() { mFlags |= FD_FILEDNA_IS_MEMDNA; }

  bPtrMap& getLibPointers() { return mLibPointers; }
  ...
  // pre-swap the endianness, so that data loaded on a target with different
  // endianness doesn't need to be swapped
  void preSwap();
