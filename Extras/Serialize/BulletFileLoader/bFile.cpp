...
void bFile::preSwap() {
  ...
  while (1) {
    // one behind
    if (dataChunk.code == SDNA || dataChunk.code == DNA1 ||
        dataChunk.code == TYPE || dataChunk.code == TLEN ||
        dataChunk.code == STRC) {
      swapDNA(dataPtr);
      break;
    } else {
...
#define setEle(value, current, type, cast, size, ptr) \
  ...
  for (int i = 0; i < arrayLen; i++) {
    ...
    getEle(value, old, "ushort", unsigned short, sizeof(unsigned short),
           oldPtr);
    setEle(value, cur, "ushort", unsigned short, sizeof(unsigned short),
           curData);
...
void bFile::safeSwapPtr(char *dst, const char *src) {
  ...
  if (ptrFile == ptrMem) {
    ...
    if (oldPtr->m_uniqueIds[0] == oldPtr->m_uniqueIds[1]) {
      ...
      // trailing zeros
      if (mFlags & FD_ENDIAN_SWAP) SWITCH_LONGINT(longValue);
