...
// this define will force traversal of structures, to check backward (and
...
// ----------------------------------------------------- //
int bDNA::getNumStructs() { return (int)mStructs.size(); }
...
// ----------------------------------------------------- //
int bDNA::getPointerSize() { return mPtrLen; }
...
//#define SLOW_FORWARD_COMPATIBLE 1
    ...
    if (curStruct[1] == oldStruct[1]) {
      ...
      if (mTlens[oldStruct[0]] == memDNA->mTlens[curStruct[0]]) {
        ...
        for (int j = 0; j < elementLength;
             j++, curStruct += 2, oldStruct += 2) {
          // type the same
          ...
          // name the same
          if (strcmp(m_Names[oldStruct[1]].m_name,
                     memDNA->m_Names[curStruct[1]].m_name) != 0) {
            isSame = false;
            break;
          }
        }
        // flag valid ==
        if (isSame) mCMPFlags[i] = FDF_STRUCT_EQU;
      }
    }
#endif
  }
...
// recurse in
for (i = 0; i < (int)mStructs.size(); i++) {
  if (mCMPFlags[i] == FDF_STRUCT_NEQU) initRecurseCmpFlags(i);
}
}
...
void bDNA::init(char *data, int len, bool swap) {
  ...
  */
  ...
  for (i = 0; i < dataLen; i++, shtPtr++) {
    if (swap) shtPtr[0] = ChunkUtils::swapShort(shtPtr[0]);
