...
btBulletFile::btBulletFile() : bFile("", "BULLET ") {
  mMemoryDNA = new bDNA();  // this memory gets released in the bFile::~bFile
                            // destructor,@todo not consistent with the rule
                            // 'who allocates it, has to deallocate it"
...
btBulletFile::btBulletFile(const char* fileName) : bFile(fileName, "BULLET ") {
  m_DnaCopy = 0;
}
...
btBulletFile::~btBulletFile() {
  if (m_DnaCopy) btAlignedFree(m_DnaCopy);
...
void btBulletFile::parseData() {
  ...
  // invalid/empty file?
  if (remain < sizeof(bChunkInd)) return;
  ...
  if (mFlags & FD_ENDIAN_SWAP) swapLen(dataPtr);
...
void btBulletFile::addStruct(const char* structType, void* data, int len,
                             void* oldPtr, int code) {
