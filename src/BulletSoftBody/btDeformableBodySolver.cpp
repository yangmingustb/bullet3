...
btDeformableBodySolver::btDeformableBodySolver()
    : m_numNodes(0),
      ...
      m_lineSearch(false) {
  m_objective =
      new btDeformableBackwardEulerObjective(m_softBodies, m_backupVelocity);
}

btDeformableBodySolver::~btDeformableBodySolver() { delete m_objective; }
  ...
  if (!m_implicit) {
    ...
    for (int i = 0; i < m_maxNewtonIterations; ++i) {
      ...
      if (m_lineSearch) {
        ...
        btScalar alpha = 0.01, beta = 0.5;  // Boyd & Vandenberghe suggested
                                            // alpha between 0.01 and 0.3, beta
                                            // between 0.1 to 0.8
        btScalar scale = 2;
        btScalar f0 = m_objective->totalEnergy(solverdt) + kineticEnergy(), f1,
                 f2;
        backupDv();
        do {
          scale *= beta;
        ...
        } while (!(f1 < f2 + SIMD_EPSILON));  // if anything here is nan then
...
void btDeformableBodySolver::reinitialize(
    const btAlignedObjectArray<btSoftBody*>& softBodies, btScalar dt) {
...
btScalar btDeformableBodySolver::solveSplitImpulse(
    const btContactSolverInfo& infoGlobal) {
  BT_PROFILE("solveSplitImpulse");
  return m_objective->m_projection.solveSplitImpulse(infoGlobal);
}

void btDeformableBodySolver::splitImpulseSetup(
    const btContactSolverInfo& infoGlobal) {
...
void btDeformableBodySolver::updateVelocity() {
  ...
  for (int i = 0; i < m_softBodies.size(); ++i) {
    ...
    for (int j = 0; j < psb->m_nodes.size(); ++j) {
      ...
      psb->m_maxSpeedSquared =
          btMax(psb->m_maxSpeedSquared, psb->m_nodes[j].m_v.length2());
      ++counter;
    }
  }
}
...
void btDeformableBodySolver::setupDeformableSolve(bool implicit) {
  ...
  for (int i = 0; i < m_softBodies.size(); ++i) {
    ...
    for (int j = 0; j < psb->m_nodes.size(); ++j) {
      if (implicit) {
        if ((psb->m_nodes[j].m_v - m_backupVelocity[counter]).norm() <
            SIMD_EPSILON)
        ...
        m_dv[counter] = psb->m_nodes[j].m_v - m_backupVelocity[counter];
      psb->m_nodes[j].m_v =
          m_backupVelocity[counter] + psb->m_nodes[j].m_vsplit;
      ++counter;
    }
  }
}
