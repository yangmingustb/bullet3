...
class btSoftBodySolver {
 ...
 public:
  btSoftBodySolver() : m_numberOfPositionIterations(10), m_timeScale(1) {
  ...
  /** Perform necessary per-step updates of soft bodies such as recomputing
   * normals and bounding boxes */
  virtual void updateSoftBodies() = 0;

  /** Process a collision between one of the world's soft bodies and another
   * collision object */
  virtual void processCollision(btSoftBody *,
                                const struct btCollisionObjectWrapper *) = 0;
 ...
 * This version is abstract. Subclasses will have custom pairings for different
 * combinations.
 */
class btSoftBodySolverOutput {
 ...
 public:
  ...
  /** Output current computed vertex data to the vertex buffers for all cloths
   * in the solver. */
  virtual void copySoftBodyToVertexBuffer(
      const btSoftBody *const softBody,
      btVertexBufferDescriptor *vertexBuffer) = 0;
};
