...
static inline int PolarDecomposition(const btMatrix3x3& m, btMatrix3x3& q,
                                     btMatrix3x3& s) {
  ...
  virtual void addScaledElasticForce(btScalar scale, TVStack& force) {
    ...
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < psb->m_tetras.size(); ++j) {
        ...
        btVector3 force_on_node0 =
            P * (tetra.m_Dm_inverse.transpose() * grad_N_hat_1st_col);
  ...
  void firstPiola(const btMatrix3x3& F, btMatrix3x3& P) {
    ...
    if (m_mu > SIMD_EPSILON) {
      ...
      if (J < 1024 * SIMD_EPSILON)
        ...
        PolarDecomposition(
            F, R,
            S);  // this QR is not robust, consider using implicit shift svd
