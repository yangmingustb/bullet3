...
class btDeformableBodySolver : public btSoftBodySolver {
 ...
 protected:
  ...
  TVStack m_backupVelocity;  // backed up v, equals v_n for implicit, equals
                             // v_{n+1}^* for explicit
  btScalar m_dt;             // dt
  ...
  btScalar
      m_newtonTolerance;  // stop newton iterations if f(x) < m_newtonTolerance
  bool m_lineSearch;      // If true, use newton's method with line search under
                          // implicit scheme
 ...
 public:
  ...
  virtual SolverTypes getSolverType() const { return DEFORMABLE_SOLVER; }
  ...
  btScalar solveContactConstraints(btCollisionObject** deformableBodies,
                                   int numDeformableBodies,
                                   const btContactSolverInfo& infoGlobal);

  // solve the position error  between deformable and rigid as well as among
  // deformables;
  ...
  // add in elastic forces and gravity to obtain v_{n+1}^* and calls
  ...
  // set m_dv and m_backupVelocity to desired value to prepare for momentum
  // solve
  ...
  // calculate the change in dv resulting from the momentum solve when line
  // search is turned on
  btScalar computeDescentStep(TVStack& ddv, const TVStack& residual,
                              bool verbose = false);

  virtual void copySoftBodyToVertexBuffer(
  ...
  virtual void processCollision(btSoftBody* softBody,
  ...
  // If true, newton's method with line search is used when implicit time
  // stepping scheme is turned on
