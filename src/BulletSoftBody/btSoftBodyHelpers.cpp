...
static void drawVertex(btIDebugDraw* idraw, const btVector3& x, btScalar s,
                       const btVector3& c) {
...
static void drawBox(btIDebugDraw* idraw, const btVector3& mins,
                    const btVector3& maxs, const btVector3& color) {
...
//
static void drawTree(btIDebugDraw* idraw, const btDbvtNode* node, int depth,
                     const btVector3& ncolor, const btVector3& lcolor,
                     int mindepth, int maxdepth) {
  if (node) {
    if (node->isinternal() && ((depth < maxdepth) || (maxdepth < 0))) {
      drawTree(idraw, node->childs[0], depth + 1, ncolor, lcolor, mindepth,
               maxdepth);
      drawTree(idraw, node->childs[1], depth + 1, ncolor, lcolor, mindepth,
               maxdepth);
    }
    if (depth >= mindepth) {
...
//
void btSoftBodyHelpers::Draw(btSoftBody* psb, btIDebugDraw* idraw,
                             int drawflags) {
...
#define USE_NEW_CONVEX_HULL_COMPUTER
        ...
        computer.compute(&vertices[0].getX(), stride, count, shrink,
                         shrinkClamp);
        for (int i = 0; i < computer.faces.size(); i++) {
          ...
          const btConvexHullComputer::Edge* edge =
              firstEdge->getNextEdgeOfFace();

          int v0 = firstEdge->getSourceVertex();
          int v1 = firstEdge->getTargetVertex();
          while (edge != firstEdge) {
            int v2 = edge->getTargetVertex();
            idraw->drawTriangle(computer.vertices[v0], computer.vertices[v1],
                                computer.vertices[v2], color, 1);
        ...
        for (j = 0; j < (int)hres.mNumFaces; ++j) {
          const int idx[] = {hres.m_Indices[j * 3 + 0],
                             hres.m_Indices[j * 3 + 1],
                             hres.m_Indices[j * 3 + 2]};
          idraw->drawTriangle(hres.m_OutputVertices[idx[0]],
                              hres.m_OutputVertices[idx[1]],
                              hres.m_OutputVertices[idx[2]], color, 1);
        }
...
/* Velocities	*/
    ...
    if (0 != (drawflags & fDrawFlags::Nodes)) {
      for (i = 0; i < psb->m_nodes.size(); ++i) {
        ...
        if (0 == (n.m_material->m_flags & btSoftBody::fMaterial::DebugDraw))
          continue;
        idraw->drawLine(n.m_x - btVector3(scl, 0, 0),
                        n.m_x + btVector3(scl, 0, 0), btVector3(1, 0, 0));
        idraw->drawLine(n.m_x - btVector3(0, scl, 0),
                        n.m_x + btVector3(0, scl, 0), btVector3(0, 1, 0));
        idraw->drawLine(n.m_x - btVector3(0, 0, scl),
                        n.m_x + btVector3(0, 0, scl), btVector3(0, 0, 1));
      }
    }
    /* Links	*/
    ...
    if (0 != (drawflags & fDrawFlags::Contacts)) {
      ...
      for (i = 0; i < psb->m_rcontacts.size(); ++i) {
        ...
        const btVector3 x =
            btCross(c.m_cti.m_normal, axis[c.m_cti.m_normal.minAxis()])
                .normalized();
    ...
    if (0 != (drawflags & fDrawFlags::Faces)) {
      ...
      for (i = 0; i < psb->m_faces.size(); ++i) {
        ...
        if (0 == (f.m_material->m_flags & btSoftBody::fMaterial::DebugDraw))
          continue;
    ...
    if (0 != (drawflags & fDrawFlags::Tetras)) {
      ...
      for (int i = 0; i < psb->m_tetras.size(); ++i) {
        ...
        if (0 == (t.m_material->m_flags & btSoftBody::fMaterial::DebugDraw))
          continue;
        const btVector3 x[] = {t.m_n[0]->m_x, t.m_n[1]->m_x, t.m_n[2]->m_x,
                               t.m_n[3]->m_x};
        ...
        idraw->drawTriangle((x[0] - c) * scl + c, (x[1] - c) * scl + c,
                            (x[3] - c) * scl + c, col, alp);
        idraw->drawTriangle((x[1] - c) * scl + c, (x[2] - c) * scl + c,
                            (x[3] - c) * scl + c, col, alp);
        idraw->drawTriangle((x[2] - c) * scl + c, (x[0] - c) * scl + c,
                            (x[3] - c) * scl + c, col, alp);
      }
    }
  }
    ...
    for (i = 0; i < psb->m_joints.size(); ++i) {
      ...
      switch (pj->Type()) {
        case btSoftBody::Joint::eType::Linear: {
          ...
          idraw->drawLine(pj->m_bodies[0].xform().getOrigin(), a0,
                          btVector3(1, 1, 0));
          idraw->drawLine(pj->m_bodies[1].xform().getOrigin(), a1,
                          btVector3(0, 1, 1));
        ...
        case btSoftBody::Joint::eType::Angular: {
          ...
          const btVector3 a0 =
              pj->m_bodies[0].xform().getBasis() * pj->m_refs[0];
          const btVector3 a1 =
              pj->m_bodies[1].xform().getBasis() * pj->m_refs[1];
...
//
void btSoftBodyHelpers::DrawInfos(btSoftBody* psb, btIDebugDraw* idraw,
                                  bool masses, bool areas, bool /*stress*/) {
...
//
void btSoftBodyHelpers::DrawNodeTree(btSoftBody* psb, btIDebugDraw* idraw,
                                     int mindepth, int maxdepth) {
  drawTree(idraw, psb->m_ndbvt.m_root, 0, btVector3(1, 0, 1),
           btVector3(1, 1, 1), mindepth, maxdepth);
}

//
void btSoftBodyHelpers::DrawFaceTree(btSoftBody* psb, btIDebugDraw* idraw,
                                     int mindepth, int maxdepth) {
  drawTree(idraw, psb->m_fdbvt.m_root, 0, btVector3(0, 1, 0),
           btVector3(1, 0, 0), mindepth, maxdepth);
}

//
void btSoftBodyHelpers::DrawClusterTree(btSoftBody* psb, btIDebugDraw* idraw,
                                        int mindepth, int maxdepth) {
  drawTree(idraw, psb->m_cdbvt.m_root, 0, btVector3(0, 1, 1),
           btVector3(1, 0, 0), mindepth, maxdepth);
}

// The btSoftBody object from the BulletSDK includes an array of Nodes and
// Links. These links appear
// to be first set up to connect a node to between 5 and 6 of its neighbors [480
// links],
// and then to the rest of the nodes after the execution of the Floyd-Warshall
// graph algorithm
//[another 930 links].
// The way the links are stored by default, we have a number of cases where
// adjacent links share a node in common
// - this leads to the creation of a data dependency through memory.
// The PSolve_Links() function reads and writes nodes as it iterates over each
// link.
...
// because L and L+1 have one node in common, and iteration X updates the
// positions of that node,
// and iteration X+1 reads in the position of that shared node.
//
// Such a memory dependency limits the ability of a modern CPU to speculate
// beyond
...
// If we re-order the links such that we minimize the cases where a link L and
// L+1 share a common node,
// we create a temporal gap between when the node position is written,
// and when it is subsequently read. This in turn allows the CPU to continue
// execution without
// risking a dependency violation. Such a reordering would result in significant
// speedups on
...
// The attached source file includes a single function (ReoptimizeLinkOrder)
// which can be called on a
// btSoftBody object in the solveConstraints() function before the actual solver
// is invoked,
...
void btSoftBodyHelpers::ReoptimizeLinkOrder(
    btSoftBody* psb /* This can be replaced by a btSoftBody pointer */) {
  ...
  int* nodeWrittenAt =
      new int[nNodes +
              1];  // What link calculation produced this node's current values?
  int* linkDepA = new int[nLinks];  // Link calculation input is dependent upon
                                    // prior calculation #N
  int* linkDepB = new int[nLinks];
  int* readyList = new int[nLinks];  // List of ready-to-process link
                                     // calculations (# of links, maximum)
  LinkDeps_t* linkDepFreeList =
      new LinkDeps_t[2 * nLinks];  // Dependent-on-me list elements (2x# of
                                   // links, maximum)
  LinkDepsPtr_t* linkDepListStarts =
      new LinkDepsPtr_t[nLinks];  // Start nodes of dependent-on-me lists, one
                                  // for each link
  ...
  for (i = 0; i < nLinks; i++) {
    // Note which prior link calculations we are dependent upon & build up
    // dependence lists
    lr = &(psb->m_links[i]);
    ...
    // Add this link to the initial ready list, if it is not dependent on any
    // other links
    if ((linkDepA[i] == REOP_NOT_DEPENDENT) &&
        (linkDepB[i] == REOP_NOT_DEPENDENT)) {
      readyList[readyListTail++] = i;
      linkDepA[i] = linkDepB[i] =
          REOP_NODE_COMPLETE;  // Probably not needed now
    }
  ...
  // -- By treating the ready list as a queue, we maximize the distance between
  // any
  //    inter-dependent node calculations
  // -- All other (non-related) nodes in the ready list will automatically be
  // inserted
  ...
  while (readyListHead != readyListTail) {
    ...
    while (linkDep) {
      ...
      // Add this dependent link calculation to the ready list if *both* inputs
      // are clear
      if ((linkDepA[depLink] == REOP_NOT_DEPENDENT) &&
          (linkDepB[depLink] == REOP_NOT_DEPENDENT)) {
        readyList[readyListTail++] = depLink;
        linkDepA[depLink] = linkDepB[depLink] =
            REOP_NODE_COMPLETE;  // Probably not needed now
      }
...
btSoftBody* btSoftBodyHelpers::CreatePatch(btSoftBodyWorldInfo& worldInfo,
                                           ...
                                           int resy, int fixeds, bool gendiags,
    ...
    for (int ix = 0; ix < rx; ++ix) {
      ...
      if (mdx && mdy) {
        if ((ix + iy) & 1) {
          ...
          psb->appendFace(IDX(ix, iy + 1), IDX(ix + 1, iy),
                          IDX(ix + 1, iy + 1));
          if (gendiags) {
...
btSoftBody* btSoftBodyHelpers::CreatePatchUV(btSoftBodyWorldInfo& worldInfo,
                                             ...
                                             int resx, int resy, int fixeds,
                                             bool gendiags, float* tex_coords) {
/*
*
*  corners:
*
...
float btSoftBodyHelpers::CalculateUV(int resx, int resy, int ix, int iy,
                                     int id) {
  /*
  *
  *
...
//
btSoftBody* btSoftBodyHelpers::CreateFromConvexHull(
    btSoftBodyWorldInfo& worldInfo, const btVector3* vertices, int nvertices,
    bool randomizeConstraints) {
 ...
 */
btSoftBody* btSoftBodyHelpers::CreateFromTetGenData(
    btSoftBodyWorldInfo& worldInfo, const char* ele, const char* face,
    const char* node, bool bfacelinks, bool btetralinks,
...
// Given a simplex with vertices a,b,c,d, find the barycentric weights of p in
// this simplex
void btSoftBodyHelpers::getBarycentricWeights(
    const btVector3& a, const btVector3& b, const btVector3& c,
    const btVector3& d, const btVector3& p, btVector4& bary) {
  btVector3 vap = p - a;
  btVector3 vbp = p - b;
...
// Iterate through all render nodes to find the simulation tetrahedron that
// contains the render node and record the barycentric weights
// If the node is not inside any tetrahedron, assign it to the tetrahedron in
// which the node has the least negative barycentric weight
void btSoftBodyHelpers::interpolateBarycentricWeights(btSoftBody* psb) {
  ...
  for (int i = 0; i < psb->m_renderNodes.size(); ++i) {
    ...
    for (int j = 0; j < psb->m_tetras.size(); ++j) {
      ...
      getBarycentricWeights(t.m_n[0]->m_x, t.m_n[1]->m_x, t.m_n[2]->m_x,
                            t.m_n[3]->m_x, p, bary);
