...
// Given a multibody link, a contact point and a contact direction, fill in the
// jacobian data needed to calculate the velocity change given an impulse in the
// contact direction
static SIMD_FORCE_INLINE void findJacobian(
    const btMultiBodyLinkCollider* multibodyLinkCol,
    btMultiBodyJacobianData& jacobianData, const btVector3& contact_point,
    const btVector3& dir) {
  ...
  multibodyLinkCol->m_multiBody->fillContactJacobianMultiDof(
      multibodyLinkCol->m_link, contact_point, dir, jac, jacobianData.scratch_r,
      jacobianData.scratch_v, jacobianData.scratch_m);
  multibodyLinkCol->m_multiBody->calcAccelerationDeltasMultiDof(
      &jacobianData.m_jacobians[0],
      &jacobianData.m_deltaVelocitiesUnitImpulse[0], jacobianData.scratch_r,
      jacobianData.scratch_v);
}
static SIMD_FORCE_INLINE btVector3
generateUnitOrthogonalVector(const btVector3& u) {
  btScalar ux = u.getX();
  btScalar uy = u.getY();
  btScalar uz = u.getZ();
...
static SIMD_FORCE_INLINE bool proximityTest(
    const btVector3& x1, const btVector3& x2, const btVector3& x3,
    const btVector3& x4, const btVector3& normal, const btScalar& mrg,
    btVector3& bary) {
  btVector3 x43 = x4 - x3;
  if (std::abs(x43.dot(normal)) > mrg) return false;
  btVector3 x13 = x1 - x3;
  btVector3 x23 = x2 - x3;
  ...
  if (det < SIMD_EPSILON) return false;
  ...
  for (int i = 0; i < 3; ++i) {
    if (bary[i] < -delta || bary[i] > 1 + delta) return false;
  }
  return true;
}
...
static btVector3 dop[KDOP_COUNT] = {
    ...
    btVector3(1, 1, 0),  btVector3(1, 0, 1),  btVector3(0, 1, 1),
    btVector3(1, -1, 0), btVector3(1, 0, -1), btVector3(0, 1, -1),
...
static SIMD_FORCE_INLINE bool hasSeparatingPlane(const btSoftBody::Face* face,
  ...
  for (int i = 0; i < KDOP_COUNT; ++i) {
    ...
    int j = 0;
    for (; j < 6; ++j) {
      if (getSign(dop[i], hex[j]) == s) break;
    }
    if (j == 6) return true;
  }
  return false;
...
static SIMD_FORCE_INLINE bool sameSign(const btScalar& a, const btScalar& b) {
  return (nearZero(a) || nearZero(b) ||
          (a > SAFE_EPSILON && b > SAFE_EPSILON) ||
          (a < -SAFE_EPSILON && b < -SAFE_EPSILON));
}
static SIMD_FORCE_INLINE bool diffSign(const btScalar& a, const btScalar& b) {
  return !sameSign(a, b);
}
inline btScalar evaluateBezier2(const btScalar& p0, const btScalar& p1,
                                const btScalar& p2, const btScalar& t,
                                const btScalar& s) {
  btScalar s2 = s * s;
  btScalar t2 = t * t;
...
inline btScalar evaluateBezier(const btScalar& p0, const btScalar& p1,
                               const btScalar& p2, const btScalar& p3,
                               const btScalar& t, const btScalar& s) {
  btScalar s2 = s * s;
  btScalar s3 = s2 * s;
  btScalar t2 = t * t;
  btScalar t3 = t2 * t;

  return p0 * s3 + p1 * btScalar(3.0) * s2 * t + p2 * btScalar(3.0) * s * t2 +
         p3 * t3;
}
static SIMD_FORCE_INLINE bool getSigns(bool type_c, const btScalar& k0,
                                       const btScalar& k1, const btScalar& k2,
                                       const btScalar& k3, const btScalar& t0,
                                       const btScalar& t1, btScalar& lt0,
                                       btScalar& lt1) {
  if (sameSign(t0, t1)) {
    lt0 = t0;
    lt1 = t0;
...
static SIMD_FORCE_INLINE void getBernsteinCoeff(const btSoftBody::Face* face,
                                                const btSoftBody::Node* node,
                                                const btScalar& dt,
                                                btScalar& k0, btScalar& k1,
                                                btScalar& k2, btScalar& k3) {
...
static SIMD_FORCE_INLINE void polyDecomposition(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btScalar& j0, const btScalar& j1,
    const btScalar& j2, btScalar& u0, btScalar& u1, btScalar& v0,
    btScalar& v1) {
  btScalar denom = 4.0 * (j1 - j2) * (j1 - j0) + (j2 - j0) * (j2 - j0);
  u0 = (2.0 * (j1 - j2) * (3.0 * k1 - 2.0 * k0 - k3) -
        (j0 - j2) * (3.0 * k2 - 2.0 * k3 - k0)) /
       denom;
  u1 = (2.0 * (j1 - j0) * (3.0 * k2 - 2.0 * k3 - k0) -
        (j2 - j0) * (3.0 * k1 - 2.0 * k0 - k3)) /
       denom;
  v0 = k0 - u0 * j0;
  v1 = k3 - u1 * j2;
}
...
static SIMD_FORCE_INLINE bool rootFindingLemma(const btScalar& k0,
  ...
  if (sameSign(v0, v1)) {
    btScalar Ypa = j0 * (1.0 - v0) * (1.0 - v0) + 2.0 * j1 * v0 * (1.0 - v0) +
                   j2 * v0 * v0;  // Y'(v0)
    if (sameSign(Ypa, j0)) {
...
static SIMD_FORCE_INLINE void getJs(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btSoftBody::Node* a, const btSoftBody::Node* b,
    const btSoftBody::Node* c, const btSoftBody::Node* p, const btScalar& dt,
    btScalar& j0, btScalar& j1, btScalar& j2) {
  ...
  btScalar l2 =
      btScalar(1) / btScalar(6) * (m0.dot(n1) + m_hat.dot(n_hat) + m1.dot(n0));
...
static SIMD_FORCE_INLINE bool signDetermination1Internal(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btScalar& u0, const btScalar& u1,
    const btScalar& v0, const btScalar& v1) {
  btScalar Yu0 = k0 * (1.0 - u0) * (1.0 - u0) * (1.0 - u0) +
                 3.0 * k1 * u0 * (1.0 - u0) * (1.0 - u0) +
                 3.0 * k2 * u0 * u0 * (1.0 - u0) + k3 * u0 * u0 * u0;  // Y(u0)
  btScalar Yv0 = k0 * (1.0 - v0) * (1.0 - v0) * (1.0 - v0) +
                 3.0 * k1 * v0 * (1.0 - v0) * (1.0 - v0) +
                 3.0 * k2 * v0 * v0 * (1.0 - v0) + k3 * v0 * v0 * v0;  // Y(v0)
...
static SIMD_FORCE_INLINE bool signDetermination2Internal(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btScalar& j0, const btScalar& j1,
    const btScalar& j2, const btScalar& u0, const btScalar& u1,
    const btScalar& v0, const btScalar& v1) {
  btScalar Yu0 = k0 * (1.0 - u0) * (1.0 - u0) * (1.0 - u0) +
                 3.0 * k1 * u0 * (1.0 - u0) * (1.0 - u0) +
                 3.0 * k2 * u0 * u0 * (1.0 - u0) + k3 * u0 * u0 * u0;  // Y(u0)
  ...
  if (diffSign(sign_Ytp, k0)) {
    ...
    btScalar Yp_u0 = j0 * (1.0 - u0) * (1.0 - u0) + 2.0 * j1 * (1.0 - u0) * u0 +
                     j2 * u0 * u0;
  ...
  btScalar Yv0 = k0 * (1.0 - v0) * (1.0 - v0) * (1.0 - v0) +
                 3.0 * k1 * v0 * (1.0 - v0) * (1.0 - v0) +
                 3.0 * k2 * v0 * v0 * (1.0 - v0) + k3 * v0 * v0 * v0;  // Y(uv0)
  sign_Ytp = (v0 > v1) ? Yv0 : -Yv0;
  btScalar K1, K2;
  if (diffSign(sign_Ytp, k0)) {
    K1 = v0;
    ...
    btScalar Yp_v0 = j0 * (1.0 - v0) * (1.0 - v0) + 2.0 * j1 * (1.0 - v0) * v0 +
                     j2 * v0 * v0;
...
static SIMD_FORCE_INLINE bool signDetermination1(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btSoftBody::Face* face,
    const btSoftBody::Node* node, const btScalar& dt) {
  btScalar j0, j1, j2, u0, u1, v0, v1;
  // p1
  getJs(k0, k1, k2, k3, face->m_n[0], face->m_n[1], face->m_n[2], node, dt, j0,
        j1, j2);
  if (nearZero(j0 + j2 - j1 * 2.0)) {
    btScalar lt0, lt1;
    getSigns(true, k0, k1, k2, k3, j0, j2, lt0, lt1);
    if (lt0 < -SAFE_EPSILON) return false;
  } else {
    polyDecomposition(k0, k1, k2, k3, j0, j1, j2, u0, u1, v0, v1);
  ...
  // p2
  getJs(k0, k1, k2, k3, face->m_n[1], face->m_n[2], face->m_n[0], node, dt, j0,
        j1, j2);
  if (nearZero(j0 + j2 - j1 * 2.0)) {
    btScalar lt0, lt1;
    getSigns(true, k0, k1, k2, k3, j0, j2, lt0, lt1);
  ...
  // p3
  getJs(k0, k1, k2, k3, face->m_n[2], face->m_n[0], face->m_n[1], node, dt, j0,
        j1, j2);
  if (nearZero(j0 + j2 - j1 * 2.0)) {
    btScalar lt0, lt1;
    getSigns(true, k0, k1, k2, k3, j0, j2, lt0, lt1);
...
static SIMD_FORCE_INLINE bool signDetermination2(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btSoftBody::Face* face,
    const btSoftBody::Node* node, const btScalar& dt) {
  btScalar j0, j1, j2, u0, u1, v0, v1;
  ...
  if (nearZero(j0 + j2 - j1 * 2.0)) {
    ...
    if (lt0 < -SAFE_EPSILON) bt0 = false;
    if (lt1 < -SAFE_EPSILON) bt1 = false;
    if (!bt0 && !bt1) return false;
  } else {
    polyDecomposition(k0, k1, k2, k3, j0, j1, j2, u0, u1, v0, v1);
...
static SIMD_FORCE_INLINE bool coplanarAndInsideTest(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btSoftBody::Face* face,
    const btSoftBody::Node* node, const btScalar& dt) {
  // Coplanar test
  if (diffSign(k1 - k0, k3 - k2)) {
    // Case b:
    if (sameSign(k0, k3) && !rootFindingLemma(k0, k1, k2, k3)) return false;
    // inside test
    return signDetermination2(k0, k1, k2, k3, face, node, dt);
  } else {
    // Case c:
    if (sameSign(k0, k3)) return false;
    // inside test
...
static SIMD_FORCE_INLINE bool conservativeCulling(const btScalar& k0,
                                                  ...
                                                  const btScalar& k3,
                                                  const btScalar& mrg) {
  if (k0 > mrg && k1 > mrg && k2 > mrg && k3 > mrg) return true;
  if (k0 < -mrg && k1 < -mrg && k2 < -mrg && k3 < -mrg) return true;
  return false;
}

static SIMD_FORCE_INLINE bool bernsteinVFTest(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btScalar& mrg, const btSoftBody::Face* face,
    const btSoftBody::Node* node, const btScalar& dt) {
  if (conservativeCulling(k0, k1, k2, k3, mrg)) return false;
  return coplanarAndInsideTest(k0, k1, k2, k3, face, node, dt);
}

static SIMD_FORCE_INLINE void deCasteljau(
    const btScalar& k0, const btScalar& k1, const btScalar& k2,
    const btScalar& k3, const btScalar& t0, btScalar& k10, btScalar& k20,
    btScalar& k30, btScalar& k21, btScalar& k12) {
  k10 = k0 * (1.0 - t0) + k1 * t0;
...
static SIMD_FORCE_INLINE bool bernsteinVFTest(const btSoftBody::Face* face,
  ...
  if (diffSign(k2 - 2.0 * k1 + k0, k3 - 2.0 * k2 + k1)) {
    ...
    return bernsteinVFTest(k0, k10, k20, k30, mrg, face, node, dt) ||
           bernsteinVFTest(k30, k21, k12, k3, mrg, face, node, dt);
  }
  return coplanarAndInsideTest(k0, k1, k2, k3, face, node, dt);
}

static SIMD_FORCE_INLINE bool continuousCollisionDetection(
    const btSoftBody::Face* face, const btSoftBody::Node* node,
    const btScalar& dt, const btScalar& mrg, btVector3& bary) {
  if (hasSeparatingPlane(face, node, dt)) return false;
  ...
  if (num_roots > 1) {
    if (roots[0] > roots[1]) btSwap(roots[0], roots[1]);
  }
  if (num_roots > 2) {
    if (roots[0] > roots[2]) btSwap(roots[0], roots[2]);
    if (roots[1] > roots[2]) btSwap(roots[1], roots[2]);
  }
  for (int r = 0; r < num_roots; ++r) {
    double root = roots[r];
    if (root <= 0) continue;
    if (root > dt + SIMD_EPSILON) return false;
    ...
    if (proximityTest(x1, x2, x3, x4, normal, mrg, bary)) return true;
  }
  return false;
}
static SIMD_FORCE_INLINE bool bernsteinCCD(const btSoftBody::Face* face,
                                           ...
                                           const btScalar& mrg,
                                           btVector3& bary) {
  if (!bernsteinVFTest(face, node, dt, mrg)) return false;
  if (!continuousCollisionDetection(face, node, dt, 1e-6, bary)) return false;
  return true;
}
...
class btSoftBodyCollisionShape : public btConcaveShape {
 public:
  ...
  void processAllTriangles(btTriangleCallback* /*callback*/,
  ...
  virtual void getAabb(const btTransform& t, btVector3& aabbMin,
    ...
    /* t is usually identity, except when colliding against btCompoundShape. See
     * Issue 512 */
  ...
  virtual const char* getName() const { return "SoftBody"; }
};
...
//
static inline btMatrix3x3 Lerp(const btMatrix3x3& a, const btMatrix3x3& b,
                               btScalar t) {
  btMatrix3x3 r;
  r[0] = Lerp(a[0], b[0], t);
  r[1] = Lerp(a[1], b[1], t);
...
static inline btScalar Dot(const btScalar* a, const btScalar* b, int ndof) {
  btScalar result = 0;
  for (int i = 0; i < ndof; ++i) result += a[i] * b[i];
  return result;
}

static inline btMatrix3x3 OuterProduct(const btScalar* v1, const btScalar* v2,
                                       const btScalar* v3, const btScalar* u1,
                                       const btScalar* u2, const btScalar* u3,
                                       int ndof) {
  btMatrix3x3 m;
...
//
static inline btMatrix3x3 MassMatrix(btScalar im, const btMatrix3x3& iwi,
                                     const btVector3& r) {
...
//
static inline btMatrix3x3 ImpulseMatrix(btScalar dt, btScalar ima, btScalar imb,
...
//
static inline btMatrix3x3 ImpulseMatrix(btScalar ima, const btMatrix3x3& iia,
                                        const btVector3& ra, btScalar imb,
...
//
static inline void ProjectOrigin(const btVector3& a, const btVector3& b,
                                 btVector3& prj, btScalar& sqd) {
...
//
static inline void ProjectOrigin(const btVector3& a, const btVector3& b,
                                 const btVector3& c, btVector3& prj,
                                 btScalar& sqd) {
...
//
static inline bool rayIntersectsTriangle(const btVector3& origin,
                                         const btVector3& dir,
                                         const btVector3& v0,
                                         const btVector3& v1,
                                         const btVector3& v2, btScalar& t) {
  btScalar a, f, u, v;
...
static inline bool lineIntersectsTriangle(const btVector3& rayStart,
                                          const btVector3& rayEnd,
                                          const btVector3& p1,
                                          const btVector3& p2,
                                          const btVector3& p3, btVector3& sect,
                                          btVector3& normal) {
  btVector3 dir = rayEnd - rayStart;
  btScalar dir_norm = dir.norm();
  if (dir_norm < SIMD_EPSILON) return false;
  dir.normalize();
...
//
template <typename T>
static inline T BaryEval(const T& a, const T& b, const T& c,
                         const btVector3& coord) {
  return (a * coord.x() + b * coord.y() + c * coord.z());
}
//
static inline btVector3 BaryCoord(const btVector3& a, const btVector3& b,
                                  const btVector3& c, const btVector3& p) {
...
inline static btScalar ImplicitSolve(btSoftBody::ImplicitFn* fn,
                                     const btVector3& a, const btVector3& b,
...
static inline btDbvtVolume VolumeOf(const btSoftBody::Face& f,
                                    btScalar margin) {
  const btVector3* pts[] = {&f.m_n[0]->m_x, &f.m_n[1]->m_x, &f.m_n[2]->m_x};
...
//
static inline btScalar AreaOf(const btVector3& x0, const btVector3& x1,
                              const btVector3& x2) {
...
//
static inline btScalar VolumeOf(const btVector3& x0, const btVector3& x1,
...
//
static inline void ApplyClampedForce(btSoftBody::Node& n, const btVector3& f,
                                     btScalar dt) {
  const btScalar dtim = dt * n.m_im;
  if ((f * dtim).length2() > n.m_v.length2()) { /* Clamp	*/
...
// straitforward implementation of
// http://math.fullerton.edu/mathews/n2003/JacobiMethodMod.html
// outputs are NOT sorted.
//
struct btEigen {
  static int system(btMatrix3x3& a, btMatrix3x3* vectors,
                    btVector3* values = 0) {
    ...
    do {
      ...
      if (btFabs(a[p][q]) > accuracy) {
        ...
        if (t == t) /* [WARNING] let hope that one does not get thrown aways by
                       some compilers... */
        {
 ...
 private:
  static inline void mulTPQ(btMatrix3x3& a, btScalar c, btScalar s, int p,
                            int q) {
  ...
  static inline void mulPQ(btMatrix3x3& a, btScalar c, btScalar s, int p,
                           int q) {
...
//
// Polar decomposition,
// "Computing the Polar Decomposition with Applications", Nicholas J. Higham,
// 1986.
//
static inline int PolarDecompose(const btMatrix3x3& m, btMatrix3x3& q,
                                 btMatrix3x3& s) {
...
struct btSoftColliders {
  ...
  struct ClusterBase : btDbvt::ICollide {
    ...
    bool SolveContact(const btGjkEpaSolver2::sResults& res, btSoftBody::Body ba,
                      const btSoftBody::Body bb, btSoftBody::CJoint& joint) {
      if (res.distance < m_margin) {
        ...
        joint.m_friction =
            fv.length2() < (rvac * friction * rvac * friction) ? 1 : friction;
        joint.m_massmatrix =
            ImpulseMatrix(ba.invMass(), ba.invWorldInertia(), joint.m_rpos[0],
  ...
  struct CollideCL_RS : ClusterBase {
    ...
    void Process(const btDbvtNode* leaf) {
      ...
      const btConvexShape* rshape =
          (const btConvexShape*)m_colObjWrap->getCollisionShape();

      /// don't collide an anchored cluster with a static/kinematic object
      if (m_colObjWrap->getCollisionObject()->isStaticOrKinematicObject() &&
          cluster->m_containsAnchor)
        return;

      btGjkEpaSolver2::sResults res;
      if (btGjkEpaSolver2::SignedDistance(
              &cshape, btTransform::getIdentity(), rshape,
              m_colObjWrap->getWorldTransform(), btVector3(1, 0, 0), res)) {
        btSoftBody::CJoint joint;
        if (SolveContact(res, cluster, m_colObjWrap->getCollisionObject(),
                         joint))  // prb,joint))
        {
          btSoftBody::CJoint* pj =
              new (btAlignedAlloc(sizeof(btSoftBody::CJoint), 16))
                  btSoftBody::CJoint();
          *pj = joint;
    ...
    void ProcessColObj(btSoftBody* ps,
                       const btCollisionObjectWrapper* colObWrap) {
      psb = ps;
      m_colObjWrap = colObWrap;
      idt = ps->m_sst.isdt;
      m_margin = m_colObjWrap->getCollisionShape()->getMargin() +
                 psb->getCollisionShape()->getMargin();
      /// Bullet rigid body uses multiply instead of minimum to determine
      /// combined friction. Some customization would be useful.
      friction = btMin(psb->m_cfg.kDF,
                       m_colObjWrap->getCollisionObject()->getFriction());
      btVector3 mins;
      btVector3 maxs;

      ATTRIBUTE_ALIGNED16(btDbvtVolume)
      volume;
      colObWrap->getCollisionShape()->getAabb(colObWrap->getWorldTransform(),
                                              mins, maxs);
  ...
  struct CollideCL_SS : ClusterBase {
    ...
    void Process(const btDbvtNode* la, const btDbvtNode* lb) {
      ...
      if ((bodies[0] == bodies[1]) &&
          (bodies[0]->m_clusterConnectivity.size())) {
        connected =
            bodies[0]->m_clusterConnectivity[cla->m_clusterIndex +
                                             bodies[0]->m_clusters.size() *
                                                 clb->m_clusterIndex];
      }
        ...
        if (btGjkEpaSolver2::SignedDistance(&csa, btTransform::getIdentity(),
          ...
          if (SolveContact(res, cla, clb, joint)) {
            ...
            *pj = joint;
            ...
            pj->m_split *=
                (bodies[0]->m_cfg.kSS_SPLT_CL + bodies[1]->m_cfg.kSS_SPLT_CL) /
                2;
          }
        }
      } else {
    ...
    void ProcessSoftSoft(btSoftBody* psa, btSoftBody* psb) {
      idt = psa->m_sst.isdt;
      // m_margin		=
      // (psa->getCollisionShape()->getMargin()+psb->getCollisionShape()->getMargin())/2;
      m_margin = (psa->getCollisionShape()->getMargin() +
                  psb->getCollisionShape()->getMargin());
  ...
  struct CollideSDF_RS : btDbvt::ICollide {
    ...
    void DoNode(btSoftBody::Node& n) const {
      ...
      if ((!n.m_battach) &&
        ...
        if (ms > 0) {
          const btTransform& wtr =
              m_rigidBody
                  ? m_rigidBody->getWorldTransform()
                  : m_colObj1Wrap->getCollisionObject()->getWorldTransform();
          static const btMatrix3x3 iwiStatic(0, 0, 0, 0, 0, 0, 0, 0, 0);
          const btMatrix3x3& iwi =
              m_rigidBody ? m_rigidBody->getInvInertiaTensorWorld() : iwiStatic;
          const btVector3 ra = n.m_x - wtr.getOrigin();
          const btVector3 va =
              m_rigidBody
                  ? m_rigidBody->getVelocityInLocalPoint(ra) * psb->m_sst.sdt
                  : btVector3(0, 0, 0);
          ...
          const btScalar fc =
              psb->m_cfg.kDF *
              m_colObj1Wrap->getCollisionObject()->getFriction();
          c.m_node = &n;
          ...
          c.m_c4 =
              m_colObj1Wrap->getCollisionObject()->isStaticOrKinematicObject()
                  ? psb->m_cfg.kKHR
                  : psb->m_cfg.kCHR;
          psb->m_rcontacts.push_back(c);
          if (m_rigidBody) m_rigidBody->activate();
        }
      }
    }
    btSoftBody* psb;
  ...
  struct CollideSDF_RD : btDbvt::ICollide {
    ...
    void DoNode(btSoftBody::Node& n) const {
      ...
      if (!n.m_battach) {
        ...
        if (psb->checkDeformableContact(m_colObj1Wrap, n.m_q, m, c.m_cti,
                                        /*predict = */ true)) {
          const btScalar ima = n.m_im;
          // todo: collision between multibody and fixed deformable node will be
          // missed.
          const btScalar imb = m_rigidBody ? m_rigidBody->getInvMass() : 0.f;
          const btScalar ms = ima + imb;
          if (ms > 0) {
            ...
            psb->checkDeformableContact(m_colObj1Wrap, n.m_x, m, c.m_cti,
                                        /*predict = */ false);
            ...
            if (cti.m_colObj->getInternalType() ==
              ...
              const btTransform& wtr = m_rigidBody
                                           ? m_rigidBody->getWorldTransform()
                                           : m_colObj1Wrap->getCollisionObject()
                                                 ->getWorldTransform();
              static const btMatrix3x3 iwiStatic(0, 0, 0, 0, 0, 0, 0, 0, 0);
              const btMatrix3x3& iwi =
              ...
              if (multibodyLinkCol) {
                ...
                btMultiBodyJacobianData jacobianData_normal, jacobianData_t1,
                    jacobianData_t2;
                findJacobian(multibodyLinkCol, jacobianData_normal,
                             c.m_node->m_x, normal);
                findJacobian(multibodyLinkCol, jacobianData_t1, c.m_node->m_x,
                             t1);
                findJacobian(multibodyLinkCol, jacobianData_t2, c.m_node->m_x,
                             t2);
                ...
                btScalar* u_n =
                    &jacobianData_normal.m_deltaVelocitiesUnitImpulse[0];
                btScalar* u_t1 =
                    &jacobianData_t1.m_deltaVelocitiesUnitImpulse[0];
                btScalar* u_t2 =
                    &jacobianData_t2.m_deltaVelocitiesUnitImpulse[0];

                btMatrix3x3 rot(normal.getX(), normal.getY(), normal.getZ(),
                                t1.getX(), t1.getY(), t1.getZ(), t2.getX(),
                                t2.getY(),
                                t2.getZ());  // world frame to local frame
                const int ndof =
                    multibodyLinkCol->m_multiBody->getNumDofs() + 6;
  ...
  struct CollideSDF_RDF : btDbvt::ICollide {
    ...
    void DoNode(btSoftBody::Face& f) const {
      ...
      if (n0->m_constrained && n1->m_constrained && n2->m_constrained) return;
      const btScalar m = (n0->m_im > 0 && n1->m_im > 0 && n2->m_im > 0)
                             ? dynmargin
                             : stamargin;
      btSoftBody::DeformableFaceRigidContact c;
      btVector3 contact_point;
      btVector3 bary;
      if (psb->checkDeformableFaceContact(m_colObj1Wrap, f, contact_point, bary,
                                          m, c.m_cti, true)) {
        f.m_pcontact[3] = 1;
        btScalar ima = n0->m_im + n1->m_im + n2->m_im;
        const btScalar imb = m_rigidBody ? m_rigidBody->getInvMass() : 0.f;
        // todo: collision between multibody and fixed deformable face will be
        // missed.
        const btScalar ms = ima + imb;
        if (ms > 0) {
          // resolve contact at x_n
          psb->checkDeformableFaceContact(m_colObj1Wrap, f, contact_point, bary,
                                          m, c.m_cti, /*predict = */ false);
          btSoftBody::sCti& cti = c.m_cti;
          c.m_contactPoint = contact_point;
          c.m_bary = bary;
          // todo xuchenhan@: this is assuming mass of all vertices are the
          // same. Need to modify if mass are different for distinct vertices
          ...
          // the effective inverse mass of the face as in
          // https://graphics.stanford.edu/papers/cloth-sig02/cloth.pdf
          ima = bary.getX() * c.m_weights.getX() * n0->m_im +
                bary.getY() * c.m_weights.getY() * n1->m_im +
                bary.getZ() * c.m_weights.getZ() * n2->m_im;
          ...
          if (cti.m_colObj->getInternalType() ==
            ...
            if (multibodyLinkCol) {
              ...
              btMultiBodyJacobianData jacobianData_normal, jacobianData_t1,
                  jacobianData_t2;
              findJacobian(multibodyLinkCol, jacobianData_normal, contact_point,
                           normal);
              findJacobian(multibodyLinkCol, jacobianData_t1, contact_point,
                           t1);
              findJacobian(multibodyLinkCol, jacobianData_t2, contact_point,
                           t2);
  ...
  struct CollideVF_SS : btDbvt::ICollide {
    void Process(const btDbvtNode* lnode, const btDbvtNode* lface) {
      ...
      for (int i = 0; i < 3; ++i) {
        if (face->m_n[i] == node) continue;
      }

      btVector3 o = node->m_x;
      btVector3 p;
      btScalar d = SIMD_INFINITY;
      ProjectOrigin(face->m_n[0]->m_x - o, face->m_n[1]->m_x - o,
                    face->m_n[2]->m_x - o, p, d);
      const btScalar m = mrg + (o - node->m_q).length() * 2;
      if (d < (m * m)) {
        const btSoftBody::Node* n[] = {face->m_n[0], face->m_n[1],
                                       face->m_n[2]};
        const btVector3 w = BaryCoord(n[0]->m_x, n[1]->m_x, n[2]->m_x, p + o);
        const btScalar ma = node->m_im;
        btScalar mb = BaryEval(n[0]->m_im, n[1]->m_im, n[2]->m_im, w);
        if ((n[0]->m_im <= 0) || (n[1]->m_im <= 0) || (n[2]->m_im <= 0)) {
          mb = 0;
        }
  ...
  struct CollideVF_DD : btDbvt::ICollide {
    void Process(const btDbvtNode* lnode, const btDbvtNode* lface) {
      ...
      btSoftBody::Face* face = (btSoftBody::Face*)lface->data;
      btVector3 bary;
      if (proximityTest(face->m_n[0]->m_x, face->m_n[1]->m_x, face->m_n[2]->m_x,
                        node->m_x, face->m_normal, mrg, bary)) {
        const btSoftBody::Node* n[] = {face->m_n[0], face->m_n[1],
                                       face->m_n[2]};
        ...
        if (ms > 0) {
          ...
          if (!useFaceNormal &&
              c.m_normal.dot(node->m_x - face->m_n[2]->m_x) < 0)
            c.m_normal = -face->m_normal;
          c.m_margin = mrg;
          c.m_node = node;
          c.m_face = face;
...
//#define REPEL_NEIGHBOR 1
      ...
      for (int node_id = 0; node_id < 3; ++node_id) {
        ...
        btVector3 bary;
        if (!proximityTest(face->m_n[0]->m_x, face->m_n[1]->m_x,
                           face->m_n[2]->m_x, node->m_x, face->m_normal, mrg,
                           bary))
          continue;
        btSoftBody::DeformableFaceNodeContact c;
        c.m_normal = face->m_normal;
