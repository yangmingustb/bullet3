...
btSoftBodyRigidBodyCollisionConfiguration::
  ...
  mem = btAlignedAlloc(sizeof(btSoftRigidCollisionAlgorithm::CreateFunc), 16);
  m_swappedSoftRigidConvexCreateFunc =
      new (mem) btSoftRigidCollisionAlgorithm::CreateFunc;
  m_swappedSoftRigidConvexCreateFunc->m_swapped = true;

#ifdef ENABLE_SOFTBODY_CONCAVE_COLLISIONS
  mem = btAlignedAlloc(sizeof(btSoftBodyConcaveCollisionAlgorithm::CreateFunc),
                       16);
  m_softRigidConcaveCreateFunc =
      new (mem) btSoftBodyConcaveCollisionAlgorithm::CreateFunc;

  mem = btAlignedAlloc(sizeof(btSoftBodyConcaveCollisionAlgorithm::CreateFunc),
                       16);
  m_swappedSoftRigidConcaveCreateFunc =
      new (mem) btSoftBodyConcaveCollisionAlgorithm::SwappedCreateFunc;
...
/// creation of soft-soft and soft-rigid, and otherwise fallback to base class
/// implementation
...
btSoftBodyRigidBodyCollisionConfiguration::getCollisionAlgorithmCreateFunc(
  ...
  if ((proxyType0 == SOFTBODY_SHAPE_PROXYTYPE) &&
      (proxyType1 == SOFTBODY_SHAPE_PROXYTYPE)) {
  ...
  /// convex versus soft body
  if (btBroadphaseProxy::isConcave(proxyType0) &&
      proxyType1 == SOFTBODY_SHAPE_PROXYTYPE) {
