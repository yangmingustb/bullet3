...
class btDeformableMassSpringForce : public btDeformableLagrangianForce {
 ...
 public:
  ...
  btDeformableMassSpringForce()
      : m_momentum_conserving(false),
        m_elasticStiffness(1),
        m_dampingStiffness(0.05) {}
  btDeformableMassSpringForce(btScalar k, btScalar d,
                              bool conserve_angular = true,
                              double bending_k = -1)
      : m_momentum_conserving(conserve_angular),
        m_elasticStiffness(k),
        m_dampingStiffness(d),
        m_bendingStiffness(bending_k) {
  ...
  virtual void addScaledElasticForce(btScalar scale, TVStack& force) {
    ...
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < psb->m_links.size(); ++j) {
        ...
        btVector3 dir_normalized =
            (dir.norm() > SIMD_EPSILON) ? dir.normalized() : btVector3(0, 0, 0);
        btScalar scaled_stiffness =
            scale * (link.m_bbending ? m_bendingStiffness : m_elasticStiffness);
      ...
      for (int j = 0; j < psb->m_links.size(); ++j) {
        ...
        if (m_momentum_conserving) {
          if ((node2->m_x - node1->m_x).norm() > SIMD_EPSILON) {
            btVector3 dir = (node2->m_x - node1->m_x).normalized();
            local_scaled_df =
                scaled_k_damp * (dv[id2] - dv[id1]).dot(dir) * dir;
          }
        }
  ...
  virtual double totalElasticEnergy(btScalar dt) {
    ...
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < psb->m_links.size(); ++j) {
        ...
        btVector3 dir = (node2->m_q - node1->m_q);
        energy +=
            0.5 * m_elasticStiffness * (dir.norm() - r) * (dir.norm() - r);
      }
    }
    return energy;
  }
  ...
  virtual void addScaledElasticForceDifferential(btScalar scale,
    ...
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < psb->m_links.size(); ++j) {
        ...
        btVector3 dir_normalized =
            (dir_norm > SIMD_EPSILON) ? dir.normalized() : btVector3(0, 0, 0);
        ...
        if (dir_norm > SIMD_EPSILON) {
          ...
          scaled_df += scaled_k * dir_normalized.dot(dx_diff) *
                       ((dir_norm - r) / dir_norm) * dir_normalized;
