...
class btDeformableLagrangianForce {
 public:
  ...
  virtual void addSoftBody(btSoftBody* psb) { m_softBodies.push_back(psb); }

  virtual void setIndices(
      const btAlignedObjectArray<btSoftBody::Node*>* nodes) {
    m_nodes = nodes;
  }

  // Calculate the incremental deformable generated from the input dx
  virtual btMatrix3x3 Ds(int id0, int id1, int id2, int id3,
                         const TVStack& dx) {
  ...
  virtual btMatrix3x3 DsFromVelocity(const btSoftBody::Node* n0,
  ...
  virtual void testDerivative() {
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < psb->m_nodes.size(); ++j) {
        psb->m_nodes[j].m_q +=
            btVector3(randomDouble(-.1, .1), randomDouble(-.1, .1),
                      randomDouble(-.1, .1));
      }
    ...
    for (int it = 0; it < 10; ++it) {
      ...
      std::cout << "Iteration = " << it << ", f1 = " << f1 << ", f2 = " << f2
  ...
  //
  virtual double totalElasticEnergy(btScalar dt) { return 0; }
