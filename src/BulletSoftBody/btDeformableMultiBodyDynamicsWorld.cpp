...
/*
A single step of the deformable body simulation contains the following main
components:
Call internalStepSimulation multiple times, to achieve 240Hz (4 steps of 60Hz).
1. Deformable maintaintenance of rest lengths and volume preservation. Forces
only depend on position: Update velocity to a temporary state v_{n+1}^* = v_n +
explicit_force * dt / mass, where explicit forces include gravity and elastic
forces.
2. Detect discrete collisions between rigid and deformable bodies at position
x_{n+1}^* = x_n + dt * v_{n+1}^*.

3a. Solve all constraints, including LCP. Contact, position correction due to
numerical drift, friction, and anchors for deformable.
    TODO: add option for positional drift correction (using vel_target += erp *
pos_error/dt

3b. 5 Newton steps (multiple step). Conjugent Gradient solves linear system.
Deformable Damping: Then velocities of deformable bodies v_{n+1} are solved in
        M(v_{n+1} - v_{n+1}^*) = damping_force * dt / mass,
   by a conjugate gradient solver, where the damping force is implicit and
depends on v_{n+1}.
   Make sure contact constraints are not violated in step b by performing
velocity projections as in the paper by Baraff and Witkin
https://www.cs.cmu.edu/~baraff/papers/sig98.pdf. Dynamic frictions are treated
as a force and added to the rhs of the CG solve, whereas static frictions are
treated as constraints similar to contact.
4. Position is updated via x_{n+1} = x_n + dt * v_{n+1}.


The algorithm also closely resembles the one in
http://physbam.stanford.edu/~fedkiw/papers/stanford2008-03.pdf
 */
...
btDeformableMultiBodyDynamicsWorld::btDeformableMultiBodyDynamicsWorld(
    ...
    : btMultiBodyDynamicsWorld(dispatcher, pairCache,
  ...
  m_solverDeformableBodyIslandCallback =
      new DeformableBodyInplaceSolverIslandCallback(constraintSolver,
                                                    dispatcher);
}
...
void btDeformableMultiBodyDynamicsWorld::updateActivationState(
    btScalar timeStep) {
...
void btDeformableMultiBodyDynamicsWorld::applyRepulsionForce(
    btScalar timeStep) {
...
void btDeformableMultiBodyDynamicsWorld::performGeometricCollisions(
    btScalar timeStep) {
  ...
  for (int i = 0; i < m_softBodies.size(); ++i) {
    ...
    for (int j = 0; j < m_softBodies[i]->m_faces.size(); ++j) {
      ...
      f.m_n0 =
          (f.m_n[1]->m_x - f.m_n[0]->m_x).cross(f.m_n[2]->m_x - f.m_n[0]->m_x);
    }
  }
  ...
  for (int r = 0; r < m_ccdIterations; ++r) {
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < m_softBodies[i]->m_nodes.size(); ++j) {
        m_softBodies[i]->m_nodes[j].m_q =
            m_softBodies[i]->m_nodes[j].m_x +
            timeStep * m_softBodies[i]->m_nodes[j].m_v;
      }
      for (int j = 0; j < m_softBodies[i]->m_faces.size(); ++j) {
        btSoftBody::Face& f = m_softBodies[i]->m_faces[j];
        f.m_n1 = (f.m_n[1]->m_q - f.m_n[0]->m_q)
                     .cross(f.m_n[2]->m_q - f.m_n[0]->m_q);
        f.m_vn = (f.m_n[1]->m_v - f.m_n[0]->m_v)
                     .cross(f.m_n[2]->m_v - f.m_n[0]->m_v) *
                 timeStep * timeStep;
      }
    }
...
void btDeformableMultiBodyDynamicsWorld::positionCorrection(btScalar timeStep) {
  // correct the position of rigid bodies with temporary velocity generated from
  // split impulse
  ...
  for (int i = 0; i < m_nonStaticRigidBodies.size(); ++i) {
    ...
    if (pushVelocity[0] != 0.f || pushVelocity[1] != 0 ||
        pushVelocity[2] != 0 || turnVelocity[0] != 0.f ||
        turnVelocity[1] != 0 || turnVelocity[2] != 0) {
      btTransformUtil::integrateTransform(
          rb->getWorldTransform(), pushVelocity,
          turnVelocity * infoGlobal.m_splitImpulseTurnErp, timeStep,
          newTransform);
...
void btDeformableMultiBodyDynamicsWorld::integrateTransforms(
  ...
  for (int i = 0; i < m_softBodies.size(); ++i) {
    ...
    for (int j = 0; j < psb->m_deformableAnchors.size(); ++j) {
      ...
      if (a.m_cti.m_colObj->getInternalType() ==
        ...
        if (multibodyLinkCol) {
          ...
          psb->m_worldInfo->m_sparsesdf.Evaluate(wtr.invXform(n->m_x), shp, nrm,
                                                 0);
          ...
          btMultiBodyJacobianData jacobianData_normal, jacobianData_t1,
              jacobianData_t2;
          findJacobian(multibodyLinkCol, jacobianData_normal, a.m_node->m_x,
                       normal);
          ...
          btMatrix3x3 rot(normal.getX(), normal.getY(), normal.getZ(),
                          t1.getX(), t1.getY(), t1.getZ(), t2.getX(), t2.getY(),
...
void btDeformableMultiBodyDynamicsWorld::solveConstraints(btScalar timeStep) {
  ...
  // set up constraints among multibodies and between multibodies and deformable
  // bodies
  setupConstraints();
  ...
  // set up the directions in which the velocity does not change in the momentum
  // solve
...
void btDeformableMultiBodyDynamicsWorld::setupConstraints() {
  ...
  // set up constraints among multibodies
  {
    ...
    btMultiBodyConstraint** sortedMultiBodyConstraints =
        m_sortedMultiBodyConstraints.size() ? &m_sortedMultiBodyConstraints[0]
                                            : 0;
    btTypedConstraint** constraintsPtr =
        getNumConstraints() ? &m_sortedConstraints[0] : 0;
    m_solverDeformableBodyIslandCallback->setup(
        &m_solverInfo, constraintsPtr, m_sortedConstraints.size(),
        sortedMultiBodyConstraints, m_sortedMultiBodyConstraints.size(),
        getDebugDrawer());

    // build islands
    m_islandManager->buildIslands(getCollisionWorld()->getDispatcher(),
                                  getCollisionWorld());
  }
}
...
void btDeformableMultiBodyDynamicsWorld::sortConstraints() {
  ...
  m_sortedMultiBodyConstraints.quickSort(
...
void btDeformableMultiBodyDynamicsWorld::solveContactConstraints() {
  ...
  m_islandManager->processIslands(getCollisionWorld()->getDispatcher(),
                                  getCollisionWorld(),
                                  m_solverDeformableBodyIslandCallback);
  ...
  // write joint feedback
  {
    for (int i = 0; i < this->m_multiBodies.size(); i++) {
      ...
      if (bod->getBaseCollider() &&
          bod->getBaseCollider()->getActivationState() == ISLAND_SLEEPING) {
        isSleeping = true;
      }
      for (int b = 0; b < bod->getNumLinks(); b++) {
        if (bod->getLink(b).m_collider &&
            bod->getLink(b).m_collider->getActivationState() == ISLAND_SLEEPING)
          isSleeping = true;
      }

      if (!isSleeping) {
        // useless? they get resized in stepVelocities once again (AND
        // DIFFERENTLY)
        m_scratch_r.resize(
            bod->getNumLinks() +
        ...
        if (bod->internalNeedsJointFeedback()) {
          if (!bod->isUsingRK4Integration()) {
            if (bod->internalNeedsJointFeedback()) {
              ...
              bod->computeAccelerationsArticulatedBodyAlgorithmMultiDof(
                  m_solverInfo.m_timeStep, m_scratch_r, m_scratch_v,
                  m_scratch_m, isConstraintPass,
...
void btDeformableMultiBodyDynamicsWorld::addSoftBody(btSoftBody* body,
  ...
  btCollisionWorld::addCollisionObject(body, collisionFilterGroup,
                                       collisionFilterMask);
}

void btDeformableMultiBodyDynamicsWorld::predictUnconstraintMotion(
    btScalar timeStep) {
...
void btDeformableMultiBodyDynamicsWorld::applyRigidBodyGravity(
    btScalar timeStep) {
  // Gravity is applied in stepSimulation and then cleared here and then applied
  // here and then cleared here again
  // so that 1) gravity is applied to velocity before constraint solve and 2)
  // gravity is applied in each substep
...
void btDeformableMultiBodyDynamicsWorld::beforeSolverCallbacks(
    btScalar timeStep) {
...
void btDeformableMultiBodyDynamicsWorld::afterSolverCallbacks(
    btScalar timeStep) {
  if (0 != m_solverCallback) {
    (*m_solverCallback)(m_internalTime, this);
  }
...
void btDeformableMultiBodyDynamicsWorld::addForce(
    btSoftBody* psb, btDeformableLagrangianForce* force) {
  btAlignedObjectArray<btDeformableLagrangianForce*>& forces =
      m_deformableBodySolver->m_objective->m_lf;
  bool added = false;
...
void btDeformableMultiBodyDynamicsWorld::removeCollisionObject(
