...
/// btSoftRigidCollisionAlgorithm  provides collision detection between
/// btSoftBody and btRigidBody
class btSoftRigidCollisionAlgorithm : public btCollisionAlgorithm {
  ...
  /// for rigid versus soft (instead of soft versus rigid), we use this swapped
  /// boolean
  bool m_isSwapped;
  ...
  struct CreateFunc : public btCollisionAlgorithmCreateFunc {
    virtual btCollisionAlgorithm* CreateCollisionAlgorithm(
      ...
      void* mem = ci.m_dispatcher1->allocateCollisionAlgorithm(
          sizeof(btSoftRigidCollisionAlgorithm));
      if (!m_swapped) {
        return new (mem)
            btSoftRigidCollisionAlgorithm(0, ci, body0Wrap, body1Wrap, false);
      } else {
        return new (mem)
            btSoftRigidCollisionAlgorithm(0, ci, body0Wrap, body1Wrap, true);
      }
    }
  };
};
