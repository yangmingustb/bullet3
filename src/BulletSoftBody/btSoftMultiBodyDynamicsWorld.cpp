...
btSoftMultiBodyDynamicsWorld::btSoftMultiBodyDynamicsWorld(
    ...
    : btMultiBodyDynamicsWorld(dispatcher, pairCache, constraintSolver,
...
void btSoftMultiBodyDynamicsWorld::predictUnconstraintMotion(
    btScalar timeStep) {
...
void btSoftMultiBodyDynamicsWorld::internalSingleStepSimulation(
    btScalar timeStep) {
...
void btSoftMultiBodyDynamicsWorld::solveSoftBodiesConstraints(
    btScalar timeStep) {
...
void btSoftMultiBodyDynamicsWorld::removeCollisionObject(
  ...
  if (getDebugDrawer()) {
    ...
    for (i = 0; i < this->m_softBodies.size(); i++) {
      ...
      if (m_debugDrawer &&
        ...
        if (m_drawClusterTree)
          btSoftBodyHelpers::DrawClusterTree(psb, m_debugDrawer);
      }
    }
  }
}
...
struct btSoftSingleRayCallback : public btBroadphaseRayCallback {
  ...
  btSoftSingleRayCallback(const btVector3& rayFromWorld,
    ...
    m_rayDirectionInverse[0] =
        rayDir[0] == btScalar(0.0) ? btScalar(1e30) : btScalar(1.0) / rayDir[0];
    m_rayDirectionInverse[1] =
        rayDir[1] == btScalar(0.0) ? btScalar(1e30) : btScalar(1.0) / rayDir[1];
    m_rayDirectionInverse[2] =
        rayDir[2] == btScalar(0.0) ? btScalar(1e30) : btScalar(1.0) / rayDir[2];
...
void btSoftMultiBodyDynamicsWorld::rayTest(
...
void btSoftMultiBodyDynamicsWorld::rayTestSingle(
  ...
  if (collisionShape->isSoftBody()) {
    ...
    if (softBody) {
      ...
      if (softBody->rayTest(rayFromTrans.getOrigin(), rayToTrans.getOrigin(),
                            softResult)) {
        if (softResult.fraction <= resultCallback.m_closestHitFraction) {
          ...
          btCollisionWorld::LocalRayResult rayResult(
              collisionObject, &shapeInfo, normal, softResult.fraction);
    ...
    btCollisionWorld::rayTestSingle(rayFromTrans, rayToTrans, collisionObject,
                                    collisionShape, colObjWorldTransform,
                                    resultCallback);
  }
}

void btSoftMultiBodyDynamicsWorld::serializeSoftBodies(
