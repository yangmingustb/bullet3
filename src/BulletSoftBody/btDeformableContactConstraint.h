...
// btDeformableContactConstraint is an abstract class specifying the method that
// each type of contact constraint needs to implement
class btDeformableContactConstraint {
 public:
  ...
  btDeformableContactConstraint(const btVector3& normal,
      ...
      : m_static(false), m_normal(normal), m_infoGlobal(&infoGlobal) {}

  btDeformableContactConstraint(bool isStatic, const btVector3& normal,
                                const btContactSolverInfo& infoGlobal)
      : m_static(isStatic), m_normal(normal), m_infoGlobal(&infoGlobal) {}
  ...
  // solve the constraint with inelastic impulse and return the error, which is
  // the square of normal component of velocity diffrerence
  // the constraint is solved by calculating the impulse between object A and B
  // in the contact and apply the impulse to both objects involved in the
  // contact
  virtual btScalar solveConstraint(const btContactSolverInfo& infoGlobal) = 0;

  // solve the position error by applying an inelastic impulse that changes only
  // the position (not velocity)
...
class btDeformableStaticConstraint : public btDeformableContactConstraint {
 public:
  ...
  btDeformableStaticConstraint(btSoftBody::Node* node,
                               const btContactSolverInfo& infoGlobal)
      : m_node(node),
        btDeformableContactConstraint(false, btVector3(0, 0, 0), infoGlobal) {}
  ...
  virtual btVector3 getVa() const { return btVector3(0, 0, 0); }

  virtual btVector3 getVb() const { return btVector3(0, 0, 0); }
...
class btDeformableNodeAnchorConstraint : public btDeformableContactConstraint {
 public:
  ...
  btDeformableNodeAnchorConstraint(
      const btSoftBody::DeformableNodeRigidAnchor& c,
      const btContactSolverInfo& infoGlobal);
  btDeformableNodeAnchorConstraint(
...
class btDeformableRigidContactConstraint
    : public btDeformableContactConstraint {
 public:
  ...
  virtual void setPenetrationScale(btScalar scale) { m_penetration *= scale; }
};

//
// Constraint between rigid/multi body and deformable objects nodes
class btDeformableNodeRigidContactConstraint
    : public btDeformableRigidContactConstraint {
 public:
  ...
  const btSoftBody::DeformableNodeRigidContact* getContact() const {
    return static_cast<const btSoftBody::DeformableNodeRigidContact*>(
        m_contact);
  }
...
class btDeformableFaceRigidContactConstraint
    : public btDeformableRigidContactConstraint {
 public:
  ...
  const btSoftBody::DeformableFaceRigidContact* getContact() const {
    return static_cast<const btSoftBody::DeformableFaceRigidContact*>(
        m_contact);
  }
...
class btDeformableFaceNodeContactConstraint
    : public btDeformableContactConstraint {
 public:
  btSoftBody::Node* m_node;
