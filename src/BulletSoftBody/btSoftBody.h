...
struct btSoftBodyWorldInfo {
  ...
  btSoftBodyWorldInfo()
      : air_density((btScalar)1.2),
        water_density(0),
        water_offset(0),
        m_maxDisplacement(1000.f),  // avoid soft body from 'exploding' so use
                                    // some upper threshold of maximum motion
...
/// There is two-way interaction between btSoftBody and
 ...
 public:
  ...
  struct eAeroModel {
    enum _ {
      ...
      V_TwoSidedLiftDrag,  /// Vertex normals are flipped to match velocity and
                           /// lift and drag forces are applied
      V_OneSided,          /// Vertex normals are taken as it is
      F_TwoSided,          /// Face normals are flipped to match velocity
      F_TwoSidedLiftDrag,  /// Face normals are flipped to match velocity and
                           /// lift and drag forces are applied
  ...
  struct eSolverPresets {
    enum _ { Positions, Velocities, Default = Positions, END };
  };

  /// eFeature
  struct eFeature {
    enum _ { None, Node, Link, Face, Tetra, END };
  };
  ...
  struct sRayCast {
    ...
    btScalar
        fraction;  /// time of impact fraction (rayorg+(rayto-rayfrom)*fraction)
  };
  ...
  struct Node : Feature {
    ...
    btVector3 m_vsplit;  // Temporary Velocity in addintion to velocity used in
                         // split impulse
  ...
  class DeformableFaceRigidContact : public DeformableRigidContact {
   public:
    ...
    btVector3
        m_weights;  // v_contactPoint * m_weights[i] = m_face->m_node[i]->m_v;
  };
  ...
  struct Body {
    ...
    Body(const btCollisionObject* colObj)
        : m_soft(0), m_collisionObject(colObj) {
      m_rigid = (btRigidBody*)btRigidBody::upcast(m_collisionObject);
    }

    void activate() const {
      if (m_rigid) m_rigid->activate();
      if (m_collisionObject) m_collisionObject->activate();
    }
    ...
    void applyImpulse(const Impulse& impulse, const btVector3& rpos) const {
      if (impulse.m_asVelocity) {
        //				printf("impulse.m_velocity =
        //%f,%f,%f\n",impulse.m_velocity.getX(),impulse.m_velocity.getY(),impulse.m_velocity.getZ());
        applyVImpulse(impulse.m_velocity, rpos);
      }
      if (impulse.m_asDrift) {
        //				printf("impulse.m_drift =
  ...
  struct Joint {
    struct eType {
      enum _ { Linear = 0, Angular, Contact };
    };
    struct Specs {
  ...
  struct RayFromToCaster : btDbvt::ICollide {
    ...
    Face* m_face;
    int m_tests;
    RayFromToCaster(const btVector3& rayFrom, const btVector3& rayTo,
                    btScalar mxt);
    void Process(const btDbvtNode* leaf);

    static /*inline*/ btScalar rayFromToTriangle(
        const btVector3& rayFrom, const btVector3& rayTo,
        const btVector3& rayNormalizedDirection, const btVector3& a,
        const btVector3& b, const btVector3& c, btScalar maxt = SIMD_INFINITY);
  };
  ...
  btAlignedObjectArray<btVector3>
      m_quads;  // quadrature points for collision detection
  btScalar repulsionStiffness;

  btAlignedObjectArray<btVector4> m_renderNodesInterpolationWeights;
  btAlignedObjectArray<btAlignedObjectArray<const btSoftBody::Node*> >
      m_renderNodesParents;
  bool m_useSelfCollision;
  bool m_usePostCollisionDamping;

  btAlignedObjectArray<bool>
      m_clusterConnectivity;  // cluster connectivity, for self-collision
  ...
  /* ctor
   */
  btSoftBody(btSoftBodyWorldInfo* worldInfo, int node_count, const btVector3* x,
             const btScalar* m);
  ...
  void setUseFaceContact(bool useFaceContact) { m_useFaceContact = false; }

  ///@todo: avoid internal softbody shape hack and move collision code to
  ///collision library
  ...
  /* Append note
   */
  void appendNote(const char* text, const btVector3& o,
                  const btVector4& c = btVector4(1, 0, 0, 0), Node* n0 = 0,
                  Node* n1 = 0, Node* n2 = 0, Node* n3 = 0);
  ...
  void appendLink(int node0, int node1, Material* mat = 0,
                  bool bcheckexist = false);
  void appendLink(Node* node0, Node* node1, Material* mat = 0,
                  bool bcheckexist = false);
  /* Append face
   */
  ...
  void appendAnchor(int node, btRigidBody* body, const btVector3& localPivot,
                    bool disableCollisionBetweenLinkedBodies = false,
                    btScalar influence = 1);
  /* Append linear joint
   */
  void appendLinearJoint(const LJoint::Specs& specs, Cluster* body0,
                         Body body1);
  void appendLinearJoint(const LJoint::Specs& specs, Body body = Body());
  void appendLinearJoint(const LJoint::Specs& specs, btSoftBody* body);
  /* Append linear joint
   */
  void appendAngularJoint(const AJoint::Specs& specs, Cluster* body0,
                          Body body1);
   ...
   */
  static btVector3 clusterVelocity(const Cluster* cluster,
                                   const btVector3& rpos);
  /* Cluster impulse
   */
  static void clusterVImpulse(Cluster* cluster, const btVector3& rpos,
                              const btVector3& impulse);
  static void clusterDImpulse(Cluster* cluster, const btVector3& rpos,
                              const btVector3& impulse);
  static void clusterImpulse(Cluster* cluster, const btVector3& rpos,
   ...
   */
  /// generateClusters with k=0 will create a convex cluster for each
  ...
  bool rayTest(const btVector3& rayFrom, const btVector3& rayTo,
               sRayCast& results);
  /* Solver presets
   */
   ...
   */
  static void solveCommonConstraints(btSoftBody** bodies, int count,
                                     int iterations);
  /* solveClusters
   */
  ...
  static btSoftBody* upcast(btCollisionObject* colObj) {
    if (colObj->getInternalType() == CO_SOFT_BODY) return (btSoftBody*)colObj;
    return 0;
  }
  ...
  int rayTest(const btVector3& rayFrom, const btVector3& rayTo, btScalar& mint,
              eFeature::_& feature, int& index, bool bcountonly) const;
  void initializeFaceTree();
  void rebuildNodeTree();
  btVector3 evaluateCom() const;
  bool checkDeformableContact(const btCollisionObjectWrapper* colObjWrap,
                              const btVector3& x, btScalar margin,
                              btSoftBody::sCti& cti,
                              bool predict = false) const;
  bool checkDeformableFaceContact(const btCollisionObjectWrapper* colObjWrap,
                                  Face& f, btVector3& contact_point,
                                  btVector3& bary, btScalar margin,
                                  btSoftBody::sCti& cti,
                                  bool predict = false) const;
...
#define SAFE_EPSILON SIMD_EPSILON * 10.0
  void updateNode(btDbvtNode* node, bool use_velocity, bool margin) {
    if (node->isleaf()) {
      ...
      btScalar pad = margin ? m_sst.radmrg : SAFE_EPSILON;  // use user defined
                                                            // margin or margin
                                                            // for floating
                                                            // point precision
      if (use_velocity) {
  ...
  void updateNodeTree(bool use_velocity, bool margin) {
    if (m_ndbvt.m_root) updateNode(m_ndbvt.m_root, use_velocity, margin);
  }
  ...
  void updateFace(DBVTNODE* node, bool use_velocity, bool margin) {
    if (node->isleaf()) {
      ...
      if (use_velocity) {
        btVector3 points[6] = {
            f->m_n[0]->m_x, f->m_n[0]->m_x + m_sst.sdt * f->m_n[0]->m_v,
        ...
        btVector3 points[3] = {f->m_n[0]->m_x, f->m_n[1]->m_x, f->m_n[2]->m_x};
  ...
  void updateFaceTree(bool use_velocity, bool margin) {
    if (m_fdbvt.m_root) updateFace(m_fdbvt.m_root, use_velocity, margin);
    if (m_fdbvnt) updateFace(m_fdbvnt, use_velocity, margin);
  }
  ...
  void applyRepulsionForce(btScalar timeStep, bool applySpringForce) {
    btAlignedObjectArray<int> indices;
    {
      ...
      for (int i = 0; i < m_faceNodeContacts.size(); ++i) indices[i] = i;
//			static unsigned long seed = 243703;
#define NEXTRAND (seed = (1664525L * seed + 1013904223L) & 0xffffffff)
    ...
    for (int k = 0; k < m_faceNodeContacts.size(); ++k) {
      ...
      btVector3 l = node->m_x - BaryEval(face->m_n[0]->m_x, face->m_n[1]->m_x,
                                         face->m_n[2]->m_x, w);
      ...
      btVector3 vb =
          BaryEval(face->m_n[0]->m_v, face->m_n[1]->m_v, face->m_n[2]->m_v, w);
      btVector3 vr = va - vb;
      const btScalar vn = btDot(vr, n);  // dn < 0 <==> opposing
      if (vn > OVERLAP_REDUCTION_FACTOR * d / timeStep) continue;
      btVector3 vt = vr - vn * n;
      btScalar I = 0;
      if (applySpringForce)
        I = -btMin(repulsionStiffness * timeStep * d,
                   btScalar(1) / node->m_im *
                       (OVERLAP_REDUCTION_FACTOR * d / timeStep - vn));
      if (vn < 0) I += btScalar(0.5) / node->m_im * vn;
      ...
      if (face_constrained || node_constrained) I_tilde *= 2.0;
      if (!face_constrained) {
        for (int j = 0; j < 3; ++j)
      ...
      if (vt_norm > SIMD_EPSILON) {
        ...
        btScalar vt_new =
            btMax(btScalar(1) - mu * delta_vn / (vt_norm + SIMD_EPSILON),
                  btScalar(0)) *
            vt_norm;
