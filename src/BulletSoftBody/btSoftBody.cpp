...
//
static inline btDbvtNode* buildTreeBottomUp(
  ...
  while (N > 1) {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      for (int j = 0; j < adj[i].size(); ++j) {
        ...
        if (!marked[adj[i][j]]) {
          btDbvtNode* node =
              new (btAlignedAlloc(sizeof(btDbvtNode), 16)) btDbvtNode();
          node->parent = NULL;
    ...
    for (int i = 0; i < newLeafNodes.size(); ++i) {
      for (int j = i + 1; j < newLeafNodes.size(); ++j) {
        bool neighbor = false;
        const btAlignedObjectArray<int>& leftChildNeighbors =
            adj[childIds[i].first];
        for (int k = 0; k < leftChildNeighbors.size(); ++k) {
          if (leftChildNeighbors[k] == childIds[j].first ||
              leftChildNeighbors[k] == childIds[j].second) {
            neighbor = true;
            break;
          }
        }
        if (!neighbor && childIds[i].second != -1) {
          const btAlignedObjectArray<int>& rightChildNeighbors =
              adj[childIds[i].second];
          for (int k = 0; k < rightChildNeighbors.size(); ++k) {
            if (rightChildNeighbors[k] == childIds[j].first ||
                rightChildNeighbors[k] == childIds[j].second) {
              neighbor = true;
              break;
            }
          }
    ...
    // this assignment leaks memory, the assignment doesn't do a deep copy, for
    // now a manual copy
    // adj = newAdj;
    adj.clear();
...
btSoftBody::btSoftBody(btSoftBodyWorldInfo* worldInfo, int node_count,
                       const btVector3* x, const btScalar* m)
...
btSoftBody::~btSoftBody() {
  ...
  releaseClusters();
  for (i = 0; i < m_materials.size(); ++i) btAlignedFree(m_materials[i]);
  for (i = 0; i < m_joints.size(); ++i) btAlignedFree(m_joints[i]);
  if (m_fdbvnt) delete m_fdbvnt;
}
...
bool btSoftBody::checkFace(int node0, int node1, int node2) const {
  const Node* n[] = {&m_nodes[node0], &m_nodes[node1], &m_nodes[node2]};
  for (int i = 0, ni = m_faces.size(); i < ni; ++i) {
    const Face& f = m_faces[i];
    int c = 0;
    for (int j = 0; j < 3; ++j) {
      if ((f.m_n[j] == n[0]) || (f.m_n[j] == n[1]) || (f.m_n[j] == n[2]))
        c |= 1 << j;
      else
        break;
    }
...
//
void btSoftBody::appendNote(const char* text, const btVector3& o,
                            const btVector4& c, Node* n0, Node* n1, Node* n2,
                            Node* n3) {
  Note n;
  ZeroInitialize(n);
  n.m_rank = 0;
...
void btSoftBody::appendNote(const char* text, const btVector3& o,
  ...
  appendNote(text, o, btVector4(w, w, 0, 0), feature->m_n[0], feature->m_n[1]);
}
...
void btSoftBody::appendNote(const char* text, const btVector3& o,
  ...
  appendNote(text, o, btVector4(w, w, w, 0), feature->m_n[0], feature->m_n[1],
             feature->m_n[2]);
}
...
//
void btSoftBody::appendLink(int node0, int node1, Material* mat,
                            bool bcheckexist) {
  appendLink(&m_nodes[node0], &m_nodes[node1], mat, bcheckexist);
}

//
void btSoftBody::appendLink(Node* node0, Node* node1, Material* mat,
                            bool bcheckexist) {
...
//
void btSoftBody::appendTetra(int node0, int node1, int node2, int node3,
                             Material* mat) {
  appendTetra(-1, mat);
...
void btSoftBody::appendAnchor(int node, btRigidBody* body,
  ...
  if (disableCollisionBetweenLinkedBodies) {
    if (m_collisionDisabledObjects.findLinearSearch(body) ==
        m_collisionDisabledObjects.size()) {
...
void btSoftBody::appendDeformableAnchor(int node, btRigidBody* body) {
  ...
  btScalar dst = m_worldInfo->m_sparsesdf.Evaluate(
      wtr.invXform(m_nodes[node].m_x), shp, nrm, 0);
...
void btSoftBody::appendDeformableAnchor(int node,
  ...
  btMatrix3x3 rot(normal.getX(), normal.getY(), normal.getZ(), t1.getX(),
                  t1.getY(), t1.getZ(), t2.getX(), t2.getY(),
                  t2.getZ());  // world frame to local frame
  const int ndof = link->m_multiBody->getNumDofs() + 6;
  btMatrix3x3 local_impulse_matrix =
      (Diagonal(n.m_im) + OuterProduct(J_n, J_t1, J_t2, u_n, u_t1, u_t2, ndof))
          .inverse();
...
//
void btSoftBody::appendAngularJoint(const AJoint::Specs& specs, Cluster* body0,
                                    Body body1) {
...
void btSoftBody::addAeroForceToNode(const btVector3& windVelocity,
  ...
  const bool as_vaero =
      as_aero && (m_cfg.aeromodel < btSoftBody::eAeroModel::F_TwoSided);
    ...
    if (as_vaero) {
      ...
      if (rel_v2 > SIMD_EPSILON) {
        ...
        if (m_cfg.aeromodel == btSoftBody::eAeroModel::V_TwoSidedLiftDrag) {
          ...
          fDrag = 0.5f * kDG * medium.m_density * rel_v2 * tri_area * n_dot_v *
                  (-rel_v_nrm);

          // Check angle of attack
          // cos(10º) = 0.98480
          if (0 < n_dot_v && n_dot_v < 0.98480f)
            fLift = 0.5f * kLF * medium.m_density * rel_v_len * tri_area *
                    btSqrt(1.0f - n_dot_v * n_dot_v) *
                    (nrm.cross(rel_v_nrm).cross(rel_v_nrm));

          // Check if the velocity change resulted by aero drag force exceeds
          ...
          n.m_f += fDrag;
          n.m_f += fLift;
        } else if (m_cfg.aeromodel == btSoftBody::eAeroModel::V_Point ||
                   m_cfg.aeromodel == btSoftBody::eAeroModel::V_OneSided ||
                   m_cfg.aeromodel == btSoftBody::eAeroModel::V_TwoSided) {
...
void btSoftBody::addAeroForceToFace(const btVector3& windVelocity,
  ...
  if (as_faero) {
    ...
    if (rel_v2 > SIMD_EPSILON) {
      ...
      } else if (m_cfg.aeromodel == btSoftBody::eAeroModel::F_OneSided ||
...
void btSoftBody::setTotalMass(btScalar mass, bool fromfaces) {
  ...
  if (fromfaces) {
    ...
    for (i = 0; i < m_faces.size(); ++i) {
      const Face& f = m_faces[i];
      const btScalar twicearea =
          AreaOf(f.m_n[0]->m_x, f.m_n[1]->m_x, f.m_n[2]->m_x);
      for (int j = 0; j < 3; ++j) {
...
//
btScalar btSoftBody::getRestLengthScale() { return m_restLengthScale; }
...
void btSoftBody::setRestLengthScale(btScalar restLengthScale) {
  ...
  if (getActivationState() == ISLAND_SLEEPING) activate();
}
...
void btSoftBody::setPose(bool bvolume, bool bframe) {
  ...
  for (i = 0, ni = m_nodes.size(); i < ni; ++i) {
    Node& n = m_nodes[i];
    m_pose.m_wgh[i] =
        n.m_im > 0 ? 1 / (m_nodes[i].m_im * tmass) : kmass / tmass;
  }
  /* Pos		*/
  ...
  m_pose.m_scl.setIdentity();
  /* Aqq		*/
  m_pose.m_aqq[0] = m_pose.m_aqq[1] = m_pose.m_aqq[2] = btVector3(0, 0, 0);
  for (i = 0, ni = m_nodes.size(); i < ni; ++i) {
...
btScalar btSoftBody::getVolume() const {
  ...
  if (m_nodes.size() > 0) {
    ...
    for (i = 0, ni = m_faces.size(); i < ni; ++i) {
      const Face& f = m_faces[i];
      vol += btDot(f.m_n[0]->m_x - org,
                   btCross(f.m_n[1]->m_x - org, f.m_n[2]->m_x - org));
    }
    vol /= (btScalar)6;
  }
  return (vol);
...
//
int btSoftBody::clusterCount() const { return (m_clusters.size()); }
...
//
void btSoftBody::clusterVImpulse(Cluster* cluster, const btVector3& rpos,
...
//
void btSoftBody::clusterDImpulse(Cluster* cluster, const btVector3& rpos,
                                 const btVector3& impulse) {
  const btVector3 li = cluster->m_imass * impulse;
  const btVector3 ai = cluster->m_invwi * btCross(rpos, impulse);
...
//
void btSoftBody::clusterImpulse(Cluster* cluster, const btVector3& rpos,
...
#define IDX(_x_, _y_) ((_y_)*n + (_x_))
    ...
    if (distance == 2) {
      ...
      for (i = 0; i < m_links.size(); ++i) {
        const int ia = (int)(m_links[i].m_n[0] - &m_nodes[0]);
        const int ib = (int)(m_links[i].m_n[1] - &m_nodes[0]);
        if (nodeLinks[ia].m_links.findLinearSearch(ib) ==
            nodeLinks[ia].m_links.size())
          nodeLinks[ia].m_links.push_back(ib);

        if (nodeLinks[ib].m_links.findLinearSearch(ia) ==
            nodeLinks[ib].m_links.size())
...
int btSoftBody::generateClusters(int k, int maxiterations) {
  ...
  if (k > 0) {
    ...
    for (i = 0; i < m_nodes.size(); ++i) {
      cog += m_nodes[i].m_x;
      m_clusters[(i * 29873) % m_clusters.size()]->m_nodes.push_back(
          &m_nodes[i]);
    }
    ...
    for (i = 0; i < m_faces.size(); ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        for (int q = 1; q < 3; ++q) {
          ...
          if (cids[kid] != cid) {
            if (m_clusters[cid]->m_nodes.findLinearSearch(&m_nodes[kid]) ==
                m_clusters[cid]->m_nodes.size()) {
...
void btSoftBody::refine(ImplicitFn* ifn, btScalar accurary, bool cut) {
  ...
  for (i = 0, ni = m_links.size(); i < ni; ++i) {
    Link& feat = m_links[i];
    const int idx[] = {int(feat.m_n[0] - nbase), int(feat.m_n[1] - nbase)};
  ...
  if (cut) {
    ...
    for (i = 0; i < m_links.size(); ++i) {
      const int id[] = {int(m_links[i].m_n[0] - nbase),
                        int(m_links[i].m_n[1] - nbase)};
      const bool sg[] = {ranks[id[0]] == 1, ranks[id[1]] == 1};
      if (sg[0] || sg[1]) {
        --ranks[id[0]];
        --ranks[id[1]];
        btSwap(m_links[i], m_links[m_links.size() - 1]);
...
//
bool btSoftBody::cutLink(const Node* node0, const Node* node1,
                         btScalar position) {
  return (
      cutLink(int(node0 - &m_nodes[0]), int(node1 - &m_nodes[0]), position));
}
...
bool btSoftBody::cutLink(int node0, int node1, btScalar position) {
  ...
  Node* pn[2] = {&m_nodes[m_nodes.size() - 2], &m_nodes[m_nodes.size() - 1]};
  pn[0]->m_v = v;
  pn[1]->m_v = v;
  for (i = 0, ni = m_links.size(); i < ni; ++i) {
...
//
bool btSoftBody::rayTest(const btVector3& rayFrom, const btVector3& rayTo,
                         sRayCast& results) {
  ...
  return (rayTest(rayFrom, rayTo, results.fraction, results.feature,
...
void btSoftBody::predictMotion(btScalar dt) {
  ...
  for (i = 0, ni = m_nodes.size(); i < ni; ++i) {
    ...
    m_ndbvt.update(n.m_leaf, vol, n.m_v * m_sst.velmrg, m_sst.updmrg);
  }
  /* Faces                */
  if (!m_fdbvt.empty()) {
    for (int i = 0; i < m_faces.size(); ++i) {
      ...
      m_fdbvt.update(f.m_leaf, vol, v * m_sst.velmrg, m_sst.updmrg);
    }
  }
  /* Pose                    */
  updatePose();
...
void btSoftBody::solveConstraints() {
  ...
  for (i = 0, ni = m_anchors.size(); i < ni; ++i) {
    ...
    a.m_c0 = ImpulseMatrix(m_sst.sdt, a.m_node->m_im, a.m_body->getInvMass(),
...
//
void btSoftBody::solveCommonConstraints(btSoftBody** /*bodies*/, int /*count*/,
                                        int /*iterations*/) {
  /// placeholder
}

//
void btSoftBody::solveClusters(
    const btAlignedObjectArray<btSoftBody*>& bodies) {
...
void btSoftBody::RayFromToCaster::Process(const btDbvtNode* leaf) {
  ...
  const btScalar t =
      rayFromToTriangle(m_rayFrom, m_rayTo, m_rayNormalizedDirection,
                        f.m_n[0]->m_x, f.m_n[1]->m_x, f.m_n[2]->m_x, m_mint);
  if ((t > 0) && (t < m_mint)) {
    m_mint = t;
    m_face = &f;
  }
...
btScalar btSoftBody::RayFromToCaster::rayFromToTriangle(
    ...
    const btVector3& rayNormalizedDirection, const btVector3& a,
...
int btSoftBody::rayTest(const btVector3& rayFrom, const btVector3& rayTo,
                        btScalar& mint, eFeature::_& feature, int& index,
                        bool bcountonly) const {
  int cnt = 0;
  btVector3 dir = rayTo - rayFrom;

  if (bcountonly || m_fdbvt.empty()) { /* Full search	*/

    for (int i = 0, ni = m_faces.size(); i < ni; ++i) {
      const btSoftBody::Face& f = m_faces[i];

      const btScalar t = RayFromToCaster::rayFromToTriangle(
          rayFrom, rayTo, dir, f.m_n[0]->m_x, f.m_n[1]->m_x, f.m_n[2]->m_x,
          mint);
      if (t > 0) {
        ++cnt;
        if (!bcountonly) {
...
void btSoftBody::rebuildNodeTree() {
  ...
  for (int i = 0; i < m_nodes.size(); ++i) old_id[i] = m_nodes[i].index;
  for (int i = 0; i < m_nodes.size(); ++i) m_nodes[i].index = i;
  ...
  for (int i = 0; i < m_nodes.size(); ++i) m_nodes[i].index = old_id[i];
}
...
bool btSoftBody::checkContact(const btCollisionObjectWrapper* colObjWrap,
                              const btVector3& x, btScalar margin,
                              btSoftBody::sCti& cti) const {
  btVector3 nrm;
  const btCollisionShape* shp = colObjWrap->getCollisionShape();
  //    const btRigidBody *tmpRigid =
  //    btRigidBody::upcast(colObjWrap->getCollisionObject());
  // const btTransform &wtr = tmpRigid ? tmpRigid->getWorldTransform() :
  // colObjWrap->getWorldTransform();
  const btTransform& wtr = colObjWrap->getWorldTransform();
  // todo: check which transform is needed here

  btScalar dst =
      m_worldInfo->m_sparsesdf.Evaluate(wtr.invXform(x), shp, nrm, margin);
  if (dst < 0) {
...
//
bool btSoftBody::checkDeformableContact(
    const btCollisionObjectWrapper* colObjWrap, const btVector3& x,
    btScalar margin, btSoftBody::sCti& cti, bool predict) const {
  btVector3 nrm;
  const btCollisionShape* shp = colObjWrap->getCollisionShape();
  const btCollisionObject* tmpCollisionObj = colObjWrap->getCollisionObject();
  // use the position x_{n+1}^* = x_n + dt * v_{n+1}^* where v_{n+1}^* = v_n +
  // dtg for collision detect
  // but resolve contact at x_n
  btTransform wtr =
      (predict) ? (colObjWrap->m_preTransform != NULL
                       ? tmpCollisionObj->getInterpolationWorldTransform() *
                             (*colObjWrap->m_preTransform)
                       : tmpCollisionObj->getInterpolationWorldTransform())
                : colObjWrap->getWorldTransform();
  btScalar dst =
      m_worldInfo->m_sparsesdf.Evaluate(wtr.invXform(x), shp, nrm, margin);
...
static void getBarycentric(const btVector3& p, btVector3& a, btVector3& b,
                           btVector3& c, btVector3& bary) {
...
//
bool btSoftBody::checkDeformableFaceContact(
    const btCollisionObjectWrapper* colObjWrap, Face& f,
    btVector3& contact_point, btVector3& bary, btScalar margin,
...
// use the contact position of the previous collision
  ...
  if (f.m_pcontact[3] != 0) {
    for (int i = 0; i < 3; ++i) bary[i] = f.m_pcontact[i];
    contact_point = BaryEval(f.m_n[0]->m_x, f.m_n[1]->m_x, f.m_n[2]->m_x, bary);
    dst = m_worldInfo->m_sparsesdf.Evaluate(wtr.invXform(contact_point), shp,
                                            nrm, margin);
    ...
    btTriangleShape triangle(btVector3(0, 0, 0), f.m_n[1]->m_x - f.m_n[0]->m_x,
                             f.m_n[2]->m_x - f.m_n[0]->m_x);
    btVector3 guess(0, 0, 0);
    const btConvexShape* csh = static_cast<const btConvexShape*>(shp);
    btGjkEpaSolver2::SignedDistance(&triangle, triangle_transform, csh, wtr,
                                    guess, results);
    dst = results.distance - margin;
    contact_point = results.witnesses[0];
    getBarycentric(contact_point, f.m_n[0]->m_x, f.m_n[1]->m_x, f.m_n[2]->m_x,
                   bary);
    nrm = results.normal;
    cti.m_colObj = colObjWrap->getCollisionObject();
    for (int i = 0; i < 3; ++i) f.m_pcontact[i] = bary[i];
  }
  return (dst < 0);
#endif
...
// use collision quadrature point
#ifdef USE_QUADRATURE
  {
    ...
    for (int q = 0; q < m_quads.size(); ++q) {
      ...
      btScalar local_dst = m_worldInfo->m_sparsesdf.Evaluate(
          wtr.invXform(p), shp, local_nrm, margin);
      if (local_dst < dst) {
        if (local_dst < 0 && predict) return true;
        dst = local_dst;
        contact_point = p;
        bary = m_quads[q];
        nrm = local_nrm;
  ...
  // regular face contact
  {
    ...
    if (predict) {
      ...
      triangle =
          btTriangleShape(btVector3(0, 0, 0), f.m_n[1]->m_q - f.m_n[0]->m_q,
                          f.m_n[2]->m_q - f.m_n[0]->m_q);
    }
    const btConvexShape* csh = static_cast<const btConvexShape*>(shp);
    btGjkEpaSolver2::SignedDistance(&triangle, triangle_transform, csh, wtr,
                                    guess, results);
...
void btSoftBody::updateNormals() {
  ...
  for (i = 0, ni = m_faces.size(); i < ni; ++i) {
    ...
    const btVector3 n =
        btCross(f.m_n[1]->m_x - f.m_n[0]->m_x, f.m_n[2]->m_x - f.m_n[0]->m_x);
    f.m_normal = n;
  ...
  for (i = 0, ni = m_nodes.size(); i < ni; ++i) {
    ...
    if (len > SIMD_EPSILON) m_nodes[i].m_n /= len;
  }
}
...
void btSoftBody::updateBounds() {
  ...
  {
          // If we have an accelerated softbody we need to obtain the bounds
  correctly
          // For now (slightly hackily) just have a very large AABB
          // TODO: Write get bounds kernel
          // If that is updating in place, atomic collisions might be low (when
  the cloth isn't perfectly aligned to an axis) and we could
  ...
  if (m_nodes.size()) {
    ...
    for (int i = 1; i < m_nodes.size(); ++i) {
      for (int d = 0; d < 3; ++d) {
        if (m_nodes[i].m_x[d] > maxs[d]) maxs[d] = m_nodes[i].m_x[d];
        if (m_nodes[i].m_x[d] < mins[d]) mins[d] = m_nodes[i].m_x[d];
      }
    }
    const btScalar csm = getCollisionShape()->getMargin();
    const btVector3 mrg = btVector3(csm, csm, csm);
    m_bounds[0] = mins - mrg;
    m_bounds[1] = maxs + mrg;
    if (0 != getBroadphaseHandle()) {
      m_worldInfo->m_broadphase->setAabb(getBroadphaseHandle(), m_bounds[0],
                                         m_bounds[1],
                                         m_worldInfo->m_dispatcher);
    }
  } else {
...
void btSoftBody::updatePose() {
  if (m_pose.m_bframe) {
    ...
    if (m_cfg.maxvolume > 1) {
      const btScalar idet =
          Clamp<btScalar>(1 / pose.m_scl.determinant(), 1, m_cfg.maxvolume);
...
/* Inertia			*/
#if 1 /* Constant	*/
      c.m_invwi = c.m_framexform.getBasis() * c.m_locii *
      ...
      c.m_vimpulses[0] = c.m_vimpulses[1] = btVector3(0, 0, 0);
      c.m_dimpulses[0] = c.m_dimpulses[1] = btVector3(0, 0, 0);
      c.m_nvimpulses = 0;
      c.m_ndimpulses = 0;
      /* Matching				*/
      if (c.m_matching > 0) {
      ...
      if (c.m_collide) {
        ...
        if (c.m_leaf)
          m_cdbvt.update(c.m_leaf, bounds, c.m_lv * m_sst.sdt * 3,
                         m_sst.radmrg);
        else
...
void btSoftBody::applyClusters(bool drift) {
  ...
  for (i = 0; i < m_clusters.size(); ++i) {
    ...
    if (0 < (drift ? c.m_ndimpulses : c.m_nvimpulses)) {
      const btVector3 v =
          (drift ? c.m_dimpulses[0] : c.m_vimpulses[0]) * m_sst.sdt;
      const btVector3 w =
          (drift ? c.m_dimpulses[1] : c.m_vimpulses[1]) * m_sst.sdt;
      for (int j = 0; j < c.m_nodes.size(); ++j) {
  ...
  for (i = 0; i < m_clusters.size(); ++i) {
    ...
    if (c.m_ndamping > 0) {
      for (int j = 0; j < c.m_nodes.size(); ++j) {
        ...
        if (n.m_im > 0) {
          const btVector3 vx =
              c.m_lv + btCross(c.m_av, c.m_nodes[j]->m_q - c.m_com);
...
void btSoftBody::initializeDmInverse() {
  ...
  for (int i = 0; i < m_tetras.size(); ++i) {
    ...
    btMatrix3x3 Dm(c1.getX(), c2.getX(), c3.getX(), c1.getY(), c2.getY(),
                   c3.getY(), c1.getZ(), c2.getZ(), c3.getZ());
...
void btSoftBody::updateDeformation() {
  for (int i = 0; i < m_tetras.size(); ++i) {
    ...
    btMatrix3x3 Ds(c1.getX(), c2.getX(), c3.getX(), c1.getY(), c2.getY(),
                   c3.getY(), c1.getZ(), c2.getZ(), c3.getZ());
...
void btSoftBody::LJoint::Prepare(btScalar dt, int iterations) {
  ...
  m_massmatrix = ImpulseMatrix(
      m_bodies[0].invMass(), m_bodies[0].invWorldInertia(), m_rpos[0],
...
void btSoftBody::AJoint::Prepare(btScalar dt, int iterations) {
  ...
  m_drift *= btMin(
      maxdrift, btAcos(Clamp<btScalar>(btDot(m_axis[0], m_axis[1]), -1, +1)));
  m_drift *= m_erp / dt;
  m_massmatrix = AngularImpulseMatrix(m_bodies[0].invWorldInertia(),
                                      m_bodies[1].invWorldInertia());
  if (m_split > 0) {
    m_sdrift = m_massmatrix * (m_drift * m_split);
    m_drift *= 1 - m_split;
  }
...
void btSoftBody::CJoint::Solve(btScalar dt, btScalar sor) {
  ...
  if (m_bodies[0].m_soft == m_bodies[1].m_soft) {
    if ((impulse.m_velocity.getX() == impulse.m_velocity.getX()) &&
        (impulse.m_velocity.getY() == impulse.m_velocity.getY()) &&
        (impulse.m_velocity.getZ() == impulse.m_velocity.getZ())) {
      if (impulse.m_asVelocity) {
        if (impulse.m_velocity.length() <
            m_bodies[0].m_soft->m_maxSelfCollisionImpulse) {
        } else {
          m_bodies[0].applyImpulse(
              -impulse * m_bodies[0].m_soft->m_selfCollisionImpulseFactor,
              m_rpos[0]);
          m_bodies[1].applyImpulse(
              impulse * m_bodies[0].m_soft->m_selfCollisionImpulseFactor,
              m_rpos[1]);
        }
      }
  ...
  for (int i = 0; i < m_renderNodes.size(); ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      if (m_renderNodesParents[i].size()) {
        n.m_x += m_renderNodesParents[i][j]->m_x *
                 m_renderNodesInterpolationWeights[i][j];
      }
    }
  }
}
...
void btSoftBody::setCollisionQuadrature(int N) {
  for (int i = 0; i <= N; ++i) {
    for (int j = 0; i + j <= N; ++j) {
      m_quads.push_back(btVector3(btScalar(i) / btScalar(N),
                                  btScalar(j) / btScalar(N),
                                  btScalar(N - i - j) / btScalar(N)));
    }
  }
}
  ...
  for (int i = 0, ni = psb->m_rcontacts.size(); i < ni; ++i) {
    ...
    if (cti.m_colObj->hasContactResponse()) {
      ...
      if (cti.m_colObj->getInternalType() == btCollisionObject::CO_RIGID_BODY) {
        ...
        va = rigidCol ? rigidCol->getVelocityInLocalPoint(c.m_c1) * dt
                      : btVector3(0, 0, 0);
      } else if (cti.m_colObj->getInternalType() ==
        ...
        if (multibodyLinkCol) {
          ...
          multibodyLinkCol->m_multiBody->fillContactJacobianMultiDof(
              multibodyLinkCol->m_link, c.m_node->m_x, cti.m_normal, jac,
              jacobianData.scratch_r, jacobianData.scratch_v,
              jacobianData.scratch_m);
          deltaV = &jacobianData.m_deltaVelocitiesUnitImpulse[0];
          multibodyLinkCol->m_multiBody->calcAccelerationDeltasMultiDof(
              &jacobianData.m_jacobians[0], deltaV, jacobianData.scratch_r,
              jacobianData.scratch_v);

          btScalar vel = 0.0;
          for (int j = 0; j < ndof; ++j) {
            vel +=
                multibodyLinkCol->m_multiBody->getVelocityVector()[j] * jac[j];
          }
      ...
      if (dn <= SIMD_EPSILON) {
        const btScalar dp =
            btMin((btDot(c.m_node->m_x, cti.m_normal) + cti.m_offset), mrg);
        const btVector3 fv = vr - (cti.m_normal * dn);
        // c0 is the impulse matrix, c3 is 1 - the friction coefficient or 0, c4
        // is the contact hardness coefficient
        const btVector3 impulse =
            c.m_c0 *
            ((vr - (fv * c.m_c3) + (cti.m_normal * (dp * c.m_c4))) * kst);
        c.m_node->m_x -= impulse * c.m_c2;

        if (cti.m_colObj->getInternalType() ==
            btCollisionObject::CO_RIGID_BODY) {
          if (rigidCol) rigidCol->applyImpulse(impulse, c.m_c1);
        } else if (cti.m_colObj->getInternalType() ==
                   btCollisionObject::CO_FEATHERSTONE_LINK) {
          if (multibodyLinkCol) {
            double multiplier = 0.5;
            multibodyLinkCol->m_multiBody->applyDeltaVeeMultiDof(
                deltaV, -impulse.length() * multiplier);
          }
        }
      }
    }
...
void btSoftBody::PSolve_SContacts(btSoftBody* psb, btScalar, btScalar ti) {
  ...
  for (int i = 0, ni = psb->m_scontacts.size(); i < ni; ++i) {
    ...
    Face& f = *c.m_face;
    const btVector3 p =
        BaryEval(f.m_n[0]->m_x, f.m_n[1]->m_x, f.m_n[2]->m_x, c.m_weights);
    const btVector3 q =
        BaryEval(f.m_n[0]->m_q, f.m_n[1]->m_q, f.m_n[2]->m_q, c.m_weights);
...
bool btSoftBody::useSelfCollision() { return m_useSelfCollision; }

//
void btSoftBody::defaultCollisionHandler(
    const btCollisionObjectWrapper* pcoWrap) {
  switch (m_cfg.collisions & fCollision::RVSmask) {
    case fCollision::SDF_RS: {
      btSoftColliders::CollideSDF_RS docollide;
      btRigidBody* prb1 =
          (btRigidBody*)btRigidBody::upcast(pcoWrap->getCollisionObject());
      ...
      ATTRIBUTE_ALIGNED16(btDbvtVolume)
      volume;
      pcoWrap->getCollisionShape()->getAabb(pcoWrap->getWorldTransform(), mins,
                                            maxs);
    ...
    case fCollision::SDF_RD: {
      ...
      if (pcoWrap->getCollisionObject()->isActive() || this->isActive()) {
        ...
        //                const btTransform ctr = pcoWrap->getWorldTransform();
        //                const btScalar timemargin = (wtr.getOrigin() -
        //                ctr.getOrigin()).length();
...
void btSoftBody::geometricCollisionHandler(btSoftBody* psb) {
  if (psb->isActive() || this->isActive()) {
    if (this != psb) {
      ...
      /* common                    */
      docollide.mrg =
          SAFE_EPSILON;  // for rounding error instead of actual margin
      ...
      if (psb->useSelfCollision()) {
        ...
        /* psb0 faces vs psb0 faces    */
        calculateNormalCone(
            this->m_fdbvnt);  // should compute this outside of this scope
        this->m_fdbvt.selfCollideT(m_fdbvnt, docollide);
      }
    }
  }
...
const char* btSoftBody::serialize(void* dataBuffer,
  ...
  sbd->m_materials = sbd->m_numMaterials
                         ? (SoftBodyMaterialData**)serializer->getUniquePointer(
                               (void*)&m_materials)
                         : 0;
  ...
  if (sbd->m_materials) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      ...
      *memPtr =
          mat ? (SoftBodyMaterialData*)serializer->getUniquePointer((void*)mat)
              : 0;
      if (!serializer->findPointer(mat)) {
        ...
        serializer->finalizeChunk(chunk, "SoftBodyMaterialData",
                                  BT_SBMATERIAL_CODE, mat);
      }
    }
    serializer->finalizeChunk(chunk, "SoftBodyMaterialData", BT_ARRAY_CODE,
                              (void*)&m_materials);
  }
  ...
  sbd->m_nodes =
      sbd->m_numNodes
          ? (SoftBodyNodeData*)serializer->getUniquePointer((void*)&m_nodes)
          : 0;
  if (sbd->m_nodes) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      ...
      memPtr->m_material =
          m_nodes[i].m_material
              ? (SoftBodyMaterialData*)serializer->getUniquePointer(
                    (void*)m_nodes[i].m_material)
              : 0;
    ...
    serializer->finalizeChunk(chunk, "SoftBodyNodeData", BT_SBNODE_CODE,
                              (void*)&m_nodes);
  }
  ...
  sbd->m_links =
      sbd->m_numLinks
          ? (SoftBodyLinkData*)serializer->getUniquePointer((void*)&m_links[0])
          : 0;
  if (sbd->m_links) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      ...
      memPtr->m_material =
          m_links[i].m_material
              ? (SoftBodyMaterialData*)serializer->getUniquePointer(
                    (void*)m_links[i].m_material)
              : 0;
      memPtr->m_nodeIndices[0] =
          m_links[i].m_n[0] ? m_links[i].m_n[0] - &m_nodes[0] : -1;
      memPtr->m_nodeIndices[1] =
          m_links[i].m_n[1] ? m_links[i].m_n[1] - &m_nodes[0] : -1;
    ...
    serializer->finalizeChunk(chunk, "SoftBodyLinkData", BT_ARRAY_CODE,
                              (void*)&m_links[0]);
  }
  ...
  sbd->m_faces =
      sbd->m_numFaces
          ? (SoftBodyFaceData*)serializer->getUniquePointer((void*)&m_faces[0])
          : 0;
  if (sbd->m_faces) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      memPtr->m_material =
          m_faces[i].m_material
              ? (SoftBodyMaterialData*)serializer->getUniquePointer(
                    (void*)m_faces[i].m_material)
              : 0;
      m_faces[i].m_normal.serializeFloat(memPtr->m_normal);
      for (int j = 0; j < 3; j++) {
        memPtr->m_nodeIndices[j] =
            m_faces[i].m_n[j] ? m_faces[i].m_n[j] - &m_nodes[0] : -1;
      }
      memPtr->m_restArea = m_faces[i].m_ra;
    }
    serializer->finalizeChunk(chunk, "SoftBodyFaceData", BT_ARRAY_CODE,
                              (void*)&m_faces[0]);
  }

  sbd->m_numTetrahedra = m_tetras.size();
  sbd->m_tetrahedra = sbd->m_numTetrahedra
                          ? (SoftBodyTetraData*)serializer->getUniquePointer(
                                (void*)&m_tetras[0])
                          : 0;
  if (sbd->m_tetrahedra) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      for (int j = 0; j < 4; j++) {
        ...
        memPtr->m_nodeIndices[j] =
            m_tetras[i].m_n[j] ? m_tetras[i].m_n[j] - &m_nodes[0] : -1;
      }
      memPtr->m_c1 = m_tetras[i].m_c1;
      memPtr->m_c2 = m_tetras[i].m_c2;
      memPtr->m_material =
          m_tetras[i].m_material
              ? (SoftBodyMaterialData*)serializer->getUniquePointer(
                    (void*)m_tetras[i].m_material)
              : 0;
      memPtr->m_restVolume = m_tetras[i].m_rv;
    }
    serializer->finalizeChunk(chunk, "SoftBodyTetraData", BT_ARRAY_CODE,
                              (void*)&m_tetras[0]);
  }

  sbd->m_numAnchors = m_anchors.size();
  sbd->m_anchors = sbd->m_numAnchors
                       ? (SoftRigidAnchorData*)serializer->getUniquePointer(
                             (void*)&m_anchors[0])
                       : 0;
  if (sbd->m_anchors) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      ...
      memPtr->m_nodeIndex =
          m_anchors[i].m_node ? m_anchors[i].m_node - &m_nodes[0] : -1;

      memPtr->m_rigidBody =
          m_anchors[i].m_body
              ? (btRigidBodyData*)serializer->getUniquePointer(
                    (void*)m_anchors[i].m_body)
              : 0;
      btAssert(memPtr->m_nodeIndex < m_nodes.size());
    }
    serializer->finalizeChunk(chunk, "SoftRigidAnchorData", BT_ARRAY_CODE,
                              (void*)&m_anchors[0]);
  }
  ...
  {
    sbd->m_pose =
        (SoftBodyPoseData*)serializer->getUniquePointer((void*)&m_pose);
    ...
    memPtr->m_positions =
        memPtr->m_numPositions
            ? (btVector3FloatData*)serializer->getUniquePointer(
                  (void*)&m_pose.m_pos[0])
            : 0;
    if (memPtr->m_numPositions) {
      ...
      serializer->finalizeChunk(chunk, "btVector3FloatData", BT_ARRAY_CODE,
                                (void*)&m_pose.m_pos[0]);
    }
    ...
    memPtr->m_weights =
        memPtr->m_numWeigts
            ? (float*)serializer->getUniquePointer((void*)&m_pose.m_wgh[0])
            : 0;
    if (memPtr->m_numWeigts) {
      ...
      serializer->finalizeChunk(chunk, "float", BT_ARRAY_CODE,
                                (void*)&m_pose.m_wgh[0]);
    }

    serializer->finalizeChunk(chunk, "SoftBodyPoseData", BT_ARRAY_CODE,
                              (void*)&m_pose);
  }
  ...
  sbd->m_clusters = sbd->m_numClusters
                        ? (SoftBodyClusterData*)serializer->getUniquePointer(
                              (void*)m_clusters[0])
                        : 0;
  if (sbd->m_numClusters) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      ...
      memPtr->m_framerefs =
          memPtr->m_numFrameRefs
              ? (btVector3FloatData*)serializer->getUniquePointer(
                    (void*)&m_clusters[i]->m_framerefs[0])
              : 0;
      ...
      memPtr->m_masses = memPtr->m_numMasses
                             ? (float*)serializer->getUniquePointer(
                                   (void*)&m_clusters[i]->m_masses[0])
                             : 0;
      if (memPtr->m_masses) {
      ...
      memPtr->m_nodeIndices = memPtr->m_numNodes
                                  ? (int*)serializer->getUniquePointer(
                                        (void*)&m_clusters[i]->m_nodes)
                                  : 0;
      if (memPtr->m_nodeIndices) {
        ...
        serializer->finalizeChunk(chunk, "int", BT_ARRAY_CODE,
                                  (void*)&m_clusters[i]->m_nodes);
      }
    }
    serializer->finalizeChunk(chunk, "SoftBodyClusterData", BT_ARRAY_CODE,
                              (void*)m_clusters[0]);
  }

  sbd->m_numJoints = m_joints.size();
  sbd->m_joints = m_joints.size()
                      ? (btSoftBodyJointData*)serializer->getUniquePointer(
                            (void*)&m_joints[0])
                      : 0;
...
bool btSoftBody::wantsSleeping() {
  ...
  if (gDisableDeactivation || (gDeactivationTime == btScalar(0.))) return false;
