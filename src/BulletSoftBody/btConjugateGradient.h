...
class btConjugateGradient {
 ...
 public:
  btConjugateGradient(const int max_it_in) : max_iterations(max_it_in) {
  ...
  btScalar squaredNorm(const TVStack& a) { return dot(a, a); }

  btScalar dot(const TVStack& a, const TVStack& b) {
    btScalar ans(0);
    for (int i = 0; i < a.size(); ++i) ans += a[i].dot(b[i]);
    return ans;
  }
  ...
  void multAndAddTo(btScalar s, const TVStack& a, TVStack& result) {
    ...
    for (int i = 0; i < a.size(); ++i) result[i] += s * a[i];
  }
  ...
  TVStack multAndAdd(btScalar s, const TVStack& a, const TVStack& b) {
    ...
    for (int i = 0; i < a.size(); ++i) result[i] = s * a[i] + b[i];
    return result;
  }
};
