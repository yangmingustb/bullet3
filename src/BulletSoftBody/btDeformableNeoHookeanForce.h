...
// This energy is as described in
...
class btDeformableNeoHookeanForce : public btDeformableLagrangianForce {
 public:
  ...
  btDeformableNeoHookeanForce(btScalar mu, btScalar lambda,
                              btScalar damping = 0.05)
  ...
  // The damping matrix is calculated using the time n state as described in
  // https://www.math.ucla.edu/~jteran/papers/GSSJT15.pdf to allow line search
  virtual void addScaledDampingForce(btScalar scale, TVStack& force) {
    if (m_mu_damp == 0 && m_lambda_damp == 0) return;
    ...
    for (int i = 0; i < m_softBodies.size(); ++i) {
      ...
      for (int j = 0; j < psb->m_tetras.size(); ++j) {
        ...
        btMatrix3x3 dF =
            DsFromVelocity(node0, node1, node2, node3) * tetra.m_Dm_inverse;
        btMatrix3x3 I;
        I.setIdentity();
        btMatrix3x3 dP = (dF + dF.transpose()) * m_mu_damp +
                         I * (dF[0][0] + dF[1][1] + dF[2][2]) * m_lambda_damp;
        //                firstPiolaDampingDifferential(psb->m_tetraScratchesTn[j],
        //                dF, dP);
        btVector3 df_on_node0 =
            dP * (tetra.m_Dm_inverse.transpose() * grad_N_hat_1st_col);
  ...
  // The damping energy is formulated as in
  // https://www.math.ucla.edu/~jteran/papers/GSSJT15.pdf to allow line search
  virtual double totalDampingEnergy(btScalar dt) {
    ...
    for (int i = 0; i < dampingForce.size(); ++i) dampingForce[i].setZero();
  ...
  double elasticEnergyDensity(const btSoftBody::TetraScratch& s) {
    ...
    density += m_lambda * 0.5 * (s.m_J - 1. - 0.75 * m_mu / m_lambda) *
               (s.m_J - 1. - 0.75 * m_mu / m_lambda);
...
#ifdef USE_SVD
        if (max_p > 0) {
          // since we want to clamp the principal stress to max_p, we only need
          // to
          // calculate SVD when sigma_0^2 + sigma_1^2 + sigma_2^2 > max_p *
          // max_p
...
#endif
        //                btVector3 force_on_node0 = P *
        //                (tetra.m_Dm_inverse.transpose()*grad_N_hat_1st_col);
  ...
  // The damping matrix is calculated using the time n state as described in
  // https://www.math.ucla.edu/~jteran/papers/GSSJT15.pdf to allow line search
  virtual void addScaledDampingForceDifferential(btScalar scale,
                                                 const TVStack& dv,
                                                 TVStack& df) {
    if (m_mu_damp == 0 && m_lambda_damp == 0) return;
    int numNodes = getNumNodes();
  ...
  virtual void addScaledElasticForceDifferential(btScalar scale,
                                                 const TVStack& dx,
                                                 TVStack& df) {
    int numNodes = getNumNodes();
    btAssert(numNodes <= df.size());
  ...
  void firstPiolaDifferential(const btSoftBody::TetraScratch& s,
                              const btMatrix3x3& dF, btMatrix3x3& dP) {
    btScalar c1 = m_mu * (1. - 1. / (s.m_trace + 1.));
    btScalar c2 = (2. * m_mu) * DotProduct(s.m_F, dF) *
                  (1. / ((1. + s.m_trace) * (1. + s.m_trace)));
    btScalar c3 = (m_lambda * DotProduct(s.m_cofF, dF));
    dP = dF * c1 + s.m_F * c2;
    addScaledCofactorMatrixDifferential(
        s.m_F, dF, m_lambda * (s.m_J - 1.) - 0.75 * m_mu, dP);
    dP += s.m_cofF * c3;
  }

  // Let Q be the damping stress.
  // This function calculates the dP = dQ/dF * dF
  void firstPiolaDampingDifferential(const btSoftBody::TetraScratch& s,
                                     const btMatrix3x3& dF, btMatrix3x3& dP) {
    btScalar c1 = (m_mu_damp * (1. - 1. / (s.m_trace + 1.)));
    btScalar c2 = ((2. * m_mu_damp) * DotProduct(s.m_F, dF) *
                   (1. / ((1. + s.m_trace) * (1. + s.m_trace))));
    btScalar c3 = (m_lambda_damp * DotProduct(s.m_cofF, dF));
    dP = dF * c1 + s.m_F * c2;
    addScaledCofactorMatrixDifferential(
        s.m_F, dF, m_lambda_damp * (s.m_J - 1.) - 0.75 * m_mu_damp, dP);
    dP += s.m_cofF * c3;
  }
  ...
  void addScaledCofactorMatrixDifferential(const btMatrix3x3& F,
                                           const btMatrix3x3& dF,
                                           btScalar scale, btMatrix3x3& M) {
