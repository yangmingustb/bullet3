...
int SolveP2(btScalar* x, btScalar a,
            btScalar b) {  // solve equation x^2 + a*x + b = 0
...
int SolveP3(btScalar* x, btScalar a, btScalar b,
            btScalar c) {  // solve cubic equation x^3 + a*x^2 + b*x + c = 0
  btScalar a2 = a * a;
  btScalar q = (a2 - 3 * b) / 9;
  if (q < 0) q = eps;
...
//---------------------------------------------------------------------------
// a>=0!
void CSqrt(btScalar x, btScalar y, btScalar& a,
           btScalar& b)  // returns:  a+i*s = sqrt(x+i*y)
{
...
//---------------------------------------------------------------------------
int SolveP4Bi(btScalar* x, btScalar b,
              btScalar d)  // solve equation x^4 + b*x^2 + d = 0
{
  btScalar D = b * b - 4 * d;
  if (D >= 0) {
    btScalar sD = sqrt(D);
...
}  // SolveP4Bi(btScalar *x, btScalar b, btScalar d)    // solve equation x^4 +
   // b*x^2 d
//---------------------------------------------------------------------------
#define SWAP(a, b) \
  {                \
...
static void dblSort3(btScalar& a, btScalar& b,
                     btScalar& c)  // make: a <= b <= c
{
  btScalar t;
...
//---------------------------------------------------------------------------
int SolveP4De(btScalar* x, btScalar b, btScalar c,
              btScalar d)  // solve equation x^4 + b*x^2 + c*x + d
{
  ...
  x[3] = szi;
  return 2;
}  // SolveP4De(btScalar *x, btScalar b, btScalar c, btScalar d)    // solve
   // equation x^4 + b*x^2 + c*x + d
//-----------------------------------------------------------------------------
btScalar N4Step(
    btScalar x, btScalar a, btScalar b, btScalar c,
    btScalar d)  // one Newton step for x^4 + a*x^3 + b*x^2 + c*x + d
{
...
int SolveP4(btScalar* x, btScalar a, btScalar b, btScalar c,
            btScalar d) {  // solve equation x^4 + a*x^3 + b*x^2 + c*x + d by
                           // Dekart-Euler method
  // move to a=0:
...
//-----------------------------------------------------------------------------
btScalar SolveP5_1(btScalar a, btScalar b, btScalar c, btScalar d,
                   btScalar e)  // return real root of x^5 + a*x^4 + b*x^3 +
                                // c*x^2 + d*x + e = 0
{
  int cnt;
  ...
  return x2;
}  // SolveP5_1(btScalar a,btScalar b,btScalar c,btScalar d,btScalar e)    //
   // return real root of x^5 + a*x^4 + b*x^3 + c*x^2 + d*x + e = 0
//-----------------------------------------------------------------------------
int SolveP5(
    btScalar* x, btScalar a, btScalar b, btScalar c, btScalar d,
    btScalar e)  // solve equation x^5 + a*x^4 + b*x^3 + c*x^2 + d*x + e = 0
{
  btScalar r = x[0] = SolveP5_1(a, b, c, d, e);
  btScalar a1 = a + r, b1 = b + r * a1, c1 = c + r * b1, d1 = d + r * c1;
  return 1 + SolveP4(x + 1, a1, b1, c1, d1);
}  // SolveP5(btScalar *x,btScalar a,btScalar b,btScalar c,btScalar d,btScalar
   // e)    // solve equation x^5 + a*x^4 + b*x^3 + c*x^2 + d*x + e = 0
//-----------------------------------------------------------------------------
