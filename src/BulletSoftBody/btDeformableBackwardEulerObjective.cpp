...
btDeformableBackwardEulerObjective::btDeformableBackwardEulerObjective(
    btAlignedObjectArray<btSoftBody*>& softBodies, const TVStack& backup_v)
...
void btDeformableBackwardEulerObjective::setDt(btScalar dt) { m_dt = dt; }
...
void btDeformableBackwardEulerObjective::multiply(const TVStack& x,
  ...
  for (int i = 0; i < m_softBodies.size(); ++i) {
    ...
    for (int j = 0; j < psb->m_nodes.size(); ++j) {
      ...
      b[counter] =
          (node.m_im == 0) ? btVector3(0, 0, 0) : x[counter] / node.m_im;
      ++counter;
    }
  }
...
void btDeformableBackwardEulerObjective::applyForce(TVStack& force,
  ...
  for (int i = 0; i < m_softBodies.size(); ++i) {
    ...
    for (int j = 0; j < psb->m_nodes.size(); ++j) {
      btScalar one_over_mass =
          (psb->m_nodes[j].m_im == 0) ? 0 : psb->m_nodes[j].m_im;
      psb->m_nodes[j].m_v += one_over_mass * force[counter++];
    }
  }
  if (setZero) {
    for (int i = 0; i < force.size(); ++i) force[i].setZero();
  }
}
