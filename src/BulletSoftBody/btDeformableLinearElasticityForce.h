...
class btDeformableLinearElasticityForce : public btDeformableLagrangianForce {
 public:
  ...
  btDeformableLinearElasticityForce(btScalar mu, btScalar lambda,
                                    btScalar damping = 0.05)
  ...
  double elasticEnergyDensity(const btSoftBody::TetraScratch& s) {
    double density = 0;
    btMatrix3x3 epsilon =
        (s.m_F + s.m_F.transpose()) * 0.5 - btMatrix3x3::getIdentity();
    btScalar trace = epsilon[0][0] + epsilon[1][1] + epsilon[2][2];
    density += m_mu * (epsilon[0].length2() + epsilon[1].length2() +
                       epsilon[2].length2());
  ...
  void firstPiolaDampingDifferential(const btSoftBody::TetraScratch& s,
                                     const btMatrix3x3& dF, btMatrix3x3& dP) {
    btScalar trace = (dF[0][0] + dF[1][1] + dF[2][2]);
    dP = (dF + dF.transpose()) * m_mu_damp +
         btMatrix3x3::getIdentity() * m_lambda_damp * trace;
  }
