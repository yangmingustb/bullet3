...
/// 2.75 release to increase ragdoll performance
/// If you experience problems with capsule-capsule collision, try to define
/// BT_DISABLE_CAPSULE_CAPSULE_COLLIDER and report it in the Bullet forums
...
static SIMD_FORCE_INLINE void segmentsClosestPoints(
    btVector3& ptsVector, btVector3& offsetA, btVector3& offsetB, btScalar& tA,
    btScalar& tB, const btVector3& translation, const btVector3& dirA,
    btScalar hlenA, const btVector3& dirB, btScalar hlenB) {
...
static SIMD_FORCE_INLINE btScalar capsuleCapsuleDistance(
    btVector3& normalOnB, btVector3& pointOnB, btScalar capsuleLengthA,
    btScalar capsuleRadiusA, btScalar capsuleLengthB, btScalar capsuleRadiusB,
    int capsuleAxisA, int capsuleAxisB, const btTransform& transformA,
    const btTransform& transformB, btScalar distanceThreshold) {
  ...
  btVector3 offsetA,
      offsetB;      // offsets from segment centers to their closest points
  ...
  if (lenSqr <= (SIMD_EPSILON * SIMD_EPSILON)) {
    // degenerate case where 2 capsules are likely at the same location: take a
    // vector tangential to 'directionA'
    btVector3 q;
...
btConvexConvexAlgorithm::btConvexConvexAlgorithm(
    ...
    : btActivatingCollisionAlgorithm(ci, body0Wrap, body1Wrap),
      ...
      m_sepDistance((static_cast<btConvexShape*>(body0->getCollisionShape()))
                        ->getAngularMotionDisc(),
                    (static_cast<btConvexShape*>(body1->getCollisionShape()))
                        ->getAngularMotionDisc()),
#endif
...
struct btPerturbedContactResult : public btManifoldResult {
  ...
  btPerturbedContactResult(btManifoldResult* originalResult,
                           ...
                           bool perturbA, btIDebugDraw* debugDrawer)
...
//#define DEBUG_CONTACTS 1
    ...
    m_originalManifoldResult->addContactPoint(normalOnBInWorld, startPt,
                                              newDepth);
  }
};
...
//
void btConvexConvexAlgorithm::processCollision(
    const btCollisionObjectWrapper* body0Wrap,
    const btCollisionObjectWrapper* body1Wrap,
  ...
  const btConvexShape* min0 =
      static_cast<const btConvexShape*>(body0Wrap->getCollisionShape());
  const btConvexShape* min1 =
      static_cast<const btConvexShape*>(body1Wrap->getCollisionShape());
  ...
  if ((min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE) &&
    ...
    btScalar dist = capsuleCapsuleDistance(
        normalOnB, pointOnBWorld, capsuleA->getHalfHeight(),
        capsuleA->getRadius(), capsuleB->getHalfHeight(), capsuleB->getRadius(),
  ...
  if ((min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE) &&
      (min1->getShapeType() == SPHERE_SHAPE_PROXYTYPE)) {
    // m_manifoldPtr->clearManifold();
    ...
    btScalar dist = capsuleCapsuleDistance(
        normalOnB, pointOnBWorld, capsuleA->getHalfHeight(),
        capsuleA->getRadius(), 0., capsuleB->getRadius(), capsuleA->getUpAxis(),
        1, body0Wrap->getWorldTransform(), body1Wrap->getWorldTransform(),
        threshold);
  ...
  if ((min0->getShapeType() == SPHERE_SHAPE_PROXYTYPE) &&
      (min1->getShapeType() == CAPSULE_SHAPE_PROXYTYPE)) {
    // m_manifoldPtr->clearManifold();
    ...
    btScalar dist = capsuleCapsuleDistance(
        normalOnB, pointOnBWorld, 0., capsuleA->getRadius(),
        capsuleB->getHalfHeight(), capsuleB->getRadius(), 1,
        capsuleB->getUpAxis(), body0Wrap->getWorldTransform(),
        body1Wrap->getWorldTransform(), threshold);
  ...
  if (dispatchInfo.m_useConvexConservativeDistanceUtil) {
    m_sepDistance.updateSeparatingDistance(body0->getWorldTransform(),
                                           body1->getWorldTransform());
  }

  if (!dispatchInfo.m_useConvexConservativeDistanceUtil ||
      m_sepDistance.getConservativeSeparatingDistance() <= 0.f)
...

  {
...
#endif  // USE_SEPDISTANCE_UTIL2
    {
      ...
      //{
      //	input.m_maximumDistanceSquared = min0->getMargin() +
      //min1->getMargin() + m_manifoldPtr->getContactProcessingThreshold();
      //} else
      //{
    ...
    if (min0->isPolyhedral() && min1->isPolyhedral()) {
      ...
      struct btWithoutMarginResult
        ...
        btWithoutMarginResult(
            ...
            btScalar marginOnA, btScalar marginOnB)
        ...
        virtual void addContactPoint(const btVector3& normalOnBInWorld,
          ...
          btVector3 adjustedPointB =
              pointInWorldOrg - normalOnBInWorld * m_marginOnB;
      ...
      /// btBoxShape is an exception: its vertices are created WITH margin so
      /// don't subtract it

      btScalar min0Margin =
          min0->getShapeType() == BOX_SHAPE_PROXYTYPE ? 0.f : min0->getMargin();
      btScalar min1Margin =
          min1->getShapeType() == BOX_SHAPE_PROXYTYPE ? 0.f : min1->getMargin();

      btWithoutMarginResult withoutMargin(resultOut, min0Margin, min1Margin);

      btPolyhedralConvexShape* polyhedronA = (btPolyhedralConvexShape*)min0;
      btPolyhedralConvexShape* polyhedronB = (btPolyhedralConvexShape*)min1;
      if (polyhedronA->getConvexPolyhedron() &&
...
// gjkPairDetector.getClosestPoints(input,dummy,dispatchInfo.m_debugDraw);
          ...
          // if (l2>SIMD_EPSILON)
          {
            sepNormalWorldSpace =
                withoutMargin
                    .m_reportedNormalOnWorld;  // gjkPairDetector.getCachedSeparatingAxis()*(1.f/l2);
...
#ifdef ZERO_MARGIN
            foundSepAxis =
                true;  // gjkPairDetector.getCachedSeparatingDistance()<0.f;
#else
            foundSepAxis =
                withoutMargin.m_foundResult &&
        ...
        if (foundSepAxis) {
          ...
          btPolyhedralContactClipping::clipHullAgainstHull(
              ...
              *polyhedronB->getConvexPolyhedron(),
              body0Wrap->getWorldTransform(), body1Wrap->getWorldTransform(),
              minDist - threshold, threshold, worldVertsB1, worldVertsB2,
              *resultOut);
        }
        if (m_ownManifold) {
          resultOut->refreshContactPoints();
        ...
        // we can also deal with convex versus triangle (without connectivity
        // data)
        if (dispatchInfo.m_enableSatConvex &&
            polyhedronA->getConvexPolyhedron() &&
          ...
          worldSpaceVertices.push_back(body1Wrap->getWorldTransform() *
                                       tri->m_vertices1[0]);
          worldSpaceVertices.push_back(body1Wrap->getWorldTransform() *
                                       tri->m_vertices1[1]);
          worldSpaceVertices.push_back(body1Wrap->getWorldTransform() *
          ...
          if (useSatSepNormal) {
...
#endif
            {
              btVector3 uniqueEdges[3] = {
                  tri->m_vertices1[1] - tri->m_vertices1[0],
...

              {
                ...
                for (int v = 0; v < combinedFaceA.m_indices.size(); v++) {
                  btScalar eq =
                      tri->m_vertices1[combinedFaceA.m_indices[v]].dot(
                          faceNormal);
                  if (planeEq > eq) {
                    planeEq = eq;
                  }
              ...
              }
              {
                ...
                for (int v = 0; v < combinedFaceB.m_indices.size(); v++) {
                  btScalar eq =
                      tri->m_vertices1[combinedFaceB.m_indices[v]].dot(
                          faceNormal);
                  if (planeEq > eq) {
                    planeEq = eq;
                  }
            ...
            if (dummy.m_hasContact && dummy.m_depth < 0) {
              ...
              sepNormalWorldSpace.setValue(0, 0,
                                           1);  // = dummy.m_normalOnBInWorld;
          ...
          if (foundSepAxis) {
            ...
            btPolyhedralContactClipping::clipFaceAgainstHull(
                sepNormalWorldSpace, *polyhedronA->getConvexPolyhedron(),
                body0Wrap->getWorldTransform(), worldSpaceVertices,
                worldVertsB2, minDist - threshold, maxDist, *resultOut);
          }
    ...
    // now perform 'm_numPerturbationIterations' collision queries with the
    // perturbated collision objects

    // perform perturbation when more then
    // 'm_minimumPointsPerturbationThreshold' points
      ...
      if (l2 > SIMD_EPSILON) {
        ...
        for (i = 0; i < m_numPerturbationIterations; i++) {
          if (v0.length2() > SIMD_EPSILON) {
            ...
            btScalar iterationAngle =
                i * (SIMD_2_PI / btScalar(m_numPerturbationIterations));
            btQuaternion rotq(sepNormalWorldSpace, iterationAngle);

            if (perturbeA) {
              input.m_transformA.setBasis(
                  btMatrix3x3(rotq.inverse() * perturbeRot * rotq) *
            ...
            btPerturbedContactResult perturbedResultOut(
                resultOut, input.m_transformA, input.m_transformB,
                unPerturbedTransform, perturbeA, dispatchInfo.m_debugDraw);
...
#ifdef USE_SEPDISTANCE_UTIL2
    if (dispatchInfo.m_useConvexConservativeDistanceUtil &&
        (sepDist > SIMD_EPSILON)) {
      m_sepDistance.initSeparatingDistance(
          gjkPairDetector.getCachedSeparatingAxis(), sepDist,
...
btScalar btConvexConvexAlgorithm::calculateTimeOfImpact(
    btCollisionObject* col0, btCollisionObject* col1,
    const btDispatcherInfo& dispatchInfo, btManifoldResult* resultOut) {
  (void)resultOut;
  (void)dispatchInfo;
  /// Rather then checking ALL pairs, only calculate TOI when motion exceeds
  /// threshold

  /// Linear motion for one of objects needs to exceed
  /// m_ccdSquareMotionThreshold
  /// col0->m_worldTransform,
  btScalar resultFraction = btScalar(1.);

  btScalar squareMot0 = (col0->getInterpolationWorldTransform().getOrigin() -
                         col0->getWorldTransform().getOrigin())
                            .length2();
  btScalar squareMot1 = (col1->getInterpolationWorldTransform().getOrigin() -
                         col1->getWorldTransform().getOrigin())
                            .length2();
  ...
  if (disableCcd) return btScalar(1.);

  // An adhoc way of testing the Continuous Collision Detection algorithms
  // One object is approximated as a sphere, to simplify things
  // Starting in penetration should report no time of impact
  // For proper CCD, better accuracy and handling of 'allowed' penetration
  // should be added
  // also the mainloop of the physics should have a kind of toi queue (something
  // like Brian Mirtich's application of Timewarp for Rigidbodies)

  /// Convex0 against sphere for Convex1
  {
    btConvexShape* convex0 =
        static_cast<btConvexShape*>(col0->getCollisionShape());

    btSphereShape sphere1(col1->getCcdSweptSphereRadius());  // todo: allow
                                                             // non-zero sphere
                                                             // sizes, for
                                                             // better
                                                             // approximation
  ...
  /// Sphere (for convex0) against Convex1
  {
    btConvexShape* convex1 =
        static_cast<btConvexShape*>(col1->getCollisionShape());

    btSphereShape sphere0(col0->getCcdSweptSphereRadius());  // todo: allow
                                                             // non-zero sphere
                                                             // sizes, for
                                                             // better
                                                             // approximation
