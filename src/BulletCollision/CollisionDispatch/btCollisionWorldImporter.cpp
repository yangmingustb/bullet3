...
btCollisionWorldImporter::btCollisionWorldImporter(btCollisionWorld* world)
    : m_collisionWorld(world), m_verboseMode(0) {}
...
bool btCollisionWorldImporter::convertAllObjects(
  ...
  for (i = 0; i < arrays->m_collisionObjectDataDouble.size(); i++) {
    btCollisionObjectDoubleData* colObjData =
        arrays->m_collisionObjectDataDouble[i];
    ...
    if (shapePtr && *shapePtr) {
      ...
      btCollisionObject* body =
          createCollisionObject(startTransform, shape, colObjData->m_name);
  ...
  for (i = 0; i < arrays->m_collisionObjectDataFloat.size(); i++) {
    btCollisionObjectFloatData* colObjData =
        arrays->m_collisionObjectDataFloat[i];
    btCollisionShape** shapePtr = m_shapeMap.find(colObjData->m_collisionShape);
    if (shapePtr && *shapePtr) {
      btTransform startTransform;
      colObjData->m_worldTransform.m_origin.m_floats[3] = 0.f;
...
btCollisionShape* btCollisionWorldImporter::convertCollisionShape(
  ...
  switch (shapeData->m_shapeType) {
    ...
    case COMPOUND_SHAPE_PROXYTYPE: {
      ...
      // &compoundData->m_childShapePtr[0];
...
btTriangleIndexVertexArray* btCollisionWorldImporter::createMeshInterface(
...
const btTransform& startTransform,btCollisionShape* shape,const char* bodyName)
{
...
btCollisionObject* btCollisionWorldImporter::getCollisionObjectByName(
    const char* name) {
...
btCollisionObject* btCollisionWorldImporter::createCollisionObject(
...
btCollisionShape* btCollisionWorldImporter::createPlaneShape(
...
btCollisionShape* btCollisionWorldImporter::createBoxShape(
...
btCollisionShape* btCollisionWorldImporter::createCapsuleShapeX(
...
btCollisionShape* btCollisionWorldImporter::createCapsuleShapeY(
    btScalar radius, btScalar height) {
...
btCollisionShape* btCollisionWorldImporter::createCapsuleShapeZ(
    btScalar radius, btScalar height) {
...
btCollisionShape* btCollisionWorldImporter::createCylinderShapeX(
    btScalar radius, btScalar height) {
...
btCollisionShape* btCollisionWorldImporter::createCylinderShapeY(
    btScalar radius, btScalar height) {
...
btCollisionShape* btCollisionWorldImporter::createCylinderShapeZ(
    btScalar radius, btScalar height) {
...
btBvhTriangleMeshShape* btCollisionWorldImporter::createBvhTriangleMeshShape(
...
btCollisionShape* btCollisionWorldImporter::createConvexTriangleMeshShape(
    btStridingMeshInterface* trimesh) {
  return 0;
}
#ifdef SUPPORT_GIMPACT_SHAPE_IMPORT
btGImpactMeshShape* btCollisionWorldImporter::createGimpactShape(
    btStridingMeshInterface* trimesh) {
...
btMultiSphereShape* btCollisionWorldImporter::createMultiSphereShape(
...
btCollisionShape* btCollisionWorldImporter::getCollisionShapeByIndex(
    int index) {
  return m_allocatedCollisionShapes[index];
}

btCollisionShape* btCollisionWorldImporter::getCollisionShapeByName(
    const char* name) {
...
btCollisionObject* btCollisionWorldImporter::getRigidBodyByIndex(
    int index) const {
...
btTriangleInfoMap* btCollisionWorldImporter::getTriangleInfoMapByIndex(
    int index) const {
