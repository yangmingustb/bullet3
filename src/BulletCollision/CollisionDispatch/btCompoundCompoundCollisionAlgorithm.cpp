...
// USE_LOCAL_STACK will avoid most (often all) dynamic memory allocations due to
// resizing in processCollision and MycollideTT
...
btCompoundCompoundCollisionAlgorithm::btCompoundCompoundCollisionAlgorithm(
  ...
  const btCompoundShape* compoundShape0 =
      static_cast<const btCompoundShape*>(col0ObjWrap->getCollisionShape());
  m_compoundShapeRevision0 = compoundShape0->getUpdateRevision();

  const btCompoundShape* compoundShape1 =
      static_cast<const btCompoundShape*>(col1ObjWrap->getCollisionShape());
...
void btCompoundCompoundCollisionAlgorithm::getAllContactManifolds(
    btManifoldArray& manifoldArray) {
  int i;
  btSimplePairArray& pairs =
      m_childCollisionAlgorithmCache->getOverlappingPairArray();
  for (i = 0; i < pairs.size(); i++) {
    if (pairs[i].m_userPointer) {
      ((btCollisionAlgorithm*)pairs[i].m_userPointer)
...
struct btCompoundCompoundLeafCallback : btDbvt::ICollide {
  ...
  btCompoundCompoundLeafCallback(
      const btCollisionObjectWrapper* compound1ObjWrap,
      const btCollisionObjectWrapper* compound0ObjWrap,
      btDispatcher* dispatcher, const btDispatcherInfo& dispatchInfo,
  ...
  void Process(const btDbvtNode* leaf0, const btDbvtNode* leaf1) {
    ...
    const btCompoundShape* compoundShape0 = static_cast<const btCompoundShape*>(
        m_compound0ColObjWrap->getCollisionShape());
    btAssert(childIndex0 < compoundShape0->getNumChildShapes());

    const btCompoundShape* compoundShape1 = static_cast<const btCompoundShape*>(
        m_compound1ColObjWrap->getCollisionShape());
    btAssert(childIndex1 < compoundShape1->getNumChildShapes());

    const btCollisionShape* childShape0 =
        compoundShape0->getChildShape(childIndex0);
    const btCollisionShape* childShape1 =
        compoundShape1->getChildShape(childIndex1);

    // backup
    btTransform orgTrans0 = m_compound0ColObjWrap->getWorldTransform();
    const btTransform& childTrans0 =
        compoundShape0->getChildTransform(childIndex0);
    btTransform newChildWorldTrans0 = orgTrans0 * childTrans0;

    btTransform orgTrans1 = m_compound1ColObjWrap->getWorldTransform();
    const btTransform& childTrans1 =
        compoundShape1->getChildTransform(childIndex1);
    ...
    if (TestAabbAgainstAabb2(aabbMin0, aabbMax0, aabbMin1, aabbMax1)) {
      btCollisionObjectWrapper compoundWrap0(
          this->m_compound0ColObjWrap, childShape0,
          m_compound0ColObjWrap->getCollisionObject(), newChildWorldTrans0, -1,
          childIndex0);
      btCollisionObjectWrapper compoundWrap1(
          this->m_compound1ColObjWrap, childShape1,
          m_compound1ColObjWrap->getCollisionObject(), newChildWorldTrans1, -1,
          childIndex1);
      ...
      if (m_resultOut->m_closestPointDistanceThreshold > 0) {
        colAlgo = m_dispatcher->findAlgorithm(&compoundWrap0, &compoundWrap1, 0,
        ...
        if (pair) {
          ...
          pair = m_childCollisionAlgorithmCache->addOverlappingPair(
...
static inline void MycollideTT(const btDbvtNode* root0, const btDbvtNode* root1,
  ...
  if (root0 && root1) {
    ...
    stkStack.initializeFromBuffer(&localStack, btDbvt::DOUBLE_STACKSIZE,
...
void btCompoundCompoundCollisionAlgorithm::processCollision(
    const btCollisionObjectWrapper* body0Wrap,
  ...
  if (!tree0 || !tree1) {
    return btCompoundCollisionAlgorithm::processCollision(
        body0Wrap, body1Wrap, dispatchInfo, resultOut);
  }
  ...
  /// btCompoundShape can nested more then 1 level deep
  ...
  btCompoundCompoundLeafCallback callback(
      col0ObjWrap, col1ObjWrap, this->m_dispatcher, dispatchInfo, resultOut,
      this->m_childCollisionAlgorithmCache, m_sharedManifold);

  const btTransform xform = col0ObjWrap->getWorldTransform().inverse() *
                            col1ObjWrap->getWorldTransform();
  MycollideTT(tree0->m_root, tree1->m_root, xform, &callback,
              resultOut->m_closestPointDistanceThreshold);

  // printf("#compound-compound child/leaf overlap =%d
    ...
    for (i = 0; i < pairs.size(); i++) {
      if (pairs[i].m_userPointer) {
...

        {
          ...
          const btTransform& childTrans0 =
              compoundShape0->getChildTransform(pairs[i].m_indexA);
        ...
        aabbMax0 += thresholdVec;
        {
          ...
          const btTransform& childTrans1 =
              compoundShape1->getChildTransform(pairs[i].m_indexB);
        ...
        if (!TestAabbAgainstAabb2(aabbMin0, aabbMax0, aabbMin1, aabbMax1)) {
          ...
          m_removePairs.push_back(
              btSimplePair(pairs[i].m_indexA, pairs[i].m_indexB));
        }
      }
    }
    for (int i = 0; i < m_removePairs.size(); i++) {
      m_childCollisionAlgorithmCache->removeOverlappingPair(
          m_removePairs[i].m_indexA, m_removePairs[i].m_indexB);
    }
    m_removePairs.clear();
  }
}

btScalar btCompoundCompoundCollisionAlgorithm::calculateTimeOfImpact(
