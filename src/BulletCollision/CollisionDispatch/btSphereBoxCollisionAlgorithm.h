...
class btSphereBoxCollisionAlgorithm : public btActivatingCollisionAlgorithm {
 ...
 public:
  ...
  bool getSphereDistance(const btCollisionObjectWrapper* boxObjWrap,
                         btVector3& v3PointOnBox, btVector3& normal,
                         btScalar& penetrationDepth,
                         const btVector3& v3SphereCenter, btScalar fRadius,
                         btScalar maxContactDistance);

  btScalar getSpherePenetration(btVector3 const& boxHalfExtent,
  ...
  struct CreateFunc : public btCollisionAlgorithmCreateFunc {
    virtual btCollisionAlgorithm* CreateCollisionAlgorithm(
      ...
      void* mem = ci.m_dispatcher1->allocateCollisionAlgorithm(
          sizeof(btSphereBoxCollisionAlgorithm));
      if (!m_swapped) {
        return new (mem)
            btSphereBoxCollisionAlgorithm(0, ci, body0Wrap, body1Wrap, false);
      } else {
        return new (mem)
            btSphereBoxCollisionAlgorithm(0, ci, body0Wrap, body1Wrap, true);
      }
    }
  };
};
