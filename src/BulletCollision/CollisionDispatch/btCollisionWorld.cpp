...
// RECALCULATE_AABB is slower, but benefit is that you don't need to call
// 'stepSimulation'  or 'updateAabbs' before using a rayTest
//#define RECALCULATE_AABB_RAYCAST 1

// When the user doesn't provide dispatcher or broadphase, create basic versions
// (and delete them in destructor)
...
void btCollisionWorld::refreshBroadphaseProxy(
    btCollisionObject* collisionObject) {
  if (collisionObject->getBroadphaseHandle()) {
    int collisionFilterGroup =
        collisionObject->getBroadphaseHandle()->m_collisionFilterGroup;
    int collisionFilterMask =
        collisionObject->getBroadphaseHandle()->m_collisionFilterMask;

    getBroadphase()->destroyProxy(collisionObject->getBroadphaseHandle(),
                                  getDispatcher());
    ...
    collisionObject->setBroadphaseHandle(getBroadphase()->createProxy(
        minAabb, maxAabb, type, collisionObject, collisionFilterGroup,
        collisionFilterMask, m_dispatcher1));
  }
}
...
void btCollisionWorld::addCollisionObject(btCollisionObject* collisionObject,
  ...
  btAssert(m_collisionObjects.findLinearSearch(collisionObject) ==
           m_collisionObjects.size());
  btAssert(collisionObject->getWorldArrayIndex() ==
           -1);  // do not add the same object to more than one collision world
...
void btCollisionWorld::updateSingleAabb(btCollisionObject* colObj) {
  ...
  colObj->getCollisionShape()->getAabb(colObj->getWorldTransform(), minAabb,
                                       maxAabb);
  // need to increase the aabb for contact thresholds
  btVector3 contactThreshold(gContactBreakingThreshold,
  ...
  if (getDispatchInfo().m_useContinuous &&
    ...
    colObj->getCollisionShape()->getAabb(
        colObj->getInterpolationWorldTransform(), minAabb2, maxAabb2);
  ...
  if (colObj->isStaticObject() ||
      ((maxAabb - minAabb).length2() < btScalar(1e12))) {
  ...
  btDispatcher* dispatcher = getDispatcher();
  {
    ...
    if (dispatcher)
      dispatcher->dispatchAllCollisionPairs(
          m_broadphasePairCache->getOverlappingPairCache(), dispatchInfo,
          m_dispatcher1);
  }
}

void btCollisionWorld::removeCollisionObject(
    btCollisionObject* collisionObject) {
  ...
  //    btAssert(iObj >= 0 && iObj < m_collisionObjects.size()); // trying to
  //    remove an object that was never added or already removed previously?
...
void btCollisionWorld::rayTestSingle(const btTransform& rayFromTrans,
  ...
  btCollisionObjectWrapper colObWrap(0, collisionShape, collisionObject,
...
void btCollisionWorld::rayTestSingleInternal(
  ...
  if (collisionShape->isConvex()) {
    ...
    btSubsimplexConvexCast subSimplexConvexCaster(castShape, convexShape,
                                                  &simplexSolver);

    btGjkConvexCast gjkConvexCaster(castShape, convexShape, &simplexSolver);

    // btContinuousConvexCollision
    // convexCaster(castShape,convexShape,&simplexSolver,0);
...
// todo: figure out what this is about. When is rayFromTest.getBasis() not
// identity?
          ...
          btCollisionWorld::LocalRayResult localRayResult(
              collisionObjectWrap->getCollisionObject(), 0, castResult.m_normal,
    ...
    if (collisionShape->isConcave()) {
      ...
      struct BridgeTriangleRaycastCallback : public btTriangleRaycastCallback {
        ...
        virtual btScalar reportHit(const btVector3& hitNormalLocal,
                                   btScalar hitFraction, int partId,
                                   int triangleIndex) {
          ...
          btVector3 hitNormalWorld =
              m_colObjWorldTransform.getBasis() * hitNormalLocal;

          btCollisionWorld::LocalRayResult rayResult(
              m_collisionObject, &shapeInfo, hitNormalWorld, hitFraction);
      ...
      btVector3 rayFromLocal =
          worldTocollisionObject * rayFromTrans.getOrigin();
      ...
      if (collisionShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE) {
        ...
        BridgeTriangleRaycastCallback rcb(
            rayFromLocal, rayToLocal, &resultCallback,
        ...
        btScaledBvhTriangleMeshShape* scaledTriangleMesh =
            (btScaledBvhTriangleMeshShape*)collisionShape;
        btBvhTriangleMeshShape* triangleMesh =
            (btBvhTriangleMeshShape*)scaledTriangleMesh->getChildShape();
        ...
        BridgeTriangleRaycastCallback rcb(
            rayFromLocalScaled, rayToLocalScaled, &resultCallback,
            collisionObjectWrap->getCollisionObject(), triangleMesh,
            colObjWorldTransform);
        rcb.m_hitFraction = resultCallback.m_closestHitFraction;
        triangleMesh->performRaycast(&rcb, rayFromLocalScaled,
                                     rayToLocalScaled);
      } else if (((resultCallback.m_flags &
                   btTriangleRaycastCallback::
                       kF_DisableHeightfieldAccelerator) == 0) &&
        ...
        BridgeTriangleRaycastCallback rcb(
            rayFromLocal, rayToLocal, &resultCallback,
            collisionObjectWrap->getCollisionObject(), heightField,
            colObjWorldTransform);
        rcb.m_hitFraction = resultCallback.m_closestHitFraction;
        ...
        struct BridgeTriangleRaycastCallback
        ...
        concaveShape->processAllTriangles(&rcb, rayAabbMinLocal,
                                          rayAabbMaxLocal);
      }
    } else {
      ...
      if (collisionShape->isCompound()) {
        ...
        struct RayTester : btDbvt::ICollide {
          ...
          void ProcessLeaf(int i) {
            const btCollisionShape* childCollisionShape =
                m_compoundShape->getChildShape(i);
            const btTransform& childTrans =
                m_compoundShape->getChildTransform(i);
            btTransform childWorldTrans = m_colObjWorldTransform * childTrans;

            btCollisionObjectWrapper tmpOb(0, childCollisionShape,
                                           m_collisionObject, childWorldTrans,
                                           -1, i);
            // replace collision shape so that callback can determine the
            // triangle
          ...
          void Process(const btDbvtNode* leaf) { ProcessLeaf(leaf->dataAsInt); }
        };
        ...
        RayTester rayCB(collisionObjectWrap->getCollisionObject(),
                        compoundShape, colObjWorldTransform, rayFromTrans,
...
void btCollisionWorld::objectQuerySingle(
    const btConvexShape* castShape, const btTransform& convexFromTrans,
    const btTransform& convexToTrans, btCollisionObject* collisionObject,
    const btCollisionShape* collisionShape,
    const btTransform& colObjWorldTransform,
    ConvexResultCallback& resultCallback, btScalar allowedPenetration) {
  btCollisionObjectWrapper tmpOb(0, collisionShape, collisionObject,
                                 colObjWorldTransform, -1, -1);
  btCollisionWorld::objectQuerySingleInternal(
      castShape, convexFromTrans, convexToTrans, &tmpOb, resultCallback,
      allowedPenetration);
}
...
void btCollisionWorld::objectQuerySingleInternal(
  ...
  if (collisionShape->isConvex()) {
    ...
    // convexCaster3(castShape,convexShape,&simplexSolver);
      ...
      if (castResult.m_normal.length2() > btScalar(0.0001)) {
        if (castResult.m_fraction < resultCallback.m_closestHitFraction) {
          ...
          btCollisionWorld::LocalConvexResult localConvexResult(
              colObjWrap->getCollisionObject(), 0, castResult.m_normal,
              castResult.m_hitPoint, castResult.m_fraction);
    ...
    if (collisionShape->isConcave()) {
      if (collisionShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE) {
        ...
        btVector3 convexFromLocal =
            worldTocollisionObject * convexFromTrans.getOrigin();
        btVector3 convexToLocal =
            worldTocollisionObject * convexToTrans.getOrigin();
        // rotation of box in local mesh space = MeshRotation^-1 *
        // ConvexToRotation
        btTransform rotationXform = btTransform(
            worldTocollisionObject.getBasis() * convexToTrans.getBasis());

        // ConvexCast::CastResult
        struct BridgeTriangleConvexcastCallback
          ...
          BridgeTriangleConvexcastCallback(
              const btConvexShape* castShape, const btTransform& from,
              const btTransform& to,
              btCollisionWorld::ConvexResultCallback* resultCallback,
              const btCollisionObject* collisionObject,
              btTriangleMeshShape* triangleMesh,
              const btTransform& triangleToWorld)
              : btTriangleConvexcastCallback(castShape, from, to,
                                             triangleToWorld,
                                             triangleMesh->getMargin()),
                m_resultCallback(resultCallback),
                m_collisionObject(collisionObject),
                m_triangleMesh(triangleMesh) {}
          ...
          virtual btScalar reportHit(const btVector3& hitNormalLocal,
            ...
            if (hitFraction <= m_resultCallback->m_closestHitFraction) {
              btCollisionWorld::LocalConvexResult convexResult(
                  m_collisionObject, &shapeInfo, hitNormalLocal, hitPointLocal,
                  hitFraction);
        ...
        BridgeTriangleConvexcastCallback tccb(
            ...
            colObjWrap->getCollisionObject(), triangleMesh,
            colObjWorldTransform);
        ...
        if (collisionShape->getShapeType() == STATIC_PLANE_PROXYTYPE) {
          ...
          if (castPtr->calcTimeOfImpact(convexFromTrans, convexToTrans,
            ...
            if (castResult.m_normal.length2() > btScalar(0.0001)) {
              if (castResult.m_fraction < resultCallback.m_closestHitFraction) {
                ...
                bool normalInWorldSpace = true;
                resultCallback.addSingleResult(localConvexResult,
                                               normalInWorldSpace);
              }
            }
          }
          ...
          struct BridgeTriangleConvexcastCallback
            ...
            virtual btScalar reportHit(const btVector3& hitNormalLocal,
              ...
              if (hitFraction <= m_resultCallback->m_closestHitFraction) {
                btCollisionWorld::LocalConvexResult convexResult(
                    m_collisionObject, &shapeInfo, hitNormalLocal,
          ...
          BridgeTriangleConvexcastCallback tccb(
              castShape, convexFromTrans, convexToTrans, &resultCallback,
              colObjWrap->getCollisionObject(), concaveShape,
              colObjWorldTransform);
          tccb.m_hitFraction = resultCallback.m_closestHitFraction;
          tccb.m_allowedPenetration = allowedPenetration;
          btVector3 boxMinLocal, boxMaxLocal;
          ...
          concaveShape->processAllTriangles(&tccb, rayAabbMinLocal,
                                            rayAabbMaxLocal);
        }
      }
    } else {
      if (collisionShape->isCompound()) {
        struct btCompoundLeafCallback : btDbvt::ICollide {
          btCompoundLeafCallback(const btCollisionObjectWrapper* colObjWrap,
         ...
         public:
          void ProcessChild(int index, const btTransform& childTrans,
                            const btCollisionShape* childCollisionShape) {
            btTransform childWorldTrans = m_colObjWorldTransform * childTrans;
            ...
            struct LocalInfoAdder : public ConvexResultCallback {
              ...
              virtual btScalar addSingleResult(
                  btCollisionWorld::LocalConvexResult& r, bool b) {
                btCollisionWorld::LocalShapeInfo shapeInfo;
                shapeInfo.m_shapePart = -1;
                shapeInfo.m_triangleIndex = m_i;
                if (r.m_localShapeInfo == NULL) r.m_localShapeInfo = &shapeInfo;
            ...
            btCollisionObjectWrapper tmpObj(m_colObjWrap, childCollisionShape,
                                            m_colObjWrap->getCollisionObject(),
                                            childWorldTrans, -1, index);

            objectQuerySingleInternal(m_castShape, m_convexFromTrans,
                                      m_convexToTrans, &tmpObj, my_cb,
          ...
          void Process(const btDbvtNode* leaf) {
            ...
            const btCollisionShape* childCollisionShape =
                m_compoundShape->getChildShape(index);
        ...
        castShape->getAabb(colObjWorldTransform.inverse() * convexFromTrans,
                           fromLocalAabbMin, fromLocalAabbMax);
        castShape->getAabb(colObjWorldTransform.inverse() * convexToTrans,
                           toLocalAabbMin, toLocalAabbMax);

        fromLocalAabbMin.setMin(toLocalAabbMin);
        fromLocalAabbMax.setMax(toLocalAabbMax);

        btCompoundLeafCallback callback(colObjWrap, castShape, convexFromTrans,
                                        convexToTrans, allowedPenetration,
                                        compoundShape, colObjWorldTransform,
                                        resultCallback);

        const btDbvt* tree = compoundShape->getDynamicAabbTree();
        if (tree) {
          const ATTRIBUTE_ALIGNED16(btDbvtVolume) bounds =
              btDbvtVolume::FromMM(fromLocalAabbMin, fromLocalAabbMax);
...
struct btSingleRayCallback : public btBroadphaseRayCallback {
  ...
  btSingleRayCallback(const btVector3& rayFromWorld,
  ...
  virtual bool process(const btBroadphaseProxy* proxy) {
    ...
    if (m_resultCallback.m_closestHitFraction == btScalar(0.f)) return false;

    btCollisionObject* collisionObject =
        (btCollisionObject*)proxy->m_clientObject;

    // only perform raycast if filterMask matches
    if (m_resultCallback.needsCollision(
...
// btVector3 collisionObjectAabbMin,collisionObjectAabbMax;
      ...
      {
        m_world->rayTestSingle(m_rayFromTrans, m_rayToTrans, collisionObject,
...
struct btSingleSweepCallback : public btBroadphaseRayCallback {
  ...
  btSingleSweepCallback(const btConvexShape* castShape,
                        const btTransform& convexFromTrans,
                        const btTransform& convexToTrans,
                        const btCollisionWorld* world,
                        btCollisionWorld::ConvexResultCallback& resultCallback,
    ...
    btVector3 unnormalizedRayDir =
        (m_convexToTrans.getOrigin() - m_convexFromTrans.getOrigin());
  ...
  virtual bool process(const btBroadphaseProxy* proxy) {
    /// terminate further convex sweep tests, once the closestHitFraction
    /// reached zero
    if (m_resultCallback.m_closestHitFraction == btScalar(0.f)) return false;
    ...
    if (m_resultCallback.needsCollision(
      ...
      m_world->objectQuerySingle(m_castShape, m_convexFromTrans,
                                 m_convexToTrans, collisionObject,
                                 collisionObject->getCollisionShape(),
                                 collisionObject->getWorldTransform(),
...
void btCollisionWorld::convexSweepTest(const btConvexShape* castShape,
  ...
  /// aabb
  /// and for each object with ray-aabb overlap, perform an exact ray test
  /// unfortunately the implementation for rayTest and convexSweepTest
  /// duplicated, albeit practically identical
  ...
  /* Compute AABB that encompasses angular movement */
  {
    ...
    castShape->calculateTemporalAabb(R, zeroLinVel, angVel, 1.0f,
                                     castShapeAabbMin, castShapeAabbMax);
  }

#ifndef USE_BRUTEFORCE_RAYBROADPHASE

  btSingleSweepCallback convexCB(castShape, convexFromWorld, convexToWorld,
                                 this, resultCallback, allowedCcdPenetration);

  m_broadphasePairCache->rayTest(convexFromTrans.getOrigin(),
                                 convexToTrans.getOrigin(), convexCB,
                                 castShapeAabbMin, castShapeAabbMax);

#else
  /// go over all objects, and if the ray intersects their aabb + cast shape
  /// aabb,
  ...
  for (i = 0; i < m_collisionObjects.size(); i++) {
    ...
    if (resultCallback.needsCollision(collisionObject->getBroadphaseHandle())) {
      ...
      collisionObject->getCollisionShape()->getAabb(
          collisionObject->getWorldTransform(), collisionObjectAabbMin,
          collisionObjectAabbMax);
      AabbExpand(collisionObjectAabbMin, collisionObjectAabbMax,
                 castShapeAabbMin, castShapeAabbMax);
      btScalar hitLambda =
          btScalar(1.);  // could use resultCallback.m_closestHitFraction, but
                         // needs testing
      btVector3 hitNormal;
      if (btRayAabb(convexFromWorld.getOrigin(), convexToWorld.getOrigin(),
                    collisionObjectAabbMin, collisionObjectAabbMax, hitLambda,
                    hitNormal)) {
        objectQuerySingle(castShape, convexFromTrans, convexToTrans,
                          collisionObject, collisionObject->getCollisionShape(),
...
struct btBridgedManifoldResult : public btManifoldResult {
  ...
  virtual void addContactPoint(const btVector3& normalOnBInWorld,
    ...
    btVector3 localB;
    if (isSwapped) {
      localA = m_body1Wrap->getCollisionObject()->getWorldTransform().invXform(
          pointA);
      localB = m_body0Wrap->getCollisionObject()->getWorldTransform().invXform(
          pointInWorld);
    } else {
      localA = m_body0Wrap->getCollisionObject()->getWorldTransform().invXform(
          pointA);
      localB = m_body1Wrap->getCollisionObject()->getWorldTransform().invXform(
          pointInWorld);
    }
    ...
    m_resultCallback.addSingleResult(newPt, obj0Wrap, newPt.m_partId0,
                                     newPt.m_index0, obj1Wrap, newPt.m_partId1,
                                     newPt.m_index1);
  }
};
...
struct btSingleContactCallback : public btBroadphaseAabbCallback {
  ...
  btSingleContactCallback(
      btCollisionObject* collisionObject, btCollisionWorld* world,
      btCollisionWorld::ContactResultCallback& resultCallback)
      : m_collisionObject(collisionObject),
        m_world(world),
        m_resultCallback(resultCallback) {}
  ...
  virtual bool process(const btBroadphaseProxy* proxy) {
    ...
    if (m_resultCallback.needsCollision(
      ...
      btCollisionObjectWrapper ob0(
          0, m_collisionObject->getCollisionShape(), m_collisionObject,
          m_collisionObject->getWorldTransform(), -1, -1);
      btCollisionObjectWrapper ob1(
          0, collisionObject->getCollisionShape(), collisionObject,
          collisionObject->getWorldTransform(), -1, -1);

      btCollisionAlgorithm* algorithm = m_world->getDispatcher()->findAlgorithm(
          &ob0, &ob1, 0, BT_CLOSEST_POINT_ALGORITHMS);
      if (algorithm) {
        btBridgedManifoldResult contactPointResult(&ob0, &ob1,
                                                   m_resultCallback);
...
void btCollisionWorld::contactTest(btCollisionObject* colObj,
  ...
  colObj->getCollisionShape()->getAabb(colObj->getWorldTransform(), aabbMin,
                                       aabbMax);
...
/// contactTest performs a discrete collision test between two collision objects
/// and calls the resultCallback if overlap if detected.
/// it reports one or more contact points (including the one with deepest
/// penetration)
void btCollisionWorld::contactPairTest(btCollisionObject* colObjA,
                                       btCollisionObject* colObjB,
                                       ContactResultCallback& resultCallback) {
  btCollisionObjectWrapper obA(0, colObjA->getCollisionShape(), colObjA,
                               colObjA->getWorldTransform(), -1, -1);
  btCollisionObjectWrapper obB(0, colObjB->getCollisionShape(), colObjB,
                               colObjB->getWorldTransform(), -1, -1);

  btCollisionAlgorithm* algorithm = getDispatcher()->findAlgorithm(
      &obA, &obB, 0, BT_CLOSEST_POINT_ALGORITHMS);
  if (algorithm) {
    btBridgedManifoldResult contactPointResult(&obA, &obB, resultCallback);
    contactPointResult.m_closestPointDistanceThreshold =
        resultCallback.m_closestDistanceThreshold;
    // discrete collision detection query
    algorithm->processCollision(&obA, &obB, getDispatchInfo(),
                                &contactPointResult);
...
class DebugDrawcallback : public btTriangleCallback,
 ...
 public:
  DebugDrawcallback(btIDebugDraw* debugDrawer, const btTransform& worldTrans,
                    const btVector3& color)
      : m_debugDrawer(debugDrawer), m_color(color), m_worldTrans(worldTrans) {}

  virtual void internalProcessTriangleIndex(btVector3* triangle, int partId,
                                            int triangleIndex) {
...
void btCollisionWorld::debugDrawObject(const btTransform& worldTransform,
  ...
  if (getDebugDrawer() &&
      getDebugDrawer()->getDebugMode() & btIDebugDraw::DBG_DrawFrames) {
  ...
  if (shape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE) {
    ...
    switch (shape->getShapeType()) {
      case BOX_SHAPE_PROXYTYPE: {
        ...
        getDebugDrawer()->drawBox(-halfExtents, halfExtents, worldTransform,
                                  color);
        break;
      }
      ...
      case SPHERE_SHAPE_PROXYTYPE: {
        ...
        btScalar radius = sphereShape->getMargin();  // radius doesn't include
                                                     // the margin, so draw with
                                                     // margin
      ...
      case MULTI_SPHERE_SHAPE_PROXYTYPE: {
        ...
        for (int i = multiSphereShape->getSphereCount() - 1; i >= 0; i--) {
          ...
          getDebugDrawer()->drawSphere(multiSphereShape->getSphereRadius(i),
                                       worldTransform * childTransform, color);
        }

        break;
      }
      case CAPSULE_SHAPE_PROXYTYPE: {
        const btCapsuleShape* capsuleShape =
            static_cast<const btCapsuleShape*>(shape);

        btScalar radius = capsuleShape->getRadius();
        btScalar halfHeight = capsuleShape->getHalfHeight();

        int upAxis = capsuleShape->getUpAxis();
        getDebugDrawer()->drawCapsule(radius, halfHeight, upAxis,
                                      worldTransform, color);
        break;
      }
      case CONE_SHAPE_PROXYTYPE: {
        ...
        getDebugDrawer()->drawCone(radius, height, upAxis, worldTransform,
                                   color);
        break;
      }
      case CYLINDER_SHAPE_PROXYTYPE: {
        ...
        getDebugDrawer()->drawCylinder(radius, halfHeight, upAxis,
                                       worldTransform, color);
        break;
      }
      ...
      case STATIC_PLANE_PROXYTYPE: {
        ...
        getDebugDrawer()->drawPlane(planeNormal, planeConst, worldTransform,
                                    color);
        break;
      }
      default: {
          ...
          if (polyshape->getConvexPolyhedron()) {
            ...
            for (i = 0; i < poly->m_faces.size(); i++) {
              ...
              if (numVerts) {
                ...
                for (int v = 0; v < poly->m_faces[i].m_indices.size(); v++) {
                  ...
                  getDebugDrawer()->drawLine(
                      worldTransform * poly->m_vertices[lastV],
                      worldTransform * poly->m_vertices[curVert], color);
                  lastV = curVert;
                }
              }
              ...
              if (getDebugDrawer()->getDebugMode() &
                ...
                btVector3 normalColor(1, 1, 0);
                btVector3 faceNormal(poly->m_faces[i].m_plane[0],
                                     poly->m_faces[i].m_plane[1],
                                     poly->m_faces[i].m_plane[2]);
                getDebugDrawer()->drawLine(
                    worldTransform * centroid,
                    worldTransform * (centroid + faceNormal), normalColor);
              }
            }
          } else {
        ...
        if (shape->isConcave()) {
          ...
          ///@todo pass camera, for some culling? no -> we are not a graphics
          ///lib
          btVector3 aabbMax(btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT),
                            btScalar(BT_LARGE_FLOAT));
          btVector3 aabbMin(btScalar(-BT_LARGE_FLOAT),
                            btScalar(-BT_LARGE_FLOAT),
                            btScalar(-BT_LARGE_FLOAT));

          DebugDrawcallback drawCallback(getDebugDrawer(), worldTransform,
                                         color);
        ...
        if (shape->getShapeType() == CONVEX_TRIANGLEMESH_SHAPE_PROXYTYPE) {
          ...
          DebugDrawcallback drawCallback(getDebugDrawer(), worldTransform,
                                         color);
          convexMesh->getMeshInterface()->InternalProcessAllTriangles(
              &drawCallback, aabbMin, aabbMax);
        }
      }
    }
  }
...
void btCollisionWorld::debugDrawWorld() {
  if (getDebugDrawer()) {
    ...
    btIDebugDraw::DefaultColors defaultColors =
        getDebugDrawer()->getDefaultColors();
      ...
      if (getDispatcher()) {
        ...
        for (int i = 0; i < numManifolds; i++) {
          ...
          for (int j = 0; j < numContacts; j++) {
            ...
            getDebugDrawer()->drawContactPoint(
                cp.m_positionWorldOnB, cp.m_normalWorldOnB, cp.getDistance(),
                cp.getLifeTime(), defaultColors.m_contactPoint);
          }
        }
      }
    }
    ...
    if ((getDebugDrawer()->getDebugMode() &
      ...
      for (i = 0; i < m_collisionObjects.size(); i++) {
        btCollisionObject* colObj = m_collisionObjects[i];
        if ((colObj->getCollisionFlags() &
             btCollisionObject::CF_DISABLE_VISUALIZE_OBJECT) == 0) {
          ...
          if (m_debugDrawer &&
              (m_debugDrawer->getDebugMode() & btIDebugDraw::DBG_DrawAabb)) {
            btVector3 minAabb, maxAabb;
  ...
  if (serializer->getSerializationFlags() & BT_SERIALIZE_CONTACT_MANIFOLDS) {
    ...
    for (int i = 0; i < numManifolds; i++) {
      const btPersistentManifold* manifold =
          getDispatcher()->getInternalManifoldPointer()[i];
      // don't serialize empty manifolds, they just take space
      //(may have to do it anyway if it destroys determinism)
      if (manifold->getNumContacts() == 0) continue;

      btChunk* chunk =
          serializer->allocate(manifold->calculateSerializeBufferSize(), 1);
      const char* structType =
          manifold->serialize(manifold, chunk->m_oldPtr, serializer);
      serializer->finalizeChunk(chunk, structType, BT_CONTACTMANIFOLD_CODE,
                                (void*)manifold);
    }
  }
}
