...
void btSphereSphereCollisionAlgorithm::processCollision(
    const btCollisionObjectWrapper* col0Wrap,
    const btCollisionObjectWrapper* col1Wrap,
  ...
  btVector3 diff = col0Wrap->getWorldTransform().getOrigin() -
  ...
  /// btVector3 pos0 = col0->getWorldTransform().getOrigin() - radius0 *
  /// normalOnSurfaceB;
  /// point on B (worldspace)
  btVector3 pos1 =
      col1Wrap->getWorldTransform().getOrigin() + radius1 * normalOnSurfaceB;
...
btScalar btSphereSphereCollisionAlgorithm::calculateTimeOfImpact(
