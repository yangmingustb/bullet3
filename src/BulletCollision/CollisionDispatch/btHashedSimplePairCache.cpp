...
btSimplePair* btHashedSimplePairCache::findPair(int indexA, int indexB) {
  ...
  int hash = static_cast<int>(getHash(static_cast<unsigned int>(indexA),
  ...
  while (index != BT_SIMPLE_NULL_PAIR &&
         equalsPair(m_overlappingPairArray[index], indexA, indexB) == false) {
    index = m_next[index];
  }
...
void btHashedSimplePairCache::growTables() {
  ...
  if (m_hashTable.size() < newCapacity) {
    ...
    for (i = 0; i < curHashtableSize; i++) {
      ...
      int hashValue =
          static_cast<int>(getHash(static_cast<unsigned int>(indexA),
                                   static_cast<unsigned int>(indexB)) &
...
btSimplePair* btHashedSimplePairCache::internalAddPair(int indexA, int indexB) {
  int hash = static_cast<int>(
      getHash(static_cast<unsigned int>(indexA),
              static_cast<unsigned int>(indexB)) &
  ...
  if (oldCapacity < newCapacity) {
    ...
    hash = static_cast<int>(getHash(static_cast<unsigned int>(indexA),
                                    static_cast<unsigned int>(indexB)) &
                            (m_overlappingPairArray.capacity() - 1));
  }
...
void* btHashedSimplePairCache::removeOverlappingPair(int indexA, int indexB) {
  ...
  int lastHash =
      static_cast<int>(getHash(static_cast<unsigned int>(last->m_indexA),
                               static_cast<unsigned int>(last->m_indexB)) &
                       (m_overlappingPairArray.capacity() - 1));
