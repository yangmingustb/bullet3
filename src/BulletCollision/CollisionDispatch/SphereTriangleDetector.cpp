...
SphereTriangleDetector::SphereTriangleDetector(
    btSphereShape* sphere, btTriangleShape* triangle,
...
void SphereTriangleDetector::getClosestPoints(const ClosestPointInput& input,
  ...
  if (collide(sphereInTr.getOrigin(), point, normal, depth, timeOfImpact,
    ...
    if (swapResults) {
      ...
      output.addContactPoint(transformB.getBasis() * normal, transformB * point,
                             depth);
    }
  }
}
...
btScalar SegmentSqrDistance(const btVector3& from, const btVector3& to,
                            const btVector3& p, btVector3& nearest);
...
bool SphereTriangleDetector::collide(const btVector3& sphereCenter,
                                     btVector3& point, btVector3& resultNormal,
                                     btScalar& depth, btScalar& timeOfImpact,
                                     btScalar contactBreakingThreshold) {
    ...
    if (isInsideContactPlane) {
      if (facecontains(sphereCenter, vertices, normal)) {
        ...
        btScalar contactCapsuleRadiusSqr =
            radiusWithThreshold * radiusWithThreshold;
        ...
        for (int i = 0; i < m_triangle->getNumEdges(); i++) {
          ...
          btScalar distanceSqr =
              SegmentSqrDistance(pa, pb, sphereCenter, nearestOnEdge);
          if (distanceSqr < minDistSqr) {
            // Yep, we're inside a capsule, and record the capsule with smallest
            // distance
