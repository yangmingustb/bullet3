...
* The b2CollidePolygons routines are Copyright (c) 2006-2007 Erin Catto
...
/// btBox2dBox2dCollisionAlgorithm, with modified b2CollidePolygons routines
/// from the Box2D library.
/// The modifications include: switching from b2Vec to btVector3, redefinition
/// of b2Dot, b2Cross
...
btBox2dBox2dCollisionAlgorithm::btBox2dBox2dCollisionAlgorithm(
    ...
    : btActivatingCollisionAlgorithm(ci, obj0Wrap, obj1Wrap),
      ...
      m_manifoldPtr(mf) {
  if (!m_manifoldPtr &&
      m_dispatcher->needsCollision(obj0Wrap->getCollisionObject(),
                                   obj1Wrap->getCollisionObject())) {
    m_manifoldPtr = m_dispatcher->getNewManifold(
        obj0Wrap->getCollisionObject(), obj1Wrap->getCollisionObject());
    m_ownManifold = true;
  }
}
...
void b2CollidePolygons(btManifoldResult* manifold, const btBox2dShape* polyA,
                       const btTransform& xfA, const btBox2dShape* polyB,
                       const btTransform& xfB);

//#include <stdio.h>
void btBox2dBox2dCollisionAlgorithm::processCollision(
    const btCollisionObjectWrapper* body0Wrap,
    const btCollisionObjectWrapper* body1Wrap,
    const btDispatcherInfo& dispatchInfo, btManifoldResult* resultOut) {
  if (!m_manifoldPtr) return;

  const btBox2dShape* box0 =
      (const btBox2dShape*)body0Wrap->getCollisionShape();
  const btBox2dShape* box1 =
      (const btBox2dShape*)body1Wrap->getCollisionShape();

  resultOut->setPersistentManifold(m_manifoldPtr);

  b2CollidePolygons(resultOut, box0, body0Wrap->getWorldTransform(), box1,
...
btScalar btBox2dBox2dCollisionAlgorithm::calculateTimeOfImpact(
...
static btScalar EdgeSeparation(const btBox2dShape* poly1,
                               const btTransform& xf1, int edge1,
  ...
  if (count2 > 0) index = (int)normal1.minDot(vertices2, count2, minDot);
...
// poly1.
static btScalar FindMaxSeparation(int* edgeIndex, const btBox2dShape* poly1,
  ...
  btVector3 d =
      b2Mul(xf2, poly2->getCentroid()) - b2Mul(xf1, poly1->getCentroid());
  btVector3 dLocal1 = b2MulT(xf1.getBasis(), d);

  // Find edge normal on poly1 that has the largest projection onto d.
  int edge = 0;
  btScalar maxDot;
  if (count1 > 0) edge = (int)dLocal1.maxDot(normals1, count1, maxDot);
...
static void FindIncidentEdge(ClipVertex c[2], const btBox2dShape* poly1,
                             const btTransform& xf1, int edge1,
                             const btBox2dShape* poly2,
                             const btTransform& xf2) {
  const btVector3* normals1 = poly1->getNormals();
  ...
  btVector3 normal1 =
      b2MulT(xf2.getBasis(), b2Mul(xf1.getBasis(), normals1[edge1]));
