...
btGImpactMeshShapePart::btGImpactMeshShapePart(
  ...
  // (The setting of BT_THREADSAFE may differ between various cpp files, so it
  // is best to
  ...
  //  model with the btStridingMeshInterface -- lock once when the object is
  //  constructed
  //  and unlock once in the destructor.
  // The other way of locking and unlocking for each collision check in the
  // narrowphase
  // is not threadsafe.  Note these are not thread-locks, they are calls to the
  // meshInterface's
  // getLockedReadOnlyVertexIndexBase virtual function, which by default just
  // returns a couple of
  // pointers.  In theory a client could override the lock function to do all
  // sorts of
  // things like reading data from GPU memory, or decompressing data on the fly,
  // but such things
...
// moved from .h to .cpp because of conditional compilation
  ...
  TrimeshPrimitiveManager* dummymanager =
      static_cast<TrimeshPrimitiveManager*>(dummy);
  dummymanager->lock();
#endif
}
...
void btGImpactCompoundShape::calculateLocalInertia(btScalar mass,
  ...
  while (i--) {
    ...
    if (childrenHasTransform()) {
      inertia = gim_inertia_add_transformed(inertia, temp_inertia,
                                            m_childTransforms[i]);
    } else {
      inertia = gim_inertia_add_transformed(inertia, temp_inertia,
...
void btGImpactMeshShape::rayTest(
    const btVector3& rayFrom, const btVector3& rayTo,
    btCollisionWorld::RayResultCallback& resultCallback) const {}

void btGImpactMeshShapePart::processAllTrianglesRay(
    btTriangleCallback* callback, const btVector3& rayFrom,
...
void btGImpactMeshShapePart::processAllTriangles(
...
void btGImpactMeshShape::processAllTrianglesRay(btTriangleCallback* callback,
