...
struct MyCallback : public btTriangleRaycastCallback {
  ...
  MyCallback(const btVector3& from, const btVector3& to, int ignorePart,
  ...
  virtual btScalar reportHit(const btVector3& hitNormalLocal,
    ...
    if (partId != m_ignorePart || triangleIndex != m_ignoreTriangleIndex) {
      if (hitFraction < m_hitFraction) return hitFraction;
    }
...
struct MyInternalTriangleIndexCallback
  ...
  MyInternalTriangleIndexCallback(btCompoundShape* colShape,
                                  ...
                                  btScalar depth)
      : m_colShape(colShape), m_gimpactShape(meshShape), m_depth(depth) {}
...
btCompoundShape* btCreateCompoundFromGimpactShape(
    const btGImpactMeshShape* gimpactMesh, btScalar depth) {
  ...
  gimpactMesh->getMeshInterface()->InternalProcessAllTriangles(&cb, aabbMin,
                                                               aabbMax);
