...
#define VEC_NEAR_EQUAL(v1, v2)                                     \
  (GIM_NEAR_EQUAL(v1[0], v2[0]) && GIM_NEAR_EQUAL(v1[1], v2[1]) && \
   GIM_NEAR_EQUAL(v1[2], v2[2]))
...
/*! multiply matrix by scalar. Each columns is scaled by each scalar vector
 * component */
...
#define MAT_TRANSFORM_PLANE_4X4(pout, m, plane)                               \
  {                                                                           \
    ...
    pout[3] =                                                                 \
        m[0][3] * pout[0] + m[1][3] * pout[1] + m[2][3] * pout[2] + plane[3]; \
  }
...
#define COFACTOR_4X4_IJ(fac, m, i, j)                                          \
  {                                                                            \
    ...
    (fac) -=                                                                   \
        m[__ii[0]][__jj[1]] * (m[__ii[1]][__jj[0]] * m[__ii[2]][__jj[2]] -     \
                               m[__ii[1]][__jj[2]] * m[__ii[2]][__jj[0]]);     \
    (fac) +=                                                                   \
        m[__ii[0]][__jj[2]] * (m[__ii[1]][__jj[0]] * m[__ii[2]][__jj[1]] -     \
...
#define MAT_DOT_ROW(mat, vec3, rowindex)                     \
  (vec3[0] * mat[rowindex][0] + vec3[1] * mat[rowindex][1] + \
   vec3[2] * mat[rowindex][2])

//! Returns the dot product between a vec2f and the row of a matrix
#define MAT_DOT_ROW2(mat, vec2, rowindex) \
  (vec2[0] * mat[rowindex][0] + vec2[1] * mat[rowindex][1])

//! Returns the dot product between a vec4f and the row of a matrix
#define MAT_DOT_ROW4(mat, vec4, rowindex)                    \
  (vec4[0] * mat[rowindex][0] + vec4[1] * mat[rowindex][1] + \
   vec4[2] * mat[rowindex][2] + vec4[3] * mat[rowindex][3])

//! Returns the dot product between a vec3f and the col of a matrix
#define MAT_DOT_COL(mat, vec3, colindex)                     \
  (vec3[0] * mat[0][colindex] + vec3[1] * mat[1][colindex] + \
   vec3[2] * mat[2][colindex])

//! Returns the dot product between a vec2f and the col of a matrix
#define MAT_DOT_COL2(mat, vec2, colindex) \
  (vec2[0] * mat[0][colindex] + vec2[1] * mat[1][colindex])

//! Returns the dot product between a vec4f and the col of a matrix
#define MAT_DOT_COL4(mat, vec4, colindex)                    \
  (vec4[0] * mat[0][colindex] + vec4[1] * mat[1][colindex] + \
   vec4[2] * mat[2][colindex] + vec4[3] * mat[3][colindex])
