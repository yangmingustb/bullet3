...
This tells relevant info for the Bounding Box set classes, which take care of
space classification.
This class can manage Compound shapes and trimeshes, and if it is managing
trimesh then the  Hierarchy Bounding Box classes will take advantage of
primitive Vs Box overlapping tests for getting optimal results and less Per Box
compairisons.
*/
class GIM_PRIMITIVE_MANAGER_PROTOTYPE {
 public:
  virtual ~GIM_PRIMITIVE_MANAGER_PROTOTYPE() {}
  //! determines if this manager consist on only triangles, which special case
  //! will be optimized
  virtual bool is_trimesh() = 0;
  virtual GUINT get_primitive_count() = 0;
  virtual void get_primitive_box(GUINT prim_index, GIM_AABB& primbox) = 0;
  virtual void get_primitive_triangle(GUINT prim_index,
...
struct GIM_BOX_TREE_NODE {
  ...
  SIMD_FORCE_INLINE bool is_leaf_node() const { return (!m_left && !m_right); }
};
...
class GIM_BOX_TREE {
 ...
 protected:
  ...
  GUINT _calc_splitting_axis(gim_array<GIM_AABB_DATA>& primitive_boxes,
                             GUINT startIndex, GUINT endIndex);

  void _build_sub_tree(gim_array<GIM_AABB_DATA>& primitive_boxes,
                       GUINT startIndex, GUINT endIndex);

 public:
  GIM_BOX_TREE() { m_num_nodes = 0; }
  ...
  //! node count
  SIMD_FORCE_INLINE GUINT getNodeCount() const { return m_num_nodes; }
...
class GIM_BOX_TREE_TEMPLATE_SET {
 ...
 public:
  ...
  SIMD_FORCE_INLINE void setPrimitiveManager(
      const _GIM_PRIMITIVE_MANAGER_PROTOTYPE& primitive_manager) {
  ...
  SIMD_FORCE_INLINE void update() { refit(); }
  ...
  SIMD_FORCE_INLINE bool boxQuery(const GIM_AABB& box,
  ...
  //! returns the indices of the primitives in the m_primitive_manager
  SIMD_FORCE_INLINE bool boxQueryTrans(
      const GIM_AABB& box, const btTransform& transform,
      gim_array<GUINT>& collided_results) const {
  ...
  //! returns the indices of the primitives in the m_primitive_manager
  SIMD_FORCE_INLINE bool rayQuery(const btVector3& ray_dir,
  ...
  SIMD_FORCE_INLINE bool hasHierarchy() const { return true; }
  ...
  SIMD_FORCE_INLINE void getNodeTriangle(GUINT nodeindex,
                                         GIM_TRIANGLE& triangle) const {
    m_primitive_manager.get_primitive_triangle(getNodeData(nodeindex),
                                               triangle);
  }
};
...
*/
template <typename _GIM_PRIMITIVE_MANAGER_PROTOTYPE>
class GIM_BOX_TREE_SET
    : public GIM_BOX_TREE_TEMPLATE_SET<_GIM_PRIMITIVE_MANAGER_PROTOTYPE,
                                       GIM_BOX_TREE> {
 public:
};
...
class GIM_TREE_TREE_COLLIDER {
 ...
 protected:
  ...
  SIMD_FORCE_INLINE bool node_collision(GUINT node0, GUINT node1) {
    ...
    bool result =
        m_box0.overlapping_trans_cache(m_box1, trans_cache_1to0, true);
    ...
    if (t0_is_trimesh && node0_is_leaf) {
      ...
      result = m_box1.collide_triangle_exact(
          m_tri0.m_vertices[0], m_tri0.m_vertices[1], m_tri0.m_vertices[2],
          m_tri0_plane);
      ...
      result = m_box0.collide_triangle_exact(
          m_tri1.m_vertices[0], m_tri1.m_vertices[1], m_tri1.m_vertices[2],
          m_tri1_plane);
  ...
  void find_collision_pairs() {
    ...
    while (stack_collisions.size()) {
      ...
      if (node_collision(node0, node1))  // a collision is found
      {
        if (node0_is_leaf) {
          if (node1_is_leaf) {
            m_collision_pairs->push_pair(m_boxset0->getNodeData(node0),
                                         m_boxset1->getNodeData(node1));
          } else {
            // collide left
            stack_collisions.push_pair(node0,
                                       m_boxset1->getLeftNodeIndex(node1));

            // collide right
            stack_collisions.push_pair(node0,
                                       m_boxset1->getRightNodeIndex(node1));
          }
        } else {
          if (node1_is_leaf) {
            // collide left
            stack_collisions.push_pair(m_boxset0->getLeftNodeIndex(node0),
                                       node1);
            // collide right
            stack_collisions.push_pair(m_boxset0->getRightNodeIndex(node0),
                                       node1);
          } else {
