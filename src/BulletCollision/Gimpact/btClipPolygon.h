...
SIMD_FORCE_INLINE void bt_vec_blend(btVector3 &vr, const btVector3 &va,
                                    const btVector3 &vb,
...
SIMD_FORCE_INLINE void bt_plane_clip_polygon_collect(
    const btVector3 &point0, const btVector3 &point1, btScalar dist0,
    btScalar dist1, btVector3 *clipped, int &clipped_count) {
...
SIMD_FORCE_INLINE int bt_plane_clip_polygon(const btVector4 &plane,
  ...
  for (int i = 1; i < polygon_point_count; i++) {
    ...
    bt_plane_clip_polygon_collect(polygon_points[i - 1], polygon_points[i],
  ...
  bt_plane_clip_polygon_collect(polygon_points[polygon_point_count - 1],
...
SIMD_FORCE_INLINE int bt_plane_clip_triangle(
    const btVector4 &plane, const btVector3 &point0, const btVector3 &point1,
  ...
  bt_plane_clip_polygon_collect(point0, point1, olddist, dist, clipped,
                                clipped_count);
  ...
  bt_plane_clip_polygon_collect(point1, point2, olddist, dist, clipped,
                                clipped_count);
  olddist = dist;

  // RETURN TO FIRST  point0
  bt_plane_clip_polygon_collect(point2, point0, olddist, firstdist, clipped,
                                clipped_count);
