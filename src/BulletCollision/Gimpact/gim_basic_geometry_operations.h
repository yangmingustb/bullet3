...
PLANE_CLIP_SEGMENT2(const CLASS_POINT &s1, const CLASS_POINT &s2,
...
SIMD_FORCE_INLINE eLINE_PLANE_INTERSECTION_TYPE PLANE_CLIP_SEGMENT_CLOSEST(
    const CLASS_POINT &s1, const CLASS_POINT &s2, const CLASS_PLANE &plane,
    CLASS_POINT &clipped1, CLASS_POINT &clipped2) {
  eLINE_PLANE_INTERSECTION_TYPE intersection_type =
...
/*!
Intersects plane in one way only. The ray must face the plane (normals must be
in opossite directions).<br/>
...
SIMD_FORCE_INLINE bool INTERSECT_PLANES(const CLASS_PLANE &p1,
                                        const CLASS_PLANE &p2, CLASS_POINT &p,
                                        CLASS_POINT &d) {
  VEC_CROSS(d, p1, p2);
...
SIMD_FORCE_INLINE void SEGMENT_COLLISION(
    const CLASS_POINT &vA1, const CLASS_POINT &vA2, const CLASS_POINT &vB1,
    const CLASS_POINT &vB2, CLASS_POINT &vPointA, CLASS_POINT &vPointB) {
...
*/
template <typename T>
SIMD_FORCE_INLINE bool BOX_AXIS_INTERSECT(T pos, T dir, T bmin, T bmax,
                                          T &tfirst, T &tlast) {
  if (GIM_IS_ZERO(dir)) {
...
SIMD_FORCE_INLINE void SORT_3_INDICES(const T *values, GUINT *order_indices) {
  // get minimum
  order_indices[0] = values[0] < values[1] ? (values[0] < values[2] ? 0 : 2)
