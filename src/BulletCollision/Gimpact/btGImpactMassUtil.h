...
SIMD_FORCE_INLINE btVector3 gim_inertia_add_transformed(
    const btVector3& source_inertia, const btVector3& added_inertia,
    const btTransform& transform) {
  btMatrix3x3 rotatedTensor = transform.getBasis().scaled(added_inertia) *
  ...
  return btVector3(source_inertia[0] + ix, source_inertia[1] + iy,
                   source_inertia[2] + iz);
}
