...
It could be a simple sorted array without the overhead of the hash key bucked,
or could
be a formely hash table with an array of keys.
You can use switch_to_hashtable() and switch_to_sorted_array for saving space or
increase speed.
</br>

<ul>
<li> if node_size = 0, then this container becomes a simple sorted array
allocator. reserve_size is used for reserve memory in m_nodes.
When the array size reaches the size equivalent to 'min_hash_table_size', then
it becomes a hash table by calling check_for_switching_to_hashtable.
...
class gim_hash_table {
 protected:
  ...
  /// Hash table data management. The hash table has the indices to the
  /// corresponding m_nodes array
  GUINT* m_hash_table;  //!<
  GUINT m_table_size;   //!<
  GUINT m_node_size;    //!<
  ...
  //! Find the avaliable cell for the hashkey, and return an existing cell if it
...
\post reserves the memory for the hash table, an initializes all elements to
GIM_INVALID_HASH.
*/
  ...
  //! Invalidates the keys (Assigning GIM_INVALID_HASH to all) Reorders the hash
  //! keys
  inline void _rehash() {
    _invalidate_keys();
    ...
    for (GUINT i = 0; i < (GUINT)m_nodes.size(); i++) {
      ...
      if (nodekey != GIM_INVALID_HASH) {
        ...
        if (m_hash_table[index] !=
            GIM_INVALID_HASH) {  // The new index is alreade used... discard
                                 // this new incomming object, repeated key
  ...
  //! Finds an avaliable hash table cell, and resizes the table if there isn't
  //! space
...
\return the index in the array of the existing element,or GIM_INVALID_HASH if
...
*/
  inline GUINT _insert_hash_table_replace(GUINT hashkey, const T& value) {
    ...
    if (value_key != GIM_INVALID_HASH) {  // replaces the existing
  ...
  /// Sorted array data management. The hash table has the indices to the
  /// corresponding m_nodes array
  ...
  /*!
Also checks if it is needed to transform this container to a hash table, by
calling check_for_switching_to_hashtable
*/
  ...
  inline GUINT _insert_sorted(GUINT hashkey, const T& value) {
    ...
    bool found = gim_binary_search_ex(ptr, 0, last_index, result_ind, hashkey,
...
*/
  ...
  inline void set_sorted(bool value) { m_sorted = value; }

  //! Retrieves the amount of keys.
  inline GUINT size() const { return m_nodes.size(); }

  //! Retrieves the hash key.
  inline GUINT get_key(GUINT index) const { return m_nodes[index].m_key; }

  //! Retrieves the value by index
  /*!
*/
  inline T* get_value_by_index(GUINT index) { return &m_nodes[index].m_data; }

  inline const T& operator[](GUINT index) const {
    return m_nodes[index].m_data;
  }

  inline T& operator[](GUINT index) { return m_nodes[index].m_data; }
...
*/
  ...
  /*!
\return If GIM_INVALID_HASH, the object has been inserted succesfully. Else it
...
*/
  ...
  //! Insert an element into the hash, and could overrite an existing object
  //! with the same hash.
  /*!
\return If GIM_INVALID_HASH, the object has been inserted succesfully. Else it
returns the position
...
*/
  ...
  //! Insert an element into the hash,But if this container is a sorted array,
  //! this inserts it unsorted
  /*!
*/
