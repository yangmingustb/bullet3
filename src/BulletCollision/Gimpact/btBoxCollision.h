...
//	btScalar pmin = pointa[component_index0]*dir0 +
//pointa[component_index1]*dir1;
//	btScalar pmax = pointb[component_index0]*dir0 +
//pointb[component_index1]*dir1;
//	//find minmax
//	if(pmin>pmax)
//	{
...
//	}
//	//find extends
//	const btScalar rad = extend[component_index0] *
//absolute_edge[dir_index0] +
//					extend[component_index1] *
//absolute_edge[dir_index1];
//
...
//
//	return
//test_cross_edge_box(edge,absolute_edge,pointa,pointb,extend,2,1,1,2);
//}
//
//
...
//
//	return
//test_cross_edge_box(edge,absolute_edge,pointa,pointb,extend,0,2,2,0);
//}
//
...
//
//	return
//test_cross_edge_box(edge,absolute_edge,pointa,pointb,extend,1,0,0,1);
//}

#define TEST_CROSS_EDGE_BOX_MCR(edge, absolute_edge, pointa, pointb, _extend, \
                                i_dir_0, i_dir_1, i_comp_0, i_comp_1)         \
  {                                                                           \
    ...
    const btScalar rad =                                                      \
        _extend[i_comp_0] * abs_dir0 + _extend[i_comp_1] * abs_dir1;          \
    if (pmin > rad || -rad > pmax) return false;                              \
  }

#define TEST_CROSS_EDGE_BOX_X_AXIS_MCR(edge, absolute_edge, pointa, pointb,  \
                                       _extend)                              \
  {                                                                          \
    TEST_CROSS_EDGE_BOX_MCR(edge, absolute_edge, pointa, pointb, _extend, 2, \
                            1, 1, 2);                                        \
  }

#define TEST_CROSS_EDGE_BOX_Y_AXIS_MCR(edge, absolute_edge, pointa, pointb,  \
                                       _extend)                              \
  {                                                                          \
    TEST_CROSS_EDGE_BOX_MCR(edge, absolute_edge, pointa, pointb, _extend, 0, \
                            2, 2, 0);                                        \
  }

#define TEST_CROSS_EDGE_BOX_Z_AXIS_MCR(edge, absolute_edge, pointa, pointb,  \
                                       _extend)                              \
  {                                                                          \
    TEST_CROSS_EDGE_BOX_MCR(edge, absolute_edge, pointa, pointb, _extend, 1, \
                            0, 0, 1);                                        \
  }
...
SIMD_FORCE_INLINE btScalar bt_mat3_dot_col(const btMatrix3x3 &mat,
                                           ...
                                           int colindex) {
  return vec3[0] * mat[0][colindex] + vec3[1] * mat[1][colindex] +
...
BT_BOX_BOX_TRANSFORM_CACHE {
 public:
  ...
  btMatrix3x3 m_R1to0;  //!< Transforms Rotation of model1 to model 0, equal  to
                        //!R0' * R1
...
btAABB {
 public:
  ...
  SIMD_FORCE_INLINE void calc_from_triangle(
      const CLASS_POINT &V1, const CLASS_POINT &V2, const CLASS_POINT &V3) {
    m_min[0] = BT_MIN3(V1[0], V2[0], V3[0]);
    m_min[1] = BT_MIN3(V1[1], V2[1], V3[1]);
    m_min[2] = BT_MIN3(V1[2], V2[2], V3[2]);
  ...
  //! Apply a transform to an AABB
  SIMD_FORCE_INLINE void appy_transform_trans_cache(
      const BT_BOX_BOX_TRANSFORM_CACHE &trans) {
    btVector3 center = (m_max + m_min) * 0.5f;
    btVector3 extends = m_max - center;
    // Compute new center
  ...
  SIMD_FORCE_INLINE bool has_collision(const btAABB &other) const {
    if (m_min[0] > other.m_max[0] || m_max[0] < other.m_min[0] ||
        m_min[1] > other.m_max[1] || m_max[1] < other.m_min[1] ||
        m_min[2] > other.m_max[2] || m_max[2] < other.m_min[2]) {
      return false;
    }
    return true;
  }
  ...
  */
  ...
  SIMD_FORCE_INLINE void projection_interval(
      const btVector3 &direction, btScalar &vmin, btScalar &vmax) const {
    btVector3 center = (m_max + m_min) * 0.5f;
  ...
  SIMD_FORCE_INLINE eBT_PLANE_INTERSECTION_TYPE plane_classify(
  ...
  SIMD_FORCE_INLINE bool overlapping_trans_conservative(
      const btAABB &box, btTransform &trans1_to_0) const {
    btAABB tbox = box;
    tbox.appy_transform(trans1_to_0);
    return has_collision(tbox);
  }

  SIMD_FORCE_INLINE bool overlapping_trans_conservative2(
      const btAABB &box, const BT_BOX_BOX_TRANSFORM_CACHE &trans1_to_0) const {
    btAABB tbox = box;
  ...
  SIMD_FORCE_INLINE bool overlapping_trans_cache(
      const btAABB &box, const BT_BOX_BOX_TRANSFORM_CACHE &transcache,
      bool fulltest) const {
    // Taken from OPCODE
  ...
  SIMD_FORCE_INLINE bool collide_triangle_exact(
      const btVector3 &p1, const btVector3 &p2, const btVector3 &p3,
