...
void bt_begin_gim02_q_tree_time() { g_q_tree_clock.reset(); }
...
void btQuantizedBvhTree::calc_quantization(GIM_BVH_DATA_ARRAY& primitive_boxes,
  ...
  bt_calc_quantization_parameters(m_global_bound.m_min, m_global_bound.m_max,
                                  m_bvhQuantization, global_bound.m_min,
                                  global_bound.m_max, boundMargin);
}
...
void btQuantizedBvhTree::_build_sub_tree(GIM_BVH_DATA_ARRAY& primitive_boxes,
  ...
  splitIndex =
      _sort_and_calc_splitting_index(primitive_boxes, startIndex, endIndex,
...
bool btGImpactQuantizedBvh::boxQuery(
    const btAABB& box, btAlignedObjectArray<int>& collided_results) const {
  int curIndex = 0;
  ...
  while (curIndex < numNodes) {
    ...
    bool aabbOverlap = m_box_tree.testQuantizedBoxOverlapp(
        curIndex, quantizedMin, quantizedMax);
...
SIMD_FORCE_INLINE bool _quantized_node_collision(
  ...
  return box0.overlapping_trans_cache(box1, trans_cache_1to0,
...
static void _find_quantized_collision_pairs_recursive(
    ...
    const BT_BOX_BOX_TRANSFORM_CACHE& trans_cache_1to0, int node0, int node1,
    bool complete_primitive_tests) {
  if (_quantized_node_collision(boxset0, boxset1, trans_cache_1to0, node0,
                                node1, complete_primitive_tests) == false)
    return;  // avoid colliding internal nodes

  if (boxset0->isLeafNode(node0)) {
    if (boxset1->isLeafNode(node1)) {
      // collision result
      collision_pairs->push_pair(boxset0->getNodeData(node0),
                                 boxset1->getNodeData(node1));
      return;
    } else {
      // collide left recursive

      _find_quantized_collision_pairs_recursive(
          boxset0, boxset1, collision_pairs, trans_cache_1to0, node0,
    ...
    if (boxset1->isLeafNode(node1)) {
      // collide left recursive
      _find_quantized_collision_pairs_recursive(
          boxset0, boxset1, collision_pairs, trans_cache_1to0,
...
void btGImpactQuantizedBvh::find_collision(const btGImpactQuantizedBvh* boxset0,
  ...
  _find_quantized_collision_pairs_recursive(boxset0, boxset1, &collision_pairs,
                                            trans_cache_1to0, 0, 0, true);
#ifdef TRI_COLLISION_PROFILING
