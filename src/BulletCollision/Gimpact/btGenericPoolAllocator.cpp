...
void btGenericMemoryPool::init_pool(size_t element_size, size_t element_count) {
  ...
  m_pool =
      (unsigned char *)btAlignedAlloc(m_element_size * m_max_element_count, 16);
  m_free_nodes =
      (size_t *)btAlignedAlloc(sizeof(size_t) * m_max_element_count, 16);
  m_allocated_sizes =
      (size_t *)btAlignedAlloc(sizeof(size_t) * m_max_element_count, 16);
...
btGenericMemoryPool *btGenericPoolAllocator::push_new_pool() {
  ...
  btGenericMemoryPool *newptr =
      (btGenericMemoryPool *)btAlignedAlloc(sizeof(btGenericMemoryPool), 16);
...
class GIM_STANDARD_ALLOCATOR : public btGenericPoolAllocator {
 public:
  GIM_STANDARD_ALLOCATOR()
      : btGenericPoolAllocator(BT_DEFAULT_POOL_ELEMENT_SIZE,
...
void *btPoolAlloc(size_t size) { return g_main_allocator.allocate(size); }
...
void btPoolFree(void *ptr) { g_main_allocator.freeMemory(ptr); }
