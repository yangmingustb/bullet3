...
struct GIM_TRIANGLE_CONTACT {
  ...
  GIM_TRIANGLE_CONTACT(const GIM_TRIANGLE_CONTACT& other) { copy_from(other); }

  //! classify points that are closer
  void merge_points(const btVector4& plane, btScalar margin,
                    const btVector3* points, int point_count);
};
...
class btPrimitiveTriangle {
 public:
  ...
  SIMD_FORCE_INLINE void buildTriPlane() {
    btVector3 normal =
        (m_vertices[1] - m_vertices[0]).cross(m_vertices[2] - m_vertices[0]);
    normal.normalize();
    m_plane.setValue(normal[0], normal[1], normal[2],
                     m_vertices[0].dot(normal));
  }
  ...
  */
  SIMD_FORCE_INLINE void get_edge_plane(int edge_index,
  ...
  /*!
  \pre clipped_points must have MAX_TRI_CLIPPING size, and this triangle must
  have its plane calculated.
  ...
  */
  bool find_triangle_collision_clip_method(btPrimitiveTriangle& other,
                                           GIM_TRIANGLE_CONTACT& contacts);
};

//! Helper class for colliding Bullet Triangle Shapes
/*!
This class implements a better getAabb method than the previous btTriangleShape
class
*/
class btTriangleShapeEx : public btTriangleShape {
 public:
  btTriangleShapeEx()
      : btTriangleShape(btVector3(0, 0, 0), btVector3(0, 0, 0),
                        btVector3(0, 0, 0)) {}

  btTriangleShapeEx(const btVector3& p0, const btVector3& p1,
                    const btVector3& p2)
      : btTriangleShape(p0, p1, p2) {}

  btTriangleShapeEx(const btTriangleShapeEx& other)
      : btTriangleShape(other.m_vertices1[0], other.m_vertices1[1],
                        other.m_vertices1[2]) {}
  ...
  SIMD_FORCE_INLINE void buildTriPlane(btVector4& plane) const {
    btVector3 normal = (m_vertices1[1] - m_vertices1[0])
