...
class GIM_TRIANGLE_CALCULATION_CACHE {
 public:
  ...
  SIMD_FORCE_INLINE bool compute_intervals(
      const GREAL &D0, const GREAL &D1, const GREAL &D2, const GREAL &D0D1,
      const GREAL &D0D2, GREAL &scale_edge0, GREAL &scale_edge1,
      GUINT &edge_index0, GUINT &edge_index1) {
    if (D0D1 > 0.0f) {
  ...
  /*!
  */
  SIMD_FORCE_INLINE GUINT clip_triangle(const btVector4 &tri_plane,
    ...
    clipped_count = PLANE_CLIP_POLYGON3D(edgeplane, temp_points, clipped_count,
                                         temp_points1);
    ...
    clipped_count = PLANE_CLIP_POLYGON3D(edgeplane, temp_points1, clipped_count,
                                         clip_points);
  ...
  SIMD_FORCE_INLINE void sort_isect(GREAL &isect0, GREAL &isect1, GUINT &e0,
                                    GUINT &e1, btVector3 &vec0,
                                    btVector3 &vec1) {
    if (isect1 < isect0) {
      // swap
  ...
  */
  SIMD_FORCE_INLINE GUINT cross_line_intersection_test() {
    ...
    if (!compute_intervals(du[0], du[1], du[2], du0du1, du0du2, tu_scale_e0,
                           tu_scale_e1, tu_e0, tu_e1))
      return 0;
    ...
    if (!compute_intervals(dv[0], dv[1], dv[2], dv0dv1, dv0dv2, tv_scale_e0,
                           tv_scale_e1, tv_e0, tv_e1))
      return 0;

    // proyected vertices
    btVector3 up_e0 =
        tu_vertices[tu_e0].lerp(tu_vertices[(tu_e0 + 1) % 3], tu_scale_e0);
    btVector3 up_e1 =
        tu_vertices[tu_e1].lerp(tu_vertices[(tu_e1 + 1) % 3], tu_scale_e1);

    btVector3 vp_e0 =
        tv_vertices[tv_e0].lerp(tv_vertices[(tv_e0 + 1) % 3], tv_scale_e0);
    btVector3 vp_e1 =
        tv_vertices[tv_e1].lerp(tv_vertices[(tv_e1 + 1) % 3], tv_scale_e1);
    ...
    if (midpoint_u < midpoint_v) {
      ...
      if (isect_u[1] + MIN_EDGE_EDGE_DIS <
          isect_v[0])  // calc distance between two lines instead
      {
        SEGMENT_COLLISION(tu_vertices[tu_e1], tu_vertices[(tu_e1 + 1) % 3],
                          tv_vertices[tv_e0], tv_vertices[(tv_e0 + 1) % 3],
                          closest_point_u, closest_point_v);
      ...
      if (isect_v[1] + MIN_EDGE_EDGE_DIS <
          isect_u[0])  // calc distance between two lines instead
      {
        SEGMENT_COLLISION(tu_vertices[tu_e0], tu_vertices[(tu_e0 + 1) % 3],
  ...
  SIMD_FORCE_INLINE bool triangle_collision(
      const btVector3 &u0, const btVector3 &u1, const btVector3 &u2,
      GREAL margin_u, const btVector3 &v0, const btVector3 &v1,
      const btVector3 &v2, GREAL margin_v,
...
bool GIM_TRIANGLE::collide_triangle_hard_test(
  ...
  return calc_cache.triangle_collision(
      ...
      other.m_vertices[0], other.m_vertices[1], other.m_vertices[2],
      other.m_margin, contact_data);
}
