...
class btTetrahedronShapeEx : public btBU_Simplex1to4 {
 public:
  btTetrahedronShapeEx() { m_numVertices = 4; }

  SIMD_FORCE_INLINE void setVertices(const btVector3& v0, const btVector3& v1,
...
class btGImpactShapeInterface : public btConcaveShape {
 ...
 public:
  ...
  \pre postUpdate() must be called for attemps to calculating the box set, else
  this function
          will does nothing.
  ...
  */
  ...
  //! If the Bounding box is not updated, then this class attemps to calculate
  //! it.
  /*!
\post Calls updateBound() for update the box set.
*/
  void getAabb(const btTransform& t, btVector3& aabbMin,
  ...
  virtual void postUpdate() { m_needs_update = true; }

  //! Obtains the local box, which is the global calculated box of the total of
  //! subshapes
  SIMD_FORCE_INLINE const btAABB& getLocalBox() { return m_localAABB; }

  virtual int getShapeType() const { return GIMPACT_SHAPE_PROXYTYPE; }
  ...
  */
  ...
  virtual const btVector3& getLocalScaling() const { return localScaling; }
  ...
  virtual void getBulletTriangle(int prim_index,
                                 btTriangleShapeEx& triangle) const = 0;

  virtual void getBulletTetrahedron(
      int prim_index, btTetrahedronShapeEx& tetrahedron) const = 0;
  ...
  //! if this trimesh
  SIMD_FORCE_INLINE void getPrimitiveTriangle(
      int index, btPrimitiveTriangle& triangle) const {
  ...
  /*!
*/
  virtual void getChildAabb(int child_index, const btTransform& t,
                            btVector3& aabbMin, btVector3& aabbMax) const {
    btAABB child_aabb;
  ...
  */
  ...
  It gives the triangles in local space
  */
  virtual void processAllTrianglesRay(btTriangleCallback* /*callback*/,
                                      const btVector3& /*rayFrom*/,
                                      const btVector3& /*rayTo*/) const {}

  //!@}
};

//! btGImpactCompoundShape allows to handle multiple btCollisionShape objects at
//! once
/*!
...
class btGImpactCompoundShape : public btGImpactShapeInterface {
 public:
  ...
  class CompoundPrimitiveManager : public btPrimitiveManagerBase {
   public:
    ...
    CompoundPrimitiveManager() { m_compoundShape = NULL; }

    virtual bool is_trimesh() const { return false; }
    ...
    virtual void get_primitive_box(int prim_index, btAABB& primbox) const {
      ...
      const btCollisionShape* shape =
          m_compoundShape->getChildShape(prim_index);
      shape->getAabb(prim_trans, primbox.m_min, primbox.m_max);
    }

    virtual void get_primitive_triangle(int prim_index,
 ...
 public:
  ...
  //! Gets the number of children
  virtual int getNumChildShapes() const { return m_childShapes.size(); }
...
*/
  virtual void getChildAabb(int child_index, const btTransform& t,
    ...
    if (childrenHasTransform()) {
      m_childShapes[child_index]->getAabb(t * m_childTransforms[child_index],
                                          aabbMin, aabbMax);
    } else {
  ...
  */
  ...
  //! Determines if this shape has triangles
  virtual bool needsRetrieveTriangles() const { return false; }

  //! Determines if this shape has tetrahedrons
  virtual bool needsRetrieveTetrahedrons() const { return false; }
  ...
  virtual void getBulletTetrahedron(int prim_index,
  ...
  virtual const char* getName() const { return "GImpactCompound"; }
...
//! This class manages a sub part of a mesh supplied by the
//! btStridingMeshInterface interface.
/*!
- Simply create this shape by passing the btStridingMeshInterface to the
constructor btGImpactMeshShapePart, then you must call updateBound() after
creating the mesh
- When making operations with this shape, you must call <b>lock</b> before
accessing to the trimesh primitives, and then call <b>unlock</b>
...
class btGImpactMeshShapePart : public btGImpactShapeInterface {
 public:
  ...
  /*!
  Manages the info from btStridingMeshInterface object and controls the
   ...
   public:
    ...
    void lock() {
      ...
      m_meshInterface->getLockedReadOnlyVertexIndexBase(
          &vertexbase, numverts, type, stride, &indexbase, indexstride,
          numfaces, indicestype, m_part);
    ...
    virtual bool is_trimesh() const { return true; }

    virtual int get_primitive_count() const { return (int)numfaces; }

    SIMD_FORCE_INLINE int get_vertex_count() const { return (int)numverts; }

    SIMD_FORCE_INLINE void get_indices(int face_index, unsigned int& i0,
                                       unsigned int& i1,
                                       unsigned int& i2) const {
      if (indicestype == PHY_SHORT) {
        unsigned short* s_indices =
    ...
    SIMD_FORCE_INLINE void get_vertex(unsigned int vertex_index,
    ...
    virtual void get_primitive_box(int prim_index, btAABB& primbox) const {
      ...
      primbox.calc_from_triangle_margin(
          ...
          triangle.m_vertices[2], triangle.m_margin);
    }
    ...
    SIMD_FORCE_INLINE void get_bullet_triangle(
        int prim_index, btTriangleShapeEx& triangle) const {
      unsigned int indices[3];
      get_indices(prim_index, indices[0], indices[1], indices[2]);
 ...
 public:
  ...
  //! if true, then its children must get transforms.
  virtual bool childrenHasTransform() const { return false; }
  ...
  */
  ...
  virtual const char* getName() const { return "GImpactMeshShapePart"; }
  ...
  //! Determines if this shape has triangles
  virtual bool needsRetrieveTriangles() const { return true; }
...
- Simply create this shape by passing the btStridingMeshInterface to the
constructor btGImpactMeshShape, then you must call updateBound() after creating
the mesh
...
class btGImpactMeshShape : public btGImpactShapeInterface {
 ...
 protected:
  ...
  void buildMeshParts(btStridingMeshInterface* meshInterface) {
    for (int i = 0; i < meshInterface->getNumSubParts(); ++i) {
      btGImpactMeshShapePart* newpart =
          new btGImpactMeshShapePart(meshInterface, i);
 ...
 public:
  ...
  btStridingMeshInterface* getMeshInterface() { return m_meshInterface; }

  const btStridingMeshInterface* getMeshInterface() const {
    return m_meshInterface;
  }

  int getMeshPartCount() const { return m_mesh_parts.size(); }

  btGImpactMeshShapePart* getMeshPart(int index) { return m_mesh_parts[index]; }
  ...
  //! call when reading child shapes
  virtual void lockChildShapes() const { btAssert(0); }

  virtual void unlockChildShapes() const { btAssert(0); }
...
*/
  ...
  */
  ...
  virtual const char* getName() const { return "GImpactMesh"; }
