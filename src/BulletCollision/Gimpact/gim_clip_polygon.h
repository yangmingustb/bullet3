...
SIMD_FORCE_INLINE GUINT PLANE_CLIP_POLYGON_GENERIC(
    const CLASS_PLANE& plane, const CLASS_POINT* polygon_points,
    GUINT polygon_point_count, CLASS_POINT* clipped,
  ...
  for (GUINT _i = 1; _i < polygon_point_count; _i++) {
    ...
    PLANE_CLIP_POLYGON_COLLECT(polygon_points[_i - 1], polygon_points[_i],
                               olddist, dist, clipped, clipped_count);
  ...
  PLANE_CLIP_POLYGON_COLLECT(polygon_points[polygon_point_count - 1],
                             polygon_points[0], olddist, firstdist, clipped,
                             clipped_count);
...
SIMD_FORCE_INLINE GUINT PLANE_CLIP_TRIANGLE_GENERIC(
    const CLASS_PLANE& plane, const CLASS_POINT& point0,
    const CLASS_POINT& point1, const CLASS_POINT& point2, CLASS_POINT* clipped,
    DISTANCE_PLANE_FUNC distance_func) {
  GUINT clipped_count = 0;
  ...
  PLANE_CLIP_POLYGON_COLLECT(point0, point1, olddist, dist, clipped,
                             clipped_count);
  ...
  PLANE_CLIP_POLYGON_COLLECT(point1, point2, olddist, dist, clipped,
                             clipped_count);
  olddist = dist;

  // RETURN TO FIRST  point
  PLANE_CLIP_POLYGON_COLLECT(point2, point0, olddist, firstdist, clipped,
                             clipped_count);
...
SIMD_FORCE_INLINE GUINT PLANE_CLIP_POLYGON3D(const CLASS_PLANE& plane,
                                             ...
                                             CLASS_POINT* clipped) {
  return PLANE_CLIP_POLYGON_GENERIC<CLASS_POINT, CLASS_PLANE>(
      plane, polygon_points, polygon_point_count, clipped,
...
SIMD_FORCE_INLINE GUINT PLANE_CLIP_TRIANGLE3D(const CLASS_PLANE& plane,
                                              ...
                                              CLASS_POINT* clipped) {
  return PLANE_CLIP_TRIANGLE_GENERIC<CLASS_POINT, CLASS_PLANE>(
      plane, point0, point1, point2, clipped, DISTANCE_PLANE_3D_FUNC());
}
