...
/// Functions for managing and sorting contacts resulting from a collision
/// query.
/// Contact lists must be create by calling \ref GIM_CREATE_CONTACT_LIST
/// After querys, contact lists must be destroy by calling \ref
/// GIM_DYNARRAY_DESTROY
/// Contacts can be merge for avoid duplicate results by calling \ref
/// gim_merge_contacts
class GIM_CONTACT {
 public:
  btVector3 m_point;
 ...
 public:
  ...
  GIM_CONTACT(const btVector3 &point, const btVector3 &normal, GREAL depth,
  ...
  SIMD_FORCE_INLINE GUINT calc_key_contact() const {
    GINT _coords[] = {(GINT)(m_point[0] * 1000.0f + 1.0f),
  ...
  SIMD_FORCE_INLINE void interpolate_normals(btVector3 *normals,
                                             GUINT normal_count) {
...
class gim_contact_array : public gim_array<GIM_CONTACT> {
 public:
  ...
  SIMD_FORCE_INLINE void push_contact(const btVector3 &point,
                                      const btVector3 &normal, GREAL depth,
                                      GUINT feature1, GUINT feature2) {
    push_back_mem();
  ...
  SIMD_FORCE_INLINE void push_triangle_contacts(
      const GIM_TRIANGLE_CONTACT_DATA &tricontact, GUINT feature1,
      GUINT feature2) {
  ...
  void merge_contacts(const gim_contact_array &contacts,
