...
/// The btCylinderShape class implements a cylinder shape primitive, centered
/// around the origin. Its central axis aligned with the Y axis.
/// btCylinderShapeX is aligned with the X axis and btCylinderShapeZ around the
/// Z axis.
...

{
 ...
 public:
  ...
  const btVector3& getHalfExtentsWithoutMargin() const {
    return m_implicitShapeDimensions;  // changed in Bullet 2.63: assume the
                                       // scaling and margin are included
  }
  ...
  virtual void setLocalScaling(const btVector3& scaling) {
    ...
    btVector3 unScaledImplicitShapeDimensionsWithMargin =
        implicitShapeDimensionsWithMargin / m_localScaling;

    btConvexInternalShape::setLocalScaling(scaling);

    m_implicitShapeDimensions =
        (unScaledImplicitShapeDimensionsWithMargin * m_localScaling) -
        oldMargin;
  }

  // debugging
  virtual const char* getName() const { return "CylinderY"; }
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btCylinderShape::serialize(
