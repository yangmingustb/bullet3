...
btPolyhedralConvexShape::btPolyhedralConvexShape()
    : btConvexInternalShape(), m_polyhedron(0) {}
...
void btPolyhedralConvexShape::setPolyhedralFeatures(
...
bool btPolyhedralConvexShape::initializePolyhedralFeatures(
  ...
  if (shiftVerticesByMargin) {
    ...
    conv.compute(&tmpVertices[0].getX(), sizeof(btVector3), tmpVertices.size(),
                 0.f, 0.f);
  } else {
    conv.compute(&orgVertices[0].getX(), sizeof(btVector3), orgVertices.size(),
                 0.f, 0.f);
  }
  ...
  for (int j = 0; j < conv.faces.size(); j++) {
    ...
    while (v1 != v0) {
      ...
      newEdge.normalize();
      if (numEdges < 2) edges[numEdges++] = newEdge;
    ...
    for (int v = 0; v < combinedFace.m_indices.size(); v++) {
      btScalar eq =
          m_polyhedron->m_vertices[combinedFace.m_indices[v]].dot(faceNormal);
      if (planeEq > eq) {
        planeEq = eq;
      }
    }
  ...
  for (int i = 0; i < numFaces; i++) {
    ...
    for (int v = 0; v < tmpFaces[i].m_indices.size(); v++) {
      btScalar eq = m_polyhedron->m_vertices[tmpFaces[i].m_indices[v]].dot(
          faceNormals[i]);
      if (planeEq > eq) {
        planeEq = eq;
      }
  ...
  while (todoFaces.size()) {
    ...
    for (int j = todoFaces.size() - 1; j >= 0; j--) {
      ...
      btVector3 faceNormalB(faceB.m_plane[0], faceB.m_plane[1],
                            faceB.m_plane[2]);
    ...
    if (coplanarFaceGroup.size() > 1) {
      ...
      for (int i = 0; i < coplanarFaceGroup.size(); i++) {
        ...
        for (int f = 0; f < face.m_indices.size(); f++) {
          ...
          for (int i = 0; i < orgpoints.size(); i++) {
            ...
            // ((rotatedPt-orgpoints[i]).length2()<0.0001))
            if (orgpoints[i].m_orgIndex == orgIndex) {
              found = true;
              break;
            }
          }
          if (!found) orgpoints.push_back(GrahamVector3(pt, orgIndex));
        }
      }
      ...
      for (int i = 0; i < orgpoints.size(); i++) {
        if (orgpoints[i].m_orgIndex == -1) continue;  // this is in the hull...
...
btVector3 btPolyhedralConvexShape::localGetSupportingVertexWithoutMargin(
...
void btPolyhedralConvexShape::batchedUnitVectorGetSupportingVertexWithoutMargin(
...
void btPolyhedralConvexAabbCachingShape::setLocalScaling(
...
void btPolyhedralConvexAabbCachingShape::getAabb(const btTransform& trans,
                                                 btVector3& aabbMin,
...
void btPolyhedralConvexAabbCachingShape::recalcLocalAabb() {
  ...
  btVector3 _supporting[] = {btVector3(0., 0., 0.), btVector3(0., 0., 0.),
                             btVector3(0., 0., 0.), btVector3(0., 0., 0.),
                             btVector3(0., 0., 0.), btVector3(0., 0., 0.)};

  batchedUnitVectorGetSupportingVertexWithoutMargin(_directions, _supporting,
                                                    6);
