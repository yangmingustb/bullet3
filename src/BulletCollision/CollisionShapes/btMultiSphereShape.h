...
/// The btMultiSphereShape represents the convex hull of a collection of
/// spheres. You can create special capsules or other smooth volumes.
/// It is possible to animate the spheres for deformation, but call
/// 'recalcLocalAabb' after changing any sphere position/radius
...
btMultiSphereShape : public btConvexInternalAabbCachingShape {
 ...
 public:
  ...
  int getSphereCount() const { return m_localPositionArray.size(); }

  const btVector3& getSpherePosition(int index) const {
    return m_localPositionArray[index];
  }

  btScalar getSphereRadius(int index) const { return m_radiArray[index]; }

  virtual const char* getName() const { return "MultiSphere"; }
