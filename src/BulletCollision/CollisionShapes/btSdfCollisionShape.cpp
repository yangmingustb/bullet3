...
struct btSdfCollisionShapeInternalData {
  ...
  btSdfCollisionShapeInternalData() : m_localScaling(1, 1, 1), m_margin(0) {}
};
...
btSdfCollisionShape::~btSdfCollisionShape() { delete m_data; }

void btSdfCollisionShape::getAabb(const btTransform& t, btVector3& aabbMin,
...
btScalar btSdfCollisionShape::getMargin() const { return m_data->m_margin; }
...
bool btSdfCollisionShape::queryPoint(const btVector3& ptInSDF,
                                     btScalar& distOut, btVector3& normal) {
  int field = 0;
  btVector3 grad;
  double dist;
