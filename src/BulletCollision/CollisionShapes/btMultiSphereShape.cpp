...
btMultiSphereShape::btMultiSphereShape(const btVector3* positions,
                                       const btScalar* radi, int numSpheres)
...
btVector3 btMultiSphereShape::localGetSupportingVertexWithoutMargin(
  ...
  for (int k = 0; k < numSpheres; k += 128) {
    ...
    for (long i = 0; i < inner_count; i++) {
      temp[i] = (*pos) * m_localScaling + vec * m_localScaling * (*rad) -
                vec * getMargin();
      pos++;
      rad++;
    }
...
void btMultiSphereShape::batchedUnitVectorGetSupportingVertexWithoutMargin(
...
const char* btMultiSphereShape::serialize(void* dataBuffer,
  ...
  shapeData->m_localPositionArrayPtr =
      numElem
          ? (btPositionAndRadius*)serializer->getUniquePointer(
                (void*)&m_localPositionArray[0])
          : 0;
  ...
  if (numElem) {
    ...
    serializer->finalizeChunk(chunk, "btPositionAndRadius", BT_ARRAY_CODE,
                              (void*)&m_localPositionArray[0]);
  }
