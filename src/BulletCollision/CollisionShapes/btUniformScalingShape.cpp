...
btVector3 btUniformScalingShape::localGetSupportingVertexWithoutMargin(
...
void btUniformScalingShape::batchedUnitVectorGetSupportingVertexWithoutMargin(
    const btVector3* vectors, btVector3* supportVerticesOut,
    int numVectors) const {
  m_childConvexShape->batchedUnitVectorGetSupportingVertexWithoutMargin(
...
btVector3 btUniformScalingShape::localGetSupportingVertex(
    const btVector3& vec) const {
  btVector3 tmpVertex;
...
void btUniformScalingShape::calculateLocalInertia(btScalar mass,
  ...
  /// this linear upscaling is not realistic, but we don't deal with large mass
  /// ratios...
...
void btUniformScalingShape::getAabb(const btTransform& trans,
                                    btVector3& aabbMin,
...
void btUniformScalingShape::getAabbSlow(const btTransform& t,
                                        ...
                                        btVector3& aabbMax) const {
#if 1
  btVector3 _directions[] = {btVector3(1., 0., 0.),  btVector3(0., 1., 0.),
...
void btUniformScalingShape::getPreferredPenetrationDirection(
    int index, btVector3& penetrationVector) const {
  m_childConvexShape->getPreferredPenetrationDirection(index,
                                                       penetrationVector);
}
