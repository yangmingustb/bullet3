...
/// The btStaticPlaneShape simulates an infinite non-moving (static) collision
/// plane.
...
btStaticPlaneShape : public btConcaveShape {
 ...
 public:
  ...
  const btVector3& getPlaneNormal() const { return m_planeNormal; }

  const btScalar& getPlaneConstant() const { return m_planeConstant; }
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btStaticPlaneShape::serialize(
