...
btHeightfieldTerrainShape::btHeightfieldTerrainShape(
    ...
    : m_userIndex2(-1), m_userValue3(0), m_triangleInfoMap(0) {
  initialize(heightStickWidth, heightStickLength, heightfieldData, heightScale,
             minHeight, maxHeight, upAxis, hdt, flipQuadEdges);
}

btHeightfieldTerrainShape::btHeightfieldTerrainShape(
    int heightStickWidth, int heightStickLength, const void* heightfieldData,
    btScalar maxHeight, int upAxis, bool useFloatData, bool flipQuadEdges)
    : m_userIndex2(-1), m_userValue3(0), m_triangleInfoMap(0) {
...
void btHeightfieldTerrainShape::initialize(
  ...
  btAssert(upAxis >= 0 &&
           upAxis < 3);  // && "bad upAxis--should be in range [0,2]");
  btAssert(hdt != PHY_UCHAR || hdt != PHY_FLOAT ||
           hdt != PHY_SHORT);  // && "Bad height data type enum");
...
btHeightfieldTerrainShape::~btHeightfieldTerrainShape() { clearAccelerator(); }

void btHeightfieldTerrainShape::getAabb(const btTransform& t,
                                        btVector3& aabbMin,
                                        btVector3& aabbMax) const {
  btVector3 halfExtents =
      (m_localAabbMax - m_localAabbMin) * m_localScaling * btScalar(0.5);
  ...
  switch (m_heightDataType) {
    ...
    case PHY_UCHAR: {
      unsigned char heightFieldValue =
          m_heightfieldDataUnsignedChar[(y * m_heightStickWidth) + x];
...
void btHeightfieldTerrainShape::getVertex(int x, int y,
  ...
  switch (m_upAxis) {
    case 0: {
      vertex.setValue(height - m_localOrigin.getX(),
...
/**
  basic algorithm:
    - convert input aabb to local coordinates (scale down and shift for local
  origin)
    - convert input aabb to a range of heightfield grid points (quantize)
    - iterate over all triangles in that subset of the grid
 */
void btHeightfieldTerrainShape::processAllTriangles(
    btTriangleCallback* callback, const btVector3& aabbMin,
    const btVector3& aabbMax) const {
  // scale down the input aabb's so they are in local (non-scaled) coordinates
  btVector3 localAabbMin =
      aabbMin * btVector3(1.f / m_localScaling[0], 1.f / m_localScaling[1],
                          1.f / m_localScaling[2]);
  btVector3 localAabbMax =
      aabbMax * btVector3(1.f / m_localScaling[0], 1.f / m_localScaling[1],
                          1.f / m_localScaling[2]);
  ...
  // TODO If m_vboundsGrid is available, use it to determine if we really need
  ...
  for (int j = startJ; j < endJ; j++) {
    for (int x = startX; x < endX; x++) {
      ...
      if (m_flipQuadEdges || (m_useDiamondSubdivision && !((j + x) & 1)) ||
        ...
        // second triangle
        //  getVertex(x,j,vertices[0]);//already got this vertex before, thanks
        //  to Danny Chapman
...
void btHeightfieldTerrainShape::calculateLocalInertia(
    btScalar, btVector3& inertia) const {
...
/// and executes an action on each cell intersecting the given segment, ordered
/// from begin to end.
/// Initially inspired by http://www.cse.yorku.ca/~amana/research/grid.pdf
template <typename Action_T>
void gridRaycast(Action_T& quadAction, const btVector3& beginPos,
                 const btVector3& endPos, int indices[3]) {
  GridRaycastState rs;
  ...
  rs.maxDistanceFlat = btSqrt(rayDirectionFlatX * rayDirectionFlatX +
                              rayDirectionFlatZ * rayDirectionFlatZ);
  ...
  const btScalar paramDeltaX =
      xiStep != 0 ? 1.f / btFabs(rayDirectionFlatX) : infinite;
  const btScalar paramDeltaZ =
      ziStep != 0 ? 1.f / btFabs(rayDirectionFlatZ) : infinite;

  // pos = param * dir
  btScalar
      paramCrossX;  // At which value of `param` we will cross a x-axis lane?
  btScalar
      paramCrossZ;  // At which value of `param` we will cross a z-axis lane?
  ...
  if (xiStep != 0) {
    if (xiStep == 1) {
      ...
      paramCrossX =
          (beginPos[indices[0]] - floor(beginPos[indices[0]])) * paramDeltaX;
    }
  } else {
  ...
  if (ziStep != 0) {
    if (ziStep == 1) {
      ...
      paramCrossZ =
          (beginPos[indices[2]] - floor(beginPos[indices[2]])) * paramDeltaZ;
    }
  } else {
  ...
  if (paramCrossX == 0.0) {
    ...
    // If going backwards, we should ignore the position we would get by the
    // above flooring,
...
struct ProcessTrianglesAction {
  ...
  void exec(int x, int z) const {
    ...
    // TODO Since this is for raycasts, we could greatly benefit from an early
...
struct ProcessVBoundsAction {
  ...
  ProcessVBoundsAction(
      const btAlignedObjectArray<btHeightfieldTerrainShape::Range>& bnd,
      int* indices)
      : vbounds(bnd), m_indices(indices) {}
  void operator()(const GridRaycastState& rs) const {
    ...
    if (rs.maxDistanceFlat > 0.0001) {
      ...
      // Consider the ray vertical
      // (though we shouldn't reach this often because there is an early check
      // up-front)
      enterPos = rayBegin;
      exitPos = rayEnd;
    }
    ...
    // Note: it could be possible to have more than one grid at different
    // levels,
    // to do this there would be a branch using a pointer to another
    // ProcessVBoundsAction
  }
};

// TODO How do I interrupt the ray when there is a hit? `callback` does not
// return any result
/// Performs a raycast using a hierarchical Bresenham algorithm.
/// Does not allocate any memory by itself.
void btHeightfieldTerrainShape::performRaycast(
    btTriangleCallback* callback, const btVector3& raySource,
  ...
  if (m_vboundsGrid.size() == 0) {
    ...
    btScalar flatDistance2 = rayDiff[indices[0]] * rayDiff[indices[0]] +
                             rayDiff[indices[2]] * rayDiff[indices[2]];
    ...
    gridRaycast(processVBounds, beginPos / m_vboundsChunkSize,
                endPos / m_vboundsChunkSize, indices);
  }
}

/// Builds a grid data structure storing the min and max heights of the terrain
/// in chunks.
...
void btHeightfieldTerrainShape::clearAccelerator() { m_vboundsGrid.clear(); }