...
btConvexHullShape::btConvexHullShape(const btScalar* points, int numPoints,
                                     int stride)
...
btVector3 btConvexHullShape::localGetSupportingVertexWithoutMargin(
  ...
  // Here we take advantage of dot(a, b*c) = dot(a*b, c).  Note: This is true
  // mathematically, but not numerically.
  if (0 < m_unscaledPoints.size()) {
    btVector3 scaled = vec * m_localScaling;
    int index = (int)scaled.maxDot(
        &m_unscaledPoints[0], m_unscaledPoints.size(),
        maxDot);  // FIXME: may violate encapsulation of m_unscaledPoints
...
void btConvexHullShape::batchedUnitVectorGetSupportingVertexWithoutMargin(
  ...
  for (int j = 0; j < numVectors; j++) {
    ...
    if (0 < m_unscaledPoints.size()) {
      int i = (int)vec.maxDot(&m_unscaledPoints[0], m_unscaledPoints.size(),
                              newDot);
...
btVector3 btConvexHullShape::localGetSupportingVertex(
    const btVector3& vec) const {
...
void btConvexHullShape::optimizeConvexHull() {
  ...
  conv.compute(&m_unscaledPoints[0].getX(), sizeof(btVector3),
               m_unscaledPoints.size(), 0.f, 0.f);
...
int btConvexHullShape::getNumEdges() const { return m_unscaledPoints.size(); }
...
int btConvexHullShape::getNumPlanes() const { return 0; }
...
const char* btConvexHullShape::serialize(void* dataBuffer,
  ...
  shapeData->m_unscaledPointsDoublePtr =
      numElem
          ? (btVector3Data*)serializer->getUniquePointer(
                (void*)&m_unscaledPoints[0])
          : 0;
#else
  shapeData->m_unscaledPointsFloatPtr =
      numElem
          ? (btVector3Data*)serializer->getUniquePointer(
                (void*)&m_unscaledPoints[0])
          : 0;
  ...
  if (numElem) {
    ...
    serializer->finalizeChunk(chunk, btVector3DataName, BT_ARRAY_CODE,
                              (void*)&m_unscaledPoints[0]);
  }
...
void btConvexHullShape::project(const btTransform& trans, const btVector3& dir,
                                btScalar& minProj, btScalar& maxProj,
