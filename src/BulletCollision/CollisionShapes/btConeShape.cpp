...
btConeShape::btConeShape(btScalar radius, btScalar height)
    : btConvexInternalShape(), m_radius(radius), m_height(height) {
...
btVector3 btConeShape::coneLocalSupport(const btVector3& v) const {
  ...
  if (v[m_coneIndices[1]] > v.length() * m_sinAngle) {
    ...
    btScalar s = btSqrt(v[m_coneIndices[0]] * v[m_coneIndices[0]] +
                        v[m_coneIndices[2]] * v[m_coneIndices[2]]);
    if (s > SIMD_EPSILON) {
...
void btConeShape::setLocalScaling(const btVector3& scaling) {
  ...
  m_radius *=
      (scaling[r1] / m_localScaling[r1] + scaling[r2] / m_localScaling[r2]) / 2;
  m_sinAngle = (m_radius / btSqrt(m_radius * m_radius + m_height * m_height));
