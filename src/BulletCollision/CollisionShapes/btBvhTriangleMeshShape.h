...
/// used for fixed/non-moving objects.
...
/// meshes.
/// btBvhTriangleMeshShape has several optimizations, such as bounding volume
/// hierarchy and
/// cache friendly traversal for PlayStation 3 Cell SPU.
/// It is recommended to enable useQuantizedAabbCompression for better memory
/// usage.
/// It takes a triangle mesh as input, for example a btTriangleMesh or
...
btBvhTriangleMeshShape : public btTriangleMeshShape {
 ...
 public:
  ...
  /// optionally pass in a larger bvh aabb, used for quantization. This allows
  /// for deformations within this aabb
  btBvhTriangleMeshShape(btStridingMeshInterface * meshInterface,
                         bool useQuantizedAabbCompression,
                         const btVector3& bvhAabbMin,
                         const btVector3& bvhAabbMax, bool buildBvh = true);

  virtual ~btBvhTriangleMeshShape();

  bool getOwnsBvh() const { return m_ownsBvh; }

  void performRaycast(btTriangleCallback * callback, const btVector3& raySource,
                      const btVector3& rayTarget);
  void performConvexcast(btTriangleCallback * callback,
  ...
  /// for a fast incremental refit of parts of the tree. Note: the entire AABB
  /// of the tree will become more conservative, it never shrinks
  ...
  btOptimizedBvh* getOptimizedBvh() { return m_bvh; }
  ...
  btTriangleInfoMap* getTriangleInfoMap() { return m_triangleInfoMap; }
