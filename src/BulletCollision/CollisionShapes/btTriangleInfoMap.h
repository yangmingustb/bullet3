...
/// The btTriangleInfoMap stores edge angle information for some triangles. You
/// can compute this information yourself or using btGenerateInternalEdgeInfo.
struct btTriangleInfoMap : public btInternalTriangleInfoMap {
  btScalar m_convexEpsilon;  /// used to determine if an edge or contact normal
                             /// is convex, using the dot product
  btScalar m_planarEpsilon;  /// used to determine if a triangle edge is planar
                             /// with zero angle
  btScalar m_equalVertexThreshold;  /// used to compute connectivity: if the
                                    /// distance between two vertices is smaller
                                    /// than m_equalVertexThreshold, they are
                                    /// considered to be 'shared'
  btScalar m_edgeDistanceThreshold;  /// used to determine edge contacts: if the
                                     /// closest distance between a contact
                                     /// point and an edge is smaller than this
                                     /// distance threshold it is considered to
                                     /// "hit the edge"
  btScalar m_maxEdgeAngleThreshold;  // ignore edges that connect triangles at
                                     // an angle larger than this
                                     // m_maxEdgeAngleThreshold
  btScalar m_zeroAreaThreshold;  /// used to determine if a triangle is
                                 /// degenerate (length squared of cross product
                                 /// of 2 triangle edges < threshold)
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btTriangleInfoMap::serialize(
  ...
  tmapData->m_hashTablePtr =
      tmapData->m_hashTableSize
          ? (int*)serializer->getUniquePointer((void*)&m_hashTable[0])
          : 0;
  ...
  tmapData->m_nextPtr =
      tmapData->m_nextSize
          ? (int*)serializer->getUniquePointer((void*)&m_next[0])
          : 0;
  ...
  tmapData->m_valueArrayPtr =
      tmapData->m_numValues
          ? (btTriangleInfoData*)serializer->getUniquePointer(
                (void*)&m_valueArray[0])
          : 0;
  if (tmapData->m_valueArrayPtr) {
    ...
    serializer->finalizeChunk(chunk, "btTriangleInfoData", BT_ARRAY_CODE,
                              (void*)&m_valueArray[0]);
  }

  tmapData->m_numKeys = m_keyArray.size();
  tmapData->m_keyArrayPtr =
      tmapData->m_numKeys
          ? (int*)serializer->getUniquePointer((void*)&m_keyArray[0])
          : 0;
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE void btTriangleInfoMap::deSerialize(
    btTriangleInfoMapData& tmapData) {
  ...
  for (i = 0; i < tmapData.m_numValues; i++) {
    m_valueArray[i].m_edgeV0V1Angle =
        tmapData.m_valueArrayPtr[i].m_edgeV0V1Angle;
    m_valueArray[i].m_edgeV1V2Angle =
        tmapData.m_valueArrayPtr[i].m_edgeV1V2Angle;
    m_valueArray[i].m_edgeV2V0Angle =
        tmapData.m_valueArrayPtr[i].m_edgeV2V0Angle;
