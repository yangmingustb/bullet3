...
/// The btScaledBvhTriangleMeshShape allows to instance a scaled version of an
/// existing btBvhTriangleMeshShape.
/// Note that each btBvhTriangleMeshShape still can have its own local scaling,
/// independent from this btScaledBvhTriangleMeshShape 'localScaling'
...
btScaledBvhTriangleMeshShape : public btConcaveShape {
 ...
 public:
  ...
  btBvhTriangleMeshShape* getChildShape() { return m_bvhTriMeshShape; }
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btScaledBvhTriangleMeshShape::serialize(
    void* dataBuffer, btSerializer* serializer) const {
  btScaledTriangleMeshShapeData* scaledMeshData =
