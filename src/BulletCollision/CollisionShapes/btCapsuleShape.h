...
/// The btCapsuleShape represents a capsule around the Y axis, there is also the
/// btCapsuleShapeX aligned around the X axis and btCapsuleShapeZ around the Z
/// axis.
...
/// The btCapsuleShape is a convex hull of two spheres. The btMultiSphereShape
/// is a more general collision shape that takes the convex hull of multiple
/// sphere, so it can also represent a capsule when just using two spheres.
...
btCapsuleShape : public btConvexInternalShape {
 ...
 public:
  ...
  btScalar getHalfHeight() const { return m_implicitShapeDimensions[m_upAxis]; }

  virtual void setLocalScaling(const btVector3& scaling) {
    btVector3 unScaledImplicitShapeDimensions =
        m_implicitShapeDimensions / m_localScaling;
  ...
  SIMD_FORCE_INLINE void deSerializeFloat(struct btCapsuleShapeData *
                                          dataBuffer);
};
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btCapsuleShape::serialize(
    void* dataBuffer, btSerializer* serializer) const {
  btCapsuleShapeData* shapeData = (btCapsuleShapeData*)dataBuffer;

  btConvexInternalShape::serialize(&shapeData->m_convexInternalShapeData,
                                   serializer);
...
SIMD_FORCE_INLINE void btCapsuleShape::deSerializeFloat(
    btCapsuleShapeData* dataBuffer) {
  m_implicitShapeDimensions.deSerializeFloat(
      dataBuffer->m_convexInternalShapeData.m_implicitShapeDimensions);
  m_collisionMargin = dataBuffer->m_convexInternalShapeData.m_collisionMargin;
  m_localScaling.deSerializeFloat(
      dataBuffer->m_convexInternalShapeData.m_localScaling);
  // it is best to already pre-allocate the matching btCapsuleShape*(X/Z)
  // version to match m_upAxis
