...
struct btShapeMatrix {
  ...
  inline double& operator[](int i) { return m_vec[i]; }

  inline const double& operator[](int i) const { return m_vec[i]; }
};
...
struct btShapeGradients {
  ...
  inline btScalar& operator()(int i, int j) { return m_vec[i][j]; }
};
...
struct btMiniSDF {
  ...
  btShapeMatrix shape_function_(btVector3 const& xi,
                                btShapeGradients* gradient = 0) const;

  bool interpolate(unsigned int field_id, double& dist, btVector3 const& x,
