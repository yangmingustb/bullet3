...
bool btGjkConvexCast::calcTimeOfImpact(const btTransform& fromA,
  ...
  btGjkPairDetector gjk(m_convexA, m_convexB, m_simplexSolver,
  ...
  if (hasResult) {
    ...
    while (dist > radius) {
      ...
      if (lambda > btScalar(1.)) return false;

      if (lambda < btScalar(0.)) return false;
      ...
      input.m_transformA.getOrigin().setInterpolate3(fromA.getOrigin(),
                                                     toA.getOrigin(), lambda);
      input.m_transformB.getOrigin().setInterpolate3(fromB.getOrigin(),
