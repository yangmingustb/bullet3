...
class btTriangleRaycastCallback : public btTriangleCallback {
 public:
  ...
  enum EFlags {
    ...
    kF_KeepUnflippedNormal = 1 << 1,  // Prevents returned face normal getting
                                      // flipped when a ray hits a back-facing
                                      // triangle
    /// SubSimplexConvexCastRaytest is the default, even if kF_None is set.
    kF_UseSubSimplexConvexCastRaytest = 1 << 2,  // Uses an approximate but
                                                 // faster ray versus convex
                                                 // intersection algorithm
    kF_UseGjkConvexCastRaytest = 1 << 3,
    kF_DisableHeightfieldAccelerator =
        1 << 4,  // don't use the heightfield raycast accelerator. See
...
class btTriangleConvexcastCallback : public btTriangleCallback {
 public:
  ...
  btTriangleConvexcastCallback(const btConvexShape* convexShape,
