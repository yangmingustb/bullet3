...
btGjkPairDetector::btGjkPairDetector(
    const btConvexShape *objectA, const btConvexShape *objectB, int shapeTypeA,
    int shapeTypeB, btScalar marginA, btScalar marginB,
    btSimplexSolverInterface *simplexSolver,
    btConvexPenetrationDepthSolver *penetrationDepthSolver)
    : m_cachedSeparatingAxis(btScalar(0.), btScalar(1.), btScalar(0.)),
      m_penetrationDepthSolver(penetrationDepthSolver),
...
static void btComputeSupport(const btConvexShape *convexA,
                             ...
                             const btVector3 &dir, bool check2d,
                             btVector3 &supAworld, btVector3 &supBworld,
                             btVector3 &aMinb) {
  btVector3 separatingAxisInA = (dir)*localTransA.getBasis();
  btVector3 separatingAxisInB = (-dir) * localTransB.getBasis();

  btVector3 pInANoMargin =
      convexA->localGetSupportVertexWithoutMarginNonVirtual(separatingAxisInA);
  btVector3 qInBNoMargin =
      convexB->localGetSupportVertexWithoutMarginNonVirtual(separatingAxisInB);
...
inline int ccdEq(btScalar _a, btScalar _b) {
  ...
  ab = btFabs(_a - _b);
  if (btFabs(ab) < SIMD_EPSILON) return 1;
...
inline btScalar btVec3PointSegmentDist2(const btVector3 *P, const btVector3 *x0,
                                        const btVector3 *b,
                                        btVector3 *witness) {
  ...
  if (t < btScalar(0) || btFuzzyZero(t)) {
    ...
    if (witness) btVec3Copy(witness, x0);
  } else if (t > btScalar(1) || ccdEq(t, btScalar(1))) {
    dist = ccdVec3Dist2(b, P);
    if (witness) btVec3Copy(witness, b);
  } else {
    if (witness) {
...
btScalar btVec3PointTriDist2(const btVector3 *P, const btVector3 *x0,
  ...
  if ((btFuzzyZero(s) || s > btScalar(0)) &&
    ...
    if (dist2 < dist) {
      dist = dist2;
      if (witness) btVec3Copy(witness, &witness2);
    }
...
#ifdef __SPU__
void btGjkPairDetector::getClosestPointsNonVirtual(
    const ClosestPointInput &input, Result &output,
    class btIDebugDraw *debugDraw)
#else
void btGjkPairDetector::getClosestPointsNonVirtual(
    const ClosestPointInput &input, Result &output,
...
#endif
{
  ...
  btVector3 positionOffset =
      (localTransA.getOrigin() + localTransB.getOrigin()) * btScalar(0.5);
  ...
  m_curIter = 0;
  int gGjkMaxIter =
      1000;  // this is to catch invalid input, perhaps check for #NaN?
  ...
  // note, for large differences in shapes, use double precision build!
  {
...

    {
      ...
      btVector3 supAworld;
      btVector3 supBworld;
      btComputeSupport(m_minkowskiA, localTransA, m_minkowskiB, localTransB,
                       dir, check2d, supAworld, supBworld, lastSupV);
    ...
    if (1) {
      ...
      // while (true)
      {
        btVector3 separatingAxisInA =
            (-m_cachedSeparatingAxis) * localTransA.getBasis();
        btVector3 separatingAxisInB =
            m_cachedSeparatingAxis * localTransB.getBasis();
        ...
        if ((delta > btScalar(0.0)) &&
            (delta * delta >
             squaredDistance * input.m_maximumDistanceSquared)) {
        ...
        // exit 0: the new point is already in the simplex, or we didn't come
        // any closer
        ...
        if (m_curIter++ > gGjkMaxIter) {
          ...
          printf(
              ...
              "shapeTypeA=%i,shapeTypeB=%i\n",
              m_cachedSeparatingAxis.getX(), m_cachedSeparatingAxis.getY(),
              m_cachedSeparatingAxis.getZ(), squaredDistance,
              m_minkowskiA->getShapeType(), m_minkowskiB->getShapeType());
        ...
        // bool check = (!m_simplexSolver->fullSimplex() && squaredDistance >
        // SIMD_EPSILON * m_simplexSolver->maxVertex());
    ...
    bool catchDegeneratePenetrationCase =
        (m_catchDegeneracies && m_penetrationDepthSolver &&
         m_degenerateSimplex &&
    ...
    if ((checkPenetration && (!isValid || catchDegeneratePenetrationCase)) ||
      ...
      if (m_penetrationDepthSolver) {
        ...
        bool isValid2 = m_penetrationDepthSolver->calcPenDepth(
            *m_simplexSolver, m_minkowskiA, m_minkowskiB, localTransA,
            localTransB, m_cachedSeparatingAxis, tmpPointOnA, tmpPointOnB,
            debugDraw);
        ...
        if (m_cachedSeparatingAxis.length2()) {
          ...
          {
            /// this is another degenerate case, where the initial GJK
            /// calculation reports a degenerate case
            /// EPA reports no penetration, and the second GJK (using the
            /// supporting vector without margin)
            /// reports a valid positive distance. Use the results of the second
            /// GJK instead of failing.
            /// thanks to Jacob.Langford for the reproduction case
            /// http://code.google.com/p/bullet/issues/detail?id=250

            if (m_cachedSeparatingAxis.length2() > btScalar(0.)) {
              btScalar distance2 =
                  (tmpPointOnA - tmpPointOnB).length() - margin;
  ...
  if (isValid && ((distance < 0) ||
    ...
    if (1) {
      ...
      {
        btVector3 separatingAxisInA =
            (-normalInB) * input.m_transformA.getBasis();
