  ...
  if (oldPtr) {
    ...
    for (i = 0; i < m_cachedPoints; i++) {
      if (m_pointCache[i].m_userPersistentData == oldPtr) {
        occurance++;
        if (occurance > 1) printf("error in clearUserCache\n");
      }
    }
...
static inline btScalar calcArea4Points(const btVector3& p0, const btVector3& p1,
                                       const btVector3& p2,
                                       const btVector3& p3) {
  // It calculates possible 3 area constructed from random 4 points and returns
  // the biggest one.
...
#define KEEP_DEEPEST_POINT 1
  ...
  btScalar res0(btScalar(0.)), res1(btScalar(0.)), res2(btScalar(0.)),
      res3(btScalar(0.));
  ...
  if (gContactCalcArea3Points) {
    if (maxPenetrationIndex != 0) {
      ...
      btVector3 b0 =
          m_pointCache[3].m_localPointA - m_pointCache[2].m_localPointA;
    ...
    if (maxPenetrationIndex != 2) {
      ...
      btVector3 b2 =
          m_pointCache[3].m_localPointA - m_pointCache[1].m_localPointA;
    ...
    if (maxPenetrationIndex != 3) {
      ...
      btVector3 b3 =
          m_pointCache[2].m_localPointA - m_pointCache[1].m_localPointA;
...
int btPersistentManifold::getCacheEntry(const btManifoldPoint& newPoint) const {
  btScalar shortestDist =
      getContactBreakingThreshold() * getContactBreakingThreshold();
...
void btPersistentManifold::refreshContactPoints(const btTransform& trA,
  ...
  printf("refreshContactPoints posA = (%f,%f,%f) posB = (%f,%f,%f)\n",
         trA.getOrigin().getX(), trA.getOrigin().getY(), trA.getOrigin().getZ(),
         trB.getOrigin().getX(), trB.getOrigin().getY(),
         trB.getOrigin().getZ());
#endif  // DEBUG_PERSISTENCY
  ...
  for (i = getNumContacts() - 1; i >= 0; i--) {
    ...
    manifoldPoint.m_distance1 =
        (manifoldPoint.m_positionWorldOnA - manifoldPoint.m_positionWorldOnB)
  ...
  for (i = getNumContacts() - 1; i >= 0; i--) {
    ...
    if (!validContactDistance(manifoldPoint)) {
      ...
      projectedPoint =
          ...
          manifoldPoint.m_normalWorldOnB * manifoldPoint.m_distance1;
      ...
      if (distance2d >
        ...
        if (gContactProcessedCallback)
          (*gContactProcessedCallback)(manifoldPoint, (void*)m_body0,
                                       (void*)m_body1);
      }
    }
  }
...
const char* btPersistentManifold::serialize(
    const class btPersistentManifold* manifold, void* dataBuffer,
  ...
  dataOut->m_body0 = (btCollisionObjectData*)serializer->getUniquePointer(
      (void*)manifold->getBody0());
  dataOut->m_body1 = (btCollisionObjectData*)serializer->getUniquePointer(
      (void*)manifold->getBody1());
  ...
  for (int i = 0; i < this->getNumContacts(); i++) {
    ...
    dataOut->m_pointCacheAppliedImpulseLateral1[i] =
        pt.m_appliedImpulseLateral1;
    dataOut->m_pointCacheAppliedImpulseLateral2[i] =
    ...
    dataOut->m_pointCacheCombinedContactDamping1[i] =
        pt.m_combinedContactDamping1;
    dataOut->m_pointCacheCombinedContactStiffness1[i] =
    ...
    pt.m_lateralFrictionDir1.serialize(
        dataOut->m_pointCacheLateralFrictionDir1[i]);
    pt.m_lateralFrictionDir2.serialize(
        dataOut->m_pointCacheLateralFrictionDir2[i]);
    dataOut->m_pointCacheCombinedRollingFriction[i] =
        pt.m_combinedRollingFriction;
    dataOut->m_pointCacheCombinedSpinningFriction[i] =
...
void btPersistentManifold::deSerialize(
    const struct btPersistentManifoldDoubleData* manifoldDataPtr) {
  ...
  for (int i = 0; i < this->getNumContacts(); i++) {
    ...
    pt.m_localPointA.deSerializeDouble(
        manifoldDataPtr->m_pointCacheLocalPointA[i]);
    pt.m_localPointB.deSerializeDouble(
        manifoldDataPtr->m_pointCacheLocalPointB[i]);
    pt.m_normalWorldOnB.deSerializeDouble(
    ...
    pt.m_combinedRestitution =
        manifoldDataPtr->m_pointCacheCombinedRestitution[i];
...
void btPersistentManifold::deSerialize(
    const struct btPersistentManifoldFloatData* manifoldDataPtr) {
  m_contactBreakingThreshold = manifoldDataPtr->m_contactBreakingThreshold;
  m_contactProcessingThreshold = manifoldDataPtr->m_contactProcessingThreshold;
  m_cachedPoints = manifoldDataPtr->m_numCachedPoints;
  m_companionIdA = manifoldDataPtr->m_companionIdA;
  ...
  for (int i = 0; i < this->getNumContacts(); i++) {
    ...
    pt.m_normalWorldOnB.deSerialize(
        manifoldDataPtr->m_pointCacheNormalWorldOnB[i]);
    pt.m_distance1 = manifoldDataPtr->m_pointCacheDistance[i];
    pt.m_combinedContactDamping1 =
        manifoldDataPtr->m_pointCacheCombinedContactDamping1[i];
    ...
    pt.m_partId0 = manifoldDataPtr->m_pointCachePartId0[i];
    pt.m_partId1 = manifoldDataPtr->m_pointCachePartId1[i];
    pt.m_positionWorldOnA.deSerialize(
        manifoldDataPtr->m_pointCachePositionWorldOnA[i]);
    pt.m_positionWorldOnB.deSerialize(
        manifoldDataPtr->m_pointCachePositionWorldOnB[i]);
    pt.m_combinedFriction = manifoldDataPtr->m_pointCacheCombinedFriction[i];
    pt.m_lateralFrictionDir1.deSerialize(
        manifoldDataPtr->m_pointCacheLateralFrictionDir1[i]);
    pt.m_lateralFrictionDir2.deSerialize(
        manifoldDataPtr->m_pointCacheLateralFrictionDir2[i]);
    pt.m_combinedRollingFriction =
        manifoldDataPtr->m_pointCacheCombinedRollingFriction[i];
    pt.m_combinedSpinningFriction =
