...
bool btMinkowskiPenetrationDepthSolver::calcPenDepth(
    btSimplexSolverInterface& simplexSolver, const btConvexShape* convexA,
    const btConvexShape* convexB, const btTransform& transA,
    const btTransform& transB, btVector3& v, btVector3& pa, btVector3& pb,
  ...
  // just take fixed number of orientation, and sample the penetration depth in
  // that direction
...
#define USE_BATCHED_SUPPORT 1
  ...
  btVector3 supportVerticesBBatch[NUM_UNITSPHERE_POINTS +
                                  MAX_PREFERRED_PENETRATION_DIRECTIONS * 2];
  btVector3 separatingAxisInABatch[NUM_UNITSPHERE_POINTS +
                                   MAX_PREFERRED_PENETRATION_DIRECTIONS * 2];
  btVector3 separatingAxisInBBatch[NUM_UNITSPHERE_POINTS +
                                   MAX_PREFERRED_PENETRATION_DIRECTIONS * 2];
  int i;
...

  {
    ...
    if (numPDA) {
      for (int i = 0; i < numPDA; i++) {
        ...
        separatingAxisInABatch[numSampleDirections] =
  ...
  convexA->batchedUnitVectorGetSupportingVertexWithoutMargin(
      separatingAxisInABatch, supportVerticesABatch, numSampleDirections);
  convexB->batchedUnitVectorGetSupportingVertexWithoutMargin(
      separatingAxisInBBatch, supportVerticesBBatch, numSampleDirections);
  ...
  for (int i = 0; i < numSampleDirections; i++) {
    ...
    pInA = convexA->localGetSupportVertexWithoutMarginNonVirtual(
        separatingAxisInA);
    qInB = convexB->localGetSupportVertexWithoutMarginNonVirtual(
        separatingAxisInB);
    pWorld = transA(pInA);
    qWorld = transB(qInB);
    w = qWorld - pWorld;
  ...
  // no penetration
  if (minProj < btScalar(0.)) return false;

  btScalar extraSeparation = 0.5f;  /// scale dependent
  minProj += extraSeparation +
