...
void btVoronoiSimplexSolver::reset() {
  ...
  m_numVertices = 0;
  m_needsUpdate = true;
  m_lastW = btVector3(btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT),
                      btScalar(BT_LARGE_FLOAT));
  m_cachedBC.reset();
}

// add a vertex
void btVoronoiSimplexSolver::addVertex(const btVector3& w, const btVector3& p,
                                       const btVector3& q) {
  m_lastW = w;
  m_needsUpdate = true;
  ...
  if (m_needsUpdate) {
    ...
    switch (numVertices()) {
      ...
      case 1: {
        ...
        m_cachedBC.reset();
        m_cachedBC.setBarycentricCoordinates(btScalar(1.), btScalar(0.),
...
int btVoronoiSimplexSolver::getSimplex(btVector3* pBuf, btVector3* qBuf,
...
void btVoronoiSimplexSolver::backup_closest(btVector3& v) { v = m_cachedV; }
...
bool btVoronoiSimplexSolver::closestPtPointTriangle(
    const btVector3& p, const btVector3& a, const btVector3& b,
    const btVector3& c, btSubSimplexClosestResult& result) {
  ...
  if (va <= btScalar(0.0) && (d4 - d3) >= btScalar(0.0) &&
      (d5 - d6) >= btScalar(0.0)) {
...
bool btVoronoiSimplexSolver::closestPtPointTetrahedron(
    const btVector3& p, const btVector3& a, const btVector3& b,
