...
typedef bool (*ContactProcessedCallback)(btManifoldPoint& cp, void* body0,
                                         void* body1);
...
/// btPersistentManifold is a contact point cache, it stays persistent as long
/// as objects are overlapping in the broadphase.
/// Those contact points are created by the collision narrow phase.
/// The cache can be empty, or hold 1,2,3 or 4 points. Some collision algorithms
/// (GJK) might only add one point at a time.
...
/// following rules:
/// the contact point with deepest penetration is always kept, and it tries to
/// maximuze the area covered by the points
...
btPersistentManifold : public btTypedObject {
  ...
  int findContactPoint(const btManifoldPoint* unUsed, int numUnused,
 ...
 public:
  ...
  btPersistentManifold(
      const btCollisionObject* body0, const btCollisionObject* body1, int,
  ...
  /// the setNumContacts API is usually not used, except when you gather/fill
  /// all contacts manually
  void setNumContacts(int cachedPoints) { m_cachedPoints = cachedPoints; }
...
#define MAINTAIN_PERSISTENCY 1
    ...
    btScalar appliedLateralImpulse1 =
        m_pointCache[insertIndex].m_appliedImpulseLateral1;
    btScalar appliedLateralImpulse2 =
        m_pointCache[insertIndex].m_appliedImpulseLateral2;
    ...
    if (newPoint.m_contactPointFlags & BT_CONTACT_FLAG_FRICTION_ANCHOR) {
      ...
      btScalar eps = 0;  // we could allow to enlarge or shrink the tolerance to
                         // check against the friction cone a bit, say 1e-7
      btScalar a = appliedLateralImpulse1 * appliedLateralImpulse1 +
  ...
  const char* serialize(const class btPersistentManifold* manifold,
                        void* dataBuffer, class btSerializer* serializer) const;
  void deSerialize(
