...
bool btGjkEpaCalcPenDepth(const btConvexTemplate& a, const btConvexTemplate& b,
                          ...
                          btVector3& v, btVector3& wWitnessOnA,
                          btVector3& wWitnessOnB) {
  (void)v;

  //	const btScalar				radialmargin(btScalar(0.));

  btVector3 guessVector(
      b.getWorldTransform().getOrigin() -
      a.getWorldTransform().getOrigin());  //?? why not use the GJK input?
...
int btComputeGjkEpaPenetration(const btConvexTemplate& a,
  ...
  int m_curIter = 0;
  int gGjkMaxIter = colDesc.m_maxGjkIterations;  // this is to catch invalid
                                                 // input, perhaps check for
                                                 // #NaN?
...

  {
    ...
    // while (true)
    {
      ...
      if ((delta > btScalar(0.0)) &&
          (delta * delta >
           squaredDistance * colDesc.m_maximumDistanceSquared)) {
      ...
      // exit 0: the new point is already in the simplex, or we didn't come any
      // closer
    ...
    bool catchDegeneratePenetrationCase =
        (m_catchDegeneracies && m_degenerateSimplex &&
    ...
    if (checkPenetration && (!isValid || catchDegeneratePenetrationCase)) {
      ...
      bool isValid2 = btGjkEpaCalcPenDepth(
          a, b, colDesc, m_cachedSeparatingAxis, tmpPointOnA, tmpPointOnB);
  ...
  if (isValid && ((distance < 0) ||
                  (distance * distance < colDesc.m_maximumDistanceSquared))) {
