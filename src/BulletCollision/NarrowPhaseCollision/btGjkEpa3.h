  ...
  struct sResults {
    enum eStatus {
      ...
      Penetrating, /* Shapes are penetrating
                      */
      GJK_Failed,  /* GJK phase fail, no big issue, shapes are probably just
                      'touching'	*/
      EPA_Failed /* EPA phase fail, bigger problem, need to save parameters, and
                    debug	*/
    } status;
...
struct GJK {
  ...
  GJK(const btConvexTemplate& a, const btConvexTemplate& b) : m_shape(a, b) {
    Initialize();
  }
  void Initialize() {
  ...
  eGjkStatus Evaluate(const MinkowskiDiff<btConvexTemplate>& shapearg,
  ...
  static btScalar det(const btVector3& a, const btVector3& b,
                      const btVector3& c) {
    return (a.y() * b.z() * c.x() + a.z() * b.x() * c.y() -
            a.x() * b.z() * c.y() - a.y() * b.x() * c.z() +
            a.x() * b.y() * c.z() - a.z() * b.y() * c.x());
  }
  static btScalar projectorigin(const btVector3& a, const btVector3& b,
                                btScalar* w, U& m) {
  ...
  static btScalar projectorigin(const btVector3& a, const btVector3& b,
                                const btVector3& c, btScalar* w, U& m) {
...
struct EPA {
  ...
  eEpaStatus Evaluate(GJK<btConvexTemplate>& gjk, const btVector3& guess) {
    ...
    if ((simplex.rank > 1) && gjk.EncloseOrigin()) {
      ...
      if (m_hull.count == 4) {
        ...
        m_result.p[0] =
            btCross(outer.c[1]->w - projection, outer.c[2]->w - projection)
                .length();
        m_result.p[1] =
            btCross(outer.c[2]->w - projection, outer.c[0]->w - projection)
                .length();
        m_result.p[2] =
            btCross(outer.c[0]->w - projection, outer.c[1]->w - projection)
                .length();
  ...
  bool getedgedist(sFace* face, typename GJK<btConvexTemplate>::sSV* a,
                   typename GJK<btConvexTemplate>::sSV* b, btScalar& dist) {
    const btVector3 ba = b->w - a->w;
    const btVector3 n_ab = btCross(
        ba,
        face->n);  // Outward facing edge normal direction, on triangle plane
    const btScalar a_dot_nab =
        btDot(a->w, n_ab);  // Only care about the sign to determine
    ...
    if (a_dot_nab < 0) {
      ...
      if (a_dot_ba > 0) {
        ...
        dist = btSqrt(
            btMax((a->w.length2() * b->w.length2() - a_dot_b * a_dot_b) / ba_l2,
                  (btScalar)0));
      }
  ...
  sFace* newface(typename GJK<btConvexTemplate>::sSV* a,
                 typename GJK<btConvexTemplate>::sSV* b,
                 typename GJK<btConvexTemplate>::sSV* c, bool forced) {
    if (m_stock.root) {
  ...
  bool expand(U pass, typename GJK<btConvexTemplate>::sSV* w, sFace* f, U e,
              sHorizon& horizon) {
...
static void Initialize(const btConvexTemplate& a, const btConvexTemplate& b,
  ...
  shape.m_toshape1 = b.getWorldTransform().getBasis().transposeTimes(
...
bool btGjkEpaSolver3_Distance(const btConvexTemplate& a,
                              const btConvexTemplate& b, const btVector3& guess,
                              btGjkEpaSolver3::sResults& results) {
  ...
  switch (gjk_status) {
    case eGjkInside: {
      ...
      if (epa_status != eEpaFailed) {
        ...
        results.witnesses[0] = a.getWorldTransform() * w0;
        results.witnesses[1] =
            a.getWorldTransform() * (w0 - epa.m_normal * epa.m_depth);
...
int btComputeGjkDistance(const btConvexTemplate& a, const btConvexTemplate& b,
  ...
  bool isSeparated = btGjkEpaSolver3_Distance(a, b, guess, results);
  if (isSeparated) {
    distInfo->m_distance = results.distance;
    distInfo->m_pointOnA = results.witnesses[0];
    distInfo->m_pointOnB = results.witnesses[1];
