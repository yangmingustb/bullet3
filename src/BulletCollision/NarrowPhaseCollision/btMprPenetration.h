...
inline void btPrintPortalVertex(_btMprSimplex_t *portal, int index) {
  printf("portal[%d].v = %f,%f,%f, v1=%f,%f,%f, v2=%f,%f,%f\n", index,
         portal->ps[index].v.x(), portal->ps[index].v.y(),
         portal->ps[index].v.z(), portal->ps[index].v1.x(),
         portal->ps[index].v1.y(), portal->ps[index].v1.z(),
         portal->ps[index].v2.x(), portal->ps[index].v2.y(),
         portal->ps[index].v2.z());
}
#endif  // DEBUG_MPR

inline int btMprSimplexSize(const btMprSimplex_t *s) { return s->last + 1; }

inline const btMprSupport_t *btMprSimplexPoint(const btMprSimplex_t *s,
                                               int idx) {
...
inline void btMprSimplexSet(btMprSimplex_t *s, size_t pos,
...
inline int btMprIsZero(float val) { return BT_MPR_FABS(val) < FLT_EPSILON; }
...
inline int btMprEq(float _a, float _b) {
  ...
  if (BT_MPR_FABS(ab) < FLT_EPSILON) return 1;
...
inline int btMprVec3Eq(const btVector3 *a, const btVector3 *b) {
  return btMprEq((*a).x(), (*b).x()) && btMprEq((*a).y(), (*b).y()) &&
         btMprEq((*a).z(), (*b).z());
}

template <typename btConvexTemplate>
inline void btFindOrigin(const btConvexTemplate &a, const btConvexTemplate &b,
...
inline void btMprVec3Add(btVector3 *v, const btVector3 *w) { *v += *w; }

inline void btMprVec3Copy(btVector3 *v, const btVector3 *w) { *v = *w; }

inline void btMprVec3Scale(btVector3 *d, float k) { *d *= k; }
...
inline float btMprVec3Len2(const btVector3 *v) { return btMprVec3Dot(v, v); }
...
inline void btMprVec3Cross(btVector3 *d, const btVector3 *a,
                           const btVector3 *b) {
  *d = btCross(*a, *b);
}

inline void btMprVec3Sub2(btVector3 *d, const btVector3 *v,
                          const btVector3 *w) {
  *d = *v - *w;
}
...
inline void btExpandPortal(btMprSimplex_t *portal, const btMprSupport_t *v4) {
  float dot;
  btVector3 v4v0;
...
static int btDiscoverPortal(const btConvexTemplate &a,
  ...
  if (btMprIsZero(dot) || dot < 0.f) return -1;
...
template <typename btConvexTemplate>
static int btRefinePortal(const btConvexTemplate &a, const btConvexTemplate &b,
                          const btMprCollisionDescription &colDesc,
                          btMprSimplex_t *portal) {
  btVector3 dir;
  btMprSupport_t v4;
...
inline float _btMprVec3PointSegmentDist2(const btVector3 *P,
  ...
  if (t < 0.f || btMprIsZero(t)) {
    ...
    if (witness) btMprVec3Copy(witness, x0);
  } else if (t > 1.f || btMprEq(t, 1.f)) {
    dist = btMprVec3Dist2(b, P);
    if (witness) btMprVec3Copy(witness, b);
  } else {
    if (witness) {
...
inline float btMprVec3PointTriDist2(const btVector3 *P, const btVector3 *x0,
  ...
  if ((btMprIsZero(s) || s > 0.f) && (btMprEq(s, 1.f) || s < 1.f) &&
      (btMprIsZero(t) || t > 0.f) && (btMprEq(t, 1.f) || t < 1.f) &&
    ...
    if (dist2 < dist) {
      dist = dist2;
      if (witness) btMprVec3Copy(witness, &witness2);
    }
...
static void btFindPenetr(const btConvexTemplate &a, const btConvexTemplate &b,
                         const btMprCollisionDescription &colDesc,
                         btMprSimplex_t *portal, float *depth, btVector3 *pdir,
                         btVector3 *pos) {
  btVector3 dir;
  btMprSupport_t v4;
  ...
  // while (1)
  {
    ...
    if (portalReachTolerance(portal, &v4, &dir) ||
      ...
      *depth = btMprVec3PointTriDist2(origin, &btMprSimplexPoint(portal, 1)->v,
                                      &btMprSimplexPoint(portal, 2)->v,
                                      &btMprSimplexPoint(portal, 3)->v, pdir);
      *depth = BT_MPR_SQRT(*depth);

      if (btMprIsZero((*pdir).x()) && btMprIsZero((*pdir).y()) &&
          btMprIsZero((*pdir).z())) {
        *pdir = dir;
      }
...
static void btFindPenetrTouch(btMprSimplex_t *portal, float *depth,
                              btVector3 *dir, btVector3 *pos) {
...
static void btFindPenetrSegment(btMprSimplex_t *portal, float *depth,
                                btVector3 *dir, btVector3 *pos) {
// Origin lies on v0-v1 segment.
...
template <typename btConvexTemplate>
inline int btMprPenetration(const btConvexTemplate &a,
                            const btConvexTemplate &b,
                            const btMprCollisionDescription &colDesc,
                            float *depthOut, btVector3 *dirOut,
                            btVector3 *posOut) {
  btMprSimplex_t portal;
...
inline int btComputeMprPenetration(const btConvexTemplate &a,
                                   const btConvexTemplate &b,
                                   const btMprCollisionDescription &colDesc,
