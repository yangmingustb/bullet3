...
struct b3GrahamVector3 : public b3Vector3 {
  b3GrahamVector3(const b3Vector3& org, int orgIndex)
      : b3Vector3(org), m_orgIndex(orgIndex) {}
  b3Scalar m_angle;
  int m_orgIndex;
};
...
struct b3AngleCompareFunc {
  b3Vector3 m_anchor;
  b3AngleCompareFunc(const b3Vector3& anchor) : m_anchor(anchor) {}
...
inline void b3GrahamScanConvexHull2D(
    b3AlignedObjectArray<b3GrahamVector3>& originalPoints,
    b3AlignedObjectArray<b3GrahamVector3>& hull, const b3Vector3& normalAxis) {
  ...
  // step1 : find anchor point with smallest projection on axis0 and move it to
  // first location
  for (int i = 0; i < originalPoints.size(); i++) {
  ...
  int i;
  for (i = 0; i < 2; i++) hull.push_back(originalPoints[i]);

  // step 3: keep all 'convex' points and discard concave points (using back
  // tracking)
