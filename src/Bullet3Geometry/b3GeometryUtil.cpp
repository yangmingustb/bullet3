...
bool b3GeometryUtil::isPointInsidePlanes(
    const b3AlignedObjectArray<b3Vector3>& planeEquations,
    const b3Vector3& point, b3Scalar margin) {
...
bool b3GeometryUtil::areVerticesBehindPlane(
    const b3Vector3& planeNormal,
    const b3AlignedObjectArray<b3Vector3>& vertices, b3Scalar margin) {
  int numvertices = vertices.size();
  for (int i = 0; i < numvertices; i++) {
    const b3Vector3& N1 = vertices[i];
    b3Scalar dist =
        b3Scalar(planeNormal.dot(N1)) + b3Scalar(planeNormal[3]) - margin;
    if (dist > b3Scalar(0.)) {
      return false;
    }
  }
...
bool notExist(const b3Vector3& planeEquation,
              const b3AlignedObjectArray<b3Vector3>& planeEquations);
...
void b3GeometryUtil::getPlaneEquationsFromVertices(
    b3AlignedObjectArray<b3Vector3>& vertices,
    b3AlignedObjectArray<b3Vector3>& planeEquationsOut) {
        ...
        for (int ww = 0; ww < 2; ww++) {
          ...
          if (planeEquation.length2() > b3Scalar(0.0001)) {
            ...
            if (notExist(planeEquation, planeEquationsOut)) {
              ...
              if (areVerticesBehindPlane(planeEquation, vertices,
                                         b3Scalar(0.01))) {
...
void b3GeometryUtil::getVerticesFromPlaneEquations(
    const b3AlignedObjectArray<b3Vector3>& planeEquations,
      ...
      for (int k = j + 1; k < numbrushes; k++) {
        ...
        if ((n2n3.length2() > b3Scalar(0.0001)) &&
          ...
          if (b3Fabs(quotient) > b3Scalar(0.000001)) {
            ...
            // check if inside, and replace supportingVertexOut if needed
            if (isPointInsidePlanes(planeEquations, potentialVertex,
                                    b3Scalar(0.01))) {
