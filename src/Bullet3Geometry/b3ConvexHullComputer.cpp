...
class b3ConvexHullInternal {
 public:
  ...
  class Point32 {
   public:
    ...
    Point32(btInt32_t x, btInt32_t y, btInt32_t z)
        : x(x), y(y), z(z), index(-1) {}
  ...
  class Int128 {
   public:
    ...
    Int128(btInt64_t value)
        : low(value), high((value >= 0) ? 0 : (btUint64_t)-1LL) {}
    ...
    b3Scalar toScalar() const {
      return ((btInt64_t)high >= 0)
                 ? b3Scalar(high) *
                           (b3Scalar(0x100000000LL) * b3Scalar(0x100000000LL)) +
                       b3Scalar(low)
                 : -(-*this).toScalar();
    }
  ...
  class Rational64 {
   ...
   public:
    ...
    b3Scalar toScalar() const {
      return sign * ((m_denominator == 0)
                         ? B3_INFINITY
                         : (b3Scalar)m_numerator / m_denominator);
    }
  };
  ...
  class Rational128 {
   ...
   public:
    ...
    b3Scalar toScalar() const {
      return sign * ((denominator.getSign() == 0)
                         ? B3_INFINITY
  ...
  class PointR128 {
   public:
    ...
    b3Scalar xvalue() const { return x.toScalar() / denominator.toScalar(); }

    b3Scalar yvalue() const { return y.toScalar() / denominator.toScalar(); }

    b3Scalar zvalue() const { return z.toScalar() / denominator.toScalar(); }
  };
...
#ifdef DEBUG_CONVEX_HULL
    void print() {
      b3Printf("E%p : %d -> %d,  n=%p p=%p   (0 %d\t%d\t%d) -> (%d %d %d)",
               this, reverse->target->point.index, target->point.index, next,
               prev, reverse->target->point.x, reverse->target->point.y,
               reverse->target->point.z, target->point.x, target->point.y,
               target->point.z);
    }
#endif
  };
  ...
  class DMul {
   private:
    ...
    static btUint32_t low(btUint64_t value) { return (btUint32_t)value; }

    static btUint64_t mul(btUint32_t a, btUint32_t b) {
      return (btUint64_t)a * (btUint64_t)b;
    }

    static void shlHalf(btUint64_t& value) { value <<= 32; }

    static btUint64_t high(Int128 value) { return value.high; }

    static btUint64_t low(Int128 value) { return value.low; }

    static Int128 mul(btUint64_t a, btUint64_t b) { return Int128::mul(a, b); }
 ...
 private:
    ...
    T* newObject() {
      ...
      if (!o) {
        ...
        if (p) {
          ...
          p = new (b3AlignedAlloc(sizeof(PoolArray<T>), 16))
              PoolArray<T>(arraySize);
          p->next = arrays;
          arrays = p;
        }
        o = p->init();
  ...
  static Orientation getOrientation(const Edge* prev, const Edge* next,
                                    const Point32& s, const Point32& t);
  Edge* findMaxAngle(bool ccw, const Vertex* start, const Point32& s,
                     const Point64& rxs, const Point64& sxrxs,
                     Rational64& minCot);
  void findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1, Edge*& e0, Edge*& e1,
                                Vertex* stop0, Vertex* stop1);
  ...
  bool mergeProjection(IntermediateHull& h0, IntermediateHull& h1, Vertex*& c0,
                       Vertex*& c1);
  ...
  bool shiftFace(Face* face, b3Scalar amount,
...
b3ConvexHullInternal::Int128 b3ConvexHullInternal::Int128::operator*(
    btInt64_t b) const {
...
b3ConvexHullInternal::Int128 b3ConvexHullInternal::Int128::mul(btInt64_t a,
                                                               btInt64_t b) {
  Int128 result;
  ...
  DMul<btUint64_t, btUint32_t>::mul((btUint64_t)a, (btUint64_t)b, result.low,
                                    result.high);
  return negative ? -result : result;
#endif
}

b3ConvexHullInternal::Int128 b3ConvexHullInternal::Int128::mul(btUint64_t a,
                                                               btUint64_t b) {
  Int128 result;
...
//(numerator * b.denominator < b.numerator * denominator) ? -sign : 0;
  ...
  __asm__(
      ...
      : "a"(denominator), [bn] "g"(b.numerator), [tn] "g"(numerator),
        [bd] "g"(b.denominator)
      : "%rdx", "cc");
  ...
  return sign *
         Int128::mul(m_numerator, b.m_denominator)
             .ucmp(Int128::mul(m_denominator, b.m_numerator));
...
b3ConvexHullInternal::Edge* b3ConvexHullInternal::newEdgePair(Vertex* from,
                                                              Vertex* to) {
  b3Assert(from && to);
...
bool b3ConvexHullInternal::mergeProjection(IntermediateHull& h0,
                                           IntermediateHull& h1, Vertex*& c0,
                                           Vertex*& c1) {
  Vertex* v0 = h0.maxYx;
  Vertex* v1 = h1.minYx;
...
void b3ConvexHullInternal::computeInternal(int start, int end,
...
b3ConvexHullInternal::Orientation b3ConvexHullInternal::getOrientation(
    const Edge* prev, const Edge* next, const Point32& s, const Point32& t) {
  b3Assert(prev->reverse->target == next->reverse->target);
  if (prev->next == next) {
    if (prev->prev == next) {
      Point64 n = t.cross(s);
      Point64 m = (*prev->target - *next->reverse->target)
                      .cross(*next->target - *next->reverse->target);
      b3Assert(!m.isZero());
...
b3ConvexHullInternal::Edge* b3ConvexHullInternal::findMaxAngle(
    bool ccw, const Vertex* start, const Point32& s, const Point64& rxs,
...
#ifdef DEBUG_CONVEX_HULL
        b3Printf("      Angle is %f (%d) for ", (float)b3Atan(cot.toScalar()),
                 (int)cot.isNaN());
        e->print();
#endif
        if (cot.isNaN()) {
          ...
          if (minEdge == NULL) {
            ...
            minCot = cot;
            minEdge = e;
          } else if ((cmp == 0) && (ccw == (getOrientation(minEdge, e, s, t) ==
                                            COUNTER_CLOCKWISE))) {
            minEdge = e;
          }
        }
...
void b3ConvexHullInternal::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1,
                                                    Edge*& e0, Edge*& e1,
                                                    Vertex* stop0,
                                                    Vertex* stop1) {
  Edge* start0 = e0;
  ...
  Point64 normal =
      ((start0 ? start0 : start1)->target->point - c0->point).cross(s);
      ...
      if (e0 && (e0->target != stop0)) {
        ...
        if (f0->copy > mergeStamp) {
          ...
          if ((dx0 == 0)
                  ? (dy0 < 0)
                  : ((dx0 < 0) &&
                     (Rational64(dy0, dx0).compare(Rational64(dy, dx)) >= 0))) {
            et0 = f0->target->point;
            dx = (et1 - et0).dot(perp);
      ...
      if (e1 && (e1->target != stop1)) {
        ...
        if (f1->copy > mergeStamp) {
          ...
          if (d1.dot(normal) == 0) {
            ...
            if ((dxn > 0) &&
                ((dx1 == 0) ? (dy1 < 0)
                            : ((dx1 < 0) && (Rational64(dy1, dx1).compare(
                                                 Rational64(dy, dx)) > 0)))) {
              e1 = f1;
              et1 = e1->target->point;
              dx = dxn;
      ...
      if (e1 && (e1->target != stop1)) {
        ...
        if (f1->copy > mergeStamp) {
          ...
          if ((dx1 == 0)
                  ? (dy1 > 0)
                  : ((dx1 < 0) &&
                     (Rational64(dy1, dx1).compare(Rational64(dy, dx)) <= 0))) {
            et1 = f1->target->point;
            dx = (et1 - et0).dot(perp);
      ...
      if (e0 && (e0->target != stop0)) {
        ...
        if (f0->copy > mergeStamp) {
          ...
          if (d0.dot(normal) == 0) {
            ...
            if ((dxn < 0) &&
                ((dx0 == 0) ? (dy0 > 0)
                            : ((dx0 < 0) && (Rational64(dy0, dx0).compare(
                                                 Rational64(dy, dx)) < 0)))) {
              e0 = f0;
              et0 = e0->target->point;
              dx = dxn;
...
void b3ConvexHullInternal::merge(IntermediateHull& h0, IntermediateHull& h1) {
    ...
    if (e) {
      do {
        ...
        if ((dot == 0) && ((*e->target - *c0).dot(t) > 0)) {
          if (!start0 ||
              (getOrientation(start0, e, s, Point32(0, 0, -1)) == CLOCKWISE)) {
            start0 = e;
          }
        }
        e = e->next;
    ...
    if (e) {
      do {
        ...
        if ((dot == 0) && ((*e->target - *c1).dot(t) > 0)) {
          if (!start1 || (getOrientation(start1, e, s, Point32(0, 0, -1)) ==
                          COUNTER_CLOCKWISE)) {
            start1 = e;
          }
        }
  ...
  while (true) {
    ...
    if (!min0 && !min1) {
      ...
      b3Printf("    -> Result %d\n", cmp);
#endif
      if (firstRun || ((cmp >= 0) ? !minCot1.isNegativeInfinity()
...
static bool b3PointCmp(const b3ConvexHullInternal::Point32& p,
                       const b3ConvexHullInternal::Point32& q) {
  return (p.y < q.y) ||
         ((p.y == q.y) && ((p.x < q.x) || ((p.x == q.x) && (p.z < q.z))));
}

void b3ConvexHullInternal::compute(const void* coords, bool doubleCoords,
                                   int stride, int count) {
  b3Vector3 min = b3MakeVector3(b3Scalar(1e30), b3Scalar(1e30), b3Scalar(1e30)),
            max = b3MakeVector3(b3Scalar(-1e30), b3Scalar(-1e30),
                                b3Scalar(-1e30));
  ...
  if (doubleCoords) {
    for (int i = 0; i < count; i++) {
      ...
      b3Vector3 p =
          b3MakeVector3((b3Scalar)v[0], (b3Scalar)v[1], (b3Scalar)v[2]);
      ptr += stride;
      min.setMin(p);
      max.setMax(p);
    }
      ...
      do {
        ...
        if (e->copy != stamp) {
          ...
          do {
            if (a && b) {
              btInt64_t vol =
                  (v->point - ref).dot((a->point - ref).cross(b->point - ref));
              b3Assert(vol >= 0);
...
bool b3ConvexHullInternal::shiftFace(Face* face, b3Scalar amount,
                                     b3AlignedObjectArray<Vertex*> stack) {
  ...
  Point32 shift((btInt32_t)origShift[medAxis], (btInt32_t)origShift[maxAxis],
                (btInt32_t)origShift[minAxis]);
  if (shift.isZero()) {
    return true;
  }
  ...
  b3Printf("\nShrinking face (%d %d %d) (%d %d %d) (%d %d %d) by (%d %d %d)\n",
           face->origin.x, face->origin.y, face->origin.z, face->dir0.x,
           face->dir0.y, face->dir0.z, face->dir1.x, face->dir1.y, face->dir1.z,
  ...
  while (true) {
    ...
    if (cmp > 0) {
      ...
      b3Printf("1: Removed part contains (%d %d %d)\n", removed->point.x,
               removed->point.y, removed->point.z);
#endif
      ...
      btInt64_t r1 =
          (intersection->reverse->face->origin - shiftedOrigin).dot(n1);
      ...
      v->point128 = PointR128(
          Int128::mul(face->dir0.x * r0, m11) -
              Int128::mul(face->dir0.x * r1, m01) +
              Int128::mul(face->dir1.x * r1, m00) -
              Int128::mul(face->dir1.x * r0, m10) + det * shiftedOrigin.x,
          Int128::mul(face->dir0.y * r0, m11) -
              Int128::mul(face->dir0.y * r1, m01) +
              Int128::mul(face->dir1.y * r1, m00) -
              Int128::mul(face->dir1.y * r0, m10) + det * shiftedOrigin.y,
          Int128::mul(face->dir0.z * r0, m11) -
              Int128::mul(face->dir0.z * r1, m01) +
              Int128::mul(face->dir1.z * r1, m00) -
...
static int getVertexCopy(
    b3ConvexHullInternal::Vertex* vertex,
    b3AlignedObjectArray<b3ConvexHullInternal::Vertex*>& vertices) {
...
b3Scalar b3ConvexHullComputer::compute(const void* coords, bool doubleCoords,
                                       int stride, int count, b3Scalar shrink,
                                       b3Scalar shrinkClamp) {
  if (count <= 0) {
    vertices.clear();
    edges.clear();
    faces.clear();
