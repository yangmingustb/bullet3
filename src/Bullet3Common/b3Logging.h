...
/// We add the do/while so that the statement "if (condition) b3Printf("test");
/// else {...}" would fail
/// You can also customize the message by uncommenting out a different line
/// below
#define b3Printf(...) b3OutputPrintfVarArgsInternal(__VA_ARGS__)
//#define b3Printf(...) do
//{b3OutputPrintfVarArgsInternal("b3Printf[%s,%d]:",__FILE__,__LINE__);b3OutputPrintfVarArgsInternal(__VA_ARGS__);
//} while(0)
...
class b3ProfileZone {
 public:
  b3ProfileZone(const char* name) { b3EnterProfileZone(name); }

  ~b3ProfileZone() { b3LeaveProfileZone(); }
};
...
/// Don't use those internal functions directly, use the b3Printf or
/// b3SetCustomPrintfFunc instead (or warning/error version)
