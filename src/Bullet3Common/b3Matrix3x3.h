...
/**@brief The b3Matrix3x3 class implements a 3x3 rotation matrix, to perform
* linear algebra in combination with b3Quaternion, b3Transform and b3Vector3.
...
b3Matrix3x3 {
  ...
  */
  ...
  B3_FORCE_INLINE b3Matrix3x3(const b3SimdFloat4 v0, const b3SimdFloat4 v1,
                              const b3SimdFloat4 v2) {
    m_el[0].mVec128 = v0;
    m_el[1].mVec128 = v1;
    m_el[2].mVec128 = v2;
  }

  B3_FORCE_INLINE b3Matrix3x3(const b3Vector3& v0, const b3Vector3& v1,
                              const b3Vector3& v2) {
    m_el[0] = v0;
    m_el[1] = v1;
    m_el[2] = v2;
  ...
  *  @param q The Quaternion to match */
  void setRotation(const b3Quaternion& q) {
    ...
    V1 = b3CastiTo128f(
        _mm_shuffle_epi32(Qi, B3_SHUFFLE(1, 0, 2, 3)));  // Y X Z W
    V2 = _mm_shuffle_ps(NQ, Q, B3_SHUFFLE(0, 0, 1, 3));  // -X -X  Y  W
    V3 = b3CastiTo128f(
        _mm_shuffle_epi32(Qi, B3_SHUFFLE(2, 1, 0, 3)));  // Z Y X W
    V1 = _mm_xor_ps(V1, b3vMPPP);  //	change the sign of the first element

    V11 = b3CastiTo128f(
        _mm_shuffle_epi32(Qi, B3_SHUFFLE(1, 1, 0, 3)));   // Y Y X W
    ...
    Y = b3CastiTo128f(
        _mm_shuffle_epi32(NQi, B3_SHUFFLE(3, 2, 0, 3)));  // -W -Z -X -W
    Z = b3CastiTo128f(
        _mm_shuffle_epi32(Qi, B3_SHUFFLE(1, 0, 1, 3)));  //  Y  X  Y  W
    ...
    setValue(b3Scalar(1.0) - (yy + zz), xy - wz, xz + wy, xy + wz,
             b3Scalar(1.0) - (xx + zz), yz - wx, xz - wy, yz + wx,
             b3Scalar(1.0) - (xx + yy));
#endif
  }
  ...
  */
  void setEulerYPR(const b3Scalar& yaw, const b3Scalar& pitch,
                   const b3Scalar& roll) {
  ...
  **/
...
#else
    setValue(b3Scalar(1.0), b3Scalar(0.0), b3Scalar(0.0), b3Scalar(0.0),
             b3Scalar(1.0), b3Scalar(0.0), b3Scalar(0.0), b3Scalar(0.0),
             b3Scalar(1.0));
#endif
  }
...
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE)) || defined(B3_USE_NEON)
    static const b3Matrix3x3 identityMatrix(b3v1000, b3v0100, b3v0010);
#else
    static const b3Matrix3x3 identityMatrix(
        b3Scalar(1.0), b3Scalar(0.0), b3Scalar(0.0), b3Scalar(0.0),
        b3Scalar(1.0), b3Scalar(0.0), b3Scalar(0.0), b3Scalar(0.0),
        b3Scalar(1.0));
#endif
  ...
  /**@brief Fill the rotational part of an OpenGL matrix and clear the
  * shear/perspective
  * @param m The array to be filled */
  void getOpenGLSubMatrix(b3Scalar * m) const {
    ...
    v2 = b3CastdTo128f(
        _mm_move_sd(b3CastfTo128d(v2), b3CastfTo128d(vT)));  // z0 z1 z2 0

    vm[0] = v0;
    vm[1] = v1;
    vm[2] = v2;
#elif defined(B3_USE_NEON)
    // note: zeros the w channel. We can preserve it at the cost of two more
    // vtrn instructions.
    static const uint32x2_t zMask = (const uint32x2_t){-1, 0};
    float32x4_t* vm = (float32x4_t*)m;
    float32x4x2_t top = vtrnq_f32(
        m_el[0].mVec128, m_el[1].mVec128);  // {x0 x1 z0 z1}, {y0 y1 w0 w1}
    float32x2x2_t bl = vtrn_f32(vget_low_f32(m_el[2].mVec128),
                                vdup_n_f32(0.0f));  // {x2  0 }, {y2 0}
    float32x4_t v0 = vcombine_f32(vget_low_f32(top.val[0]), bl.val[0]);
    float32x4_t v1 = vcombine_f32(vget_low_f32(top.val[1]), bl.val[1]);
    float32x2_t q = (float32x2_t)vand_u32(
        (uint32x2_t)vget_high_f32(m_el[2].mVec128), zMask);
  ...
  * are possible values*/
  void getEulerZYX(b3Scalar & yaw, b3Scalar & pitch, b3Scalar & roll,
  ...
  * @param s Scaling vector The elements of the vector will scale each column */
...
#else
    return b3Matrix3x3(m_el[0].getX() * s.getX(), m_el[0].getY() * s.getY(),
                       m_el[0].getZ() * s.getZ(), m_el[1].getX() * s.getX(),
                       m_el[1].getY() * s.getY(), m_el[1].getZ() * s.getZ(),
                       m_el[2].getX() * s.getX(), m_el[2].getY() * s.getY(),
                       m_el[2].getZ() * s.getZ());
#endif
  }
  ...
  B3_FORCE_INLINE b3Scalar tdotx(const b3Vector3& v) const {
    return m_el[0].getX() * v.getX() + m_el[1].getX() * v.getY() +
           m_el[2].getX() * v.getZ();
  }
  B3_FORCE_INLINE b3Scalar tdoty(const b3Vector3& v) const {
    return m_el[0].getY() * v.getX() + m_el[1].getY() * v.getY() +
           m_el[2].getY() * v.getZ();
  }
  B3_FORCE_INLINE b3Scalar tdotz(const b3Vector3& v) const {
    return m_el[0].getZ() * v.getX() + m_el[1].getZ() * v.getY() +
           m_el[2].getZ() * v.getZ();
  }

  /**@brief diagonalizes this matrix by the Jacobi method.
  * @param rot stores the rotation from the coordinate system in which the
  * matrix is diagonal to the original
  * coordinate system, i.e., old_this = rot * new_this * rot^T.
  * @param threshold See iteration
  * @param iteration The iteration stops when all off-diagonal elements are less
  * than the threshold multiplied
  ...
  */
  void diagonalize(b3Matrix3x3 & rot, b3Scalar threshold, int maxSteps) {
    ...
    for (int step = maxSteps; step > 0; step--) {
      ...
      b3Scalar t = threshold * (b3Fabs(m_el[0][0]) + b3Fabs(m_el[1][1]) +
                                b3Fabs(m_el[2][2]));
      if (max <= t) {
        if (max <= B3_EPSILON * t) {
          return;
...
B3_FORCE_INLINE b3Matrix3x3& b3Matrix3x3::operator*=(const b3Matrix3x3& m) {
#if defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE)
...
B3_FORCE_INLINE b3Matrix3x3 operator*(const b3Matrix3x3& m, const b3Scalar& k) {
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE))
  __m128 vk = b3_splat_ps(_mm_load_ss((float*)&k), 0x80);
  return b3Matrix3x3(_mm_mul_ps(m[0].mVec128, vk), _mm_mul_ps(m[1].mVec128, vk),
                     _mm_mul_ps(m[2].mVec128, vk));
#elif defined(B3_USE_NEON)
  return b3Matrix3x3(vmulq_n_f32(m[0].mVec128, k), vmulq_n_f32(m[1].mVec128, k),
                     vmulq_n_f32(m[2].mVec128, k));
#else
  return b3Matrix3x3(m[0].getX() * k, m[0].getY() * k, m[0].getZ() * k,
                     m[1].getX() * k, m[1].getY() * k, m[1].getZ() * k,
                     m[2].getX() * k, m[2].getY() * k, m[2].getZ() * k);
#endif
}

B3_FORCE_INLINE b3Matrix3x3 operator+(const b3Matrix3x3& m1,
                                      const b3Matrix3x3& m2) {
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE)) || defined(B3_USE_NEON)
  return b3Matrix3x3(m1[0].mVec128 + m2[0].mVec128,
...
B3_FORCE_INLINE b3Scalar b3Matrix3x3::determinant() const {
  return b3Triple((*this)[0], (*this)[1], (*this)[2]);
}

B3_FORCE_INLINE b3Matrix3x3 b3Matrix3x3::absolute() const {
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE))
  return b3Matrix3x3(_mm_and_ps(m_el[0].mVec128, b3vAbsfMask),
...
B3_FORCE_INLINE b3Matrix3x3 b3Matrix3x3::transpose() const {
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE))
  __m128 v0 = m_el[0].mVec128;
  __m128 v1 = m_el[1].mVec128;
  __m128 v2 = m_el[2].mVec128;  //  x2 y2 z2 w2
...
#elif defined(B3_USE_NEON)
  // note: zeros the w channel. We can preserve it at the cost of two more vtrn
  // instructions.
  static const uint32x2_t zMask = (const uint32x2_t){-1, 0};
  float32x4x2_t top = vtrnq_f32(
      m_el[0].mVec128, m_el[1].mVec128);  // {x0 x1 z0 z1}, {y0 y1 w0 w1}
  ...
  float32x4_t v1 = vcombine_f32(vget_low_f32(top.val[1]), bl.val[1]);
  float32x2_t q =
      (float32x2_t)vand_u32((uint32x2_t)vget_high_f32(m_el[2].mVec128), zMask);
  float32x4_t v2 = vcombine_f32(vget_high_f32(top.val[0]), q);  // z0 z1 z2  0
  return b3Matrix3x3(v0, v1, v2);
#else
...
B3_FORCE_INLINE b3Matrix3x3 b3Matrix3x3::adjoint() const {
  return b3Matrix3x3(cofac(1, 1, 2, 2), cofac(0, 2, 2, 1), cofac(0, 1, 1, 2),
                     cofac(1, 2, 2, 0), cofac(0, 0, 2, 2), cofac(0, 2, 1, 0),
                     cofac(1, 0, 2, 1), cofac(0, 1, 2, 0), cofac(0, 0, 1, 1));
}

B3_FORCE_INLINE b3Matrix3x3 b3Matrix3x3::inverse() const {
  b3Vector3 co =
      b3MakeVector3(cofac(1, 1, 2, 2), cofac(1, 2, 2, 0), cofac(1, 0, 2, 1));
...
b3Matrix3x3::transposeTimes(const b3Matrix3x3& m) const {
  ...
  float32x4_t m0 =
      (float32x4_t)vandq_u32((uint32x4_t)m.getRow(0).mVec128, xyzMask);
  float32x4_t m1 =
      (float32x4_t)vandq_u32((uint32x4_t)m.getRow(1).mVec128, xyzMask);
  float32x4_t m2 =
      (float32x4_t)vandq_u32((uint32x4_t)m.getRow(2).mVec128, xyzMask);
...
#else
  return b3Matrix3x3(
      m_el[0].getX() * m[0].getX() + m_el[1].getX() * m[1].getX() +
          m_el[2].getX() * m[2].getX(),
      m_el[0].getX() * m[0].getY() + m_el[1].getX() * m[1].getY() +
          m_el[2].getX() * m[2].getY(),
      m_el[0].getX() * m[0].getZ() + m_el[1].getX() * m[1].getZ() +
          m_el[2].getX() * m[2].getZ(),
      m_el[0].getY() * m[0].getX() + m_el[1].getY() * m[1].getX() +
          m_el[2].getY() * m[2].getX(),
      m_el[0].getY() * m[0].getY() + m_el[1].getY() * m[1].getY() +
          m_el[2].getY() * m[2].getY(),
      m_el[0].getY() * m[0].getZ() + m_el[1].getY() * m[1].getZ() +
          m_el[2].getY() * m[2].getZ(),
      m_el[0].getZ() * m[0].getX() + m_el[1].getZ() * m[1].getX() +
          m_el[2].getZ() * m[2].getX(),
      m_el[0].getZ() * m[0].getY() + m_el[1].getZ() * m[1].getY() +
          m_el[2].getZ() * m[2].getY(),
      m_el[0].getZ() * m[0].getZ() + m_el[1].getZ() * m[1].getZ() +
          m_el[2].getZ() * m[2].getZ());
#endif
}
...
b3Matrix3x3::timesTranspose(const b3Matrix3x3& m) const {
  ...
  b3Matrix3x3 mT = m.transpose();  // we rely on transpose() zeroing w channel
...
#else
  return b3Matrix3x3(m_el[0].dot(m[0]), m_el[0].dot(m[1]), m_el[0].dot(m[2]),
                     m_el[1].dot(m[0]), m_el[1].dot(m[1]), m_el[1].dot(m[2]),
                     m_el[2].dot(m[0]), m_el[2].dot(m[1]), m_el[2].dot(m[2]));
#endif
}

B3_FORCE_INLINE b3Vector3 operator*(const b3Matrix3x3& m, const b3Vector3& v) {
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE)) || defined(B3_USE_NEON)
...
B3_FORCE_INLINE b3Vector3 operator*(const b3Vector3& v, const b3Matrix3x3& m) {
#if (defined(B3_USE_SSE_IN_API) && defined(B3_USE_SSE))
...
#else
  return b3Matrix3x3(m2.tdotx(m1[0]), m2.tdoty(m1[0]), m2.tdotz(m1[0]),
...
/*
B3_FORCE_INLINE b3Matrix3x3 b3MultTransposeLeft(const b3Matrix3x3& m1, const
b3Matrix3x3& m2) {
return b3Matrix3x3(
...
#else
  return (m1[0][0] == m2[0][0] && m1[1][0] == m2[1][0] &&
          m1[2][0] == m2[2][0] && m1[0][1] == m2[0][1] &&
          m1[1][1] == m2[1][1] && m1[2][1] == m2[2][1] &&
          m1[0][2] == m2[0][2] && m1[1][2] == m2[1][2] && m1[2][2] == m2[2][2]);
#endif
}
...
B3_FORCE_INLINE void b3Matrix3x3::serialize(
    struct b3Matrix3x3Data& dataOut) const {
  for (int i = 0; i < 3; i++) m_el[i].serialize(dataOut.m_el[i]);
}

B3_FORCE_INLINE void b3Matrix3x3::serializeFloat(
    struct b3Matrix3x3FloatData& dataOut) const {
  for (int i = 0; i < 3; i++) m_el[i].serializeFloat(dataOut.m_el[i]);
}

B3_FORCE_INLINE void b3Matrix3x3::deSerialize(
    const struct b3Matrix3x3Data& dataIn) {
  for (int i = 0; i < 3; i++) m_el[i].deSerialize(dataIn.m_el[i]);
}

B3_FORCE_INLINE void b3Matrix3x3::deSerializeFloat(
    const struct b3Matrix3x3FloatData& dataIn) {
  for (int i = 0; i < 3; i++) m_el[i].deSerializeFloat(dataIn.m_el[i]);
}

B3_FORCE_INLINE void b3Matrix3x3::deSerializeDouble(
    const struct b3Matrix3x3DoubleData& dataIn) {
