...
#define b3AlignedAlloc(a, b) b3AlignedAllocInternal(a, b, __LINE__, __FILE__)

#define b3AlignedFree(ptr) b3AlignedFreeInternal(ptr, __LINE__, __FILE__)

void* b3AlignedAllocInternal(size_t size, int alignment, int line,
                             char* filename);
...
/// The developer can let all Bullet memory allocations go through a custom
/// memory allocator, using b3AlignedAllocSetCustom
void b3AlignedAllocSetCustom(b3AllocFunc* allocFunc, b3FreeFunc* freeFunc);
/// If the developer has already an custom aligned allocator, then
/// b3AlignedAllocSetCustomAligned can be used. The default aligned allocator
/// pre-allocates extra memory using the non-aligned allocator, and instruments
/// it.
void b3AlignedAllocSetCustomAligned(b3AlignedAllocFunc* allocFunc,
                                    b3AlignedFreeFunc* freeFunc);

/// The b3AlignedAllocator is a portable class for aligned memory allocations.
/// Default implementations for unaligned and aligned allocations can be
/// overridden by a custom allocator using b3AlignedAllocSetCustom and
  ...
  */
  ...
  pointer allocate(btSizeType n, const_pointer* hint = 0) {
    ...
    return reinterpret_cast<pointer>(
        b3AlignedAlloc(sizeof(value_type) * n, Alignment));
  }
  void construct(pointer ptr, const value_type& value) {
    new (ptr) value_type(value);
  }
  void deallocate(pointer ptr) { b3AlignedFree(reinterpret_cast<void*>(ptr)); }
