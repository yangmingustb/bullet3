...
/**@brief The b3Transform class supports rigid transforms with only translation
 *and rotation and no scaling/shear.
 *It can be used in combination with b3Vector3, b3Quaternion and b3Matrix3x3
...
* @param c Translation from Vector (default 0,0,0) */
  explicit B3_FORCE_INLINE b3Transform(
...
* @param m A pointer to a 15 element array (12 rotation(row major padded on the
* right by 1), and 3 translation */
...
* @param origin The vector to set the translation to */
  B3_FORCE_INLINE void setOrigin(const b3Vector3& origin) { m_origin = origin; }

  B3_FORCE_INLINE b3Vector3 invXform(const b3Vector3& inVec) const;

  /**@brief Set the rotational element by b3Matrix3x3 */
  B3_FORCE_INLINE void setBasis(const b3Matrix3x3& basis) { m_basis = basis; }
...
B3_FORCE_INLINE b3Vector3 b3Transform::invXform(const b3Vector3& inVec) const {
...
b3Transform::inverseTimes(const b3Transform& t) const {
  ...
  return b3Transform(m_basis.transposeTimes(t.m_basis), v * m_basis);
}

B3_FORCE_INLINE b3Transform b3Transform::operator*(const b3Transform& t) const {
  return b3Transform(m_basis * t.m_basis, (*this)(t.m_origin));
}
...
B3_FORCE_INLINE void b3Transform::serializeFloat(
    b3TransformFloatData& dataOut) const {
...
B3_FORCE_INLINE void b3Transform::deSerializeFloat(
    const b3TransformFloatData& dataIn) {
  m_basis.deSerializeFloat(dataIn.m_basis);
  m_origin.deSerializeFloat(dataIn.m_origin);
}

B3_FORCE_INLINE void b3Transform::deSerializeDouble(
    const b3TransformDoubleData& dataIn) {
