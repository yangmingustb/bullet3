...
inline int b3GetVersion() { return B3_BULLET_VERSION; }
...
//disable but used in MSCV Stl warning.
#pragma warning(disable : 4996)  // Turn off warnings about deprecated C
                                 // routines
//			#pragma warning(disable:4786) // Disable the "debug name too
//long" warning
...
#define B3_USE_SSE
// B3_USE_SSE_IN_API is enabled on Mac OSX by default, because memory is
// automatically aligned on 16-byte boundaries
...
/// The b3Scalar type abstracts floating point numbers, to easily switch between
/// double and single floating point precision.
...
#define B3_DECLARE_ALIGNED_ALLOCATOR()                                      \
  B3_FORCE_INLINE void *operator new(size_t sizeInBytes) {                  \
    return b3AlignedAlloc(sizeInBytes, 16);                                 \
  }                                                                         \
...
#define b3RecipSqrt(x)                                                        \
  ((b3Scalar)(b3Scalar(1.0) / b3Sqrt(b3Scalar(x)))) /* reciprocal square root \
                                                       */
...
/// b3Select avoids branches, which makes performance much better for consoles
/// like Playstation 3 and XBox 360
/// Thanks Phil Knight. See also
/// http://www.cellperformance.com/articles/2006/04/more_techniques_for_eliminatin_1.html
B3_FORCE_INLINE unsigned b3Select(unsigned condition,
...
B3_FORCE_INLINE int b3Select(unsigned condition, int valueIfConditionNonZero,
...
B3_FORCE_INLINE float b3Select(unsigned condition,
                               float valueIfConditionNonZero,
                               float valueIfConditionZero) {
#ifdef B3_HAVE_NATIVE_FSEL
  return (float)b3Fsel((b3Scalar)condition - b3Scalar(1.0f),
...
////b3SwapFloat/b3SwapDouble will NOT return a float, because the machine might
///'correct' invalid floating point values
/// Not all values of sign/exponent/mantissa are valid floating point numbers
/// according to IEEE 754.
/// When a floating point unit is faced with an invalid value, it may actually
/// change the value, or worse, throw an exception.
/// In most systems, running user mode code, you wouldn't get an exception, but
/// instead the hardware/os/runtime will 'fix' the number for you.
...
struct b3TypedObject {
  b3TypedObject(int objectType) : m_objectType(objectType) {}
  int m_objectType;
