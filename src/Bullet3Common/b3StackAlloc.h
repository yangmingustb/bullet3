...
/// The b3Block class is an internal structure for the b3StackAlloc memory
/// allocator.
struct b3Block {
  b3Block* previous;
...
/// The StackAlloc class provides some fast stack-based memory allocator (LIFO
/// last-in first-out)
class b3StackAlloc {
 public:
  ...
  inline void destroy() {
    ...
    if (usedsize == 0) {
      if (!ischild && data) b3AlignedFree(data);
