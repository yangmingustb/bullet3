...
MultiBodyTree::MultiBodyImpl::MultiBodyImpl(int num_bodies_, int num_dofs_)
    : m_num_bodies(num_bodies_),
      m_num_dofs(num_dofs_)
...
const char *MultiBodyTree::MultiBodyImpl::jointTypeToString(
    const JointType &type) const {
  switch (type) {
    case FIXED:
      return "fixed";
    case REVOLUTE:
...
inline void indent(const int &level) {
  for (int j = 0; j < level; j++) id_printf("  ");  // indent
}
...
void MultiBodyTree::MultiBodyImpl::printTree(int index, int indentation) {
  ...
  for (int i = 0; i < num_children; i++) {
    ...
    id_printf("body %.2d[%s]: %.2d is child no. %d (qi= %d .. %d) \n", index,
              jointTypeToString(m_body_list[index].m_joint_type), child_index,
              (count++) + 1, m_body_list[index].m_q_index,
...
int MultiBodyTree::MultiBodyImpl::generateIndexSets() {
  ...
  for (idArrayIdx child = 1; child < m_parent_index.size(); child++) {
    ...
    if (parent >= 0 && parent < (static_cast<int>(m_parent_index.size()) - 1)) {
      ...
      if (-1 == parent) {
        // multiple bodies are directly linked to the environment, ie, not a
        // single root
        bt_id_error_message(
        ...
        // should never happen
        bt_id_error_message(
            "building index sets. parent_index[%zu]= %d, but "
            "m_parent_index.size()= %d\n",
...
int MultiBodyTree::MultiBodyImpl::calculateInverseDynamics(const vecx &q,
  ...
  for (int body_idx = m_body_list.size() - 1; body_idx >= 0; body_idx--) {
    ...
    for (idArrayIdx child_list_idx = 0;
         child_list_idx < m_child_indices[body_idx].size(); child_list_idx++) {
...
int MultiBodyTree::MultiBodyImpl::calculateKinematics(
    const vecx &q, const vecx &u, const vecx &dot_u, const KinUpdateType type) {
  if (q.size() != m_num_dofs || u.size() != m_num_dofs ||
  ...
  for (idArrayIdx i = 0; i < m_body_floating_list.size(); i++) {
    ...
    if (type >= POSITION_VELOCITY_ACCELERATION) {
      ...
      body.m_parent_acc_rel =
          body.m_body_T_parent.transpose() * body.m_parent_acc_rel;
    }
  }
  ...
  // calculations for root body
  {
    ...
    if (type >= POSITION_VELOCITY_ACCELERATION) {
      ...
      // NOTE: To get correct acceleration kinematics, just set world_gravity to
      // zero
      body.m_body_acc =
  ...
  for (idArrayIdx i = 1; i < m_body_list.size(); i++) {
    ...
    if (type >= POSITION_VELOCITY_ACCELERATION) {
      ...
      // NOTE: assumption: dot(J_JR) = 0; true here, but not for general joints
      body.m_body_ang_acc = body.m_body_T_parent * parent.m_body_ang_acc -
                            body.m_body_ang_vel_rel.cross(
                                body.m_body_T_parent * parent.m_body_ang_vel) +
...
void MultiBodyTree::MultiBodyImpl::addRelativeJacobianComponent(
    RigidBody &body) {
    ...
    if (type >= POSITION_VELOCITY) {
      ...
      body.m_body_dot_Jac_T_u =
          ...
          (parent.m_body_dot_Jac_T_u +
           parent.m_body_dot_Jac_R_u.cross(body.m_parent_pos_parent_body) +
           parent.m_body_ang_vel.cross(
               parent.m_body_ang_vel.cross(body.m_parent_pos_parent_body)) +
           2.0 * parent.m_body_ang_vel.cross(body.m_parent_vel_rel));
    }
...
static inline void setSixDoFJacobians(const int dof, vec3 &Jac_JR,
                                      vec3 &Jac_JT) {
  switch (dof) {
    // rotational part
    case 0:
...
int MultiBodyTree::MultiBodyImpl::calculateMassMatrix(
  ...
  // formulation centered
  // on the origin of the body-fixed frame to avoid re-computing various
  // quantities at the com.
  ...
  for (int i = m_body_list.size() - 1; i >= 0; i--) {
    ...
    for (idArrayIdx c = 0; c < m_child_indices[i].size(); c++) {
      ...
      if (child.m_subtree_mass > 0) {
        ...
        // (First shift from child origin to child com, then from there to this
        // body's
        // origin)
        vec3 r_com =
            body_T_child * child.m_body_subtree_mass_com / child.m_subtree_mass;
        mat33 tilde_r_child_com = tildeOperator(r_com);
        mat33 tilde_r_body_com =
            tildeOperator(child.m_parent_pos_parent_body + r_com);
        body.m_body_subtree_I_body +=
            child.m_subtree_mass * (tilde_r_child_com * tilde_r_child_com -
    ...
    for (int col = q_index_max; col >= q_index_min; col--) {
      ...
      // rest of the mass matrix column upwards
      {
        ...
        while (parent_idx >= 0) {
          ...
          body_eom_rot +=
              child_body.m_parent_pos_parent_body.cross(body_eom_trans);

          const int parent_body_q_index_min = parent_body.m_q_index;
          const int parent_body_q_index_max =
              parent_body_q_index_min + jointNumDoFs(parent_body.m_joint_type) -
              1;
          ...
          for (int row = parent_body_q_index_max;
            ...
            if (SPHERICAL == parent_body.m_joint_type) {
              // todo: review
              setThreeDoFJacobians(row - parent_body_q_index_min, Jac_JR,
                                   Jac_JT);
            }
            // set jacobians for 6-DoF joints
...
int MultiBodyTree::MultiBodyImpl::getJointTypeStr(
    const int body_index, const char **joint_type) const {
  CHECK_IF_BODY_INDEX_IS_VALID(body_index);
...
int MultiBodyTree::MultiBodyImpl::getBodyJacobianRot(
    const int body_index, mat3x *world_jac_rot) const {
  CHECK_IF_BODY_INDEX_IS_VALID(body_index);
  const RigidBody &body = m_body_list[body_index];
