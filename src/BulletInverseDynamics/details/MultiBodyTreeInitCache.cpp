...
int MultiBodyTree::InitCache::addBody(
    const int body_index, const int parent_index, const JointType joint_type,
    const vec3& parent_r_parent_body_ref, const mat33& body_T_parent_ref,
    const vec3& body_axis_of_motion, const idScalar mass,
    const vec3& body_r_body_com, const mat33& body_I_body, const int user_int,
    void* user_ptr) {
  switch (joint_type) {
    case REVOLUTE:
    case PRISMATIC:
  ...
  if (-1 == parent_index) {
    if (m_root_index >= 0) {
      bt_id_error_message(
          "trying to add body %d as root, but already added %d as root body\n",
...
int MultiBodyTree::InitCache::getInertiaData(const int index,
                                             InertiaData* inertia) const {
...
int MultiBodyTree::InitCache::getUserPtr(const int index,
                                         void** user_ptr) const {
...
int MultiBodyTree::InitCache::getJointData(const int index,
                                           JointData* joint) const {
