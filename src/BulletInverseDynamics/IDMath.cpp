...
void mul(const mat33 &a, const mat3x &b, mat3x *result) {
  if (b.cols() != result->cols()) {
    bt_id_error_message("size missmatch. b.cols()= %d, result->cols()= %d\n",
                        ...
                        static_cast<int>(result->cols()));
    abort();
  }

  for (idArrayIdx col = 0; col < b.cols(); col++) {
    const idScalar x =
        a(0, 0) * b(0, col) + a(0, 1) * b(1, col) + a(0, 2) * b(2, col);
    const idScalar y =
        a(1, 0) * b(0, col) + a(1, 1) * b(1, col) + a(1, 2) * b(2, col);
    const idScalar z =
        a(2, 0) * b(0, col) + a(2, 1) * b(1, col) + a(2, 2) * b(2, col);
...
void getVecMatFromDH(idScalar theta, idScalar d, idScalar a, idScalar alpha,
                     vec3 *r, mat33 *T) {
...
void bodyTParentFromAxisAngle(const vec3 &axis, const idScalar &angle,
                              mat33 *T) {
...
bool isValidInertiaMatrix(const mat33 &I, const int index,
                          bool has_fixed_joint) {
  // TODO(Thomas) do we really want this?
  //			  in cases where the inertia tensor about the center of mass is
  //zero,
  //			  the determinant of the inertia tensor about the joint axis is
  //almost
  ...
  if (!has_fixed_joint) {
    if (I(0, 0) + I(1, 1) < I(2, 2)) {
      bt_id_error_message(
          "invalid inertia tensor for body %d, I(0,0) + I(1,1) < I(2,2)\n",
          index);
      bt_id_error_message(
          "matrix is:\n"
          "[%.20e %.20e %.20e;\n"
          "%.20e %.20e %.20e;\n"
          "%.20e %.20e %.20e]\n",
          I(0, 0), I(0, 1), I(0, 2), I(1, 0), I(1, 1), I(1, 2), I(2, 0),
          I(2, 1), I(2, 2));
      return false;
    }
    if (I(0, 0) + I(1, 1) < I(2, 2)) {
    ...
    if (I(1, 1) + I(2, 2) < I(0, 0)) {
      bt_id_error_message(
          "invalid inertia tensor for body %d, I(1,1) + I(2,2) < I(0,0)\n",
          index);
      bt_id_error_message(
          "matrix is:\n"
          "[%.20e %.20e %.20e;\n"
  ...
  if (BT_ID_FABS(I(1, 2) - I(2, 1)) > kIsZero) {
    bt_id_error_message(
        "invalid inertia tensor body %d I(1,2)!=I(2,1). I(1,2)-I(2,1)= %e\n",
        index, I(1, 2) - I(2, 1));
    return false;
  }
  return true;
...
#define print_mat(x)                                                        \
  ...
  for (int i = 0; i < 3; i++) {
    const idScalar length_minus_1 = BT_ID_FABS(
        m(0, i) * m(0, i) + m(1, i) * m(1, i) + m(2, i) * m(2, i) - 1.0);
  ...
  if (BT_ID_FABS(m(0, 0) * m(0, 1) + m(1, 0) * m(1, 1) + m(2, 0) * m(2, 1)) >
      kAxisLengthEpsilon) {
    bt_id_error_message(
        "Not a valid rotation matrix (columns 0 and 1 not orthogonal)\n");
    print_mat(m);
    return false;
  }
  if (BT_ID_FABS(m(0, 0) * m(0, 2) + m(1, 0) * m(1, 2) + m(2, 0) * m(2, 2)) >
      kAxisLengthEpsilon) {
    bt_id_error_message(
        "Not a valid rotation matrix (columns 0 and 2 not orthogonal)\n");
    print_mat(m);
    return false;
  }
  if (BT_ID_FABS(m(0, 1) * m(0, 2) + m(1, 1) * m(1, 2) + m(2, 1) * m(2, 2)) >
      kAxisLengthEpsilon) {
    bt_id_error_message(
        "Not a valid rotation matrix (columns 0 and 2 not orthogonal)\n");
    print_mat(m);
...
bool isUnitVector(const vec3 &vector) {
  return BT_ID_FABS(vector(0) * vector(0) + vector(1) * vector(1) +
                    vector(2) * vector(2) - 1.0) < kIsZero;
}
...
vec3 rpyFromMatrix(const mat33 &rot) {
  ...
  rpy(1) = BT_ID_ATAN2(
      rot(2, 0), BT_ID_COS(rpy(2)) * rot(0, 0) - BT_ID_SIN(rpy(0)) * rot(1, 0));
