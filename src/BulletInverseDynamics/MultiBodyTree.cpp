...
int MultiBodyTree::getBodyOrigin(const int body_index,
...
int MultiBodyTree::calculateInverseDynamics(const vecx &q, const vecx &u,
                                            const vecx &dot_u,
                                            vecx *joint_forces) {
...
int MultiBodyTree::calculateMassMatrix(const vecx &q,
...
int MultiBodyTree::calculateKinematics(const vecx &q, const vecx &u,
                                       const vecx &dot_u) {
...
int MultiBodyTree::calculatePositionKinematics(const vecx &q) {
  ...
  if (-1 ==
      m_impl->calculateKinematics(
          q, q, q, MultiBodyTree::MultiBodyImpl::POSITION_VELOCITY)) {
    bt_id_error_message("error in kinematics calculation\n");
    return -1;
  }
  return 0;
...
int MultiBodyTree::calculatePositionAndVelocityKinematics(const vecx &q,
                                                          const vecx &u) {
  if (false == m_is_finalized) {
    bt_id_error_message("system has not been initialized\n");
    return -1;
  ...
  if (-1 ==
      m_impl->calculateKinematics(
          q, u, u, MultiBodyTree::MultiBodyImpl::POSITION_VELOCITY)) {
    bt_id_error_message("error in kinematics calculation\n");
    return -1;
  }
  return 0;
...
int MultiBodyTree::calculateJacobians(const vecx &q, const vecx &u) {
  ...
  if (-1 ==
      m_impl->calculateJacobians(
          q, u, MultiBodyTree::MultiBodyImpl::POSITION_VELOCITY)) {
...
int MultiBodyTree::getBodyDotJacobianTransU(const int body_index,
                                            vec3 *world_dot_jac_trans_u) const {
  return m_impl->getBodyDotJacobianTransU(body_index, world_dot_jac_trans_u);
}

int MultiBodyTree::getBodyDotJacobianRotU(const int body_index,
                                          vec3 *world_dot_jac_rot_u) const {
  return m_impl->getBodyDotJacobianRotU(body_index, world_dot_jac_rot_u);
}

int MultiBodyTree::getBodyJacobianTrans(const int body_index,
                                        mat3x *world_jac_trans) const {
  return m_impl->getBodyJacobianTrans(body_index, world_jac_trans);
}

int MultiBodyTree::getBodyJacobianRot(const int body_index,
...
int MultiBodyTree::addBody(int body_index, int parent_index,
                           ...
                           const vec3 &body_r_body_com,
                           const mat33 &body_I_body, const int user_int,
                           void *user_ptr) {
  if (body_index < 0) {
    ...
    case PRISMATIC:
      ...
      if (!isUnitVector(body_axis_of_motion)) {
        bt_id_warning_message(
            "axis of motion not a unit axis ([%f %f %f]), will use normalized "
            "vector\n",
            body_axis_of_motion(0), body_axis_of_motion(1),
            body_axis_of_motion(2));
  ...
  return m_init_cache->addBody(body_index, parent_index, joint_type,
                               parent_r_parent_body_ref, body_T_parent_ref,
                               body_axis_of_motion, mass, body_r_body_com,
                               body_I_body, user_int, user_ptr);
}

int MultiBodyTree::getParentIndex(const int body_index,
...
int MultiBodyTree::finalize() {
  ...
  for (int index = 0; index < num_bodies; index++) {
    ...
    rigid_body.m_parent_pos_parent_body_ref =
        joint.m_parent_pos_parent_child_ref;
    ...
    // Set joint Jacobians. Note that the dimension is always 3x1 here to avoid
    // variable sized
    // matrices.
...
int MultiBodyTree::getJointType(const int body_index,
                                JointType *joint_type) const {
  return m_impl->getJointType(body_index, joint_type);
}

int MultiBodyTree::getJointTypeStr(const int body_index,
...
int MultiBodyTree::setBodyFirstMassMoment(const int body_index,
                                          const vec3 &first_mass_moment) {
  return m_impl->setBodyFirstMassMoment(body_index, first_mass_moment);
}

int MultiBodyTree::setBodySecondMassMoment(const int body_index,
...
int MultiBodyTree::getBodyFirstMassMoment(const int body_index,
                                          vec3 *first_mass_moment) const {
  return m_impl->getBodyFirstMassMoment(body_index, first_mass_moment);
}

int MultiBodyTree::getBodySecondMassMoment(const int body_index,
...
int MultiBodyTree::addUserMoment(const int body_index,
