...
/// useNewBatchingKernel  is a rewritten kernel using just a single thread of
/// the warp, for experiments
...
b3Solver::b3Solver(cl_context ctx, cl_device_id device, cl_command_queue queue,
                   int pairCapacity)
    : m_context(ctx),
      m_device(device),
      m_queue(queue),
  ...
  m_numConstraints =
      new b3OpenCLArray<unsigned int>(ctx, queue, B3_SOLVER_N_CELLS);
...

  {
    ...
    m_solveFrictionKernel = b3OpenCLUtils::compileCLKernelFromString(
    ...
    m_solveContactKernel = b3OpenCLUtils::compileCLKernelFromString(
    ...
    m_contactToConstraintKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverSetupSource, "ContactToConstraintKernel", &pErrNum,
        solverSetupProg, additionalMacros);
    b3Assert(m_contactToConstraintKernel);

    m_setSortDataKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverSetup2Source, "SetSortDataKernel", &pErrNum,
        solverSetup2Prog, additionalMacros);
    b3Assert(m_setSortDataKernel);

    m_reorderContactKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverSetup2Source, "ReorderContactKernel", &pErrNum,
        solverSetup2Prog, additionalMacros);
    b3Assert(m_reorderContactKernel);

    m_copyConstraintKernel = b3OpenCLUtils::compileCLKernelFromString(
...

  {
    cl_program batchingProg = b3OpenCLUtils::compileCLProgramFromString(
        ...
        B3_BATCHING_PATH);
    // cl_program batchingProg = b3OpenCLUtils::compileCLProgramFromString( ctx,
    // device, 0, &pErrNum,additionalMacros, B3_BATCHING_PATH,true);
    b3Assert(batchingProg);

    m_batchingKernel = b3OpenCLUtils::compileCLKernelFromString(
  ...
  }
  {
    ...
    m_batchingKernelNew = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, batchKernelNewSource, "CreateBatchesNew", &pErrNum,
        batchingNewProg, additionalMacros);
    // m_batchingKernelNew = b3OpenCLUtils::compileCLKernelFromString( ctx,
    // device, batchKernelNewSource, "CreateBatchesBruteForce", &pErrNum,
    // batchingNewProg,additionalMacros );
...
static __inline void solveContact(b3GpuConstraint4& cs, const b3Vector3& posA,
                                  b3Vector3& linVelA, b3Vector3& angVelA,
                                  float invMassA,
                                  const b3Matrix3x3& invInertiaA,
                                  const b3Vector3& posB, b3Vector3& linVelB,
                                  b3Vector3& angVelB, float invMassB,
                                  const b3Matrix3x3& invInertiaB,
  ...
  for (int ic = 0; ic < 4; ic++) {
...

    {
      ...
      setLinearAndAngular((const b3Vector3&)cs.m_linear, (const b3Vector3&)r0,
                          (const b3Vector3&)r1, &linear, &angular0, &angular1);

      float rambdaDt =
          calcRelVel((const b3Vector3&)cs.m_linear,
                     (const b3Vector3&)-cs.m_linear, angular0, angular1,
...
static __inline void solveFriction(b3GpuConstraint4& cs, const b3Vector3& posA,
                                   b3Vector3& linVelA, b3Vector3& angVelA,
                                   float invMassA,
                                   const b3Matrix3x3& invInertiaA,
                                   const b3Vector3& posB, b3Vector3& linVelB,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    float rambdaDt = calcRelVel(linear, -linear, angular0, angular1, linVelA,
                                angVelA, linVelB, angVelB);
...
{
  SolveTask(b3AlignedObjectArray<b3RigidBodyData>& bodies,
            b3AlignedObjectArray<b3InertiaData>& shapes,
            b3AlignedObjectArray<b3GpuConstraint4>& constraints, int start,
            int nConstraints, int maxNumBatches,
            b3AlignedObjectArray<int>* wgUsedBodies, int curWgidx,
            b3AlignedObjectArray<int>* batchSizes, int cellIndex)
      : m_bodies(bodies),
        m_shapes(shapes),
  ...
  void run(int tIdx) {
    ...
    for (int ii = 0; ii < B3_MAX_NUM_BATCHES; ii++) {
      ...
      for (int jj = 0; jj < numInBatch; jj++) {
        ...
        if (!m_solveFriction) {
          ...
          solveContact<false>(
              m_constraints[i], (b3Vector3&)bodyA.m_pos,
              (b3Vector3&)bodyA.m_linVel, (b3Vector3&)bodyA.m_angVel,
              bodyA.m_invMass,
              (const b3Matrix3x3&)m_shapes[aIdx].m_invInertiaWorld,
              (b3Vector3&)bodyB.m_pos, (b3Vector3&)bodyB.m_linVel,
              (b3Vector3&)bodyB.m_angVel, bodyB.m_invMass,
              (const b3Matrix3x3&)m_shapes[bIdx].m_invInertiaWorld, maxRambdaDt,
              minRambdaDt);
        } else {
          float maxRambdaDt[4] = {FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX};
          float minRambdaDt[4] = {0.f, 0.f, 0.f, 0.f};
          ...
          solveFriction(m_constraints[i], (b3Vector3&)bodyA.m_pos,
                        (b3Vector3&)bodyA.m_linVel, (b3Vector3&)bodyA.m_angVel,
                        bodyA.m_invMass,
                        (const b3Matrix3x3&)m_shapes[aIdx].m_invInertiaWorld,
                        (b3Vector3&)bodyB.m_pos, (b3Vector3&)bodyB.m_linVel,
                        (b3Vector3&)bodyB.m_angVel, bodyB.m_invMass,
                        (const b3Matrix3x3&)m_shapes[bIdx].m_invInertiaWorld,
    ...
    {FLT_MAX,FLT_MAX,FLT_MAX,FLT_MAX};
                            ...
                            solveContact<false>( m_constraints[i],
    (b3Vector3&)bodyA.m_pos, (b3Vector3&)bodyA.m_linVel,
    (b3Vector3&)bodyA.m_angVel, bodyA.m_invMass, (const b3Matrix3x3
    &)m_shapes[aIdx].m_invInertiaWorld,
                                            (b3Vector3&)bodyB.m_pos,
    (b3Vector3&)bodyB.m_linVel, (b3Vector3&)bodyB.m_angVel, bodyB.m_invMass,
    (const b3Matrix3x3 &)m_shapes[bIdx].m_invInertiaWorld,
                                    maxRambdaDt, minRambdaDt );
                    }
                    else
    ...
    {FLT_MAX,FLT_MAX,FLT_MAX,FLT_MAX};
                            ...
                            solveFriction( m_constraints[i],
    (b3Vector3&)bodyA.m_pos, (b3Vector3&)bodyA.m_linVel,
    (b3Vector3&)bodyA.m_angVel, bodyA.m_invMass,(const b3Matrix3x3 &)
    m_shapes[aIdx].m_invInertiaWorld,
                                    (b3Vector3&)bodyB.m_pos,
    (b3Vector3&)bodyB.m_linVel, (b3Vector3&)bodyB.m_angVel,
    bodyB.m_invMass,(const b3Matrix3x3 &) m_shapes[bIdx].m_invInertiaWorld,
                                    maxRambdaDt, minRambdaDt );
...
void b3Solver::solveContactConstraintHost(
    b3OpenCLArray<b3RigidBodyData>* bodyBuf,
    b3OpenCLArray<b3InertiaData>* shapeBuf,
    b3OpenCLArray<b3GpuConstraint4>* constraint, void* additionalData, int n,
    int maxNumBatches, b3AlignedObjectArray<int>* batchSizes) {
#if 0
	{	
  ...
  if (useBatches) {
    for (int iter = 0; iter < m_nIterations; iter++) {
      for (int cellBatch = 0; cellBatch < B3_SOLVER_N_BATCHES; cellBatch++) {
        ...
        for (int wgIdx = 0; wgIdx < numWorkgroups; wgIdx++) {
          int zIdx =
              (wgIdx / ((nSplitX * nSplitY) / 4)) * 2 + ((cellBatch & 4) >> 2);
          int remain = (wgIdx % ((nSplitX * nSplitY) / 4));
          int yIdx = (remain / (nSplitX / 2)) * 2 + ((cellBatch & 2) >> 1);
          int xIdx = (remain % (nSplitX / 2)) * 2 + (cellBatch & 1);
          int cellIdx = xIdx + yIdx * nSplitX + zIdx * (nSplitX * nSplitY);

          if (numConstraintsHost[cellIdx] == 0) continue;

          // printf("wgIdx %d: xIdx=%d, yIdx=%d, zIdx=%d, cellIdx=%d, cell Batch
          // %d\n",wgIdx,xIdx,yIdx,zIdx,cellIdx,cellBatch);
          ...
          if (iter == 0) {
            ...
            // printf("cellBatch=%d, wgIdx=%d, #constraints in
          ...
          //				const int end = start +
          //numConstraintsInCell;

          SolveTask task(bodyNative, shapeNative, constraintNative, start,
                         numConstraintsInCell, maxNumBatches, usedBodies, wgIdx,
                         batchSizes, cellIdx);
    ...
    for (int iter = 0; iter < m_nIterations; iter++) {
      for (int cellBatch = 0; cellBatch < B3_SOLVER_N_BATCHES; cellBatch++) {
        ...
        for (int wgIdx = 0; wgIdx < numWorkgroups; wgIdx++) {
          ...
          SolveTask task(bodyNative, shapeNative, constraintNative, start,
                         numConstraintsInCell, maxNumBatches, 0, 0, batchSizes,
                         cellIdx);
...
void checkConstraintBatch(const b3OpenCLArray<b3RigidBodyData>* bodyBuf,
                          ...
                          b3OpenCLArray<unsigned int>* m_numConstraints,
                          b3OpenCLArray<unsigned int>* m_offsets, int batchId) {
...
void b3Solver::solveContactConstraint(
...

    {
      ...
      for (int iter = 0; iter < m_nIterations; iter++) {
        for (int ib = 0; ib < B3_SOLVER_N_BATCHES; ib++) {
          if (verify) {
            checkConstraintBatch(bodyBuf, shapeBuf, constraint,
                                 m_numConstraints, m_offsets, ib);
          }
...
void b3Solver::convertToConstraints(
    const b3OpenCLArray<b3RigidBodyData>* bodyBuf,
    const b3OpenCLArray<b3InertiaData>* shapeBuf,
    b3OpenCLArray<b3Contact4>* contactsIn,
    b3OpenCLArray<b3GpuConstraint4>* contactCOut, void* additionalData,
...

  {
    ...
    if (gConvertConstraintOnCpu) {
      ...
      for (int gIdx = 0; gIdx < nContacts; gIdx++) {
        ...
        setConstraint4(posA, linVelA, angVelA, invMassA, invInertiaA, posB,
                       linVelB, angVelB, invMassB, invInertiaB, &gContact[gIdx],
                       cdata.m_dt, cdata.m_positionDrift,
                       cdata.m_positionConstraintCoeff, &cs);
      ...
      b3BufferInfoCL bInfo[] = {b3BufferInfoCL(contactsIn->getBufferCL()),
                                b3BufferInfoCL(bodyBuf->getBufferCL()),
                                b3BufferInfoCL(shapeBuf->getBufferCL()),
...
void b3Solver::sortContacts(  const b3OpenCLArray<b3RigidBodyData>* bodyBuf,
                        b3OpenCLArray<b3Contact4>* contactsIn, void*
additionalData,
...
m_numConstraints;//BufferUtils::map<TYPE_CL, false>( data->m_device, &countsHost
);
                b3OpenCLArray<unsigned int>* offsetsNative =
m_offsets;//BufferUtils::map<TYPE_CL, false>( data->m_device, &offsetsHost );
                ...
                {	//	2. set cell idx
                        ...
                        cdata.m_scale =
1.f/(N_OBJ_PER_SPLIT*cfg.m_averageExtent);
                        cdata.m_nSplit = B3_SOLVER_N_SPLIT;


                        b3BufferInfoCL bInfo[] = { b3BufferInfoCL(
contactsIn->getBufferCL() ), b3BufferInfoCL( bodyBuf->getBufferCL() ),
b3BufferInfoCL( m_sortDataBuffer->getBufferCL() ) };
...
sizeof(bInfo)/sizeof(b3BufferInfoCL) );
                ...
                {	//	4. find entries
                        m_search->execute( *m_sortDataBuffer, nContacts,
*countsNative, B3_SOLVER_N_SPLIT*B3_SOLVER_N_SPLIT, b3BoundSearchCL::COUNT);

                        m_scan->execute( *countsNative, *offsetsNative,
...
//			b3OpenCLArray<b3Contact4>* out =
BufferUtils::map<TYPE_CL, false>( data->m_device, contactsIn );	//
copying contacts to this buffer

                        {
                                ...
                                b3BufferInfoCL bInfo[] = { b3BufferInfoCL(
contactsIn->getBufferCL() ), b3BufferInfoCL( m_contactBuffer->getBufferCL() ),
b3BufferInfoCL( m_sortDataBuffer->getBufferCL() ) };
...
*/
void b3Solver::batchContacts(b3OpenCLArray<b3Contact4>* contacts, int nContacts,
                             b3OpenCLArray<unsigned int>* nNative,
                             b3OpenCLArray<unsigned int>* offsetsNative,
                             int staticIdx) {
  int numWorkItems = 64 * B3_SOLVER_N_CELLS;
  {
...

    {
      ...
      cl_kernel k =
          useNewBatchingKernel ? m_batchingKernelNew : m_batchingKernel;
...
#ifdef BATCH_DEBUG
    aaaa b3Contact4* hostContacts = new b3Contact4[nContacts];
