...
static const char* solveConstraintRowsCL =
    ...
    "#define MOTIONCLAMP 100000 //unused, for debugging/safety in case "
    ...
    "float4 mtMul3(float4 a, Matrix3x3 b)\n"
    "{\n"
    "	float4 colx = mymake_float4(b.m_row[0].x, b.m_row[1].x, b.m_row[2].x, "
    "0);\n"
    "	float4 coly = mymake_float4(b.m_row[0].y, b.m_row[1].y, b.m_row[2].y, "
    "0);\n"
    "	float4 colz = mymake_float4(b.m_row[0].z, b.m_row[1].z, b.m_row[2].z, "
    "0);\n"
    "	float4 ans;\n"
    "	ans.x = dot3F4( a, colx );\n"
    "	ans.y = dot3F4( a, coly );\n"
    ...
    "	//float4		m_contactNormal2;//usually m_contactNormal2 == "
    "-m_contactNormal\n"
    ...
    "}\n"
    "__inline void internalApplyImpulse(__global b3GpuSolverBody* body,  "
    "float4 linearComponent, float4 angularComponent,float impulseMagnitude)\n"
    "{\n"
    ...
    "}\n"
    "void resolveSingleConstraintRowGeneric(__global b3GpuSolverBody* body1, "
    "__global b3GpuSolverBody* body2, __global b3SolverConstraint* c)\n"
    "{\n"
    "	float deltaImpulse = c->m_rhs-c->m_appliedImpulse*c->m_cfm;\n"
    "	float deltaVel1Dotn	=	"
    "dot3F4(c->m_contactNormal,body1->m_deltaLinearVelocity) 	+ "
    "dot3F4(c->m_relpos1CrossNormal,body1->m_deltaAngularVelocity);\n"
    "	float deltaVel2Dotn	=	"
    "-dot3F4(c->m_contactNormal,body2->m_deltaLinearVelocity) + "
    "dot3F4(c->m_relpos2CrossNormal,body2->m_deltaAngularVelocity);\n"
    ...
    "	}\n"
    "	internalApplyImpulse(body1,c->m_contactNormal*body1->m_invMass,c->m_"
    "angularComponentA,deltaImpulse);\n"
    "	internalApplyImpulse(body2,-c->m_contactNormal*body2->m_invMass,c->m_"
    "angularComponentB,deltaImpulse);\n"
    "}\n"
    "__kernel void solveJointConstraintRows(__global b3GpuSolverBody* "
    "solverBodies,\n"
    "					  __global b3BatchConstraint* "
    "batchConstraints,\n"
    "					  	__global b3SolverConstraint* "
    "rows,\n"
    "						__global unsigned int* "
    "numConstraintRowsInfo1, \n"
    "						__global unsigned int* "
    "rowOffsets,\n"
    "						__global "
    "b3GpuGenericConstraint* constraints,\n"
    "						int batchOffset,\n"
    ...
    "	if "
    "(constraints[originalConstraintIndex].m_flags&B3_CONSTRAINT_FLAG_ENABLED)"
    "\n"
    "	{\n"
    "		int numConstraintRows = "
    "numConstraintRowsInfo1[originalConstraintIndex];\n"
    "		int rowOffset = rowOffsets[originalConstraintIndex];\n"
    "		for (int jj=0;jj<numConstraintRows;jj++)\n"
    "		{\n"
    "			__global b3SolverConstraint* constraint = "
    "&rows[rowOffset+jj];\n"
    "			"
    "resolveSingleConstraintRowGeneric(&solverBodies[constraint->m_"
    "solverBodyIdA],&solverBodies[constraint->m_solverBodyIdB],constraint);\n"
    "		}\n"
    "	}\n"
    "};\n"
    "__kernel void initSolverBodies(__global b3GpuSolverBody* "
    "solverBodies,__global b3RigidBodyCL* bodiesCL, int numBodies)\n"
    "{\n"
    ...
    "(float4)(bodyCL->m_invMass,bodyCL->m_invMass,bodyCL->m_invMass,0.f);\n"
    ...
    "}\n"
    "__kernel void breakViolatedConstraintsKernel(__global "
    "b3GpuGenericConstraint* constraints, __global unsigned int* "
    "numConstraintRows, __global unsigned int* rowOffsets, __global "
    "b3SolverConstraint* rows, int numConstraints)\n"
    "{\n"
    ...
    "constraints[cid].m_breakingImpulseThreshold;\n"
    "			if (fabs(rows[rowIndex].m_appliedImpulse) >= "
    "breakingThreshold)\n"
    "			{\n"
    "				constraints[cid].m_flags =0;//&= "
    "~B3_CONSTRAINT_FLAG_ENABLED;\n"
    "			}\n"
    "		}\n"
    "	}\n"
    "}\n"
    "__kernel void getInfo1Kernel(__global unsigned int* infos, __global "
    "b3GpuGenericConstraint* constraints, int numConstraints)\n"
    "{\n"
    "	int i = get_global_id(0);\n"
    "	if (i>=numConstraints)\n"
    "		return;\n"
    ...
    "	}\n"
    "}\n"
    "__kernel void initBatchConstraintsKernel(__global unsigned int* "
    "numConstraintRows, __global unsigned int* rowOffsets, \n"
    "										"
    ...
    "	batchConstraints[i].m_bodyAPtrAndSignBit = bodies[rbA].m_invMass != 0.f "
    "? rbA : -rbA;\n"
    "	batchConstraints[i].m_bodyBPtrAndSignBit = bodies[rbB].m_invMass != 0.f "
    "? rbB : -rbB;\n"
    ...
    "	int rowskip;\n"
    "	// right hand sides of the equation J*v = c + cfm * lambda. cfm is "
    "the\n"
    ...
    "void	getSkewSymmetricMatrix(float4 vecIn, __global float4* "
    "v0,__global float4* v1,__global float4* v2)\n"
    "{\n"
    "	*v0 = (float4)(0.		,-vecIn.z		"
    ",vecIn.y,0.f);\n"
    ...
    "}\n"
    "void getInfo2Point2Point(__global b3GpuGenericConstraint* "
    "constraint,b3GpuConstraintInfo2* info,__global b3RigidBodyCL* bodies)\n"
    "{\n"
    ...
    "		// anchor points in global coordinates with respect to body "
    "PORs.\n"
    "   \n"
    "    // set jacobian\n"
    ...
    "	{\n"
    "		__global float4* angular0 = (__global "
    "float4*)(info->m_J1angularAxis);\n"
    "		__global float4* angular1 = (__global "
    "float4*)(info->m_J1angularAxis+info->rowskip);\n"
    "		__global float4* angular2 = (__global "
    "float4*)(info->m_J1angularAxis+2*info->rowskip);\n"
    "		float4 a1neg = -a1;\n"
    ...
    "	//	float4 a2n = -a2;\n"
    "		__global float4* angular0 = (__global "
    "float4*)(info->m_J2angularAxis);\n"
    "		__global float4* angular1 = (__global "
    "float4*)(info->m_J2angularAxis+info->rowskip);\n"
    "		__global float4* angular2 = (__global "
    "float4*)(info->m_J2angularAxis+2*info->rowskip);\n"
    ...
    "	return s;\n"
    "}\n"
    "void calculateDiffAxisAngleQuaternion( Quaternion orn0,Quaternion "
    "orn1a,float4* axis,float* angle)\n"
    "{\n"
    ...
    "		*axis /= sqrt(len);\n"
    "}\n"
    "void getInfo2FixedOrientation(__global b3GpuGenericConstraint* "
    "constraint,b3GpuConstraintInfo2* info,__global b3RigidBodyCL* bodies, int "
    "start_row)\n"
    "{\n"
    ...
    "	\n"
    "	calculateDiffAxisAngleQuaternion(constraint->m_relTargetAB,qrelCur,&"
    "diff,&angle);\n"
    "	diff*=-angle;\n"
    "		\n"
    ...
    "	\n"
    "}\n"
    "__kernel void writeBackVelocitiesKernel(__global b3RigidBodyCL* "
    "bodies,__global b3GpuSolverBody* solverBodies,int numBodies)\n"
    "{\n"
    "	int i = get_global_id(0);\n"
    "	if (i>=numBodies)\n"
    "		return;\n"
    ...
    "	{\n"
    "//		if "
    "(length(solverBodies[i].m_deltaLinearVelocity)<MOTIONCLAMP)\n"
    "		{\n"
    "			bodies[i].m_linVel += "
    "solverBodies[i].m_deltaLinearVelocity;\n"
    "		}\n"
    "//		if "
    "(length(solverBodies[i].m_deltaAngularVelocity)<MOTIONCLAMP)\n"
    "		{\n"
    "			bodies[i].m_angVel += "
    ...
    "	}\n"
    "}\n"
    "__kernel void getInfo2Kernel(__global b3SolverConstraint* "
    "solverConstraintRows, \n"
    "							__global unsigned int* "
    "infos, \n"
    "							__global unsigned int* "
    ...
    "b3GpuGenericConstraint* constraints, \n"
    "							__global "
    "b3BatchConstraint* batchConstraints, \n"
    "							__global b3RigidBodyCL* "
    "bodies,\n"
    "							__global BodyInertia* "
    "inertias,\n"
    "							__global "
    "b3GpuSolverBody* solverBodies,\n"
    "							float timeStep,\n"
    "							float globalErp,\n"
    "							float globalCfm,\n"
    "							float globalDamping,\n"
    ...
    "	int info1 = infos[i];\n"
    "			\n"
    "	__global b3SolverConstraint* currentConstraintRow = "
    "&solverConstraintRows[constraintRowOffsets[i]];\n"
    "	__global b3GpuGenericConstraint* constraint = &constraints[i];\n"
    ...
    "//		if "
    "(overrideNumSolverIterations>m_maxOverrideNumSolverIterations)\n"
    ...
    "			currentConstraintRow[j].m_angularComponentA = "
    "(float4)(0,0,0,0);\n"
    "			currentConstraintRow[j].m_angularComponentB = "
    "(float4)(0,0,0,0);\n"
    "			currentConstraintRow[j].m_appliedImpulse = 0.f;\n"
    "			currentConstraintRow[j].m_appliedPushImpulse = 0.f;\n"
    "			currentConstraintRow[j].m_cfm = 0.f;\n"
    "			currentConstraintRow[j].m_contactNormal = "
    "(float4)(0,0,0,0);\n"
    ...
    "0;\n"
    "			currentConstraintRow[j].m_relpos1CrossNormal = "
    "(float4)(0,0,0,0);\n"
    "			currentConstraintRow[j].m_relpos2CrossNormal = "
    "(float4)(0,0,0,0);\n"
    ...
    "solverBodyIdA;\n"
    "			currentConstraintRow[j].m_solverBodyIdB = "
    "solverBodyIdB;\n"
    "			currentConstraintRow[j].m_overrideNumSolverIterations = "
    "overrideNumSolverIterations;		\n"
    "		}\n"
    ...
    "		int rowskip = sizeof(b3SolverConstraint)/sizeof(float);//check "
    "this\n"
    "		\n"
    "		b3GpuConstraintInfo2 info2;\n"
    "		info2.fps = 1.f/timeStep;\n"
    "		info2.erp = globalErp;\n"
    "		info2.m_J1linearAxisFloat4 = "
    "&currentConstraintRow->m_contactNormal;\n"
    "		info2.m_J1angularAxisFloat4 = "
    "&currentConstraintRow->m_relpos1CrossNormal;\n"
    "		info2.m_J2linearAxisFloat4 = 0;\n"
    "		info2.m_J2angularAxisFloat4 = "
    "&currentConstraintRow->m_relpos2CrossNormal;\n"
    "		info2.rowskip = "
    "sizeof(b3SolverConstraint)/sizeof(float);//check this\n"
    "		///the size of b3SolverConstraint needs be a multiple of "
    "float\n"
    "//		b3Assert(info2.rowskip*sizeof(float)== "
    ...
    "		for ( j=0;j<info1;j++)\n"
    "		{\n"
    "			__global b3SolverConstraint* solverConstraint = "
    "&currentConstraintRow[j];\n"
    "			if "
    "(solverConstraint->m_upperLimit>=constraint->m_breakingImpulseThreshold)\n"
    "			{\n"
    "				solverConstraint->m_upperLimit = "
    "constraint->m_breakingImpulseThreshold;\n"
    "			}\n"
    "			if "
    ...
    "							\n"
    "			Matrix3x3 invInertiaWorldA= "
    "inertias[constraint->m_rbA].m_invInertiaWorld;\n"
    "			{\n"
    "				//float4 angularFactorA(1,1,1);\n"
    "				float4 ftorqueAxis1 = "
    "solverConstraint->m_relpos1CrossNormal;\n"
    "				solverConstraint->m_angularComponentA = "
    "mtMul1(invInertiaWorldA,ftorqueAxis1);//*angularFactorA;\n"
    "			}\n"
    "						\n"
    "			Matrix3x3 invInertiaWorldB= "
    "inertias[constraint->m_rbB].m_invInertiaWorld;\n"
    "			{\n"
    "				float4 ftorqueAxis2 = "
    "solverConstraint->m_relpos2CrossNormal;\n"
    "				solverConstraint->m_angularComponentB = "
    "mtMul1(invInertiaWorldB,ftorqueAxis2);//"
    "*constraint->m_rbB.getAngularFactor();\n"
    "			}\n"
    "			{\n"
    "				//it is ok to use "
    "solverConstraint->m_contactNormal instead of "
    "-solverConstraint->m_contactNormal\n"
    "				//because it gets multiplied iMJlB\n"
    "				float4 iMJlA = "
    "solverConstraint->m_contactNormal*rbA->m_invMass;\n"
    "				float4 iMJaA = "
    "mtMul3(solverConstraint->m_relpos1CrossNormal,invInertiaWorldA);\n"
    "				float4 iMJlB = "
    "solverConstraint->m_contactNormal*rbB->m_invMass;//sign of normal?\n"
    "				float4 iMJaB = "
    "mtMul3(solverConstraint->m_relpos2CrossNormal,invInertiaWorldB);\n"
    "				float sum = "
    "dot3F4(iMJlA,solverConstraint->m_contactNormal);\n"
    "				sum += "
    "dot3F4(iMJaA,solverConstraint->m_relpos1CrossNormal);\n"
    "				sum += "
    "dot3F4(iMJlB,solverConstraint->m_contactNormal);\n"
    "				sum += "
    "dot3F4(iMJaB,solverConstraint->m_relpos2CrossNormal);\n"
    "				float fsum = fabs(sum);\n"
    "				if (fsum>FLT_EPSILON)\n"
    "				{\n"
    ...
    "				float rel_vel;\n"
    "				float vel1Dotn = "
    "dot3F4(solverConstraint->m_contactNormal,rbA->m_linVel) + "
    "dot3F4(solverConstraint->m_relpos1CrossNormal,rbA->m_angVel);\n"
    "				float vel2Dotn = "
    "-dot3F4(solverConstraint->m_contactNormal,rbB->m_linVel) + "
    "dot3F4(solverConstraint->m_relpos2CrossNormal,rbB->m_angVel);\n"
    ...
    "				float	velocityImpulse = velocityError "
    "*solverConstraint->m_jacDiagABInv;\n"
