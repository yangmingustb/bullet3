...
void b3GpuGenericConstraint::getInfo1(unsigned int* info,
...
void getInfo2Point2Point(b3GpuGenericConstraint* constraint,
                         b3GpuConstraintInfo2* info,
                         const b3RigidBodyData* bodies) {
  b3Transform trA;
  trA.setIdentity();
  ...
  for (j = 0; j < 3; j++) {
    info->m_constraintError[j * info->rowskip] =
        k * (a2[j] + trB.getOrigin()[j] - a1[j] - trA.getOrigin()[j]);
...
void b3GpuGenericConstraint::getInfo2(b3GpuConstraintInfo2* info,
                                      const b3RigidBodyData* bodies) {
  switch (m_constraintType) {
    case B3_GPU_POINT2POINT_CONSTRAINT_TYPE: {
