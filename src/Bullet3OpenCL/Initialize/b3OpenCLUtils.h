...
/// CL Context optionally takes a GL context. This is a generic type because we
/// don't really want this code
/// to have to understand GL types. It is a HGLRC in _WIN32 or a GLXContext
/// otherwise.
cl_context b3OpenCLUtils_createContextFromType(cl_device_type deviceType,
                                               cl_int* pErrNum, void* pGLCtx,
                                               void* pGLDC,
...
cl_program b3OpenCLUtils_compileCLProgramFromString(
    cl_context clContext, cl_device_id device, const char* kernelSource,
    cl_int* pErrNum, const char* additionalMacros,
    const char* srcFileNameForCaching, bool disableBinaryCaching);

// the following optional APIs provide access using specific platform
// information
...
cl_context b3OpenCLUtils_createContextFromPlatform(cl_platform_id platform,
                                                   cl_device_type deviceType,
                                                   cl_int* pErrNum,
                                                   void* pGLCtx, void* pGLDC,
                                                   int preferredDeviceIndex,
                                                   int preferredPlatformIndex);
...
struct b3OpenCLUtils {
  /// CL Context optionally takes a GL context. This is a generic type because
  /// we don't really want this code
  /// to have to understand GL types. It is a HGLRC in _WIN32 or a GLXContext
  /// otherwise.
  static inline cl_context createContextFromType(
      cl_device_type deviceType, cl_int* pErrNum, void* pGLCtx = 0,
      void* pGLDC = 0, int preferredDeviceIndex = -1,
      int preferredPlatformIndex = -1, cl_platform_id* platformId = 0) {
    return b3OpenCLUtils_createContextFromType(
        deviceType, pErrNum, pGLCtx, pGLDC, preferredDeviceIndex,
        preferredPlatformIndex, platformId);
  }
  ...
  static inline cl_kernel compileCLKernelFromString(
      cl_context clContext, cl_device_id device, const char* kernelSource,
      const char* kernelName, cl_int* pErrNum = 0, cl_program prog = 0,
      const char* additionalMacros = "") {
    return b3OpenCLUtils_compileCLKernelFromString(
        clContext, device, kernelSource, kernelName, pErrNum, prog,
        additionalMacros);
  }

  // optional
  static inline cl_program compileCLProgramFromString(
      cl_context clContext, cl_device_id device, const char* kernelSource,
      cl_int* pErrNum = 0, const char* additionalMacros = "",
      const char* srcFileNameForCaching = 0,
      bool disableBinaryCaching = false) {
    return b3OpenCLUtils_compileCLProgramFromString(
        clContext, device, kernelSource, pErrNum, additionalMacros,
        srcFileNameForCaching, disableBinaryCaching);
  }
  ...
  static void getPlatformInfo(cl_platform_id platform,
  ...
  static inline cl_context createContextFromPlatform(
      cl_platform_id platform, cl_device_type deviceType, cl_int* pErrNum,
      void* pGLCtx = 0, void* pGLDC = 0, int preferredDeviceIndex = -1,
      int preferredPlatformIndex = -1) {
    return b3OpenCLUtils_createContextFromPlatform(
        platform, deviceType, pErrNum, pGLCtx, pGLDC, preferredDeviceIndex,
        preferredPlatformIndex);
  }
