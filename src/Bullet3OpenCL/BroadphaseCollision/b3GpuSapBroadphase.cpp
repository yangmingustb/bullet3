...
b3GpuSapBroadphase::b3GpuSapBroadphase(cl_context ctx, cl_device_id device,
                                       cl_command_queue q,
                                       b3GpuSapKernelType kernelType)
    : m_context(ctx),
      m_device(device),
      m_queue(q),
  ...
  b3Assert(m_context);
  b3Assert(m_device);
  cl_program sapProg = b3OpenCLUtils::compileCLProgramFromString(
      m_context, m_device, sapSrc, &errNum, "", B3_BROADPHASE_SAP_PATH);
  ...
  switch (kernelType) {
    ...
    case B3_GPU_SAP_KERNEL_BRUTE_FORCE_GPU: {
      m_sapKernel = b3OpenCLUtils::compileCLKernelFromString(
          m_context, m_device, sapSrc, "computePairsKernelBruteForce", &errNum,
          sapProg);
      break;
    }
    ...
    case B3_GPU_SAP_KERNEL_ORIGINAL: {
      m_sapKernel = b3OpenCLUtils::compileCLKernelFromString(
          m_context, m_device, sapSrc, "computePairsKernelOriginal", &errNum,
          sapProg);
      break;
    }
    case B3_GPU_SAP_KERNEL_BARRIER: {
      m_sapKernel = b3OpenCLUtils::compileCLKernelFromString(
          m_context, m_device, sapSrc, "computePairsKernelBarrier", &errNum,
          sapProg);
      break;
    }
    case B3_GPU_SAP_KERNEL_LOCAL_SHARED_MEMORY: {
      m_sapKernel = b3OpenCLUtils::compileCLKernelFromString(
          m_context, m_device, sapSrc, "computePairsKernelLocalSharedMemory",
          &errNum, sapProg);
      break;
    }
    ...
    default: {
      m_sapKernel = b3OpenCLUtils::compileCLKernelFromString(
          ...
          &errNum, sapProg);
      b3Error(
          "Unknown 3D GPU SAP provided, fallback to "
          "computePairsKernelLocalSharedMemory");
    }
  };

  m_sap2Kernel = b3OpenCLUtils::compileCLKernelFromString(
      m_context, m_device, sapSrc, "computePairsKernelTwoArrays", &errNum,
      sapProg);
  b3Assert(errNum == CL_SUCCESS);

  m_prepareSumVarianceKernel = b3OpenCLUtils::compileCLKernelFromString(
      m_context, m_device, sapSrc, "prepareSumVarianceKernel", &errNum,
      sapProg);
  b3Assert(errNum == CL_SUCCESS);

  m_flipFloatKernel = b3OpenCLUtils::compileCLKernelFromString(
      m_context, m_device, sapSrc, "flipFloatKernel", &errNum, sapProg);

  m_copyAabbsKernel = b3OpenCLUtils::compileCLKernelFromString(
      m_context, m_device, sapSrc, "copyAabbsKernel", &errNum, sapProg);

  m_scatterKernel = b3OpenCLUtils::compileCLKernelFromString(
      m_context, m_device, sapSrc, "scatterKernel", &errNum, sapProg);
...
static bool TestAabbAgainstAabb2(const b3Vector3& aabbMin1,
                                 const b3Vector3& aabbMax1,
                                 const b3Vector3& aabbMin2,
                                 const b3Vector3& aabbMax2) {
  bool overlap = true;
  overlap =
      ...
      for (int buf = 0; buf < 2; buf++) {
        ...
        if (buf == m_currentBuffer) {
          for (int i = 0; i < totalNumAabbs; i++) {
            m_sortedAxisCPU[axis][buf][i * 2].m_key =
                FloatFlip(m_allAabbsCPU[i].m_min[axis]) - 1;
            m_sortedAxisCPU[axis][buf][i * 2].m_value = i * 2;
            m_sortedAxisCPU[axis][buf][i * 2 + 1].m_key =
                FloatFlip(m_allAabbsCPU[i].m_max[axis]) + 1;
...
void b3GpuSapBroadphase::calculateOverlappingPairsHostIncremental3Sap() {
  ...
  if (0) {
    {
      printf("ab[40].min=%f,%f,%f,ab[40].max=%f,%f,%f\n",
             m_allAabbsCPU[40].m_min[0], m_allAabbsCPU[40].m_min[1],
             m_allAabbsCPU[40].m_min[2], m_allAabbsCPU[40].m_max[0],
             m_allAabbsCPU[40].m_max[1], m_allAabbsCPU[40].m_max[2]);
    }

    {
      printf("ab[53].min=%f,%f,%f,ab[53].max=%f,%f,%f\n",
             m_allAabbsCPU[53].m_min[0], m_allAabbsCPU[53].m_min[1],
             m_allAabbsCPU[53].m_min[2], m_allAabbsCPU[53].m_max[0],
             m_allAabbsCPU[53].m_max[1], m_allAabbsCPU[53].m_max[2]);
    }

    {
      ...
      {
        int overlap =
            TestAabbAgainstAabb2((const b3Vector3&)m_allAabbsCPU[40].m_min,
                                 (const b3Vector3&)m_allAabbsCPU[40].m_max,
                                 (const b3Vector3&)m_allAabbsCPU[53].m_min,
                                 (const b3Vector3&)m_allAabbsCPU[53].m_max);
        printf("overlap=%d\n", overlap);
      }

      if (preAabbs.size()) {
        int prevOverlap =
            TestAabbAgainstAabb2((const b3Vector3&)preAabbs[40].m_min,
                                 (const b3Vector3&)preAabbs[40].m_max,
                                 (const b3Vector3&)preAabbs[53].m_min,
                                 (const b3Vector3&)preAabbs[53].m_max);
  ...
  if (0) {
    for (int i = 0; i < m_allAabbsCPU.size(); i++) {
      // printf("aabb[%d] min=%f,%f,%f
      // max=%f,%f,%f\n",i,m_allAabbsCPU[i].m_min[0],m_allAabbsCPU[i].m_min[1],m_allAabbsCPU[i].m_min[2],
      // m_allAabbsCPU[i].m_max[0],m_allAabbsCPU[i].m_max[1],m_allAabbsCPU[i].m_max[2]);
    }
...

  {
    ...
    for (int i = 0; i < totalNumAabbs; i++) {
      ...
      for (int axis = 0; axis < 3; axis++) {
        ...
        m_sortedAxisCPU[axis][m_currentBuffer][i * 2 + 1].m_key =
            keyMax[axis] + 1;
  ...
  m_computePairsIncremental3dSapKernel,"m_computePairsIncremental3dSapKernel");
  ...
  */
  {
    ...
    for (int i = 0; i < numObjects; i++) {
      ...
      for (int axis = 0; axis < 3; axis++) {
        ...
        unsigned int curMinIndex =
            m_objectMinMaxIndexCPU[axis][m_currentBuffer][i].x;
        unsigned int curMaxIndex =
            m_objectMinMaxIndexCPU[axis][m_currentBuffer][i].y;
        unsigned int prevMinIndex =
            m_objectMinMaxIndexCPU[axis][1 - m_currentBuffer][i].x;
        int dmin = curMinIndex - prevMinIndex;

        unsigned int prevMaxIndex =
            m_objectMinMaxIndexCPU[axis][1 - m_currentBuffer][i].y;
          ...
          if (dmin != 0) {
            ...
            for (int j = prevMinIndex; j != curMinIndex; j += stepMin) {
              ...
              if (otherIndex != i) {
                ...
                if (otherIsMax) {
                  // bool overlap = TestAabbAgainstAabb2((const
                  // b3Vector3&)m_allAabbsCPU[i].m_min, (const
                  // b3Vector3&)m_allAabbsCPU[i].m_max,(const
                  // b3Vector3&)m_allAabbsCPU[otherIndex].m_min,(const
                  // b3Vector3&)m_allAabbsCPU[otherIndex].m_max);
                  // bool prevOverlap = TestAabbAgainstAabb2((const
                  // b3Vector3&)preAabbs[i].m_min, (const
                  // b3Vector3&)preAabbs[i].m_max,(const
                  // b3Vector3&)preAabbs[otherIndex].m_min,(const
                  // b3Vector3&)preAabbs[otherIndex].m_max);

                  bool overlap = true;

                  for (int ax = 0; ax < 3; ax++) {
                    if ((m_objectMinMaxIndexCPU[ax][m_currentBuffer][i].x >
                         m_objectMinMaxIndexCPU[ax][m_currentBuffer][otherIndex]
                             .y) ||
                        (m_objectMinMaxIndexCPU[ax][m_currentBuffer][i].y <
                         m_objectMinMaxIndexCPU[ax][m_currentBuffer][otherIndex]
                             .x))
                      overlap = false;
                  }
                  ...
                  for (int ax = 0; ax < 3; ax++) {
                    if ((m_objectMinMaxIndexCPU[ax][1 - m_currentBuffer][i].x >
                         m_objectMinMaxIndexCPU[ax][1 - m_currentBuffer]
                                               [otherIndex]
                                                   .y) ||
                        (m_objectMinMaxIndexCPU[ax][1 - m_currentBuffer][i].y <
                         m_objectMinMaxIndexCPU[ax][1 - m_currentBuffer]
                                               [otherIndex]
                                                   .x))
                      prevOverlap = false;
...
void b3GpuSapBroadphase::calculateOverlappingPairsHost(int maxPairs) {
  ...
  int axis = 0;
  {
    ...
    for (int i = 0; i < numRigidBodies; i++) {
      ...
      b3Vector3 maxAabb =
          b3MakeVector3(aabb.m_max[0], aabb.m_max[1], aabb.m_max[2]);
      b3Vector3 minAabb =
          b3MakeVector3(aabb.m_min[0], aabb.m_min[1], aabb.m_min[2]);
...

  {
    ...
    for (int i = 0; i < numSmallAabbs; i++) {
      ...
      for (int j = i + 1; j < numSmallAabbs; j++) {
        ...
        if (TestAabbAgainstAabb2(
                (b3Vector3&)smallAabbi.m_min, (b3Vector3&)smallAabbi.m_max,
...

  {
...

    {
      ...
      if (numLargeAabbs && numSmallAabbs) {
        ...
        //@todo: use actual maximum work item sizes of the device instead of
        //hardcoded values
    ...
    if (f) {
      ...
      pairCount.setFromOpenCLBuffer(launcher.m_arrays[2]->getBufferCL(),
                                    numElements);
      ...
      tmpGpuPairs.setFromOpenCLBuffer(launcher.m_arrays[1]->getBufferCL(),
                                      numPairs);
...
void b3GpuSapBroadphase::writeAabbsToGpu() {
  ...
  m_allAabbsGPU.copyFromHost(m_allAabbsCPU);  // might not be necessary, the
                                              // 'setupGpuAabbsFull' already
                                              // takes care of this
}

void b3GpuSapBroadphase::createLargeProxy(const b3Vector3& aabbMin,
                                          const b3Vector3& aabbMax, int userPtr,
...
void b3GpuSapBroadphase::createProxy(const b3Vector3& aabbMin,
                                     const b3Vector3& aabbMax, int userPtr,
                                     int collisionFilterGroup,
                                     int collisionFilterMask) {
  int index = userPtr;
...
int b3GpuSapBroadphase::getNumOverlap() { return m_overlappingPairs.size(); }
