...
b3GpuParallelLinearBvh::b3GpuParallelLinearBvh(cl_context context,
                                               cl_device_id device,
  ...
  m_parallelLinearBvhProgram = b3OpenCLUtils::compileCLProgramFromString(
      context, device, kernelSource, &error, additionalMacros, CL_PROGRAM_PATH);
  b3Assert(m_parallelLinearBvhProgram);

  m_separateAabbsKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "separateAabbs", &error,
      m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_separateAabbsKernel);
  m_findAllNodesMergedAabbKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "findAllNodesMergedAabb", &error,
      m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_findAllNodesMergedAabbKernel);
  m_assignMortonCodesAndAabbIndiciesKernel =
      b3OpenCLUtils::compileCLKernelFromString(
          context, device, kernelSource, "assignMortonCodesAndAabbIndicies",
          &error, m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_assignMortonCodesAndAabbIndiciesKernel);

  m_computeAdjacentPairCommonPrefixKernel =
      b3OpenCLUtils::compileCLKernelFromString(
          context, device, kernelSource, "computeAdjacentPairCommonPrefix",
          &error, m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_computeAdjacentPairCommonPrefixKernel);
  m_buildBinaryRadixTreeLeafNodesKernel =
      b3OpenCLUtils::compileCLKernelFromString(
          context, device, kernelSource, "buildBinaryRadixTreeLeafNodes",
          &error, m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_buildBinaryRadixTreeLeafNodesKernel);
  m_buildBinaryRadixTreeInternalNodesKernel =
      b3OpenCLUtils::compileCLKernelFromString(
          context, device, kernelSource, "buildBinaryRadixTreeInternalNodes",
          &error, m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_buildBinaryRadixTreeInternalNodesKernel);
  m_findDistanceFromRootKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "findDistanceFromRoot", &error,
      m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_findDistanceFromRootKernel);
  m_buildBinaryRadixTreeAabbsRecursiveKernel =
      b3OpenCLUtils::compileCLKernelFromString(
          context, device, kernelSource, "buildBinaryRadixTreeAabbsRecursive",
          &error, m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_buildBinaryRadixTreeAabbsRecursiveKernel);

  m_findLeafIndexRangesKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "findLeafIndexRanges", &error,
      m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_findLeafIndexRangesKernel);

  m_plbvhCalculateOverlappingPairsKernel =
      b3OpenCLUtils::compileCLKernelFromString(
          context, device, kernelSource, "plbvhCalculateOverlappingPairs",
          &error, m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_plbvhCalculateOverlappingPairsKernel);
  m_plbvhRayTraverseKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "plbvhRayTraverse", &error,
      m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_plbvhRayTraverseKernel);
  m_plbvhLargeAabbAabbTestKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "plbvhLargeAabbAabbTest", &error,
      m_parallelLinearBvhProgram, additionalMacros);
  b3Assert(m_plbvhLargeAabbAabbTestKernel);
  m_plbvhLargeAabbRayTestKernel = b3OpenCLUtils::compileCLKernelFromString(
      context, device, kernelSource, "plbvhLargeAabbRayTest", &error,
      m_parallelLinearBvhProgram, additionalMacros);
...
void b3GpuParallelLinearBvh::build(
    const b3OpenCLArray<b3SapAabb>& worldSpaceAabbs,
    const b3OpenCLArray<int>& smallAabbIndices,
  ...
  // with 2 additional arrays used to indicate the indices of large and small
  // AABBs,
  // it is necessary to separate the AABBs so that the large AABBs will not
  // degrade the quality of the BVH.
  {
    ...
    // Write large AABBs into m_largeAabbs
    {
      ...
      b3LauncherCL launcher(m_queue, m_separateAabbsKernel,
                            "m_separateAabbsKernel");
      launcher.setBuffers(bufferInfo,
                          sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
  ...
  //
  int numLeaves = numSmallAabbs;  // Number of leaves in the BVH == Number of
                                  // rigid bodies with small AABBs
  int numInternalNodes = numLeaves - 1;

  if (numLeaves < 2) {
    // Number of leaf nodes is checked in calculateOverlappingPairs() and
    ...
    // Since the AABBs need to be rearranged(sorted) for the BVH construction
    // algorithm,
    // m_mortonCodesAndAabbIndicies.m_value is used to map a sorted AABB index
    // to the unsorted AABB index
    // instead of directly moving the AABBs. It needs to be set for the ray cast
    // traversal kernel to work.
    //( m_mortonCodesAndAabbIndicies[].m_value == unsorted index == index of
    //m_leafNodeAabbs )
    if (numLeaves == 1) {
      b3SortData leaf;
      leaf.m_value =
          0;  // 1 leaf so index is always 0; leaf.m_key does not need to be set
  ...
  // each cell in the virtual grid for the next kernel(2^10 cells in each
  // dimension).
  {
    B3_PROFILE("Find AABB of merged nodes");

    m_mergedAabb.copyFromOpenCLArray(m_leafNodeAabbs);  // Need to make a copy
                                                        // since the kernel
                                                        // modifies the array

    for (int numAabbsNeedingMerge = numLeaves; numAabbsNeedingMerge >= 2;
         numAabbsNeedingMerge =
             numAabbsNeedingMerge / 2 + numAabbsNeedingMerge % 2) {
      b3BufferInfoCL bufferInfo[] = {
          b3BufferInfoCL(m_mergedAabb.getBufferCL())  // Resulting AABB is
                                                      // stored in
                                                      // m_mergedAabb[0]
      };

      b3LauncherCL launcher(m_queue, m_findAllNodesMergedAabbKernel,
                            "m_findAllNodesMergedAabbKernel");
      launcher.setBuffers(bufferInfo,
                          sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
  ...
  // Since it is a sorted binary radix tree, each internal node contains a
  // contiguous subset of leaf node indices.
  // The root node contains leaf node indices in the range [0, numLeafNodes -
  // 1].
  // The child nodes of each node split their parent's index range into 2
  // contiguous halves.
  //
  // For example, if the root has indices [0, 31], its children might partition
  // that range into [0, 11] and [12, 31].
  // The next level in the tree could then split those ranges into [0, 2], [3,
  // 11], [12, 22], and [23, 31].
  //
  // This property can be used for optimizing calculateOverlappingPairs(), to
  // avoid testing each AABB pair twice
  {
    ...
    b3LauncherCL launcher(m_queue, m_findLeafIndexRangesKernel,
                          "m_findLeafIndexRangesKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
...
void b3GpuParallelLinearBvh::calculateOverlappingPairs(
    b3OpenCLArray<b3Int4>& out_overlappingPairs) {
  ...
  if (m_leafNodeAabbs.size() > 1) {
    ...
    b3LauncherCL launcher(m_queue, m_plbvhCalculateOverlappingPairsKernel,
                          "m_plbvhCalculateOverlappingPairsKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
  ...
  if (numLargeAabbRigids > 0 && m_leafNodeAabbs.size() > 0) {
    ...
    b3LauncherCL launcher(m_queue, m_plbvhLargeAabbAabbTestKernel,
                          "m_plbvhLargeAabbAabbTestKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
    launcher.setConst(maxPairs);
...
void b3GpuParallelLinearBvh::testRaysAgainstBvhAabbs(
    const b3OpenCLArray<b3RayInfo>& rays,
    b3OpenCLArray<int>& out_numRayRigidPairs,
    b3OpenCLArray<b3Int2>& out_rayRigidPairs) {
  ...
  if (m_leafNodeAabbs.size() > 0) {
    ...
    b3LauncherCL launcher(m_queue, m_plbvhRayTraverseKernel,
                          "m_plbvhRayTraverseKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
  ...
  if (numLargeAabbRigids > 0) {
    ...
    b3LauncherCL launcher(m_queue, m_plbvhLargeAabbRayTestKernel,
                          "m_plbvhLargeAabbRayTestKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
    launcher.setConst(numLargeAabbRigids);
    launcher.setConst(maxRayRigidPairs);
  ...
  if (numRayRigidPairs > maxRayRigidPairs)
    b3Error(
        "Error running out of rayRigid pairs: numRayRigidPairs = %d, "
        "maxRayRigidPairs = %d.\n",
...
void b3GpuParallelLinearBvh::constructBinaryRadixTree() {
  ...
  // these 2 adjacent leaf nodes, it is possible to quickly construct a binary
  // radix tree.
  {
    ...
    b3LauncherCL launcher(m_queue, m_computeAdjacentPairCommonPrefixKernel,
                          "m_computeAdjacentPairCommonPrefixKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
    launcher.setConst(numInternalNodes);
  ...
  // comparing the 2 nearest internal nodes and assign child node indices
  {
    ...
    b3LauncherCL launcher(m_queue, m_buildBinaryRadixTreeLeafNodesKernel,
                          "m_buildBinaryRadixTreeLeafNodesKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
    launcher.setConst(numLeaves);
  ...
  // For each internal node, perform 2 binary searches among the other internal
  // nodes
  // to its left and right to find its potential parent nodes and assign child
  // node indices
  {
    ...
    b3LauncherCL launcher(m_queue, m_buildBinaryRadixTreeInternalNodesKernel,
                          "m_buildBinaryRadixTreeInternalNodesKernel");
    launcher.setBuffers(bufferInfo,
                        sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
    launcher.setConst(numInternalNodes);
  ...
  // Also determine the maximum number of nodes separating an internal node and
  // the root node.
  {
    ...
    b3LauncherCL launcher(m_queue, m_findDistanceFromRootKernel,
  ...
  // Starting from the internal nodes nearest to the leaf nodes, recursively
  // move up
  // the tree towards the root to set the AABBs of each internal node; each
  // internal node
  // checks its children and merges their AABBs
  {
    ...
    for (int distanceFromRoot = maxDistanceFromRoot; distanceFromRoot >= 0;
         --distanceFromRoot) {
      b3BufferInfoCL bufferInfo[] = {
          b3BufferInfoCL(m_distanceFromRoot.getBufferCL()),
          b3BufferInfoCL(m_mortonCodesAndAabbIndicies.getBufferCL()),
      ...
      b3LauncherCL launcher(m_queue, m_buildBinaryRadixTreeAabbsRecursiveKernel,
                            "m_buildBinaryRadixTreeAabbsRecursiveKernel");
      launcher.setBuffers(bufferInfo,
                          sizeof(bufferInfo) / sizeof(b3BufferInfoCL));
      ...
      // much smaller number of nodes is actually processed, but this is
      // actually
      // faster than determining the exact nodes that are ready to merge their
      // child AABBs.
      launcher.launch1D(numInternalNodes);
    }
