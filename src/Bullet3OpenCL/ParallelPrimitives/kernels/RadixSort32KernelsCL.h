...
static const char* radixSort32KernelsCL =
    ...
    "	return sum;\n"
    "}\n"
    "u32 localPrefixSum( u32 pData, uint lIdx, uint* totalSum, __local u32* "
    "sorterSharedMemory, int wgSize /*64 or 128*/ )\n"
    "{\n"
    "	{	//	Set data\n"
    ...
    "sorterSharedMemory[idx-64];\n"
    "				GROUP_MEM_FENCE;\n"
    "			}\n"
    "			sorterSharedMemory[idx-1] += "
    ...
    "uint4 localPrefixSum128V( uint4 pData, uint lIdx, uint* totalSum, __local "
    "u32* sorterSharedMemory )\n"
    "{\n"
    ...
    "uint4 localPrefixSum64V( uint4 pData, uint lIdx, uint* totalSum, __local "
    "u32* sorterSharedMemory )\n"
    "{\n"
    "	u32 s4 = prefixScanVectorEx( &pData );\n"
    ...
    "void StreamCountKernel( __global u32* gSrc, __global u32* histogramOut, "
    "int4 cb )\n"
    "{\n"
    ...
    "	int addr = blockSize*nBlocksPerWG*wgIdx + "
    "ELEMENTS_PER_WORK_ITEM*lIdx;\n"
    "	for(int iblock=0; iblock<min(nBlocksPerWG, nBlocks); iblock++, "
    "addr+=blockSize)\n"
    "	{\n"
    "		//	MY_HISTOGRAM( localKeys.x ) ++ is much expensive than "
    "atomic add as it requires read and write while atomics can just add on "
    "AMD\n"
    "		//	Using registers didn't perform well. It seems like use "
    "localKeys to address requires a lot of alu ops\n"
    ...
    "		{\n"
    "			sum += "
    "localHistogramMat[lIdx*WG_SIZE+(i+lIdx)%GET_GROUP_SIZE];\n"
    "		}\n"
    ...
    "__kernel\n"
    "__attribute__((reqd_work_group_size(WG_SIZE,1,1)))\n"
    "void StreamCountSortDataKernel( __global SortDataCL* gSrc, __global u32* "
    "histogramOut, int4  cb )\n"
    "{\n"
    "	__local u32 localHistogramMat[NUM_BUCKET*WG_SIZE];\n"
    "	u32 gIdx = GET_GLOBAL_IDX;\n"
    "	u32 lIdx = GET_LOCAL_IDX;\n"
    ...
    "#endif\n"
    "			{\n"
    "				localKey = (gSrc[addr+i].m_key>>startBit) & "
    "0xf;\n"
    "#if defined(NV_GPU)\n"
    "				MY_HISTOGRAM( localKey )++;\n"
    "#else\n"
    ...
    "ldsTopScanData );\n"
    "//	for(int j=0; j<4; j++) //	somehow it introduces a lot of "
    "branches\n"
    "	{	int j = 0;\n"
    "		u32 sum = 0;\n"
    "		for(int i=0; i<nPerLane; i++)\n"
    ...
    "}\n"
    "//	4 scan, 4 exchange\n"
    "void sort4Bits(u32 sortData[4], int startBit, int lIdx, __local u32* "
    "ldsSortData)\n"
    "{\n"
    "	for(int bitIdx=0; bitIdx<BITS_PER_PASS; bitIdx++)\n"
    "	{\n"
    "		u32 mask = (1<<bitIdx);\n"
    "		uint4 cmpResult = make_uint4( (sortData[0]>>startBit) & mask, "
    "(sortData[1]>>startBit) & mask, (sortData[2]>>startBit) & mask, "
    "(sortData[3]>>startBit) & mask );\n"
    "		uint4 prefixSum = SELECT_UINT4( make_uint4(1,1,1,1), "
    "make_uint4(0,0,0,0), cmpResult != make_uint4(0,0,0,0) );\n"
    "		u32 total;\n"
    "		prefixSum = localPrefixSum64V( prefixSum, lIdx, &total, "
    "ldsSortData );\n"
    "		{\n"
    "			uint4 localAddr = "
    "make_uint4(lIdx*4+0,lIdx*4+1,lIdx*4+2,lIdx*4+3);\n"
    "			uint4 dstAddr = localAddr - prefixSum + make_uint4( "
    "total, total, total, total );\n"
    "			dstAddr = SELECT_UINT4( prefixSum, dstAddr, cmpResult "
    "!= make_uint4(0, 0, 0, 0) );\n"
    "			GROUP_LDS_BARRIER;\n"
    ...
    "}\n"
    "//	2 scan, 2 exchange\n"
    "void sort4Bits1(u32 sortData[4], int startBit, int lIdx, __local u32* "
    "ldsSortData)\n"
    "{\n"
    ...
    "			key4 = sKeyPacked[0] + sKeyPacked[1] + sKeyPacked[2] + "
    "sKeyPacked[3];\n"
    "		}\n"
    "		u32 rankPacked;\n"
    "		u32 sumPacked;\n"
    "		{\n"
    "			rankPacked = localPrefixSum( key4, lIdx, &sumPacked, "
    "ldsSortData, WG_SIZE );\n"
    "		}\n"
    "		GROUP_LDS_BARRIER;\n"
    ...
    "					newOffset[0] = unpack4Key( "
    "scannedKeys[0], ie );\n"
    "				}\n"
    "				{	u32 ie = b.y;\n"
    "					scannedKeys[1] += sumPlusRank;\n"
    "					newOffset[1] = unpack4Key( "
    "scannedKeys[1], ie );\n"
    "				}\n"
    "				{	u32 ie = b.z;\n"
    "					scannedKeys[2] += sumPlusRank;\n"
    "					newOffset[2] = unpack4Key( "
    "scannedKeys[2], ie );\n"
    "				}\n"
    "				{	u32 ie = b.w;\n"
    "					scannedKeys[3] += sumPlusRank;\n"
    "					newOffset[3] = unpack4Key( "
    "scannedKeys[3], ie );\n"
    "				}\n"
    "			}\n"
    "		}\n"
    ...
    "__kernel\n"
    "__attribute__((reqd_work_group_size(WG_SIZE,1,1)))\n"
    "void SortAndScatterKernel( __global const u32* restrict gSrc, __global "
    "const u32* rHistogram, __global u32* restrict gDst, int4  cb )\n"
    "{\n"
    ...
    "		for(int i=0; i<ELEMENTS_PER_WORK_ITEM; i++)\n"
    "#if defined(CHECK_BOUNDARY)\n"
    "			sortData[i] = ( addr+i < n )? gSrc[ addr+i ] : "
    "0xffffffff;\n"
    "#else\n"
    ...
    "				GROUP_MEM_FENCE;\n"
    "				localHistogram[hIdx] += "
    "localHistogram[hIdx-2];\n"
    "				GROUP_MEM_FENCE;\n"
    "				localHistogram[hIdx] += "
    "localHistogram[hIdx-4];\n"
    "				GROUP_MEM_FENCE;\n"
    "				localHistogram[hIdx] += "
    "localHistogram[hIdx-8];\n"
    "				GROUP_MEM_FENCE;\n"
    "			}\n"
    "#endif\n"
    "			GROUP_LDS_BARRIER;\n"
    ...
    "				int binIdx = keys[ie];\n"
    "				int groupOffset = "
    "localHistogramToCarry[binIdx];\n"
    "				int myIdx = dataIdx - "
    "localHistogram[NUM_BUCKET+binIdx];\n"
    "#if defined(CHECK_BOUNDARY)\n"
    "				if( addr+ie < n )\n"
    "#endif\n"
    ...
    "}\n"
    "//	2 scan, 2 exchange\n"
    "void sort4Bits1KeyValue(u32 sortData[4], int sortVal[4], int startBit, "
    "int lIdx, __local u32* ldsSortData, __local int *ldsSortVal)\n"
    "{\n"
    "	for(uint ibit=0; ibit<BITS_PER_PASS; ibit+=2)\n"
    "	{\n"
    "		uint4 b = make_uint4((sortData[0]>>(startBit+ibit)) & 0x3, \n"
    ...
    "__kernel\n"
    "__attribute__((reqd_work_group_size(WG_SIZE,1,1)))\n"
    "void SortAndScatterSortDataKernel( __global const SortDataCL* restrict "
    "gSrc, __global const u32* rHistogram, __global SortDataCL* restrict gDst, "
    "int4 cb)\n"
    "{\n"
    ...
    "#if defined(CHECK_BOUNDARY)\n"
    "		{\n"
    "			sortData[i] = ( addr+i < n )? gSrc[ addr+i ].m_key : "
    "0xffffffff;\n"
    "			sortVal[i] = ( addr+i < n )? gSrc[ addr+i ].m_value : "
    "0xffffffff;\n"
    "		}\n"
    "#else\n"
    "		{\n"
    ...
    "__kernel\n"
    "__attribute__((reqd_work_group_size(WG_SIZE,1,1)))\n"
    "void SortAndScatterSortDataKernelSerial( __global const SortDataCL* "
    "restrict gSrc, __global const u32* rHistogram, __global SortDataCL* "
    "restrict gDst, int4 cb)\n"
    "{\n"
    "    \n"
    "	u32 gIdx = GET_GLOBAL_IDX;\n"
    ...
    " 	{\n"
    "        int addr2 = iblock*blockSize + blockSize*nBlocksPerWG*wgIdx + "
    "ELEMENTS_PER_WORK_ITEM*lIdx;\n"
    "        \n"
    ...
    "			{\n"
    "                int tableIdx;\n"
    "				tableIdx = (gSrc[i].m_key>>startBit) & "
    "0xf;//0xf = NUM_TABLES-1\n"
    "                gDst[rHistogram[tableIdx*nWGs+wgIdx] + counter[tableIdx]] "
    "= gSrc[i];\n"
    "                counter[tableIdx] ++;\n"
    "			}\n"
    "		}\n"
    ...
    "__kernel\n"
    "__attribute__((reqd_work_group_size(WG_SIZE,1,1)))\n"
    "void SortAndScatterKernelSerial( __global const u32* restrict gSrc, "
    "__global const u32* rHistogram, __global u32* restrict gDst, int4  cb )\n"
    "{\n"
    "    \n"
    "	u32 gIdx = GET_GLOBAL_IDX;\n"
    "	u32 realLocalIdx = GET_LOCAL_IDX;\n"
    ...
    "			{\n"
    "                int tableIdx;\n"
    "				tableIdx = (gSrc[i]>>startBit) & 0xf;//0xf = "
    "NUM_TABLES-1\n"
    "                gDst[rHistogram[tableIdx*nWGs+wgIdx] + counter[tableIdx]] "
    "= gSrc[i];\n"
    "                counter[tableIdx] ++;\n"
