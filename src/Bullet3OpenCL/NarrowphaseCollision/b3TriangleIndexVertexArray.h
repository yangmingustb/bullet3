...
/// The b3IndexedMesh indexes a single vertex and index array. Multiple
/// b3IndexedMesh objects can be passed into a b3TriangleIndexVertexArray using
/// addIndexedMesh.
...
b3IndexedMesh {
  ...
  // Size in byte of the indices for one triangle (3*sizeof(index_type) if the
  ...
  // (float or double)
  // but can be set manually if you for example run Bullet with double precision
  // but have
...
/// The b3TriangleIndexVertexArray allows to access multiple triangle meshes, by
/// indexing into existing triangle/index arrays.
/// Additional meshes can be added using addIndexedMesh
/// No duplcate is made of the vertex/index data, it only indexes into external
/// vertex/index arrays.
/// So keep those arrays around during the lifetime of this
...
b3TriangleIndexVertexArray : public b3StridingMeshInterface {
 ...
 public:
  ...
  b3TriangleIndexVertexArray(int numTriangles, int* triangleIndexBase,
                             int triangleIndexStride, int numVertices,
                             b3Scalar* vertexBase, int vertexStride);

  void addIndexedMesh(const b3IndexedMesh& mesh,
  ...
  virtual void getLockedVertexIndexBase(
      ...
      int& vertexStride, unsigned char** indexbase, int& indexstride,
  ...
  virtual void getLockedReadOnlyVertexIndexBase(
      ...
      int& vertexStride, const unsigned char** indexbase, int& indexstride,
  ...
  virtual int getNumSubParts() const { return (int)m_indexedMeshes.size(); }

  IndexedMeshArray& getIndexedMeshArray() { return m_indexedMeshes; }
