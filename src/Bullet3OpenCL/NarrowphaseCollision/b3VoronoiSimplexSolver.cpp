...
software
        for any purpose, commercial or non-commercial as long as the following
credit is included
...
void b3VoronoiSimplexSolver::reset() {
  ...
  m_numVertices = 0;
  m_needsUpdate = true;
  m_lastW = b3MakeVector3(b3Scalar(B3_LARGE_FLOAT), b3Scalar(B3_LARGE_FLOAT),
                          b3Scalar(B3_LARGE_FLOAT));
  m_cachedBC.reset();
}

// add a vertex
void b3VoronoiSimplexSolver::addVertex(const b3Vector3& w, const b3Vector3& p,
                                       const b3Vector3& q) {
  m_lastW = w;
  m_needsUpdate = true;
  ...
  if (m_needsUpdate) {
    ...
    switch (numVertices()) {
      ...
      case 1: {
        ...
        m_cachedBC.reset();
        m_cachedBC.setBarycentricCoordinates(b3Scalar(1.), b3Scalar(0.),
                                             b3Scalar(0.), b3Scalar(0.));
      ...
      case 2: {
        ...
        m_cachedP1 = m_simplexPointsP[0] +
                     t * (m_simplexPointsP[1] - m_simplexPointsP[0]);
        m_cachedP2 = m_simplexPointsQ[0] +
                     t * (m_simplexPointsQ[1] - m_simplexPointsQ[0]);
...
int b3VoronoiSimplexSolver::getSimplex(b3Vector3* pBuf, b3Vector3* qBuf,
                                       b3Vector3* yBuf) const {
  int i;
...
bool b3VoronoiSimplexSolver::inSimplex(const b3Vector3& w) {
  ...
  if (w == m_lastW) return true;

  return found;
}

void b3VoronoiSimplexSolver::backup_closest(b3Vector3& v) { v = m_cachedV; }
...
bool b3VoronoiSimplexSolver::closestPtPointTriangle(
    const b3Vector3& p, const b3Vector3& a, const b3Vector3& b,
    const b3Vector3& c, b3SubSimplexClosestResult& result) {
  ...
  if (va <= b3Scalar(0.0) && (d4 - d3) >= b3Scalar(0.0) &&
      (d5 - d6) >= b3Scalar(0.0)) {
  ...
  return true;
  //	return a + ab * v + ac * w; // = u*a + v*b + w*c, u = va * denom =
  //b3Scalar(1.0) - v - w
}

/// Test if point p and d lie on opposite sides of plane through abc
int b3VoronoiSimplexSolver::pointOutsideOfPlane(const b3Vector3& p,
                                                const b3Vector3& a,
                                                const b3Vector3& b,
                                                const b3Vector3& c,
                                                const b3Vector3& d) {
...
bool b3VoronoiSimplexSolver::closestPtPointTetrahedron(
    const b3Vector3& p, const b3Vector3& a, const b3Vector3& b,
  ...
  if (!pointOutsideABC && !pointOutsideACD && !pointOutsideADB &&
      !pointOutsideBDC) {
    return false;
  }
  ...
  if (pointOutsideABC) {
    ...
    if (sqDist < bestSqDist) {
      ...
      finalResult.m_usedVertices.reset();
      finalResult.m_usedVertices.usedVertexA =
          tempResult.m_usedVertices.usedVertexA;
      finalResult.m_usedVertices.usedVertexB =
          tempResult.m_usedVertices.usedVertexB;
      finalResult.m_usedVertices.usedVertexC =
          tempResult.m_usedVertices.usedVertexC;
  ...
  if (pointOutsideACD) {
    ...
    if (sqDist < bestSqDist) {
      ...
      finalResult.m_usedVertices.usedVertexD =
          tempResult.m_usedVertices.usedVertexC;
      finalResult.setBarycentricCoordinates(
          tempResult.m_barycentricCoords[VERTA], 0,
          tempResult.m_barycentricCoords[VERTB],
  ...
  if (pointOutsideBDC) {
    ...
    if (sqDist < bestSqDist) {
      ...
      finalResult.setBarycentricCoordinates(
          0, tempResult.m_barycentricCoords[VERTA],
          tempResult.m_barycentricCoords[VERTC],
          tempResult.m_barycentricCoords[VERTB]);
    }
  }
