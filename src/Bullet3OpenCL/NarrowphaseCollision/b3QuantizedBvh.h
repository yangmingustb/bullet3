...
/// Node can be used for leafnode or internal node. Leafnodes can point to
/// 32-bit triangle index (non-negative range).
...
/// Total node size is 44 bytes / node. You can use the compressed version of 16
/// bytes.
B3_ATTRIBUTE_ALIGNED16(struct)
b3OptimizedBvhNode {
  B3_DECLARE_ALIGNED_ALLOCATOR();
...
/// The b3QuantizedBvh class stores an AABB tree that can be quickly traversed
/// on CPU and Cell SPU.
/// It is used by the b3BvhTriangleMeshShape as midphase
/// It is recommended to use quantization for better performance and lower
/// memory requirements.
B3_ATTRIBUTE_ALIGNED16(class)
b3QuantizedBvh {
 public:
 ...
 protected:
  int m_bulletVersion;  // for serialization versioning. It could also be used
                        // to detect endianess.
  ...
  // This is only used for serialization so we don't have to add serialization
  // directly to b3AlignedObjectArray
  mutable int m_subtreeHeaderCount;

  /// two versions, one for quantized and normal nodes. This allows code-reuse
  /// while maintaining readability (no template/macro!)
  /// this might be refactored into a virtual, it is usually not calculated at
  /// run-time
  void setInternalNodeAabbMin(int nodeIndex, const b3Vector3& aabbMin) {
    if (m_useQuantization) {
      quantize(&m_quantizedContiguousNodes[nodeIndex].m_quantizedAabbMin[0],
               aabbMin, 0);
    } else {
  ...
  void setInternalNodeAabbMax(int nodeIndex, const b3Vector3& aabbMax) {
    if (m_useQuantization) {
      quantize(&m_quantizedContiguousNodes[nodeIndex].m_quantizedAabbMax[0],
               aabbMax, 1);
    } else {
  ...
  void mergeInternalNodeAabb(int nodeIndex, const b3Vector3& newAabbMin,
                             const b3Vector3& newAabbMax) {
    if (m_useQuantization) {
      ...
      for (int i = 0; i < 3; i++) {
        if (m_quantizedContiguousNodes[nodeIndex].m_quantizedAabbMin[i] >
            quantizedAabbMin[i])
          m_quantizedContiguousNodes[nodeIndex].m_quantizedAabbMin[i] =
              quantizedAabbMin[i];

        if (m_quantizedContiguousNodes[nodeIndex].m_quantizedAabbMax[i] <
            quantizedAabbMax[i])
          m_quantizedContiguousNodes[nodeIndex].m_quantizedAabbMax[i] =
              quantizedAabbMax[i];
      }
    } else {
      // non-quantized
 ...
 protected:
  ...
  void walkStacklessTree(b3NodeOverlapCallback * nodeCallback,
                         const b3Vector3& aabbMin, const b3Vector3& aabbMax)
      const;

  void walkStacklessQuantizedTreeAgainstRay(
  ...
  void walkStacklessQuantizedTree(b3NodeOverlapCallback * nodeCallback,
                                  unsigned short int* quantizedQueryAabbMin,
                                  unsigned short int* quantizedQueryAabbMax,
                                  int startNodeIndex, int endNodeIndex) const;
  ...
  /// use the 16-byte stackless 'skipindex' node tree to do a recursive
  /// traversal
  void walkRecursiveQuantizedTreeAgainstQueryAabb(
      const b3QuantizedBvhNode* currentNode,
      b3NodeOverlapCallback* nodeCallback,
      unsigned short int* quantizedQueryAabbMin,
      unsigned short int* quantizedQueryAabbMax) const;
  ...
  void walkRecursiveQuantizedTreeAgainstQuantizedTree(
      const b3QuantizedBvhNode* treeNodeA, const b3QuantizedBvhNode* treeNodeB,
      b3NodeOverlapCallback* nodeCallback) const;
 ...
 public:
  ...
  ///*************************
  void setQuantizationValues(const b3Vector3& bvhAabbMin,
                             const b3Vector3& bvhAabbMax,
                             b3Scalar quantizationMargin = b3Scalar(1.0));
  ...
  void reportAabbOverlappingNodex(b3NodeOverlapCallback * nodeCallback,
                                  const b3Vector3& aabbMin,
                                  const b3Vector3& aabbMax) const;
  void reportRayOverlappingNodex(b3NodeOverlapCallback * nodeCallback,
                                 const b3Vector3& raySource,
                                 const b3Vector3& rayTarget) const;
  void reportBoxCastOverlappingNodex(
      b3NodeOverlapCallback * nodeCallback, const b3Vector3& raySource,
      const b3Vector3& rayTarget, const b3Vector3& aabbMin,
      const b3Vector3& aabbMax) const;

  B3_FORCE_INLINE void quantize(unsigned short* out, const b3Vector3& point,
                                int isMax) const {
    ...
    /// Make sure rounding is done in a way that
    /// unQuantize(quantizeWithClamp(...)) is conservative
    /// end-points always set the first bit, so that they are sorted properly
    /// (so that neighbouring AABBs overlap properly)
    ///@todo: double-check this
    if (isMax) {
      out[0] =
          (unsigned short)(((unsigned short)(v.getX() + b3Scalar(1.)) | 1));
      out[1] =
          (unsigned short)(((unsigned short)(v.getY() + b3Scalar(1.)) | 1));
      out[2] =
          (unsigned short)(((unsigned short)(v.getZ() + b3Scalar(1.)) | 1));
    } else {
  ...
  B3_FORCE_INLINE void quantizeWithClamp(
      unsigned short* out, const b3Vector3& point2, int isMax) const {
    b3Assert(m_useQuantization);
  ...
  B3_FORCE_INLINE b3Vector3 unQuantize(const unsigned short* vecIn) const {
    b3Vector3 vecOut;
    vecOut.setValue((b3Scalar)(vecIn[0]) / (m_bvhQuantization.getX()),
  ...
  virtual bool serialize(void* o_alignedDataBuffer, unsigned i_dataBufferSize,
                         bool i_swapEndian) const;

  /// deSerializeInPlace loads and initializes a BVH from a buffer in memory 'in
  /// place'
  static b3QuantizedBvh* deSerializeInPlace(void* i_alignedDataBuffer,
  ...
  virtual const char* serialize(void* dataBuffer, b3Serializer* serializer)
      const;

  virtual void deSerializeFloat(struct b3QuantizedBvhFloatData &
                                quantizedBvhFloatData);

  virtual void deSerializeDouble(struct b3QuantizedBvhDoubleData &
                                 quantizedBvhDoubleData);

  ////////////////////////////////////////////////////////////////////

  B3_FORCE_INLINE bool isQuantized() { return m_useQuantization; }

 private:
  // Special "copy" constructor that allows for in-place deserialization
  // Prevents b3Vector3's default constructor from being called, but doesn't
  // inialize much else
  // ownsMemory should most likely be false if deserializing, and if you are
  // not, don't call this (it also changes the function signature, which we
  // need)
