...
/// disable next define, or use
/// defaultCollisionConfiguration->getSimplexSolver()->setEqualVertexThreshold(0.f)
/// to disable/configure
//#define BT_USE_EQUAL_VERTEX_THRESHOLD
#define VORONOI_DEFAULT_EQUAL_VERTEX_THRESHOLD 0.0001f

struct b3UsageBitfield {
  b3UsageBitfield() { reset(); }
...
struct b3SubSimplexClosestResult {
  ...
  void setBarycentricCoordinates(b3Scalar a = b3Scalar(0.),
                                 b3Scalar b = b3Scalar(0.),
                                 b3Scalar c = b3Scalar(0.),
                                 b3Scalar d = b3Scalar(0.)) {
...
/// b3VoronoiSimplexSolver is an implementation of the closest point distance
...
b3VoronoiSimplexSolver {
 public:
  ...
  bool closestPtPointTetrahedron(const b3Vector3& p, const b3Vector3& a,
                                 const b3Vector3& b, const b3Vector3& c,
                                 const b3Vector3& d,
                                 b3SubSimplexClosestResult& finalResult);
  int pointOutsideOfPlane(const b3Vector3& p, const b3Vector3& a,
                          const b3Vector3& b, const b3Vector3& c,
                          const b3Vector3& d);
  bool closestPtPointTriangle(const b3Vector3& p, const b3Vector3& a,
                              const b3Vector3& b, const b3Vector3& c,
 ...
 public:
  ...
  b3Scalar getEqualVertexThreshold() const { return m_equalVertexThreshold; }
