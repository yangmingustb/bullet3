...
struct b3MinkowskiDiff {
  ...
  void EnableMargin(bool enable) { m_enableMargin = enable; }
  inline b3Vector3 Support0(
      const b3Vector3& d,
      const b3AlignedObjectArray<b3Vector3>& verticesA) const {
    if (m_enableMargin) {
  ...
  inline b3Vector3 Support1(
      const b3Vector3& d,
      const b3AlignedObjectArray<b3Vector3>& verticesB) const {
    if (m_enableMargin) {
      return m_toshape0 * (localGetSupportVertexWithMargin(
                              m_toshape1 * d, m_shapes[1], verticesB, 0.f));
    } else {
      return m_toshape0 * (localGetSupportVertexWithoutMargin(
                              m_toshape1 * d, m_shapes[1], verticesB));
    }
  }

  inline b3Vector3 Support(
      const b3Vector3& d, const b3AlignedObjectArray<b3Vector3>& verticesA,
      const b3AlignedObjectArray<b3Vector3>& verticesB) const {
    return (Support0(d, verticesA) - Support1(-d, verticesB));
  }
  b3Vector3 Support(const b3Vector3& d, unsigned int index,
...
struct b3GJK {
  ...
  eStatus::_ m_status;
  /* Methods		*/
  b3GJK(const b3AlignedObjectArray<b3Vector3>& verticesA,
  ...
  eStatus::_ Evaluate(const tShape& shapearg, const b3Vector3& guess) {
    ...
    do {
      ...
      if (rl <
          GJK_MIN_DISTANCE) { /* Touching or inside */
      ...
      if (((rl - alpha) - (GJK_ACCURACY * rl)) <=
          0) { /* Return old simplex				*/
        removevertice(m_simplices[m_current]);
        break;
      }
      /* Reduce simplex						*/
      ...
      switch (cs.rank) {
        ...
        case 3:
          sqdist =
              projectorigin(cs.c[0]->w, cs.c[1]->w, cs.c[2]->w, weights, mask);
          break;
        case 4:
          sqdist = projectorigin(cs.c[0]->w, cs.c[1]->w, cs.c[2]->w, cs.c[3]->w,
                                 weights, mask);
          break;
      }
      if (sqdist >= 0) { /* Valid	*/
      ...
      m_status =
          ((++iterations) < GJK_MAX_ITERATIONS) ? m_status : eStatus::Failed;
  ...
  static b3Scalar det(const b3Vector3& a, const b3Vector3& b,
                      const b3Vector3& c) {
    return (a.y * b.z * c.x + a.z * b.x * c.y - a.x * b.z * c.y -
            a.y * b.x * c.z + a.x * b.y * c.z - a.z * b.y * c.x);
  }
  static b3Scalar projectorigin(const b3Vector3& a, const b3Vector3& b,
  ...
  static b3Scalar projectorigin(const b3Vector3& a, const b3Vector3& b,
                                const b3Vector3& c, const b3Vector3& d,
                                b3Scalar* w, unsigned int& m) {
    static const unsigned int imd3[] = {1, 2, 0};
...
struct b3EPA {
  ...
  static inline void bind(sFace* fa, unsigned int ea, sFace* fb,
                          unsigned int eb) {
  ...
  eStatus::_ Evaluate(b3GJK& gjk, const b3Vector3& guess) {
    ...
    if ((simplex.rank > 1) && gjk.EncloseOrigin()) {
      ...
      if (m_hull.count == 4) {
        ...
        m_result.p[0] =
            b3Cross(outer.c[1]->w - projection, outer.c[2]->w - projection)
                .length();
        m_result.p[1] =
            b3Cross(outer.c[2]->w - projection, outer.c[0]->w - projection)
                .length();
        m_result.p[2] =
            b3Cross(outer.c[0]->w - projection, outer.c[1]->w - projection)
                .length();
  ...
  bool getedgedist(sFace* face, sSV* a, sSV* b, b3Scalar& dist) {
    ...
    const b3Vector3 n_ab = b3Cross(
        ba,
        face->n);  // Outward facing edge normal direction, on triangle plane
    const b3Scalar a_dot_nab =
        b3Dot(a->w, n_ab);  // Only care about the sign to determine
                            // inside/outside, so not normalization required
    ...
    if (a_dot_nab < 0) {
      ...
      if (a_dot_ba > 0) {
        ...
        dist = b3Sqrt(
            b3Max((a->w.length2() * b->w.length2() - a_dot_b * a_dot_b) / ba_l2,
                  (b3Scalar)0));
      }
  ...
  bool expand(unsigned int pass, sSV* w, sFace* f, unsigned int e,
              sHorizon& horizon) {
...
static void Initialize(const b3Transform& transA, const b3Transform& transB,
                       ...
                       const b3AlignedObjectArray<b3Vector3>& verticesA,
                       const b3AlignedObjectArray<b3Vector3>& verticesB,
                       b3GjkEpaSolver2::sResults& results, tShape& shape,
                       bool withmargins) {
  /* Results		*/
  results.witnesses[0] = results.witnesses[1] = b3MakeVector3(0, 0, 0);
...
//
bool b3GjkEpaSolver2::Distance(const b3Transform& transA,
                               const b3Transform& transB,
                               const b3ConvexPolyhedronData* hullA,
                               const b3ConvexPolyhedronData* hullB,
                               const b3AlignedObjectArray<b3Vector3>& verticesA,
                               const b3AlignedObjectArray<b3Vector3>& verticesB,
                               const b3Vector3& guess, sResults& results) {
  tShape shape;
  Initialize(transA, transB, hullA, hullB, verticesA, verticesB, results, shape,
             false);
  ...
  if (gjk_status == b3GJK::eStatus::Valid) {
    ...
    results.normal /=
        results.distance > GJK_MIN_DISTANCE ? results.distance : 1;
    return (true);
  } else {
...
//
bool b3GjkEpaSolver2::Penetration(
    const b3Transform& transA, const b3Transform& transB,
    const b3ConvexPolyhedronData* hullA, const b3ConvexPolyhedronData* hullB,
    const b3AlignedObjectArray<b3Vector3>& verticesA,
    const b3AlignedObjectArray<b3Vector3>& verticesB, const b3Vector3& guess,
    sResults& results, bool usemargins) {
  tShape shape;
  Initialize(transA, transB, hullA, hullB, verticesA, verticesB, results, shape,
             usemargins);
  b3GJK gjk(verticesA, verticesB);
    ...
    case b3GJK::eStatus::Inside: {
      ...
      if (epa_status != b3EPA::eStatus::Failed) {
        ...
        for (unsigned int i = 0; i < epa.m_result.rank; ++i) {
          w0 += shape.Support(epa.m_result.c[i]->d, 0, verticesA, verticesB) *
                epa.m_result.p[i];
        }
