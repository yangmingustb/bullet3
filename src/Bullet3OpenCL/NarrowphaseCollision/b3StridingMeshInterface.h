...
///	The b3StridingMeshInterface is the interface class for high performance
///generic access to triangle meshes, used in combination with
///b3BvhTriangleMeshShape and some other collision shapes.
/// Using index striding of 3*sizeof(integer) it can use triangle arrays, using
/// index striding of 1*sizeof(integer) it can handle triangle strips.
/// It allows for sharing graphics and collision meshes. Also it provides
/// locking/unlocking of graphics meshes that are in gpu memory.
...
b3StridingMeshInterface {
 ...
 public:
  ...
  virtual void InternalProcessAllTriangles(
      b3InternalTriangleIndexCallback * callback, const b3Vector3& aabbMin,
  ...
  virtual void getLockedVertexIndexBase(
      ...
      int& stride, unsigned char** indexbase, int& indexstride, int& numfaces,
  ...
  virtual void getLockedReadOnlyVertexIndexBase(
      ...
      int& numfaces, PHY_ScalarType& indicestype, int subpart = 0) const = 0;

  /// unLockVertexBase finishes the access to a subpart of the triangle mesh
  /// make a call to unLockVertexBase when the read and write access (using
  ...
  virtual void setPremadeAabb(const b3Vector3& aabbMin,
                              const b3Vector3& aabbMax) const {
    (void)aabbMin;
    (void)aabbMax;
  }
  ...
  const b3Vector3& getScaling() const { return m_scaling; }
  void setScaling(const b3Vector3& scaling) { m_scaling = scaling; }
...
B3_FORCE_INLINE int b3StridingMeshInterface::calculateSerializeBufferSize()
    const {
