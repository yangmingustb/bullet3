...
void b3HashedOverlappingPairCache::cleanOverlappingPair(
    b3BroadphasePair& pair, b3Dispatcher* dispatcher) {
  /*	if (pair.m_algorithm)
  {
          {
...
void b3HashedOverlappingPairCache::cleanProxyFromPairs(
    int proxy, b3Dispatcher* dispatcher) {
  class CleanPairCallback : public b3OverlapCallback {
   ...
   public:
    CleanPairCallback(int cleanProxy, b3OverlappingPairCache* pairCache,
                      b3Dispatcher* dispatcher)
        : m_cleanProxy(cleanProxy),
          m_pairCache(pairCache),
          m_dispatcher(dispatcher) {}
    virtual bool processOverlap(b3BroadphasePair& pair) {
      if ((pair.x == m_cleanProxy) || (pair.y == m_cleanProxy)) {
...
void b3HashedOverlappingPairCache::removeOverlappingPairsContainingProxy(
    int proxy, b3Dispatcher* dispatcher) {
  class RemovePairCallback : public b3OverlapCallback {
    int m_obsoleteProxy;

   public:
    RemovePairCallback(int obsoleteProxy) : m_obsoleteProxy(obsoleteProxy) {}
    virtual bool processOverlap(b3BroadphasePair& pair) {
...
b3BroadphasePair* b3HashedOverlappingPairCache::findPair(int proxy0,
                                                         int proxy1) {
  b3g_findPairs++;
  if (proxy0 > proxy1) b3Swap(proxy0, proxy1);
  int proxyId1 = proxy0;
  int proxyId2 = proxy1;
  ...
  int hash = static_cast<int>(getHash(static_cast<unsigned int>(proxyId1),
                                      static_cast<unsigned int>(proxyId2)) &
  ...
  while (index != B3_NULL_PAIR &&
         equalsPair(m_overlappingPairArray[index], proxyId1, proxyId2) ==
             false) {
    index = m_next[index];
  }
...
b3BroadphasePair* b3HashedOverlappingPairCache::internalAddPair(int proxy0,
                                                                int proxy1) {
  if (proxy0 > proxy1) b3Swap(proxy0, proxy1);
  int proxyId1 = proxy0;
  int proxyId2 = proxy1;
  ...
  int hash = static_cast<int>(
      getHash(static_cast<unsigned int>(proxyId1),
              static_cast<unsigned int>(proxyId2)) &
  ...
  if (oldCapacity < newCapacity) {
    ...
    hash = static_cast<int>(getHash(static_cast<unsigned int>(proxyId1),
                                    static_cast<unsigned int>(proxyId2)) &
                            (m_overlappingPairArray.capacity() - 1));
  }
...
void* b3HashedOverlappingPairCache::removeOverlappingPair(
    int proxy0, int proxy1, b3Dispatcher* dispatcher) {
  b3g_removePairs++;
  if (proxy0 > proxy1) b3Swap(proxy0, proxy1);
  int proxyId1 = proxy0;
  int proxyId2 = proxy1;
  ...
  int lastHash = static_cast<int>(getHash(static_cast<unsigned int>(last->x),
                                          static_cast<unsigned int>(last->y)) &
                                  (m_overlappingPairArray.capacity() - 1));
...
void b3HashedOverlappingPairCache::processAllOverlappingPairs(
    b3OverlapCallback* callback, b3Dispatcher* dispatcher) {
  int i;
...
void b3HashedOverlappingPairCache::sortOverlappingPairs(
    b3Dispatcher* dispatcher) {
...
void* b3SortedOverlappingPairCache::removeOverlappingPair(
    int proxy0, int proxy1, b3Dispatcher* dispatcher) {
  if (!hasDeferredRemoval()) {
    ...
    if (findIndex < m_overlappingPairArray.size()) {
      ...
      cleanOverlappingPair(pair, dispatcher);
      // if (m_ghostPairCallback)
      //	m_ghostPairCallback->removeOverlappingPair(proxy0,
      //proxy1,dispatcher);

      m_overlappingPairArray.swap(findIndex,
                                  m_overlappingPairArray.capacity() - 1);
      m_overlappingPairArray.pop_back();
      return 0;
    }
...
b3BroadphasePair* b3SortedOverlappingPairCache::addOverlappingPair(int proxy0,
                                                                   int proxy1) {
...
/// this findPair becomes really slow. Either sort the list to speedup the
/// query, or
/// use a different solution. It is mainly used for Removing overlapping pairs.
/// Removal could be delayed.
/// we could keep a linked list in each proxy, and store pair in one of the
/// proxies (with lowest memory address)
/// Also we can use a 2D bitmap, which can be useful for a future GPU
/// implementation
b3BroadphasePair* b3SortedOverlappingPairCache::findPair(int proxy0,
                                                         int proxy1) {
  if (!needsBroadphaseCollision(proxy0, proxy1)) return 0;
...
void b3SortedOverlappingPairCache::processAllOverlappingPairs(
    b3OverlapCallback* callback, b3Dispatcher* dispatcher) {
  int i;
...
void b3SortedOverlappingPairCache::cleanOverlappingPair(
    b3BroadphasePair& pair, b3Dispatcher* dispatcher) {
  /*	if (pair.m_algorithm)
  {
          {
...
void b3SortedOverlappingPairCache::cleanProxyFromPairs(
    int proxy, b3Dispatcher* dispatcher) {
  class CleanPairCallback : public b3OverlapCallback {
    int m_cleanProxy;
    b3OverlappingPairCache* m_pairCache;
...
void b3SortedOverlappingPairCache::removeOverlappingPairsContainingProxy(
    int proxy, b3Dispatcher* dispatcher) {
  class RemovePairCallback : public b3OverlapCallback {
    int m_obsoleteProxy;
...
void b3SortedOverlappingPairCache::sortOverlappingPairs(
    b3Dispatcher* dispatcher) {
