...
//
static B3_DBVT_INLINE void b3DeleteNode(b3DynamicBvh* pdbvt, b3DbvtNode* node) {
  b3AlignedFree(pdbvt->m_free);
  pdbvt->m_free = node;
}

//
static void b3RecurseDeleteNode(b3DynamicBvh* pdbvt, b3DbvtNode* node) {
  if (!node->isleaf()) {
...
//
static void b3InsertLeaf(b3DynamicBvh* pdbvt, b3DbvtNode* root,
                         b3DbvtNode* leaf) {
  if (!pdbvt->m_root) {
    pdbvt->m_root = leaf;
    leaf->parent = 0;
    ...
    if (!root->isleaf()) {
      do {
        root = root->childs[b3Select(leaf->volume, root->childs[0]->volume,
    ...
    if (prev) {
      ...
      do {
        if (!prev->volume.Contain(node->volume))
          b3Merge(prev->childs[0]->volume, prev->childs[1]->volume,
                  prev->volume);
        else
          break;
        node = prev;
...
//
static b3DbvtNode* b3RemoveLeaf(b3DynamicBvh* pdbvt, b3DbvtNode* leaf) {
...
//
static void b3FetchLeaves(b3DynamicBvh* pdbvt, b3DbvtNode* root,
                          b3NodeArray& leaves, int depth = -1) {
...
static bool b3LeftOfAxis(const b3DbvtNode* node, const b3Vector3& org,
...
// of axis. returns N.
static int b3Split(b3DbvtNode** leaves, int count, const b3Vector3& org,
                   const b3Vector3& axis) {
  int begin = 0;
  int end = count;
  for (;;) {
...
//
static b3DbvtVolume b3Bounds(b3DbvtNode** leaves, int count) {
#if B3_DBVT_MERGE_IMPL == B3_DBVT_IMPL_SSE
...
//
static void b3BottomUp(b3DynamicBvh* pdbvt, b3DbvtNode** leaves, int count) {
  while (count > 1) {
    ...
    for (int i = 0; i < count; ++i) {
      for (int j = i + 1; j < count; ++j) {
        const b3Scalar sz =
            b3Size(b3Merge(leaves[i]->volume, leaves[j]->volume));
        if (sz < minsize) {
          minsize = sz;
          minidx[0] = i;
          minidx[1] = j;
...
//
static b3DbvtNode* b3TopDown(b3DynamicBvh* pdbvt, b3DbvtNode** leaves,
                             int count, int bu_treshold) {
  static const b3Vector3 axis[] = {
      b3MakeVector3(1, 0, 0), b3MakeVector3(0, 1, 0), b3MakeVector3(0, 0, 1)};
    ...
    if (count > bu_treshold) {
      ...
      for (i = 0; i < 3; ++i) {
        if ((splitcount[i][0] > 0) && (splitcount[i][1] > 0)) {
          const int midp =
              (int)b3Fabs(b3Scalar(splitcount[i][0] - splitcount[i][1]));
          if (midp < bestmidp) {
            bestaxis = i;
            bestmidp = midp;
          }
      ...
      node->childs[1] =
          b3TopDown(pdbvt, &leaves[partition], count - partition, bu_treshold);
...
//
b3DynamicBvh::~b3DynamicBvh() { clear(); }

//
void b3DynamicBvh::clear() {
  if (m_root) b3RecurseDeleteNode(this, m_root);
...
void b3DynamicBvh::clone(b3DynamicBvh& dest, IClone* iclone) const {
  ...
  if (m_root != 0) {
    ...
    do {
      ...
      b3DbvtNode* n =
          b3CreateNode(&dest, e.parent, e.node->volume, e.node->data);
      stack.pop_back();
      if (e.parent != 0)
...
//
void b3DynamicBvh::extractLeaves(
    const b3DbvtNode* node, b3AlignedObjectArray<const b3DbvtNode*>& leaves) {
  if (node->isinternal()) {
...
/Ox /Ob2 /Oi /Ot /I "." /I "..\.." /I "..\..\src" /D "NDEBUG" /D "_LIB" /D
"_WINDOWS" /D "_CRT_SECURE_NO_DEPRECATE" /D "_CRT_NONSTDC_NO_DEPRECATE" /D
"WIN32"
/GF /FD /MT /GS- /Gy /arch:SSE2 /Zc:wchar_t-
/Fp"..\..\out\release8\build\libbulletcollision\libbulletcollision.pch"
...
struct b3DbvtBenchmark {
  ...
  static b3Scalar RandUnit() { return (rand() / (b3Scalar)RAND_MAX); }
  ...
  static b3DbvtVolume RandVolume(b3Scalar cs, b3Scalar eb, b3Scalar es) {
    return (b3DbvtVolume::FromCE(RandVector3(cs),
                                 b3Vector3(eb, eb, eb) + RandVector3() * es));
  }
  static b3Transform RandTransform(b3Scalar cs) {
    b3Transform t;
    t.setOrigin(RandVector3(cs));
    t.setRotation(b3Quaternion(RandUnit() * B3_PI * 2, RandUnit() * B3_PI * 2,
                               RandUnit() * B3_PI * 2)
                      .normalized());
    return (t);
  }
  static void RandTree(b3Scalar cs, b3Scalar eb, b3Scalar es, int leaves,
                       b3DynamicBvh& dbvt) {
    dbvt.clear();
...
void b3DynamicBvh::benchmark() {
  ...
  if (cfgBenchmark1_Enable) {  // Benchmark 1
    ...
    for (int i = 0; i < cfgLeaves; ++i) {
      volumes[i] = b3DbvtBenchmark::RandVolume(
          cfgVolumeCenterScale, cfgVolumeExentsBase, cfgVolumeExentsScale);
    }
    ...
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark1_Reference) * 100 / time);
  }
  if (cfgBenchmark2_Enable) {  // Benchmark 2
    ...
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark2_Reference) * 100 / time);
  }
  if (cfgBenchmark3_Enable) {  // Benchmark 3
    srand(380843);
    b3DynamicBvh dbvt[2];
    b3DbvtBenchmark::NilPolicy policy;
    b3DbvtBenchmark::RandTree(cfgVolumeCenterScale, cfgVolumeExentsBase,
                              cfgVolumeExentsScale, cfgLeaves, dbvt[0]);
    b3DbvtBenchmark::RandTree(cfgVolumeCenterScale, cfgVolumeExentsBase,
                              cfgVolumeExentsScale, cfgLeaves, dbvt[1]);
    ...
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark3_Reference) * 100 / time);
  }
  if (cfgBenchmark4_Enable) {  // Benchmark 4
    ...
    b3DbvtBenchmark::NilPolicy policy;
    b3DbvtBenchmark::RandTree(cfgVolumeCenterScale, cfgVolumeExentsBase,
                              cfgVolumeExentsScale, cfgLeaves, dbvt);
    ...
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark4_Reference) * 100 / time);
  }
  if (cfgBenchmark5_Enable) {  // Benchmark 5
    ...
    for (int i = 0; i < transforms.size(); ++i) {
      transforms[i] = b3DbvtBenchmark::RandTransform(cfgVolumeCenterScale *
                                                     cfgBenchmark5_OffsetScale);
    }
    b3DbvtBenchmark::RandTree(cfgVolumeCenterScale, cfgVolumeExentsBase,
                              cfgVolumeExentsScale, cfgLeaves, dbvt[0]);
    b3DbvtBenchmark::RandTree(cfgVolumeCenterScale, cfgVolumeExentsBase,
    ...
    for (int i = 0; i < cfgBenchmark5_Iterations; ++i) {
      b3DynamicBvh::collideTT(dbvt[0].m_root, dbvt[1].m_root, transforms[i],
                              policy);
    }
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark5_Reference) * 100 / time);
  }
  if (cfgBenchmark6_Enable) {  // Benchmark 6
    ...
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark6_Reference) * 100 / time);
  }
  if (cfgBenchmark7_Enable) {  // Benchmark 7
    ...
    for (int i = 0; i < cfgBenchmark7_Passes; ++i) {
      for (int j = 0; j < cfgBenchmark7_Iterations; ++j) {
        b3DynamicBvh::rayTest(dbvt.m_root, rayorg[j], rayorg[j] + raydir[j],
                              policy);
      }
    }
    const int time = (int)wallclock.getTimeMilliseconds();
    ...
    printf("%u ms (%i%%),(%u r/s)\r\n", time,
           (time - cfgBenchmark7_Reference) * 100 / time, (rays * 1000) / time);
  }
  if (cfgBenchmark8_Enable) {  // Benchmark 8
    ...
    for (int i = 0; i < cfgBenchmark8_Passes; ++i) {
      for (int j = 0; j < cfgBenchmark8_Iterations; ++j) {
        dbvt.remove(dbvt.insert(b3DbvtBenchmark::RandVolume(
                                    cfgVolumeCenterScale, cfgVolumeExentsBase,
                                    cfgVolumeExentsScale),
                                0));
      }
    }
    ...
    printf("%u ms (%i%%),(%u ir/s)\r\n", time,
           (time - cfgBenchmark8_Reference) * 100 / time, ir * 1000 / time);
  }
  if (cfgBenchmark9_Enable) {  // Benchmark 9
    ...
    for (int i = 0; i < cfgBenchmark9_Passes; ++i) {
      for (int j = 0; j < cfgBenchmark9_Iterations; ++j) {
        dbvt.update(const_cast<b3DbvtNode*>(leaves[rand() % cfgLeaves]),
                    b3DbvtBenchmark::RandVolume(cfgVolumeCenterScale,
                                                cfgVolumeExentsBase,
                                                cfgVolumeExentsScale));
      }
    }
    ...
    printf("%u ms (%i%%),(%u u/s)\r\n", time,
           (time - cfgBenchmark9_Reference) * 100 / time, up * 1000 / time);
  }
  if (cfgBenchmark10_Enable) {  // Benchmark 10
    ...
    for (int i = 0; i < vectors.size(); ++i) {
      vectors[i] = (b3DbvtBenchmark::RandVector3() * 2 - b3Vector3(1, 1, 1)) *
                   cfgBenchmark10_Scale;
    }
    b3DbvtBenchmark::RandTree(cfgVolumeCenterScale, cfgVolumeExentsBase,
                              cfgVolumeExentsScale, cfgLeaves, dbvt);
    dbvt.optimizeTopDown();
    ...
    for (int i = 0; i < cfgBenchmark10_Passes; ++i) {
      for (int j = 0; j < cfgBenchmark10_Iterations; ++j) {
        ...
        b3DbvtVolume v =
            b3DbvtVolume::FromMM(l->volume.Mins() + d, l->volume.Maxs() + d);
        dbvt.update(l, v);
      }
    }
    const int time = (int)wallclock.getTimeMilliseconds();
    ...
    printf("%u ms (%i%%),(%u u/s)\r\n", time,
           (time - cfgBenchmark10_Reference) * 100 / time, up * 1000 / time);
  }
  if (cfgBenchmark11_Enable) {  // Benchmark 11
    ...
    printf("%u ms (%i%%),(%u o/s)\r\n", time,
           (time - cfgBenchmark11_Reference) * 100 / time, op / time * 1000);
  }
  if (cfgBenchmark12_Enable) {  // Benchmark 12
    ...
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark12_Reference) * 100 / time);
  }
  if (cfgBenchmark13_Enable) {  // Benchmark 13
    ...
    printf("%u ms (%i%%),(%u t/s)\r\n", time,
           (time - cfgBenchmark13_Reference) * 100 / time, (t * 1000) / time);
  }
  if (cfgBenchmark14_Enable) {  // Benchmark 14
    ...
    printf("%u ms (%i%%),(%u t/s)\r\n", time,
           (time - cfgBenchmark14_Reference) * 100 / time, (t * 1000) / time);
  }
  if (cfgBenchmark15_Enable) {  // Benchmark 15
    ...
    printf("%u ms (%i%%),(%u t/s)\r\n", time,
           (time - cfgBenchmark15_Reference) * 100 / time, (t * 1000) / time);
  }
  if (cfgBenchmark16_Enable) {  // Benchmark 16
    ...
    for (int i = 0; i < cfgBenchmark16_Passes; ++i) {
      for (int j = 0; j < cfgBenchmark16_BatchCount; ++j) {
        batch.push_back(
            dbvt.insert(b3DbvtBenchmark::RandVolume(cfgVolumeCenterScale,
                                                    cfgVolumeExentsBase,
                                                    cfgVolumeExentsScale),
                        0));
      }
    ...
    printf("%u ms (%i%%),(%u bir/s)\r\n", time,
           (time - cfgBenchmark16_Reference) * 100 / time,
           int(ir * 1000.0 / time));
  }
  if (cfgBenchmark17_Enable) {  // Benchmark 17
    ...
    const int time = (int)wallclock.getTimeMilliseconds();
    printf("%u ms (%i%%)\r\n", time,
           (time - cfgBenchmark17_Reference) * 100 / time);
  }
  printf("\r\n\r\n");
}
#endif
