...
/// The b3OverlappingPairCache provides an interface for overlapping pair
/// management (add, remove, storage), used by the b3BroadphaseInterface
/// broadphases.
/// The b3HashedOverlappingPairCache and b3SortedOverlappingPairCache classes
/// are two implementations.
class b3OverlappingPairCache {
 public:
  virtual ~b3OverlappingPairCache() {
  ...
  virtual void cleanOverlappingPair(b3BroadphasePair& pair,
                                    b3Dispatcher* dispatcher) = 0;
  ...
  virtual void processAllOverlappingPairs(b3OverlapCallback*,
                                          b3Dispatcher* dispatcher) = 0;
...
class b3HashedOverlappingPairCache : public b3OverlappingPairCache {
 ...
 public:
  ...
  virtual void removeOverlappingPairsContainingProxy(int proxy,
  ...
  B3_FORCE_INLINE bool needsBroadphaseCollision(int proxy0, int proxy1) const {
    ...
    bool collides = true;  //(proxy0->m_collisionFilterGroup &
  ...
  int getNumOverlappingPairs() const { return m_overlappingPairArray.size(); }
 ...
 private:
  ...
  B3_FORCE_INLINE bool equalsPair(const b3BroadphasePair& pair, int proxyId1,
                                  int proxyId2) {
  ...
  */
  ...
  B3_FORCE_INLINE unsigned int getHash(unsigned int proxyId1,
                                       unsigned int proxyId2) {
    int key = static_cast<int>(((unsigned int)proxyId1) |
  ...
  B3_FORCE_INLINE b3BroadphasePair* internalFindPair(int proxy0, int proxy1,
                                                     int hash) {
    int proxyId1 = proxy0;
    int proxyId2 = proxy1;
#if 0  // wrong, 'equalsPair' use unsorted uids, copy-past devil striked again.
       // Nat.
  ...
  virtual bool hasDeferredRemoval() { return false; }

  /*	virtual	void
  setInternalGhostPairCallback(b3OverlappingPairCallback* ghostPairCallback)
  {
...
class b3SortedOverlappingPairCache : public b3OverlappingPairCache {
 protected:
  ...
  // if set, use the callback instead of the built in filter in
 ...
 public:
  ...
  virtual bool hasDeferredRemoval() { return m_hasDeferredRemoval; }
...
/// b3NullPairCache skips add/removal of overlapping pairs. Userful for
...
class b3NullPairCache : public b3OverlappingPairCache {
 ...
 public:
  ...
  virtual int getNumOverlappingPairs() const { return 0; }

  virtual void cleanProxyFromPairs(int /*proxy*/,
                                   b3Dispatcher* /*dispatcher*/) {}
  ...
  virtual bool hasDeferredRemoval() { return true; }
  ...
  virtual void removeOverlappingPairsContainingProxy(
      int /*proxy0*/, b3Dispatcher* /*dispatcher*/) {}
