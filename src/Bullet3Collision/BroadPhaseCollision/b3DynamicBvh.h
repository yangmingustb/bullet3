...
struct b3DbvtAabbMm {
  ...
  B3_DBVT_INLINE b3Scalar ProjectMinimum(const b3Vector3& v,
 ...
 private:
  B3_DBVT_INLINE void AddSpan(const b3Vector3& d, b3Scalar& smi,
                              b3Scalar& smx) const;
...
/// on axis aligned bounding boxes (aabb tree).
/// This b3DynamicBvh is used for soft body collision detection and for the
/// b3DynamicBvhBroadphase. It has a fast insert, remove and update of nodes.
/// Unlike the b3QuantizedBvh, nodes can be dynamically moved around, which
...
struct b3DynamicBvh {
  ...
  struct sStkNPS {
    ...
    b3Scalar value;
    sStkNPS() {}
    sStkNPS(const b3DbvtNode* n, unsigned m, b3Scalar v)
  ...
  struct IWriter {
    ...
    virtual void WriteNode(const b3DbvtNode*, int index, int parent, int child0,
                           int child1) = 0;
  ...
  B3_DBVT_PREFIX
  void collideTT(const b3DbvtNode* root0, const b3DbvtNode* root1,
                 B3_DBVT_IPOLICY);
  ...
  /// rayTest is a re-entrant ray test, and can be called in parallel as long as
  /// the b3AlignedAlloc is thread-safe (uses locking etc)
  /// rayTest is slower than rayTestInternal, because it builds a local stack,
  /// using memory allocations, and it recomputes signs/rayDirectionInverses
  /// each time
  B3_DBVT_PREFIX
  static void rayTest(const b3DbvtNode* root, const b3Vector3& rayFrom,
                      const b3Vector3& rayTo, B3_DBVT_IPOLICY);
  /// rayTestInternal is faster than rayTest, because it uses a persistent stack
  /// (to reduce dynamic memory allocations to a minimum) and it uses
  /// precomputed signs/rayInverseDirections
  /// rayTestInternal is used by b3DynamicBvhBroadphase to accelerate world ray
  /// casts
  B3_DBVT_PREFIX
  void rayTestInternal(const b3DbvtNode* root, const b3Vector3& rayFrom,
                       const b3Vector3& rayTo,
                       const b3Vector3& rayDirectionInverse,
                       unsigned int signs[3], b3Scalar lambda_max,
                       const b3Vector3& aabbMin, const b3Vector3& aabbMax,
                       B3_DBVT_IPOLICY) const;

  B3_DBVT_PREFIX
  static void collideKDOP(const b3DbvtNode* root, const b3Vector3* normals,
                          const b3Scalar* offsets, int count, B3_DBVT_IPOLICY);
  B3_DBVT_PREFIX
  static void collideOCL(const b3DbvtNode* root, const b3Vector3* normals,
                         const b3Scalar* offsets, const b3Vector3& sortaxis,
                         int count, B3_DBVT_IPOLICY, bool fullsort = true);
  B3_DBVT_PREFIX
  static void collideTU(const b3DbvtNode* root, B3_DBVT_IPOLICY);
  // Helpers
  static B3_DBVT_INLINE int nearest(const int* i,
                                    const b3DynamicBvh::sStkNPS* a, b3Scalar v,
                                    int l, int h) {
    int m = 0;
    while (l < h) {
      m = (l + h) >> 1;
...
//
inline b3DbvtAabbMm b3DbvtAabbMm::FromCE(const b3Vector3& c,
                                         const b3Vector3& e) {
  b3DbvtAabbMm box;
  box.mi = c - e;
  box.mx = c + e;
...
//
inline b3DbvtAabbMm b3DbvtAabbMm::FromMM(const b3Vector3& mi,
                                         const b3Vector3& mx) {
  b3DbvtAabbMm box;
  box.mi = mi;
  box.mx = mx;
...
//
B3_DBVT_INLINE int b3DbvtAabbMm::Classify(const b3Vector3& n, b3Scalar o,
                                          int s) const {
  b3Vector3 pi, px;
  switch (s) {
    case (0 + 0 + 0):
...
B3_DBVT_INLINE b3Scalar b3DbvtAabbMm::ProjectMinimum(const b3Vector3& v,
  ...
  const b3Vector3 p = b3MakeVector3(
      b[(signs >> 0) & 1]->x, b[(signs >> 1) & 1]->y, b[(signs >> 2) & 1]->z);
  return (b3Dot(p, v));
}
...
#else
  return ((a.mi.x <= b.mx.x) && (a.mx.x >= b.mi.x) && (a.mi.y <= b.mx.y) &&
          (a.mx.y >= b.mi.y) && (a.mi.z <= b.mx.z) && (a.mx.z >= b.mi.z));
#endif
}
...
//
B3_DBVT_INLINE int b3Select(const b3DbvtAabbMm& o, const b3DbvtAabbMm& a,
                            const b3DbvtAabbMm& b) {
#if B3_DBVT_SELECT_IMPL == B3_DBVT_IMPL_SSE

#if defined(_WIN32)
  static B3_ATTRIBUTE_ALIGNED16(const unsigned __int32)
      mask[] = {0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff};
#else
  static B3_ATTRIBUTE_ALIGNED16(const unsigned int)
      mask[] = {0x7fffffff, 0x7fffffff, 0x7fffffff, 0x00000000 /*0x7fffffff*/};
#endif
///@todo: the intrinsic version is 11% slower
#if B3_DBVT_USE_INTRINSIC_SSE

  union b3SSEUnion  /// NOTE: if we use more intrinsics, move b3SSEUnion into
...
B3_DBVT_INLINE bool b3NotEqual(const b3DbvtAabbMm& a, const b3DbvtAabbMm& b) {
  return ((a.mi.x != b.mi.x) || (a.mi.y != b.mi.y) || (a.mi.z != b.mi.z) ||
...
B3_DBVT_PREFIX
inline void b3DynamicBvh::rayTestInternal(
    const b3DbvtNode* root, const b3Vector3& rayFrom, const b3Vector3& rayTo,
    const b3Vector3& rayDirectionInverse, unsigned int signs[3],
    b3Scalar lambda_max, const b3Vector3& aabbMin, const b3Vector3& aabbMax,
    B3_DBVT_IPOLICY) const {
  (void)rayTo;
  B3_DBVT_CHECKTYPE
  if (root) {
    ...
    do {
      ...
      result1 = b3RayAabb2(rayFrom, rayDirectionInverse, signs, bounds, tmin,
...
inline void b3DynamicBvh::rayTest(const b3DbvtNode* root,
  ...
  if (root) {
    ...
    do {
      ...
      b3Scalar tmin = 1.f, lambda_min = 0.f;
      unsigned int result1 = b3RayAabb2(rayFrom, rayDirectionInverse, signs,
                                        bounds, tmin, lambda_min, lambda_max);

#ifdef COMPARE_BTRAY_AABB2
      b3Scalar param = 1.f;
      bool result2 = b3RayAabb(rayFrom, rayTo, node->volume.Mins(),
...
inline void b3DynamicBvh::collideKDOP(const b3DbvtNode* root,
                                      ...
                                      const b3Scalar* offsets, int count,
                                      B3_DBVT_IPOLICY) {
  B3_DBVT_CHECKTYPE
  if (root) {
...
inline void b3DynamicBvh::collideOCL(const b3DbvtNode* root,
                                     ...
                                     const b3Vector3& sortaxis, int count,
                                     B3_DBVT_IPOLICY, bool fsort) {
  B3_DBVT_CHECKTYPE
  if (root) {
...
//
B3_DBVT_PREFIX
inline void b3DynamicBvh::collideTU(const b3DbvtNode* root, B3_DBVT_IPOLICY) {
  B3_DBVT_CHECKTYPE
  if (root) {
    b3AlignedObjectArray<const b3DbvtNode*> stack;
    stack.reserve(B3_SIMPLE_STACKSIZE);
