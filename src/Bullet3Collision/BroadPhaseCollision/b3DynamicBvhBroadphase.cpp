...
struct b3DbvtTreeCollider : b3DynamicBvh::ICollide {
  ...
  void Process(const b3DbvtNode* na, const b3DbvtNode* nb) {
    if (na != nb) {
      ...
      if (pa->m_uniqueId > pb->m_uniqueId) b3Swap(pa, pb);
#endif
      pbp->m_paircache->addOverlappingPair(pa->getUid(), pb->getUid());
      ++pbp->m_newpairs;
    }
  }
  void Process(const b3DbvtNode* n) { Process(n, proxy->leaf); }
};
...
b3DynamicBvhBroadphase::b3DynamicBvhBroadphase(
    int proxyCapacity, b3OverlappingPairCache* paircache) {
  ...
  m_paircache =
      paircache ? paircache
                : new (b3AlignedAlloc(sizeof(b3HashedOverlappingPairCache), 16))
...
void b3DynamicBvhBroadphase::destroyProxy(b3BroadphaseProxy* absproxy,
  ...
  m_paircache->removeOverlappingPairsContainingProxy(proxy->getUid(),
                                                     dispatcher);
...
/*
void	b3DynamicBvhBroadphase::getAabb(b3BroadphaseProxy*
absproxy,b3Vector3& aabbMin, b3Vector3& aabbMax ) const
{
        b3DbvtProxy*
proxy=(b3DbvtProxy*)absproxy;
        aabbMin = proxy->m_aabbMin;
        aabbMax = proxy->m_aabbMax;
}
*/
...
void b3DynamicBvhBroadphase::rayTest(const b3Vector3& rayFrom,
  ...
  m_sets[0].rayTestInternal(m_sets[0].m_root, rayFrom, rayTo,
                            rayCallback.m_rayDirectionInverse,
                            rayCallback.m_signs, rayCallback.m_lambda_max,
                            aabbMin, aabbMax, callback);

  m_sets[1].rayTestInternal(m_sets[1].m_root, rayFrom, rayTo,
                            rayCallback.m_rayDirectionInverse,
                            rayCallback.m_signs, rayCallback.m_lambda_max,
                            aabbMin, aabbMax, callback);
}
...
void b3DynamicBvhBroadphase::aabbTest(const b3Vector3& aabbMin,
                                      const b3Vector3& aabbMax,
                                      b3BroadphaseAabbCallback& aabbCallback) {
  BroadphaseAabbTester callback(aabbCallback);

  const B3_ATTRIBUTE_ALIGNED16(b3DbvtVolume) bounds =
      b3DbvtVolume::FromMM(aabbMin, aabbMax);
...
//
void b3DynamicBvhBroadphase::setAabb(int objectId, const b3Vector3& aabbMin,
                                     const b3Vector3& aabbMax,
                                     b3Dispatcher* /*dispatcher*/) {
...
#endif
  {
    ...
    if (proxy->stage == STAGECOUNT) { /* fixed -> dynamic set	*/
      ...
      ++m_updates_call;
      if (b3Intersect(proxy->leaf->volume,
                      aabb)) { /* Moving				*/

        const b3Vector3 delta = aabbMin - proxy->m_aabbMin;
        b3Vector3 velocity(((proxy->m_aabbMax - proxy->m_aabbMin) / 2) *
                           m_prediction);
    ...
    if (docollide) {
      ...
      if (!m_deferedcollide) {
        b3DbvtTreeCollider collider(this);
        m_sets[1].collideTTpersistentStack(m_sets[1].m_root, proxy->leaf,
                                           collider);
        m_sets[0].collideTTpersistentStack(m_sets[0].m_root, proxy->leaf,
                                           collider);
      }
    }
  }
...
//
void b3DynamicBvhBroadphase::calculateOverlappingPairs(
    b3Dispatcher* dispatcher) {
  collide(dispatcher);
#if B3_DBVT_BP_PROFILE
  if (0 == (m_pid % B3_DBVT_BP_PROFILING_RATE)) {
    printf("fixed(%u) dynamics(%u) pairs(%u)\r\n", m_sets[1].m_leaves,
           m_sets[0].m_leaves, m_paircache->getNumOverlappingPairs());
    unsigned int total = m_profiling.m_total;
    if (total <= 0) total = 1;
    printf("ddcollide: %u%% (%uus)\r\n",
           (50 + m_profiling.m_ddcollide * 100) / total,
           m_profiling.m_ddcollide / B3_DBVT_BP_PROFILING_RATE);
    printf("fdcollide: %u%% (%uus)\r\n",
           (50 + m_profiling.m_fdcollide * 100) / total,
           m_profiling.m_fdcollide / B3_DBVT_BP_PROFILING_RATE);
    printf("cleanup:   %u%% (%uus)\r\n",
           (50 + m_profiling.m_cleanup * 100) / total,
           m_profiling.m_cleanup / B3_DBVT_BP_PROFILING_RATE);
    printf("total:     %uus\r\n", total / B3_DBVT_BP_PROFILING_RATE);
    const unsigned long sum = m_profiling.m_ddcollide +
                              m_profiling.m_fdcollide + m_profiling.m_cleanup;
    printf("leaked: %u%% (%uus)\r\n", 100 - ((50 + sum * 100) / total),
           (total - sum) / B3_DBVT_BP_PROFILING_RATE);
    printf("job counts: %u%%\r\n",
           (m_profiling.m_jobcount * 100) /
               ((m_sets[0].m_leaves + m_sets[1].m_leaves) *
                B3_DBVT_BP_PROFILING_RATE));
    b3Clear(m_profiling);
    m_clock.reset();
  }
#endif
...
void b3DynamicBvhBroadphase::performDeferredRemoval(b3Dispatcher* dispatcher) {
  if (m_paircache->hasDeferredRemoval()) {
    b3BroadphasePairArray& overlappingPairArray =
    ...
    for (i = 0; i < overlappingPairArray.size(); i++) {
      ...
      if (!isDuplicate) {
        // important to perform AABB check that is consistent with the
        // broadphase
...
*/
  ...
  if (m_needcleanup) {
    ...
    if (pairs.size() > 0) {
      int ni = b3Min(pairs.size(),
                     b3Max<int>(m_newpairs, (pairs.size() * m_cupdates) / 100));
      for (int i = 0; i < ni; ++i) {
        ...
        if (!b3Intersect(pa->leaf->volume, pb->leaf->volume)) {
          ...
          if (pa->m_uniqueId > pb->m_uniqueId) b3Swap(pa, pb);
#endif
          m_paircache->removeOverlappingPair(pa->getUid(), pb->getUid(),
                                             dispatcher);
          --ni;
          --i;
        }
...
//
const b3OverlappingPairCache* b3DynamicBvhBroadphase::getOverlappingPairCache()
    const {
  return (m_paircache);
}

//
void b3DynamicBvhBroadphase::getBroadphaseAabb(b3Vector3& aabbMin,
                                               b3Vector3& aabbMax) const {
  B3_ATTRIBUTE_ALIGNED16(b3DbvtVolume)
  bounds;
  ...
  if (!m_sets[0].empty())
    if (!m_sets[1].empty())
      b3Merge(m_sets[0].m_root->volume, m_sets[1].m_root->volume, bounds);
    else
...
struct b3BroadphaseBenchmark {
  ...
  struct Object {
    ...
    b3Scalar time;
    void update(b3Scalar speed, b3Scalar amplitude,
                b3BroadphaseInterface* pbi) {
      time += speed;
      center[0] = b3Cos(time * (b3Scalar)2.17) * amplitude +
                  b3Sin(time) * amplitude / 2;
      center[1] =
          b3Cos(time * (b3Scalar)1.38) * amplitude + b3Sin(time) * amplitude;
...
{"8192o.10%",8192,10,0,8192,(b3Scalar)0.005,(b3Scalar)100},*/
  ...
  for (int iexp = 0; iexp < nexperiments; ++iexp) {
    ...
    for (int i = 0; i < object_count; ++i) {
      ...
      po->proxy = pbi->createProxy(po->center - po->extents,
                                   po->center + po->extents, 0, po, 1, 1, 0, 0);
    ...
    b3BroadphaseBenchmark::OutputTime("\tUpdate", wallclock,
...
#else
/*void
b3DynamicBvhBroadphase::benchmark(b3BroadphaseInterface*)
{}
*/
#endif
