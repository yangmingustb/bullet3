...
b3BroadphaseProxy {
  ...
  int m_uniqueId;  // m_uniqueId is introduced for paircache. could get rid of
                   // this, by calculating the address offset etc.

  b3Vector3 m_aabbMin;
  b3Vector3 m_aabbMax;

  B3_FORCE_INLINE int getUid() const { return m_uniqueId; }

  // used for memory pools
  b3BroadphaseProxy() : m_clientObject(0) {}

  b3BroadphaseProxy(const b3Vector3& aabbMin, const b3Vector3& aabbMax,
                    void* userPtr, int collisionFilterGroup,
...
struct b3DbvtProxy : b3BroadphaseProxy {
  ...
  b3DbvtProxy(const b3Vector3& aabbMin, const b3Vector3& aabbMax, void* userPtr,
              int collisionFilterGroup, int collisionFilterMask)
      : b3BroadphaseProxy(aabbMin, aabbMax, userPtr, collisionFilterGroup,
                          collisionFilterMask) {
    links[0] = links[1] = 0;
  }
};
...
/// The b3DynamicBvhBroadphase implements a broadphase using two dynamic AABB
/// bounding volume hierarchies/trees (see b3DynamicBvh).
/// One tree is used for static/non-moving objects, and another tree is used for
/// dynamic objects. Objects can move from one tree to the other.
/// This is a very fast broadphase, especially for very dynamic worlds where
/// many objects are moving. Its insert/add and remove of objects is generally
/// faster than the sweep and prune broadphases b3AxisSweep3 and
/// b332BitAxisSweep3.
struct b3DynamicBvhBroadphase {
  ...
  b3DynamicBvhBroadphase(int proxyCapacity,
                         b3OverlappingPairCache* paircache = 0);
  virtual ~b3DynamicBvhBroadphase();
  void collide(b3Dispatcher* dispatcher);
  void optimize();

  /* b3BroadphaseInterface Implementation	*/
  b3BroadphaseProxy* createProxy(const b3Vector3& aabbMin,
                                 const b3Vector3& aabbMax, int objectIndex,
                                 void* userPtr, int collisionFilterGroup,
                                 int collisionFilterMask);
  virtual void destroyProxy(b3BroadphaseProxy* proxy, b3Dispatcher* dispatcher);
  virtual void setAabb(int objectId, const b3Vector3& aabbMin,
                       const b3Vector3& aabbMax, b3Dispatcher* dispatcher);
  virtual void rayTest(const b3Vector3& rayFrom, const b3Vector3& rayTo,
                       b3BroadphaseRayCallback& rayCallback,
                       const b3Vector3& aabbMin = b3MakeVector3(0, 0, 0),
                       const b3Vector3& aabbMax = b3MakeVector3(0, 0, 0));
  virtual void aabbTest(const b3Vector3& aabbMin, const b3Vector3& aabbMax,
                        b3BroadphaseAabbCallback& callback);

  // virtual void					getAabb(b3BroadphaseProxy* proxy,b3Vector3&
  // aabbMin, b3Vector3& aabbMax ) const;
  virtual void getAabb(int objectId, b3Vector3& aabbMin,
  ...
  void setVelocityPrediction(b3Scalar prediction) { m_prediction = prediction; }
  b3Scalar getVelocityPrediction() const { return m_prediction; }

  /// this setAabbForceUpdate is similar to setAabb but always forces the aabb
  /// update.
  /// it is not part of the b3BroadphaseInterface but specific to
  /// b3DynamicBvhBroadphase.
  /// it bypasses certain optimizations that prevent aabb updates (when the aabb
  /// shrinks), see
  /// http://code.google.com/p/bullet/issues/detail?id=223
  void setAabbForceUpdate(b3BroadphaseProxy* absproxy, const b3Vector3& aabbMin,
                          const b3Vector3& aabbMax,
                          b3Dispatcher* /*dispatcher*/);

  // static void
  // benchmark(b3BroadphaseInterface*);
};
