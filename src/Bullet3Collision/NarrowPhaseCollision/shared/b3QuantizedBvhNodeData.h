...
inline void b3QuantizeWithClamp(unsigned short* out, b3Float4ConstArg point2,
                                int isMax, b3Float4ConstArg bvhAabbMin,
                                b3Float4ConstArg bvhAabbMax,
...
inline int b3TestQuantizedAabbAgainstQuantizedAabbSlow(
  ...
  return 1;
  // overlap = ((aabbMin1[0] > aabbMax2[0]) || (aabbMax1[0] < aabbMin2[0])) ? 0
  // : overlap;
  // overlap = ((aabbMin1[2] > aabbMax2[2]) || (aabbMax1[2] < aabbMin2[2])) ? 0
  // : overlap;
  // overlap = ((aabbMin1[1] > aabbMax2[1]) || (aabbMax1[1] < aabbMin2[1])) ? 0
  // : overlap;
  // return overlap;
}
