...
bool b3ConvexUtility::initializePolyhedralFeatures(
    const b3Vector3* orgVertices, int numPoints, bool mergeCoplanarTriangles) {
  ...
  while (todoFaces.size()) {
    ...
    b3Vector3 faceNormalA =
        b3MakeVector3(faceA.m_plane[0], faceA.m_plane[1], faceA.m_plane[2]);
    for (int j = todoFaces.size() - 1; j >= 0; j--) {
      int i = todoFaces[j];
      b3MyFace& faceB = tmpFaces[i];
      b3Vector3 faceNormalB =
          b3MakeVector3(faceB.m_plane[0], faceB.m_plane[1], faceB.m_plane[2]);
    ...
    if (coplanarFaceGroup.size() > 1) {
      ...
      for (int i = 0; i < coplanarFaceGroup.size(); i++) {
        ...
        b3Vector3 faceNormal =
            b3MakeVector3(face.m_plane[0], face.m_plane[1], face.m_plane[2]);
        ...
        for (int f = 0; f < face.m_indices.size(); f++) {
          ...
          if (!found) orgpoints.push_back(b3GrahamVector3(pt, orgIndex));
        }
      }
...
inline bool IsAlmostZero(const b3Vector3& v) {
  if (fabsf(v.getX()) > 1e-6 || fabsf(v.getY()) > 1e-6 ||
      fabsf(v.getZ()) > 1e-6)
    return false;
  return true;
}
...
struct b3InternalVertexPair {
  b3InternalVertexPair(short int v0, short int v1) : m_v0(v0), m_v1(v1) {
    if (m_v1 > m_v0) b3Swap(m_v0, m_v1);
  }
  short int m_v0;
  short int m_v1;
  int getHash() const { return m_v0 + (m_v1 << 16); }
...
struct b3InternalEdge {
  b3InternalEdge() : m_face0(-1), m_face1(-1) {}
  short int m_face0;
  short int m_face1;
};
...
bool b3ConvexUtility::testContainment() const {
  for (int p = 0; p < 8; p++) {
    ...
    if (p == 0)
      LocalPt =
          m_localCenter + b3Vector3(m_extents[0], m_extents[1], m_extents[2]);
    else if (p == 1)
      LocalPt =
          m_localCenter + b3Vector3(m_extents[0], m_extents[1], -m_extents[2]);
    else if (p == 2)
      ...
      LocalPt = m_localCenter +
                b3Vector3(-m_extents[0], -m_extents[1], -m_extents[2]);

    for (int i = 0; i < m_faces.size(); i++) {
      const b3Vector3 Normal(m_faces[i].m_plane[0], m_faces[i].m_plane[1],
                             m_faces[i].m_plane[2]);
  ...
  for (int i = 0; i < m_faces.size(); i++) {
    ...
    for (int j = 0; j < NbTris; j++) {
      ...
      for (int p = 0; p < m_uniqueEdges.size(); p++) {
        ...
        if (IsAlmostZero(diff) || IsAlmostZero(diff2)) {
          found = true;
          break;
        }
      }
  ...
  if (1) {
    ...
    for (int i = 0; i < m_faces.size(); i++) {
      const b3Vector3 Normal(m_faces[i].m_plane[0], m_faces[i].m_plane[1],
                             m_faces[i].m_plane[2]);
      const b3Scalar dist =
          b3Fabs(m_localCenter.dot(Normal) + m_faces[i].m_plane[3]);
