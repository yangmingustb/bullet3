...
struct btVectorX {
  ...
  btVectorX() {}
  btVectorX(int numRows) { m_storage.resize(numRows); }

  void resize(int rows) { m_storage.resize(rows); }
  int cols() const { return 1; }
  int rows() const { return m_storage.size(); }
  ...
  T* getBufferPointerWritable() { return m_storage.size() ? &m_storage[0] : 0; }
...
struct btMatrixX {
  ...
  /// we don't want this read/write operator(), because we cannot keep track of
  /// non-zero elements, use setElem instead
  ...
  */
    ...
    for (int i = 0; i < rows(); ++i) {
      {
        for (int j = 0; j < other.cols(); ++j) {
          ...
          if (dotProd) res.setElem(i, j, dotProd);
        }
      }
    }
    return res;
  ...
  void multiplyAdd2_p8r(const btScalar* B, const btScalar* C, int numRows,
                        int numRowsOther, int row, int col) {
    const btScalar* bb = B;
  ...
  void multiply2_p8r(const btScalar* B, const btScalar* C, int numRows,
                     int numRowsOther, int row, int col) {
  ...
  void setSubMatrix(int rowstart, int colstart, int rowend, int colend,
                    const T value) {
