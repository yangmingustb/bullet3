...
struct btSpatialForceVector {
  ...
  btSpatialForceVector(const btVector3 &angular, const btVector3 &linear)
      : m_topVec(linear), m_bottomVec(angular) {}
  btSpatialForceVector(const btScalar &ax, const btScalar &ay,
                       const btScalar &az, const btScalar &lx,
                       const btScalar &ly, const btScalar &lz) {
  ...
  void setValue(const btScalar &ax, const btScalar &ay, const btScalar &az,
                const btScalar &lx, const btScalar &ly, const btScalar &lz) {
  ...
  void addValue(const btScalar &ax, const btScalar &ay, const btScalar &az,
                const btScalar &lx, const btScalar &ly, const btScalar &lz) {
    m_bottomVec[0] += ax;
    m_bottomVec[1] += ay;
    m_bottomVec[2] += az;
  ...
  btSpatialForceVector operator-(const btSpatialForceVector &vec) const {
    return btSpatialForceVector(m_bottomVec - vec.m_bottomVec,
                                m_topVec - vec.m_topVec);
  }
  btSpatialForceVector operator+(const btSpatialForceVector &vec) const {
    return btSpatialForceVector(m_bottomVec + vec.m_bottomVec,
  ...
  // btSpatialForceVector & operator = (const btSpatialForceVector &vec) {
  // m_topVec = vec.m_topVec; m_bottomVec = vec.m_bottomVec; return *this; }
};
...
struct btSpatialMotionVector {
  ...
  void cross(const SpatialVectorType &b, SpatialVectorType &out) const {
    ...
    out.m_bottomVec =
        m_bottomVec.cross(b.m_topVec) + m_topVec.cross(b.m_bottomVec);
  }
  template <typename SpatialVectorType>
  ...
  btSpatialMotionVector operator-(const btSpatialMotionVector &vec) const {
    return btSpatialMotionVector(m_topVec - vec.m_topVec,
                                 m_bottomVec - vec.m_bottomVec);
  }
  btSpatialMotionVector operator+(const btSpatialMotionVector &vec) const {
    return btSpatialMotionVector(m_topVec + vec.m_topVec,
...
struct btSymmetricSpatialDyad {
  ...
  btSymmetricSpatialDyad(const btMatrix3x3 &topLeftMat,
                         const btMatrix3x3 &topRightMat,
                         const btMatrix3x3 &bottomLeftMat) {
    setMatrix(topLeftMat, topRightMat, bottomLeftMat);
  }
  //
  void setMatrix(const btMatrix3x3 &topLeftMat, const btMatrix3x3 &topRightMat,
                 const btMatrix3x3 &bottomLeftMat) {
  ...
  //
  void addMatrix(const btMatrix3x3 &topLeftMat, const btMatrix3x3 &topRightMat,
                 const btMatrix3x3 &bottomLeftMat) {
    m_topLeftMat += topLeftMat;
    m_topRightMat += topRightMat;
    m_bottomLeftMat += bottomLeftMat;
...
struct btSpatialTransformationMatrix {
  ...
  //
  template <typename SpatialVectorType>
  void transform(const SpatialVectorType &inVec, SpatialVectorType &outVec,
                 eOutputOperation outOp = None) {
    if (outOp == None) {
      outVec.m_topVec = m_rotMat * inVec.m_topVec;
      outVec.m_bottomVec =
          -m_trnVec.cross(outVec.m_topVec) + m_rotMat * inVec.m_bottomVec;
    } else if (outOp == Add) {
      outVec.m_topVec += m_rotMat * inVec.m_topVec;
      outVec.m_bottomVec +=
          -m_trnVec.cross(outVec.m_topVec) + m_rotMat * inVec.m_bottomVec;
  ...
  void transformInverse(const SpatialVectorType &inVec,
    ...
    if (outOp == None) {
      ...
      outVec.m_bottomVec = m_rotMat.transpose() *
                           (inVec.m_bottomVec + m_trnVec.cross(inVec.m_topVec));
    } else if (outOp == Add) {
      outVec.m_topVec += m_rotMat.transpose() * inVec.m_topVec;
      outVec.m_bottomVec +=
          m_rotMat.transpose() *
  ...
  void transformInverse(const btSymmetricSpatialDyad &inMat,
                        ...
                        eOutputOperation outOp = None) {
    const btMatrix3x3 r_cross(0, -m_trnVec[2], m_trnVec[1], m_trnVec[2], 0,
                              -m_trnVec[0], -m_trnVec[1], m_trnVec[0], 0);

    if (outOp == None) {
      outMat.m_topLeftMat =
          m_rotMat.transpose() *
          (inMat.m_topLeftMat - inMat.m_topRightMat * r_cross) * m_rotMat;
      outMat.m_topRightMat =
          m_rotMat.transpose() * inMat.m_topRightMat * m_rotMat;
      outMat.m_bottomLeftMat =
          m_rotMat.transpose() *
          (r_cross * (inMat.m_topLeftMat - inMat.m_topRightMat * r_cross) +
           inMat.m_bottomLeftMat - inMat.m_topLeftMat.transpose() * r_cross) *
          m_rotMat;
    } else if (outOp == Add) {
      outMat.m_topLeftMat +=
          m_rotMat.transpose() *
...
template <typename SpatialVectorType>
btSymmetricSpatialDyad symmetricSpatialOuterProduct(
    const SpatialVectorType &a, const SpatialVectorType &b) {
  btSymmetricSpatialDyad out;
