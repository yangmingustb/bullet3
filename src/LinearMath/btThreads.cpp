...
// use Microsoft Parallel Patterns Library (installed with Visual Studio 2010
// and later)
...
// Using ordinary system-provided mutexes like Windows critical sections was
// noticeably slower
// presumably because when it fails to lock at first it would sleep the thread
// and trigger costly
// context switching.
//
...
bool btSpinMutex::tryLock() {
  ...
  int expected = 0;
  return std::atomic_compare_exchange_weak_explicit(aDest, &expected, int(1),
...
bool btSpinMutex::tryLock() {
  ...
  return __atomic_compare_exchange_n(&mLock, &expected, int(1), weak,
                                     memOrderSuccess, memOrderFail);
}
...
static int gThreadsRunningCounter =
    0;  // useful for detecting if we are trying to do nested parallel-for calls
static btSpinMutex gThreadsRunningCounterMutex;
static ThreadsafeCounter gThreadCounter;

//
// BT_DETECT_BAD_THREAD_INDEX tries to detect when there are multiple threads
// assigned the same thread index.
//
...
//      1. destroys all worker threads and creates all new ones in the correct
//      number, OR
//      2. never destroys a worker thread
//
// We make that assumption because we can't easily enumerate the worker threads
// of a task scheduler
// to assign nice sequential thread-indexes. We also do not get notified if a
// worker thread is destroyed,
...
//
// Our simple thread-counting scheme falls apart if the task scheduler destroys
// some threads but
// continues to re-use other threads and the application repeatedly resizes the
// thread pool of the
// task scheduler.
// In order to prevent the thread-counter from exceeding the global max
// (BT_MAX_THREAD_COUNT), we
// wrap the thread counter back to 1. This should only happen if the worker
// threads have all been
...
static ThreadId_t getDebugThreadId() { return GetCurrentThreadId(); }

#endif  // #if BT_DETECT_BAD_THREAD_INDEX

// return a unique index per thread, main thread is 0, worker threads are in [1,
// BT_MAX_THREAD_COUNT)
  ...
  if (gBtTaskScheduler && sThreadIndex > 0) {
    ...
    if (gDebugThreadIds[sThreadIndex] == kInvalidThreadId) {
      ...
      if (gDebugThreadIds[sThreadIndex] != tid) {
        // this could indicate the task scheduler is breaking our assumptions
        // about
...
bool btIsMainThread() { return btGetCurrentThreadIndex() == 0; }
...
void btITaskScheduler::activate() {
  // gThreadCounter is used to assign a thread-index to each worker thread in a
  // task scheduler.
  // The main thread is always thread-index 0, and worker threads are numbered
  // from 1 to 63 (BT_MAX_THREAD_COUNT-1)
  // The thread-indexes need to be unique amongst the threads that can be
  // running simultaneously.
  // Since only one task scheduler can be used at a time, it is OK for a pair of
  // threads that belong to different
  // task schedulers to share the same thread index because they can't be
  // running at the same time.
  // So each task scheduler needs to keep its own thread counter value
  if (!m_isActive) {
    gThreadCounter.mCounter =
        m_savedThreadCounter;  // restore saved thread counter
    m_isActive = true;
  }
}
...
bool btThreadsAreRunning() { return gThreadsRunningCounter != 0; }

void btSetTaskScheduler(btITaskScheduler* ts) {
  int threadId = btGetCurrentThreadIndex();  // make sure we call this on main
                                             // thread at least once before any
                                             // workers run
  if (threadId != 0) {
...
btITaskScheduler* btGetTaskScheduler() { return gBtTaskScheduler; }

void btParallelFor(int iBegin, int iEnd, int grainSize,
  ...
  btAssert(
      gBtTaskScheduler !=
      NULL);  // call btSetTaskScheduler() with a valid task scheduler first!
...
btScalar btParallelSum(int iBegin, int iEnd, int grainSize,
...
///                              (really just useful for testing performance of
...
class btTaskSchedulerSequential : public btITaskScheduler {
 public:
  ...
  virtual void parallelFor(int iBegin, int iEnd, int grainSize,
                           const btIParallelForBody& body) BT_OVERRIDE {
    BT_PROFILE("parallelFor_sequential");
    body.forLoop(iBegin, iEnd);
  }
  virtual btScalar parallelSum(int iBegin, int iEnd, int grainSize,
                               const btIParallelSumBody& body) BT_OVERRIDE {
...
class btTaskSchedulerOpenMP : public btITaskScheduler {
 ...
 public:
  btTaskSchedulerOpenMP() : btITaskScheduler("OpenMP") { m_numThreads = 0; }
  virtual int getMaxNumThreads() const BT_OVERRIDE {
    return omp_get_max_threads();
  }
  virtual int getNumThreads() const BT_OVERRIDE { return m_numThreads; }
  virtual void setNumThreads(int numThreads) BT_OVERRIDE {
    // With OpenMP, because it is a standard with various implementations, we
    // can't
    // know for sure if every implementation has the same behavior of destroying
    // all
    // previous threads when resizing the threadpool
    m_numThreads =
        (std::max)(1, (std::min)(int(BT_MAX_THREAD_COUNT), numThreads));
    omp_set_num_threads(
        1);  // hopefully, all previous threads get destroyed here
...
#if BT_USE_TBB && BT_THREADSAFE
///
/// btTaskSchedulerTBB -- wrapper around Intel Threaded Building Blocks task
/// scheduler
///
class btTaskSchedulerTBB : public btITaskScheduler {
 ...
 public:
  ...
  virtual void parallelFor(int iBegin, int iEnd, int grainSize,
    ...
    btPushThreadsAreRunning();
    tbb::parallel_for(tbb::blocked_range<int>(iBegin, iEnd, grainSize), tbbBody,
  ...
  struct SumBodyAdapter {
    ...
    SumBodyAdapter(const btIParallelSumBody* body)
        : mBody(body), mSum(btScalar(0)) {}
    SumBodyAdapter(const SumBodyAdapter& src, tbb::split)
        : mBody(src.mBody), mSum(btScalar(0)) {}
  ...
  virtual btScalar parallelSum(int iBegin, int iEnd, int grainSize,
    ...
    tbb::parallel_deterministic_reduce(
        tbb::blocked_range<int>(iBegin, iEnd, grainSize), tbbBody);
    btPopThreadsAreRunning();
    return tbbBody.mSum;
  }
};
...
#if BT_USE_PPL && BT_THREADSAFE
///
/// btTaskSchedulerPPL -- wrapper around Microsoft Parallel Patterns Lib task
/// scheduler
///
class btTaskSchedulerPPL : public btITaskScheduler {
 ...
 public:
  btTaskSchedulerPPL() : btITaskScheduler("PPL") { m_numThreads = 0; }
  virtual int getMaxNumThreads() const BT_OVERRIDE {
  ...
  virtual void setNumThreads(int numThreads) BT_OVERRIDE {
    ...
    {
      // PPL seems to destroy threads when threadpool is shrunk, but keeps
      // reusing old threads
  ...
  struct ForBodyAdapter {
    ...
    ForBodyAdapter(const btIParallelForBody* body, int grainSize, int end)
        : mBody(body), mGrainSize(grainSize), mIndexEnd(end) {}
  ...
  virtual void parallelFor(int iBegin, int iEnd, int grainSize,
    ...
    concurrency::parallel_for(iBegin, iEnd, grainSize, pplBody);
    btPopThreadsAreRunning();
  }
  struct SumBodyAdapter {
    const btIParallelSumBody* mBody;
    ...
    SumBodyAdapter(const btIParallelSumBody* body,
                   concurrency::combinable<btScalar>* sum, int grainSize,
                   int end)
        : mBody(body), mSum(sum), mGrainSize(grainSize), mIndexEnd(end) {}
    void operator()(int i) const {
