...
class btConvexHullInternal {
 public:
  class Point64 {
   public:
    ...
    int64_t dot(const Point64& b) const { return x * b.x + y * b.y + z * b.z; }
  };
  ...
  class Point32 {
   public:
    ...
    int64_t dot(const Point32& b) const { return x * b.x + y * b.y + z * b.z; }
  ...
  class Int128 {
   public:
    ...
    Int128(int64_t value)
        : low(value), high((value >= 0) ? 0 : (uint64_t)-1LL) {}
    ...
    btScalar toScalar() const {
      return ((int64_t)high >= 0)
                 ? btScalar(high) *
                           (btScalar(0x100000000LL) * btScalar(0x100000000LL)) +
                       btScalar(low)
                 : -(-*this).toScalar();
    }
   ...
   public:
    ...
    btScalar toScalar() const {
      return sign * ((m_denominator == 0)
                         ? SIMD_INFINITY
                         : (btScalar)m_numerator / m_denominator);
    }
  };
  ...
  class PointR128 {
   public:
    ...
    btScalar xvalue() const { return x.toScalar() / denominator.toScalar(); }

    btScalar yvalue() const { return y.toScalar() / denominator.toScalar(); }

    btScalar zvalue() const { return z.toScalar() / denominator.toScalar(); }
  };
...
#ifdef DEBUG_CONVEX_HULL
    void print() {
      printf("E%p : %d -> %d,  n=%p p=%p   (0 %d\t%d\t%d) -> (%d %d %d)", this,
             reverse->target->point.index, target->point.index, next, prev,
             reverse->target->point.x, reverse->target->point.y,
  ...
  class DMul {
   private:
    static uint32_t high(uint64_t value) { return (uint32_t)(value >> 32); }

    static uint32_t low(uint64_t value) { return (uint32_t)value; }

    static uint64_t mul(uint32_t a, uint32_t b) {
      return (uint64_t)a * (uint64_t)b;
    }

    static void shlHalf(uint64_t& value) { value <<= 32; }

    static uint64_t high(Int128 value) { return value.high; }

    static uint64_t low(Int128 value) { return value.low; }

    static Int128 mul(uint64_t a, uint64_t b) { return Int128::mul(a, b); }
 ...
 private:
    ...
    T* newObject() {
      ...
      if (!o) {
        ...
        if (p) {
          ...
          p = new (btAlignedAlloc(sizeof(PoolArray<T>), 16))
  ...
  bool shiftFace(Face* face, btScalar amount,
...
btConvexHullInternal::Int128 btConvexHullInternal::Int128::operator*(
    int64_t b) const {
...
btConvexHullInternal::Int128 btConvexHullInternal::Int128::mul(int64_t a,
  ...
  DMul<uint64_t, uint32_t>::mul((uint64_t)a, (uint64_t)b, result.low,
                                result.high);
  return negative ? -result : result;
#endif
}
...
//(numerator * b.denominator < b.numerator * denominator) ? -sign : 0;
  ...
  __asm__(
      ...
      : "a"(m_denominator), [bn] "g"(b.m_numerator), [tn] "g"(m_numerator),
        [bd] "g"(b.m_denominator)
      : "%rdx", "cc");
...
void btConvexHullInternal::computeInternal(int start, int end,
...
btConvexHullInternal::Orientation btConvexHullInternal::getOrientation(
...
btConvexHullInternal::Edge* btConvexHullInternal::findMaxAngle(
...
#ifdef DEBUG_CONVEX_HULL
        printf("      Angle is %f (%d) for ", (float)btAtan(cot.toScalar()),
               (int)cot.isNaN());
        e->print();
#endif
        if (cot.isNaN()) {
...
void btConvexHullInternal::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1,
                                                    Edge*& e0, Edge*& e1,
                                                    Vertex* stop0,
                                                    Vertex* stop1) {
  Edge* start0 = e0;
...
class pointCmp {
 public:
  bool operator()(const btConvexHullInternal::Point32& p,
                  const btConvexHullInternal::Point32& q) const {
    return (p.y < q.y) ||
           ((p.y == q.y) && ((p.x < q.x) || ((p.x == q.x) && (p.z < q.z))));
  }
};

void btConvexHullInternal::compute(const void* coords, bool doubleCoords,
                                   int stride, int count) {
  btVector3 min(btScalar(1e30), btScalar(1e30), btScalar(1e30)),
      max(btScalar(-1e30), btScalar(-1e30), btScalar(-1e30));
...
bool btConvexHullInternal::shiftFace(Face* face, btScalar amount,
                                     btAlignedObjectArray<Vertex*> stack) {
  ...
  Point32 shift((int32_t)origShift[medAxis], (int32_t)origShift[maxAxis],
...
btScalar btConvexHullComputer::compute(const void* coords, bool doubleCoords,
                                       int stride, int count, btScalar shrink,
