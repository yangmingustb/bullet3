...
btVector3 PlaneLineIntersection(const btPlane &plane, const btVector3 &p0,
                                const btVector3 &p1);
btVector3 PlaneProject(const btPlane &plane, const btVector3 &point);

btVector3 ThreePlaneIntersection(const btPlane &p0, const btPlane &p1,
                                 const btPlane &p2);
btVector3 ThreePlaneIntersection(const btPlane &p0, const btPlane &p1,
                                 const btPlane &p2) {
  ...
  btVector3 result(potentialVertex.getX(), potentialVertex.getY(),
                   potentialVertex.getZ());
  return result;
}

btScalar DistanceBetweenLines(const btVector3 &ustart, const btVector3 &udir,
                              const btVector3 &vstart, const btVector3 &vdir,
                              btVector3 *upoint = NULL,
                              btVector3 *vpoint = NULL);
btVector3 TriNormal(const btVector3 &v0, const btVector3 &v1,
                    const btVector3 &v2);
...
btScalar DistanceBetweenLines(const btVector3 &ustart, const btVector3 &udir,
                              const btVector3 &vstart, const btVector3 &vdir,
                              btVector3 *upoint, btVector3 *vpoint) {
  btVector3 cp;
...
template <class T>
int maxdirfiltered(const T *p, int count, const T &dir,
                   btAlignedObjectArray<int> &allow) {
  btAssert(count);
  int m = -1;
  for (int i = 0; i < count; i++)
    if (allow[i]) {
      if (m == -1 || btDot(p[i], dir) > btDot(p[m], dir)) m = i;
    }
  btAssert(m != -1);
  return m;
}
...
template <class T>
int maxdirsterid(const T *p, int count, const T &dir,
                 btAlignedObjectArray<int> &allow) {
  int m = -1;
  while (m == -1) {
    ...
    T v = btCross(u, dir);
    int ma = -1;
    for (btScalar x = btScalar(0.0); x <= btScalar(360.0);
         x += btScalar(45.0)) {
      btScalar s = btSin(SIMD_RADS_PER_DEG * (x));
      btScalar c = btCos(SIMD_RADS_PER_DEG * (x));
      int mb = maxdirfiltered(p, count, dir + (u * s + v * c) * btScalar(0.025),
                              allow);
      if (ma == m && mb == m) {
        allow[m] = 3;
        return m;
      ...
      if (ma != -1 && ma != mb)  // Yuck - this is really ugly
      {
        ...
        for (btScalar xx = x - btScalar(40.0); xx <= x; xx += btScalar(5.0)) {
          ...
          int md = maxdirfiltered(
              p, count, dir + (u * s + v * c) * btScalar(0.025), allow);
          if (mc == m && md == m) {
            allow[m] = 3;
            return m;
          }
...
int above(btVector3 *vertices, const int3 &t, const btVector3 &p,
          btScalar epsilon);
int above(btVector3 *vertices, const int3 &t, const btVector3 &p,
          btScalar epsilon) {
...
int4 HullLibrary::FindSimplex(btVector3 *verts, int verts_count,
                              btAlignedObjectArray<int> &allow) {
  btVector3 basis[3];
  ...
  if (p0 == p1 || basis[0] == btVector3(0, 0, 0)) return int4(-1, -1, -1, -1);
  basis[1] =
      btCross(btVector3(btScalar(1), btScalar(0.02), btScalar(0)), basis[0]);
  basis[2] =
      btCross(btVector3(btScalar(-0.02), btScalar(1), btScalar(0)), basis[0]);
...
int HullLibrary::calchullgen(btVector3 *verts, int verts_count, int vlimit) {
  ...
  btVector3 center = (verts[p[0]] + verts[p[1]] + verts[p[2]] + verts[p[3]]) /
                     btScalar(4.0);  // a valid interior point
  ...
  while (vlimit > 0 && ((te = extrudable(epsilon)) != 0)) {
    ...
    while (j--) {
      ...
      int3 nt = *m_tris[j];
      if (above(verts, nt, center, btScalar(0.01) * epsilon) ||
          btCross(verts[nt[1]] - verts[nt[0]], verts[nt[2]] - verts[nt[1]])
                  .length() < epsilon * epsilon * btScalar(0.1)) {
...
int HullLibrary::calchull(btVector3 *verts, int verts_count,
                          TUIntArray &tris_out, int &tris_count, int vlimit) {
  ...
  for (i = 0; i < m_tris.size(); i++) {
    if (m_tris[i]) {
      for (int j = 0; j < 3; j++) ts.push_back((*m_tris[i])[j]);
...
bool HullLibrary::ComputeHull(unsigned int vcount, const btVector3 *vertices,
                              PHullResult &result, unsigned int vlimit) {
  int tris_count;
  int ret = calchull((btVector3 *)vertices, (int)vcount, result.m_Indices,
                     tris_count, static_cast<int>(vlimit));
  if (!ret) return false;
    ...
    HullResult &result)    // contains the resulst
{
  ...
  bool ok =
      CleanupVertices(desc.mVcount, desc.mVertices, desc.mVertexStride, ovcount,
                      &vertexSource[0], desc.mNormalEpsilon,
                      scale);  // normalize point cloud, remove duplicates!
  ...
  if (ok) {
    ...
    if (ok) {
      ...
      BringOutYourDead(hr.mVertices, hr.mVcount, &vertexScratch[0], ovcount,
                       &hr.m_Indices[0], hr.mIndexCount);
              ...
              QF_TRIANGLES))  // if he wants the results as triangle!
      {
        ...
        memcpy(&result.m_OutputVertices[0], &vertexScratch[0],
        ...
        if (desc.HasHullFlag(QF_REVERSE_ORDER)) {
          ...
          memcpy(&result.m_Indices[0], &hr.m_Indices[0],
...
static void addPoint(unsigned int &vcount, btVector3 *p, btScalar x, btScalar y,
                     btScalar z) {
  // XXX, might be broken
...
bool HullLibrary::CleanupVertices(
    unsigned int svcount, const btVector3 *svertices, unsigned int stride,
    unsigned int &vcount,  // output number of vertices
    btVector3 *vertices,   // location to store the results.
    btScalar normalepsilon, btVector3 &scale) {
...
#define EPSILON                                                              \
  btScalar(                                                                  \
      0.000001) /* close enough to consider two btScalaring point numbers to \
                   be 'the same'. */
...
void HullLibrary::BringOutYourDead(const btVector3 *verts, unsigned int vcount,
                                   btVector3 *overts, unsigned int &ocount,
  ...
  for (i = 0; i < int(indexcount); i++) {
    ...
    if (usedIndices[static_cast<int>(v)])  // if already remapped
    {
      ...
      for (int k = 0; k < m_vertexIndexMapping.size(); k++) {
        if (tmpIndices[k] == int(v)) m_vertexIndexMapping[k] = ocount;
      }
