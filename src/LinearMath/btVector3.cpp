...
long _maxdot_large(const float *vv, const float *vec, unsigned long count,
  ...
  float4 dotMax =
      btAssign128(-BT_INFINITY, -BT_INFINITY, -BT_INFINITY, -BT_INFINITY);
  float4 vvec = _mm_loadu_ps(vec);
  float4 vHi =
      btCastiTo128f(_mm_shuffle_epi32(btCastfTo128i(vvec), 0xaa));  /// zzzz
...
// memset( stack_array, -1, STACK_ARRAY_COUNT * sizeof(stack_array[0]) );
  ...
  for (segment = 0; segment + STACK_ARRAY_COUNT * 4 <= count;
       segment += STACK_ARRAY_COUNT * 4) {
    max = dotMax;

    for (index = 0; index < STACK_ARRAY_COUNT;
         index += 4) {  // do four dot products at a time. Carefully avoid
                        // touching the w element.
      ...
      x = x + z;
      stack_array[index] = x;
      max = _mm_max_ps(x, max);  // control the order here so that max is never
                                 // NaN even if x is nan
      ...
      // It is too costly to keep the index of the max here. We will look for it
      // again later.  We save a lot of work this way.
    }
    ...
    if (0xf != _mm_movemask_ps((float4)_mm_cmpeq_ps(max, dotMax))) {
      ...
      size_t test;
      for (index = 0;
           0 == (test = _mm_movemask_ps(_mm_cmpeq_ps(stack_array[index], max)));
           index++)  // local_count must be a multiple of 4
      {
      }
      // record where it is.
  ...
  if (localCount) {
    ...
    asm volatile(
        ".align 4                                                                   \n\
         ...
         jnz     0b                                          \n\
     "
        : [max] "+x"(max), [t0] "=&x"(t0), [t1] "=&x"(t1), [t2] "=&x"(t2),
          [t3] "=&x"(t3), [t4] "=&x"(t4), [byteIndex] "+r"(byteIndex)
        :
...
#else
    {
      for (unsigned int i = 0; i < localCount / 4;
           i++, index++) {  // do four dot products at a time. Carefully avoid
                            // touching the w element.
        float4 v0 = vertices[0];
        float4 v1 = vertices[1];
        float4 v2 = vertices[2];
        ...
        x = x + z;
        stack_array[index] = x;
        max = _mm_max_ps(x, max);  // control the order here so that max is
                                   // never NaN even if x is nan
      }
    }
#endif  //__APPLE__
  }
  ...
  // if we found a new max.
  if (0 == segment ||
      0xf != _mm_movemask_ps((float4)_mm_cmpeq_ps(
                 max, dotMax))) {  // we found a new max. Search for it
    // find max across the max vector, place in all elements of max -- big
    // latency hit here
    max = _mm_max_ps(max, (float4)_mm_shuffle_ps(max, max, 0x4e));
    max = _mm_max_ps(max, (float4)_mm_shuffle_ps(max, max, 0xb1));

    // It is slightly faster to do this part in scalar code when count < 8.
    // However, the common case for
    // this where it actually makes a difference is handled in the early out at
    // the top of the function,
    // so it is less than a 1% difference here. I opted for improved code size,
    // fewer branches and reduced
...
long _mindot_large(const float *vv, const float *vec, unsigned long count,
                   float *dotResult);
...
// memset( stack_array, -1, STACK_ARRAY_COUNT * sizeof(stack_array[0]) );
  ...
  for (segment = 0; segment + STACK_ARRAY_COUNT * 4 <= count;
    ...
    for (index = 0; index < STACK_ARRAY_COUNT;
      ...
      x = x + z;
      stack_array[index] = x;
      min = _mm_min_ps(x, min);  // control the order here so that min is never
                                 // NaN even if x is nan
      ...
      // It is too costly to keep the index of the min here. We will look for it
      // again later.  We save a lot of work this way.
    }
    ...
    if (0xf != _mm_movemask_ps((float4)_mm_cmpeq_ps(min, dotmin))) {
      ...
      size_t test;
      for (index = 0;
           0 == (test = _mm_movemask_ps(_mm_cmpeq_ps(stack_array[index], min)));
           index++)  // local_count must be a multiple of 4
      {
      }
      // record where it is.
  ...
  if (localCount) {
    ...
    asm volatile(
        ".align 4                                                                   \n\
             ...
             jnz     0b                                          \n\
             "
        : [min] "+x"(min), [t0] "=&x"(t0), [t1] "=&x"(t1), [t2] "=&x"(t2),
          [t3] "=&x"(t3), [t4] "=&x"(t4), [byteIndex] "+r"(byteIndex)
        :
        [vLo] "x"(vLo), [vHi] "x"(vHi), [vertices] "r"(vertices), [sap] "r"(sap)
        : "memory", "cc");
...
#else
    {
      for (unsigned int i = 0; i < localCount / 4;
        ...
        x = x + z;
        stack_array[index] = x;
        min = _mm_min_ps(x, min);  // control the order here so that max is
                                   // never NaN even if x is nan
      }
    }
  ...
  if (0 == segment ||
      0xf != _mm_movemask_ps((float4)_mm_cmpeq_ps(
                 min, dotmin))) {  // we found a new min. Search for it
    // find min across the min vector, place in all elements of min -- big
    // latency hit here
    min = _mm_min_ps(min, (float4)_mm_shuffle_ps(min, min, 0x4e));
    min = _mm_min_ps(min, (float4)_mm_shuffle_ps(min, min, 0xb1));
...
static long _maxdot_large_v0(const float *vv, const float *vec,
                             unsigned long count, float *dotResult);
static long _maxdot_large_v1(const float *vv, const float *vec,
                             unsigned long count, float *dotResult);
static long _maxdot_large_sel(const float *vv, const float *vec,
                              unsigned long count, float *dotResult);
static long _mindot_large_v0(const float *vv, const float *vec,
                             unsigned long count, float *dotResult);
static long _mindot_large_v1(const float *vv, const float *vec,
                             unsigned long count, float *dotResult);
static long _mindot_large_sel(const float *vv, const float *vec,
                              unsigned long count, float *dotResult);
...
static inline uint32_t btGetCpuCapabilities(void) {
  ...
  if (0 == testedCapabilities) {
    ...
    int err = sysctlbyname("hw.optional.neon_hpfp", &hasFeature, &featureSize,
                           NULL, 0);

    if (0 == err && hasFeature) capabilities |= 0x2000;
...
#if defined __arm__
#define vld1q_f32_aligned_postincrement(_ptr)                              \
  ({                                                                       \
    float32x4_t _r;                                                        \
    asm("vld1.f32 {%0}, [%1, :128]!\n" : "=w"(_r), "+r"(_ptr)); /*return*/ \
    _r;                                                                    \
  })
#else
// support 64bit arm
#define vld1q_f32_aligned_postincrement(_ptr)                        \
  ({                                                                 \
    float32x4_t _r = ((float32x4_t *)(_ptr))[0];                     \
    (_ptr) = (const float *)((const char *)(_ptr) + 16L); /*return*/ \
    _r;                                                              \
  })
#endif

long _maxdot_large_v0(const float *vv, const float *vec, unsigned long count,
                      float *dotResult) {
  unsigned long i = 0;
  ...
  uint32x2_t iLo =
      (uint32x2_t){static_cast<uint32_t>(-1), static_cast<uint32_t>(-1)};
  uint32x2_t iHi =
      (uint32x2_t){static_cast<uint32_t>(-1), static_cast<uint32_t>(-1)};
...
long _maxdot_large_v1(const float *vv, const float *vec, unsigned long count,
                      float *dotResult) {
  float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
  ...
  uint32x4_t index =
      (uint32x4_t){static_cast<uint32_t>(-1), static_cast<uint32_t>(-1),
                   static_cast<uint32_t>(-1), static_cast<uint32_t>(-1)};
  float32x4_t maxDot =
      (float32x4_t){-BT_INFINITY, -BT_INFINITY, -BT_INFINITY, -BT_INFINITY};
  ...
  float32x2_t maxDot2 =
      vbsl_f32(mask, vget_high_f32(maxDot), vget_low_f32(maxDot));
...
long _mindot_large_v0(const float *vv, const float *vec, unsigned long count,
                      float *dotResult) {
  unsigned long i = 0;
  float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
  float32x2_t vLo = vget_low_f32(vvec);
...
long _mindot_large_v1(const float *vv, const float *vec, unsigned long count,
                      float *dotResult) {
  float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
  float32x4_t vLo = vcombine_f32(vget_low_f32(vvec), vget_low_f32(vvec));
  float32x4_t vHi = vdupq_lane_f32(vget_high_f32(vvec), 0);
  ...
  float32x2_t minDot2 =
      vbsl_f32(mask, vget_high_f32(minDot), vget_low_f32(minDot));
  uint32x2_t index2 = vbsl_u32(mask, vget_high_u32(index), vget_low_u32(index));
