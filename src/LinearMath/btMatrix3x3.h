...
/**@brief The btMatrix3x3 class implements a 3x3 rotation matrix, to perform
* linear algebra in combination with btQuaternion, btTransform and btVector3.
...
btMatrix3x3 {
  ...
  */
  ...
  SIMD_FORCE_INLINE btMatrix3x3(const btSimdFloat4 v0, const btSimdFloat4 v1,
                                const btSimdFloat4 v2) {
    m_el[0].mVec128 = v0;
    m_el[1].mVec128 = v1;
    m_el[2].mVec128 = v2;
  }

  SIMD_FORCE_INLINE btMatrix3x3(const btVector3& v0, const btVector3& v1,
                                const btVector3& v2) {
    m_el[0] = v0;
    m_el[1] = v1;
    m_el[2] = v2;
  ...
  *  @param q The Quaternion to match */
  void setRotation(const btQuaternion& q) {
    ...
    V1 = btCastiTo128f(
        _mm_shuffle_epi32(Qi, BT_SHUFFLE(1, 0, 2, 3)));  // Y X Z W
    V2 = _mm_shuffle_ps(NQ, Q, BT_SHUFFLE(0, 0, 1, 3));  // -X -X  Y  W
    V3 = btCastiTo128f(
        _mm_shuffle_epi32(Qi, BT_SHUFFLE(2, 1, 0, 3)));  // Z Y X W
    V1 = _mm_xor_ps(V1, vMPPP);  //	change the sign of the first element

    V11 = btCastiTo128f(
        _mm_shuffle_epi32(Qi, BT_SHUFFLE(1, 1, 0, 3)));   // Y Y X W
    ...
    Y = btCastiTo128f(
        _mm_shuffle_epi32(NQi, BT_SHUFFLE(3, 2, 0, 3)));  // -W -Z -X -W
    Z = btCastiTo128f(
        _mm_shuffle_epi32(Qi, BT_SHUFFLE(1, 0, 1, 3)));  //  Y  X  Y  W
    ...
    setValue(btScalar(1.0) - (yy + zz), xy - wz, xz + wy, xy + wz,
             btScalar(1.0) - (xx + zz), yz - wx, xz - wy, yz + wx,
             btScalar(1.0) - (xx + yy));
#endif
  }
  ...
  */
  void setEulerYPR(const btScalar& yaw, const btScalar& pitch,
                   const btScalar& roll) {
  ...
  **/
  ...
  void setIdentity() {
    ...
    m_el[2] = v0010;
#else
    setValue(btScalar(1.0), btScalar(0.0), btScalar(0.0), btScalar(0.0),
             btScalar(1.0), btScalar(0.0), btScalar(0.0), btScalar(0.0),
             btScalar(1.0));
#endif
  }
...
#if (defined(BT_USE_SSE_IN_API) && defined(BT_USE_SSE)) || defined(BT_USE_NEON)
    static const btMatrix3x3 identityMatrix(v1000, v0100, v0010);
#else
    static const btMatrix3x3 identityMatrix(
        btScalar(1.0), btScalar(0.0), btScalar(0.0), btScalar(0.0),
        btScalar(1.0), btScalar(0.0), btScalar(0.0), btScalar(0.0),
        btScalar(1.0));
#endif
  ...
  * @param m The array to be filled */
  void getOpenGLSubMatrix(btScalar * m) const {
    ...
    v2 = btCastdTo128f(
        _mm_move_sd(btCastfTo128d(v2), btCastfTo128d(vT)));  // z0 z1 z2 0
  ...
  * are possible values*/
  void getEulerZYX(btScalar & yaw, btScalar & pitch, btScalar & roll,
  ...
  * @param s Scaling vector The elements of the vector will scale each column */
  ...
  /// extractRotation is from "A robust method to extract the rotational part of
  /// deformations"
  ...
  SIMD_FORCE_INLINE void extractRotation(
      btQuaternion & q, btScalar tolerance = 1.0e-9, int maxIter = 100) {
    int iter = 0;
    btScalar w;
    ...
    for (iter = 0; iter < maxIter; iter++) {
      btMatrix3x3 R(q);
      btVector3 omega =
          (R.getColumn(0).cross(A.getColumn(0)) +
           R.getColumn(1).cross(A.getColumn(1)) +
           R.getColumn(2).cross(A.getColumn(2))) *
          (btScalar(1.0) / btFabs(R.getColumn(0).dot(A.getColumn(0)) +
                                  R.getColumn(1).dot(A.getColumn(1)) +
                                  R.getColumn(2).dot(A.getColumn(2))) +
           tolerance);
      w = omega.norm();
      if (w < tolerance) break;
      q = btQuaternion(btVector3((btScalar(1.0) / w) * omega), w) * q;
      q.normalize();
    }
  }
  ...
  */
  void diagonalize(btMatrix3x3 & rot, btScalar threshold, int maxSteps) {
    ...
    for (int step = maxSteps; step > 0; step--) {
      ...
      btScalar t = threshold * (btFabs(m_el[0][0]) + btFabs(m_el[1][1]) +
                                btFabs(m_el[2][2]));
      if (max <= t) {
...
SIMD_FORCE_INLINE btMatrix3x3 operator*(const btMatrix3x3& m,
  ...
  return btMatrix3x3(_mm_mul_ps(m[0].mVec128, vk), _mm_mul_ps(m[1].mVec128, vk),
                     _mm_mul_ps(m[2].mVec128, vk));
#elif defined(BT_USE_NEON)
  return btMatrix3x3(vmulq_n_f32(m[0].mVec128, k), vmulq_n_f32(m[1].mVec128, k),
                     vmulq_n_f32(m[2].mVec128, k));
#else
  return btMatrix3x3(m[0].x() * k, m[0].y() * k, m[0].z() * k, m[1].x() * k,
                     m[1].y() * k, m[1].z() * k, m[2].x() * k, m[2].y() * k,
                     m[2].z() * k);
#endif
}
...
#if (defined(BT_USE_SSE_IN_API) && defined(BT_USE_SSE)) || defined(BT_USE_NEON)
  return btMatrix3x3(m1[0].mVec128 + m2[0].mVec128,
...
SIMD_FORCE_INLINE btMatrix3x3 btMatrix3x3::absolute() const {
  ...
  return btMatrix3x3(_mm_and_ps(m_el[0].mVec128, btvAbsfMask),
...
btMatrix3x3::timesTranspose(const btMatrix3x3& m) const {
  ...
  btMatrix3x3 mT = m.transpose();  // we rely on transpose() zeroing w channel
...
#else
  return btMatrix3x3(m_el[0].dot(m[0]), m_el[0].dot(m[1]), m_el[0].dot(m[2]),
...
#else
  return btMatrix3x3(m2.tdotx(m1[0]), m2.tdoty(m1[0]), m2.tdotz(m1[0]),
...
/*
SIMD_FORCE_INLINE btMatrix3x3 btMultTransposeLeft(const btMatrix3x3& m1, const
btMatrix3x3& m2) {
return btMatrix3x3(
...
SIMD_FORCE_INLINE void btMatrix3x3::serialize(
    struct btMatrix3x3Data& dataOut) const {
  for (int i = 0; i < 3; i++) m_el[i].serialize(dataOut.m_el[i]);
}

SIMD_FORCE_INLINE void btMatrix3x3::serializeFloat(
    struct btMatrix3x3FloatData& dataOut) const {
  for (int i = 0; i < 3; i++) m_el[i].serializeFloat(dataOut.m_el[i]);
}

SIMD_FORCE_INLINE void btMatrix3x3::deSerialize(
    const struct btMatrix3x3Data& dataIn) {
  for (int i = 0; i < 3; i++) m_el[i].deSerialize(dataIn.m_el[i]);
}

SIMD_FORCE_INLINE void btMatrix3x3::deSerializeFloat(
    const struct btMatrix3x3FloatData& dataIn) {
  for (int i = 0; i < 3; i++) m_el[i].deSerializeFloat(dataIn.m_el[i]);
}

SIMD_FORCE_INLINE void btMatrix3x3::deSerializeDouble(
    const struct btMatrix3x3DoubleData& dataIn) {
