...
#define _XOPEN_SOURCE \
  600  // for definition of pthread_barrier_t, see
...
int btGetNumHardwareThreads() {
  return btMax(1u,
               btMin(BT_MAX_THREAD_COUNT, std::thread::hardware_concurrency()));
}
...
int btGetNumHardwareThreads() {
  return btMax(1,
               btMin<int>(BT_MAX_THREAD_COUNT, sysconf(_SC_NPROCESSORS_ONLN)));
}

#endif

// btThreadSupportPosix helps to initialize/shutdown libspe2, start/stop SPU
...
class btThreadSupportPosix : public btThreadSupportInterface {
 ...
 private:
  ...
  // m_mainSemaphoresemaphore will signal, if and how many threads are finished
  // with their work
  sem_t* m_mainSemaphore;
  int m_numThreads;
...
static void* threadFunction(void* argument) {
  btThreadSupportPosix::btThreadStatus* status =
      (btThreadSupportPosix::btThreadStatus*)argument;
...
void btThreadSupportPosix::startThreads(
    const ConstructionInfo& threadConstructionInfo) {
  ...
  for (int i = 0; i < m_numThreads; i++) {
    ...
    checkPThreadFunction(pthread_create(&threadStatus.thread, NULL,
                                        &threadFunction, (void*)&threadStatus));
  }
}
...
class btCriticalSectionPosix : public btCriticalSection {
 ...
 public:
  btCriticalSectionPosix() { pthread_mutex_init(&m_mutex, NULL); }
  virtual ~btCriticalSectionPosix() { pthread_mutex_destroy(&m_mutex); }
