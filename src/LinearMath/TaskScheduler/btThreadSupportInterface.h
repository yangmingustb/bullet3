...
class btThreadSupportInterface {
 public:
  ...
  virtual int getNumWorkerThreads() const = 0;  // number of worker threads
                                                // (total number of logical
                                                // processors - 1)
  virtual int getCacheFriendlyNumThreads()
      const = 0;  // the number of logical processors sharing a single L3 cache
  virtual int getLogicalToPhysicalCoreRatio() const = 0;  // the number of
                                                          // logical processors
                                                          // per physical
                                                          // processor (usually
                                                          // 1 or 2)
  ...
  struct ConstructionInfo {
    ConstructionInfo(const char* uniqueName, ThreadFunc userThreadFunc,
