    ...
    switch (info->Relationship) {
      ...
      case RelationCache:
        if (info->Cache.Level == 1) {
          ...
          procInfo->numL3Cache++;
          // processors that share L3 cache are considered to be on the same
          // team
          // because they can more easily work together on the same data.
          // Large performance penalties will occur if 2 or more threads from
          // different
          // teams attempt to frequently read and modify the same cache lines.
          //
          // On the AMD Ryzen 7 CPU for example, the 8 cores on the CPU are
          // split into
          // 2 CCX units of 4 cores each. Each CCX has a separate L3 cache, so
          // if both
          // CCXs are operating on the same data, many cycles will be spent
          // keeping the
          // two caches coherent.
          if (procInfo->numTeamMasks < btProcessorInfo::maxNumTeamMasks) {
            procInfo->processorTeamMasks[procInfo->numTeamMasks] =
                info->ProcessorMask;
...
/// btThreadSupportWin32 helps to initialize/shutdown libspe2, start/stop SPU
...
btThreadSupportWin32::~btThreadSupportWin32() { stopThreads(); }

DWORD WINAPI win32threadStartFunc(LPVOID lpParam) {
  btThreadSupportWin32::btThreadStatus* status =
      (btThreadSupportWin32::btThreadStatus*)lpParam;
...
void btThreadSupportWin32::startThreads(
    const ConstructionInfo& threadConstructionInfo) {
  ...
  if (!GetProcessAffinityMask(GetCurrentProcess(), &dwProcessAffinityMask,
                              &dwSystemAffinityMask)) {
    dwProcessAffinityMask = 0;
  }
  /// The number of threads should be equal to the number of available cores - 1
  m_numThreads =
      btMin(procInfo.numLogicalProcessors, int(BT_MAX_THREAD_COUNT)) -
      1;  // cap to max thread count (-1 because main thread already exists)
  ...
  if (DWORD_PTR mask =
          dwProcessAffinityMask & getProcessorTeamMask(procInfo, 0)) {
  ...
  for (int i = 0; i < m_numThreads; i++) {
    ...
    sprintf(threadStatus.m_eventCompleteHandleName, "ec%.8s%d%d",
            threadConstructionInfo.m_uniqueName, uniqueId, i);
    threadStatus.m_eventCompleteHandle =
        CreateEventA(0, false, false, threadStatus.m_eventCompleteHandleName);
    ...
    // highest priority -- can cause erratic performance when numThreads >
    // numCores
    //                     we don't want worker threads to be higher priority
    //                     than the main thread or the main thread could get
    //                     totally shut out and unable to tell the workers to
    //                     stop
...

    {
      ...
      if (teamMask) {
        ...
        //  - for single-socket Intel x86 CPUs this has no effect (only a
        //  single, shared L3 cache so there is only 1 team)
        //  - for multi-socket Intel this will keep threads from migrating from
        //  one socket to another
        //  - for AMD Ryzen this will keep threads from migrating from one CCX
        //  to another
...
class btWin32CriticalSection : public btCriticalSection {
 ...
 public:
  btWin32CriticalSection() { InitializeCriticalSection(&mCriticalSection); }
...
btCriticalSection* btThreadSupportWin32::createCriticalSection() {
  unsigned char* mem =
      (unsigned char*)btAlignedAlloc(sizeof(btWin32CriticalSection), 16);
  btWin32CriticalSection* cs = new (mem) btWin32CriticalSection();
  return cs;
}

void btThreadSupportWin32::deleteCriticalSection(
    btCriticalSection* criticalSection) {
  criticalSection->~btCriticalSection();
  btAlignedFree(criticalSection);
}

btThreadSupportInterface* btThreadSupportInterface::create(
