...
class ParallelSumJob : public IJob {
 ...
 public:
  ParallelSumJob(int iBegin, int iEnd, const btIParallelSumBody& body,
  ...
  virtual void executeJob(int threadId) BT_OVERRIDE {
    ...
    // by truncating bits of the result, we can make the parallelSum
    // deterministic (at the expense of precision)
...
JobQueue {
 ...
 public:
  ...
  void init(btThreadSupportInterface * threadSup,
            btAlignedObjectArray<JobQueue> * contextArray) {
  ...
  void setupJobStealing(btAlignedObjectArray<JobQueue> * contextArray,
                        int numActiveContexts) {
    ...
    for (int i = 0; i < numOffsets && m_neighborContexts.size() < numNeighbors;
         i++) {
  ...
  void submitJob(IJob * job) {
    btAssert(reinterpret_cast<char*>(job) >= &m_jobMem[0] &&
             reinterpret_cast<char*>(job) < &m_jobMem[0] + m_allocSize);
...
static void WorkerThreadFunc(void* userPtr) {
  ...
  while (!shouldSleep) {
    ...
    while (jobQueue->isQueueEmpty()) {
      ...
      btSpinPause();
      if (localStorage->m_directive->getDirective(threadId) ==
          WorkerThreadDirectives::kGoToSleep) {
        shouldSleep = true;
        break;
      }
      // if jobs are incoming,
      if (localStorage->m_directive->getDirective(threadId) ==
          WorkerThreadDirectives::kScanForJobs) {
        clockStart =
            localStorage->m_clock->getTimeMicroseconds();  // reset clock
      } else {
        for (int i = 0; i < 50; ++i) {
          btSpinPause();
          btSpinPause();
          ...
          if (localStorage->m_directive->getDirective(threadId) ==
                  WorkerThreadDirectives::kScanForJobs ||
              !jobQueue->isQueueEmpty()) {
            break;
          }
        }
        // if no jobs incoming and queue has been empty for the cooldown time,
        // sleep
        btU64 timeElapsed =
 ...
 public:
  ...
  void init() {
    btThreadSupportInterface::ConstructionInfo constructionInfo(
        "TaskScheduler", WorkerThreadFunc);
    m_threadSupport = btThreadSupportInterface::create(constructionInfo);
    m_workerDirective = static_cast<WorkerThreadDirectives*>(
        btAlignedAlloc(sizeof(*m_workerDirective), 64));
    ...
    // ideal to have one job queue for each physical processor (except for the
    // main thread which needs no queue)
    int numThreadsPerQueue = m_threadSupport->getLogicalToPhysicalCoreRatio();
    int numJobQueues = (numThreadsPerQueue == 1)
                           ? (m_maxNumThreads - 1)
                           : (m_maxNumThreads / numThreadsPerQueue);
    ...
    for (int i = 0; i < m_numThreads; i++) {
      ...
      storage.m_cooldownTime = 100;  // 100 microseconds, threads go to sleep
                                     // after this long if they have nothing to
                                     // do
      storage.m_clock = &m_clock;
      storage.m_queue = m_perThreadJobQueues[i];
    }
    setWorkerDirectives(
        WorkerThreadDirectives::kGoToSleep);  // no work for them yet
    setNumThreads(m_threadSupport->getCacheFriendlyNumThreads());
  }

  void setWorkerDirectives(WorkerThreadDirectives::Type dir) {
    m_workerDirective->setDirectiveByRange(kFirstWorkerThreadId, m_numThreads,
                                           dir);
  }

  virtual int getMaxNumThreads() const BT_OVERRIDE { return m_maxNumThreads; }
  ...
  virtual void setNumThreads(int numThreads) BT_OVERRIDE {
    ...
    if (m_numWorkerThreads > 0) {
      // re-setup job stealing between queues to avoid attempting to steal from
      // an inactive job queue
    ...
    m_workerDirective->setDirectiveByRange(m_numThreads, BT_MAX_THREAD_COUNT,
                                           WorkerThreadDirectives::kGoToSleep);
  }
  ...
  void waitJobs() {
    ...
    while (true) {
      ...
      for (int iThread = kFirstWorkerThreadId; iThread < m_numThreads;
           ++iThread) {
  ...
  void wakeWorkers(int numWorkersToWake) {
    ...
    btAssert(m_workerDirective->getDirective(1) ==
             WorkerThreadDirectives::kScanForJobs);
    int numDesiredWorkers = btMin(numWorkersToWake, m_numWorkerThreads);
    int numActiveWorkers = 0;
    for (int iWorker = 0; iWorker < m_numWorkerThreads; ++iWorker) {
      // note this count of active workers is not necessarily totally reliable,
      // because a worker thread could be
      // just about to put itself to sleep. So we may on occasion fail to wake
      // up all the workers. It should be rare.
      ThreadLocalStorage& storage =
          m_threadLocalStorage[kFirstWorkerThreadId + iWorker];
      if (storage.m_status != WorkerThreadStatus::kSleeping) {
        numActiveWorkers++;
      }
    }
    for (int iWorker = 0;
         iWorker < m_numWorkerThreads && numActiveWorkers < numDesiredWorkers;
         ++iWorker) {
      ThreadLocalStorage& storage =
          m_threadLocalStorage[kFirstWorkerThreadId + iWorker];
      if (storage.m_status == WorkerThreadStatus::kSleeping) {
  ...
  virtual void sleepWorkerThreadsHint() BT_OVERRIDE {
    ...
    // hint the task scheduler that we may not be using these threads for a
    // little while
    setWorkerDirectives(WorkerThreadDirectives::kGoToSleep);
  }
  ...
  virtual void parallelFor(int iBegin, int iEnd, int grainSize,
    ...
    if (iterationCount > grainSize && m_numWorkerThreads > 0 &&
        m_antiNestingLock.tryLock()) {
      ...
      for (int i = iBegin; i < iEnd; i += grainSize) {
        ...
        JobType* job =
            new (jobMem) ParallelForJob(i, iE, body);  // placement new
        jq->submitJob(job);
        iJob++;
        iThread++;
      ...
      // put the main thread to work on emptying the job queue and then wait for
  ...
  virtual btScalar parallelSum(int iBegin, int iEnd, int grainSize,
    ...
    if (iterationCount > grainSize && m_numWorkerThreads > 0 &&
      ...
      for (int i = iBegin; i < iEnd; i += grainSize) {
        ...
        btAssert((jq - &m_jobQueues[0]) < m_numActiveJobQueues);
        void* jobMem = jq->allocJobMem(jobSize);
        JobType* job = new (jobMem) ParallelSumJob(
            i, iE, body, &m_threadLocalStorage[0]);  // placement new
        jq->submitJob(job);
        iJob++;
        iThread++;
        if (iThread >= m_numThreads) {
...
btITaskScheduler* btCreateDefaultTaskScheduler() { return NULL; }
