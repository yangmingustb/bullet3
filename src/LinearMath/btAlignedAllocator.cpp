...
static void *btAllocDefault(size_t size) { return malloc(size); }

static void btFreeDefault(void *ptr) { free(ptr); }
...
static void btAlignedFreeDefault(void *ptr) { _aligned_free(ptr); }
...
static inline void btAlignedFreeDefault(void *ptr) { free(ptr); }
#else
...
int btDumpMemoryLeaks() {
  ...
  if (totalLeak) {
    printf(
        "Error: memory leaks: %d allocations were not freed and leaked "
        "together %d bytes\n",
...
void *btAlignedAllocInternal(size_t size, int alignment, int line,
  ...
  printf(
      "allocation %d at address %x, from %s,line %d, size %d (total allocated "
      "= %d)\n",
...
void btAlignedFreeInternal(void *ptr, int line, char *filename) {
  ...
  if (ptr) {
    ...
    printf(
        "free %d at address %x, from %s,line %d, size %d (total remain = %d in "
        "%d non-freed allocations)\n",
