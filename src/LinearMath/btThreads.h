...
void btResetThreadIndexCounter();  // notify that all worker threads have been
                                   // destroyed

///
/// btSpinMutex -- lightweight spin-mutex implemented with atomic ops, never
/// puts
///               a thread to sleep because it is designed to be used with a
///               task scheduler
///               which has one thread per core and the threads don't sleep
///               until they
...
//
// btIParallelForBody -- subclass this to express work that can be done in
// parallel
//
...
//
// btIParallelSumBody -- subclass this to express work that can be done in
// parallel
...
//
// btITaskScheduler -- subclass this to implement a task scheduler that can
// dispatch work to
//                     worker threads
//
class btITaskScheduler {
 public:
  ...
  virtual btScalar parallelSum(int iBegin, int iEnd, int grainSize,
                               const btIParallelSumBody& body) = 0;
...
//                 (iterations may be done out of order, so no dependencies are
//                 allowed)
void btParallelFor(int iBegin, int iEnd, int grainSize,
                   const btIParallelForBody& body);

// btParallelSum -- call this to dispatch work like a for-loop, returns the sum
// of all iterations
//                 (iterations may be done out of order, so no dependencies are
//                 allowed)
