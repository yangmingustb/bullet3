...
struct b3ContactSolverInfoData {
  b3Scalar m_tau;
  b3Scalar m_damping;  // global non-contact constraint damping, can be locally
...
struct b3ContactSolverInfo : public b3ContactSolverInfoData {
  inline b3ContactSolverInfo() {
    ...
    // m_solverMode =  B3_SOLVER_USE_WARMSTARTING |  B3_SOLVER_SIMD |
    // B3_SOLVER_DISABLE_VELOCITY_DEPENDENT_FRICTION_DIRECTION|B3_SOLVER_USE_2_FRICTION_DIRECTIONS|B3_SOLVER_ENABLE_FRICTION_DIRECTION_CACHING;//
    // | B3_SOLVER_RANDMIZE_ORDER;
    m_solverMode = B3_SOLVER_USE_WARMSTARTING |
                   B3_SOLVER_SIMD;             // | B3_SOLVER_RANDMIZE_ORDER;
    m_restingContactRestitutionThreshold = 2;  // unused as of 2.81
    m_minimumSolverBatchSize = 128;  // try to combine islands until the amount
                                     // of constraints reaches this limit
    m_maxGyroscopicForce = 100.f;  /// only used to clamp forces for bodies that
                                   /// have their B3_ENABLE_GYROPSCOPIC_FORCE
                                   /// flag set (using b3RigidBody::setFlag)
