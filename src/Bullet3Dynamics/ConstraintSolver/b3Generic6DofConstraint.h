...
class b3RotationalLimitMotor {
 public:
  ...
  b3Scalar
      m_stopCFM;      //!< Constraint force mixing factor when joint is at limit
  ...
  */
  ...
  b3Scalar solveAngularLimits(b3Scalar timeStep, b3Vector3& axis,
                              b3Scalar jacDiagABInv, b3RigidBodyData* body0,
...
class b3TranslationalLimitMotor {
 public:
  ...
  b3Vector3
      m_currentLinearDiff;   //!  Current relative offset of constraint frames
...
*/
  ...
  b3Scalar solveLinearAxis(b3Scalar timeStep, b3Scalar jacDiagABInv,
                           b3RigidBodyData& body1, const b3Vector3& pointInA,
                           b3RigidBodyData& body2, const b3Vector3& pointInB,
                           int limit_index, const b3Vector3& axis_normal_on_a,
...
/// b3Generic6DofConstraint between two rigidbodies each with a pivotpoint that
/// descibes the axis location in local space
/*!
b3Generic6DofConstraint can leave any of the 6 degree of freedom 'free' or
'locked'.
currently this limit supports rotational motors<br>
<ul>
<li> For Linear limits, use b3Generic6DofConstraint.setLinearUpperLimit,
b3Generic6DofConstraint.setLinearLowerLimit. You can set the parameters with the
b3TranslationalLimitMotor structure accsesible through the
b3Generic6DofConstraint.getTranslationalLimitMotor method.
At this moment translational motors are not supported. May be in the future.
</li>

<li> For Angular limits, use the b3RotationalLimitMotor structure for
...
b3Generic6DofConstraint : public b3TypedConstraint {
 ...
 protected:
  ...
  b3Vector3 m_AnchorPos;  // point betwen pivots of bodies A and B to solve
                          // linear axes
  ...
  int setAngularLimits(b3ConstraintInfo2 * info, int row_offset,
  ...
  int setLinearLimits(b3ConstraintInfo2 * info, int row,
                      const b3Transform& transA, const b3Transform& transB,
                      const b3Vector3& linVelA, const b3Vector3& linVelB,
                      const b3Vector3& angVelA, const b3Vector3& angVelB);
 ...
 public:
  ...
  b3Generic6DofConstraint(int rbA, int rbB, const b3Transform& frameInA,
  ...
  calcs the agle differences between the bodies.
  \sa b3Generic6DofConstraint.getCalculatedTransformA ,
  b3Generic6DofConstraint.getCalculatedTransformB,
  b3Generic6DofConstraint.calculateAngleInfo
  */
  void calculateTransforms(const b3Transform& transA, const b3Transform& transB,
                           const b3RigidBodyData* bodies);
  ...
  /*!
\sa b3Generic6DofConstraint.getFrameOffsetA,
...
*/
  ...
  const b3Transform& getFrameOffsetA() const { return m_frameInA; }

  const b3Transform& getFrameOffsetB() const { return m_frameInB; }

  b3Transform& getFrameOffsetA() { return m_frameInA; }

  b3Transform& getFrameOffsetB() { return m_frameInB; }
  ...
  void getInfo2NonVirtual(b3ConstraintInfo2 * info, const b3Transform& transA,
  ...
  */
  ...
  void setFrames(const b3Transform& frameA, const b3Transform& frameB,
                 const b3RigidBodyData* bodies);
...
*/
  ...
  int get_limit_motor_info2(
      ...
      const b3Transform& transB, const b3Vector3& linVelA,
      const b3Vector3& linVelB, const b3Vector3& angVelA,
      const b3Vector3& angVelB, b3ConstraintInfo2* info, int row,
      b3Vector3& ax1, int rotational, int rotAllowed = false);
  ...
  void setAxis(const b3Vector3& axis1, const b3Vector3& axis2,
               const b3RigidBodyData* bodies);
};
