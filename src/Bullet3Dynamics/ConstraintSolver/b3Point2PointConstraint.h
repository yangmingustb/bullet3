...
enum b3Point2PointFlags { B3_P2P_FLAGS_ERP = 1, B3_P2P_FLAGS_CFM = 2 };

/// point to point constraint between two rigidbodies each with a pivotpoint
/// that descibes the 'ballsocket' location in local space
...
b3Point2PointConstraint : public b3TypedConstraint {
 ...
 public:
  ...
  b3Point2PointConstraint(int rbA, int rbB, const b3Vector3& pivotInA,
                          const b3Vector3& pivotInB);
  ...
  void getInfo2NonVirtual(b3ConstraintInfo2 * info,
                          const b3Transform& body0_trans,
                          const b3Transform& body1_trans);

  void updateRHS(b3Scalar timeStep);

  void setPivotA(const b3Vector3& pivotA) { m_pivotInA = pivotA; }

  void setPivotB(const b3Vector3& pivotB) { m_pivotInB = pivotB; }

  const b3Vector3& getPivotInA() const { return m_pivotInA; }

  const b3Vector3& getPivotInB() const { return m_pivotInB; }
...
/*
B3_FORCE_INLINE	int
b3Point2PointConstraint::calculateSerializeBufferSize() const
{
        ...
        ///fills the dataBuffer and returns the struct name (and 0 on failure)
B3_FORCE_INLINE	const char*
b3Point2PointConstraint::serialize(void* dataBuffer, b3Serializer* serializer)
const
{
        b3Point2PointConstraintData* p2pData =
(b3Point2PointConstraintData*)dataBuffer;
