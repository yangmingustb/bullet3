...
b3Point2PointConstraint::b3Point2PointConstraint(int rbA, int rbB,
                                                 const b3Vector3& pivotInA,
                                                 const b3Vector3& pivotInB)
    : b3TypedConstraint(B3_POINT2POINT_CONSTRAINT_TYPE, rbA, rbB),
      m_pivotInA(pivotInA),
      m_pivotInB(pivotInB),
      m_flags(0) {}

/*
b3Point2PointConstraint::b3Point2PointConstraint(int  rbA,const b3Vector3&
pivotInA)
...
void b3Point2PointConstraint::getInfo1NonVirtual(
    b3ConstraintInfo1* info, const b3RigidBodyData* bodies) {
  info->m_numConstraintRows = 3;
  info->nub = 3;
}

void b3Point2PointConstraint::getInfo2(b3ConstraintInfo2* info,
                                       const b3RigidBodyData* bodies) {
  b3Transform trA;
  trA.setIdentity();
...
void b3Point2PointConstraint::getInfo2NonVirtual(
    b3ConstraintInfo2* info, const b3Transform& body0_trans,
...

  {
    ...
    b3Vector3* angular2 =
        (b3Vector3*)(info->m_J1angularAxis + 2 * info->rowskip);
    b3Vector3 a1neg = -a1;
...

  {
    ...
    b3Vector3* angular2 =
        (b3Vector3*)(info->m_J2angularAxis + 2 * info->rowskip);
  ...
  for (j = 0; j < 3; j++) {
    info->m_constraintError[j * info->rowskip] =
        k * (a2[j] + body1_trans.getOrigin()[j] - a1[j] -
...
void b3Point2PointConstraint::updateRHS(b3Scalar timeStep) { (void)timeStep; }
