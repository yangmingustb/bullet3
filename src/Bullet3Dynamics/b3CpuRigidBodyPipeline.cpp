...
b3CpuRigidBodyPipeline::b3CpuRigidBodyPipeline(
    class b3CpuNarrowPhase* narrowphase,
...
b3CpuRigidBodyPipeline::~b3CpuRigidBodyPipeline() { delete m_data; }
  ...
  for (int i = 0; i < this->getNumBodies(); i++) {
    ...
    if (shapeIndex >= 0) {
      ...
      b3TransformAabb2(localAabb.m_minVec, localAabb.m_maxVec, margin, position,
                       orientation, &worldAabb.m_minVec, &worldAabb.m_maxVec);
...
void b3CpuRigidBodyPipeline::computeOverlappingPairs() {
  int numPairs =
      m_data->m_bp->getOverlappingPairCache()->getNumOverlappingPairs();
...
void b3CpuRigidBodyPipeline::computeContactPoints() {
  b3AlignedObjectArray<b3Int4>& pairs =
      m_data->m_bp->getOverlappingPairCache()->getOverlappingPairArray();

  m_data->m_np->computeContacts(pairs, m_data->m_aabbWorldSpace,
                                m_data->m_rigidBodies);
}
...
static inline float b3CalcRelVel(const b3Vector3& l0, const b3Vector3& l1,
                                 const b3Vector3& a0, const b3Vector3& a1,
                                 const b3Vector3& linVel0,
                                 const b3Vector3& angVel0,
                                 const b3Vector3& linVel1,
                                 const b3Vector3& angVel1) {
  return b3Dot(l0, linVel0) + b3Dot(a0, angVel0) + b3Dot(l1, linVel1) +
         b3Dot(a1, angVel1);
}

static inline void b3SetLinearAndAngular(const b3Vector3& n,
                                         const b3Vector3& r0,
                                         const b3Vector3& r1, b3Vector3& linear,
                                         b3Vector3& angular0,
                                         b3Vector3& angular1) {
  linear = -n;
...
static inline void b3SolveContact(b3ContactConstraint4& cs,
                                  const b3Vector3& posA, b3Vector3& linVelA,
                                  b3Vector3& angVelA, float invMassA,
  ...
  for (int ic = 0; ic < 4; ic++) {
...

    {
      ...
      b3SetLinearAndAngular((const b3Vector3&)-cs.m_linear,
                            (const b3Vector3&)r0, (const b3Vector3&)r1, linear,
                            angular0, angular1);

      float rambdaDt =
          b3CalcRelVel((const b3Vector3&)cs.m_linear,
...
static inline void b3SolveFriction(b3ContactConstraint4& cs,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    float rambdaDt = b3CalcRelVel(linear, -linear, angular0, angular1, linVelA,
...
struct b3SolveTask  // : public ThreadPool::Task
{
  b3SolveTask(b3AlignedObjectArray<b3RigidBodyData>& bodies,
              ...
              int start, int nConstraints, int maxNumBatches,
  ...
  void run(int tIdx) {
    ...
    for (int bb = 0; bb < m_maxNumBatches; bb++) {
      ...
      {
        int i = m_start + ic;
        if (m_constraints[i].m_batchIdx != bb) continue;
        ...
        if (!m_solveFriction) {
          ...
          b3SolveContact(m_constraints[i], (b3Vector3&)bodyA.m_pos,
          ...
          b3SolveFriction(m_constraints[i], (b3Vector3&)bodyA.m_pos,
                          (b3Vector3&)bodyA.m_linVel,
                          (b3Vector3&)bodyA.m_angVel, bodyA.m_invMass,
                          (const b3Matrix3x3&)m_shapes[aIdx].m_invInertiaWorld,
                          (b3Vector3&)bodyB.m_pos, (b3Vector3&)bodyB.m_linVel,
                          (b3Vector3&)bodyB.m_angVel, bodyB.m_invMass,
                          (const b3Matrix3x3&)m_shapes[bIdx].m_invInertiaWorld,
...
void b3CpuRigidBodyPipeline::solveContactConstraints() {
  ...
  //	const b3AlignedObjectArray<b3Contact4Data>& contacts =
  //m_data->m_np->getContacts();
  ...
  for (int iter = 0; iter < m_nIterations; iter++) {
    b3SolveTask task(m_data->m_rigidBodies, m_data->m_inertias,
                     contactConstraints, 0, n, maxNumBatches, 0, 0);
...
void b3CpuRigidBodyPipeline::integrate(float deltaTime) {
  ...
  // integrate transforms (external forces/gravity should be moved into
  // constraint solver)
  for (int i = 0; i < m_data->m_rigidBodies.size(); i++) {
    b3IntegrateTransform(&m_data->m_rigidBodies[i], deltaTime, angDamping,
                         gravityAcceleration);
  }
}

int b3CpuRigidBodyPipeline::registerPhysicsInstance(float mass,
  ...
  if (collidableIndex >= 0) {
    ...
    t.setRotation(b3Quaternion(orientation[0], orientation[1], orientation[2],
                               orientation[3]));
    b3TransformAabb(localAabbMin, localAabbMax, margin, t, worldAabb.m_minVec,
                    worldAabb.m_maxVec);

    m_data->m_bp->createProxy(worldAabb.m_minVec, worldAabb.m_maxVec, bodyIndex,
                              0, 1, 1);
