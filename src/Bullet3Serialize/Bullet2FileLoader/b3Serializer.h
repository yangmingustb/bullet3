...
class b3Serializer {
 public:
  ...
  virtual void finalizeChunk(b3Chunk* chunk, const char* structType,
...
class b3DefaultSerializer : public b3Serializer {
 ...
 protected:
  ...
  void initDNA(const char* bdnaOrg, int dnalen) {
    ...
    // Parse names
    if (!littleEndian) *intPtr = b3SwapEndian(*intPtr);
    ...
    for (i = 0; i < dataLen; i++, shtPtr++) {
      if (!littleEndian) shtPtr[0] = b3SwapEndian(shtPtr[0]);
 ...
 public:
  ...
  virtual ~b3DefaultSerializer() {
    if (m_buffer) b3AlignedFree(m_buffer);
    if (m_dna) b3AlignedFree(m_dna);
  }
  ...
  virtual b3Chunk* allocate(size_t size, int numElements) {
    unsigned char* ptr =
        internalAlloc(int(size) * numElements + sizeof(b3Chunk));
