...
void bFile::parseHeader() {
  if (!mFileLen || !mFileBuffer) return;
  ...
  if (header[7] == '-') {
    mFlags |= FD_FILE_64;
    if (!VOID_IS_8) mFlags |= FD_BITS_VARIES;
  } else if (VOID_IS_8)
    mFlags |= FD_BITS_VARIES;

  // swap endian...
  if (header[8] == 'V') {
    if (littleEndian == 1) mFlags |= FD_ENDIAN_SWAP;
...
void bFile::parseInternal(int verboseMode, char *memDna, int memDnaLength) {
  ...
  for (int i = 0; i < mFileLen; i++) {
    ...
    if (!mDataStart && strncmp(tempBuffer, "REND", 4) == 0) mDataStart = i;

    if (strncmp(tempBuffer, "DNA1", 4) == 0) {
      // read the DNA1 block and extract SDNA
      if (getNextBlock(&dna, tempBuffer, mFlags) > 0) {
        if (strncmp((tempBuffer + ChunkUtils::getOffset(mFlags)), "SDNANAME",
                    8) == 0)
  ...
  /// mFileDNA->init will convert part of DNA file endianness to current CPU
  ...
  ///@todo we need a better version check, add version/sub version info from
  ///FileGlobal into memory DNA/header files
  if (mMemoryDNA->getNumNames() != mFileDNA->getNumNames()) {
    mFlags |= FD_VERSION_VARIES;
    // printf ("Warning, file DNA is different than built in, performance is
    // reduced. Best to re-export file with a matching version/platform");
  }
...
void bFile::preSwap() {
  ...
  while (1) {
    // one behind
    if (dataChunk.code == B3_SDNA || dataChunk.code == B3_DNA1 ||
        dataChunk.code == B3_TYPE || dataChunk.code == B3_TLEN ||
        dataChunk.code == B3_STRC) {
      swapDNA(dataPtr);
      break;
    } else {
...
char *bFile::readStruct(char *head, bChunkInd &dataChunk) {
  ...
  if (mFlags & FD_ENDIAN_SWAP) swap(head, dataChunk, ignoreEndianFlag);
  ...
  if (!mFileDNA->flagEqual(dataChunk.dna_nr)) {
    ...
    /// don't try to convert Link block data, just memcpy it. Other data can be
    /// converted.
    if (strcmp("Link", oldType) != 0) {
      ...
      if ((reverseOld != -1)) {
        // make sure it's here
        // assert(reverseOld!= -1 && "getReverseType() returned -1, struct
        // required!");
        ...
        assert((strcmp(oldType, newType) == 0) &&
...
void bFile::parseStruct(char *strcPtr, char *dtPtr, int old_dna, int new_dna,
  ...
  for (int ele = 0; ele < elementLength; ele++, memoryStruct += 2) {
    ...
    if (revType != -1 && memoryStruct[0] >= firstStructType &&
        memName[0] != '*') {
...
// ----------------------------------------------------- //
static void getElement(int arrayLen, const char *cur, const char *old,
                       char *oldPtr, char *curData) {
...
#define b3SetEle(value, current, type, cast, size, ptr) \
  ...
  for (int i = 0; i < arrayLen; i++) {
    ...
    b3GetEle(value, old, "ushort", unsigned short, sizeof(unsigned short),
             oldPtr);
    b3SetEle(value, cur, "ushort", unsigned short, sizeof(unsigned short),
             curData);
...
void bFile::safeSwapPtr(char *dst, const char *src) {
  ...
  if (ptrFile == ptrMem) {
    ...
    if (oldPtr->m_uniqueIds[0] == oldPtr->m_uniqueIds[1]) {
      // Bullet stores the 32bit unique ID in both upper and lower part of 64bit
      // pointers
      ...
      // endian swap for 64bit pointer otherwise truncation will fail due to
      // trailing zeros
      if (mFlags & FD_ENDIAN_SWAP) B3_SWITCH_LONGINT(longValue);
...
// ----------------------------------------------------- //
void bFile::getMatchingFileDNA(short *dna_addr, const char *lookupName,
                               const char *lookupType, char *strcData,
                               char *data, bool fixupPointers) {
...
// ----------------------------------------------------- //
char *bFile::getFileElement(short *firstStruct, char *lookupName,
                            char *lookupType, char *data, short **foundPos) {
...
/// this loop only works fine if the Blender DNA structure of the file matches
/// the headerfiles
...
int bFile::resolvePointersStructRecursive(char *strcPtr, int dna_nr,
                                          int verboseMode, int recursion) {
  bParse::bDNA *fileDna = mFileDNA ? mFileDNA : mMemoryDNA;
  ...
  for (int ele = 0; ele < elementLength; ele++, oldStruct += 2) {
    ...
    if (memName[0] == '*') {
      if (arrayLen > 1) {
        ...
        if (ptr) {
          //				printf("Fixup pointer at 0x%x from 0x%x to
          //0x%x!\n",ptrptr,*ptrptr,ptr);
          *(ptrptr) = ptr;
          ...
          //				printf("Cannot fixup pointer at 0x%x from 0x%x to
          //0x%x!\n",ptrptr,*ptrptr,ptr);
        }
      }
    } else {
      ...
      if (oldStruct[0] >= firstStructType)  // revType != -1 &&
      {
        ...
        for (int i = 0; i < arrayLen; i++) {
          byteOffset += resolvePointersStructRecursive(
              elemPtr + byteOffset, revType, verboseMode, recursion + 1);
        }
        if (verboseMode & FD_VERBOSE_EXPORT_XML) {
          for (int i = 0; i < recursion; i++) {
            printf("  ");
        ...
        if (verboseMode & FD_VERBOSE_EXPORT_XML) {
          if (arrayLen > MAX_ARRAY_LENGTH) {
            ...
            bool isIntegerType = (strcmp(memType, "char") == 0) ||
                                 (strcmp(memType, "int") == 0) ||
                                 (strcmp(memType, "short") == 0);
            ...
            if (isIntegerType) {
              ...
              if (dbPtr) {
                ...
                for (i = 0; i < recursion; i++) printf("  ");
                if (arrayLen == 1)
                  printf("<%s type=\"%s\">", cleanName, memType);
                else
                  printf("<%s type=\"%s\" count=%d>", cleanName, memType,
                         arrayLen);
                for (i = 0; i < arrayLen; i++) printf(" %d ", dbPtr[i]);
                printf("</%s>\n", cleanName);
              }
            } else {
              ...
              if (dbPtr) {
                ...
                for (i = 0; i < arrayLen; i++) printf(" %f ", dbPtr[i]);
                printf("</%s>\n", cleanName);
              }
            }
          }
...
/// Resolve pointers replaces the original pointers in structures, and linked
/// lists by the new in-memory structures
...

  {
    ...
    for (int i = 0; i < m_chunks.size(); i++) {
      ...
      if (!mFileDNA || fileDna->flagEqual(dataChunk.dna_nr)) {
        ...
        if (verboseMode & FD_VERBOSE_EXPORT_XML) printf(" </%s>\n", oldType);
      } else {
...
void bFile::writeChunks(FILE *fp, bool fixupPointers) {
  ...
  for (int i = 0; i < m_chunks.size(); i++) {
    ...
    if ((reverseOld != -1)) {
      ...
      char *cur = fixupPointers ? (char *)findLibPointer(dataChunk.oldPtr)
                                : (char *)dataChunk.oldPtr;
...
// ----------------------------------------------------- //
int bFile::getNextBlock(bChunkInd *dataChunk, const char *dataPtr,
                        const int flags) {
  bool swap = false;
  bool varies = false;
  ...
  if (VOID_IS_8) {
    if (varies) {
      ...
      if (swap) {
        if ((chunk.code & 0xFFFF) == 0) chunk.code >>= 16;
    ...
    if (varies) {
      ...
      if (head.m_uniqueInts[0] == head.m_uniqueInts[1]) {
        ...
        if (swap) B3_SWITCH_LONGINT(oldPtr);
