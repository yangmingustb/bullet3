...
/// This solver is mainly for debug/learning purposes: it is functionally
/// equivalent to the btSequentialImpulseConstraintSolver solver, but much
/// slower (it builds the full LCP matrix)
class btSolveProjectedGaussSeidel : public btMLCPSolverInterface {
 public:
  ...
  btSolveProjectedGaussSeidel()
      : m_leastSquaresResidualThreshold(0), m_leastSquaresResidual(0) {}
    ...
    for (int k = 0; k < numIterations; k++) {
      ...
      for (i = 0; i < numRows; i++) {
        ...
        if (useSparsity) {
          ...
          for (j = i + 1; j < numRows; j++) delta += A(i, j) * x[j];
        }
        ...
        if (x[i] < lo[i] * s) x[i] = lo[i] * s;
        if (x[i] > hi[i] * s) x[i] = hi[i] * s;
