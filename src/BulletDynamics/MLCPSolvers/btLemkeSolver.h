...
/// The btLemkeSolver is based on "Fast Implementation of Lemke’s Algorithm for
/// Rigid Body Contact Simulation (John E. Lloyd) "
/// It is a slower but more accurate solver. Increase the m_maxLoops for better
/// convergence, at the cost of more CPU time.
/// The original implementation of the btLemkeAlgorithm was done by Kilian
/// Grundl from the MBSim team
class btLemkeSolver : public btMLCPSolverInterface {
 ...
 public:
  ...
  virtual bool solveMLCP(const btMatrixXu& A, const btVectorXu& b,
                         btVectorXu& x, const btVectorXu& lo,
                         const btVectorXu& hi,
    ...
    if (m_useLoHighBounds) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        // this is some hack/safety mechanism, to discard invalid solutions from
        // the Lemke solver
        // we need to figure out why it happens, and fix it, or detect it
        // properly)
        if (x[i] > m_maxValue) {
      ...
      if (fail) {
        ...
        if (errorIndexMin < 0) errorValueMin = 0.f;
        if (errorIndexMax < 0) errorValueMax = 0.f;
        m_errorCountTimes++;
        //	printf("Error (x[%d] = %f, x[%d] = %f), resetting %d times\n",
        //errorIndexMin,errorValueMin, errorIndexMax, errorValueMax,
        //errorCountTimes++);
        for (int i = 0; i < n; i++) {
          x[i] = 0.f;
        }
...

    {
      ...
      //		printf("================ solving using
      //Lemke/Newton/Fixpoint\n");
