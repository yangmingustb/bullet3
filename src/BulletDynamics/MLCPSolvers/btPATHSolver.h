...
class btPathSolver : public btMLCPSolverInterface {
 public:
  btPathSolver() {
    License_SetString(
        "2069810742&Courtesy_License&&&USR&2013&14_12_2011&1000&PATH&GEN&31_12_"
        "2013&0_0_0&0&0_0");
    e.error_data = 0;
    e.warning = MyWarning;
    e.error = MyError;
  ...
  virtual bool solveMLCP(const btMatrixXu &A, const btVectorXu &b,
    ...
    SimpleLCP(numVariables, numNonZero, &rowIndices[0], &colIndices[0],
              &values[0], &rhs[0], &lowerBounds[0], &upperBounds[0], &status,
              &zResult[0]);
