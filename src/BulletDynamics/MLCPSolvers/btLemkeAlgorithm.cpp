...
Code was converted for the Bullet Continuous Collision Detection and Physics
Library
...
// This file is re-distributed under the ZLib license, with permission of the
// original author
// Math library was replaced from fmatvec to a the file
...
btVectorXu btLemkeAlgorithm::solve(unsigned int maxloops /* = 0*/) {
  ...
  for (int i = 0; i < dim; i++) basis.push_back(i);
  ...
  for (int i = 0; i < dim; i++) {
    ...
    if (v < 0) greaterZero = false;
  }

  //  int pivotRowIndex = q_.minIndex();//minIndex(q_);     // first row is that
  //  with lowest q-value
  int z0Row =
      pivotRowIndex;  // remember the col of z0 for ending algorithm afterwards
  ...
  if (DEBUGLEVEL >= 3) {
    ...
    cout << "Basis: ";
    for (int i = 0; i < basis.size(); i++) cout << basis[i] << " ";
    cout << endl;
  }
#endif  // BT_DEBUG_OSTREAM
  ...
  if (!greaterZero) {
    if (maxloops == 0) {
      maxloops = 100;
      //        maxloops = UINT_MAX; //TODO: not a really nice way, problem is:
      //        maxloops should be 2^dim (=1<<dim), but this could exceed
      //        UINT_MAX and thus the result would be 0 and therefore the lemke
      //        algorithm wouldn't start but probably would find a solution
      //        within less then UINT_MAX steps. Therefore this constant is used
      //        as a upper border right now...
    }
    ...
    for (steps = 0; steps < maxloops; steps++) {
      ...
      if (basis[pivotRowIndex] <
          dim)  // if a w-value left the basis get in the correspondent z-value
      ...
      if (z0Row == pivotRowIndex) {  // if z0 leaves the basis the solution is
                                     // found --> one last elimination step is
                                     // necessary
        GaussJordanEliminationStep(A, pivotRowIndex, pivotColIndex, basis);
...
int btLemkeAlgorithm::findLexicographicMinimum(const btMatrixXu& A,
  ...
  for (int row = 0; row < dim; row++) {
    ...
    if (a > 0) {
      ...
      for (int j = 2; j < dim + 1; j++) Rows[row][j] = A(row, j - 1) / a;
      ...
      for (; j < Rows.size(); j++) {
        if (i != j) {
          if (Rows[j].nrm2() > 0.) {
            ...
            //=Rows[j] - Rows[i]
            if (!LexicographicPositive(test)) break;
          }
        }
      }
...
bool btLemkeAlgorithm::LexicographicPositive(const btVectorXu& v) {
  ...
  while (i < v.size() - 1 && fabs(v[i]) < btMachEps()) i++;
...
void btLemkeAlgorithm::GaussJordanEliminationStep(
    btMatrixXu& A, int pivotRowIndex, int pivotColumnIndex,
