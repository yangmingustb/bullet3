...
void btMultiBodyDynamicsWorld::addMultiBody(btMultiBody* body, int group,
                                            int mask) {
  ...
  for (int i = 0; i < m_multiBodies.size(); i++) {
    btMultiBody* body = m_multiBodies[i];
    {
      ...
      for (int b = 0; b < body->getNumLinks(); b++) {
        ...
        if (cur && !cur->isStaticOrKinematicObject()) prev = cur;
      }
    }
  }
...
void btMultiBodyDynamicsWorld::getAnalyticsData(
    btAlignedObjectArray<btSolverAnalyticsData>& islandAnalyticsData) const {
...
btMultiBodyDynamicsWorld::btMultiBodyDynamicsWorld(
  ...
  m_solverMultiBodyIslandCallback =
      new MultiBodyInplaceSolverIslandCallback(constraintSolver, dispatcher);
}
...
void btMultiBodyDynamicsWorld::setMultiBodyConstraintSolver(
...
void btMultiBodyDynamicsWorld::solveConstraints(
...
void btMultiBodyDynamicsWorld::solveInternalConstraints(
    btContactSolverInfo& solverInfo) {
          ...
          if (!bod->isUsingRK4Integration()) {
            if (bod->internalNeedsJointFeedback()) {
              ...
              bod->computeAccelerationsArticulatedBodyAlgorithmMultiDof(
                  solverInfo.m_timeStep, m_scratch_r, m_scratch_v, m_scratch_m,
...
void btMultiBodyDynamicsWorld::solveExternalForces(
    btContactSolverInfo& solverInfo) {
  forwardKinematics();
  ...
  m_solverMultiBodyIslandCallback->setup(
      &solverInfo, constraintsPtr, m_sortedConstraints.size(),
          ...
          if (!bod->isUsingRK4Integration()) {
            ...
            for (int link = 0; link < bod->getNumLinks(); ++link) {
              for (int dof = 0; dof < bod->getLink(link).m_posVarCount; ++dof)
                scratch_q0[7 + bod->getLink(link).m_cfgOffset + dof] =
                    bod->getLink(link).m_jointPos[dof];
            }
            //
            ...
            //
            struct {
              void operator()(btScalar dt, const btScalar* pDer,
                              const btScalar* pCurVal, btScalar* pVal,
                              int size) {
            ...
            struct {
              void operator()(btMultiBody* pBody, const btScalar* pData) {
                btScalar* pVel =
                    const_cast<btScalar*>(pBody->getVelocityVector());
            ...
            //
            struct {
              void operator()(const btScalar* pSrc, btScalar* pDst, int start,
                              int size) {
                for (int i = 0; i < size; ++i) pDst[i] = pSrc[start + i];
              }
            } pCopy;
            //
...
#define output &m_scratch_r[bod->getNumDofs()]
            ...
            bod->computeAccelerationsArticulatedBodyAlgorithmMultiDof(
                0., m_scratch_r, m_scratch_v, m_scratch_m, isConstraintPass,
                getSolverInfo().m_jointFeedbackInWorldSpace,
                getSolverInfo().m_jointFeedbackInJointFrame);
            pCopy(output, scratch_qdd0, 0, numDofs);
            // calc q1 = q0 + h/2 * qd0
            pResetQx();
            bod->stepPositionsMultiDof(btScalar(.5) * h, scratch_qx,
                                       scratch_qd0);
            // calc qd1 = qd0 + h/2 * qdd0
            pEulerIntegrate(btScalar(.5) * h, scratch_qdd0, scratch_qd0,
                            scratch_qd1, numDofs);
            //
            ...
            pEulerIntegrate(btScalar(.5) * h, scratch_qdd1, scratch_qd0,
                            scratch_qd2, numDofs);
            //
            ...
            for (int i = 0; i < numDofs; ++i) {
              delta_q[i] =
                  h / btScalar(6.) * (scratch_qd0[i] + 2 * scratch_qd1[i] +
                                      2 * scratch_qd2[i] + scratch_qd3[i]);
              delta_qd[i] =
                  h / btScalar(6.) * (scratch_qdd0[i] + 2 * scratch_qdd1[i] +
                                      2 * scratch_qdd2[i] + scratch_qdd3[i]);
            ...
            if (!doNotUpdatePos) {
              btScalar* pRealBuf =
                  const_cast<btScalar*>(bod->getVelocityVector());
              pRealBuf +=
                  6 + bod->getNumDofs() + bod->getNumDofs() * bod->getNumDofs();

              for (int i = 0; i < numDofs; ++i) pRealBuf[i] = delta_q[i];

              // bod->stepPositionsMultiDof(1, 0, &delta_q[0]);
              bod->setPosUpdated(true);
            }

            // ugly hack which resets the cached data to t0 (needed for
            // constraint solver)
            {
              for (int link = 0; link < bod->getNumLinks(); ++link)
...
void btMultiBodyDynamicsWorld::predictMultiBodyTransforms(btScalar timeStep) {
  ...
  for (int b = 0; b < m_multiBodies.size(); b++) {
    ...
    if (!isSleeping) {
      ...
      m_scratch_world_to_local.resize(nLinks + 1);
      m_scratch_local_origin.resize(nLinks + 1);
      bod->updateCollisionObjectInterpolationWorldTransforms(
          m_scratch_world_to_local, m_scratch_local_origin);
    } else {
      bod->clearVelocities();
    }
  }
...
void btMultiBodyDynamicsWorld::addMultiBodyConstraint(
    btMultiBodyConstraint* constraint) {
  m_multiBodyConstraints.push_back(constraint);
}

void btMultiBodyDynamicsWorld::removeMultiBodyConstraint(
    btMultiBodyConstraint* constraint) {
  m_multiBodyConstraints.remove(constraint);
}

void btMultiBodyDynamicsWorld::debugDrawMultiBodyConstraint(
    btMultiBodyConstraint* constraint) {
...
void btMultiBodyDynamicsWorld::debugDrawWorld() {
  ...
  if (getDebugDrawer()) {
    ...
    if (drawConstraints) {
      ...
      for (int b = 0; b < m_multiBodies.size(); b++) {
        btMultiBody* bod = m_multiBodies[b];
        bod->forwardKinematics(m_scratch_world_to_local1,
        ...
        for (int m = 0; m < bod->getNumLinks(); m++) {
          ...
          if (bod->getLink(m).m_jointType == btMultibodyLink::eRevolute) {
            btVector3 vec = quatRotate(tr.getRotation(),
                                       bod->getLink(m).m_axes[0].m_topVec) *
                            0.1;
          ...
          if (bod->getLink(m).m_jointType == btMultibodyLink::eFixed) {
            btVector3 vec = quatRotate(tr.getRotation(),
                                       bod->getLink(m).m_axes[0].m_bottomVec) *
                            0.1;
...
void btMultiBodyDynamicsWorld::serializeMultiBodies(btSerializer* serializer) {
  ...
  for (i = 0; i < m_collisionObjects.size(); i++) {
    ...
    if (colObj->getInternalType() == btCollisionObject::CO_FEATHERSTONE_LINK) {
      ...
      serializer->finalizeChunk(chunk, structType, BT_MB_LINKCOLLIDER_CODE,
                                colObj);
    }
  }
}
