...
btMultiBodySliderConstraint::btMultiBodySliderConstraint(
    btMultiBody* body, int link, btRigidBody* bodyB, const btVector3& pivotInA,
    const btVector3& pivotInB, const btMatrix3x3& frameInA,
  ...
  m_data.resize(
      BTMBSLIDERCONSTRAINT_DIM);  // at least store the applied impulses
}
...
btMultiBodySliderConstraint::btMultiBodySliderConstraint(
    ...
    : btMultiBodyConstraint(bodyA, bodyB, linkA, linkB,
                            BTMBSLIDERCONSTRAINT_DIM, false),
      m_rigidBodyA(0),
      m_rigidBodyB(0),
      m_pivotInA(pivotInA),
      m_pivotInB(pivotInB),
...
void btMultiBodySliderConstraint::createConstraintRows(
  ...
  if (m_rigidBodyA) {
    ...
    frameAworld =
        m_frameInA.transpose() * btMatrix3x3(m_rigidBodyA->getOrientation());
  ...
  if (m_rigidBodyB) {
    ...
    frameBworld =
        m_frameInB.transpose() * btMatrix3x3(m_rigidBodyB->getOrientation());
  } else if (m_bodyB) {
  ...
  for (int i = 0; i < numDim; i++) {
    ...
    if (i < 2) {
      ...
      fillMultiBodyConstraint(constraintRow, data, 0, 0, constraintNormalAng,
                              ...
                              posError, infoGlobal, -m_maxAppliedImpulse,
                              m_maxAppliedImpulse);
    } else {  // i>=2
