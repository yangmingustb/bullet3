...
class btMultiBodyMLCPConstraintSolver : public btMultiBodyConstraintSolver {
 protected:
  ...
  /// Indices of normal contact constraint associated with frictional contact
  /// constraint for rigid bodies.
  ///
  /// This is used by the MLCP solver to update the upper bounds of frictional
  /// contact impulse given intermediate
  /// normal contact impulse. For example, i-th element represents the index of
  /// a normal constraint that is
  /// accosiated with i-th frictional contact constraint if i-th constraint is a
  /// frictional contact constraint.
  /// Otherwise, -1.
  btAlignedObjectArray<int> m_limitDependencies;

  /// Indices of normal contact constraint associated with frictional contact
  /// constraint for multibodies.
  ///
  /// This is used by the MLCP solver to update the upper bounds of frictional
  /// contact impulse given intermediate
  /// normal contact impulse. For example, i-th element represents the index of
  ...
  /// Count of fallbacks of using btSequentialImpulseConstraintSolver, which
  /// happens when the MLCP solver fails.
  int m_fallback;

  /// \name MLCP Scratch Variables
  /// The following scratch variables are not stateful -- contents are cleared
  /// prior to each use.
  /// They are only cached here to avoid extra memory allocations and
  /// deallocations and to ensure
  ...
  /// Constructs MLCP terms for constraints of two multi-bodies or one rigid
  /// body and one multibody
  ...
  btScalar solveGroupCacheFriendlySetup(btCollisionObject** bodies,
                                        int numBodies,
 ...
 public:
  ...
  /// \param[in] maxLCPSize Maximum size of LCP to solve using MLCP solver. If
  /// the MLCP size exceeds this number, sequaltial impulse method will be used.
  ...
  /// Returns the number of fallbacks of using
  /// btSequentialImpulseConstraintSolver, which happens when the MLCP
  /// solver fails.
  int getNumFallbacks() const;

  /// Sets the number of fallbacks. This function may be used to reset the
  /// number to zero.
