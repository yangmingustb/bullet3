...
btScalar btMultiBodyConstraintSolver::solveSingleIteration(
  ...
  // printf("m_multiBodyNonContactConstraints =
  // %d\n",m_multiBodyNonContactConstraints.size());

  for (int j = 0; j < m_multiBodyNonContactConstraints.size(); j++) {
    int index =
        iteration & 1 ? j : m_multiBodyNonContactConstraints.size() - 1 - j;

    btMultiBodySolverConstraint& constraint =
        m_multiBodyNonContactConstraints[index];

    btScalar residual = resolveSingleConstraintRowGeneric(constraint);
    leastSquaredResidual = btMax(leastSquaredResidual, residual * residual);

    if (constraint.m_multiBodyA) constraint.m_multiBodyA->setPosUpdated(false);
    if (constraint.m_multiBodyB) constraint.m_multiBodyB->setPosUpdated(false);
  }

  // solve featherstone normal contact
  for (int j0 = 0; j0 < m_multiBodyNormalContactConstraints.size(); j0++) {
    int index = j0;  // iteration&1? j0 :
                     // m_multiBodyNormalContactConstraints.size()-1-j0;

    btMultiBodySolverConstraint& constraint =
        m_multiBodyNormalContactConstraints[index];
  ...
  if (infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS &&
      ((infoGlobal.m_solverMode & SOLVER_DISABLE_IMPLICIT_CONE_FRICTION) ==
       0)) {
    for (int j1 = 0;
         j1 < this->m_multiBodySpinningFrictionContactConstraints.size();
         j1++) {
      if (iteration < infoGlobal.m_numIterations) {
        int index = j1;

        btMultiBodySolverConstraint& frictionConstraint =
            m_multiBodySpinningFrictionContactConstraints[index];
        btScalar totalImpulse =
            m_multiBodyNormalContactConstraints[frictionConstraint
                                                    .m_frictionIndex]
                .m_appliedImpulse;
        // adjust friction limits here
        if (totalImpulse > btScalar(0)) {
          frictionConstraint.m_lowerLimit =
              -(frictionConstraint.m_friction * totalImpulse);
          frictionConstraint.m_upperLimit =
              frictionConstraint.m_friction * totalImpulse;
          btScalar residual =
              resolveSingleConstraintRowGeneric(frictionConstraint);
          leastSquaredResidual =
    ...
    for (int j1 = 0;
         j1 < this->m_multiBodyTorsionalFrictionContactConstraints.size();
         j1++) {
      if (iteration < infoGlobal.m_numIterations) {
        int index =
            j1;  // iteration&1? j1 :
                 // m_multiBodyTorsionalFrictionContactConstraints.size()-1-j1;

        btMultiBodySolverConstraint& frictionConstraint =
            m_multiBodyTorsionalFrictionContactConstraints[index];
        btScalar totalImpulse =
            m_multiBodyNormalContactConstraints[frictionConstraint
                                                    .m_frictionIndex]
                .m_appliedImpulse;
        j1++;
        int index2 = j1;
        btMultiBodySolverConstraint& frictionConstraintB =
            m_multiBodyTorsionalFrictionContactConstraints[index2];
        // adjust friction limits here
        if (totalImpulse > btScalar(0) &&
            frictionConstraint.m_frictionIndex ==
                frictionConstraintB.m_frictionIndex) {
          frictionConstraint.m_lowerLimit =
              -(frictionConstraint.m_friction * totalImpulse);
          frictionConstraint.m_upperLimit =
              frictionConstraint.m_friction * totalImpulse;
          frictionConstraintB.m_lowerLimit =
              -(frictionConstraintB.m_friction * totalImpulse);
          frictionConstraintB.m_upperLimit =
              frictionConstraintB.m_friction * totalImpulse;

          btScalar residual = resolveConeFrictionConstraintRows(
              frictionConstraint, frictionConstraintB);
          leastSquaredResidual =
              btMax(leastSquaredResidual, residual * residual);
    ...
    for (int j1 = 0; j1 < this->m_multiBodyFrictionContactConstraints.size();
         j1++) {
      if (iteration < infoGlobal.m_numIterations) {
        int index = j1;  // iteration&1? j1 :
                         // m_multiBodyFrictionContactConstraints.size()-1-j1;
        btMultiBodySolverConstraint& frictionConstraint =
            m_multiBodyFrictionContactConstraints[index];
        ...
        btScalar totalImpulse =
            m_multiBodyNormalContactConstraints[frictionConstraint
                ...
                .m_appliedImpulse;
        j1++;
        int index2 = j1;  // iteration&1? j1 :
                          // m_multiBodyFrictionContactConstraints.size()-1-j1;
        btMultiBodySolverConstraint& frictionConstraintB =
            m_multiBodyFrictionContactConstraints[index2];
        btAssert(frictionConstraint.m_frictionIndex ==
                 frictionConstraintB.m_frictionIndex);

        if (frictionConstraint.m_frictionIndex ==
            frictionConstraintB.m_frictionIndex) {
          frictionConstraint.m_lowerLimit =
              -(frictionConstraint.m_friction * totalImpulse);
          frictionConstraint.m_upperLimit =
...
btScalar btMultiBodyConstraintSolver::solveGroupCacheFriendlySetup(
  ...
  for (int i = 0; i < numBodies; i++) {
    const btMultiBodyLinkCollider* fcA =
        btMultiBodyLinkCollider::upcast(bodies[i]);
    if (fcA) {
...
void btMultiBodyConstraintSolver::applyDeltaVee(btScalar* delta_vee,
                                                btScalar impulse,
                                                int velocityIndex, int ndof) {
  for (int i = 0; i < ndof; ++i)
    m_data.m_deltaVelocities[velocityIndex + i] += delta_vee[i] * impulse;
}

btScalar btMultiBodyConstraintSolver::resolveSingleConstraintRowGeneric(
    const btMultiBodySolverConstraint& c) {
  ...
  if (c.m_multiBodyA) {
    ...
    for (int i = 0; i < ndofA; ++i)
      deltaVelADotn += m_data.m_jacobians[c.m_jacAindex + i] *
                       m_data.m_deltaVelocities[c.m_deltaVelAindex + i];
  ...
  if (c.m_multiBodyB) {
    ...
    for (int i = 0; i < ndofB; ++i)
      deltaVelBDotn += m_data.m_jacobians[c.m_jacBindex + i] *
                       m_data.m_deltaVelocities[c.m_deltaVelBindex + i];
  ...
  deltaImpulse -= deltaVelADotn * c.m_jacDiagABInv;  // m_jacDiagABInv =
                                                     // 1./denom
  ...
  if (c.m_multiBodyA) {
    applyDeltaVee(&m_data.m_deltaVelocitiesUnitImpulse[c.m_jacAindex],
                  deltaImpulse, c.m_deltaVelAindex, ndofA);
#ifdef DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
    // note: update of the actual velocities (below) in the multibody does not
    // have to happen now since m_deltaVelocities can be applied after all
    // iterations
    // it would make the multibody solver more like the regular one with
    // m_deltaVelocities being equivalent to
    // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
    c.m_multiBodyA->applyDeltaVeeMultiDof2(
  ...
  if (c.m_multiBodyB) {
    applyDeltaVee(&m_data.m_deltaVelocitiesUnitImpulse[c.m_jacBindex],
                  deltaImpulse, c.m_deltaVelBindex, ndofB);
#ifdef DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
    // note: update of the actual velocities (below) in the multibody does not
    // have to happen now since m_deltaVelocities can be applied after all
    // iterations
    ...
    // m_deltaVelocities being equivalent to
    // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
    c.m_multiBodyB->applyDeltaVeeMultiDof2(
...
btScalar btMultiBodyConstraintSolver::resolveConeFrictionConstraintRows(
    const btMultiBodySolverConstraint& cA1,
  ...
  btScalar sumB = 0.f;
  {
    ...
    if (cB.m_multiBodyA) {
      ...
      for (int i = 0; i < ndofA; ++i)
        deltaVelADotn += m_data.m_jacobians[cB.m_jacAindex + i] *
                         m_data.m_deltaVelocities[cB.m_deltaVelAindex + i];
    } else if (cB.m_solverBodyIdA >= 0) {
      bodyA = &m_tmpSolverBodyPool[cB.m_solverBodyIdA];
      deltaVelADotn +=
          cB.m_contactNormal1.dot(bodyA->internalGetDeltaLinearVelocity()) +
          cB.m_relpos1CrossNormal.dot(bodyA->internalGetDeltaAngularVelocity());
    }

    if (cB.m_multiBodyB) {
      ndofB = cB.m_multiBodyB->getNumDofs() + 6;
      for (int i = 0; i < ndofB; ++i)
        deltaVelBDotn += m_data.m_jacobians[cB.m_jacBindex + i] *
                         m_data.m_deltaVelocities[cB.m_deltaVelBindex + i];
    } else if (cB.m_solverBodyIdB >= 0) {
      bodyB = &m_tmpSolverBodyPool[cB.m_solverBodyIdB];
      deltaVelBDotn +=
          cB.m_contactNormal2.dot(bodyB->internalGetDeltaLinearVelocity()) +
          cB.m_relpos2CrossNormal.dot(bodyB->internalGetDeltaAngularVelocity());
    }

    deltaImpulseB -=
        deltaVelADotn * cB.m_jacDiagABInv;  // m_jacDiagABInv = 1./denom
  ...
  if (cA.m_multiBodyA) {
    applyDeltaVee(&m_data.m_deltaVelocitiesUnitImpulse[cA.m_jacAindex],
                  deltaImpulseA, cA.m_deltaVelAindex, ndofA);
#ifdef DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
    // note: update of the actual velocities (below) in the multibody does not
    // have to happen now since m_deltaVelocities can be applied after all
    // iterations
    ...
    // m_deltaVelocities being equivalent to
    // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
    cA.m_multiBodyA->applyDeltaVeeMultiDof2(
        &m_data.m_deltaVelocitiesUnitImpulse[cA.m_jacAindex], deltaImpulseA);
#endif  // DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
  } else if (cA.m_solverBodyIdA >= 0) {
    bodyA->internalApplyImpulse(
        cA.m_contactNormal1 * bodyA->internalGetInvMass(),
        cA.m_angularComponentA, deltaImpulseA);
  }
  if (cA.m_multiBodyB) {
    applyDeltaVee(&m_data.m_deltaVelocitiesUnitImpulse[cA.m_jacBindex],
                  deltaImpulseA, cA.m_deltaVelBindex, ndofB);
#ifdef DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
    // note: update of the actual velocities (below) in the multibody does not
    // have to happen now since m_deltaVelocities can be applied after all
    // iterations
    ...
    // m_deltaVelocities being equivalent to
    // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
    cA.m_multiBodyB->applyDeltaVeeMultiDof2(
        &m_data.m_deltaVelocitiesUnitImpulse[cA.m_jacBindex], deltaImpulseA);
#endif  // DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
  } else if (cA.m_solverBodyIdB >= 0) {
    bodyB->internalApplyImpulse(
        cA.m_contactNormal2 * bodyB->internalGetInvMass(),
        cA.m_angularComponentB, deltaImpulseA);
  }

  if (cB.m_multiBodyA) {
    applyDeltaVee(&m_data.m_deltaVelocitiesUnitImpulse[cB.m_jacAindex],
                  deltaImpulseB, cB.m_deltaVelAindex, ndofA);
#ifdef DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
    // note: update of the actual velocities (below) in the multibody does not
    // have to happen now since m_deltaVelocities can be applied after all
    // iterations
    ...
    // m_deltaVelocities being equivalent to
    // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
    cB.m_multiBodyA->applyDeltaVeeMultiDof2(
        &m_data.m_deltaVelocitiesUnitImpulse[cB.m_jacAindex], deltaImpulseB);
#endif  // DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
  } else if (cB.m_solverBodyIdA >= 0) {
    bodyA->internalApplyImpulse(
        cB.m_contactNormal1 * bodyA->internalGetInvMass(),
        cB.m_angularComponentA, deltaImpulseB);
  }
  if (cB.m_multiBodyB) {
    applyDeltaVee(&m_data.m_deltaVelocitiesUnitImpulse[cB.m_jacBindex],
                  deltaImpulseB, cB.m_deltaVelBindex, ndofB);
#ifdef DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
    // note: update of the actual velocities (below) in the multibody does not
    // have to happen now since m_deltaVelocities can be applied after all
    // iterations
    ...
    // m_deltaVelocities being equivalent to
    // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
    cB.m_multiBodyB->applyDeltaVeeMultiDof2(
        &m_data.m_deltaVelocitiesUnitImpulse[cB.m_jacBindex], deltaImpulseB);
#endif  // DIRECTLY_UPDATE_VELOCITY_DURING_SOLVER_ITERATIONS
  } else if (cB.m_solverBodyIdB >= 0) {
    bodyB->internalApplyImpulse(
        cB.m_contactNormal2 * bodyB->internalGetInvMass(),
        cB.m_angularComponentB, deltaImpulseB);
  }

  btScalar deltaVel =
      deltaImpulseA / cA.m_jacDiagABInv + deltaImpulseB / cB.m_jacDiagABInv;
  return deltaVel;
}

void btMultiBodyConstraintSolver::setupMultiBodyContactConstraint(
    btMultiBodySolverConstraint& solverConstraint,
    const btVector3& contactNormal, const btScalar& appliedImpulse,
    btManifoldPoint& cp, const btContactSolverInfo& infoGlobal,
    btScalar& relaxation, bool isFriction, btScalar desiredVelocity,
    btScalar cfmSlip) {
  ...
  btSolverBody* bodyA =
      multiBodyA ? 0 : &m_tmpSolverBodyPool[solverConstraint.m_solverBodyIdA];
  btSolverBody* bodyB =
      multiBodyB ? 0 : &m_tmpSolverBodyPool[solverConstraint.m_solverBodyIdB];

  btRigidBody* rb0 = multiBodyA ? 0 : bodyA->m_originalBody;
  btRigidBody* rb1 = multiBodyB ? 0 : bodyB->m_originalBody;

  if (bodyA) rel_pos1 = pos1 - bodyA->getWorldTransform().getOrigin();
  if (bodyB) rel_pos2 = pos2 - bodyB->getWorldTransform().getOrigin();
  ...
  if (isFriction) {
    ...
    if ((cp.m_contactPointFlags & BT_CONTACT_FLAG_HAS_CONTACT_CFM) ||
      ...
      if (cp.m_contactPointFlags & BT_CONTACT_FLAG_CONTACT_STIFFNESS_DAMPING) {
        btScalar denom =
            (infoGlobal.m_timeStep * cp.m_combinedContactStiffness1 +
  ...
  if (multiBodyA) {
    if (solverConstraint.m_linkA < 0) {
      ...
      rel_pos1 = pos1 -
                 multiBodyA->getLink(solverConstraint.m_linkA)
    ...
    if (solverConstraint.m_deltaVelAindex < 0) {
      ...
      btAssert(m_data.m_deltaVelocities.size() >=
               solverConstraint.m_deltaVelAindex + ndofA);
    }

    solverConstraint.m_jacAindex = m_data.m_jacobians.size();
    m_data.m_jacobians.resize(m_data.m_jacobians.size() + ndofA);
    m_data.m_deltaVelocitiesUnitImpulse.resize(
        m_data.m_deltaVelocitiesUnitImpulse.size() + ndofA);
    btAssert(m_data.m_jacobians.size() ==
             m_data.m_deltaVelocitiesUnitImpulse.size());

    btScalar* jac1 = &m_data.m_jacobians[solverConstraint.m_jacAindex];
    multiBodyA->fillContactJacobianMultiDof(
        solverConstraint.m_linkA, cp.getPositionWorldOnA(), contactNormal, jac1,
        m_data.scratch_r, m_data.scratch_v, m_data.scratch_m);
    btScalar* delta =
        &m_data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacAindex];
    multiBodyA->calcAccelerationDeltasMultiDof(
    ...
    solverConstraint.m_angularComponentA =
        rb0
            ? rb0->getInvInertiaTensorWorld() * torqueAxis0 *
  ...
  if (multiBodyB) {
    if (solverConstraint.m_linkB < 0) {
      ...
      rel_pos2 = pos2 -
                 multiBodyB->getLink(solverConstraint.m_linkB)
                     .m_cachedWorldTransform.getOrigin();
    }
    ...
    m_data.m_deltaVelocitiesUnitImpulse.resize(
        m_data.m_deltaVelocitiesUnitImpulse.size() + ndofB);
    btAssert(m_data.m_jacobians.size() ==
             m_data.m_deltaVelocitiesUnitImpulse.size());

    multiBodyB->fillContactJacobianMultiDof(
        solverConstraint.m_linkB, cp.getPositionWorldOnB(), -contactNormal,
        &m_data.m_jacobians[solverConstraint.m_jacBindex], m_data.scratch_r,
        m_data.scratch_v, m_data.scratch_m);
    multiBodyB->calcAccelerationDeltasMultiDof(
    ...
    solverConstraint.m_angularComponentB =
        rb1
            ? rb1->getInvInertiaTensorWorld() * -torqueAxis1 *
  ...
  if (!isFriction) {
    ...
    if (cp.m_contactPointFlags & BT_CONTACT_FLAG_FRICTION_ANCHOR) {
      distance = (cp.getPositionWorldOnA() - cp.getPositionWorldOnB())
                     .dot(contactNormal);
    }
  }
  ...
  int ndofB = 0;
  {
    ...
    if (multiBodyA) {
      ...
      if (rb0) {
        rel_vel +=
            (rb0->getVelocityInLocalPoint(rel_pos1) +
             (rb0->getTotalTorque() * rb0->getInvInertiaTensorWorld() *
    ...
    if (multiBodyB) {
      ...
      if (rb1) {
        rel_vel +=
            (rb1->getVelocityInLocalPoint(rel_pos2) +
             (rb1->getTotalTorque() * rb1->getInvInertiaTensorWorld() *
              infoGlobal.m_timeStep)
                 .cross(rel_pos2) +
...

  {
    ...
    btScalar velocityError =
        restitution - rel_vel;  // * damping;	//note for friction
                                // restitution is always set to 0 (check above)
                                // so it is acutally velocityError = -rel_vel
                                // for friction
    if (isFriction) {
  ...
  if (infoGlobal.m_solverMode & SOLVER_USE_ARTICULATED_WARMSTARTING) {
    if (btFabs(cp.m_prevRHS) > 1e-5 &&
        cp.m_prevRHS < 2 * solverConstraint.m_rhs &&
        solverConstraint.m_rhs < 2 * cp.m_prevRHS) {
      solverConstraint.m_appliedImpulse =
          isFriction ? 0 : cp.m_appliedImpulse / cp.m_prevRHS *
    ...
    if (solverConstraint.m_appliedImpulse) {
      if (multiBodyA) {
        ...
        applyDeltaVee(deltaV, impulse, solverConstraint.m_deltaVelAindex,
                      ndofA);
      } else {
        if (rb0)
          bodyA->internalApplyImpulse(solverConstraint.m_contactNormal1 *
                                          bodyA->internalGetInvMass() *
                                          rb0->getLinearFactor(),
                                      solverConstraint.m_angularComponentA,
                                      solverConstraint.m_appliedImpulse);
      }
      if (multiBodyB) {
        ...
        applyDeltaVee(deltaV, impulse, solverConstraint.m_deltaVelBindex,
                      ndofB);
      } else {
        if (rb1)
          bodyB->internalApplyImpulse(
...
void btMultiBodyConstraintSolver::setupMultiBodyTorsionalFrictionConstraint(
    btMultiBodySolverConstraint& solverConstraint,
    const btVector3& constraintNormal, btManifoldPoint& cp,
    btScalar combinedTorsionalFriction, const btContactSolverInfo& infoGlobal,
    btScalar& relaxation, bool isFriction, btScalar desiredVelocity,
    btScalar cfmSlip) {
  ...
  if (multiBodyA) {
    ...
    btScalar* jac1 = &m_data.m_jacobians[solverConstraint.m_jacAindex];
    multiBodyA->fillConstraintJacobianMultiDof(
        solverConstraint.m_linkA, cp.getPositionWorldOnA(), constraintNormal,
        btVector3(0, 0, 0), jac1, m_data.scratch_r, m_data.scratch_v,
        m_data.scratch_m);
    btScalar* delta =
        &m_data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacAindex];
    multiBodyA->calcAccelerationDeltasMultiDof(
  ...
  if (multiBodyB) {
    ...
    multiBodyB->fillConstraintJacobianMultiDof(
        solverConstraint.m_linkB, cp.getPositionWorldOnB(), -constraintNormal,
        btVector3(0, 0, 0), &m_data.m_jacobians[solverConstraint.m_jacBindex],
        m_data.scratch_r, m_data.scratch_v, m_data.scratch_m);
    multiBodyB->calcAccelerationDeltasMultiDof(
        &m_data.m_jacobians[solverConstraint.m_jacBindex],
        &m_data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacBindex],
    ...
    if (multiBodyA) {
      ...
      if (rb0) {
        btSolverBody* solverBodyA =
            &m_tmpSolverBodyPool[solverConstraint.m_solverBodyIdA];
        rel_vel +=
            ...
            solverConstraint.m_relpos1CrossNormal.dot(
                rb0 ? solverBodyA->m_angularVelocity : btVector3(0, 0, 0));
      }
    }
    if (multiBodyB) {
      ndofB = multiBodyB->getNumDofs() + 6;
      ...
      if (rb1) {
        btSolverBody* solverBodyB =
            &m_tmpSolverBodyPool[solverConstraint.m_solverBodyIdB];
        rel_vel +=
            ...
            solverConstraint.m_relpos2CrossNormal.dot(
                rb1 ? solverBodyB->m_angularVelocity : btVector3(0, 0, 0));
      }
    }
  ...
  {
    btScalar velocityError =
        0 - rel_vel;  // * damping;	//note for friction restitution is
                      // always set to 0 (check above) so it is acutally
                      // velocityError = -rel_vel for friction
    ...
    solverConstraint.m_cfm =
        infoGlobal.m_globalCfm * solverConstraint.m_jacDiagABInv;
  }
}

btMultiBodySolverConstraint&
btMultiBodyConstraintSolver::addMultiBodyFrictionConstraint(
    const btVector3& normalAxis, const btScalar& appliedImpulse,
    btPersistentManifold* manifold, int frictionIndex, btManifoldPoint& cp,
    btCollisionObject* colObj0, btCollisionObject* colObj1, btScalar relaxation,
    const btContactSolverInfo& infoGlobal, btScalar desiredVelocity,
    btScalar cfmSlip) {
  BT_PROFILE("addMultiBodyFrictionConstraint");
  btMultiBodySolverConstraint& solverConstraint =
      m_multiBodyFrictionContactConstraints.expandNonInitializing();
  ...
  const btMultiBodyLinkCollider* fcB =
  ...
  int solverBodyIdA =
      mbA ? -1 : getOrInitSolverBody(*colObj0, infoGlobal.m_timeStep);
  int solverBodyIdB =
      mbB ? -1 : getOrInitSolverBody(*colObj1, infoGlobal.m_timeStep);
  ...
  setupMultiBodyContactConstraint(solverConstraint, normalAxis, 0, cp,
...
btMultiBodyConstraintSolver::addMultiBodyTorsionalFrictionConstraint(
    const btVector3& normalAxis, btPersistentManifold* manifold,
  ...
  setupMultiBodyTorsionalFrictionConstraint(
      solverConstraint, normalAxis, cp, combinedTorsionalFriction, infoGlobal,
      relaxation, isFriction, desiredVelocity, cfmSlip);
  return solverConstraint;
}
...
btMultiBodyConstraintSolver::addMultiBodySpinningFrictionConstraint(
  ...
  btMultiBodySolverConstraint& solverConstraint =
      m_multiBodySpinningFrictionContactConstraints.expandNonInitializing();
  solverConstraint.m_orgConstraint = 0;
  solverConstraint.m_orgDofIndex = -1;
...
void btMultiBodyConstraintSolver::convertMultiBodyContact(
  ...
  //	btSolverBody* solverBodyA = mbA ? 0 :
  //&m_tmpSolverBodyPool[solverBodyIdA];
  //	btSolverBody* solverBodyB = mbB ? 0 :
  //&m_tmpSolverBodyPool[solverBodyIdB];

  /// avoid collision response between two static objects
  //	if (!solverBodyA || (solverBodyA->m_invMass.isZero() && (!solverBodyB ||
  //solverBodyB->m_invMass.isZero())))
  //	return;
  ...
  for (int j = 0; j < manifold->getNumContacts(); j++) {
    ...
    if (cp.getDistance() <= manifold->getContactProcessingThreshold()) {
      ...
      btMultiBodySolverConstraint& solverConstraint =
          m_multiBodyNormalContactConstraints.expandNonInitializing();
      ...
      setupMultiBodyContactConstraint(solverConstraint, cp.m_normalWorldOnB,
                                      cp.m_appliedImpulse, cp, infoGlobal,
                                      relaxation, isFriction);
...
#define ENABLE_FRICTION
      ...
      solverConstraint.m_frictionIndex =
          m_multiBodyFrictionContactConstraints.size();

      /// Bullet has several options to set the friction directions
      /// By default, each contact has only a single friction direction that is
      /// recomputed automatically every frame
      /// based on the relative linear velocity.
      /// If the relative velocity is zero, it will automatically compute a
      /// friction direction.
      ...
      if (rollingFriction > 0) {
        if (cp.m_combinedSpinningFriction > 0) {
          addMultiBodySpinningFrictionConstraint(
              cp.m_normalWorldOnB, manifold, frictionIndex, cp,
              cp.m_combinedSpinningFriction, colObj0, colObj1, relaxation,
              infoGlobal);
        }
        if (cp.m_combinedRollingFriction > 0) {
          applyAnisotropicFriction(
              colObj0, cp.m_lateralFrictionDir1,
              btCollisionObject::CF_ANISOTROPIC_ROLLING_FRICTION);
          applyAnisotropicFriction(
              colObj1, cp.m_lateralFrictionDir1,
              btCollisionObject::CF_ANISOTROPIC_ROLLING_FRICTION);
          applyAnisotropicFriction(
          ...
          addMultiBodyTorsionalFrictionConstraint(
              cp.m_lateralFrictionDir1, manifold, frictionIndex, cp,
              cp.m_combinedRollingFriction, colObj0, colObj1, relaxation,
              infoGlobal);
          addMultiBodyTorsionalFrictionConstraint(
              cp.m_lateralFrictionDir2, manifold, frictionIndex, cp,
              cp.m_combinedRollingFriction, colObj0, colObj1, relaxation,
              infoGlobal);
        }
        rollingFriction--;
      ...
      if (!(infoGlobal.m_solverMode &
          ...
          !(cp.m_contactPointFlags &
            BT_CONTACT_FLAG_LATERAL_FRICTION_INITIALIZED)) { /*
                                                                ...
                                                                * rel_vel;
                                                                   btScalar
                                                                lat_rel_vel =
                                                                cp.m_lateralFrictionDir1.length2();
                                                                   if
                                                                ...
                                                                *=
                                                                ...
                                                                SOLVER_USE_2_FRICTION_DIRECTIONS))
                                                                           ...
                                                                           addMultiBodyFrictionConstraint(cp.m_lateralFrictionDir1,solverBodyIdA,solverBodyIdB,frictionIndex,cp,rel_pos1,rel_pos2,colObj0,colObj1,
                                                                relaxation);
                                                                   ...
                                                                   */
        {
          ...
          addMultiBodyFrictionConstraint(
              cp.m_lateralFrictionDir1, cp.m_appliedImpulseLateral1, manifold,
              frictionIndex, cp, colObj0, colObj1, relaxation, infoGlobal);
          ...
          if ((infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS)) {
            ...
            applyAnisotropicFriction(
                ...
                btCollisionObject::CF_ANISOTROPIC_FRICTION);
            addMultiBodyFrictionConstraint(
                cp.m_lateralFrictionDir2, cp.m_appliedImpulseLateral2, manifold,
                frictionIndex, cp, colObj0, colObj1, relaxation, infoGlobal);
          }
        ...
        addMultiBodyFrictionConstraint(
            cp.m_lateralFrictionDir1, cp.m_appliedImpulseLateral1, manifold,
            frictionIndex, cp, colObj0, colObj1, relaxation, infoGlobal,
            cp.m_contactMotion1, cp.m_frictionCFM);

        if ((infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS))
          addMultiBodyFrictionConstraint(
              cp.m_lateralFrictionDir2, cp.m_appliedImpulseLateral2, manifold,
              frictionIndex, cp, colObj0, colObj1, relaxation, infoGlobal,
              cp.m_contactMotion2, cp.m_frictionCFM);
        solverConstraint.m_appliedImpulse = 0.f;
        solverConstraint.m_appliedPushImpulse = 0.f;
      }
...
void btMultiBodyConstraintSolver::convertContacts(
    btPersistentManifold** manifoldPtr, int numManifolds,
  ...
  for (int i = 0; i < numManifolds; i++) {
    ...
    const btMultiBodyLinkCollider* fcB =
        btMultiBodyLinkCollider::upcast(manifold->getBody1());
    if (!fcA && !fcB) {
      // the contact doesn't involve any Featherstone btMultiBody, so deal with
      // the regular btRigidBody/btCollisionObject case
  ...
  for (int i = 0; i < m_tmpNumMultiBodyConstraints; i++) {
    ...
    c->createConstraintRows(m_multiBodyNonContactConstraints, m_data,
                            infoGlobal);
  }
  ...
  if (infoGlobal.m_solverMode & SOLVER_USE_ARTICULATED_WARMSTARTING) {
    for (int i = 0; i < m_multiBodyNonContactConstraints.size(); i++) {
      ...
      solverConstraint.m_appliedImpulse =
          solverConstraint.m_orgConstraint->getAppliedImpulse(
              solverConstraint.m_orgDofIndex) *
          infoGlobal.m_articulatedWarmstartingFactor;
      ...
      if (solverConstraint.m_appliedImpulse) {
        if (multiBodyA) {
          ...
          btScalar* deltaV =
              &m_data
                   .m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacAindex];
          btScalar impulse = solverConstraint.m_appliedImpulse;
          multiBodyA->applyDeltaVeeMultiDof2(deltaV, impulse);
          applyDeltaVee(deltaV, impulse, solverConstraint.m_deltaVelAindex,
                        ndofA);
        ...
        if (multiBodyB) {
          ...
          btScalar* deltaV =
              &m_data
                   .m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacBindex];
          btScalar impulse = solverConstraint.m_appliedImpulse;
          multiBodyB->applyDeltaVeeMultiDof2(deltaV, impulse);
          applyDeltaVee(deltaV, impulse, solverConstraint.m_deltaVelBindex,
                        ndofB);
...
btScalar btMultiBodyConstraintSolver::solveGroup(
  ...
  // printf("btMultiBodyConstraintSolver::solveGroup: numBodies=%d,
...
void btMultiBodyConstraintSolver::writeBackSolverBodyToMultiBody(
    btMultiBodySolverConstraint& c, btScalar deltaTime) {
#if 1
  ...
  if (c.m_orgConstraint) {
    c.m_orgConstraint->internalSetAppliedImpulse(c.m_orgDofIndex,
                                                 c.m_appliedImpulse);
  }
  ...
  if (c.m_multiBodyA) {
    ...
    btVector3 torque =
        c.m_relpos1CrossNormal * (c.m_appliedImpulse / deltaTime);
    if (c.m_linkA < 0) {
  ...
  if (c.m_multiBodyB) {
    {
      ...
      btVector3 torque =
          c.m_relpos2CrossNormal * (c.m_appliedImpulse / deltaTime);
      if (c.m_linkB < 0) {
  ...
  if (c.m_multiBodyA) {
    c.m_multiBodyA->applyDeltaVeeMultiDof(
        &m_data.m_deltaVelocitiesUnitImpulse[c.m_jacAindex],
        c.m_appliedImpulse);
  }

  if (c.m_multiBodyB) {
    c.m_multiBodyB->applyDeltaVeeMultiDof(
        &m_data.m_deltaVelocitiesUnitImpulse[c.m_jacBindex],
        c.m_appliedImpulse);
  }
#endif
}

btScalar btMultiBodyConstraintSolver::solveGroupCacheFriendlyFinish(
    btCollisionObject** bodies, int numBodies,
    const btContactSolverInfo& infoGlobal) {
  ...
  // write back the delta v to the multi bodies, either as applied impulse
  ...
  for (int i = 0; i < numPoolConstraints; i++) {
    btMultiBodySolverConstraint& solverConstraint =
        m_multiBodyNormalContactConstraints[i];
    writeBackSolverBodyToMultiBody(solverConstraint, infoGlobal.m_timeStep);

    writeBackSolverBodyToMultiBody(
        m_multiBodyFrictionContactConstraints[solverConstraint.m_frictionIndex],
        infoGlobal.m_timeStep);

    if ((infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS)) {
      writeBackSolverBodyToMultiBody(m_multiBodyFrictionContactConstraints
...

  {
    ...
    for (int j = 0; j < numPoolConstraints; j++) {
      const btMultiBodySolverConstraint& solverConstraint =
          m_multiBodyNormalContactConstraints[j];
      btManifoldPoint* pt =
          (btManifoldPoint*)solverConstraint.m_originalContactPoint;
      btAssert(pt);
  ...
  return btSequentialImpulseConstraintSolver::solveGroupCacheFriendlyFinish(
      bodies, numBodies, infoGlobal);
}
...
void btMultiBodyConstraintSolver::solveMultiBodyGroup(
    ...
    const btContactSolverInfo& info, btIDebugDraw* debugDrawer,
    btDispatcher* dispatcher) {
  // printf("solveMultiBodyGroup: numBodies=%d, numConstraints=%d,
  // numManifolds=%d, numMultiBodyConstraints=%d\n", numBodies, numConstraints,
  // numManifolds, numMultiBodyConstraints);
