...
btMultiBodyFixedConstraint::btMultiBodyFixedConstraint(
    ...
    : btMultiBodyConstraint(body, 0, link, -1, BTMBFIXEDCONSTRAINT_DIM, false),
      ...
      m_frameInA(frameInA),
      m_frameInB(frameInB) {
  m_data.resize(BTMBFIXEDCONSTRAINT_DIM);  // at least store the applied
                                           // impulses
}
...
btMultiBodyFixedConstraint::btMultiBodyFixedConstraint(
    ...
    : btMultiBodyConstraint(bodyA, bodyB, linkA, linkB, BTMBFIXEDCONSTRAINT_DIM,
                            false),
      m_rigidBodyA(0),
      m_rigidBodyB(0),
      m_pivotInA(pivotInA),
...
void btMultiBodyFixedConstraint::createConstraintRows(
  ...
  for (int i = 0; i < numDim; i++) {
    ...
    if (m_rigidBodyA) {
      ...
      frameAworld =
          frameAworld.transpose() * btMatrix3x3(m_rigidBodyA->getOrientation());
    } else {
      if (m_bodyA) {
    ...
    if (m_rigidBodyB) {
      ...
      frameBworld =
          frameBworld.transpose() * btMatrix3x3(m_rigidBodyB->getOrientation());
    } else {
      if (m_bodyB) {
