...
class btMultiBodyLinkCollider : public btCollisionObject {
 ...
 public:
  ...
  btMultiBodyLinkCollider(btMultiBody* multiBody, int link)
      : m_multiBody(multiBody), m_link(link) {
    m_checkCollideWith = true;
    // we need to remove the 'CF_STATIC_OBJECT' flag, otherwise links/base
    // doesn't merge islands
    // this means that some constraints might point to bodies that are not in
    // the islands, causing crashes
  ...
  static const btMultiBodyLinkCollider* upcast(
  ...
  virtual bool checkCollideWithOverride(const btCollisionObject* co) const {
    ...
    if (other->m_multiBody != this->m_multiBody) return true;
    if (!m_multiBody->hasSelfCollision()) return false;
    ...
    if (m_link >= 0) {
      ...
      } else if (link.m_flags &
                 BT_MULTIBODYLINKFLAGS_DISABLE_PARENT_COLLISION) {
        if (link.m_parent == other->m_link) return false;
      }
    }

    if (other->m_link >= 0) {
      const btMultibodyLink& otherLink =
          other->m_multiBody->getLink(other->m_link);
      if (otherLink.m_flags &
        ...
        while (1) {
          ...
          if (parent_of_other == this->m_link) return false;
        }
      } else if (otherLink.m_flags &
                 BT_MULTIBODYLINKFLAGS_DISABLE_PARENT_COLLISION) {
        if (otherLink.m_parent == this->m_link) return false;
      }
    }
    return true;
  }
...
SIMD_FORCE_INLINE int btMultiBodyLinkCollider::calculateSerializeBufferSize()
    const {
  return sizeof(btMultiBodyLinkColliderData);
}

SIMD_FORCE_INLINE const char* btMultiBodyLinkCollider::serialize(
    void* dataBuffer, class btSerializer* serializer) const {
  btMultiBodyLinkColliderData* dataOut =
      (btMultiBodyLinkColliderData*)dataBuffer;
  btCollisionObject::serialize(&dataOut->m_colObjData, serializer);

  dataOut->m_link = this->m_link;
  dataOut->m_multiBody =
      (btMultiBodyData*)serializer->getUniquePointer(m_multiBody);
