...
static btScalar computeDeltaVelocityInConstraintSpace(
    const btVector3& angularDeltaVelocity, const btVector3& contactNormal,
    btScalar invMass, const btVector3& angularJacobian,
    const btVector3& linearJacobian) {
  return angularDeltaVelocity.dot(angularJacobian) +
         contactNormal.dot(linearJacobian) * invMass;
}

// Faster version of computeDeltaVelocityInConstraintSpace that can be used when
// contactNormal and linearJacobian are
// identical.
static btScalar computeDeltaVelocityInConstraintSpace(
    const btVector3& angularDeltaVelocity, btScalar invMass,
...
// Helper function to compute a delta velocity in the constraint space.
static btScalar computeDeltaVelocityInConstraintSpace(
    const btScalar* deltaVelocity, const btScalar* jacobian, int size) {
  btScalar result = 0;
  for (int i = 0; i < size; ++i) result += deltaVelocity[i] * jacobian[i];
...
static btScalar computeConstraintMatrixDiagElementMultiBody(
  ...
  if (multiBodyA) {
    ...
    const btScalar* deltaA =
        &data.m_deltaVelocitiesUnitImpulse[constraint.m_jacAindex];
    ...
    const btScalar invMassA = solverBodyA->m_originalBody
                                  ? solverBodyA->m_originalBody->getInvMass()
                                  : 0.0;
    ret += computeDeltaVelocityInConstraintSpace(
        constraint.m_relpos1CrossNormal, invMassA,
        constraint.m_angularComponentA);
  }

  if (multiBodyB) {
    const btScalar* jacB = &data.m_jacobians[constraint.m_jacBindex];
    const btScalar* deltaB =
        &data.m_deltaVelocitiesUnitImpulse[constraint.m_jacBindex];
    ...
    const btScalar invMassB = solverBodyB->m_originalBody
                                  ? solverBodyB->m_originalBody->getInvMass()
                                  : 0.0;
    ret += computeDeltaVelocityInConstraintSpace(
        constraint.m_relpos2CrossNormal, invMassB,
...
static btScalar computeConstraintMatrixOffDiagElementMultiBody(
  ...
  if (offDiagMultiBodyA) {
    const btScalar* offDiagJacA =
        &data.m_jacobians[offDiagConstraint.m_jacAindex];
    ...
    if (offDiagMultiBodyA == multiBodyA) {
      ...
      const btScalar* deltaA =
          &data.m_deltaVelocitiesUnitImpulse[constraint.m_jacAindex];
      offDiagA +=
          computeDeltaVelocityInConstraintSpace(deltaA, offDiagJacA, ndofA);
    } else if (offDiagMultiBodyA == multiBodyB) {
      const int ndofB = multiBodyB->getNumDofs() + 6;
      const btScalar* deltaB =
          &data.m_deltaVelocitiesUnitImpulse[constraint.m_jacBindex];
      offDiagA +=
          computeDeltaVelocityInConstraintSpace(deltaB, offDiagJacA, ndofB);
    }
  } else {
    const int solverBodyIdA = constraint.m_solverBodyIdA;
    const int solverBodyIdB = constraint.m_solverBodyIdB;
    ...
    if (offDiagSolverBodyIdA == solverBodyIdA) {
      ...
      offDiagA += computeDeltaVelocityInConstraintSpace(
          ...
          offDiagConstraint.m_contactNormal1, invMassA,
          constraint.m_angularComponentA, constraint.m_contactNormal1);
      ...
      offDiagA += computeDeltaVelocityInConstraintSpace(
          offDiagConstraint.m_relpos1CrossNormal,
          offDiagConstraint.m_contactNormal1, invMassB,
          constraint.m_angularComponentB, constraint.m_contactNormal2);
    }
  }

  if (offDiagMultiBodyB) {
    const btScalar* offDiagJacB =
        &data.m_jacobians[offDiagConstraint.m_jacBindex];
    ...
    if (offDiagMultiBodyB == multiBodyA) {
      ...
      const btScalar* deltaA =
          &data.m_deltaVelocitiesUnitImpulse[constraint.m_jacAindex];
      offDiagA +=
          computeDeltaVelocityInConstraintSpace(deltaA, offDiagJacB, ndofA);
    } else if (offDiagMultiBodyB == multiBodyB) {
      const int ndofB = multiBodyB->getNumDofs() + 6;
      const btScalar* deltaB =
          &data.m_deltaVelocitiesUnitImpulse[constraint.m_jacBindex];
      offDiagA +=
          computeDeltaVelocityInConstraintSpace(deltaB, offDiagJacB, ndofB);
    }
  } else {
    const int solverBodyIdA = constraint.m_solverBodyIdA;
    const int solverBodyIdB = constraint.m_solverBodyIdB;
    ...
    if (offDiagSolverBodyIdB == solverBodyIdA) {
      ...
      offDiagA += computeDeltaVelocityInConstraintSpace(
          ...
          offDiagConstraint.m_contactNormal2, invMassA,
          constraint.m_angularComponentA, constraint.m_contactNormal1);
      ...
      offDiagA += computeDeltaVelocityInConstraintSpace(
          offDiagConstraint.m_relpos2CrossNormal,
          offDiagConstraint.m_contactNormal2, invMassB,
          constraint.m_angularComponentB, constraint.m_contactNormal2);
    }
  }
...
void btMultiBodyMLCPConstraintSolver::createMLCPFast(
    const btContactSolverInfo& infoGlobal) {
  createMLCPFastRigidBody(infoGlobal);
  createMLCPFastMultiBody(infoGlobal);
}

void btMultiBodyMLCPConstraintSolver::createMLCPFastRigidBody(
    const btContactSolverInfo& infoGlobal) {
  int numContactRows = interleaveContactAndFriction1 ? 3 : 1;

  int numConstraintRows = m_allConstraintPtrArray.size();

  if (numConstraintRows == 0) return;
  ...
  }
  {
    ...
    for (int i = 0; i < m_allConstraintPtrArray.size(); i += numRows, c++) {
      ...
      numRows = i < m_tmpSolverNonContactConstraintPool.size()
                    ? m_tmpConstraintSizesPool[c].m_numConstraintRows
                    : numContactRows;
      if (orgBodyA) {
        {
          int slotA = -1;
        ...
        for (int row = 0; row < numRows; row++, cur++) {
          btVector3 normalInvMass =
              m_allConstraintPtrArray[i + row]->m_contactNormal1 *
              orgBodyA->getInvMass();
          btVector3 relPosCrossNormalInvInertia =
              m_allConstraintPtrArray[i + row]->m_relpos1CrossNormal *
              orgBodyA->getInvInertiaTensorWorld();

          for (int r = 0; r < 3; r++) {
            J3.setElem(cur, r,
                       m_allConstraintPtrArray[i + row]->m_contactNormal1[r]);
            J3.setElem(
                cur, r + 4,
                m_allConstraintPtrArray[i + row]->m_relpos1CrossNormal[r]);
      ...
      if (orgBodyB) {
        ...
        for (int row = 0; row < numRows; row++, cur++) {
          btVector3 normalInvMassB =
              m_allConstraintPtrArray[i + row]->m_contactNormal2 *
              orgBodyB->getInvMass();
          ...
          for (int r = 0; r < 3; r++) {
            J3.setElem(cur, r,
                       m_allConstraintPtrArray[i + row]->m_contactNormal2[r]);
            J3.setElem(
                cur, r + 4,
                m_allConstraintPtrArray[i + row]->m_relpos2CrossNormal[r]);
  ...
  int c = 0;
  {
    ...
    for (int i = 0; i < m_allConstraintPtrArray.size(); i += numRows, c++) {
...

      {
        ...
        while (startJointNodeA >= 0) {
          ...
          if (j0 < c) {
            int numRowsOther =
                cr0 < m_tmpSolverNonContactConstraintPool.size()
                    ? m_tmpConstraintSizesPool[j0].m_numConstraintRows
                    : numContactRows;
            size_t ofsother =
                (m_allConstraintPtrArray[cr0]->m_solverBodyIdB == sbA)
                    ? 8 * numRowsOther
                    : 0;
            // printf("%d joint i %d and j0: %d: ",count++,i,j0);
            m_A.multiplyAdd2_p8r(JinvMrow,
                                 Jptr + 2 * 8 * (size_t)ofs[j0] + ofsother,
                                 numRows, numRowsOther, row__, ofs[j0]);
          }
...

      {
        ...
        while (startJointNodeB >= 0) {
          ...
          if (j1 < c) {
            int numRowsOther =
                cj1 < m_tmpSolverNonContactConstraintPool.size()
                    ? m_tmpConstraintSizesPool[j1].m_numConstraintRows
                    : numContactRows;
            size_t ofsother =
                (m_allConstraintPtrArray[cj1]->m_solverBodyIdB == sbB)
                    ? 8 * numRowsOther
                    : 0;
            m_A.multiplyAdd2_p8r(JinvMrow + 8 * (size_t)numRows,
                                 Jptr + 2 * 8 * (size_t)ofs[j1] + ofsother,
                                 numRows, numRowsOther, row__, ofs[j1]);
          }
...

    {
      ...
      for (; row__ < numJointRows;) {
        ...
        const unsigned int infom =
            row__ < m_tmpSolverNonContactConstraintPool.size()
                ? m_tmpConstraintSizesPool[jj].m_numConstraintRows
                : numContactRows;
        ...
        if (orgBodyB) {
          m_A.multiplyAdd2_p8r(JinvMrow + 8 * (size_t)infom,
                               Jrow + 8 * (size_t)infom, infom, infom, row__,
                               row__);
        }
        row__ += infom;
        jj++;
  ...
  if (1) {
    ...
    for (int i = 0; i < m_A.rows(); ++i) {
      m_A.setElem(i, i,
                  m_A(i, i) + infoGlobal.m_globalCfm / infoGlobal.m_timeStep);
    }
  }
...
void btMultiBodyMLCPConstraintSolver::createMLCPFastMultiBody(
    const btContactSolverInfo& infoGlobal) {
  const int multiBodyNumConstraints = m_multiBodyAllConstraintPtrArray.size();

  if (multiBodyNumConstraints == 0) return;
  ...
  // 1. Compute b
  {
    ...
    for (int i = 0; i < multiBodyNumConstraints; ++i) {
      const btMultiBodySolverConstraint& constraint =
          *m_multiBodyAllConstraintPtrArray[i];
      const btScalar jacDiag = constraint.m_jacDiagABInv;

      if (!btFuzzyZero(jacDiag)) {
        // Note that rhsPenetration is currently always zero because the split
        // impulse hasn't been implemented for multibody yet.
  ...
  // 3. Construct A matrix by using the impulse testing
  {
    ...
    for (int i = 0; i < multiBodyNumConstraints; ++i) {
      ...
      const btMultiBodySolverConstraint& constraint =
          *m_multiBodyAllConstraintPtrArray[i];
      const btScalar diagA = computeConstraintMatrixDiagElementMultiBody(
          m_tmpSolverBodyPool, m_data, constraint);
      ...
      for (int j = i + 1; j < multiBodyNumConstraints; ++j) {
        const btMultiBodySolverConstraint& offDiagConstraint =
            *m_multiBodyAllConstraintPtrArray[j];
        const btScalar offDiagA =
            computeConstraintMatrixOffDiagElementMultiBody(
                m_tmpSolverBodyPool, m_data, constraint, offDiagConstraint);
  ...
  for (int i = 0; i < m_multiBodyA.rows(); ++i) {
    m_multiBodyA.setElem(
        i, i,
        m_multiBodyA(i, i) + infoGlobal.m_globalCfm / infoGlobal.m_timeStep);
  }
...
bool btMultiBodyMLCPConstraintSolver::solveMLCP(
    const btContactSolverInfo& infoGlobal) {
  bool result = true;
  ...
  if (m_A.rows() != 0) {
    ...
    if (infoGlobal.m_splitImpulse) {
      ...
      const btAlignedObjectArray<int> limitDependenciesCopy =
          m_limitDependencies;
      // TODO(JS): Do we really need these copies when solveMLCP takes them as
      // const?

      result =
          m_solver->solveMLCP(m_A, m_b, m_x, m_lo, m_hi, m_limitDependencies,
                              infoGlobal.m_numIterations);
      if (result)
        result = m_solver->solveMLCP(Acopy, m_bSplit, m_xSplit, m_lo, m_hi,
                                     limitDependenciesCopy,
                                     infoGlobal.m_numIterations);
    } else {
      result =
          m_solver->solveMLCP(m_A, m_b, m_x, m_lo, m_hi, m_limitDependencies,
  ...
  if (m_multiBodyA.rows() != 0) {
    result = m_solver->solveMLCP(
        m_multiBodyA, m_multiBodyB, m_multiBodyX, m_multiBodyLo, m_multiBodyHi,
        m_multiBodyLimitDependencies, infoGlobal.m_numIterations);
  }
...
btScalar btMultiBodyMLCPConstraintSolver::solveGroupCacheFriendlySetup(
  ...
  //   a. Collect all different kinds of constraint as pointers into one array,
  //   m_allConstraintPtrArray
  //   b. Set the index array for frictional contact constraints,
  //   m_limitDependencies
  {
    ...
    const int numRigidBodyConstraints =
        m_tmpSolverNonContactConstraintPool.size() +
        m_tmpSolverContactConstraintPool.size() +
        m_tmpSolverContactFrictionConstraintPool.size();
    const int numMultiBodyConstraints =
    ...
    for (int i = 0; i < m_tmpSolverNonContactConstraintPool.size(); ++i) {
      m_allConstraintPtrArray.push_back(
          &m_tmpSolverNonContactConstraintPool[i]);
      m_limitDependencies[dindex++] = -1;
    }

    int firstContactConstraintOffset = dindex;

    // The btSequentialImpulseConstraintSolver moves all friction constraints at
    // the very end, we can also interleave them instead
    if (interleaveContactAndFriction1) {
      for (int i = 0; i < m_tmpSolverContactConstraintPool.size(); i++) {
        const int numFrictionPerContact =
            m_tmpSolverContactConstraintPool.size() ==
                    m_tmpSolverContactFrictionConstraintPool.size()
                ? 1
                : 2;
        ...
        m_limitDependencies[dindex++] = -1;
        m_allConstraintPtrArray.push_back(
            &m_tmpSolverContactFrictionConstraintPool[i *
                                                      numFrictionPerContact]);
        int findex =
            (m_tmpSolverContactFrictionConstraintPool[i * numFrictionPerContact]
                 .m_frictionIndex *
             (1 + numFrictionPerContact));
        m_limitDependencies[dindex++] = findex + firstContactConstraintOffset;
        if (numFrictionPerContact == 2) {
          m_allConstraintPtrArray.push_back(
              &m_tmpSolverContactFrictionConstraintPool
                  [i * numFrictionPerContact + 1]);
          m_limitDependencies[dindex++] = findex + firstContactConstraintOffset;
        }
      }
    } else {
      ...
      for (int i = 0; i < m_tmpSolverContactFrictionConstraintPool.size();
           i++) {
        m_allConstraintPtrArray.push_back(
            &m_tmpSolverContactFrictionConstraintPool[i]);
        m_limitDependencies[dindex++] =
            m_tmpSolverContactFrictionConstraintPool[i].m_frictionIndex +
    ...
    for (int i = 0; i < m_multiBodyNonContactConstraints.size(); ++i) {
      m_multiBodyAllConstraintPtrArray.push_back(
    ...
    if (interleaveContactAndFriction1) {
      for (int i = 0; i < m_multiBodyNormalContactConstraints.size(); ++i) {
        const int numtiBodyNumFrictionPerContact =
            m_multiBodyNormalContactConstraints.size() ==
                    m_multiBodyFrictionContactConstraints.size()
                ? 1
                : 2;
        ...
        btMultiBodySolverConstraint& frictionContactConstraint1 =
            m_multiBodyFrictionContactConstraints
                [i * numtiBodyNumFrictionPerContact];
        m_multiBodyAllConstraintPtrArray.push_back(&frictionContactConstraint1);

        const int findex = (frictionContactConstraint1.m_frictionIndex *
                            (1 + numtiBodyNumFrictionPerContact)) +
                           firstContactConstraintOffset;

        m_multiBodyLimitDependencies[dindex++] = findex;

        if (numtiBodyNumFrictionPerContact == 2) {
          btMultiBodySolverConstraint& frictionContactConstraint2 =
              m_multiBodyFrictionContactConstraints
                  [i * numtiBodyNumFrictionPerContact + 1];
          m_multiBodyAllConstraintPtrArray.push_back(
              &frictionContactConstraint2);
      ...
      for (int i = 0; i < m_multiBodyFrictionContactConstraints.size(); ++i) {
        m_multiBodyAllConstraintPtrArray.push_back(
            &m_multiBodyFrictionContactConstraints[i]);
        m_multiBodyLimitDependencies[dindex++] =
            m_multiBodyFrictionContactConstraints[i].m_frictionIndex +
            firstContactConstraintOffset;
      }
    }
...
btScalar btMultiBodyMLCPConstraintSolver::solveGroupCacheFriendlyIterations(
    btCollisionObject** bodies, int numBodies,
    btPersistentManifold** manifoldPtr, int numManifolds,
    btTypedConstraint** constraints, int numConstraints,
    const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer) {
  ...
  // Fallback to
  // btSequentialImpulseConstraintSolver::solveGroupCacheFriendlyIterations if
  // the solution isn't valid.
  if (!result) {
    m_fallback++;
    return btMultiBodyConstraintSolver::solveGroupCacheFriendlyIterations(
...

  {
    ...
    for (int i = 0; i < m_allConstraintPtrArray.size(); ++i) {
      ...
      if (infoGlobal.m_splitImpulse) {
        ...
        solverBodyA.internalApplyPushImpulse(
            c.m_contactNormal1 * solverBodyA.internalGetInvMass(),
            c.m_angularComponentA, deltaPushImpulse);
        solverBodyB.internalApplyPushImpulse(
            c.m_contactNormal2 * solverBodyB.internalGetInvMass(),
            c.m_angularComponentB, deltaPushImpulse);
    ...
    for (int i = 0; i < m_multiBodyAllConstraintPtrArray.size(); ++i) {
      ...
      if (multiBodyA) {
        ...
        // note: update of the actual velocities (below) in the multibody does
        // not have to happen now since m_deltaVelocities can be applied after
        // all iterations
        ...
        multiBodyA->applyDeltaVeeMultiDof2(
      ...
      if (multiBodyB) {
        ...
        // m_deltaVelocities being equivalent to
        // btSolverBody::m_deltaLinearVelocity/m_deltaAngularVelocity
        multiBodyB->applyDeltaVeeMultiDof2(
