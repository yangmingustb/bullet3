...
//
// basically most of the code is general for 2 or 4 wheel vehicles, but some of
// it needs to be reviewed
//
btWheelInfo& btRaycastVehicle::addWheel(
    const btVector3& connectionPointCS, const btVector3& wheelDirectionCS0,
    const btVector3& wheelAxleCS, btScalar suspensionRestLength,
    btScalar wheelRadius, const btVehicleTuning& tuning, bool isFrontWheel) {
...
void btRaycastVehicle::updateWheelTransformsWS(btWheelInfo& wheel,
  ...
  wheel.m_raycastInfo.m_wheelDirectionWS =
      chassisTrans.getBasis() * wheel.m_wheelDirectionCS;
  wheel.m_raycastInfo.m_wheelAxleWS =
      chassisTrans.getBasis() * wheel.m_wheelAxleCS;
}
...
void btRaycastVehicle::updateVehicle(btScalar step) {
  ...
  for (i = 0; i < m_wheelInfo.size(); i++) {
    ...
    btVector3 impulse =
        wheel.m_raycastInfo.m_contactNormalWS * suspensionForce * step;
    btVector3 relpos = wheel.m_raycastInfo.m_contactPointWS -
                       getRigidBody()->getCenterOfMassPosition();
...
btScalar calcRollingFriction(btWheelContactPoint& contactPoint,
  ...
  btVector3 rel_pos1 =
      contactPosWorld - contactPoint.m_body0->getCenterOfMassPosition();
  btVector3 rel_pos2 =
      contactPosWorld - contactPoint.m_body1->getCenterOfMassPosition();
    ...
    for (int i = 0; i < getNumWheels(); i++) {
      ...
      if (groundObject) {
        ...
        resolveSingleBilateral(
            *m_chassisBody, wheelInfo.m_raycastInfo.m_contactPointWS,
  ...
  bool sliding = false;
  {
    for (int wheel = 0; wheel < getNumWheels(); wheel++) {
      ...
      if (groundObject) {
        if (wheelInfo.m_engineForce != 0.f) {
          ...
          btWheelContactPoint contactPt(
              ...
              wheelInfo.m_raycastInfo.m_contactPointWS, m_forwardWS[wheel],
              maxImpulse);
      ...
      if (groundObject) {
        ...
        btScalar maximp = wheelInfo.m_wheelsSuspensionForce * timeStep *
    ...
    for (int wheel = 0; wheel < getNumWheels(); wheel++) {
      ...
      if (m_forwardImpulse[wheel] != btScalar(0.)) {
        m_chassisBody->applyImpulse(
            m_forwardWS[wheel] * (m_forwardImpulse[wheel]), rel_pos);
      }
      if (m_sideImpulse[wheel] != btScalar(0.)) {
        class btRigidBody* groundObject =
        ...
        btVector3 vChassisWorldUp =
            getRigidBody()->getCenterOfMassTransform().getBasis().getColumn(
                m_indexUpAxis);
