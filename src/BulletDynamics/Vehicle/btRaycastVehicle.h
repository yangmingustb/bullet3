...
class btRaycastVehicle : public btActionInterface {
 ...
 public:
  ...
  btRaycastVehicle(const btVehicleTuning& tuning, btRigidBody* chassis,
  ...
  //	void	setRaycastWheelInfo( int wheelIndex , bool isInContact, const
  //btVector3& hitPoint, const btVector3& hitNormal,btScalar depth);

  btWheelInfo& addWheel(const btVector3& connectionPointCS0,
                        const btVector3& wheelDirectionCS0,
                        const btVector3& wheelAxleCS,
                        btScalar suspensionRestLength, btScalar wheelRadius,
                        const btVehicleTuning& tuning, bool isFrontWheel);
  ...
  /// Velocity of vehicle (positive if velocity vector has same direction as
  /// foward vector)
  btScalar getCurrentSpeedKmHour() const { return m_currentVehicleSpeedKmHour; }

  virtual void setCoordinateSystem(int rightIndex, int upIndex,
                                   int forwardIndex) {
...
class btDefaultVehicleRaycaster : public btVehicleRaycaster {
 ...
 public:
  btDefaultVehicleRaycaster(btDynamicsWorld* world) : m_dynamicsWorld(world) {}
