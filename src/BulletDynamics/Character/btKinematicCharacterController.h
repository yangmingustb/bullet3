...
/// btKinematicCharacterController is an object that supports a sliding motion
/// in a world.
/// It uses a ghost object and convex sweep test to test for upcoming
...
btKinematicCharacterController : public btCharacterControllerInterface {
 protected:
  ...
  btConvexShape* m_convexShape;  // is also in m_ghostObject, but it needs to be
                                 // convex, so we store it here to avoid upcast
  ...
  btScalar m_maxSlopeRadians;  // Slope angle that is set (used for returning
                               // the exact value)
  btScalar m_maxSlopeCosine;   // Cosine equivalent of m_maxSlopeRadians
                               // (calculated once when set, for optimization)
  btScalar m_gravity;
  ...
  btVector3 computeReflectionDirection(const btVector3& direction,
                                       const btVector3& normal);
  btVector3 parallelComponent(const btVector3& direction,
                              const btVector3& normal);
  btVector3 perpindicularComponent(const btVector3& direction,
                                   const btVector3& normal);
  ...
  void stepForwardAndStrafe(btCollisionWorld * collisionWorld,
 ...
 public:
  ...
  void setLinearDamping(btScalar d) {
    m_linearDamping =
        btClamped(d, (btScalar)btScalar(0.0), (btScalar)btScalar(1.0));
  }
