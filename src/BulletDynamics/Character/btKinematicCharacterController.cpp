...
class btKinematicClosestNotMeRayResultCallback
 ...
 public:
  ...
  virtual btScalar addSingleResult(btCollisionWorld::LocalRayResult& rayResult,
    ...
    if (rayResult.m_collisionObject == m_me) return 1.0;
...
class btKinematicClosestNotMeConvexResultCallback
 ...
 public:
  ...
  virtual btScalar addSingleResult(
      ...
      bool normalInWorldSpace) {
    if (convexResult.m_hitCollisionObject == m_me) return btScalar(1.0);
 ...
 */
btVector3 btKinematicCharacterController::computeReflectionDirection(
 ...
 */
btVector3 btKinematicCharacterController::parallelComponent(
    const btVector3& direction, const btVector3& normal) {
 ...
 */
btVector3 btKinematicCharacterController::perpindicularComponent(
    const btVector3& direction, const btVector3& normal) {
...
bool btKinematicCharacterController::recoverFromPenetration(
  ...
  // Here we must refresh the overlapping paircache as the penetrating movement
  // itself or the
  // previous recovery iteration might have used setWorldTransform and pushed us
  // into an object
  // that is not in the previous cache contents from the last timestep, as will
  // happen if we
  // are pushed into a new AABB overlap. Unhandled this means the next convex
  // sweep gets stuck.
  //
  // Do this by calling the broadphase's setAabb with the moved AABB, this will
  ...
  for (int i = 0;
    ...
    if (!needsCollision(obj0, obj1)) continue;
    ...
    for (int j = 0; j < m_manifoldArray.size(); j++) {
      ...
      btScalar directionSign = manifold->getBody0() == m_ghostObject
                                   ? btScalar(-1.0)
                                   : btScalar(1.0);
...
void btKinematicCharacterController::stepUp(btCollisionWorld* world) {
  ...
  if (m_verticalVelocity < 0.0) stepHeight = m_stepHeight;
  ...
  m_targetPosition =
      m_currentPosition + m_up * (stepHeight) +
      m_jumpAxis * ((m_verticalOffset > 0.f ? m_verticalOffset : 0.f));
  ...
  btKinematicClosestNotMeConvexResultCallback callback(m_ghostObject, -m_up,
                                                       m_maxSlopeCosine);
  ...
  if (callback.hasHit() && m_ghostObject->hasContactResponse() &&
    ...
    // Only modify the position if the hit was a slope and not a wall or
    // ceiling.
...
bool btKinematicCharacterController::needsCollision(
...
void btKinematicCharacterController::stepForwardAndStrafe(
  ...
  while (fraction > btScalar(0.01) && maxIter-- > 0) {
    ...
    btKinematicClosestNotMeConvexResultCallback callback(
        m_ghostObject, sweepDirNegative, btScalar(0.0));
    callback.m_collisionFilterGroup =
        getGhostObject()->getBroadphaseHandle()->m_collisionFilterGroup;
    callback.m_collisionFilterMask =
        getGhostObject()->getBroadphaseHandle()->m_collisionFilterMask;
    ...
    if (callback.hasHit() && m_ghostObject->hasContactResponse() &&
      ...
      if (distance2 > SIMD_EPSILON) {
        ...
        /* See Quake2: "If velocity is against original velocity, stop ead to
         * avoid tiny oscilations in sloping corners." */
        if (currentDir.dot(m_normalizedDirection) <= btScalar(0.0)) {
          break;
        }
      } else {
        //				printf("currentDir: don't normalize a zero
        //vector\n");
        break;
      }
    } else {
...
void btKinematicCharacterController::stepDown(btCollisionWorld* collisionWorld,
  ...
  // phase 3: down
  /*btScalar additionalDownStep = (m_wasOnGround && !onGround()) ? m_stepHeight
  : 0.0;
  btVector3 step_drop = m_up * (m_currentStepOffset + additionalDownStep);
  btScalar downVelocity = (additionalDownStep == 0.0 &&
  ...
  if (m_verticalVelocity > 0.0) return;
  ...
  btKinematicClosestNotMeConvexResultCallback callback2(m_ghostObject, m_up,
                                                        m_maxSlopeCosine);
  ...
  while (1) {
    ...
    // set double test for 2x the step drop, to check for a large drop vs small
    // drop
    ...
    if (m_useGhostObjectSweepTest) {
      ...
      if (!callback.hasHit() && m_ghostObject->hasContactResponse()) {
        // test a double fall height, to see if the character should interpolate
        // it's fall (full) or not (partial)
        m_ghostObject->convexSweepTest(
    ...
    if (bounce_fix == true)
      has_hit = (callback.hasHit() || callback2.hasHit()) &&
                m_ghostObject->hasContactResponse() &&
                needsCollision(m_ghostObject, callback.m_hitCollisionObject);
    else
    ...
    if (downVelocity2 > 0.0 && downVelocity2 < stepHeight && has_hit == true &&
        runonce == false && (m_wasOnGround || !m_wasJumping)) {
      // redo the velocity calculation when falling a small amount, for fast
      // stairs motion
      // for larger falls, use the smoother/slower interpolated movement by not
      // touching the target position
  ...
  if ((m_ghostObject->hasContactResponse() &&
       (callback.hasHit() &&
        needsCollision(m_ghostObject, callback.m_hitCollisionObject))) ||
      runonce == true) {
    ...
    if (bounce_fix == true) {
      if (full_drop == true)
        ...
        // due to errors in the closestHitFraction variable when used with large
        // polygons, calculate the hit fraction manually
        m_currentPosition.setInterpolate3(m_currentPosition, m_targetPosition,
                                          fraction);
    } else
      m_currentPosition.setInterpolate3(m_currentPosition, m_targetPosition,
...
void btKinematicCharacterController::setAngularVelocity(
...
void btKinematicCharacterController::setLinearVelocity(
    const btVector3& velocity) {
  m_walkDirection = velocity;
  ...
  if (m_walkDirection.length2() > 0) {
    ...
    if (c != 0) {
      ...
      btVector3 upComponent =
          m_up * (btSin(SIMD_HALF_PI - btAcos(c)) * m_walkDirection.length());
...
void btKinematicCharacterController::playerStep(
  ...
  if (!m_useWalkDirection &&
      (m_velocityTimeInterval <= 0.0 || m_walkDirection.fuzzyZero())) {
    //		printf("\n");
    return;  // no motion
  }
  ...
  //		m_verticalVelocity = m_jumpSpeed;
  //	}
  //	if (m_verticalVelocity < 0.0 && btFabs(m_verticalVelocity) >
  //btFabs(m_fallSpeed))
  //	{
  //		m_verticalVelocity = -btFabs(m_fallSpeed);
  //	}
...
bool btKinematicCharacterController::onGround() const {
  return (fabs(m_verticalVelocity) < SIMD_EPSILON) &&
         (fabs(m_verticalOffset) < SIMD_EPSILON);
}
...
btVector3* btKinematicCharacterController::getUpAxisDirections() {
  static btVector3 sUpAxisDirection[3] = {btVector3(1.0f, 0.0f, 0.0f),
