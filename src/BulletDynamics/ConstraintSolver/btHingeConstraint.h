...
/* Hinge Constraint by Dirk Gregorius. Limits added by Marcus Hennix at
 * Starbreeze Studios */
...
#define btHingeConstraintData \
  btHingeConstraintDoubleData2  // rename to 2 for backwards compatibility, so
                                // we can still load the
                                // 'btHingeConstraintDoubleData' version
...
btHingeConstraint : public btTypedConstraint {
 ...
 public:
  ...
  btHingeConstraint(btRigidBody & rbA, const btVector3& pivotInA,
  ...
  void getInfo2NonVirtual(btConstraintInfo2 * info, const btTransform& transA,
                          const btTransform& transB, const btVector3& angVelA,
                          const btVector3& angVelB);

  void getInfo2Internal(btConstraintInfo2 * info, const btTransform& transA,
                        const btTransform& transB, const btVector3& angVelA,
                        const btVector3& angVelB);
  void getInfo2InternalUsingFrameOffset(
  ...
  btTransform& getFrameOffsetA() { return m_rbAFrame; }

  btTransform& getFrameOffsetB() { return m_rbBFrame; }
  ...
  // extra motor API, including ability to set a target rotation (as opposed to
  // angular velocity)
  // note: setMotorTarget sets angular velocity under the hood, so you must call
  // it every tick to
  ...
  void setMotorTarget(const btQuaternion& qAinB,
                      btScalar dt);  // qAinB is rotation of body A wrt body B.
  void setMotorTarget(btScalar targetAngle, btScalar dt);

  void setLimit(btScalar low, btScalar high, btScalar _softness = 0.9f,
  ...
  void setAxis(btVector3 & axisInA) {
    ...
    m_rbBFrame.getOrigin() = m_rbB.getCenterOfMassTransform().inverse()(
    ...
    m_rbBFrame.getBasis().setValue(
        ...
        axisInB.getZ());
    m_rbBFrame.getBasis() =
        m_rbB.getCenterOfMassTransform().getBasis().inverse() *
        m_rbBFrame.getBasis();
  }
  ...
  inline bool getAngularOnly() { return m_angularOnly; }
  inline bool getEnableAngularMotor() { return m_enableAngularMotor; }
  inline btScalar getMotorTargetVelocity() { return m_motorTargetVelocity; }
  inline btScalar getMaxMotorImpulse() { return m_maxMotorImpulse; }
...
struct btHingeConstraintDoubleData {
  ...
  btTransformDoubleData
      m_rbAFrame;  // constraint axii. Assumes z is hinge axis.
...
btHingeAccumulatedAngleConstraint : public btHingeConstraint {
 ...
 public:
  ...
  btHingeAccumulatedAngleConstraint(
      btRigidBody & rbA, const btVector3& pivotInA, const btVector3& axisInA,
      bool useReferenceFrameA = false)
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btHingeConstraint::serialize(
