...
btScalar btNNCGConstraintSolver::solveGroupCacheFriendlySetup(
  ...
  m_deltafCF.resizeNoInitialize(
      m_tmpSolverContactFrictionConstraintPool.size());
  m_deltafCRF.resizeNoInitialize(
      m_tmpSolverContactRollingFrictionConstraintPool.size());

  return val;
}

btScalar btNNCGConstraintSolver::solveSingleIteration(
  ...
  btScalar deltaflengthsqr = 0;
  {
    for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++) {
      btSolverConstraint& constraint = m_tmpSolverNonContactConstraintPool
          [m_orderNonContactConstraintPool[j]];
      if (iteration < constraint.m_overrideNumSolverIterations) {
        btScalar deltaf = resolveSingleConstraintRowGeneric(
  ...
  if (m_onlyForNoneContact) {
    if (iteration == 0) {
      for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++)
        m_pNC[j] = m_deltafNC[j];
    } else {
      // deltaflengthsqrprev can be 0 only if the solver solved the problem
      // exactly in the previous iteration. In this case we should have quit,
      // but mainly for debug reason with this 'hack' it is now allowed to
      // continue the calculation
      btScalar beta = m_deltafLengthSqrPrev > 0
                          ? deltaflengthsqr / m_deltafLengthSqrPrev
                          : 2;
      if (beta > 1) {
        for (int j = 0; j < m_tmpSolverNonContactConstraintPool.size(); j++)
          m_pNC[j] = 0;
...

  {
    if (iteration < infoGlobal.m_numIterations) {
      ...
      if (infoGlobal.m_solverMode &
        ...
        int multiplier =
            (infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS) ? 2
                                                                         : 1;
        ...
        for (int c = 0; c < numPoolConstraints; c++) {
...

          {
            ...
            btScalar deltaf = resolveSingleConstraintRowLowerLimit(
  ...
  if (!m_onlyForNoneContact) {
    if (iteration == 0) {
      ...
      for (int j = 0; j < m_tmpSolverContactFrictionConstraintPool.size(); j++)
        m_pCF[j] = m_deltafCF[j];
      for (int j = 0;
           j < m_tmpSolverContactRollingFrictionConstraintPool.size(); j++)
        m_pCRF[j] = m_deltafCRF[j];
    } else {
      // deltaflengthsqrprev can be 0 only if the solver solved the problem
      // exactly in the previous iteration. In this case we should have quit,
      // but mainly for debug reason with this 'hack' it is now allowed to
      ...
      if (beta > 1) {
        ...
        for (int j = 0; j < m_tmpSolverContactConstraintPool.size(); j++)
          m_pC[j] = 0;
        for (int j = 0; j < m_tmpSolverContactFrictionConstraintPool.size();
             j++)
          m_pCF[j] = 0;
        for (int j = 0;
             j < m_tmpSolverContactRollingFrictionConstraintPool.size(); j++)
        ...
        {
          for (int j = 0;
               j < m_tmpSolverContactRollingFrictionConstraintPool.size();
               j++) {
            btSolverConstraint& constraint =
                m_tmpSolverContactRollingFrictionConstraintPool[j];
            if (iteration < infoGlobal.m_numIterations) {
              ...
              constraint.m_appliedImpulse =
                  btScalar(constraint.m_appliedImpulse) +
...
btScalar btNNCGConstraintSolver::solveGroupCacheFriendlyFinish(
    btCollisionObject** bodies, int numBodies,
