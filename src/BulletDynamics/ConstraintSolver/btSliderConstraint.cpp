...
btSliderConstraint::btSliderConstraint(btRigidBody& rbA, btRigidBody& rbB,
...
btSliderConstraint::btSliderConstraint(btRigidBody& rbB,
  ...
  //m_rbA.getCenterOfMassTransform()(m_frameInA.getOrigin());
...
void btSliderConstraint::getInfo1NonVirtual(btConstraintInfo1* info) {
  info->m_numConstraintRows =
      6;  // Fixed 2 linear + 2 angular + 1 limit (even if not used)
  info->nub = 0;
}
...
void btSliderConstraint::getInfo2(btConstraintInfo2* info) {
  getInfo2NonVirtual(info, m_rbA.getCenterOfMassTransform(),
                     m_rbB.getCenterOfMassTransform(),
                     m_rbA.getLinearVelocity(), m_rbB.getLinearVelocity(),
...
void btSliderConstraint::testAngLimits(void) {
  ...
  if (m_lowerAngLimit <= m_upperAngLimit) {
    ...
    //		btScalar rot = btAtan2Fast(axisB0.dot(axisA1),
    //axisB0.dot(axisA0));
...
btVector3 btSliderConstraint::getAncorInA(void) {
  btVector3 ancorInA;
  ancorInA = m_realPivotAInW +
             (m_lowerLinLimit + m_upperLinLimit) * btScalar(0.5) * m_sliderAxis;
...
void btSliderConstraint::getInfo2NonVirtual(
  ...
  btScalar signFact =
      m_useLinearReferenceFrameA ? btScalar(1.0f) : btScalar(-1.0f);
  ...
  // result in three equations, so we project along two orthos to the slider
  // axis
  ...
  nrow++;
  int s3 = nrow * s;
  btVector3 tmpA(0, 0, 0), tmpB(0, 0, 0), relA(0, 0, 0), relB(0, 0, 0),
      c(0, 0, 0);
  if (m_useOffsetForConstraintFrame) {
    ...
    tmpA = relA.cross(q);
    tmpB = relB.cross(q);
    if (hasStaticBody && getSolveAngLimit()) {  // to make constraint between
  ...
  if (limit || powered) {
    ...
    //
    // we have to be careful that the linear constraint forces (+/- ax1) applied
    // to the two bodies
    ...
    } else {  // The old way. May be incorrect if bodies are not on the slider
              // axis
    ...
    currERP =
        (m_flags & BT_SLIDER_FLAGS_ERP_LIMLIN) ? m_softnessLimLin : info->erp;
    if (powered) {
      ...
      btScalar mot_fact =
          getMotorFactor(m_linPos, m_lowerLinLimit, m_upperLinLimit, tag_vel,
                         info->fps * currERP);
  ...
  if (limit || powered) {
    ...
    currERP =
        (m_flags & BT_SLIDER_FLAGS_ERP_LIMANG) ? m_softnessLimAng : info->erp;
    if (powered) {
      if (m_flags & BT_SLIDER_FLAGS_CFM_DIRANG) {
        info->cfm[srow] = m_cfmDirAng;
      }
      btScalar mot_fact =
          getMotorFactor(m_angPos, m_lowerAngLimit, m_upperAngLimit,
