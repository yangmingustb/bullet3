...
bool btSequentialImpulseConstraintSolverMt::s_allowNestedParallelForLoops =
    false;  // some task schedulers don't like nested loops
int btSequentialImpulseConstraintSolverMt::
    s_minimumContactManifoldsForBatching = 250;
...
void btSequentialImpulseConstraintSolverMt::setupBatchedContactConstraints() {
  ...
  m_batchedContactConstraints.setup(&m_tmpSolverContactConstraintPool,
                                    m_tmpSolverBodyPool,
                                    s_contactBatchingMethod, s_minBatchSize,
                                    s_maxBatchSize, &m_scratchMemory);
}

void btSequentialImpulseConstraintSolverMt::setupBatchedJointConstraints() {
  BT_PROFILE("setupBatchedJointConstraints");
  m_batchedJointConstraints.setup(
      &m_tmpSolverNonContactConstraintPool, m_tmpSolverBodyPool,
      s_jointBatchingMethod, s_minBatchSize, s_maxBatchSize, &m_scratchMemory);
}

void btSequentialImpulseConstraintSolverMt::internalSetupContactConstraints(
    int iContactConstraint, const btContactSolverInfo& infoGlobal) {
  btSolverConstraint& contactConstraint =
      m_tmpSolverContactConstraintPool[iContactConstraint];
  ...
  btManifoldPoint& cp =
      *static_cast<btManifoldPoint*>(contactConstraint.m_originalContactPoint);
  ...
  setupContactConstraint(contactConstraint, solverBodyIdA, solverBodyIdB, cp,
  ...
  if (rollingFrictionIndex >= 0) {
    btSolverConstraint& spinningFrictionConstraint =
        m_tmpSolverContactRollingFrictionConstraintPool[rollingFrictionIndex];
    btAssert(spinningFrictionConstraint.m_frictionIndex == iContactConstraint);
    setupTorsionalFrictionConstraint(
        spinningFrictionConstraint, cp.m_normalWorldOnB, solverBodyIdA,
        solverBodyIdB, cp, cp.m_combinedSpinningFriction, rel_pos1, rel_pos2,
        colObj0, colObj1, relaxation, 0.0f, 0.0f);
    btVector3 axis[2];
    ...
    applyAnisotropicFriction(
        colObj0, axis[0], btCollisionObject::CF_ANISOTROPIC_ROLLING_FRICTION);
    applyAnisotropicFriction(
        colObj1, axis[0], btCollisionObject::CF_ANISOTROPIC_ROLLING_FRICTION);
    applyAnisotropicFriction(
        colObj0, axis[1], btCollisionObject::CF_ANISOTROPIC_ROLLING_FRICTION);
    applyAnisotropicFriction(
        colObj1, axis[1], btCollisionObject::CF_ANISOTROPIC_ROLLING_FRICTION);
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      btSolverConstraint& rollingFrictionConstraint =
          m_tmpSolverContactRollingFrictionConstraintPool[iRollingFric];
      ...
      if (dir.length() > kRollingFrictionThreshold) {
        setupTorsionalFrictionConstraint(
            rollingFrictionConstraint, dir, solverBodyIdA, solverBodyIdB, cp,
            cp.m_combinedRollingFriction, rel_pos1, rel_pos2, colObj0, colObj1,
            relaxation, 0.0f, 0.0f);
      } else {
  ...
  //solverBodyIdB, cp, rel_pos1, rel_pos2, colObj0, colObj1, relaxation,
  //infoGlobal, desiredVelocity, cfmSlip);
  {
    ...
    ///
    /// If you choose SOLVER_DISABLE_VELOCITY_DEPENDENT_FRICTION_DIRECTION, then
    /// the friction will be independent from the relative projected velocity.
    ///
    ...
    btSolverConstraint* frictionConstraint1 =
        &m_tmpSolverContactFrictionConstraintPool[contactConstraint
                                                      .m_frictionIndex];
    btAssert(frictionConstraint1->m_frictionIndex == iContactConstraint);

    btSolverConstraint* frictionConstraint2 = NULL;
    if (infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS) {
      frictionConstraint2 = &m_tmpSolverContactFrictionConstraintPool
                                [contactConstraint.m_frictionIndex + 1];
      btAssert(frictionConstraint2->m_frictionIndex == iContactConstraint);
    }

    if (!(infoGlobal.m_solverMode & SOLVER_ENABLE_FRICTION_DIRECTION_CACHING) ||
      ...
      if (!(infoGlobal.m_solverMode &
        ...
        applyAnisotropicFriction(colObj1, cp.m_lateralFrictionDir1,
                                 btCollisionObject::CF_ANISOTROPIC_FRICTION);
        setupFrictionConstraint(*frictionConstraint1, cp.m_lateralFrictionDir1,
        ...
        if (frictionConstraint2) {
          ...
          applyAnisotropicFriction(colObj1, cp.m_lateralFrictionDir2,
                                   btCollisionObject::CF_ANISOTROPIC_FRICTION);
          setupFrictionConstraint(*frictionConstraint2,
                                  cp.m_lateralFrictionDir2, solverBodyIdA,
                                  solverBodyIdB, cp, rel_pos1, rel_pos2,
                                  colObj0, colObj1, relaxation, infoGlobal);
        }
      } else {
      ...
      setupFrictionConstraint(
          *frictionConstraint1, cp.m_lateralFrictionDir1, solverBodyIdA,
          solverBodyIdB, cp, rel_pos1, rel_pos2, colObj0, colObj1, relaxation,
          infoGlobal, cp.m_contactMotion1, cp.m_frictionCFM);
      if (frictionConstraint2) {
        setupFrictionConstraint(
            *frictionConstraint2, cp.m_lateralFrictionDir2, solverBodyIdA,
            solverBodyIdB, cp, rel_pos1, rel_pos2, colObj0, colObj1, relaxation,
            infoGlobal, cp.m_contactMotion2, cp.m_frictionCFM);
      }
    }
  }

  setFrictionConstraintImpulse(contactConstraint, solverBodyIdA, solverBodyIdB,
                               cp, infoGlobal);
}
...
struct SetupContactConstraintsLoop : public btIParallelForBody {
  ...
  SetupContactConstraintsLoop(btSequentialImpulseConstraintSolverMt* solver,
...
int btSequentialImpulseConstraintSolverMt::getOrInitSolverBodyThreadsafe(
    btCollisionObject& body, btScalar timeStep) {
  //
  // getOrInitSolverBody is threadsafe only for a single thread per solver (with
  // potentially multiple solvers)
  //
  // getOrInitSolverBodyThreadsafe -- attempts to be fully threadsafe (however
  // may affect determinism)
  //
  int solverBodyId = -1;
...
void btSequentialImpulseConstraintSolverMt::
    internalCollectContactManifoldCachedInfo(
        btContactManifoldCachedInfo* cachedInfoArray,
  ...
  for (int i = 0; i < numManifolds; ++i) {
    ...
    int solverBodyIdA =
        getOrInitSolverBodyThreadsafe(*colObj0, infoGlobal.m_timeStep);
    int solverBodyIdB =
        getOrInitSolverBodyThreadsafe(*colObj1, infoGlobal.m_timeStep);
    ...
    // ways.
    btAssert(!m_tmpSolverBodyPool[solverBodyIdA].m_invMass.isZero() ||
             !m_tmpSolverBodyPool[solverBodyIdB].m_invMass.isZero());
    ...
    for (int j = 0; j < manifold->getNumContacts(); j++) {
      ...
      if (cp.getDistance() <= manifold->getContactProcessingThreshold()) {
        ...
        cachedInfo->contactHasRollingFriction[iContact] =
            (cp.m_combinedRollingFriction > 0.f);
        iContact++;
      }
    }
...
struct CollectContactManifoldCachedInfoLoop : public btIParallelForBody {
  ...
  void forLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    m_solver->internalCollectContactManifoldCachedInfo(
        m_cachedInfoArray + iBegin, m_manifoldPtr + iBegin, iEnd - iBegin,
        *m_infoGlobal);
  }
};

void btSequentialImpulseConstraintSolverMt::internalAllocContactConstraints(
    const btContactManifoldCachedInfo* cachedInfoArray, int numManifolds) {
  ...
  for (int iManifold = 0; iManifold < numManifolds; ++iManifold) {
    ...
    for (int i = 0; i < cachedInfo.numTouchingContacts; i++) {
      btSolverConstraint& contactConstraint =
          m_tmpSolverContactConstraintPool[contactIndex];
      ...
      for (int iDir = 0; iDir < m_numFrictionDirections; ++iDir) {
        btSolverConstraint& frictionConstraint =
            m_tmpSolverContactFrictionConstraintPool[frictionIndex];
...
struct AllocContactConstraintsLoop : public btIParallelForBody {
  btSequentialImpulseConstraintSolverMt* m_solver;
  const btSequentialImpulseConstraintSolverMt::btContactManifoldCachedInfo*
      m_cachedInfoArray;
  ...
  void forLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    m_solver->internalAllocContactConstraints(m_cachedInfoArray + iBegin,
                                              iEnd - iBegin);
  }
};
...
void btSequentialImpulseConstraintSolverMt::allocAllContactConstraints(
  ...
  btAlignedObjectArray<btContactManifoldCachedInfo>
      cachedInfoArray;  // = m_manifoldCachedInfoArray;
  cachedInfoArray.resizeNoInitialize(numManifolds);
  if (/* DISABLES CODE */ (false)) {
    // sequential
    internalCollectContactManifoldCachedInfo(&cachedInfoArray[0], manifoldPtr,
                                             numManifolds, infoGlobal);
  } else {
    // may alter ordering of bodies which affects determinism
    CollectContactManifoldCachedInfoLoop loop(this, &cachedInfoArray[0],
                                              manifoldPtr, infoGlobal);
    int grainSize = 200;
...

  {
    ...
    }
    {
      ...
      if (m_tmpSolverContactConstraintPool.capacity() < numContacts) {
        // if we need to reallocate, reserve some extra so we don't have to
        ...
        m_tmpSolverContactFrictionConstraintPool.reserve(
            (numContacts + extraReserve) * m_numFrictionDirections);
        m_tmpSolverContactRollingFrictionConstraintPool.reserve(
            numRollingFrictionConstraints + extraReserve);
      }
      ...
      m_tmpSolverContactFrictionConstraintPool.resizeNoInitialize(
          numContacts * m_numFrictionDirections);
      m_tmpSolverContactRollingFrictionConstraintPool.resizeNoInitialize(
          numRollingFrictionConstraints);
    }
  }
  {
...
void btSequentialImpulseConstraintSolverMt::convertContacts(
    btPersistentManifold** manifoldPtr, int numManifolds,
    const btContactSolverInfo& infoGlobal) {
  if (!m_useBatching) {
    btSequentialImpulseConstraintSolver::convertContacts(
        manifoldPtr, numManifolds, infoGlobal);
    return;
  }
...
void btSequentialImpulseConstraintSolverMt::internalInitMultipleJoints(
    btTypedConstraint** constraints, int iBegin, int iEnd) {
...
struct InitJointsLoop : public btIParallelForBody {
  ...
  InitJointsLoop(btSequentialImpulseConstraintSolverMt* solver,
...
void btSequentialImpulseConstraintSolverMt::internalConvertMultipleJoints(
    const btAlignedObjectArray<JointParams>& jointParamsArray,
    btTypedConstraint** constraints, int iBegin, int iEnd,
    const btContactSolverInfo& infoGlobal) {
  ...
  for (int i = iBegin; i < iEnd; ++i) {
    ...
    if (currentRow != -1) {
      ...
      convertJoint(currentConstraintRow, constraint, info1,
                   jointParams.m_solverBodyA, jointParams.m_solverBodyB,
                   infoGlobal);
    }
  }
}
...
struct ConvertJointsLoop : public btIParallelForBody {
  ...
  const btAlignedObjectArray<
      btSequentialImpulseConstraintSolverMt::JointParams>& m_jointParamsArray;
  ...
  ConvertJointsLoop(
      btSequentialImpulseConstraintSolverMt* solver,
      const btAlignedObjectArray<
          btSequentialImpulseConstraintSolverMt::JointParams>& jointParamsArray,
      btTypedConstraint** srcConstraints, const btContactSolverInfo& infoGlobal)
      : m_jointParamsArray(jointParamsArray), m_infoGlobal(infoGlobal) {
    m_solver = solver;
    m_srcConstraints = srcConstraints;
  }
  void forLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    m_solver->internalConvertMultipleJoints(
        m_jointParamsArray, m_srcConstraints, iBegin, iEnd, m_infoGlobal);
  }
};

void btSequentialImpulseConstraintSolverMt::convertJoints(
  ...
  for (int i = 0; i < numConstraints; i++) {
    ...
    if (info1.m_numConstraintRows) {
      ...
      params.m_solverBodyA = getOrInitSolverBody(constraint->getRigidBodyA(),
                                                 infoGlobal.m_timeStep);
      params.m_solverBodyB = getOrInitSolverBody(constraint->getRigidBodyB(),
                                                 infoGlobal.m_timeStep);
    } else {
  ...
  if (parallelJointSetup) {
    ...
    internalConvertMultipleJoints(jointParamsArray, constraints, 0,
                                  numConstraints, infoGlobal);
  }
  setupBatchedJointConstraints();
}

void btSequentialImpulseConstraintSolverMt::internalConvertBodies(
    btCollisionObject** bodies, int iBegin, int iEnd,
    const btContactSolverInfo& infoGlobal) {
...
void btSequentialImpulseConstraintSolverMt::convertBodies(
    btCollisionObject** bodies, int numBodies,
    const btContactSolverInfo& infoGlobal) {
...
btScalar btSequentialImpulseConstraintSolverMt::
  ...
  for (int iiCons = batchBegin; iiCons < batchEnd; ++iiCons) {
    ...
    btScalar residual =
        resolveSplitPenetrationImpulse(bodyA, bodyB, solveManifold);
...
struct ContactSplitPenetrationImpulseSolverLoop : public btIParallelSumBody {
  ...
  btScalar sumLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    ...
    for (int iBatch = iBegin; iBatch < iEnd; ++iBatch) {
      const btBatchedConstraints::Range& batch = m_bc->m_batches[iBatch];
      sum += m_solver->resolveMultipleContactSplitPenetrationImpulseConstraints(
          m_bc->m_constraintIndices, batch.begin, batch.end);
    }
    return sum;
  }
};
...
void btSequentialImpulseConstraintSolverMt::
  ...
  if (infoGlobal.m_splitImpulse) {
    for (int iteration = 0; iteration < infoGlobal.m_numIterations;
         iteration++) {
      btScalar leastSquaresResidual = 0.f;
      if (m_useBatching) {
        const btBatchedConstraints& batchedCons = m_batchedContactConstraints;
        ...
        btScalar leastSquaresResidual = 0.f;
        for (int iiPhase = 0; iiPhase < batchedCons.m_phases.size();
             ++iiPhase) {
          int iPhase = batchedCons.m_phaseOrder[iiPhase];
          const btBatchedConstraints::Range& phase =
              batchedCons.m_phases[iPhase];
          int grainSize = batchedCons.m_phaseGrainSize[iPhase];
          leastSquaresResidual +=
              btParallelSum(phase.begin, phase.end, grainSize, loop);
        }
      } else {
        // non-batched
        leastSquaresResidual =
            resolveMultipleContactSplitPenetrationImpulseConstraints(
                m_orderTmpConstraintPool, 0,
                m_tmpSolverContactConstraintPool.size());
      }
      if (leastSquaresResidual <= infoGlobal.m_leastSquaresResidualThreshold ||
...
btScalar btSequentialImpulseConstraintSolverMt::solveSingleIteration(
    int iteration, btCollisionObject** bodies, int numBodies,
    btPersistentManifold** manifoldPtr, int numManifolds,
    btTypedConstraint** constraints, int numConstraints,
    const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer) {
  if (!m_useBatching) {
    return btSequentialImpulseConstraintSolver::solveSingleIteration(
...

  {
    ...
    if (iteration < infoGlobal.m_numIterations) {
      ...
      if (infoGlobal.m_solverMode &
        ...
        // solve all contact, contact-friction, and rolling friction constraints
        // interleaved
...
btScalar btSequentialImpulseConstraintSolverMt::resolveMultipleJointConstraints(
    const btAlignedObjectArray<int>& consIndices, int batchBegin, int batchEnd,
    int iteration) {
  btScalar leastSquaresResidual = 0.f;
  for (int iiCons = batchBegin; iiCons < batchEnd; ++iiCons) {
    int iCons = consIndices[iiCons];
    const btSolverConstraint& constraint =
        m_tmpSolverNonContactConstraintPool[iCons];
    if (iteration < constraint.m_overrideNumSolverIterations) {
      ...
      btScalar residual =
          resolveSingleConstraintRowGeneric(bodyA, bodyB, constraint);
      leastSquaresResidual += residual * residual;
    }
  }
  return leastSquaresResidual;
...
btSequentialImpulseConstraintSolverMt::resolveMultipleContactConstraints(
  ...
  for (int iiCons = batchBegin; iiCons < batchEnd; ++iiCons) {
    ...
    btSolverBody& bodyB = m_tmpSolverBodyPool[solveManifold.m_solverBodyIdB];
    btScalar residual =
        resolveSingleConstraintRowLowerLimit(bodyA, bodyB, solveManifold);
    leastSquaresResidual += residual * residual;
  }
  return leastSquaresResidual;
}
...
btScalar btSequentialImpulseConstraintSolverMt::
  ...
  for (int iiCons = batchBegin; iiCons < batchEnd; ++iiCons) {
    ...
    btScalar totalImpulse =
        m_tmpSolverContactConstraintPool[iContact].m_appliedImpulse;
    ...
    if (totalImpulse > 0.0f) {
      ...
      for (int iFriction = iBegin; iFriction < iEnd; ++iFriction) {
        btSolverConstraint& solveManifold =
            m_tmpSolverContactFrictionConstraintPool[iFriction++];
        ...
        btScalar residual =
            resolveSingleConstraintRowGeneric(bodyA, bodyB, solveManifold);
        leastSquaresResidual += residual * residual;
      }
    }
  }
...
btScalar btSequentialImpulseConstraintSolverMt::
    resolveMultipleContactConstraintsInterleaved(
        const btAlignedObjectArray<int>& contactIndices, int batchBegin,
        int batchEnd) {
  btScalar leastSquaresResidual = 0.f;
  ...
  for (int iiCons = batchBegin; iiCons < batchEnd; iiCons++) {
    ...
    {
      const btSolverConstraint& solveManifold =
          m_tmpSolverContactConstraintPool[iContact];
...
void btSequentialImpulseConstraintSolverMt::randomizeBatchedConstraintOrdering(
...
void btSequentialImpulseConstraintSolverMt::randomizeConstraintOrdering(
...
struct JointSolverLoop : public btIParallelSumBody {
  ...
  JointSolverLoop(btSequentialImpulseConstraintSolverMt* solver,
                  const btBatchedConstraints* bc, int iteration) {
    m_solver = solver;
    m_bc = bc;
  ...
  btScalar sumLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    ...
    for (int iBatch = iBegin; iBatch < iEnd; ++iBatch) {
      const btBatchedConstraints::Range& batch = m_bc->m_batches[iBatch];
      sum += m_solver->resolveMultipleJointConstraints(
          m_bc->m_constraintIndices, batch.begin, batch.end, m_iteration);
    }
    return sum;
  }
};

btScalar btSequentialImpulseConstraintSolverMt::resolveAllJointConstraints(
    int iteration) {
...
struct ContactSolverLoop : public btIParallelSumBody {
  ...
  ContactSolverLoop(btSequentialImpulseConstraintSolverMt* solver,
                    const btBatchedConstraints* bc) {
    m_solver = solver;
    m_bc = bc;
  }
  btScalar sumLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    ...
    for (int iBatch = iBegin; iBatch < iEnd; ++iBatch) {
      const btBatchedConstraints::Range& batch = m_bc->m_batches[iBatch];
      sum += m_solver->resolveMultipleContactConstraints(
          m_bc->m_constraintIndices, batch.begin, batch.end);
    }
    return sum;
  }
...
struct ContactFrictionSolverLoop : public btIParallelSumBody {
  ...
  ContactFrictionSolverLoop(btSequentialImpulseConstraintSolverMt* solver,
                            const btBatchedConstraints* bc) {
    m_solver = solver;
    m_bc = bc;
  }
  btScalar sumLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    ...
    for (int iBatch = iBegin; iBatch < iEnd; ++iBatch) {
      const btBatchedConstraints::Range& batch = m_bc->m_batches[iBatch];
      sum += m_solver->resolveMultipleContactFrictionConstraints(
          m_bc->m_constraintIndices, batch.begin, batch.end);
    }
    return sum;
  }
...
struct InterleavedContactSolverLoop : public btIParallelSumBody {
  ...
  InterleavedContactSolverLoop(btSequentialImpulseConstraintSolverMt* solver,
                               const btBatchedConstraints* bc) {
    m_solver = solver;
    m_bc = bc;
  }
  btScalar sumLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    ...
    for (int iBatch = iBegin; iBatch < iEnd; ++iBatch) {
      const btBatchedConstraints::Range& batch = m_bc->m_batches[iBatch];
      sum += m_solver->resolveMultipleContactConstraintsInterleaved(
          m_bc->m_constraintIndices, batch.begin, batch.end);
    }
    return sum;
  }
...
struct ContactRollingFrictionSolverLoop : public btIParallelSumBody {
  ...
  btScalar sumLoop(int iBegin, int iEnd) const BT_OVERRIDE {
    ...
    for (int iBatch = iBegin; iBatch < iEnd; ++iBatch) {
      const btBatchedConstraints::Range& batch = m_bc->m_batches[iBatch];
      sum += m_solver->resolveMultipleContactRollingFrictionConstraints(
          m_bc->m_constraintIndices, batch.begin, batch.end);
    }
    return sum;
  }
...
btSequentialImpulseConstraintSolverMt::resolveAllRollingFrictionConstraints() {
  ...
  //
  //  1. either most bodies in the simulation have rolling friction, in which
  //  case we can use the
  //     batches for contacts and use a lookup table to translate contact
  //     indices to rolling friction
  //     (ignoring any contact indices that don't map to a rolling friction
  //     constraint). As long as
  //     most contacts have a corresponding rolling friction constraint, this
  //     should parallelize well.
  //
  //  -OR-
  //
  //  2. few bodies in the simulation have rolling friction, so it is not worth
  //  trying to use the
  //     batches from contacts as most of the contacts won't have corresponding
  //     rolling friction
  //     constraints and most threads would end up doing very little work. Most
  //     of the time would
  ...
  if (numRollingFrictionPoolConstraints >=
      m_tmpSolverContactConstraintPool.size()) {
...
void btSequentialImpulseConstraintSolverMt::internalWriteBackContacts(
  ...
  //    m_tmpSolverContactConstraintPool[ iContact ];
  //    btManifoldPoint* pt = (btManifoldPoint*)
  //    contactConstraint.m_originalContactPoint;
  //    btAssert( pt );
  //    pt->m_appliedImpulse = contactConstraint.m_appliedImpulse;
  //    pt->m_appliedImpulseLateral1 = m_tmpSolverContactFrictionConstraintPool[
  //    contactConstraint.m_frictionIndex ].m_appliedImpulse;
  //    if ( m_numFrictionDirections == 2 )
  //    {
  //        pt->m_appliedImpulseLateral2 =
  //        m_tmpSolverContactFrictionConstraintPool[
  //        contactConstraint.m_frictionIndex + 1 ].m_appliedImpulse;
  //    }
  //}
}

void btSequentialImpulseConstraintSolverMt::internalWriteBackJoints(
    int iBegin, int iEnd, const btContactSolverInfo& infoGlobal) {
  BT_PROFILE("internalWriteBackJoints");
  writeBackJoints(iBegin, iEnd, infoGlobal);
}

void btSequentialImpulseConstraintSolverMt::internalWriteBackBodies(
    int iBegin, int iEnd, const btContactSolverInfo& infoGlobal) {
...
struct WriteContactPointsLoop : public btIParallelForBody {
  ...
  WriteContactPointsLoop(btSequentialImpulseConstraintSolverMt* solver,
                         const btContactSolverInfo& infoGlobal) {
    m_solver = solver;
    m_infoGlobal = &infoGlobal;
  }
...
struct WriteJointsLoop : public btIParallelForBody {
  ...
  WriteJointsLoop(btSequentialImpulseConstraintSolverMt* solver,
                  const btContactSolverInfo& infoGlobal) {
    m_solver = solver;
    m_infoGlobal = &infoGlobal;
  }
...
struct WriteBodiesLoop : public btIParallelForBody {
  ...
  WriteBodiesLoop(btSequentialImpulseConstraintSolverMt* solver,
                  const btContactSolverInfo& infoGlobal) {
    m_solver = solver;
    m_infoGlobal = &infoGlobal;
  }
...
btScalar btSequentialImpulseConstraintSolverMt::solveGroupCacheFriendlyFinish(
    btCollisionObject** bodies, int numBodies,
    const btContactSolverInfo& infoGlobal) {
...

  {
    ...
    int grainSize = 400;
    btParallelFor(0, m_tmpSolverNonContactConstraintPool.size(), grainSize,
                  loop);
  }
  {
