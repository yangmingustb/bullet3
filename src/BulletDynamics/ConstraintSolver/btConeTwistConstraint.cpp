...
SIMD_FORCE_INLINE btScalar computeAngularImpulseDenominator(
    const btVector3& axis, const btMatrix3x3& invInertiaWorld) {
...
void btConeTwistConstraint::init() {
  ...
  setLimit(btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT),
...
void btConeTwistConstraint::getInfo2NonVirtual(
  ...
  // set right hand side
  btScalar linERP =
      (m_flags & BT_CONETWIST_FLAGS_LIN_ERP) ? m_linERP : info->erp;
  btScalar k = info->fps * linERP;
  int j;
  for (j = 0; j < 3; j++) {
    info->m_constraintError[j * info->rowskip] =
        k * (a2[j] + transB.getOrigin()[j] - a1[j] - transA.getOrigin()[j]);
  ...
  if (m_solveSwingLimit) {
    ...
    if ((m_swingSpan1 < m_fixThresh) && (m_swingSpan2 < m_fixThresh)) {
      ...
      info->m_upperLimit[srow] = (m_bMotorEnabled && m_maxMotorImpulse >= 0.0f)
                                     ? m_maxMotorImpulse
                                     : SIMD_INFINITY;
      srow += info->rowskip;
    }
...
void btConeTwistConstraint::buildJacobian() {
  if (m_useSolveConstraintObsolete) {
    ...
    if (!m_angularOnly) {
      btVector3 pivotAInW =
          m_rbA.getCenterOfMassTransform() * m_rbAFrame.getOrigin();
      btVector3 pivotBInW =
          m_rbB.getCenterOfMassTransform() * m_rbBFrame.getOrigin();
      ...
      for (int i = 0; i < 3; i++) {
        new (&m_jac[i]) btJacobianEntry(
            ...
            pivotBInW - m_rbB.getCenterOfMassPosition(), normal[i],
  ...
  if (m_useSolveConstraintObsolete) {
    ...
    if (!m_angularOnly) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        btScalar impulse =
            depth * tau / timeStep * jacDiagABInv - rel_vel * jacDiagABInv;
        m_appliedImpulse += impulse;

        btVector3 ftorqueAxis1 = rel_pos1.cross(normal);
        btVector3 ftorqueAxis2 = rel_pos2.cross(normal);
        bodyA.internalApplyImpulse(
            normal * m_rbA.getInvMass(),
            m_rbA.getInvInertiaTensorWorld() * ftorqueAxis1, impulse);
        bodyB.internalApplyImpulse(
            normal * m_rbB.getInvMass(),
            m_rbB.getInvInertiaTensorWorld() * ftorqueAxis2, -impulse);
      }
    }
    ...
    if (m_bMotorEnabled) {
      ...
      btTransformUtil::integrateTransform(trACur, zerovec, omegaA, timeStep,
                                          trAPred);
      btTransform trBPred;
      trBPred.setIdentity();
      btTransformUtil::integrateTransform(trBCur, zerovec, omegaB, timeStep,
                                          trBPred);
      ...
      btTransformUtil::calculateVelocity(trACur, trADes, timeStep, zerovec,
                                         omegaADes);
      btTransformUtil::calculateVelocity(trBCur, trBDes, timeStep, zerovec,
                                         omegaBDes);
      ...
      if (bDoTorque && avgAxis.length2() > SIMD_EPSILON) {
        ...
        if (m_maxMotorImpulse >= 0) {
          ...
          if (m_bNormalizedMotorStrength) fMaxImpulse = fMaxImpulse / kAxisAInv;
        ...
        bodyA.internalApplyImpulse(
            btVector3(0, 0, 0), m_rbA.getInvInertiaTensorWorld() * impulseAxis,
            impulseMag);
        bodyB.internalApplyImpulse(
            btVector3(0, 0, 0), m_rbB.getInvInertiaTensorWorld() * impulseAxis,
            -impulseMag);
      }
    ...
    // joint limits
    {
      ...
      if (m_solveSwingLimit) {
        ...
        m_accSwingLimitImpulse =
            btMax(m_accSwingLimitImpulse + impulseMag, btScalar(0.0));
        impulseMag = m_accSwingLimitImpulse - temp;

        btVector3 impulse = m_swingAxis * impulseMag;

        // don't let cone response affect twist
        // (this can happen since body A's twist doesn't match body B's AND we
        // use an elliptical cone limit)
        {
          btVector3 impulseTwistCouple =
              impulse.dot(m_twistAxisA) * m_twistAxisA;
        ...
        bodyA.internalApplyImpulse(
            btVector3(0, 0, 0),
            m_rbA.getInvInertiaTensorWorld() * noTwistSwingAxis, impulseMag);
        bodyB.internalApplyImpulse(
            btVector3(0, 0, 0),
            m_rbB.getInvInertiaTensorWorld() * noTwistSwingAxis, -impulseMag);
      }
      ...
      if (m_solveTwistLimit) {
        ...
        if (relTwistVel > 0)  // only damp when moving towards limit
                              // (m_twistAxis flipping is important)
        ...
        m_accTwistLimitImpulse =
            btMax(m_accTwistLimitImpulse + impulseMag, btScalar(0.0));
        impulseMag = m_accTwistLimitImpulse - temp;

        //		btVector3 impulse = m_twistAxis * impulseMag;

        bodyA.internalApplyImpulse(
            btVector3(0, 0, 0), m_rbA.getInvInertiaTensorWorld() * m_twistAxis,
            impulseMag);
        bodyB.internalApplyImpulse(
            btVector3(0, 0, 0), m_rbB.getInvInertiaTensorWorld() * m_twistAxis,
            -impulseMag);
      }
    }
  }
...
void btConeTwistConstraint::updateRHS(btScalar timeStep) { (void)timeStep; }
...
void btConeTwistConstraint::calcAngleInfo() {
  ...
  b1Axis1 = getRigidBodyA().getCenterOfMassTransform().getBasis() *
            this->m_rbAFrame.getBasis().getColumn(0);
  b2Axis1 = getRigidBodyB().getCenterOfMassTransform().getBasis() *
            this->m_rbBFrame.getBasis().getColumn(0);
  ...
  if (m_swingSpan1 >= btScalar(0.05f)) {
    b1Axis2 = getRigidBodyA().getCenterOfMassTransform().getBasis() *
              this->m_rbAFrame.getBasis().getColumn(1);
  ...
  if (m_swingSpan2 >= btScalar(0.05f)) {
    b1Axis3 = getRigidBodyA().getCenterOfMassTransform().getBasis() *
              this->m_rbAFrame.getBasis().getColumn(2);
    swx = b2Axis1.dot(b1Axis1);
  ...
  btScalar EllipseAngle = btFabs(swing1 * swing1) * RMaxAngle1Sq +
  ...
  if (EllipseAngle > 1.0f) {
    ...
    m_swingAxis = b2Axis1.cross(b1Axis2 * b2Axis1.dot(b1Axis2) +
                                b1Axis3 * b2Axis1.dot(b1Axis3));
  ...
  if (m_twistSpan >= btScalar(0.)) {
    btVector3 b2Axis2 = getRigidBodyB().getCenterOfMassTransform().getBasis() *
                        this->m_rbBFrame.getBasis().getColumn(1);
    btQuaternion rotationArc = shortestArcQuat(b2Axis1, b1Axis1);
    btVector3 TwistRef = quatRotate(rotationArc, b2Axis2);
    btScalar twist = btAtan2Fast(TwistRef.dot(b1Axis3), TwistRef.dot(b1Axis2));
    m_twistAngle = twist;

    //		btScalar lockedFreeFactor = (m_twistSpan > btScalar(0.05f)) ?
    //m_limitSoftness : btScalar(0.);
    btScalar lockedFreeFactor =
        (m_twistSpan > btScalar(0.05f)) ? btScalar(1.0f) : btScalar(0.);
...
void btConeTwistConstraint::calcAngleInfo2(
    ...
    const btMatrix3x3& invInertiaWorldA, const btMatrix3x3& invInertiaWorldB) {
  m_swingCorrection = btScalar(0.);
  m_twistLimitSign = btScalar(0.);
  m_solveTwistLimit = false;
  m_solveSwingLimit = false;
  ...
  if (m_bMotorEnabled &&
      (!m_useSolveConstraintObsolete)) {  // it is assumed that setMotorTarget()
                                          // was alredy called
...

  {
    ...
    // (all this is done from B's perspective. Maybe I should be averaging
    // axes...)
    ...
    if (m_swingSpan1 >= m_fixThresh && m_swingSpan2 >= m_fixThresh) {
      ...
      // or you're trying to set at least one of the swing limits too small. (if
      // so, do you really want a conetwist constraint?)
      ...
      if ((m_swingSpan1 < m_fixThresh) &&
          (m_swingSpan2 < m_fixThresh)) {  // fixed. We'll need to add one more
                                           // row to constraint
        ...
        if (m_swingSpan1 < m_fixThresh) {  // hinge around Y axis
          //					if(!(btFuzzyZero(y)))
        ...
        if (!btFuzzyZero(m_swingCorrection)) m_swingAxis.normalize();
      }
    }
    ...
    if (m_twistSpan >= btScalar(0.f)) {
      ...
      if (m_solveSwingLimit) m_twistAxisA = quatRotate(qA, -twistAxis);
    } else {
...
// given a cone rotation in constraint space, (pre: twist must already be
// removed)
// this method computes its corresponding swing angle and axis.
// more interestingly, it computes the cone/swing limit (angle) for this cone
// "pose".
                                                 ...
                                                 btScalar& swingLimit)   // out
{
  ...
  if (swingAngle > SIMD_EPSILON) {
    ...
    // Given a swing axis, we're looking for the intersection with the bounding
    // cone ellipse.
    // (Since we're dealing with angles, this ellipse is embedded on the surface
    // of a sphere.)

    // For starters, compute the direction from center to surface of ellipse.
    // This is just the perpendicular (ie. rotate 2D vector by PI/2) of the
    // swing axis.
    // (vSwingAxis is the cone rotation (in z,y); change vars and rotate to
    // (x,y) coords.)
    btScalar xEllipse = vSwingAxis.y();
    btScalar yEllipse = -vSwingAxis.z();

    // Now, we use the slope of the vector (using x/yEllipse) and find the
    // length
    // of the line that intersects the ellipse:
    //  x^2   y^2
    //  --- + --- = 1, where a and b are semi-major axes 2 and 1 respectively
    //  (ie. the limits)
    //  a^2   b^2
    // Do the math and it should be clear.

    swingLimit = m_swingSpan1;  // if xEllipse == 0, we have a pure vSwingAxis.z
                                // rotation: just use swingspan1
...
btVector3 btConeTwistConstraint::GetPointForAngle(btScalar fAngleInRadians,
  ...
  // of the line that intersects the ellipse:
  //  x^2   y^2
  //  --- + --- = 1, where a and b are semi-major axes 2 and 1 respectively (ie.
  //  the limits)
  //  a^2   b^2
  // Do the math and it should be clear.
  ...
  // note: twist is x-axis, swing 1 and 2 are along the z and y axes
  // respectively
...
// given a twist rotation in constraint space, (pre: cone must already be
// removed)
...
// this should never happen
  ...
  if (twistAngle > SIMD_EPSILON) vTwistAxis.normalize();
}

void btConeTwistConstraint::adjustSwingAxisToUseEllipseNormal(
    btVector3& vSwingAxis) const {
  // the swing axis is computed as the "twist-free" cone rotation,
  // but the cone limit is not circular, but elliptical (if swingspan1 !=
  // swingspan2).
  // so, if we're outside the limits, the closest way back inside the cone isn't
  // along the vector back to the center. better (and more stable) to use the
  // ellipse normal.
  ...
  // do the math...
  if (fabs(z) > SIMD_EPSILON)  // avoid division by 0. and we don't need an
                               // update if z == 0.
  {
...
void btConeTwistConstraint::setMotorTarget(const btQuaternion& q) {
  ...
  //	btTransform trConstraintCur = (trBCur * m_rbBFrame).inverse() * (trACur
  //* m_rbAFrame);
  // btQuaternion qConstraintCur = trConstraintCur.getRotation();

  btQuaternion qConstraint =
      m_rbBFrame.getRotation().inverse() * q * m_rbAFrame.getRotation();
  setMotorTargetInConstraintSpace(qConstraint);
}

void btConeTwistConstraint::setMotorTargetInConstraintSpace(
