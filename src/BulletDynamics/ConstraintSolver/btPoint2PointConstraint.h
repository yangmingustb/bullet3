...
btPoint2PointConstraint : public btTypedConstraint {
 ...
 public:
  ...
  void getInfo2NonVirtual(btConstraintInfo2 * info,
  ...
  void setPivotB(const btVector3& pivotB) { m_pivotInB = pivotB; }
...
SIMD_FORCE_INLINE int btPoint2PointConstraint::calculateSerializeBufferSize()
    const {
  return sizeof(btPoint2PointConstraintData2);
}

/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btPoint2PointConstraint::serialize(
    void* dataBuffer, btSerializer* serializer) const {
  btPoint2PointConstraintData2* p2pData =
      (btPoint2PointConstraintData2*)dataBuffer;
