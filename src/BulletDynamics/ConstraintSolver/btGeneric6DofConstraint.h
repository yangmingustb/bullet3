...
class btRotationalLimitMotor {
  ...
  */
  ...
  btScalar solveAngularLimits(btScalar timeStep, btVector3& axis,
                              btScalar jacDiagABInv, btRigidBody* body0,
                              btRigidBody* body1);
};
...
/// btGeneric6DofConstraint between two rigidbodies each with a pivotpoint that
...
<ul>
<li> For Linear limits, use btGeneric6DofConstraint.setLinearUpperLimit,
btGeneric6DofConstraint.setLinearLowerLimit. You can set the parameters with the
btTranslationalLimitMotor structure accsesible through the
...
btGeneric6DofConstraint : public btTypedConstraint {
 ...
 protected:
  ...
  btVector3 m_AnchorPos;  // point betwen pivots of bodies A and B to solve
                          // linear axes
 ...
 public:
  ...
  \sa btGeneric6DofConstraint.getCalculatedTransformA ,
...
SIMD_FORCE_INLINE int btGeneric6DofConstraint::calculateSerializeBufferSize()
    const {
  return sizeof(btGeneric6DofConstraintData2);
}

/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btGeneric6DofConstraint::serialize(
