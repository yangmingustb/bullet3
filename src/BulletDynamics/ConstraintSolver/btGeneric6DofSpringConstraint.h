...
/// Generic 6 DOF constraint that allows to set spring motors to any
...
/// 1 : translation Y
/// 2 : translation Z
/// 3 : rotation X (3rd Euler rotational around new position of X axis, range
/// [-PI+epsilon, PI-epsilon] )
/// 4 : rotation Y (2nd Euler rotational around new position of Y axis, range
/// [-PI/2+epsilon, PI/2-epsilon] )
/// 5 : rotation Z (1st Euler rotational around Z axis, range [-PI+epsilon,
/// PI-epsilon] )
...
btGeneric6DofSpringConstraint : public btGeneric6DofConstraint {
 ...
 public:
  ...
  btGeneric6DofSpringConstraint(btRigidBody & rbB, const btTransform& frameInB,
  ...
  bool isSpringEnabled(int index) const { return m_springEnabled[index]; }

  btScalar getStiffness(int index) const { return m_springStiffness[index]; }

  btScalar getDamping(int index) const { return m_springDamping[index]; }
...
/// fills the dataBuffer and returns the struct name (and 0 on failure)
SIMD_FORCE_INLINE const char* btGeneric6DofSpringConstraint::serialize(
    void* dataBuffer, btSerializer* serializer) const {
  btGeneric6DofSpringConstraintData2* dof =
      (btGeneric6DofSpringConstraintData2*)dataBuffer;
