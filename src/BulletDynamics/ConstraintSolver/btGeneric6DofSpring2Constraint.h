...
btGeneric6DofSpring2Constraint : public btTypedConstraint {
 ...
 protected:
  ...
  int setAngularLimits(btConstraintInfo2 * info, int row_offset,
                       ...
                       const btVector3& linVelA, const btVector3& linVelB,
                       const btVector3& angVelA, const btVector3& angVelB);
  int setLinearLimits(btConstraintInfo2 * info, int row,
                      const btTransform& transA, const btTransform& transB,
                      const btVector3& linVelA, const btVector3& linVelB,
                      const btVector3& angVelA, const btVector3& angVelB);
  ...
  void calculateJacobi(btRotationalLimitMotor2 * limot,
                       const btTransform& transA, const btTransform& transB,
                       btConstraintInfo2* info, int srow, btVector3& ax1,
  ...
  int get_limit_motor_info2(
      ...
      const btVector3& angVelB, btConstraintInfo2* info, int row,
      btVector3& ax1, int rotational, int rotAllowed = false);
 ...
 public:
  ...
  btGeneric6DofSpring2Constraint(
      ...
      const btTransform& frameInB, RotateOrder rotOrder = RO_XYZ);
  btGeneric6DofSpring2Constraint(btRigidBody & rbB, const btTransform& frameInB,
  ...
  virtual const char* serialize(void* dataBuffer, btSerializer* serializer)
      const;
  ...
  // Calculates the global transform for the joint offset for body A an B, and
  // also calculates the angle differences between the bodies.
  ...
  // btGeneric6DofSpring2Constraint::calculateTransforms() must be called
  // previously )
  ...
  void getAngularLowerLimit(btVector3 & angularLower) {
    for (int i = 0; i < 3; i++) angularLower[i] = m_angularLimits[i].m_loLimit;
  }

  void getAngularLowerLimitReversed(btVector3 & angularLower) {
    for (int i = 0; i < 3; i++) angularLower[i] = -m_angularLimits[i].m_hiLimit;
  }
  ...
  void getAngularUpperLimit(btVector3 & angularUpper) {
    for (int i = 0; i < 3; i++) angularUpper[i] = m_angularLimits[i].m_hiLimit;
  }

  void getAngularUpperLimitReversed(btVector3 & angularUpper) {
    for (int i = 0; i < 3; i++) angularUpper[i] = -m_angularLimits[i].m_loLimit;
  }
  ...
  void setServo(int index, bool onOff);  // set the type of the motor (servo or
                                         // not) (the motor has to be turned on
                                         // for servo also)
  ...
  void setStiffness(int index, btScalar stiffness,
                    bool limitIfNeeded =
                        true);  // if limitIfNeeded is true the system will
                                // automatically limit the stiffness in
                                // necessary situations where otherwise the
                                // spring would move unrealistically too widely
  void setDamping(int index, btScalar damping,
                  bool limitIfNeeded = true);  // if limitIfNeeded is true the
                                               // system will automatically
                                               // limit the damping in necessary
                                               // situations where otherwise the
                                               // spring would blow up
...
btGeneric6DofSpring2Constraint::calculateSerializeBufferSize() const {
  return sizeof(btGeneric6DofSpring2ConstraintData2);
}

SIMD_FORCE_INLINE const char* btGeneric6DofSpring2Constraint::serialize(
    void* dataBuffer, btSerializer* serializer) const {
  btGeneric6DofSpring2ConstraintData2* dof =
      (btGeneric6DofSpring2ConstraintData2*)dataBuffer;
  ...
  for (i = 0; i < 3; i++) {
    ...
    dof->m_angularTargetVelocity.m_floats[i] =
        m_angularLimits[i].m_targetVelocity;
    dof->m_angularMaxMotorForce.m_floats[i] =
        m_angularLimits[i].m_maxMotorForce;
    dof->m_angularServoTarget.m_floats[i] = m_angularLimits[i].m_servoTarget;
    dof->m_angularSpringStiffness.m_floats[i] =
        m_angularLimits[i].m_springStiffness;
    dof->m_angularSpringDamping.m_floats[i] =
        m_angularLimits[i].m_springDamping;
    dof->m_angularEquilibriumPoint.m_floats[i] =
        m_angularLimits[i].m_equilibriumPoint;
  }
  ...
  for (i = 0; i < 4; i++) {
    dof->m_angularEnableMotor[i] =
        i < 3 ? (m_angularLimits[i].m_enableMotor ? 1 : 0) : 0;
    dof->m_angularServoMotor[i] =
        i < 3 ? (m_angularLimits[i].m_servoMotor ? 1 : 0) : 0;
    dof->m_angularEnableSpring[i] =
        i < 3 ? (m_angularLimits[i].m_enableSpring ? 1 : 0) : 0;
    dof->m_angularSpringStiffnessLimited[i] =
        i < 3 ? (m_angularLimits[i].m_springStiffnessLimited ? 1 : 0) : 0;
    dof->m_angularSpringDampingLimited[i] =
        i < 3 ? (m_angularLimits[i].m_springDampingLimited ? 1 : 0) : 0;
  }
  ...
  for (i = 0; i < 4; i++) {
    dof->m_linearEnableMotor[i] =
        i < 3 ? (m_linearLimits.m_enableMotor[i] ? 1 : 0) : 0;
    dof->m_linearServoMotor[i] =
        i < 3 ? (m_linearLimits.m_servoMotor[i] ? 1 : 0) : 0;
    dof->m_linearEnableSpring[i] =
        i < 3 ? (m_linearLimits.m_enableSpring[i] ? 1 : 0) : 0;
    dof->m_linearSpringStiffnessLimited[i] =
        i < 3 ? (m_linearLimits.m_springStiffnessLimited[i] ? 1 : 0) : 0;
    dof->m_linearSpringDampingLimited[i] =
        i < 3 ? (m_linearLimits.m_springDampingLimited[i] ? 1 : 0) : 0;
  }
