...
SIMD_FORCE_INLINE int calcBatchCost(int bodies, int manifolds,
                                    int constraints) {
...
SIMD_FORCE_INLINE int calcBatchCost(
    const btSimulationIslandManagerMt::Island* island) {
  return calcBatchCost(island->bodyArray.size(), island->manifoldArray.size(),
...
inline int getIslandId(const btPersistentManifold* lhs) {
  const btCollisionObject* rcolObj0 =
      static_cast<const btCollisionObject*>(lhs->getBody0());
  const btCollisionObject* rcolObj1 =
      static_cast<const btCollisionObject*>(lhs->getBody1());
  int islandId = rcolObj0->getIslandTag() >= 0 ? rcolObj0->getIslandTag()
...
class IslandBatchSizeSortPredicate {
 public:
  bool operator()(const btSimulationIslandManagerMt::Island* lhs,
                  const btSimulationIslandManagerMt::Island* rhs) const {
...
void btSimulationIslandManagerMt::initIslandPools() {
  ...
  m_activeIslands.resize(0);
  m_freeIslands.resize(0);
  // check whether allocated islands are sorted by body capacity (largest to
  // smallest)
  int lastCapacity = 0;
  bool isSorted = true;
  for (int i = 0; i < m_allocatedIslands.size(); ++i) {
...
btSimulationIslandManagerMt::Island* btSimulationIslandManagerMt::getIsland(
    int id) {
...
btSimulationIslandManagerMt::allocateIsland(int id, int numBodies) {
  Island* island = NULL;
...
void btSimulationIslandManagerMt::buildIslands(
  ...
  // we are going to sort the unionfind array, and store the element id in the
  // size
  ...
  for (startIslandIndex = 0; startIslandIndex < numElem;
    ...
    for (idx = startIslandIndex; idx < endIslandIndex; idx++) {
      ...
      if ((colObj0->getIslandTag() != islandId) &&
          (colObj0->getIslandTag() != -1)) {
        //				printf("error in island management\n");
      }

      btAssert((colObj0->getIslandTag() == islandId) ||
               (colObj0->getIslandTag() == -1));
      if (colObj0->getIslandTag() == islandId) {
...
void btSimulationIslandManagerMt::addBodiesToIslands(
  ...
  for (startIslandIndex = 0; startIslandIndex < numElem;
    ...
    if (!islandSleeping) {
      // want to count the number of bodies before allocating the island to
      // optimize memory usage of the Island structures
...
void btSimulationIslandManagerMt::addManifoldsToIslands(
    btDispatcher* dispatcher) {
  // walk all the manifolds, activating bodies touched by kinematic objects, and
  // add each manifold to its Island
...
void btSimulationIslandManagerMt::addConstraintsToIslands(
...
void btSimulationIslandManagerMt::mergeIslands() {
  ...
  while (destIslandIndex < lastIndex) {
    ...
    // figure out how many islands we want to merge and find out how many
    // bodies, manifolds and constraints we will have
    while (true) {
...
void btSimulationIslandManagerMt::solveIsland(
    btConstraintSolver* solver, Island& island,
    const SolverParams& solverParams) {
  btPersistentManifold** manifolds =
      island.manifoldArray.size() ? &island.manifoldArray[0] : NULL;
  btTypedConstraint** constraintsPtr =
      island.constraintArray.size() ? &island.constraintArray[0] : NULL;
  solver->solveGroup(&island.bodyArray[0], island.bodyArray.size(), manifolds,
                     island.manifoldArray.size(), constraintsPtr,
                     island.constraintArray.size(), *solverParams.m_solverInfo,
                     solverParams.m_debugDrawer, solverParams.m_dispatcher);
}

void btSimulationIslandManagerMt::serialIslandDispatch(
    btAlignedObjectArray<Island*>* islandsPtr,
    const SolverParams& solverParams) {
  BT_PROFILE("serialIslandDispatch");
  // serial dispatch
  btAlignedObjectArray<Island*>& islands = *islandsPtr;
  btConstraintSolver* solver = solverParams.m_solverMt
                                   ? solverParams.m_solverMt
                                   : solverParams.m_solverPool;
...
void btSimulationIslandManagerMt::parallelIslandDispatch(
    btAlignedObjectArray<Island*>* islandsPtr,
    const SolverParams& solverParams) {
  BT_PROFILE("parallelIslandDispatch");
  //
  // if there are islands with many contacts, it may be faster to submit these
  // large islands *serially* to a single parallel constraint solver, and then
  // later
  // submit the remaining smaller islands in parallel to multiple sequential
  // solvers.
  //
  // Some task schedulers do not deal well with nested parallelFor loops. One
  // implementation
  // of OpenMP was actually slower than doing everything single-threaded. Intel
  // TBB
  // on the other hand, seems to do a pretty respectable job with it.
  //
  // When solving islands in parallel, the worst case performance happens when
  // there
  // is one very large island and then perhaps a smattering of very small
  // islands -- one worker thread takes the large island and the remaining
  // workers
  // tear through the smaller islands and then sit idle waiting for the first
  // worker
  // to finish. Solving islands in parallel works best when there are numerous
  // small
  // islands, roughly equal in size.
  //
  // By contrast, the other approach -- the parallel constraint solver -- is
  // only
  // able to deliver a worthwhile speedup when the island is large. For smaller
  // islands,
  // it is difficult to extract a useful amount of parallelism -- the overhead
  // of grouping
  // the constraints into batches and sending the batches to worker threads can
  // nullify
  // any gains from parallelism.
  //

  UpdateIslandDispatcher dispatcher(*islandsPtr, solverParams);
  // We take advantage of the fact the islands are sorted in order of decreasing
  // size
  int iBegin = 0;
  if (solverParams.m_solverMt) {
    while (iBegin < islandsPtr->size()) {
      btSimulationIslandManagerMt::Island* island = (*islandsPtr)[iBegin];
      if (island->manifoldArray.size() <
...
void btSimulationIslandManagerMt::buildAndProcessIslands(
    btDispatcher* dispatcher, btCollisionWorld* collisionWorld,
    btAlignedObjectArray<btTypedConstraint*>& constraints,
    const SolverParams& solverParams) {
  ...
  if (!getSplitIslands()) {
    ...
    btTypedConstraint** constraintsPtr =
        constraints.size() ? &constraints[0] : NULL;
    btConstraintSolver* solver = solverParams.m_solverMt
                                     ? solverParams.m_solverMt
                                     : solverParams.m_solverPool;
    solver->solveGroup(&collisionObjects[0], collisionObjects.size(), manifolds,
                       maxNumManifolds, constraintsPtr, constraints.size(),
                       *solverParams.m_solverInfo, solverParams.m_debugDrawer,
                       solverParams.m_dispatcher);
  } else {
    initIslandPools();

    // traverse the simulation islands, and call the solver, unless all objects
    // are sleeping/deactivated
    addBodiesToIslands(collisionWorld);
    addManifoldsToIslands(dispatcher);
    addConstraintsToIslands(constraints);

    // m_activeIslands array should now contain all non-sleeping Islands, and
    // each Island should
