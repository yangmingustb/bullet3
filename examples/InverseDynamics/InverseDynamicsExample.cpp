...
static btVector4 sJointCurveColors[8] = {
    btVector4(1, 0.3, 0.3, 1), btVector4(0.3, 1, 0.3, 1),
    btVector4(0.3, 0.3, 1, 1), btVector4(0.3, 1, 1, 1),
    btVector4(1, 0.3, 1, 1),   btVector4(1, 1, 0.3, 1),
    btVector4(1, 0.7, 0.7, 1), btVector4(0.7, 1, 1, 1),
...
class InverseDynamicsExample : public CommonMultiBodyBase {
 ...
 public:
  InverseDynamicsExample(struct GUIHelperInterface* helper,
                         btInverseDynamicsExampleOptions option);
...
InverseDynamicsExample::InverseDynamicsExample(
    struct GUIHelperInterface* helper, btInverseDynamicsExampleOptions option)
...
// implementation. Will create a separate header/source file for this.
  ...
  switch (m_option) {
    case BT_ID_LOAD_URDF: {
      ...
      if (loadOk) {
        ...
        ConvertURDF2Bullet(u2b, creation, identityTrans, m_dynamicsWorld, true,
                           u2b.getPathPrefix());
        ...
        if (m_multiBody) {
          // kuka without joint control/constraints will gain energy explode
          // soon due to timestep/integrator
          // temporarily set some extreme damping factors until we have some
          // joint control or constraints
          m_multiBody->setAngularDamping(0 * 0.99);
          m_multiBody->setLinearDamping(0 * 0.99);
          b3Printf("Root link name = %s",
                   u2b.getLinkName(u2b.getRootLinkIndex()).c_str());
        }
      }
      break;
    }
    case BT_ID_PROGRAMMATICALLY: {
      ...
      m_multiBody = createInvertedPendulumMultiBody(
          m_dynamicsWorld, m_guiHelper, baseWorldTrans, false);
      break;
    }
    default: {
  ...
  if (m_multiBody) {
    {
      if (m_guiHelper->getAppInterface() &&
        ...
        m_timeSeriesCanvas = new TimeSeriesCanvas(
            m_guiHelper->getAppInterface()->m_2dCanvasInterface, 512, 230,
    ...
    if (m_timeSeriesCanvas && m_guiHelper->getParameterInterface()) {
      for (std::size_t dof = 0; dof < qd.size(); dof++) {
        ...
        m_timeSeriesCanvas->addDataSource(q_name[dof].c_str(), color[0] * 255,
...
void InverseDynamicsExample::stepSimulation(float deltaTime) {
  if (m_multiBody) {
    ...
    btInverseDynamics::vecx nu(num_dofs), qdot(num_dofs), q(num_dofs),
        joint_force(num_dofs);
    ...
    if (useInverseModel) {
      ...
      if (m_multiBody->hasFixedBase()) {
        if (-1 !=
            m_inverseModel->calculateInverseDynamics(q, qdot, nu,
                                                     &joint_force)) {
        ...
        // the inverse dynamics model represents the 6 DOFs of the base, unlike
        // btMultiBody.
        // append some dummy values to represent the 6 DOFs of the base
        btInverseDynamics::vecx nu6(num_dofs + 6), qdot6(num_dofs + 6),
            q6(num_dofs + 6), joint_force6(num_dofs + 6);
        ...
        if (-1 !=
            m_inverseModel->calculateInverseDynamics(q6, qdot6, nu6,
                                                     &joint_force6)) {
          // joint_force(dof) += damping*dot_q(dof);
          // use inverse model: apply joint force corresponding to
          // desired acceleration nu
  ...
  if (m_timeSeriesCanvas) m_timeSeriesCanvas->nextTick();
...
CommonExampleInterface* InverseDynamicsExampleCreateFunc(
    CommonExampleOptions& options) {
  return new InverseDynamicsExample(
      options.m_guiHelper, btInverseDynamicsExampleOptions(options.m_option));
}
