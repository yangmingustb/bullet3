...
/// September 2006: VehicleDemo is work in progress, this file is mostly just a
/// placeholder
/// This VehicleDemo file is very early in development, please check it later
///@todo is a basic engine model:
/// A function that maps user input (throttle) into torque/force applied on the
/// wheels
/// with gears etc.
...
// the sequential impulse solver has difficulties dealing with large mass ratios
// (differences), between loadMass and the fork parts
...
/// btRaycastVehicle is the interface for the constraint that implements the
/// raycast vehicle
/// notice that for higher-quality slow-moving vehicles, another approach might
/// be better
/// implementing explicit hinged-wheel constraints with cylinder collision,
/// rather then raycasts
...
void ForkLiftDemo::initPhysics() {
  ...
  m_wheelShape =
      new btCylinderShapeX(btVector3(wheelWidth, wheelRadius, wheelRadius));
...

  {
    ...
    m_liftHinge =
        new btHingeConstraint(*m_carChassis, *m_liftBody, localA, localB);
    ...
    m_forkSlider =
        new btSliderConstraint(*m_liftBody, *m_forkBody, localA, localB, true);
  ...
  /// create vehicle
  {
    ...
    m_vehicle =
        new btRaycastVehicle(m_tuning, m_carChassis, m_vehicleRayCaster);
    ...
    m_vehicle->addWheel(connectionPointCS0, wheelDirectionCS0, wheelAxleCS,
                        suspensionRestLength, wheelRadius, m_tuning,
                        isFrontWheel);
    connectionPointCS0 =
        btVector3(-CUBE_HALF_EXTENTS + (0.3 * wheelWidth), connectionHeight,
                  2 * CUBE_HALF_EXTENTS - wheelRadius);
...
void ForkLiftDemo::renderScene() {
  ...
  for (int i = 0; i < m_vehicle->getNumWheels(); i++) {
    ...
    if (renderer) {
      ...
      renderer->writeSingleInstanceTransformToCPU(pos, orn,
                                                  m_wheelInstances[i]);
    }
  }
...
void ForkLiftDemo::stepSimulation(float deltaTime) {
  ...
  if (m_dynamicsWorld) {
    ...
    if (m_dynamicsWorld->getConstraintSolver()->getSolverType() ==
        BT_MLCP_SOLVER) {
      ...
      if (numFallbacks) {
        ...
        printf(
            "MLCP solver failed %d times, falling back to "
            "btSequentialImpulseSolver (SI)\n",
            totalFailures);
      }
...
bool ForkLiftDemo::keyboardCallback(int key, int state) {
  bool handled = false;
  bool isShiftPressed =
      m_guiHelper->getAppInterface()->m_window->isModifierKeyPressed(B3G_SHIFT);
    ...
    if (isShiftPressed) {
      ...
      switch (key) {
        ...
        case B3G_F7: {
          ...
          world->setLatencyMotionStateInterpolation(
        ...
        case B3G_F6: {
          ...
          if (useMCLPSolver) {
            btDantzigSolver* mlcp = new btDantzigSolver();
            // btSolveProjectedGaussSeidel* mlcp = new
...
void ForkLiftDemo::lockLiftHinge(void) {
  ...
  if (hingeAngle < lowLim) {
    ...
    //		m_liftHinge->setLimit(hingeAngle - LIFT_EPS, hingeAngle +
    //LIFT_EPS);
...
btRigidBody* ForkLiftDemo::localCreateRigidBody(
    btScalar mass, const btTransform& startTransform, btCollisionShape* shape) {
...
CommonExampleInterface* ForkLiftCreateFunc(
