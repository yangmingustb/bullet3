...
// TODO: This would actually be an Integer, but the Slider does not like
// integers, so I floor it when changed
...
static btScalar gPendulaRestitution =
    1;  // pendula restitution when hitting against each other
...
static btScalar gDisplacementForce =
    30;  // default force to displace the pendula
...
struct NewtonsCradleExample : public CommonRigidBodyBase {
  ...
  virtual void createPendulum(btSphereShape* colShape,
...
void onPendulaLengthChanged(float pendulaLength,
                            void* userPtr);  // Change the pendula length

void onPendulaRestitutionChanged(
    float pendulaRestitution, void* userPtr);  // change the pendula restitution
...
void NewtonsCradleExample::initPhysics() {
  ...
  {  // create the pendula starting at the indicated position below and where
     // each pendulum has the following mass
    btScalar pendulumMass(1.f);

    btVector3 position(0.0f, 15.0f,
                       0.0f);  // initial left-most pendulum position
    ...
    for (int i = 0; i < std::floor(gPendulaQty); i++) {
      // create pendulum
      createPendulum(pendulumShape, position, gInitialPendulumLength,
                     pendulumMass);

      // displace the pendula 1.05 sphere size, so that they all nearly touch
      // (small spacings in between
...
void NewtonsCradleExample::createPendulum(btSphereShape* colShape,
  ...
  startTransform.setOrigin(
      btVector3(position.x(), btScalar(position.y() - length), position.z()));
  ...
  // make the top sphere position "fixed" to the world by attaching with a point
  // to point constraint
  // The pivot is defined in the reference frame of topSphere, so the attachment
  // is exactly at the center of the topSphere
  ...
  // Obtain the position of topSphere in local reference frame of bottomSphere
  // (the pivot is therefore in the center of topSphere)
  ...
  btSliderConstraint* sliderConst = new btSliderConstraint(
      *topSphere, *bottomSphere, constraintPivotInTopSphereRF,
      constraintPivotInBottomSphereRF, true);
...
void NewtonsCradleExample::renderScene() { CommonRigidBodyBase::renderScene(); }
