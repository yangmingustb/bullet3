...
void BridgeExample::initPhysics() {
...

  {
    ...
    btScalar bridgeWidth =
        plankWidth * TOTAL_PLANKS + plankOffset * (TOTAL_PLANKS - 1);
    ...
    for (int i = 0; i < TOTAL_PLANKS; ++i) {
      ...
      startTransform.setOrigin(
          btVector3(btScalar(t), bridgeHeight, btScalar(0)));
      boxes.push_back(createRigidBody((i == 0 || i == lastBoxIndex) ? 0 : mass,
                                      startTransform, colShape));
    }
    ...
    for (int i = 0; i < TOTAL_PLANKS - 1; ++i) {
      ...
      btPoint2PointConstraint* leftSpring = new btPoint2PointConstraint(
          *b1, *b2, btVector3(-0.5, 0, -0.5), btVector3(0.5, 0, -0.5));
      m_dynamicsWorld->addConstraint(leftSpring);

      btPoint2PointConstraint* rightSpring = new btPoint2PointConstraint(
          *b1, *b2, btVector3(-0.5, 0, 0.5), btVector3(0.5, 0, 0.5));
