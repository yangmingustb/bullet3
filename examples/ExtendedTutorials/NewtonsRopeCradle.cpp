...
static btScalar gPendulaRestitution =
    1;  // pendula restition when hitting against each other
...
static btScalar gRopeResolution =
    1;  // default rope resolution (number of links as in a chain)
...
struct NewtonsRopeCradleExample : public CommonRigidBodyBase {
  ...
  virtual void createRopePendulum(btSphereShape* colShape,
                                  ...
                                  const btQuaternion& pendulumOrientation,
                                  btScalar width, btScalar height,
                                  btScalar mass);
...
void NewtonsRopeCradleExample::initPhysics() {
  ...
  {  // create the pendula starting at the indicated position below and where
    ...
    for (int i = 0; i < std::floor(gPendulaQty); i++) {
      // create pendulum
      createRopePendulum(pendulumShape, position, orientation,
                         gInitialPendulumWidth, gInitialPendulumHeight,
                         pendulumMass);
...
void NewtonsRopeCradleExample::connectWithRope(btRigidBody* body1,
                                               btRigidBody* body2) {
  btSoftBody* softBodyRope0 = btSoftBodyHelpers::CreateRope(
      softBodyWorldInfo, body1->getWorldTransform().getOrigin(),
...
void NewtonsRopeCradleExample::createRopePendulum(
    btSphereShape* colShape, const btVector3& position,
    const btQuaternion& pendulumOrientation, btScalar width, btScalar height,
    btScalar mass) {
  ...
  startTransform.setOrigin(startTransform *
                           topSphere1RelPosition);  // rotate this position
  startTransform.setOrigin(
      position +
      startTransform
          .getOrigin());  // add non-rotated position to the relative position
  btRigidBody* topSphere1 =
      createRigidBody(0, startTransform, colShape);  // make top sphere static

  // position the top sphere above ground with appropriate orientation
  startTransform.setOrigin(btVector3(0, 0, 0));     // no translation intitially
  startTransform.setRotation(pendulumOrientation);  // pendulum rotation
  startTransform.setOrigin(startTransform *
                           topSphere2RelPosition);  // rotate this position
  startTransform.setOrigin(
      position +
      startTransform
          .getOrigin());  // add non-rotated position to the relative position
  btRigidBody* topSphere2 =
      createRigidBody(0, startTransform, colShape);  // make top sphere static
  ...
  startTransform.setRotation(pendulumOrientation);  // pendulum rotation
  startTransform.setOrigin(startTransform *
                           bottomSphereRelPosition);  // rotate this position
  startTransform.setOrigin(
      position +
      startTransform
          .getOrigin());  // add non-rotated position to the relative position
