...
static btScalar gRampRestitution =
    0;  // set ramp restitution to 0 (no restitution)
...
static btScalar gSphereSpinningFriction =
    0.3;  // set sphere spinning friction to 0.3
...
void InclinedPlaneExample::initPhysics() {
  ...
  {  // create a static ground
    btBoxShape* groundShape =
        createBoxShape(btVector3(btScalar(50.), btScalar(50.), btScalar(50.)));
  ...
  {  // create a static inclined plane
    btBoxShape* inclinedPlaneShape =
        createBoxShape(btVector3(btScalar(20.), btScalar(1.), btScalar(10.)));
    ...
    startTransform.setOrigin(btVector3(btScalar(0), btScalar(15), btScalar(0)));
  ...
  {  // create a cube above the inclined plane
    ...
    startTransform.setOrigin(btVector3(btScalar(0), btScalar(20), btScalar(2)));

    gBox = createRigidBody(boxMass, startTransform, boxShape);
    gBox->forceActivationState(
        DISABLE_DEACTIVATION);  // to prevent the box on the ramp from disabling
  ...
  {  // create a sphere above the inclined plane
    ...
    startTransform.setOrigin(btVector3(btScalar(0), btScalar(20), btScalar(4)));

    gSphere = createRigidBody(sphereMass, startTransform, sphereShape);
    gSphere->forceActivationState(DISABLE_DEACTIVATION);  // to prevent the
                                                          // sphere on the ramp
                                                          // from disabling
...
void InclinedPlaneExample::renderScene() { CommonRigidBodyBase::renderScene(); }
...
CommonExampleInterface* ET_InclinedPlaneCreateFunc(
