...
static btScalar gPendulaQty = 2;  // TODO: This would actually be an Integer,
                                  // but the Slider does not like integers, so I
                                  // floor it when changed

static btScalar gDisplacedPendula = 1;  // TODO: This is an int as well

static btScalar gPendulaRestitution =
    1;  // Default pendulum restitution is 1 to restore all force

static btScalar gSphereRadius = 1;  // The sphere radius

static btScalar gCurrentPendulumLength = 8;

static btScalar gInitialPendulumLength =
    8;  // Default pendulum length (distance between two spheres)

static btScalar gDisplacementForce =
    30;  // The default force with which we move the pendulum

static btScalar gForceScalar =
    0;  // default force scalar to apply a displacement
...
struct MultiPendulumExample : public CommonRigidBodyBase {
  ...
  virtual void createMultiPendulum(btSphereShape* colShape, btScalar pendulaQty,
                                   const btVector3& position, btScalar length,
                                   btScalar mass);  // create a multi pendulum
                                                    // at the indicated x and y
                                                    // position, the specified
                                                    // number of pendula formed
                                                    // into a chain, each with
                                                    // indicated length and mass
  virtual void changePendulaLength(
      btScalar length);  // change the pendulum length
  virtual void changePendulaRestitution(
      btScalar restitution);  // change the pendula restitution
  virtual void stepSimulation(float deltaTime);  // step the simulation
  virtual bool keyboardCallback(int key,
                                int state);  // handle keyboard callbacks
  ...
  std::vector<btSliderConstraint*>
      constraints;                    // keep a handle to the slider constraints
  std::vector<btRigidBody*> pendula;  // keep a handle to the pendula
};

static MultiPendulumExample* mex = NULL;  // Handle to the example to access it
                                          // via functions. Do not use this in
                                          // your simulation!

void onMultiPendulaLengthChanged(float pendulaLength,
                                 void*);  // Change the pendula length

void onMultiPendulaRestitutionChanged(float pendulaRestitution,
                                      void*);  // change the pendula restitution
...
void MultiPendulumExample::initPhysics() {  // Setup your physics scene
  ...
  {  // create the multipendulum starting at the indicated position below and
     // where each pendulum has the following mass
    btScalar pendulumMass(1.f);

    btVector3 position(0.0f, 15.0f,
                       0.0f);  // initial top-most pendulum position
...
void MultiPendulumExample::stepSimulation(float deltaTime) {
  applyMForceWithForceScalar(
      gForceScalar);  // apply force defined by apply force slider
...
void MultiPendulumExample::createMultiPendulum(btSphereShape* colShape,
  ...
  // make top sphere position "fixed" in the world by attaching it to a the
  // world with a point to point constraint
  // The pivot is defined in the reference frame of topSphere, so the attachment
  // should be exactly at the center of topSphere
  btVector3 constraintPivot(0.0f, 0.0f, 0.0f);
  btPoint2PointConstraint* p2pconst =
      new btPoint2PointConstraint(*topSphere, constraintPivot);
  ...
  btRigidBody* parentSphere = topSphere;  // set the top sphere as the parent
                                          // sphere for the next sphere to be
                                          // created
  ...
  for (int i = 0; i < pendulaQty; i++) {  // produce the number of pendula
    ...
    btRigidBody* jointSphere = createRigidBody(mass, startTransform, colShape);
    ...
    // this is represented by the constraint pivot in the local frames of
    // reference of both constrained spheres
    ...
    // the orientation of a point-to-point constraint does not matter, as is has
    // no rotational limits

    // Obtain the position of parentSphere in local reference frame of the
    // jointSphere (the pivot is therefore in the center of parentSphere)
    ...
    btPoint2PointConstraint* p2pconst =
        new btPoint2PointConstraint(*parentSphere, *jointSphere,
                                    constraintPivotInParentSphereRF.getOrigin(),
                                    constraintPivotInJointSphereRF.getOrigin());

    p2pconst->setDbgDrawSize(
        btScalar(5.f));  // set the size of the debug drawing

    // add the constraint to the world
    m_dynamicsWorld->addConstraint(p2pconst, true);

    // create a slider constraint to change the length of the pendula while it
    // swings
    ...
    btRigidBody* childSphere = createRigidBody(mass, startTransform, colShape);
    ...
    // this is represented by the constraint pivot in the local frames of
    // reference of both constrained spheres
    // furthermore we need to rotate the constraint appropriately to orient it
    // correctly in space
    ...
    // Obtain the position of jointSphere in local reference frame of the
    // childSphere (the pivot is therefore in the center of jointSphere)
    ...
    // the slider constraint is x aligned per default, but we want it to be y
    // aligned, therefore we rotate it
    btQuaternion qt;
    ...
    btSliderConstraint* sliderConst = new btSliderConstraint(
        *jointSphere, *childSphere, constraintPivotInJointSphereRF,
        constraintPivotInChildSphereRF, true);

    sliderConst->setDbgDrawSize(
        btScalar(5.f));  // set the size of the debug drawing

    // set limits
    // the initial setup of the constraint defines the origins of the limit
    // dimensions,
    // therefore we set both limits directly to the current position of the
    // parentSphere
...
void MultiPendulumExample::changePendulaLength(btScalar length) {
  ...
  for (std::vector<btSliderConstraint*>::iterator sit = constraints.begin();
    ...
    // if the pendulum is being shortened beyond it's own length, we don't let
    // the lower sphere to go past the upper one
...
void MultiPendulumExample::renderScene() { CommonRigidBodyBase::renderScene(); }
...
bool MultiPendulumExample::keyboardCallback(int key, int state) {
  ...
  switch (key) {
    ...
    case '2' /*ASCII for 2*/: {
      ...
      // is being shortened beyond it's own length, we don't let the lower
      // sphere to go over the upper one
      if (0 <= newLimit) {
        changePendulaLength(newLimit);
        gCurrentPendulumLength = newLimit;
      }
...
void onMultiPendulaRestitutionChanged(
    float pendulaRestitution, void*) {  // change the pendula restitution
  if (mex) {
...
void applyMForceWithForceScalar(float forceScalar) {
  if (mex) {
    ...
    if (fabs(gForceScalar) < 0.2f) gForceScalar = 0;
