...
void b3ReadWavFile::normalize(double peak) {
  ...
  for (i = 0; i < m_frames.size(); i++) {
    if (fabs(m_frames[i]) > max) max = (double)fabs((double)m_frames[i]);
  }

  if (max > 0.0) {
    max = 1.0 / max;
    max *= peak;
    for (i = 0; i < m_frames.size(); i++) m_frames[i] *= max;
  }
}
...
double b3ReadWavFile::interpolate(double frame, unsigned int channel) const {
  ...
  if (alpha > 0.0) output += (alpha * (m_frames[iIndex + channels_] - output));
...
double b3ReadWavFile::tick(unsigned int channel, b3WavTicker *ticker) {
  ...
  if (ticker->time_ < 0.0 ||
      ticker->time_ > (double)(this->m_numFrames - 1.0)) {
...
void b3ReadWavFile::resize() { m_frames.resize(channels_ * m_numFrames); }
...
bool b3ReadWavFile::getWavInfo(const char *fileName) {
  ...
  char header[12];
  if (fread(&header, 4, 3, fd_) != 3) return false;
  bool res = false;
  ...
  char id[4];
  int chunkSize;
  if (fread(&id, 4, 1, fd_) != 1) return false;
  ...
  if (fread(&chunkSize, 4, 1, fd_) != 1) return false;  // Read fmt chunk size.
  if (fread(&format_tag, 2, 1, fd_) != 1) return false;
  if (!m_machineIsLittleEndian) {
  ...
  if (format_tag == 0xFFFE) {  // WAVE_FORMAT_EXTENSIBLE
    ...
    if (fseek(fd_, 14, SEEK_CUR) == -1) return false;
    unsigned short extSize;
    if (fread(&extSize, 2, 1, fd_) != 1) return false;
    if (!m_machineIsLittleEndian) {
      b3Swap16((unsigned char *)&extSize);
    }
    if (extSize == 0) return false;
    if (fseek(fd_, 6, SEEK_CUR) == -1) return false;
    if (fread(&format_tag, 2, 1, fd_) != 1) return false;
    if (!m_machineIsLittleEndian) {
      b3Swap16((unsigned char *)&format_tag);
    }
    if (fseek(fd_, dataOffset_, SEEK_SET) == -1) return false;
  }
  if (format_tag != 1 && format_tag != 3) {  // PCM = 1, FLOAT = 3
    //  oStream_ << "FileRead: "<< fileName << " contains an unsupported data
    //  format type (" << format_tag << ").";
    return false;
  }

  // Get number of channels from the header.
  short int temp;
  if (fread(&temp, 2, 1, fd_) != 1) return false;
  if (!m_machineIsLittleEndian) {
  ...
  int srate;
  if (fread(&srate, 4, 1, fd_) != 1) return false;
  if (!m_machineIsLittleEndian) {
  ...
  if (dataType_ == 0) {
    //   oStream_ << "FileRead: " << temp << " bits per sample with data format
    //   " << format_tag << " are not supported (" << fileName << ").";
    return false;
  }

  // Jump over any remaining part of the "fmt" chunk.
  if (fseek(fd_, chunkSize - 16, SEEK_CUR) == -1) return false;
  ...
  int bytes;
  if (fread(&bytes, 4, 1, fd_) != 1) return false;
  if (!m_machineIsLittleEndian) {
...
bool b3ReadWavFile::read(unsigned long startFrame, bool doNormalize) {
  ...
  if (nFrames == 0) {
    //    oStream_ << "FileRead::read: StkFrames m_frames size is zero ... no
    //    data read!";
  ...
  if (startFrame >= m_numFrames) {
    return false;
    // oStream_ << "FileRead::read: startFrame argument is greater than or equal
    // to the file size!";
    // Stk::handleError( StkError::FUNCTION_ARGUMENT );
  }

  // Check for file end.
  if (startFrame + nFrames > m_numFrames) nFrames = m_numFrames - startFrame;
  ...
  if (dataType_ == B3_SINT16) {
    ...
    if (fseek(fd_, dataOffset_ + (offset * 2), SEEK_SET) == -1) return false;
    if (fread(buf, nSamples * 2, 1, fd_) != 1) return false;
    if (byteswap_) {
      signed short int *ptr = buf;
      for (i = nSamples - 1; i >= 0; i--) b3Swap16((unsigned char *)ptr++);
    }
    if (doNormalize) {
      double gain = 1.0 / 32768.0;
      for (i = nSamples - 1; i >= 0; i--) m_frames[i] = buf[i] * gain;
    } else {
      for (i = nSamples - 1; i >= 0; i--) m_frames[i] = buf[i];
    }
  } else if (dataType_ == B3_SINT32) {
    int *buf = (int *)&m_frames[0];
    if (fseek(fd_, dataOffset_ + (offset * 4), SEEK_SET) == -1) return false;
    if (fread(buf, nSamples * 4, 1, fd_) != 1) return false;
    if (byteswap_) {
      int *ptr = buf;
      for (i = nSamples - 1; i >= 0; i--) b3Swap32((unsigned char *)ptr++);
    }
    if (doNormalize) {
    ...
    if (fseek(fd_, dataOffset_ + (offset * 8), SEEK_SET) == -1) return false;
    if (fread(buf, nSamples * 8, 1, fd_) != 1) return false;
    if (byteswap_) {
      double *ptr = buf;
      for (i = nSamples - 1; i >= 0; i--) b3Swap64((unsigned char *)ptr++);
    }
    for (i = nSamples - 1; i >= 0; i--) m_frames[i] = buf[i];
  } else if (dataType_ == B3_SINT8 &&
             wavFile_) {  // 8-bit WAV data is unsigned!
    unsigned char *buf = (unsigned char *)&m_frames[0];
    if (fseek(fd_, dataOffset_ + offset, SEEK_SET) == -1) return false;
    if (fread(buf, nSamples, 1, fd_) != 1) return false;
    if (doNormalize) {
      double gain = 1.0 / 128.0;
      for (i = nSamples - 1; i >= 0; i--) m_frames[i] = (buf[i] - 128) * gain;
    } else {
      for (i = nSamples - 1; i >= 0; i--) m_frames[i] = buf[i] - 128.0;
    }
  } else if (dataType_ == B3_SINT8) {  // signed 8-bit data
    ...
    double gain = 1.0 / 2147483648.0;
    if (fseek(fd_, dataOffset_ + (offset * 3), SEEK_SET) == -1) return false;
    for (i = 0; i < nSamples; i++) {
      if (m_machineIsLittleEndian) {
        if (byteswap_) {
          if (fread(ptr, 3, 1, fd_) != 1) return false;
          temp &= 0x00ffffff;
          b3Swap32((unsigned char *)ptr);
        } else {
          if (fread(ptr + 1, 3, 1, fd_) != 1) return false;
          temp &= 0xffffff00;
        }
      } else {
        if (byteswap_) {
      ...
      if (doNormalize) {
        m_frames[i] =
            (double)temp * gain;  // "gain" also  includes 1 / 256 factor.
      } else
        m_frames[i] =
            (double)temp / 256;  // right shift without affecting the sign bit
    }
  }
