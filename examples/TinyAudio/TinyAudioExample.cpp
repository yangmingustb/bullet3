...
struct MyHashString {
  ...
  MyHashString(const char* name) : m_string(name) {
...
double FrequencytoMidiPitch(double incoming_frequency) {
  return base_pitch +
         (12.0 * log(incoming_frequency / base_frequency) / log(2));
}
...
class TinyAudioExample : public CommonExampleInterface {
 ...
 public:
  TinyAudioExample(struct GUIHelperInterface* helper) : m_guiHelper(helper) {}
  ...
  virtual void exitPhysics() { m_soundEngine.exit(); }
...
CommonExampleInterface* TinyAudioExampleCreateFunc(
