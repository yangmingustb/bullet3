...
b3AudioListenerInternalData* b3AudioListener::getTickData() { return m_data; }

const b3AudioListenerInternalData* b3AudioListener::getTickData() const {
  return m_data;
}

double b3AudioListener::getSampleRate() const { return m_data->m_sampleRate; }

void b3AudioListener::setSampleRate(double sampleRate) {
  m_data->m_sampleRate = sampleRate;
}

int b3AudioListener::tick(void* outputBuffer, void* inputBuffer1,
      ...
      for (int i = 0; i < counter; i++) {
        ...
        for (int i = 0; i < MAX_SOUND_SOURCES; i++) {
          if (data->m_soundSources[i]) {
            ...
            if (data->m_soundSources[i]->computeSamples(tempOuts, 1,
                                                        data->m_sampleRate)) {
              numActiveSources++;
              // simple mixer
              outs[0] += tempOuts[0];
