...
/// The VolumetricDeformable shows the contact between volumetric deformable
...
class VolumetricDeformable : public CommonRigidBodyBase {
 ...
 public:
  ...
  void createStaticBox(const btVector3& halfEdge,
    ...
    if (isDynamic) box->calculateLocalInertia(mass, localInertia);
    // using motionstate is recommended, it provides interpolation capabilities,
    // and only synchronizes 'active' objects
    btDefaultMotionState* myMotionState = new btDefaultMotionState(Transform);
    btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, myMotionState, box,
                                                    localInertia);
...
void VolumetricDeformable::initPhysics() {
...

  {
    ...
    btCollisionShape* groundShape = new btBoxShape(
        btVector3(btScalar(150.), btScalar(50.), btScalar(150.)));
    ...
    groundTransform.setRotation(
        btQuaternion(btVector3(1, 0, 0), SIMD_PI * 0.0));
    // We can also use DemoApplication::localCreateRigidBody, but for clarity it
    // is provided here:
    btScalar mass(0.);
    // rigidbody is dynamic if and only if mass is non zero, otherwise static
  ...
  // create volumetric soft body
  {
    ...
    btDeformableNeoHookeanForce* neohookean =
        new btDeformableNeoHookeanForce(30, 100, 0.05);
...
class CommonExampleInterface* VolumetricDeformableCreateFunc(
