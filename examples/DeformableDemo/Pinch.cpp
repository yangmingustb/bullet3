...
/// The Pinch shows the frictional contact between kinematic rigid objects with
/// deformable objects
...
class Pinch : public CommonRigidBodyBase {
 ...
 public:
  Pinch(struct GUIHelperInterface* helper) : CommonRigidBodyBase(helper) {}
  virtual ~Pinch() {}
  void initPhysics();
...
void dynamics(btScalar time, btDeformableMultiBodyDynamicsWorld* world) {
  ...
  if (time < pressTime) {
    ...
    translation = initialTranslationLeft + pinchVelocityLeft * pressTime +
                  liftVelocity * (time - pressTime);

  } else if (time < shiftTime) {
    velocity = shiftVelocity;
    translation = initialTranslationLeft + pinchVelocityLeft * pressTime +
                  liftVelocity * (liftTime - pressTime) +
                  shiftVelocity * (time - liftTime);
  } else if (time < holdTime) {
    velocity = btVector3(0, 0, 0);
    translation = initialTranslationLeft + pinchVelocityLeft * pressTime +
                  liftVelocity * (liftTime - pressTime) +
                  shiftVelocity * (shiftTime - liftTime) +
                  holdVelocity * (time - shiftTime);
  } else if (time < dropTime) {
    velocity = openVelocityLeft;
    translation = initialTranslationLeft + pinchVelocityLeft * pressTime +
                  liftVelocity * (liftTime - pressTime) +
                  shiftVelocity * (shiftTime - liftTime) +
                  holdVelocity * (holdTime - shiftTime) +
                  openVelocityLeft * (time - holdTime);
  } else {
    ...
    translation = initialTranslationLeft + pinchVelocityLeft * pressTime +
                  ...
                  shiftVelocity * (shiftTime - liftTime) +
                  holdVelocity * (holdTime - shiftTime) +
                  openVelocityLeft * (dropTime - holdTime);
  }
  ...
  if (time < pressTime) {
    ...
    translation = initialTranslationRight + pinchVelocityRight * pressTime +
                  ...
                  shiftVelocity * (shiftTime - liftTime) +
                  holdVelocity * (holdTime - shiftTime) +
                  openVelocityRight * (time - holdTime);
  } else {
    velocity = holdVelocity;
    translation = initialTranslationRight + pinchVelocityRight * pressTime +
                  liftVelocity * (liftTime - pressTime) +
                  shiftVelocity * (shiftTime - liftTime) +
                  holdVelocity * (holdTime - shiftTime) +
                  openVelocityRight * (dropTime - holdTime);
  }
  rbTransform.setOrigin(translation);
  rbTransform.setRotation(btQuaternion(btVector3(1, 0, 0), SIMD_PI * 0));
...
void Pinch::initPhysics() {
  ...
  // create a soft block
  {
    btScalar verts[24] = {0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 1.f,
                          0.f, 0.f, 0.f, 1.f, 1.f, 1.f, 0.f, 0.f,
                          1.f, 1.f, 1.f, 0.f, 1.f, 1.f, 1.f, 1.f};
    int triangles[60] = {
        0, 6, 3, 0, 1, 6, 7, 5, 3, 7, 3, 6, 4, 7, 6, 4, 6, 1, 7, 2,
        5, 7, 4, 2, 0, 3, 2, 2, 3, 5, 0, 2, 4, 0, 4, 1, 0, 6, 5, 0,
        6, 4, 3, 4, 2, 3, 4, 7, 2, 7, 3, 2, 7, 1, 4, 5, 0, 4, 5, 6,
    };
    //       btSoftBody* psb =
    //       btSoftBodyHelpers::CreateFromTriMesh(getDeformableDynamicsWorld()->getWorldInfo(),
    //       &verts[0], &triangles[0], 20);
    ////
    btSoftBody* psb = btSoftBodyHelpers::CreateFromTetGenData(
        getDeformableDynamicsWorld()->getWorldInfo(), TetraCube::getElements(),
        0, TetraCube::getNodes(), false, true, true);
    ...
    btDeformableMassSpringForce* mass_spring =
        new btDeformableMassSpringForce(1, 0.05);
    ...
    btDeformableNeoHookeanForce* neohookean =
        new btDeformableNeoHookeanForce(.2, 1);
...
class CommonExampleInterface* PinchCreateFunc(
