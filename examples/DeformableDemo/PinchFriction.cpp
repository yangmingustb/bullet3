...
/// The PinchFriction shows the frictional contacts among volumetric deformable
/// objects
...
class PinchFriction : public CommonRigidBodyBase {
 ...
 public:
  ...
  virtual void renderScene() { CommonRigidBodyBase::renderScene(); }
};
...
void dynamics2(btScalar time, btDeformableMultiBodyDynamicsWorld* world) {
  ...
  if (time < pressTime) {
    ...
    translation = initialTranslationLeft +
                  PinchFrictionVelocityLeft * pressTime +
  ...
  if (time < pressTime) {
    ...
    translation = initialTranslationRight +
                  PinchFrictionVelocityRight * pressTime +
                  liftVelocity * (time - pressTime);
...
void PinchFriction::initPhysics() {
  ...
  // create a soft block
  {
    ...
    btDeformableNeoHookeanForce* neohookean =
        new btDeformableNeoHookeanForce(6, 6, .003);
  ...
  {
    btSoftBody* psb2 = btSoftBodyHelpers::CreateFromTetGenData(
        getDeformableDynamicsWorld()->getWorldInfo(), TetraCube::getElements(),
        0, TetraCube::getNodes(), false, true, true);
  ...
  {
    btSoftBody* psb3 = btSoftBodyHelpers::CreateFromTetGenData(
        getDeformableDynamicsWorld()->getWorldInfo(), TetraCube::getElements(),
        0, TetraCube::getNodes(), false, true, true);
...
class CommonExampleInterface* PinchFrictionCreateFunc(
