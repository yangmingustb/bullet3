...
void RaytestDemo::castRays() {
  ...
  // if (!m_idle)
  {
    ...
    btTransform tr =
        m_dynamicsWorld->getCollisionObjectArray()[1]->getWorldTransform();
  ...
  if (m_dynamicsWorld) {
    ...
    /// all hits
    {
      ...
      btVector3 to(30, 1, 0);
      m_dynamicsWorld->getDebugDrawer()->drawLine(from, to,
                                                  btVector4(0, 0, 0, 1));
      btCollisionWorld::AllHitsRayResultCallback allResults(from, to);
      allResults.m_flags |= btTriangleRaycastCallback::kF_KeepUnflippedNormal;
      // kF_UseGjkConvexRaytest flag is now enabled by default, use the faster
      // but more approximate algorithm
      // allResults.m_flags |=
      ...
      for (int i = 0; i < allResults.m_hitFractions.size(); i++) {
        ...
        m_dynamicsWorld->getDebugDrawer()->drawLine(
            p, p + allResults.m_hitNormalWorld[i], red);
      }
    }
...
void RaytestDemo::initPhysics() {
  ...
  {
    btVector3 convexPoints[] = {btVector3(-1, -1, -1), btVector3(-1, -1, 1),
                                btVector3(-1, 1, 1), btVector3(-1, 1, -1),
                                btVector3(2, 0, 0)};

    btVector3 quad[] = {btVector3(0, 1, -1), btVector3(0, 1, 1),
    ...
    btBvhTriangleMeshShape* trimesh =
        new btBvhTriangleMeshShape(mesh, true, true);
// btGImpactMeshShape * trimesh = new btGImpactMeshShape(mesh);
// trimesh->updateBound();

#define NUM_SHAPES 6
    btCollisionShape* colShapes[NUM_SHAPES] = {
        trimesh, new btConvexHullShape(&convexPoints[0].getX(),
                                       sizeof(convexPoints) / sizeof(btVector3),
                                       sizeof(btVector3)),
        new btSphereShape(1), new btCapsuleShape(0.2, 1),
    ...
    for (int i = 0; i < 6; i++) {
      ...
      btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, 0, colShape,
                                                      localInertia);
      ...
      body->setAnisotropicFriction(
          colShape->getAnisotropicRollingFrictionDirection(),
...
class CommonExampleInterface* RaytestCreateFunc(
