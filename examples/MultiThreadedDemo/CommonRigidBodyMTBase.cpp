...
class Profiler {
 ...
 private:
  ...
  struct Record {
    ...
    void begin(unsigned int curTime) { mStartTime = curTime; }
 ...
 public:
  ...
  void end(RecordType rt) { mRecords[rt].end(mClock.getTimeMicroseconds()); }
...
class ProfileHelper {
 ...
 public:
  ...
  ~ProfileHelper() { gProfiler.end(mRecType); }
};
...
class MySequentialImpulseConstraintSolverMt
 ...
 public:
  ...
  virtual btScalar solveGroupCacheFriendlySetup(
    ...
    btScalar ret = ParentClass::solveGroupCacheFriendlySetup(
  ...
  virtual btScalar solveGroupCacheFriendlyIterations(
    ...
    btScalar ret = ParentClass::solveGroupCacheFriendlyIterations(
        bodies, numBodies, manifoldPtr, numManifolds, constraints,
        numConstraints, infoGlobal, debugDrawer);
    return ret;
  }
  virtual btScalar solveGroupCacheFriendlyFinish(
    ...
    btScalar ret = ParentClass::solveGroupCacheFriendlyFinish(bodies, numBodies,
                                                              infoGlobal);
    return ret;
  }
  virtual btScalar solveGroup(btCollisionObject** bodies, int numBodies,
    ...
    btScalar ret = ParentClass::solveGroup(
...
class MyCollisionDispatcher : public btCollisionDispatcherMt {
 ...
 public:
  MyCollisionDispatcher(btCollisionConfiguration* config, int grainSize)
      : btCollisionDispatcherMt(config, grainSize) {}
...
///
/// myParallelIslandDispatch -- wrap default parallel dispatch for profiling and
/// to get the number of simulation islands
//
void myParallelIslandDispatch(
...
MyDiscreteDynamicsWorld : public btDiscreteDynamicsWorldMt {
 ...
 public:
  ...
  MyDiscreteDynamicsWorld(
      ...
      : btDiscreteDynamicsWorldMt(dispatcher, pairCache, constraintSolver,
                                  constraintSolverMt, collisionConfiguration) {
    btSimulationIslandManagerMt* islandMgr =
        static_cast<btSimulationIslandManagerMt*>(m_islandManager);
...
///
/// btTaskSchedulerManager -- manage a number of task schedulers so we can
/// switch between them
///
...
static btScalar gSliderMinBatchSize = btScalar(
    btSequentialImpulseConstraintSolverMt::s_minBatchSize);  // should be int
static btScalar gSliderMaxBatchSize = btScalar(
    btSequentialImpulseConstraintSolverMt::s_maxBatchSize);  // should be int
...
static void boolPtrButtonCallback(int buttonId, bool buttonState,
                                  void* userPointer) {
...
static void toggleSolverModeCallback(int buttonId, bool buttonState,
                                     void* userPointer) {
  if (buttonState) {
  ...
  if (CommonRigidBodyMTBase* crb =
          reinterpret_cast<CommonRigidBodyMTBase*>(userPointer)) {
...
void setSolverTypeComboBoxCallback(int combobox, const char* item,
                                   void* userPointer) {
...
void setTaskSchedulerComboBoxCallback(int combobox, const char* item,
                                      void* userPointer) {
#if BT_THREADSAFE
  const char** items = static_cast<const char**>(userPointer);
  for (int i = 0; i < 20; ++i) {
...
void setBatchingMethodComboBoxCallback(int combobox, const char* item,
                                       void* userPointer) {
#if BT_THREADSAFE
  const char** items = static_cast<const char**>(userPointer);
  for (int i = 0; i < btBatchedConstraints::BATCHING_METHOD_COUNT; ++i) {
    if (strcmp(item, items[i]) == 0) {
      ...
      btSequentialImpulseConstraintSolverMt::s_contactBatchingMethod =
          static_cast<btBatchedConstraints::BatchingMethod>(i);
      break;
    }
  }
#endif  // #if BT_THREADSAFE
...
static void setSolverIterationCountCallback(float val, void* userPtr) {
  if (btDiscreteDynamicsWorld* world =
          reinterpret_cast<btDiscreteDynamicsWorld*>(userPtr)) {
    world->getSolverInfo().m_numIterations =
        btMax(1, int(gSliderSolverIterations));
  }
}
...
static void setLargeIslandManifoldCountCallback(float val, void* userPtr) {
  btSequentialImpulseConstraintSolverMt::s_minimumContactManifoldsForBatching =
      int(gSliderIslandBatchingThreshold);
}
...
static void setLeastSquaresResidualThresholdCallback(float val, void* userPtr) {
  if (btDiscreteDynamicsWorld* world =
          reinterpret_cast<btDiscreteDynamicsWorld*>(userPtr)) {
    world->getSolverInfo().m_leastSquaresResidualThreshold =
...
void CommonRigidBodyMTBase::createEmptyDynamicsWorld() {
  ...
  if (NULL != btGetTaskScheduler() &&
      gTaskSchedulerMgr.getNumTaskSchedulers() > 1) {
  ...
  if (gMultithreadedWorld) {
    ...
    btConstraintSolverPoolMt* solverPool;
    {
      ...
      if (poolSolverType == SOLVER_TYPE_SEQUENTIAL_IMPULSE_MT) {
        // pool solvers shouldn't be parallel solvers, we don't allow that kind
        // of
    ...
    btDiscreteDynamicsWorld* world =
        new MyDiscreteDynamicsWorld(m_dispatcher, m_broadphase, solverPool,
...
void CommonRigidBodyMTBase::createDefaultParameters() {
  ...
  {
    // a slider for the solver leastSquaresResidualThreshold (used to run fewer
    // solver iterations when convergence is good)
    SliderParams slider("Solver residual thresh",
                        &gSliderLeastSquaresResidualThreshold);
  ...
  if (m_multithreadedWorld) {
    ...
    if (gTaskSchedulerMgr.getNumTaskSchedulers() >= 1) {
      ...
      for (int i = 0; i < gTaskSchedulerMgr.getNumTaskSchedulers(); ++i) {
        sTaskSchedulerComboBoxItems[i] =
            gTaskSchedulerMgr.getTaskScheduler(i)->getName();
    ...
    {
      // a slider for the number of manifolds an island needs to be too large
      // for parallel dispatch
      if (gSliderIslandBatchingThreshold < 1.0) {
        gSliderIslandBatchingThreshold =
            float(btSequentialImpulseConstraintSolverMt::
    ...
    }
    {
      ...
      static const char* sBatchingMethodComboBoxItems
          [btBatchedConstraints::BATCHING_METHOD_COUNT];
      {
        sBatchingMethodComboBoxItems
      ...
      comboParams.m_startItem = static_cast<int>(
          btSequentialImpulseConstraintSolverMt::s_contactBatchingMethod);
    ...
    {
      // a slider for the sequentialImpulseConstraintSolverMt min batch size
      // (when batching)
    ...
    {
      // a slider for the sequentialImpulseConstraintSolverMt max batch size
      // (when batching)
  ...
  if (gDisplayProfileInfo) {
    if (m_multithreadedWorld) {
      ...
      for (int i = 0; i < numManifolds; ++i) {
        const btPersistentManifold* man =
            m_dispatcher->getManifoldByIndexInternal(i);
        numContacts += man->getNumContacts();
      }
      const char* mtApi = btGetTaskScheduler()->getName();
      sprintf(
          msg, "islands=%d bodies=%d manifolds=%d contacts=%d [%s] threads=%d",
          gNumIslands, m_dynamicsWorld->getNumCollisionObjects(), numManifolds,
          numContacts, mtApi, btGetTaskScheduler()->getNumThreads());
    ...
    }
    {
      ...
      sprintf(msg, "solver %s mode [%s%s%s%s%s%s]",
              getSolverTypeName(m_solverType), sm & SOLVER_SIMD ? "SIMD" : "",
              sm & SOLVER_RANDMIZE_ORDER ? " randomize" : "",
              sm & SOLVER_INTERLEAVE_CONTACT_AND_FRICTION_CONSTRAINTS
                  ? " interleave"
                  : "",
              sm & SOLVER_USE_2_FRICTION_DIRECTIONS ? " friction2x" : "",
              sm & SOLVER_ENABLE_FRICTION_DIRECTION_CACHING
                  ? " frictionDirCaching"
                  : "",
    ...
    if (m_multithreadedWorld) {
      sprintf(
          msg, "DispatchCollisionPairs %5.3f ms",
          gProfiler.getAverageTime(Profiler::kRecordDispatchAllCollisionPairs) *
              0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord, yCoord, 0.4f);
      yCoord += yStep;

      sprintf(
          msg, "SolveAllIslands %5.3f ms",
          gProfiler.getAverageTime(Profiler::kRecordDispatchIslands) * 0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord, yCoord, 0.4f);
      yCoord += yStep;

      sprintf(msg, "SolverTotal %5.3f ms",
              gProfiler.getAverageTime(Profiler::kRecordSolverTotal) * 0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord, yCoord, 0.4f);
      yCoord += yStep;

      sprintf(msg, "SolverSetup %5.3f ms",
              gProfiler.getAverageTime(Profiler::kRecordSolverSetup) * 0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord + indent, yCoord,
                                               0.4f);
      yCoord += yStep;

      sprintf(
          msg, "SolverIterations %5.3f ms",
          gProfiler.getAverageTime(Profiler::kRecordSolverIterations) * 0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord + indent, yCoord,
                                               0.4f);
      yCoord += yStep;

      sprintf(msg, "SolverFinish %5.3f ms",
              gProfiler.getAverageTime(Profiler::kRecordSolverFinish) * 0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord + indent, yCoord,
                                               0.4f);
      yCoord += yStep;

      sprintf(msg, "PredictUnconstrainedMotion %5.3f ms",
              gProfiler.getAverageTime(
                  Profiler::kRecordPredictUnconstrainedMotion) *
                  0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord, yCoord, 0.4f);
      yCoord += yStep;

      sprintf(
          msg, "CreatePredictiveContacts %5.3f ms",
          gProfiler.getAverageTime(Profiler::kRecordCreatePredictiveContacts) *
              0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord, yCoord, 0.4f);
      yCoord += yStep;

      sprintf(msg, "IntegrateTransforms %5.3f ms",
              gProfiler.getAverageTime(Profiler::kRecordIntegrateTransforms) *
                  0.001f);
      m_guiHelper->getAppInterface()->drawText(msg, xCoord, yCoord, 0.4f);
      yCoord += yStep;
    }
