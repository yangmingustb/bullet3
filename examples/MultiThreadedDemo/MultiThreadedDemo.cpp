...
class MultiThreadedDemo : public CommonRigidBodyMTBase {
  ...
  void createStack(const btTransform& trans, btCollisionShape* boxShape,
                   const btVector3& halfBoxSize, int size, int width);
 ...
 public:
  ...
  virtual void stepSimulation(float deltaTime) BT_OVERRIDE {
    if (m_dynamicsWorld) {
      if (m_prevRollingFriction != gSliderRollingFriction) {
        ...
        btCollisionObjectArray& objArray =
      ...
      if (m_groundBody) {
        ...
        m_groundMovePhase -=
            std::floor(m_groundMovePhase);  // keep phase between 0 and 1
        btTransform xf = m_groundStartXf;
        float gndHOffset = btSin(m_groundMovePhase * SIMD_2_PI) *
                           gSliderGroundHorizontalAmplitude;
        float gndHVel = btCos(m_groundMovePhase * SIMD_2_PI) *
                        gSliderGroundHorizontalAmplitude * cyclesPerSecond *
                        SIMD_2_PI;  // d(gndHOffset)/dt
        float gndVOffset = btSin(m_groundMovePhase * SIMD_2_PI) *
                           gSliderGroundVerticalAmplitude;
        float gndVVel = btCos(m_groundMovePhase * SIMD_2_PI) *
                        gSliderGroundVerticalAmplitude * cyclesPerSecond *
                        SIMD_2_PI;  // d(gndVOffset)/dt
  ...
  virtual void resetCamera() BT_OVERRIDE {
    m_guiHelper->resetCamera(m_cameraDist, m_cameraYaw, m_cameraPitch,
                             m_cameraTargetPos.x(), m_cameraTargetPos.y(),
                             m_cameraTargetPos.z());
  }
};
...
btRigidBody* MultiThreadedDemo::localCreateRigidBody(
...
void MultiThreadedDemo::createStack(const btTransform& parentTrans,
                                    btCollisionShape* boxShape,
                                    const btVector3& halfBoxSize, int height,
                                    int width) {
  btTransform trans;
  trans.setIdentity();
  ...
  for (int iZ = 0; iZ < width; iZ++) {
    ...
    for (int iY = 0; iY < height; iY++) {
      ...
      for (int iX = 0; iX < rowSize; iX++) {
        btVector3 pos =
            offset + btVector3(halfBoxWidth * (1 + iX * 2 - rowSize),
                               halfBoxHeight * (1 + iY * 2), 0.0f);
...

  {
    ...
    for (int iX = 0; iX < numStackCols; ++iX) {
      for (int iZ = 0; iZ < numStackRows; ++iZ) {
        btVector3 center = btVector3(iX * stackXSpacing, 0.0f,
                                     (iZ - numStackRows / 2) * stackZSpacing);
...
CommonExampleInterface* MultiThreadedDemoCreateFunc(
