...
struct TestCanvasInterface2 : public Common2dCanvasInterface {
  ...
  TestCanvasInterface2(b3AlignedObjectArray<unsigned char>& texelsRGB,
                       int width, int height)
      : m_texelsRGB(texelsRGB), m_width(width), m_height(height) {}
...
TinyVRGui::TinyVRGui(struct ComboBoxParams& params,
                     struct CommonRenderInterface* renderer) {
...
bool TinyVRGui::init() {
  {
    ...
    m_data->m_testCanvas =
        new TestCanvasInterface2(m_data->m_texelsRGB, width, height);
    m_data->m_timeSeries = new TimeSeriesCanvas(m_data->m_testCanvas, width,
                                                height, "time series");
    ...
    m_data->m_textureId = m_data->m_renderer->registerTexture(
        &m_data->m_texelsRGB[0], width, height);

    {
      ...
      if (b3ImportMeshUtility::loadAndRegisterMeshFromFileInternal(
        ...
        shapeId = m_data->m_renderer->registerShape(
            &meshData.m_gfxShape->m_vertices->at(0).xyzw[0],
            meshData.m_gfxShape->m_numvertices,
            &meshData.m_gfxShape->m_indices->at(0),
            meshData.m_gfxShape->m_numIndices, B3_GL_TRIANGLES,
            m_data->m_textureId);
        ...
        m_data->m_gfxObjectId = m_data->m_renderer->registerGraphicsInstance(
...
void TinyVRGui::tick(b3Scalar deltaTime, const b3Transform& guiWorldTransform) {
  ...
  m_data->m_renderer->updateTexture(m_data->m_textureId,
                                    &m_data->m_texelsRGB[0]);
  m_data->m_renderer->writeSingleInstanceTransformToCPU(
      guiWorldTransform.getOrigin(), guiWorldTransform.getRotation(),
      m_data->m_gfxObjectId);
  m_data->m_renderer->writeTransforms();
}
...
void TinyVRGui::grapicalPrintf(const char* str, int rasterposx, int rasterposy,
                               unsigned char red, unsigned char green,
                               unsigned char blue, unsigned char alpha) {
  m_data->m_timeSeries->grapicalPrintf(str, sTimeSeriesFontData, rasterposx,
                                       rasterposy, red, green, blue, alpha);
}
