...
struct RaytracerPhysicsSetup : public CommonExampleInterface {
  ...
  /// worldRaytest performs a ray versus all objects in a collision world,
  /// returning true is a hit is found (filling in worldNormal and
  /// worldHitPoint)
  bool worldRaytest(const btVector3& rayFrom, const btVector3& rayTo,
                    btVector3& worldNormal, btVector3& worldHitPoint);

  /// singleObjectRaytest performs a ray versus one collision shape, returning
  /// true is a hit is found (filling in worldNormal and worldHitPoint)
  bool singleObjectRaytest(const btVector3& rayFrom, const btVector3& rayTo,
                           btVector3& worldNormal, btVector3& worldHitPoint);

  /// lowlevelRaytest performs a ray versus convex shape, returning true is a
  /// hit is found (filling in worldNormal and worldHitPoint)
  bool lowlevelRaytest(const btVector3& rayFrom, const btVector3& rayTo,
                       btVector3& worldNormal, btVector3& worldHitPoint);
...
RaytracerPhysicsSetup::~RaytracerPhysicsSetup() { delete m_internalData; }
...
void RaytracerPhysicsSetup::initPhysics() {
  ...
  if (m_internalData->m_canvas) {
    m_internalData->m_canvasIndex = m_internalData->m_canvas->createCanvas(
        "raytracer", m_internalData->m_width, m_internalData->m_height, 15, 55);
    for (int i = 0; i < m_internalData->m_width; i++) {
      for (int j = 0; j < m_internalData->m_height; j++) {
        ...
        m_internalData->m_canvas->setPixel(m_internalData->m_canvasIndex, i, j,
...
/// worldRaytest performs a ray versus all objects in a collision world,
/// returning true is a hit is found (filling in worldNormal and worldHitPoint)
bool RaytracerPhysicsSetup::worldRaytest(const btVector3& rayFrom,
...
/// singleObjectRaytest performs a ray versus one collision shape, returning
/// true is a hit is found (filling in worldNormal and worldHitPoint)
bool RaytracerPhysicsSetup::singleObjectRaytest(const btVector3& rayFrom,
                                                const btVector3& rayTo,
                                                btVector3& worldNormal,
                                                btVector3& worldHitPoint) {
  return false;
...
/// lowlevelRaytest performs a ray versus convex shape, returning true is a hit
/// is found (filling in worldNormal and worldHitPoint)
bool RaytracerPhysicsSetup::lowlevelRaytest(const btVector3& rayFrom,
                                            const btVector3& rayTo,
                                            btVector3& worldNormal,
                                            btVector3& worldHitPoint) {
  ...
  for (int s = 0; s < numObjects; s++) {
    ...
    m_internalData->m_shapePtr[s]->getAabb(m_internalData->m_transforms[s],
                                           aabbMin, aabbMax);
    ...
    if (btRayAabb(rayFrom, rayTo, aabbMin, aabbMax, hitLambda, hitNormal)) {
      ...
      btSubsimplexConvexCast convexCaster(&pointShape,
                                          m_internalData->m_shapePtr[s],
                                          &m_internalData->m_simplexSolver);
      // btGjkConvexCast convexCaster(&pointShape,shapePtr[s],&simplexSolver);
      // btContinuousConvexCollision
      // convexCaster(&pointShape,shapePtr[s],&simplexSolver,0);

      if (convexCaster.calcTimeOfImpact(
              rayFromTrans, rayToTrans, m_internalData->m_transforms[s],
              m_internalData->m_transforms[s], rayResult)) {
        if (rayResult.m_fraction < closestHitResults) {
          closestHitResults = rayResult.m_fraction;

          worldNormal =
              m_internalData->m_transforms[s].getBasis() * rayResult.m_normal;
...
void RaytracerPhysicsSetup::stepSimulation(float deltaTime) {
  ...
  if (m_app->m_renderer && m_app->m_renderer->getActiveCamera()) {
    ...
    m_app->m_renderer->getActiveCamera()->getCameraTargetPosition(
  ...
  for (x = 0; x < m_internalData->m_width; x++) {
    for (y = 0; y < m_internalData->m_height; y++) {
      ...
      if (hasHit) {
        float lightVec0 =
            worldNormal.dot(btVector3(0, -1, -1));  // 0.4f,-1.f,-0.4f));
        float lightVec1 =
            worldNormal.dot(btVector3(-1, 0, -1));  //-0.4f,-1.f,-0.4f));
        ...
        unsigned char alpha = 255;
        m_internalData->m_canvas->setPixel(m_internalData->m_canvasIndex, x, y,
                                           red, green, blue, alpha);
      } else {
...
bool RaytracerPhysicsSetup::mouseButtonCallback(int button, int state, float x,
                                                float y) {
  return false;
}
...
void RaytracerPhysicsSetup::syncPhysicsToGraphics(
    GraphicsPhysicsBridge& gfxBridge) {}

CommonExampleInterface* RayTracerCreateFunc(
