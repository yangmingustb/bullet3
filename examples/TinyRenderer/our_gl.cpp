...
void triangleClipped(mat<4, 3, float> &clipc, mat<4, 3, float> &orgClipc,
                     IShader &shader, TGAImage &image, float *zbuffer,
                     const Matrix &viewPortMatrix) {
  triangleClipped(clipc, orgClipc, shader, image, zbuffer, 0, viewPortMatrix,
                  0);
}

void triangleClipped(mat<4, 3, float> &clipc, mat<4, 3, float> &orgClipc,
                     IShader &shader, TGAImage &image, float *zbuffer,
                     int *segmentationMaskBuffer, const Matrix &viewPortMatrix,
                     int objectAndLinkIndex) {
  ...
  for (P.x = bboxmin.x; P.x <= bboxmax.x; P.x++) {
    for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++) {
      float frag_depth = 0;
      {
        ...
        Vec3f bc_clip = Vec3f(bc_screen.x / screenSpacePts[0][3],
      ...
      Vec3f bc_clip2 = Vec3f(bc_screen2.x / orgScreenSpacePts[0][3],
      ...
      if (!discard) {
        ...
        if (segmentationMaskBuffer) {
          segmentationMaskBuffer[P.x + P.y * image.get_width()] =
              objectAndLinkIndex;
        }
...
void triangle(mat<4, 3, float> &clipc, IShader &shader, TGAImage &image,
              float *zbuffer, const Matrix &viewPortMatrix) {
  triangle(clipc, shader, image, zbuffer, 0, viewPortMatrix, 0);
}

void triangle(mat<4, 3, float> &clipc, IShader &shader, TGAImage &image,
              float *zbuffer, int *segmentationMaskBuffer,
              const Matrix &viewPortMatrix, int objectAndLinkIndex) {
  mat<3, 4, float> pts =
      (viewPortMatrix * clipc)
          .transpose();  // transposed to ease access to each of the points
