...
enum LWEnumCollisionTypes { LW_PLANE_TYPE, LW_SPHERE_TYPE, LW_BOX_TYPE };
...
struct LWPose {
  ...
  LWPose() : m_position(b3MakeVector3(0, 0, 0)), m_orientation(0, 0, 0, 1) {}
...
void ComputeClosestPointsPlaneSphere(const LWPlane& planeWorld,
                                     const LWSphere& sphere,
                                     const LWPose& spherePose,
  ...
  pointOut.m_ptOnAWorld =
      spherePosWorld + sphere.m_radius * -planeWorld.m_normal;
  pointOut.m_normalOnB = planeWorld.m_normal;
}

void ComputeClosestPointsSphereSphere(const LWSphere& sphereA,
                                      const LWPose& sphereAPose,
                                      const LWSphere& sphereB,
                                      const LWPose& sphereBPose,
                                      LWContactPoint& pointOut) {
  ...
  pointOut.m_ptOnBWorld =
      pointOut.m_ptOnAWorld - pointOut.m_normalOnB * pointOut.m_distance;
}
...
struct LWRigidBody {
  ...
  void computeInvInertiaTensorWorld() {
    ...
    invInertiaLocal.setValue(
        m_localInertia.x != btScalar(0.0) ? btScalar(1.0) / m_localInertia.x
                                          : btScalar(0.0),
        m_localInertia.y != btScalar(0.0) ? btScalar(1.0) / m_localInertia.y
                                          : btScalar(0.0),
        m_localInertia.z != btScalar(0.0) ? btScalar(1.0) / m_localInertia.z
                                          : btScalar(0.0));
  ...
  const b3Vector3& getPosition() const { return m_worldPose.m_position; }
    ...
    if (m_flags & LWFLAG_USE_QUATERNION_DERIVATIVE) {
      ...
      if (fAngle < btScalar(0.001)) {
        ...
        axis = m_angularVelocity *
               (btScalar(0.5) * deltaTime -
                (deltaTime * deltaTime * deltaTime) *
                    (btScalar(0.020833333333)) * fAngle * fAngle);
      } else {
        // sync(fAngle) = sin(c*fAngle)/t
        axis = m_angularVelocity *
               (btSin(btScalar(0.5) * fAngle * deltaTime) / fAngle);
      }
      b3Quaternion dorn(axis.x, axis.y, axis.z,
                        btCos(fAngle * deltaTime * b3Scalar(0.5)));
...
b3Scalar resolveCollision(LWRigidBody& bodyA, LWRigidBody& bodyB,
  ...
  btScalar rel_vel = contactPoint.m_normalOnB.dot(bodyA.getVelocity(rel_pos1) -
                                                  bodyB.getVelocity(rel_pos2));
  if (rel_vel < -B3_EPSILON) {
    b3Vector3 temp1 = bodyA.m_invInertiaTensorWorld *
                      rel_pos1.cross(contactPoint.m_normalOnB);
    b3Vector3 temp2 = bodyB.m_invInertiaTensorWorld *
 ...
 public:
  Tutorial(GUIHelperInterface* guiHelper, int tutorialIndex)
    ...
    switch (m_tutorialIndex) {
      case TUT_VELOCITY: {
        numBodies = 10;
        m_timeSeriesCanvas0 = new TimeSeriesCanvas(
            m_app->m_2dCanvasInterface, 512, 256, "Constant Velocity");
      ...
      case TUT_ACCELERATION: {
        numBodies = 10;
        m_timeSeriesCanvas1 = new TimeSeriesCanvas(
            m_app->m_2dCanvasInterface, 256, 512, "Constant Acceleration");
      ...
      case TUT_COLLISION: {
        numBodies = 2;
        m_timeSeriesCanvas1 = new TimeSeriesCanvas(m_app->m_2dCanvasInterface,
                                                   512, 200, "Distance");
      ...
      case TUT_SOLVE_CONTACT_CONSTRAINT: {
        numBodies = 2;
        m_timeSeriesCanvas1 = new TimeSeriesCanvas(
            m_app->m_2dCanvasInterface, 512, 200, "Collision Impulse");
        m_timeSeriesCanvas1->setupTimeSeries(1.5, 60, 0);
      ...
      default: {
        m_timeSeriesCanvas0 = new TimeSeriesCanvas(m_app->m_2dCanvasInterface,
                                                   512, 256, "Unknown");
    ...
    }
    {
      ...
      b3Vector3 scaling =
          b3MakeVector3(SPHERE_RADIUS, SPHERE_RADIUS, SPHERE_RADIUS);
      for (int i = 0; i < m_bodies.size(); i++) {
        ...
        m_bodies[i]->m_graphicsIndex =
            m_app->m_renderer->registerGraphicsInstance(
                gfxShape, m_bodies[i]->m_worldPose.m_position,
                m_bodies[i]->m_worldPose.m_orientation, color, scaling);
        m_app->m_renderer->writeSingleInstanceTransformToCPU(
            m_bodies[i]->m_worldPose.m_position,
            m_bodies[i]->m_worldPose.m_orientation,
            m_bodies[i]->m_graphicsIndex);
      }
    }
    ...
    if (m_tutorialIndex == TUT_SOLVE_CONTACT_CONSTRAINT) {
      ...
      m_bodies[0]->m_collisionShape.m_sphere.computeLocalInertia(
          gMassA, m_bodies[0]->m_localInertia);

      m_bodies[1]->m_invMass = gMassB ? 1. / gMassB : 0;
      m_bodies[1]->m_collisionShape.m_sphere.computeLocalInertia(
          gMassB, m_bodies[1]->m_localInertia);

      if (gMassA) m_bodies[0]->m_linearVelocity.setValue(0, 0, 1);
      if (gMassB) m_bodies[1]->m_linearVelocity.setValue(0, 0, -1);
    }
  ...
  virtual void stepSimulation(float deltaTime) {
    switch (m_tutorialIndex) {
      ...
      case TUT_COLLISION: {
        ...
        m_timeSeriesCanvas1->insertDataAtCurrentTime(contactPoint.m_distance, 0,
                                                     true);
      ...
      case TUT_SOLVE_CONTACT_CONSTRAINT: {
        ...
        if (contactPoint.m_distance < 0) {
          ...
          b3Scalar appliedImpulse =
              resolveCollision(*m_bodies[0], *m_bodies[1], contactPoint);
    ...
    for (int i = 0; i < m_bodies.size(); i++) {
      ...
      m_app->m_renderer->writeSingleInstanceTransformToCPU(
          m_bodies[i]->m_worldPose.m_position,
          m_bodies[i]->m_worldPose.m_orientation, m_bodies[i]->m_graphicsIndex);
    }
  ...
  virtual void renderScene() {
    ...
    for (int i = 0; i < m_contactPoints.size(); i++) {
      ...
      m_app->m_renderer->drawLine(contact.m_ptOnAWorld, contact.m_ptOnBWorld,
                                  color, lineWidth);
    }
  }
...
void Tutorial::tutorial1Update(float deltaTime) {
  for (int i = 0; i < m_bodies.size(); i++) {
    switch (m_stage) {
      ...
      default: { m_bodies[i]->m_angularVelocity = b3MakeVector3(0, 0, 0); }
    };
  }
  ...
  if (m_counter > 60) {
    ...
    b3Printf("Stage = %d\n", m_stage);
    b3Printf("linVel = %f,%f,%f\n", m_bodies[0]->m_linearVelocity.x,
             m_bodies[0]->m_linearVelocity.y, m_bodies[0]->m_linearVelocity.z);
    b3Printf("angVel = %f,%f,%f\n", m_bodies[0]->m_angularVelocity.x,
             m_bodies[0]->m_angularVelocity.y,
             m_bodies[0]->m_angularVelocity.z);
  }
}

void Tutorial::tutorialSolveContactConstraintUpdate(float deltaTime,
                                                    LWContactPoint& contact) {
  ComputeClosestPointsSphereSphere(m_bodies[0]->m_collisionShape.m_sphere,
                                   ...
                                   m_bodies[1]->m_worldPose, contact);
}

void Tutorial::tutorialCollisionUpdate(float deltaTime,
                                       LWContactPoint& contact) {
  ...
  if (m_counter > 120) {
    m_counter = 0;
    m_stage++;
    if (m_stage > 5) m_stage = 0;
  }
}

class CommonExampleInterface* TutorialCreateFunc(
