...
struct Dof6ConstraintTutorialInternalData {
  ...
  Dof6ConstraintTutorialInternalData() : mDt(1. / 60.), frameID(0) {}
};
...
Dof6ConstraintTutorial::Dof6ConstraintTutorial(
  ...
  m_data->m_timeSeriesCanvas = new TimeSeriesCanvas(
      helper->get2dCanvasInterface(), 256, 256, "Position and Velocity");
...
/////// servo motor: flip its target periodically
  ...
  // btScalar pos =
  // m_data->m_ServoMotorConstraint->getRotationalLimitMotor(2)->m_currentPosition;
  // btScalar target =
...
void Dof6ConstraintTutorial::stepSimulation(float deltaTime) {
  ...
  float prevPos =
      m_data->m_TranslateSpringBody->getWorldTransform().getOrigin().x();
  m_dynamicsWorld->stepSimulation(deltaTime);
  float xPos =
      m_data->m_TranslateSpringBody->getWorldTransform().getOrigin().x();

  m_data->m_timeSeriesCanvas->insertDataAtCurrentTime(xPos, 0, true);
  m_data->m_timeSeriesCanvas->insertDataAtCurrentTime(
      m_data->m_TranslateSpringBody->getLinearVelocity().x(), 1, true);

  if (deltaTime > 0) {
    m_data->m_timeSeriesCanvas->insertDataAtCurrentTime(
        (xPos - prevPos) / deltaTime, 2, true);
  }
  prevPos = xPos;
  m_data->m_timeSeriesCanvas->nextTick();
}

class CommonExampleInterface* Dof6ConstraintTutorialCreateFunc(
