...
GLInstanceGraphicsShape* btgCreateGraphicsShapeFromWavefrontObj(
      ...
      for (int f = 0; f < faceCount; f += 3) {
        ...
        if (1) {
          ...
          if (attribute.texcoords.size()) {
            ...
            if (uv0Index >= 0 && uv1Index >= 0 &&
                (uv0Index < int(attribute.texcoords.size()) &&
                 (uv1Index < attribute.texcoords.size()))) {
          ...
          if (attribute.texcoords.size()) {
            ...
            if (uv0Index >= 0 && uv1Index >= 0 &&
                (uv0Index < attribute.texcoords.size()) &&
                (uv1Index < attribute.texcoords.size())) {
          ...
          bool hasNormals =
              (attribute.normals.size() && maxIndex < attribute.normals.size());
