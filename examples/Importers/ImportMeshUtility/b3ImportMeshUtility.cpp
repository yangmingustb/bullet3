...
struct CachedTextureResult {
  ...
  CachedTextureResult() : m_width(0), m_height(0), m_pixels(0) {}
};
...
bool b3ImportMeshUtility::loadAndRegisterMeshFromFileInternal(
    const std::string& fileName, b3ImportMeshData& meshData,
  ...
  if (fileIO->findResourcePath(fileName.c_str(), relativeFileName, 1024)) {
    ...
    tinyobj::attrib_t attribute;
    {
      ...
      std::string err = LoadFromCachedOrFromObj(
          attribute, shapes, relativeFileName, pathPrefix, fileIO);
      // std::string err = tinyobj::LoadObj(shapes, relativeFileName,
      // pathPrefix);
    }

    GLInstanceGraphicsShape* gfxShape =
        btgCreateGraphicsShapeFromWavefrontObj(attribute, shapes);
    {
      ...
      for (int i = 0; meshData.m_textureImage1 == 0 && i < shapes.size(); i++) {
        ...
        if (shape.material.diffuse_texname.length() > 0) {
          ...
          for (int i = 0; !image && i < numprefix; i++) {
            ...
            if (fileIO->findResourcePath(relativeFileName, relativeFileName2,
                                         1024)) {
              if (b3IsFileCachingEnabled()) {
                CachedTextureResult* texture =
                    gCachedTextureResults[relativeFileName];
                if (texture) {
              ...
              if (image == 0) {
                ...
                if (buffer.size()) {
                  image = stbi_load_from_memory(
                      (const unsigned char*)&buffer[0], buffer.size(), &width,
                      &height, &n, 3);
                }
