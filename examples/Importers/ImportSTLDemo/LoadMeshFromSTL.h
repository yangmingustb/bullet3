...
static GLInstanceGraphicsShape* LoadMeshFromSTL(
    const char* relativeFileName, struct CommonFileIOInterface* fileIO) {
    ...
    size = fileIO->getFileSize(fileHandle);
    {
      if (size >= 0) {
        ...
        if (actualBytesRead != size) {
          ...
          if (numTriangles) {
            {
              // perform a sanity check instead of crashing on invalid
              // triangles/STL files
            ...
            //						b3AlignedObjectArray<GLInstanceVertex>*
            //m_vertices;
            //						int
            //m_numvertices;
