...
class ImportSDFSetup : public CommonMultiBodyBase {
 ...
 public:
  ImportSDFSetup(struct GUIHelperInterface* helper, int option,
                 const char* fileName);
...
struct ImportSDFInternalData {
  ImportSDFInternalData() : m_numMotors(0) {
...
ImportSDFSetup::ImportSDFSetup(struct GUIHelperInterface* helper, int option,
                               const char* fileName)
...
void ImportSDFSetup::initPhysics() {
  ...
  if (loadOk) {
    ...
    for (int m = 0; m < u2b.getNumModels(); m++) {
      ...
      ConvertURDF2Bullet(u2b, creation, rootTrans, m_dynamicsWorld,
                         m_useMultiBody, u2b.getPathPrefix(), CUF_USE_SDF);
      ...
      if (m_useMultiBody && mb) {
        ...
        for (int i = 0; i < numLinks; i++) {
          ...
          std::string* jointName =
              new std::string(u2b.getJointName(urdfLinkIndex));
          std::string* linkName =
          ...
          if (mb->getLink(mbLinkIndex).m_jointType ==
            ...
            if (m_data->m_numMotors < MAX_NUM_MOTORS) {
              ...
              btScalar* motorVel =
                  &m_data->m_motorTargetVelocities[m_data->m_numMotors];
              *motorVel = 0.f;
    ...
    /// this extra stepSimulation call makes sure that all the btMultibody
    /// transforms are properly propagates.
  ...
  if (m_dynamicsWorld) {
    for (int i = 0; i < m_data->m_numMotors; i++) {
      if (m_data->m_jointMotors[i]) {
        m_data->m_jointMotors[i]->setVelocityTarget(
            m_data->m_motorTargetVelocities[i]);
      }
      if (m_data->m_generic6DofJointMotors[i]) {
        ...
        m_data->m_generic6DofJointMotors[i]->setTargetVelocity(
            jointInfo->m_jointAxisIndex, m_data->m_motorTargetVelocities[i]);
        // jointInfo->
      }
    }
...
class CommonExampleInterface* ImportSDFCreateFunc(
    struct CommonExampleOptions& options) {
  return new ImportSDFSetup(options.m_guiHelper, options.m_option,
                            options.m_fileName);
}
