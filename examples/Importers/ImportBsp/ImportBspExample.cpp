...
class BspDemo : public CommonRigidBodyBase {
 public:
  ...
  BspDemo(struct GUIHelperInterface* helper) : CommonRigidBodyBase(helper) {}
...
/// BspToBulletConverter  extends the BspConverter to convert to Bullet
/// datastructures
class BspToBulletConverter : public BspConverter {
  BspDemo* m_demoApp;

 public:
  BspToBulletConverter(BspDemo* demoApp) : m_demoApp(demoApp) {}

  virtual void addConvexVerticesCollider(
      btAlignedObjectArray<btVector3>& vertices, bool isEntity,
      const btVector3& entityTargetLocation) {
    ...
    if (vertices.size() > 0) {
      ...
      btCollisionShape* shape =
          new btConvexHullShape(&(vertices[0].getX()), vertices.size());
...
void BspDemo::initPhysics(const char* bspfilename) {
  ...
  if (file) {
    ...
    if (fseek(file, 0, SEEK_END) || (size = ftell(file)) == EOF ||
        fseek(file, 0, SEEK_SET)) { /* File operations denied? ok, just close
                                       and return failure */
...
char* getLastFileName() { return cleaned_filename; }
char* makeExeToBspFilename(const char* lpCmdLine) {
  // We might get a windows-style path on the command line, this can mess up the
  // DOM which expects
  // all paths to be URI's.  This block of code does some conversion to try and
  // make the input
  // compliant without breaking the ability to accept a properly formatted URI.
  // Right now this only
  // displays the first filename
  const char* in = lpCmdLine;
  char* out = cleaned_filename;
  *out = '\0';
  // If the first character is a ", skip it (filenames with spaces in them are
  // quoted)
  if (*in == '\"') {
    in++;
  }
  int i;
  for (i = 0; i < 512; i++) {
    // if we get '.' we stop as well, unless it's the first character. Then we
    // add .bsp as extension
    // If we hit a null or a quote, stop copying.  This will get just the first
    // filename.
    if (i && (in[0] == '.') && (in[1] == 'e') && (in[2] == 'x') &&
        (in[3] == 'e'))
      break;
...
CommonExampleInterface* ImportBspCreateFunc(
