...
void BspConverter::convertBsp(BspLoader& bspLoader, float scaling) {
  {
    ...
    if (bspLoader.findVectorByName(&playstartf[0], "info_player_start")) {
      ...
      if (bspLoader.findVectorByName(&playstartf[0],
    ...
    for (int i = 0; i < bspLoader.m_numleafs; i++) {
      printf("Reading bspLeaf %i from total %i (%f procent)\n", i,
             bspLoader.m_numleafs,
             (100.f * (float)i / float(bspLoader.m_numleafs)));
      ...
      for (int b = 0; b < leaf.numLeafBrushes; b++) {
        ...
        if (brush.shaderNum != -1) {
          if (bspLoader.m_dshaders[brush.shaderNum].contentFlags &
              BSPCONTENTS_SOLID) {
            brush.shaderNum = -1;
            ...
            for (int p = 0; p < brush.numSides; p++) {
              ...
              btVector3 planeEq;
              planeEq.setValue(plane.normal[0], plane.normal[1],
                               plane.normal[2]);
            ...
            if (isValidBrush) {
              ...
              btGeometryUtil::getVerticesFromPlaneEquations(planeEquations,
                                                            vertices);
        ...
        if (!strcmp(cl, "trigger_push")) {
          ...
          if (strcmp(cl, "")) {
            ...
            /*
                    ...
                    const BSPEntity* targetentity = bspLoader.getEntityByValue(
               "targetname" , cl );
                    if (targetentity)
                    {
                            if (bspLoader.getVectorForKey( targetentity ,
               "origin",&targetLocation[0]))
                            {
                ...
                if ((modelnr >= 0) && (modelnr < bspLoader.m_nummodels)) {
                  ...
                  for (int n = 0; n < model.numBrushes; n++) {
                    ...
                    const BSPBrush& brush =
                        bspLoader.m_dbrushes[model.firstBrush + n];
                    {
                      for (int p = 0; p < brush.numSides; p++) {
                        int sideid = brush.firstSide + p;
                        BSPBrushSide& brushside =
                            bspLoader.m_dbrushsides[sideid];
                        int planeid = brushside.planeNum;
                        BSPPlane& plane = bspLoader.m_dplanes[planeid];
                        btVector3 planeEq;
                        planeEq.setValue(plane.normal[0], plane.normal[1],
                      ...
                      if (isValidBrush) {
                        ...
                        btGeometryUtil::getVerticesFromPlaneEquations(
                            planeEquations, vertices);

                        bool isEntity = true;
                        addConvexVerticesCollider(vertices, isEntity,
                                                  targetLocation);
                      }
                    }
                  }
