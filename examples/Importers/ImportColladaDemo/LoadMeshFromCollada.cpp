...
struct TokenIntArray {
  ...
  TokenIntArray(btAlignedObjectArray<int>& intArray) : m_values(intArray) {}
  inline void add(const char* token) {
    float v = atoi(token);
    m_values.push_back(v);
  }
...
void tokenize(const std::string& str, AddToken& tokenAdder,
...
void readFloatArray(XMLElement* source, btAlignedObjectArray<float>& floatArray,
                    int& componentStride) {
  int numVals, stride;
  XMLElement* array = source->FirstChildElement("float_array");
  if (array) {
    componentStride = 1;
    if (source->FirstChildElement("technique_common")
            ->FirstChildElement("accessor")
            ->QueryIntAttribute("stride", &stride) != XML_NO_ATTRIBUTE) {
...
void readLibraryGeometries(
    XMLDocument& doc,
    btAlignedObjectArray<GLInstanceGraphicsShape>& visualShapes,
    btHashMap<btHashString, int>& name2Shape, float extraScaling) {
  btHashMap<btHashString, XMLElement*> allSources;
  btHashMap<btHashString, VertexSource> vertexSources;
  for (XMLElement* geometry = doc.RootElement()
    ...
    for (XMLElement* mesh = geometry->FirstChildElement("mesh"); (mesh != NULL);
         mesh = mesh->NextSiblingElement("mesh")) {
      XMLElement* vertices2 = mesh->FirstChildElement("vertices");

      for (XMLElement* source = mesh->FirstChildElement("source");
           source != NULL; source = source->NextSiblingElement("source")) {
      ...
      VertexSource vs;
      for (XMLElement* input = vertices2->FirstChildElement("input");
           input != NULL; input = input->NextSiblingElement("input")) {
      ...
      for (XMLElement* primitive = mesh->FirstChildElement("triangles");
           primitive; primitive = primitive->NextSiblingElement("triangles")) {
        trianglesAndPolylists.push_back(primitive);
      }
      for (XMLElement* primitive = mesh->FirstChildElement("polylist");
           primitive; primitive = primitive->NextSiblingElement("polylist")) {
        trianglesAndPolylists.push_back(primitive);
      }
      ...
      for (int i = 0; i < trianglesAndPolylists.size(); i++) {
        ...
        int numIndices = 0;
        {
          for (XMLElement* input = primitive->FirstChildElement("input");
               input != NULL; input = input->NextSiblingElement("input")) {
            const char* sem = input->Attribute("semantic");
            std::string semName(sem);
            int offset = atoi(input->Attribute("offset"));
            if ((offset + 1) > indexStride) indexStride = offset + 1;
            // printf("sem=%s\n",sem);
            //	const char* src = input->Attribute("source");
            ...
            if (semName == "VERTEX") {
              // now we have POSITION and possibly NORMAL too, using same index
              // array (<p>)
        ...
        for (int index = 0; index < numIndices; index++) {
          ...
          vertexPositions.push_back(
              btVector3(extraScaling * positionFloatArray[posIndex * 3 + 0],
                        extraScaling * positionFloatArray[posIndex * 3 + 1],
                        extraScaling * positionFloatArray[posIndex * 3 + 2]));

          if (normalFloatArray.size() &&
              (normalFloatArray.size() > normalIndex)) {
            vertexNormals.push_back(
                btVector3(normalFloatArray[normalIndex * 3 + 0],
                          normalFloatArray[normalIndex * 3 + 1],
                          normalFloatArray[normalIndex * 3 + 2]));
          } else {
            // add a dummy normal of length zero, so it is easy to detect that
            // it is an invalid normal
    ...
    if (shapeIndex < MAX_VISUAL_SHAPES) {
      GLInstanceGraphicsShape& visualShape = visualShapes.expand();
      {
        ...
        // b3Printf(" index_count =%dand
        // vertexPositions.size=%d\n",indices.size(), vertexPositions.size());
      ...
      b3Warning("DAE exceeds number of visual shapes (%d/%d)", shapeIndex,
                MAX_VISUAL_SHAPES);
    }
...
void readNodeHierarchy(
    XMLElement* node, btHashMap<btHashString, int>& name2Shape,
    btAlignedObjectArray<ColladaGraphicsInstance>& visualShapeInstances,
    const btMatrix4x4& parentTransMat) {
  btMatrix4x4 nodeTrans;
  nodeTrans.setIdentity();

  /// todo(erwincoumans) we probably have to read the elements 'translate',
  /// 'scale', 'rotate' and 'matrix' in-order and accumulate them...
  {
    for (XMLElement* transElem = node->FirstChildElement("matrix"); transElem;
         transElem = node->NextSiblingElement("matrix")) {
      if (transElem->GetText()) {
        ...
        if (floatArray.size() == 16) {
          ...
          b3Warning(
              "Error: expected 16 elements in a <matrix> element, skipping\n");
        }
      }
    }
  }
  ...
  {
    for (XMLElement* transElem = node->FirstChildElement("translate");
         transElem; transElem = node->NextSiblingElement("translate")) {
      if (transElem->GetText()) {
  ...
  }
  {
    for (XMLElement* rotateElem = node->FirstChildElement("rotate");
      ...
      if (rotateElem->GetText()) {
        ...
        btQuaternion orn(btVector3(rotate),
                         btRadians(rotate[3]));  // COLLADA DAE rotate is in
...
void readVisualSceneInstanceGeometries(
    XMLDocument& doc, btHashMap<btHashString, int>& name2Shape,
    btAlignedObjectArray<ColladaGraphicsInstance>& visualShapeInstances) {
  btHashMap<btHashString, XMLElement*> allVisualScenes;

  XMLElement* libVisualScenes =
      doc.RootElement()->FirstChildElement("library_visual_scenes");
  if (libVisualScenes == 0) return;
  ...
  XMLElement* scene = 0;
  {
    ...
    if (scenes) {
      XMLElement* instanceSceneReference =
          scenes->FirstChildElement("instance_visual_scene");
      if (instanceSceneReference) {
        const char* instanceSceneUrl = instanceSceneReference->Attribute("url");
        XMLElement** sceneInstancePtr =
            allVisualScenes[instanceSceneUrl + 1];  // skip #
        if (sceneInstancePtr) {
  ...
  if (scene) {
    for (XMLElement* node = scene->FirstChildElement("node"); node != NULL;
...
void getUnitMeterScalingAndUpAxisTransform(XMLDocument& doc, btTransform& tr,
                                           float& unitMeterScaling,
                                           int clientUpAxis) {
  /// todo(erwincoumans) those up-axis transformations have been quickly coded
  /// without rigorous testing

  XMLElement* unitMeter =
      doc.RootElement()->FirstChildElement("asset")->FirstChildElement("unit");
  if (unitMeter) {
  ...
  XMLElement* upAxisElem =
      doc.RootElement()->FirstChildElement("asset")->FirstChildElement(
          "up_axis");
  if (upAxisElem) {
    switch (clientUpAxis) {
      case 1: {
...
void LoadMeshFromCollada(
  ...
  if (xmlString.size() == 0) return;

  if (doc.Parse(&xmlString[0], xmlString.size()) != XML_SUCCESS)
    // if (doc.LoadFile(filename) != XML_SUCCESS)
    return;

  // We need units to be in meter, so apply a scaling using the asset/units
  // meter
  unitMeterScaling = 1;
  upAxisTransform.setIdentity();

  // Also we can optionally compensate all transforms using the asset/up_axis as
  // well as unit meter scaling
...
static void addMeshParts(const aiScene* scene, const aiNode* node,
                         GLInstanceGraphicsShape* outverts,
...
void LoadMeshFromColladaAssimp(
    const char* relativeFileName,
    btAlignedObjectArray<GLInstanceGraphicsShape>& visualShapes,
    btAlignedObjectArray<ColladaGraphicsInstance>& visualShapeInstances,
    btTransform& upAxisTrans, float& unitMeterScaling) {
  ...
  if (file) {
    int size = 0;
    if (fseek(file, 0, SEEK_END) || (size = ftell(file)) == EOF ||
      ...
      if (size) {
        ...
        aiScene const* scene = importer.ReadFile(
            relativeFileName, aiProcess_JoinIdenticalVertices |
