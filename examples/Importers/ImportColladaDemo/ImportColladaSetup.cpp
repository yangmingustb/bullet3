...
static int ColladaGraphicsInstanceSortfnc(const ColladaGraphicsInstance& a,
                                          const ColladaGraphicsInstance& b) {
...
void ImportColladaSetup::initPhysics() {
  ...
  const char* fileNames[] = {
      "duck.dae", "seymourplane_triangulate.dae",
  };
...

  {
    ...
    if (useAssimp) {
      LoadMeshFromColladaAssimp(relativeFileName, visualShapes,
                                visualShapeInstances, upAxisTrans,
                                unitMeterScaling);
      fileIndex++;
      ...
      LoadMeshFromCollada(relativeFileName, visualShapes, visualShapeInstances,
                          upAxisTrans, unitMeterScaling);
    }
    ...
    LoadMeshFromCollada(relativeFileName, visualShapes, visualShapeInstances,
                        upAxisTrans, unitMeterScaling, upAxis, &fileIO);
#endif  // COMPARE_WITH_ASSIMP

    // at the moment our graphics engine requires instances that share the same
    // visual shape to be added right after registering the shape
    ...
    for (int i = 0; i < visualShapeInstances.size(); i++) {
      ...
      btVector3 position(
          0, 0,
          0);  // =
               // scaling*btVector3(instance->m_pos[0],instance->m_pos[1],instance->m_pos[2]);
      btQuaternion orn(
          0, 0, 0,
          1);  // instance->m_orn[0],instance->m_orn[1],instance->m_orn[2],instance->m_orn[3]);
      ...
      if (gfxShape) {
        ...
        unitMeterScalingMat.setPureScaling(
            btVector3(unitMeterScaling, unitMeterScaling, unitMeterScaling));
        ...
        int shapeId = m_guiHelper->getRenderInterface()->registerShape(
            &verts[0].xyzw[0], gfxShape->m_numvertices,
            &gfxShape->m_indices->at(0), gfxShape->m_numIndices);

        // btVector3
        // instanceScaling(instance->m_scaling[0],instance->m_scaling[1],instance->m_scaling[2]);
...
class CommonExampleInterface* ImportColladaCreateFunc(
