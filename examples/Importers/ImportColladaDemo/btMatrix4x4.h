...
btMatrix4x4 {
 ...
 public:
  btMatrix4x4() {}
  btMatrix4x4(const btScalar& xx, const btScalar& xy, const btScalar& xz,
              const btScalar& xw, const btScalar& yx, const btScalar& yy,
              const btScalar& yz, const btScalar& yw, const btScalar& zx,
              const btScalar& zy, const btScalar& zz, const btScalar& zw,
              const btScalar& wx, const btScalar& wy, const btScalar& wz,
              const btScalar& ww) {
    setValue(xx, xy, xz, xw, yx, yy, yz, yw, zx, zy, zz, zw, wx, wy, wz, ww);
  }
  ...
  SIMD_FORCE_INLINE btScalar tdotw(const btVector4& v) const {
    return m_el[0].w() * v.x() + m_el[1].w() * v.y() + m_el[2].w() * v.z() +
           m_el[3].w() * v.w();
  }

  SIMD_FORCE_INLINE btMatrix4x4& operator*=(const btMatrix4x4& m) {
    setValue(
...
SIMD_FORCE_INLINE btVector3 operator*(const btMatrix4x4& m,
                                      const btVector3& v1) {
  btVector4 v(v1[0], v1[1], v1[2], 1);
  return btVector3(btDot4(m[0], v), btDot4(m[1], v), btDot4(m[2], v));
}

SIMD_FORCE_INLINE btMatrix4x4 operator*(const btMatrix4x4& m1,
                                        btMatrix4x4& m2) {
  return btMatrix4x4(
