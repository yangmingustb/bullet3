...
static bool parseVector3(btVector3& vec3, const std::string& vector_str,
                         MJCFErrorLogger* logger, bool lastThree = false) {
  vec3.setZero();
  btArray<std::string> pieces;
  btArray<float> rgba;
  btAlignedObjectArray<std::string> strArray;
  ...
  if (rgba.size() < 3) {
    logger->reportWarning(
        ("Couldn't parse vector3 '" + vector_str + "'").c_str());
    return false;
  }
  if (lastThree) {
    vec3.setValue(rgba[rgba.size() - 3], rgba[rgba.size() - 2],
                  rgba[rgba.size() - 1]);
  } else {
...
static bool parseVector6(btVector3& v0, btVector3& v1,
                         const std::string& vector_str,
                         MJCFErrorLogger* logger) {
  v0.setZero();
  v1.setZero();
  ...
  if (values.size() < 6) {
    logger->reportWarning(
        ("Couldn't parse 6 floats '" + vector_str + "'").c_str());
...
struct BulletMJCFImporterInternalData {
  ...
  // todo: for better MJCF compatibility, we would need a stack of default
  // values
  MyMJCFDefaults m_globalDefaults;
  b3HashMap<b3HashString, MyMJCFDefaults> m_classDefaults;

  // those collision shapes are deleted by caller (todo: make sure this
  // happens!)
...
#endif
    std::string str =
        m_sourceFileName.c_str() + std::string(":") + std::string(row);
    return str;
#endif
  }
  ...
  const UrdfLink* getLink(int modelIndex, int linkIndex) const {
    if (modelIndex >= 0 && modelIndex < m_models.size()) {
      UrdfLink** linkPtrPtr =
          m_models[modelIndex]->m_links.getAtIndex(linkIndex);
  ...
  void parseCompiler(XMLElement* root_xml, MJCFErrorLogger* logger) {
    ...
    m_angleUnits =
        angle ? angle
              : "degree";  // degrees by default,
                           // http://www.mujoco.org/book/modeling.html#compiler
    const char* inertiaFromGeom = root_xml->Attribute("inertiafromgeom");
    if (inertiaFromGeom &&
        inertiaFromGeom[0] == 'f')  // false, other values assumed `true`.
    {
  ...
  void parseAssets(XMLElement* root_xml, MJCFErrorLogger* logger) {
    ...
    for (XMLElement* child_xml = root_xml->FirstChildElement(); child_xml;
         child_xml = child_xml->NextSiblingElement()) {
  ...
  bool parseDefaults(MyMJCFDefaults& defaults, XMLElement* root_xml,
                     MJCFErrorLogger* logger) {
    bool handled = false;
    // rudimentary 'default' support, would need more work for better feature
    // coverage
    for (XMLElement* child_xml = root_xml->FirstChildElement(); child_xml;
         child_xml = child_xml->NextSiblingElement()) {
      std::string n = child_xml->Value();
      ...
      if (n == "geom") {
        ...
        if (conAffinityStr) {
          defaults.m_defaultCollisionMask =
              urdfLexicalCast<int>(conAffinityStr);
        }
  ...
  bool parseRootLevel(MyMJCFDefaults& defaults, XMLElement* root_xml,
                      MJCFErrorLogger* logger) {
    for (XMLElement* rootxml = root_xml->FirstChildElement(); rootxml;
      ...
      if (n == "geom") {
        ...
        parseGeom(defaults, rootxml, modelIndex, linkIndex, logger,
                  inertialShift);
      ...
      if (!handled) {
        logger->reportWarning((sourceFileLocation(rootxml) +
                               ": unhandled root element '" + n + "'")
                                  .c_str());
      }
    }
    return true;
  ...
  bool parseJoint(MyMJCFDefaults& defaults, XMLElement* link_xml,
                  int modelIndex, int parentLinkIndex, int linkIndex,
                  MJCFErrorLogger* logger, const btTransform& parentToLinkTrans,
                  btTransform& jointTransOut) {
    ...
    if (axisStr) {
      ...
      logger->reportWarning(
          (sourceFileLocation(link_xml) + ": joint without axis attribute")
              .c_str());
    }
    ...
    if (jType) {
      ...
      logger->reportWarning((sourceFileLocation(link_xml) +
                             ": expected 'type' attribute for joint")
                                .c_str());
    }
    ...
    if (isLimited) {
      ...
      if (limits.size() == 2) {
        ...
        logger->reportWarning((sourceFileLocation(link_xml) +
                               ": cannot parse 'range' attribute (units='" +
                               m_angleUnits + "'')")
                                  .c_str());
      }
    }
    ...
    // degrees of freedom created by this joint. These are constants added to
    // the
    // diagonal of the inertia matrix in generalized coordinates. They make the
    // simulation more stable, and often increase physical realism. This is
    // because
    // when a motor is attached to the system with a transmission that amplifies
    // the motor force by c, the inertia of the rotor (i.e. the moving part of
    // the
    // motor) is amplified by c*c. The same holds for gears in the early stages
    // of
  ...
  bool parseGeom(MyMJCFDefaults& defaults, XMLElement* link_xml, int modelIndex,
                 int linkIndex, MJCFErrorLogger* logger,
                 btVector3& inertialShift) {
    ...
    if (conDim > 3) {
      linkPtr->m_contactInfo.m_spinningFriction =
          defaults.m_defaultSpinningFriction;
      linkPtr->m_contactInfo.m_flags |= URDF_CONTACT_HAS_SPINNING_FRICTION;
    }
    if (conDim > 4) {
      linkPtr->m_contactInfo.m_rollingFriction =
          defaults.m_defaultRollingFriction;
      linkPtr->m_contactInfo.m_flags |= URDF_CONTACT_HAS_ROLLING_FRICTION;
    }

    {
      geom.m_localMaterial.m_matColor.m_rgbaColor =
          sGoogleColors[linkIndex & 3];
    ...
    if (gType) {
      ...
      if (geomType == "sphere") {
        ...
        if (sz) {
          ...
          logger->reportWarning((sourceFileLocation(link_xml) +
                                 ": no size field (scalar) in sphere geom")
                                    .c_str());
        }
        handledGeomType = true;
      }
      ...
      if (geomType == "capsule" || geomType == "cylinder") {
        // <geom conaffinity="0" contype="0" fromto="0 0 0 0 0 0.02" name="root"
        // rgba="0.9 0.4 0.6 1" size=".011" type="cylinder"/>
        geom.m_type =
            geomType == "cylinder" ? URDF_GEOM_CYLINDER : URDF_GEOM_CAPSULE;
        ...
        geom.m_capsuleRadius =
            2.00f;  // 2 to make it visible if something is wrong
        ...
        if (sizes.size() > 0) {
          ...
          logger->reportWarning(
              (sourceFileLocation(link_xml) +
               ": couldn't convert 'size' attribute of capsule geom")
                  .c_str());
        }
        ...
        if (fromtoStr) {
          ...
          if (sizes.size() < 2) {
            logger->reportWarning((sourceFileLocation(link_xml) +
                                   ": capsule without fromto attribute "
                                   "requires 2 sizes (radius and halfheight)")
                                      .c_str());
          } else {
            handledGeomType = true;
          }
      ...
      if (geomType == "mesh") {
        ...
        if (meshStr) {
          ...
          if (assetPtr) {
            ...
            bool exists = UrdfFindMeshFile(
                m_fileIO, m_sourceFileName, assetPtr->m_fileName,
                sourceFileLocation(link_xml), &geom.m_meshFileName,
                &geom.m_meshFileType);
      ...
      if (handledGeomType) {
        ...
        logger->reportWarning((sourceFileLocation(link_xml) +
                               ": unhandled geom type '" + geomType + "'")
                                  .c_str());
      }
    } else {
      logger->reportWarning(
          (sourceFileLocation(link_xml) + ": geom requires type").c_str());
    }
  ...
  btTransform parseTransform(XMLElement* link_xml, MJCFErrorLogger* logger) {
    ...
    if (p) {
      ...
      //			logger->reportWarning("body should have pos
      //attribute");
    }
    ...
    if (o) {
      ...
      if (parseVector4(o4, ornstr)) {
        orn.setValue(
            o4[1], o4[2], o4[3],
            o4[0]);  // MuJoCo quats are [w,x,y,z], Bullet uses [x,y,z,w]
        tr.setRotation(orn);
      }
    } else {
      //			logger->reportWarning("body doesn't have quat
      ...
      switch (col->m_geometry.m_type) {
        ...
        case URDF_GEOM_CAPSULE: {
          ...
          if (col->m_geometry.m_hasFromTo) {
            ...
            h = (col->m_geometry.m_capsuleFrom - col->m_geometry.m_capsuleTo)
                    .length();
          } else {
  ...
  bool parseBody(MyMJCFDefaults& defaults, XMLElement* link_xml, int modelIndex,
                 int orgParentLinkIndex, MJCFErrorLogger* logger) {
    ...
    for (XMLElement* xml = link_xml->FirstChildElement(); xml;
         xml = xml->NextSiblingElement()) {
      bool handled = false;
      std::string n = xml->Value();
      if (n == "inertial") {
        //   <inertial pos="0 0 0" quat="0.5 0.5 -0.5 0.5" mass="297.821"
        //   diaginertia="109.36 69.9714 69.9714" />
      ...
      if (n == "joint") {
        if (!hasJoint) {
          ...
          if (newParentLinkIndex != INVALID_LINK_INDEX || jointType != "free") {
            ...
            parseJoint(curDefaults, xml, modelIndex, newParentLinkIndex,
                       newLinkIndex, logger, linkTransform, jointTrans);

            // getLink(modelIndex,newLinkIndex)->m_linkTransformInWorld =
            // jointTrans*linkTransform;
          ...
          btTransform unused;
          parseJoint(curDefaults, xml, modelIndex, newParentLinkIndex,
                     newLinkIndex, logger, joint2nextjoint, unused);
          newParentLinkIndex = newLinkIndex;
          // todo: compute relative joint transforms (if any) and append to
          // linkTransform
          hasJoint = true;
          handled = true;
        }
      ...
      if (n == "geom") {
        btVector3 inertialShift(0, 0, 0);
        parseGeom(curDefaults, xml, modelIndex, orgChildLinkIndex, logger,
                  inertialShift);
        if (!massDefined) {
      ...
      if (!handled) {
        logger->reportWarning(
            (sourceFileLocation(xml) + ": unknown field '" + n + "'").c_str());
      }
    }
    ...
    linkPtr->m_inertia.m_linkLocalFrame =
        localInertialFrame;  // = jointTrans.inverse();
    linkPtr->m_inertia.m_mass = mass;
    return true;
  }
  ...
  bool initTreeAndRoot(UrdfModel& model, MJCFErrorLogger* logger) {
    ...
    for (int i = 0; i < model.m_joints.size(); i++) {
      ...
      if (jointPtr) {
        ...
        UrdfLink** childLinkPtr =
            model.m_links.find(joint->m_childLinkName.c_str());
        if (!childLinkPtr) {
        ...
        UrdfLink** parentLinkPtr =
            model.m_links.find(joint->m_parentLinkName.c_str());
        if (!parentLinkPtr) {
        ...
        parentLinkTree.insert(childLink->m_name.c_str(),
                              parentLink->m_name.c_str());
      }
    }
    ...
    // re-index the link indices so parent indices are always smaller than child
    // indices
...
BulletMJCFImporter::BulletMJCFImporter(struct GUIHelperInterface* helper,
                                       UrdfRenderingInterface* customConverter,
...
BulletMJCFImporter::~BulletMJCFImporter() { delete m_data; }

bool BulletMJCFImporter::loadMJCF(const char* fileName, MJCFErrorLogger* logger,
                                  bool forceFixedBase) {
  ...
  bool fileFound = (m_data->m_fileIO->findResourcePath(
                        fileName, relativeFileName, 1024) > 0);
...
bool BulletMJCFImporter::parseMJCFString(const char* xmlText,
                                         MJCFErrorLogger* logger) {
  XMLDocument xml_doc;
  ...
  for (XMLElement* link_xml = mujoco_xml->FirstChildElement("default");
       link_xml; link_xml = link_xml->NextSiblingElement("default")) {
    m_data->parseDefaults(m_data->m_globalDefaults, link_xml, logger);
  }

  for (XMLElement* link_xml = mujoco_xml->FirstChildElement("compiler");
       link_xml; link_xml = link_xml->NextSiblingElement("compiler")) {
    m_data->parseCompiler(link_xml, logger);
  }

  for (XMLElement* link_xml = mujoco_xml->FirstChildElement("asset"); link_xml;
       link_xml = link_xml->NextSiblingElement("asset")) {
    m_data->parseAssets(link_xml, logger);
  }

  for (XMLElement* link_xml = mujoco_xml->FirstChildElement("body"); link_xml;
       link_xml = link_xml->NextSiblingElement("body")) {
    m_data->parseRootLevel(m_data->m_globalDefaults, link_xml, logger);
  }

  for (XMLElement* link_xml = mujoco_xml->FirstChildElement("worldbody");
       link_xml; link_xml = link_xml->NextSiblingElement("worldbody")) {
    m_data->parseRootLevel(m_data->m_globalDefaults, link_xml, logger);
  }
...
const char* BulletMJCFImporter::getPathPrefix() { return m_data->m_pathPrefix; }

int BulletMJCFImporter::getRootLinkIndex() const {
  if (m_data->m_activeModel >= 0 &&
      m_data->m_activeModel < m_data->m_models.size()) {
...
bool BulletMJCFImporter::getLinkColor2(int linkIndex,
      ...
      if (!hasLinkColor) {
        for (int i = 0; i < link->m_collisionArray.size(); i++) {
          if (link->m_collisionArray[i].m_geometry.m_hasLocalMaterial) {
            matCol =
                link->m_collisionArray[0].m_geometry.m_localMaterial.m_matColor;
            hasLinkColor = true;
          }
          break;
        }
...
bool BulletMJCFImporter::getLinkColor(int linkIndex,
...
// MuJoCo type/affinity is different from Bullet group/mask, so we should
// implement a custom collision filter instead
//(contype1 & conaffinity2) || (contype2 & conaffinity1)
int BulletMJCFImporter::getCollisionGroupAndMask(int linkIndex, int& colGroup,
                                                 int& colMask) const {
  int flags = 0;
...
void BulletMJCFImporter::getMassAndInertia(int urdfLinkIndex, btScalar& mass,
...
void BulletMJCFImporter::getLinkChildIndices(
...
bool BulletMJCFImporter::getJointInfo(
    int urdfLinkIndex, btTransform& parent2joint,
    btTransform& linkTransformInWorld, btVector3& jointAxisInJointSpace,
    int& jointType, btScalar& jointLowerLimit, btScalar& jointUpperLimit,
...
bool BulletMJCFImporter::getRootTransformInWorld(
  ...
  switch (visual->m_geometry.m_type) {
    case URDF_GEOM_CAPSULE: {
      ...
      for (int i = 0; i < numVertices; i++) {
        ...
        btScalar u = btAtan2(transformedVertices[i].normal[0],
                             transformedVertices[i].normal[2]) /
                         (2 * SIMD_PI) +
                     0.5;
      ...
      if (visual->m_geometry.m_hasFromTo) {
        ...
        btCapsuleShapeZ* cap =
            new btCapsuleShapeZ(visual->m_geometry.m_capsuleRadius,
    ...
    case URDF_GEOM_CYLINDER: {
      ...
      for (int i = 0; i < numSteps; i++) {
        ...
        btVector3 vert(cylRadius * btSin(SIMD_2_PI * (float(i) / numSteps)),
                       cylRadius * btCos(SIMD_2_PI * (float(i) / numSteps)),
                       cylLength / 2.);
    ...
    case URDF_GEOM_BOX: {
      ...
      convexColShape->setMargin(
          m_data->m_globalDefaults.m_defaultCollisionMargin);
      break;
    }
      ...
      switch (visual->m_geometry.m_meshFileType) {
        case UrdfGeometry::FILE_OBJ: {
          ...
          if (b3ImportMeshUtility::loadAndRegisterMeshFromFileInternal(
                  visual->m_geometry.m_meshFileName, meshData,
                  m_data->m_fileIO)) {
        ...
        case UrdfGeometry::FILE_COLLADA: {
          ...
          LoadMeshFromCollada(visual->m_geometry.m_meshFileName.c_str(),
                              ...
                              unitMeterScaling, upAxis, m_data->m_fileIO);
...
int BulletMJCFImporter::convertLinkVisualShapes(
    int linkIndex, const char* pathPrefix,
    const btTransform& inertialFrame) const {
  int graphicsIndex = -1;
  if (m_data->m_flags & CUF_MJCF_COLORS_FROM_FILE) {
    ...
    if (linkPtr) {
      ...
      for (int v = 0; v < link->m_visualArray.size(); v++) {
        ...
        convertURDFToVisualShapeInternal(&vis, pathPrefix,
                                         inertialFrame.inverse() * childTrans,
...
bool BulletMJCFImporter::getLinkContactInfo(
    int linkIndex, URDFLinkContactInfo& contactInfo) const {
  const UrdfLink* link = m_data->getLink(m_data->m_activeModel, linkIndex);
  if (link) {
...
void BulletMJCFImporter::convertLinkVisualShapes2(
    int linkIndex, int urdfIndex, const char* pathPrefix,
    const btTransform& inertialFrame, class btCollisionObject* colObj,
    int objectIndex) const {
  if (m_data->m_customVisualShapesConverter) {
    const UrdfLink* link = m_data->getLink(m_data->m_activeModel, urdfIndex);
    int uid3 = m_data->m_customVisualShapesConverter->convertVisualShapes(
        linkIndex, pathPrefix, inertialFrame, link, 0,
        colObj->getBroadphaseHandle()->getUid(), objectIndex, m_data->m_fileIO);
...
static btCollisionShape* MjcfCreateConvexHullFromShapes(
    const tinyobj::attrib_t& attribute, std::vector<tinyobj::shape_t>& shapes,
    const btVector3& geomScale, btScalar collisionMargin) {
  ...
  for (int s = 0; s < (int)shapes.size(); s++) {
    ...
    for (int f = 0; f < faceCount; f += 3) {
      btVector3 pt;
      pt.setValue(
          attribute.vertices[3 * shape.mesh.indices[f].vertex_index + 0],
...
class btCompoundShape* BulletMJCFImporter::convertLinkCollisionShapes(
    int linkIndex, const char* pathPrefix,
    ...
    for (int i = 0; i < link->m_collisionArray.size(); i++) {
      ...
      switch (col->m_geometry.m_type) {
        ...
        case URDF_GEOM_CYLINDER: {
          if (col->m_geometry.m_hasFromTo) {
            ...
            btCylinderShapeZ* cyl = new btCylinderShapeZ(btVector3(
                col->m_geometry.m_capsuleRadius,
                col->m_geometry.m_capsuleRadius, btScalar(0.5) * height));
            ...
            btCylinderShapeZ* cap = new btCylinderShapeZ(
                btVector3(col->m_geometry.m_capsuleRadius,
                          col->m_geometry.m_capsuleRadius,
                          btScalar(0.5) * col->m_geometry.m_capsuleHeight));
            childShape = cap;
          }
          break;
        }
        case URDF_GEOM_MESH: {
          ...
          switch (col->m_geometry.m_meshFileType) {
            case UrdfGeometry::FILE_OBJ: {
              if (col->m_flags & URDF_FORCE_CONCAVE_TRIMESH) {
                glmesh = LoadMeshFromObj(col->m_geometry.m_meshFileName.c_str(),
                                         0, m_data->m_fileIO);
              } else {
                std::vector<tinyobj::shape_t> shapes;
                tinyobj::attrib_t attribute;
                std::string err = tinyobj::LoadObj(
                    attribute, shapes, col->m_geometry.m_meshFileName.c_str(),
                    "", m_data->m_fileIO);
                // create a convex hull for each shape, and store it in a
                // btCompoundShape

                childShape = MjcfCreateConvexHullFromShapes(
                    attribute, shapes, col->m_geometry.m_meshScale,
                    m_data->m_globalDefaults.m_defaultCollisionMargin);
              }
              break;
            }
            ...
            default:
              b3Warning(
                  ...
                  col->m_sourceFileLocation.c_str(),
                  col->m_geometry.m_meshFileType);
          }
          ...
          if (childShape) {
            ...
            b3Warning("%s: cannot extract anything useful from mesh '%s'\n",
                      col->m_sourceFileLocation.c_str(),
                      col->m_geometry.m_meshFileName.c_str());
          } else {
            // b3Printf("extracted %d verticed from STL file %s\n",
            // glmesh->m_numvertices,fullPath);
            // int shapeId =
            // m_glApp->m_instancingRenderer->registerShape(&gvertices[0].pos[0],gvertices.size(),&indices[0],indices.size());
            // convex->setUserIndex(shapeId);
            btAlignedObjectArray<btVector3> convertedVerts;
            convertedVerts.reserve(glmesh->m_numvertices);
            for (int i = 0; i < glmesh->m_numvertices; i++) {
              convertedVerts.push_back(
                  btVector3(glmesh->m_vertices->at(i).xyzw[0] *
                                col->m_geometry.m_meshScale[0],
                            glmesh->m_vertices->at(i).xyzw[1] *
                                col->m_geometry.m_meshScale[1],
                            glmesh->m_vertices->at(i).xyzw[2] *
                                col->m_geometry.m_meshScale[2]));
            }
            ...
            if (col->m_flags & URDF_FORCE_CONCAVE_TRIMESH) {
              ...
              for (int i = 0; i < glmesh->m_numIndices / 3; i++) {
                float* v0 =
                    glmesh->m_vertices->at(glmesh->m_indices->at(i * 3)).xyzw;
                float* v1 =
                    glmesh->m_vertices->at(glmesh->m_indices->at(i * 3 + 1))
                        .xyzw;
                float* v2 =
                    glmesh->m_vertices->at(glmesh->m_indices->at(i * 3 + 2))
                        .xyzw;
              ...
              btConvexHullShape* convexHull = new btConvexHullShape(
                  &convertedVerts[0].getX(), convertedVerts.size(),
                  sizeof(btVector3));
              convexHull->optimizeConvexHull();
              // convexHull->initializePolyhedralFeatures();
              convexHull->setMargin(
                  m_data->m_globalDefaults.m_defaultCollisionMargin);
        ...
        case URDF_GEOM_CAPSULE: {
          if (col->m_geometry.m_hasFromTo) {
            if (m_data->m_flags & CUF_USE_IMPLICIT_CYLINDER) {
              ...
              btCapsuleShapeZ* capsule =
                  new btCapsuleShapeZ(col->m_geometry.m_capsuleRadius, height);
            ...
            btCapsuleShapeZ* cap =
                new btCapsuleShapeZ(col->m_geometry.m_capsuleRadius,
      ...
      if (childShape) {
        ...
        compound->addChildShape(
            localInertiaFrame.inverse() * col->m_linkLocalFrame, childShape);
      }
    }
  }
  return compound;
...
class btCollisionShape* BulletMJCFImporter::getAllocatedCollisionShape(
    int index) {
...
btStridingMeshInterface* BulletMJCFImporter::getAllocatedMeshInterface(
    int index) {
  return m_data->m_allocatedMeshInterfaces[index];
}

int BulletMJCFImporter::getNumModels() const { return m_data->m_models.size(); }
