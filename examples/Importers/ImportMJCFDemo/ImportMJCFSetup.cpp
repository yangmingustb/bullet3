...
class ImportMJCFSetup : public CommonMultiBodyBase {
 ...
 public:
  ImportMJCFSetup(struct GUIHelperInterface* helper, int option,
                  const char* fileName);
...
struct ImportMJCFInternalData {
  ImportMJCFInternalData() : m_numMotors(0), m_mb(0) {
...
ImportMJCFSetup::ImportMJCFSetup(struct GUIHelperInterface* helper, int option,
                                 const char* fileName)
    : CommonMultiBodyBase(helper), m_grav(-10), m_upAxis(2) {
  ...
  if (fileName) {
    ...
    if (f) {
      int result;
      // warning: we don't avoid string buffer overflow in this basic example in
      // fscanf
      char fileName[1024];
      do {
...
void ImportMJCFSetup::initPhysics() {
  ...
  // MuJoCo uses a slightly different collision filter mode, use the
  // FILTER_GROUPAMASKB_OR_GROUPBMASKA2
  //@todo also use the modified collision filter for raycast and other collision
  //related queries
  ...
  if (result) {
    ...
    for (int m = 0; m < importer.getNumModels(); m++) {
      ...
      // normally used with PhysicsServerCommandProcessor that allocates unique
      // ids to multibodies,
      ...
      ConvertURDF2Bullet(importer, creation, rootTrans, m_dynamicsWorld,
                         m_useMultiBody, importer.getPathPrefix(),
                         CUF_USE_MJCF);
      ...
      if (mb) {
        ...
        for (int i = 0; i < numLinks; i++) {
          ...
          std::string* jointName =
              new std::string(importer.getJointName(urdfLinkIndex));
          std::string* linkName =
              new std::string(importer.getLinkName(urdfLinkIndex).c_str());
#ifdef TEST_MULTIBODY_SERIALIZATION
          ...
          mb->getLinkCollider(i)->setCollisionFlags(
              mb->getBaseCollider()->getCollisionFlags() |
              btCollisionObject::CF_HAS_FRICTION_ANCHOR);
          ...
          m_data->m_mb = mb;
          if (mb->getLink(mbLinkIndex).m_jointType ==
                  btMultibodyLink::eRevolute ||
              mb->getLink(mbLinkIndex).m_jointType ==
                  btMultibodyLink::ePrismatic) {
            if (m_data->m_numMotors < MAX_NUM_MOTORS) {
              char motorName[1024];
              sprintf(motorName, "%s q ", jointName->c_str());
              btScalar* motorPos =
                  &m_data->m_motorTargetPositions[m_data->m_numMotors];
              *motorPos = 0.f;
              ...
              btMultiBodyJointMotor* motor = new btMultiBodyJointMotor(
                  mb, mbLinkIndex, 0, 0, maxMotorImpulse);
              motor->setErp(0.1);
        ...
        if (1) {
          ...
          for (int i = 0; i < num6Dof; i++) {
            ...
            if (c->getUserConstraintPtr()) {
              GenericConstraintUserInfo* jointInfo =
                  (GenericConstraintUserInfo*)c->getUserConstraintPtr();
  ...
  if (m_dynamicsWorld) {
    ...
    for (int i = 0; i < m_data->m_numMotors; i++) {
      ...
      if (m_data->m_generic6DofJointMotors[i]) {
        GenericConstraintUserInfo* jointInfo =
            (GenericConstraintUserInfo*)m_data->m_generic6DofJointMotors[i]
                ->getUserConstraintPtr();
        m_data->m_generic6DofJointMotors[i]->setTargetVelocity(
            jointInfo->m_jointAxisIndex, m_data->m_motorTargetPositions[i]);
      }
    }

    // the maximal coordinates/iterative MLCP solver requires a smallish
    // timestep to converge
    m_dynamicsWorld->stepSimulation(deltaTime, 10, 1. / 240.);
  }
}

class CommonExampleInterface* ImportMJCFCreateFunc(
    struct CommonExampleOptions& options) {
  return new ImportMJCFSetup(options.m_guiHelper, options.m_option,
                             options.m_fileName);
}
