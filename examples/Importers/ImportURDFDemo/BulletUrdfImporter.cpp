...
struct BulletErrorLogger : public ErrorLogger {
  ...
  BulletErrorLogger() : m_numErrors(0), m_numWarnings(0) {}
  ...
  if (xml_string.length()) {
    ...
    if (m_data->m_flags & CUF_IGNORE_VISUAL_SHAPES) {
      for (int i = 0; i < m_data->m_urdfParser.getModel().m_links.size(); i++) {
        UrdfLink* linkPtr =
            *m_data->m_urdfParser.getModel().m_links.getAtIndex(i);
    ...
    if (m_data->m_urdfParser.getModel().m_rootLinks.size()) {
      if (m_data->m_flags & CUF_MERGE_FIXED_LINKS) {
        m_data->m_urdfParser.mergeFixedLinks(
            m_data->m_urdfParser.getModel(),
            m_data->m_urdfParser.getModel().m_rootLinks[0], &loggie,
            forceFixedBase, 0);
        m_data->m_urdfParser.getModel().m_links.clear();
        m_data->m_urdfParser.getModel().m_joints.clear();
        m_data->m_urdfParser.recreateModel(
            m_data->m_urdfParser.getModel(),
            m_data->m_urdfParser.getModel().m_rootLinks[0], &loggie);
      }
      if (m_data->m_flags & CUF_PRINT_URDF_INFO) {
        m_data->m_urdfParser.printTree(
            m_data->m_urdfParser.getModel().m_rootLinks[0], &loggie, 0);
      }
    }
  }
...
const char* BulletURDFImporter::getPathPrefix() { return m_data->m_pathPrefix; }

void BulletURDFImporter::setBodyUniqueId(int bodyId) {
  m_data->m_bodyId = bodyId;
}

int BulletURDFImporter::getBodyUniqueId() const { return m_data->m_bodyId; }

BulletURDFImporter::~BulletURDFImporter() { delete m_data; }
...
void BulletURDFImporter::getLinkChildIndices(
...
void BulletURDFImporter::getMassAndInertia2(int urdfLinkIndex, btScalar& mass,
...
void BulletURDFImporter::getMassAndInertia(int linkIndex, btScalar& mass,
                                           btVector3& localInertiaDiagonal,
...
bool BulletURDFImporter::getJointInfo2(
...
bool BulletURDFImporter::getJointInfo(
    int urdfLinkIndex, btTransform& parent2joint,
    btTransform& linkTransformInWorld, btVector3& jointAxisInJointSpace,
    int& jointType, btScalar& jointLowerLimit, btScalar& jointUpperLimit,
...
int BulletURDFImporter::getUrdfFromCollisionShape(
...
btCollisionShape* BulletURDFImporter::convertURDFToCollisionShape(
    const UrdfCollision* collision, const char* urdfPathPrefix) const {
  ...
  switch (collision->m_geometry.m_type) {
    ...
    case URDF_GEOM_CDF: {
      ...
      pathPrefix[0] = 0;
      if (m_data->m_fileIO->findResourcePath(
              collision->m_geometry.m_meshFileName.c_str(), relativeFileName,
              1024)) {
    ...
    case URDF_GEOM_MESH: {
      ...
      switch (collision->m_geometry.m_meshFileType) {
          ...
          if (collision->m_flags & URDF_FORCE_CONCAVE_TRIMESH) {
            ...
            glmesh =
                LoadMeshFromObj(collision->m_geometry.m_meshFileName.c_str(),
                                pathPrefix, m_data->m_fileIO);
          } else {
            std::vector<tinyobj::shape_t> shapes;
            tinyobj::attrib_t attribute;
            std::string err = tinyobj::LoadObj(
                attribute, shapes, collision->m_geometry.m_meshFileName.c_str(),
                "", m_data->m_fileIO);
            // create a convex hull for each shape, and store it in a
            // btCompoundShape
            shape = createConvexHullFromShapes(
                attribute, shapes, collision->m_geometry.m_meshScale,
                m_data->m_flags);
        ...
        case UrdfGeometry::FILE_COLLADA: {
          ...
          LoadMeshFromCollada(collision->m_geometry.m_meshFileName.c_str(),
                              visualShapes, visualShapeInstances, upAxisTrans,
                              unitMeterScaling, 2, m_data->m_fileIO);
          ...
          for (int i = 0; i < visualShapeInstances.size(); i++) {
            ...
            btMatrix4x4 worldMat =
                unitMeterScalingMat * instance->m_worldTransform * upAxisMat;
      ...
      if (collision->m_flags & URDF_FORCE_CONCAVE_TRIMESH) {
        ...
        btConvexHullShape* convexHull =
            new btConvexHullShape(&convertedVerts[0].getX(),
                                  convertedVerts.size(), sizeof(btVector3));
        convexHull->optimizeConvexHull();
        if (m_data->m_flags & CUF_INITIALIZE_SAT_FEATURES) {
          convexHull->initializePolyhedralFeatures();
        }
...
void BulletURDFImporter::convertURDFToVisualShapeInternal(
    ...
    btAlignedObjectArray<BulletURDFTexture>& texturesOut,
    struct b3ImportMeshData& meshData) const {
  ...
  if ((glmesh == 0) && convexColShape) {
    ...
    hull->buildHull(0.0);
    {
      ...
      for (int i = 0; i < numVertices; i++) {
        ...
        vtx.normal[2] = pos.z();
        btScalar u =
            btAtan2(vtx.normal[0], vtx.normal[2]) / (2 * SIMD_PI) + 0.5;
...
int BulletURDFImporter::convertLinkVisualShapes(
  ...
  if (linkPtr) {
    ...
    for (int v = 0; v < link->m_visualArray.size(); v++) {
      ...
      convertURDFToVisualShapeInternal(&vis, pathPrefix,
                                       localInertiaFrame.inverse() * childTrans,
  ...
  if (vertices.size() && indices.size()) {
    //		graphicsIndex  =
    //m_data->m_guiHelper->registerGraphicsShape(&vertices[0].xyzw[0],
    ...
    // m_data->m_guiHelper->getRenderInterface();
    ...
    if (1) {
      ...
      }
      {
        ...
        graphicsIndex = m_data->m_guiHelper->registerGraphicsShape(
            &vertices[0].xyzw[0], vertices.size(), &indices[0], indices.size(),
...
bool BulletURDFImporter::getLinkColor(int linkIndex,
...
bool BulletURDFImporter::getLinkColor2(int linkIndex,
...
void BulletURDFImporter::setLinkColor2(int linkIndex,
                                       struct UrdfMaterialColor& matCol) const {
  m_data->m_linkColors.insert(linkIndex, matCol);
}

bool BulletURDFImporter::getLinkContactInfo(
...
bool BulletURDFImporter::getLinkAudioSource(int linkIndex,
  ...
  if (m_data->m_enableTinyRenderer && m_data->m_customVisualShapesConverter) {
    ...
    if (linkPtr) {
      ...
      int uid3 = m_data->m_customVisualShapesConverter->convertVisualShapes(
          ...
          colObj->getBroadphaseHandle()->getUid(), bodyUniqueId,
          m_data->m_fileIO);
...
btStridingMeshInterface* BulletURDFImporter::getAllocatedMeshInterface(
    int index) {
...
int BulletURDFImporter::getCollisionGroupAndMask(int linkIndex, int& colGroup,
                                                 int& colMask) const {
  int result = 0;
  UrdfLink* const* linkPtr =
      m_data->m_urdfParser.getModel().m_links.getAtIndex(linkIndex);
  ...
  if (linkPtr) {
    ...
    for (int v = 0; v < link->m_collisionArray.size(); v++) {
      ...
      btCollisionShape* childShape =
          convertURDFToCollisionShape(&col, pathPrefix);
      if (childShape) {
        ...
        if (childShape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE) {
          ...
          for (int i = 0; i < compound->getNumChildShapes(); i++) {
            m_data->m_allocatedCollisionShapes.push_back(
        ...
        compoundShape->addChildShape(localInertiaFrame.inverse() * childTrans,
                                     childShape);
      }
    }
  }
