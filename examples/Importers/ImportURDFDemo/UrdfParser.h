...
class UrdfParser {
 protected:
  ...
  bool parseTransform(btTransform& tr, tinyxml2::XMLElement* xml,
                      ErrorLogger* logger, bool parseSDF = false);
  ...
  bool parseGeometry(UrdfGeometry& geom, tinyxml2::XMLElement* g,
                     ErrorLogger* logger);
  bool parseVisual(UrdfModel& model, UrdfVisual& visual,
                   tinyxml2::XMLElement* config, ErrorLogger* logger);
  ...
  bool parseLink(UrdfModel& model, UrdfLink& link, tinyxml2::XMLElement* config,
                 ErrorLogger* logger);
  bool parseSensor(UrdfModel& model, UrdfLink& link, UrdfJoint& joint,
                   tinyxml2::XMLElement* config, ErrorLogger* logger);
 ...
 public:
  ...
  bool mergeFixedLinks(UrdfModel& model, UrdfLink* link, ErrorLogger* logger,
                       bool forceFixedBase, int level);
