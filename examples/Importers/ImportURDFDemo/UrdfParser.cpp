...
UrdfParser::UrdfParser(CommonFileIOInterface* fileIO)
    : m_parseSDF(false),
      ...
      m_urdfScaling(1),
      m_fileIO(fileIO) {
  m_urdf2Model.m_sourceFile = "IN_MEMORY_STRING";  // if loadUrdf() called
                                                   // later, source file name
...
static bool parseVector3(btVector3& vec3, const std::string& vector_str,
                         ErrorLogger* logger, bool lastThree = false) {
  vec3.setZero();
  btArray<std::string> pieces;
  btArray<double> rgba;
  btAlignedObjectArray<std::string> strArray;
...
// </bullet>
static void ParseUserData(const XMLElement* element,
                          btHashMap<btHashString, std::string>& user_data,
                          ErrorLogger* logger) {
  ...
  for (const XMLElement* bullet_xml = element->FirstChildElement("bullet");
    ...
    for (const XMLElement* user_data_xml =
             bullet_xml->FirstChildElement("user-data");
         user_data_xml;
         user_data_xml = user_data_xml->NextSiblingElement("user-data")) {
...
bool UrdfParser::parseMaterial(UrdfMaterial& material, XMLElement* config,
  ...
  // color
  {
    ...
    if (c) {
      if (c->Attribute("rgba")) {
        if (!parseVector4(material.m_matColor.m_rgbaColor,
                          c->Attribute("rgba"))) {
...
bool UrdfParser::parseTransform(btTransform& tr, XMLElement* xml,
                                ErrorLogger* logger, bool parseSDF) {
  tr.setIdentity();
  ...
  if (m_parseSDF) {
    ...
    if (ixx && ixy && ixz && iyy && iyz && izz) {
      ...
      if (ixx && iyy && izz) {
        ...
        logger->reportError(
            "Inertial: inertia element must have ixx,ixy,ixz,iyy,iyz,izz child "
            "elements");
        return false;
      }
    }
    ...
    if (!(inertia_xml->Attribute("ixx") && inertia_xml->Attribute("ixy") &&
      ...
      if ((inertia_xml->Attribute("ixx") && inertia_xml->Attribute("iyy") &&
        ...
        logger->reportError(
            "Inertial: inertia element must have ixx,ixy,ixz,iyy,iyz,izz "
            "attributes");
        return false;
      }
    } else {
  ...
  if (type_name == "sphere") {
    ...
    if (m_parseSDF) {
      ...
      if (!shape->Attribute("radius")) {
        ...
        geom.m_sphereRadius =
            m_urdfScaling * urdfLexicalCast<double>(shape->Attribute("radius"));
      }
    }
    ...
    if (m_parseSDF) {
      if (XMLElement* scale = shape->FirstChildElement("radius")) {
        ...
        geom.m_capsuleRadius =
            m_urdfScaling * urdfLexicalCast<double>(scale->GetText());
      }
      ...
      geom.m_capsuleRadius =
          m_urdfScaling * urdfLexicalCast<double>(shape->Attribute("radius"));
      geom.m_capsuleHeight =
          m_urdfScaling * urdfLexicalCast<double>(shape->Attribute("length"));
    }
    ...
    bool success = UrdfFindMeshFile(m_fileIO, m_urdf2Model.m_sourceFile, fn,
                                    sourceFileLocation(shape),
    ...
    if (type_name == "plane") {
      ...
      if (this->m_parseSDF) {
        ...
        if ((0 == n) || (0 == s)) {
          logger->reportError(
              "Plane shape must have both normal and size attributes");
          return false;
        }
...
bool UrdfParser::parseCollision(UrdfCollision& collision, XMLElement* config,
  ...
  XMLElement* o = config->FirstChildElement("origin");
  if (o) {
    if (!parseTransform(collision.m_linkLocalFrame, o, logger)) return false;
  }
  // Geometry
  ...
  if (name_char) collision.m_name = name_char;

  const char* concave_char = config->Attribute("concave");
  if (concave_char) collision.m_flags |= URDF_FORCE_CONCAVE_TRIMESH;

  return true;
}

bool UrdfParser::parseVisual(UrdfModel& model, UrdfVisual& visual,
                             XMLElement* config, ErrorLogger* logger) {
  ...
  XMLElement* o = config->FirstChildElement("origin");
  if (o) {
    if (!parseTransform(visual.m_linkLocalFrame, o, logger)) return false;
  }
  // Geometry
  XMLElement* geom = config->FirstChildElement("geometry");
  ...
  const char* name_char = config->Attribute("name");
  if (name_char) visual.m_name = name_char;
  ...
  if (mat) {
    if (m_parseSDF) {
      ...
      matPtr->m_name = "mat";
      if (name_char) matPtr->m_name = name_char;
      ...
      if (t || c || s) {
        if (parseMaterial(visual.m_geometry.m_localMaterial, mat, logger)) {
          UrdfMaterial* matPtr =
              new UrdfMaterial(visual.m_geometry.m_localMaterial);

          UrdfMaterial** oldMatPtrPtr =
              model.m_materials[matPtr->m_name.c_str()];
          if (oldMatPtrPtr) {
            UrdfMaterial* oldMatPtr = *oldMatPtrPtr;
            model.m_materials.remove(matPtr->m_name.c_str());
            if (oldMatPtr) delete oldMatPtr;
...
// modified version of SDF audio_source specification in
// //http://sdformat.org/spec?ver=1.6&elem=link
    ...
    if (ci) {
      ...
      if (XMLElement* pitch_xml = ci->FirstChildElement("pitch")) {
        link.m_audioSource.m_pitch =
            urdfLexicalCast<double>(pitch_xml->GetText());
      }
      if (XMLElement* gain_xml = ci->FirstChildElement("gain")) {
        link.m_audioSource.m_gain =
            urdfLexicalCast<double>(gain_xml->GetText());
      }

      if (XMLElement* attack_rate_xml = ci->FirstChildElement("attack_rate")) {
        link.m_audioSource.m_attackRate =
            urdfLexicalCast<double>(attack_rate_xml->GetText());
      }

      if (XMLElement* decay_rate_xml = ci->FirstChildElement("decay_rate")) {
        link.m_audioSource.m_decayRate =
            urdfLexicalCast<double>(decay_rate_xml->GetText());
      }

      if (XMLElement* sustain_level_xml =
              ci->FirstChildElement("sustain_level")) {
        link.m_audioSource.m_sustainLevel =
            urdfLexicalCast<double>(sustain_level_xml->GetText());
      }

      if (XMLElement* release_rate_xml =
              ci->FirstChildElement("release_rate")) {
        link.m_audioSource.m_releaseRate =
            urdfLexicalCast<double>(release_rate_xml->GetText());
      }
      ...
      if (XMLElement* forceThreshold_xml =
              ci->FirstChildElement("collision_force_threshold")) {
        link.m_audioSource.m_collisionForceThreshold =
            urdfLexicalCast<double>(forceThreshold_xml->GetText());
      }
      ...
      if (damping_xml) {
        if (m_parseSDF) {
          link.m_contactInfo.m_inertiaScaling =
              urdfLexicalCast<double>(damping_xml->GetText());
          ...
          link.m_contactInfo.m_inertiaScaling =
              urdfLexicalCast<double>(damping_xml->Attribute("value"));
          link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_INERTIA_SCALING;
        }
      }
      {
        ...
        if (friction_xml) {
          if (m_parseSDF) {
            link.m_contactInfo.m_lateralFriction =
                urdfLexicalCast<double>(friction_xml->GetText());
          } else {
            ...
            link.m_contactInfo.m_lateralFriction =
                urdfLexicalCast<double>(friction_xml->Attribute("value"));
          }
        }
      }
        ...
        if (rolling_xml) {
          if (m_parseSDF) {
            link.m_contactInfo.m_rollingFriction =
                urdfLexicalCast<double>(rolling_xml->GetText());
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_ROLLING_FRICTION;
          } else {
            if (!rolling_xml->Attribute("value")) {
              logger->reportError(
                  "Link/contact: rolling friction element must have value "
                  "attribute");
              return false;
            }

            link.m_contactInfo.m_rollingFriction =
                urdfLexicalCast<double>(rolling_xml->Attribute("value"));
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_ROLLING_FRICTION;
          }
        }
      }
        ...
        if (restitution_xml) {
          if (m_parseSDF) {
            link.m_contactInfo.m_restitution =
                urdfLexicalCast<double>(restitution_xml->GetText());
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_RESTITUTION;
          } else {
            if (!restitution_xml->Attribute("value")) {
              logger->reportError(
                  "Link/contact: restitution element must have value "
                  "attribute");
              return false;
            }

            link.m_contactInfo.m_restitution =
                urdfLexicalCast<double>(restitution_xml->Attribute("value"));
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_RESTITUTION;
          }
        }
      }
        ...
        if (spinning_xml) {
          if (m_parseSDF) {
            link.m_contactInfo.m_spinningFriction =
                urdfLexicalCast<double>(spinning_xml->GetText());
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_SPINNING_FRICTION;
          } else {
            if (!spinning_xml->Attribute("value")) {
              logger->reportError(
                  "Link/contact: spinning friction element must have value "
                  "attribute");
              return false;
            }

            link.m_contactInfo.m_spinningFriction =
                urdfLexicalCast<double>(spinning_xml->Attribute("value"));
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_SPINNING_FRICTION;
          }
        }
      }
        ...
        if (stiffness_xml) {
          if (m_parseSDF) {
            link.m_contactInfo.m_contactStiffness =
                urdfLexicalCast<double>(stiffness_xml->GetText());
            ...
            link.m_contactInfo.m_contactStiffness =
                urdfLexicalCast<double>(stiffness_xml->Attribute("value"));
            link.m_contactInfo.m_flags |= URDF_CONTACT_HAS_STIFFNESS_DAMPING;
          }
        }
      }
  ...
  if (i) {
    ...
    if ((strlen(linkName) == 5) && (strncmp(linkName, "world", 5)) == 0) {
      ...
      logger->reportWarning(
          "No inertial data for link, using mass=1, localinertiadiagonal = "
          "1,1,1, identity local inertial frame");
...
bool UrdfParser::parseJoint(UrdfJoint& joint, XMLElement* config,
  ...
  if (m_parseSDF) {
    if (joint.m_type != URDFFloatingJoint && joint.m_type != URDFFixedJoint) {
      ...
      if (!axis_xml) {
        logger->reportWarning(
            "urdfdom: no axis elemement for Joint, defaulting to (1,0,0) axis");
        ...
        if (xyz_xml) {
          if (!parseVector3(joint.m_localJointAxis,
                            std::string(xyz_xml->GetText()), logger)) {
...
bool UrdfParser::parseSensor(UrdfModel& model, UrdfLink& link, UrdfJoint& joint,
                             XMLElement* config, ErrorLogger* logger) {
  ...
  if (parent_xml) {
    if (m_parseSDF) {
      ...
      if (!pname) {
        logger->reportError(
            "no parent link name specified for sensor. this might be the "
            "root?");
        logger->reportError(joint.m_name.c_str());
        return false;
      } else {
...
static void CalculatePrincipalAxisTransform(btScalar* masses,
                                            const btTransform* transforms,
                                            btMatrix3x3* inertiasIn,
...
bool UrdfParser::mergeFixedLinks(UrdfModel& model, UrdfLink* link,
                                 ErrorLogger* logger, bool forceFixedBase,
                                 int level) {
  ...
  for (int i = 0; i < link->m_childJoints.size();) {
    if (link->m_childJoints[i]->m_type == URDFFixedJoint) {
      ...
      if (forceFixedBase && link->m_parentJoint == 0) isStaticBase = true;
      if (link->m_inertia.m_mass == 0 && link->m_parentJoint == 0)
        isStaticBase = true;

      // skip the mass and inertia merge for a fixed base link
      if (!isStaticBase) {
        btScalar masses[2] = {(btScalar)link->m_inertia.m_mass,
                              (btScalar)childLink->m_inertia.m_mass};
        btTransform transforms[2] = {link->m_inertia.m_linkLocalFrame,
                                     childJoint->m_parentLinkToJointTransform *
                                         childLink->m_inertia.m_linkLocalFrame};
        btMatrix3x3 inertiaLink(link->m_inertia.m_ixx, link->m_inertia.m_ixy,
        ...
        btMatrix3x3 inertiaChild(
            childLink->m_inertia.m_ixx, childLink->m_inertia.m_ixy,
            childLink->m_inertia.m_ixz, childLink->m_inertia.m_ixy,
            childLink->m_inertia.m_iyy, childLink->m_inertia.m_iyz,
            childLink->m_inertia.m_ixz, childLink->m_inertia.m_iyz,
            childLink->m_inertia.m_izz);
        btMatrix3x3 inertiasIn[2] = {inertiaLink, inertiaChild};
        btVector3 inertiaOut;
        btTransform principal;
        CalculatePrincipalAxisTransform(masses, transforms, inertiasIn,
                                        principal, inertiaOut);
      ...
      mergeFixedLinks(model, link->m_childLinks[i], logger, forceFixedBase,
                      level + 1);
      i++;
    }
  }
...
bool UrdfParser::printTree(UrdfLink* link, ErrorLogger* logger, int level) {
  ...
  if (link->m_parentJoint) {
    printf("(joint %s, joint type=%s\n", link->m_parentJoint->m_name.c_str(),
           sJointNames[link->m_parentJoint->m_type].c_str());
  } else {
    printf("\n");
  }
...
bool UrdfParser::recreateModel(UrdfModel& model, UrdfLink* link,
  ...
  for (int i = 0; i < link->m_childJoints.size(); i++) {
    ...
    const char* jointName =
        link->m_childLinks[i]->m_parentJoint->m_name.c_str();
...
bool UrdfParser::loadUrdf(const char* urdfText, ErrorLogger* logger,
                          bool forceFixedBase, bool parseSensors) {
  XMLDocument xml_doc;
  ...
  for (XMLElement* material_xml = robot_xml->FirstChildElement("material");
    ...
    UrdfMaterial** mat =
        m_urdf2Model.m_materials.find(material->m_name.c_str());
    if (mat) {
      delete material;
  ...
  for (XMLElement* link_xml = robot_xml->FirstChildElement("link"); link_xml;
    ...
    if (parseLink(m_urdf2Model, *link, link_xml, logger)) {
      if (m_urdf2Model.m_links.find(link->m_name.c_str())) {
        logger->reportError(
            "Link name is not unique, link names in the same model have to be "
            "unique");
        logger->reportError(link->m_name.c_str());
        delete link;
        return false;
        ...
        for (int i = 0; i < link->m_visualArray.size(); i++) {
          ...
          if (!vis.m_geometry.m_hasLocalMaterial &&
              vis.m_materialName.c_str()) {
            UrdfMaterial** mat =
                m_urdf2Model.m_materials.find(vis.m_materialName.c_str());
            if (mat && *mat) {
  ...
  if (parseSensors) {
    ...
    for (XMLElement* sensor_xml = robot_xml->FirstChildElement("sensor");
      ...
      if (parseSensor(m_urdf2Model, *sensor, *sensor_joint, sensor_xml,
        ...
        if (m_urdf2Model.m_links.find(sensor->m_name.c_str())) {
          logger->reportError(
              "Sensor name is not unique, sensor and link names in the same "
              "model have to be unique");
          ...
          logger->reportError(
              "Sensor Joint name is not unique, joint names in the same model "
              "have to be unique");
          ...
          m_urdf2Model.m_joints.insert(sensor_joint->m_name.c_str(),
                                       sensor_joint);
        }
      } else {
...
bool UrdfParser::loadSDF(const char* sdfText, ErrorLogger* logger) {
  ...
  for (; robot_xml; robot_xml = robot_xml->NextSiblingElement("model")) {
    ...
    if (0 == pose_xml) {
      ...
      parseTransform(localModel->m_rootTransformInWorld, pose_xml, logger,
                     m_parseSDF);
    }
    ...
    for (XMLElement* material_xml = robot_xml->FirstChildElement("material");
      ...
      UrdfMaterial** mat =
          localModel->m_materials.find(material->m_name.c_str());
      if (mat) {
        logger->reportWarning("Duplicate material");
        delete material;
      } else {
    ...
    for (XMLElement* link_xml = robot_xml->FirstChildElement("link"); link_xml;
      ...
      if (parseLink(*localModel, *link, link_xml, logger)) {
        if (localModel->m_links.find(link->m_name.c_str())) {
          logger->reportError(
              "Link name is not unique, link names in the same model have to "
              "be unique");
          logger->reportError(link->m_name.c_str());
          delete link;
          return false;
        } else {
          // copy model material into link material, if link has no local
          // material
          for (int i = 0; i < link->m_visualArray.size(); i++) {
            UrdfVisual& vis = link->m_visualArray.at(i);
            if (!vis.m_geometry.m_hasLocalMaterial &&
                vis.m_materialName.c_str()) {
              UrdfMaterial** mat =
                  localModel->m_materials.find(vis.m_materialName.c_str());
              if (mat && *mat) {
                vis.m_geometry.m_localMaterial = **mat;
              } else {
                // logger->reportError("Cannot find material with name:");
    ...
    // Get all Joint elements
    for (XMLElement* joint_xml = robot_xml->FirstChildElement("joint");
...
#endif
  std::string str =
      m_urdf2Model.m_sourceFile.c_str() + std::string(":") + std::string(row);
  return str;
#endif
}
