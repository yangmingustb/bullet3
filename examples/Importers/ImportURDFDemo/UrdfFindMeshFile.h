...
static bool UrdfFindMeshFile(CommonFileIOInterface* fileIO,
  ...
  {
    for (std::list<std::string>::iterator x = shorter.begin();
         x != shorter.end(); ++x) {
      std::string attempt = *x + "/" + fn;
      int f = fileIO->fileOpen(attempt.c_str(), "rb");
      if (f < 0) {
        // b3Printf("%s: tried '%s'", error_message_prefix.c_str(),
        // attempt.c_str());
        continue;
      }
      fileIO->fileClose(f);
      existing_file = attempt;
      // b3Printf("%s: found '%s'", error_message_prefix.c_str(),
      // attempt.c_str());
      break;
    }
  }
