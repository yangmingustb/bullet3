...
struct UrdfLink;
struct UrdfModel;
/// btTransform is a position and 3x3 matrix, as defined in
/// Bullet/src/LinearMath/btTransform
class btTransform;

/// UrdfRenderingInterface is a simple rendering interface, mainly for
/// URDF-based robots.
...
struct UrdfRenderingInterface {
  ...
  /// given a URDF link, convert all visual shapes into internal renderer
  /// (loading graphics meshes, textures etc)
  /// use the visualShapeUniqueId as a unique identifier to synchronize the
  /// world transform and to remove the visual shape.
  virtual int convertVisualShapes(int linkIndex, const char* pathPrefix,
                                  const btTransform& localInertiaFrame,
                                  const UrdfLink* linkPtr,
                                  const UrdfModel* model,
                                  int visualShapeUniqueId, int bodyUniqueId,
                                  struct CommonFileIOInterface* fileIO) = 0;

  /// Given b3VisualShapeData, add render information (texture, rgbaColor etc)
  /// to the visualShape
  /// and visualShape to internal renderer.
  /// Returns a visualShapeUniqueId as a unique identifier to synchronize the
  /// world transform and to remove the visual shape.
  ...
  /// update the world transform + scaling of the visual shape, using the
  /// shapeUid
  virtual void syncTransform(int collisionObjectUid,
                             const class btTransform& worldTransform,
                             const class btVector3& localScaling) = 0;

  /// return the number of visual shapes, for a particular body unique id
  virtual int getNumVisualShapes(int bodyUniqueId) = 0;

  /// return the visual shape information, for a particular body unique id and
  /// 'shape index'
  ...
  virtual void changeShapeTexture(int objectUniqueId, int linkIndex,
                                  int shapeIndex, int textureUniqueId) = 0;
  ...
  virtual void copyCameraImageData(
      ...
      int* heightPtr, int* numPixelsCopied) = 0;
  ...
  /// register a texture using an in-memory pixel buffer of a given width and
  /// height
