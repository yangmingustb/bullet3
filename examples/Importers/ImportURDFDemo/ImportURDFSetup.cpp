...
struct ImportUrdfInternalData {
  ImportUrdfInternalData() : m_numMotors(0), m_mb(0) {
...
void ImportUrdfSetup::initPhysics() {
  ...
  // test to serialize a multibody to disk or shared memory, with base, link and
  // joint names
  ...
  if (loadOk) {
...

    {
      ...
      ConvertURDF2Bullet(u2b, creation, identityTrans, m_dynamicsWorld,
                         m_useMultiBody, u2b.getPathPrefix());
...
class CommonExampleInterface* ImportURDFCreateFunc(
    struct CommonExampleOptions& options) {
  return new ImportUrdfSetup(options.m_guiHelper, options.m_option,
                             options.m_fileName);
}
