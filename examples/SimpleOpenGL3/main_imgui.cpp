...
void ImGui_ImplBullet_CreateDeviceObjects() {
  ...
  io.Fonts->GetTexDataAsRGBA32(
      &pixels, &width, &height);  // Load as RGBA 32-bits (75% of the memory is
                                  ...
                                  // ImTextureId represent a higher-level
                                  // concept than just a GL texture id, consider
                                  // calling GetTexDataAsAlpha8() instead to
                                  // save on GPU memory.
...
void MyWheelCallback(float deltax, float deltay) {
  ...
  if (sOldWheelCB) sOldWheelCB(deltax, deltay);
}
void MyResizeCallback(float width, float height) {
  gWidth = width;
  gHeight = height;

  if (sOldResizeCB) sOldResizeCB(width, height);
}
void MyMouseMoveCallback(float x, float y) {
  ...
  if (sOldMouseMoveCB) sOldMouseMoveCB(x, y);
}
void MyMouseButtonCallback(int button, int state, float x, float y) {
  ...
  if (sOldMouseButtonCB) sOldMouseButtonCB(button, state, x, y);
}
...
void MyKeyboardCallback(int keycode, int state) {
  ...
  // use app->m_window->isModifiedPressed(...) to check for shift, escape and
  // alt keys
  printf("MyKeyboardCallback received key:%c in state %d\n", keycode, state);
  if (sOldKeyboardCB) sOldKeyboardCB(keycode, state);
}
...
int main(int argc, char* argv[]) {
  {
    ...
    if (gVideoFileName) app->dumpFramesToVideo(gVideoFileName);
    ...
    int textureHandle =
        app->m_renderer->registerTexture(image, textureWidth, textureHeight);
    ...
    app->m_renderer->registerGraphicsInstance(cubeIndex, pos, orn, color,
                                              scaling);
    ...
    do {
      ...
      // update the texels of the texture using a simple pattern, animated using
      // frame index
      ...
      // app->m_primRenderer->drawTexturedRect(100, 200, gWidth / 2 - 50,
      // gHeight / 2 - 50, color, 0, 0, 1, 1, true);
...
// app->drawText(bla, 10, 10);
...
#ifdef B3_USE_IMGUI
      {
        ...
        // Start the frame
        ImGuiIO& io = ImGui::GetIO();
        if (!g_FontTexture) ImGui_ImplBullet_CreateDeviceObjects();

        io.DisplaySize = ImVec2((float)gWidth, (float)gHeight);
        io.DisplayFramebufferScale =
            ImVec2(gWidth > 0 ? ((float)1.) : 0, gHeight > 0 ? ((float)1.) : 0);
        ...
        // storing our windows open/close state
