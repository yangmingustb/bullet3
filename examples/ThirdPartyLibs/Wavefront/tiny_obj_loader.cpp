...
// Erwin Coumans: improved performance, especially in debug mode on Visual
// Studio (25sec -> 4sec)
//
...
static bool exportFaceGroupToShape(shape_t* shape,
                                   const std::vector<face_t>& face_group,
...
std::string LoadMtl(std::map<std::string, material_t>& material_map,
...
#endif
  {
    ...
    if ((0 == strncmp(token, "newmtl", 6)) && isSpace((token[6]))) {
      ...
      material_map.insert(
          std::pair<std::string, material_t>(material.name, material));
...
std::string LoadObj(attrib_t& attrib, std::vector<shape_t>& shapes,
                    const char* filename, const char* mtl_basepath,
                    CommonFileIOInterface* fileIO) {
...
#endif
  {
    ...
    if ((0 == strncmp(token, "mtllib", 6)) && isSpace((token[6]))) {
      ...
      std::string err_mtl =
          LoadMtl(material_map, namebuf, mtl_basepath, fileIO);
      if (!err_mtl.empty()) {
