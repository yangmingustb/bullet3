        ...
        for (x = 0; x < len; ++x) {
          ...
          if (y0 < y1) {
            if (y0 > y_top && y0 < y_bottom) {
              stbtt__handle_clipped_edge(scanline, x, e, x_cur, y_cur, (float)x,
                                         y0);
              y_cur = y0;
              x_cur = (float)x;
            }
            if (y1 >= y_cur && y1 < y_bottom) {
              stbtt__handle_clipped_edge(scanline, x, e, x_cur, y_cur,
                                         (float)x + 1, y1);
              y_cur = y1;
              x_cur = (float)x + 1;
            }
          ...
          stbtt__handle_clipped_edge(scanline, x, e, x_cur, y_cur, xb,
                                     y_bottom);
        }
      }
    }
...
STBTT_DEF int stbtt_PackFontRanges(stbtt_pack_context *spc,
  ...
  n = 0;
  for (i = 0; i < num_ranges; ++i) n += ranges[i].num_chars_in_range;
  ...
  for (i = 0; i < num_ranges; ++i) {
    ...
    for (j = 0; j < ranges[i].num_chars_in_range; ++j) {
      ...
      stbtt_GetCodepointBitmapBoxSubpixel(
          &info, ranges[i].first_unicode_char_in_range + j,
          scale * spc->h_oversample, scale * spc->v_oversample, 0, 0, &x0, &y0,
          &x1, &y1);
      rects[k].w =
  ...
  for (i = 0; i < num_ranges; ++i) {
    ...
    for (j = 0; j < ranges[i].num_chars_in_range; ++j) {
      ...
      if (r->was_packed) {
        ...
        int advance, lsb, x0, y0, x1, y1;
        int glyph = stbtt_FindGlyphIndex(
            &info, ranges[i].first_unicode_char_in_range + j);
