...
string cfstring_to_string(CFStringRef cfstring) {
  ...
  if (cfstring) {
    Boolean success = CFStringGetCString(cfstring, cstring, sizeof(cstring),
...
string get_string_property(io_object_t& device, const char* property) {
  ...
  if (device) {
    CFStringRef property_as_cfstring = CFStringCreateWithCString(
        kCFAllocatorDefault, property, kCFStringEncodingASCII);

    CFTypeRef name_as_cfstring = IORegistryEntryCreateCFProperty(
        device, property_as_cfstring, kCFAllocatorDefault, 0);

    if (name_as_cfstring) {
      if (CFGetTypeID(name_as_cfstring) == CFStringGetTypeID())
        property_name =
            cfstring_to_string(static_cast<CFStringRef>(name_as_cfstring));
...
uint16_t get_int_property(io_object_t& device, const char* property) {
  ...
  if (device) {
    ...
    CFTypeRef number = IORegistryEntryCreateCFProperty(
        device, property_as_cfstring, kCFAllocatorDefault, 0);
...
vector<PortInfo> serial::list_ports(void) {
  ...
  CFDictionarySetValue(classes_to_match, CFSTR(kIOSerialBSDTypeKey),
  ...
  while ((serial_port = IOIteratorNext(serial_port_iterator))) {
    ...
    string serial_number =
        rtrim(get_string_property(parent, "USB Serial Number"));
    ...
    if (vendor_id && product_id) {
      ...
      int ret = snprintf(cstring, HARDWARE_ID_STRING_LENGTH,
                         "USB VID:PID=%04x:%04x SNR=%s", vendor_id, product_id,
                         serial_number.c_str());
