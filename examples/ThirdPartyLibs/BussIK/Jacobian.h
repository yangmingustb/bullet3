...
class Jacobian {
 public:
  ...
  void CalcDeltaThetas(
      MatrixRmn& AugMat);  // Use this only if the Current Mode has been set.
  ...
  static void CompareErrors(const Jacobian& j1, const Jacobian& j2,
                            double* weightedDist1, double* weightedDist2);
  static void CountErrors(const Jacobian& j1, const Jacobian& j2,
 ...
 public:
  ...
  int nRow;  // Total number of rows the real J (= 3*number of end effectors for
             // now)
  int nCol;  // Total number of columns in the real J (= number of joints for
             // now)
  ...
  VectorRn dS;         // delta s
  VectorRn dT1;        // delta t		--  these are delta S values clamped to
                       // smaller magnitude
  ...
  static const double PseudoInverseThresholdFactor;  // Threshold for treating
                                                     // eigenvalue as zero
                                                     // (fraction of largest
                                                     // eigenvalue)
