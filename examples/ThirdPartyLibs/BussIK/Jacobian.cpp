...
const double Jacobian::DefaultDampingLambda =
    0.6;  // Optimal for the "Y" shape (any lower gives jitter)
// const double Jacobian::DefaultDampingLambda = 1.1;			// Optimal
// for the DLS "double Y" shape (any lower gives jitter)
// const double Jacobian::DefaultDampingLambda = 0.7;			// Optimal
// for the DLS "double Y" shape with distance clamping (lower gives jitter)
...
Jacobian::Jacobian(Tree* tree) {
  ...
  dT1.SetLength(
      nRow);  // Linearized change in end effector positions based on dTheta
...
void Jacobian::SetDeltaS(VectorRn& S) { dS.Set(S); }
...
void Jacobian::CalcDeltaThetasPseudoinverse() {
  ...
  //	This an inefficient way to do Pseudoinverse, but it is convenient since
  //we need SVD anyway
...
void Jacobian::CalcDeltaThetasDLSwithSVD() {
  ...
  // Compute Singular Value Decomposition
  //	This an inefficient way to do DLS, but it is convenient since we need
  //SVD anyway
...
void Jacobian::UpdatedSClampValue(VectorR3* targets) {
  ...
  while (n) {
    if (n->IsEffector()) {
      ...
      temp -= n->GetS();
      double normSi =
          sqrt(Square(dS[i]) + Square(dS[i + 1]) + Square(dS[i + 2]));
...
void Jacobian::CompareErrors(const Jacobian& j1, const Jacobian& j2,
                             double* weightedDist1, double* weightedDist2) {
...
void Jacobian::CountErrors(const Jacobian& j1, const Jacobian& j2,
                           int* numBetter1, int* numBetter2, int* numTies) {
  const VectorRn& e1 = j1.errorArray;
  const VectorRn& e2 = j2.errorArray;
  int b1 = 0, b2 = 0, tie = 0;
  int len = e1.GetLength();
...
void Jacobian::CalcDeltaThetasSDLSrev2()
{
    ...
    assert(J.DebugCheckSVD(U, w , V));
        ...
        //	Delta target values are the dS values
        int nRows = J.GetNumRows();
        int numEndEffectors = tree->GetNumEffector();		// Equals
the number of rows of J divided by three
        int nCols = J.GetNumColumns();
        dTheta.SetZero();
        ...
        for ( i=0; i<nRows; i++ ) {
                ...
                double N = 0.0;
// N is the quasi-1-norm of the i-th column of U
                double alpha = 0.0;					//
alpha is the dot product of dS and the i-th column of U
                ...
                // P is the quasi-1-norm of the response to angles changing
according to the i-th column of V
                double P = 0.0;
                double *vx = V.GetColumnPtr(i);
                jnx = Jnorms.GetPtr();
                for ( j=nCols; j>0; j-- ) {
                ...
                if ( N<P ) {
                        lambda -= N/P;				// Scale
back maximum permissable joint angle
                }
                lambda *= lambda;
                lambda *= DampingLambdaSDLS;

                // Calculate the dTheta from pure pseudoinverse considerations
                double scale = alpha*wiInv/(Square(wiInv)+Square(lambda));
// This times i-th column of V is the SDLS response
                MatrixRmn::AddArrayScale(nCols, V.GetColumnPtr(i), 1,
dTheta.GetPtr(), 1, scale );
        }
