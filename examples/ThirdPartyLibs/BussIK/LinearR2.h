...
inline double DistSq(const VectorR2& u, const VectorR2& v) {
  return u.DistSq(v);
}
...
class VectorHgR2 {
 ...
 public:
  ...
  VectorHgR2(double xVal, double yVal) : x(xVal), y(yVal), w(1.0) {}
...
class RotationMapR2 : public Matrix2x2 {
 public:
  ...
  inline RotationMapR2 Inverse() const {
    return Transpose();
...
inline RotationMapR2 RotateToMap(const VectorR2& fromVec,
                                 const VectorR2& toVec);
...
inline VectorR2& VectorR2::ArrayProd(
    const VectorR2& v)  // Component-wise Product
{
  x *= v.x;
  y *= v.y;
  return (*this);
...
inline VectorR2&
VectorR2::MakeUnit()  // Convert to unit vector (or leave zero).
{
  double nSq = NormSq();
  if (nSq != 0.0) {
    *this /= sqrt(nSq);
...
inline VectorR2::VectorR2(const VectorHgR2& uH) : x(uH.x), y(uH.y) {
  *this /= uH.w;
}
...
inline void Matrix2x2::SetZero() { m11 = m12 = m21 = m22 = 0.0; }
...
VectorR2 Matrix2x2::Column1() const { return (VectorR2(m11, m21)); }

VectorR2 Matrix2x2::Column2() const { return (VectorR2(m12, m22)); }
...
VectorR2 Matrix2x2::Row1() const { return (VectorR2(m11, m12)); }

VectorR2 Matrix2x2::Row2() const { return (VectorR2(m21, m22)); }
...
inline void Matrix2x2::SetDiagonal(const VectorR2& u) { SetDiagonal(u.x, u.y); }
...
inline Matrix2x2&
Matrix2x2::ReNormalize()  // Re-normalizes nearly orthonormal matrix
{
...
inline double NormalizeError(const Matrix2x2& A) {
  ...
  double newdisc;
  discrepancy =
      A.m11 * A.m11 + A.m21 * A.m21 - 1.0;  // First column - inner product - 1
  if (discrepancy < 0.0) {
    discrepancy = -discrepancy;
  }
  newdisc =
      A.m12 * A.m12 + A.m22 * A.m22 - 1.0;  // Second column inner product - 1
  if (newdisc < 0.0) {
    newdisc = -newdisc;
  }
...
inline VectorR2 operator*(const Matrix2x2& A, const VectorR2& u) {
  return (VectorR2(A.m11 * u.x + A.m12 * u.y, A.m21 * u.x + A.m22 * u.y));
}
...
inline LinearMapR2::LinearMapR2(const Matrix2x2& A) : Matrix2x2(A) {}
...
inline LinearMapR2 operator+(const LinearMapR2& A, const LinearMapR2& B) {
  return (
      LinearMapR2(A.m11 + B.m11, A.m21 + B.m21, A.m12 + B.m12, A.m22 + B.m22));
}
...
inline LinearMapR2 operator*(const LinearMapR2& A, double b) {
  return (LinearMapR2(A.m11 * b, A.m21 * b, A.m12 * b, A.m22 * b));
}
...
inline LinearMapR2& LinearMapR2::operator*=(
    const Matrix2x2& B)  // Matrix product
{
...
inline RotationMapR2::RotationMapR2(double a11, double a21, double a12,
                                    double a22)
    // Values specified in column order!!!
    : Matrix2x2(a11, a21, a12, a22) {}
...
{
  return (RotationMapR2(m11, m12, m21, m22));
}

inline VectorR2 RotationMapR2::Invert(
    const VectorR2& u) const  // Returns solution
{
  return (VectorR2(m11 * u.x + m21 * u.y,  // Multiply with Transpose
                   m12 * u.x + m22 * u.y));
}

inline RotationMapR2& RotationMapR2::operator*=(
    const RotationMapR2& B)  // Matrix product
{
  (*this).Matrix2x2::operator*=(B);
...
// Returns a righthanded orthonormal basis to complement vector  u
//		The vector u must be unit.
inline VectorR2 GetOrtho(const VectorR2& u) { return VectorR2(-u.y, u.x); }
