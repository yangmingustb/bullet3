...
inline double ZeroValue(const double& x) { return 0.0; }
...
inline double Mag(double x) { return fabs(x); }

inline double Dist(double x, double y) { return fabs(x - y); }
...
inline bool EqualZeroFuzzy(double x) { return (fabs(x) <= 1.0e-14); }
