...
class VectorR4 {
 ...
 public:
  ...
  VectorR4& RotateUnitInDirection(
      const VectorR4& dir);  // rotate in direction dir
};
...
inline double DistSq(const VectorR4& u, const VectorR4& v) {
  return u.DistSq(v);
}
...
class Matrix4x4 {
 ...
 public:
  Matrix4x4();
  Matrix4x4(const VectorR4&, const VectorR4&, const VectorR4&,
            const VectorR4&);  // Sets by columns!
  Matrix4x4(double, double, double, double, double, double, double, double,
  ...
  inline void Set(const VectorR4&, const VectorR4&, const VectorR4&,
                  const VectorR4&);
  inline void Set(double, double, double, double, double, double, double,
                  double, double, double, double, double, double, double,
                  double, double);
  inline void SetByRows(const VectorR4&, const VectorR4&, const VectorR4&,
                        const VectorR4&);
...
class LinearMapR4 : public Matrix4x4 {
 public:
  LinearMapR4();
  LinearMapR4(const VectorR4&, const VectorR4&, const VectorR4&,
              const VectorR4&);  // Sets by columns!
  LinearMapR4(double, double, double, double, double, double, double, double,
              double, double, double, double, double, double, double,
              double);  // Sets by columns
...
class RotationMapR4 : public Matrix4x4 {
 public:
  RotationMapR4();
  RotationMapR4(const VectorR4&, const VectorR4&, const VectorR4&,
                const VectorR4&);  // Sets by columns!
  RotationMapR4(double, double, double, double, double, double, double, double,
                double, double, double, double, double, double, double,
                double);  // Sets by columns!
  ...
  inline RotationMapR4 Inverse() const {
    return Transpose();
...
inline void VectorProjectMap(const VectorR4& u, LinearMapR4& M) {
  ...
  double e = u.y * u.w;
  double f = u.z * u.w;
  M.Set(u.x * u.x, a, b, c, a, u.y * u.y, d, e, b, d, u.z * u.z, f, c, e, f,
        u.w * u.w);
}
...
inline void TimesTranspose(const VectorR4& u, const VectorR4& v,
                           LinearMapR4& M);
...
inline VectorR4&
VectorR4::MakeUnit()  // Convert to unit vector (or leave zero).
{
  double nSq = NormSq();
  if (nSq != 0.0) {
    *this /= sqrt(nSq);
...
inline VectorR4& VectorR4::ArrayProd(
    const VectorR4& v)  // Component-wise Product
{
  x *= v.x;
  y *= v.y;
  z *= v.z;
...
inline VectorR4& VectorR4::ArrayProd3(
...
VectorR4 Matrix4x4::Column1() const { return (VectorR4(m11, m21, m31, m41)); }

VectorR4 Matrix4x4::Column2() const { return (VectorR4(m12, m22, m32, m42)); }

VectorR4 Matrix4x4::Column3() const { return (VectorR4(m13, m23, m33, m43)); }

VectorR4 Matrix4x4::Column4() const { return (VectorR4(m14, m24, m34, m44)); }

inline void Matrix4x4::SetDiagonal(double x, double y, double z, double w) {
  m11 = x;
  m22 = y;
  m33 = z;
  m44 = w;
...
inline LinearMapR4::LinearMapR4(double a11, double a21, double a31, double a41,
                                double a12, double a22, double a32, double a42,
                                double a13, double a23, double a33, double a43,
                                double a14, double a24, double a34, double a44)
    // Values specified in column order!!!
    : Matrix4x4(a11, a21, a31, a41, a12, a22, a32, a42, a13, a23, a33, a43, a14,
                a24, a34, a44) {}

inline LinearMapR4::LinearMapR4(const Matrix4x4& A) : Matrix4x4(A) {}
...
inline LinearMapR4 operator+(const LinearMapR4& A, const LinearMapR4& B) {
  return (
      LinearMapR4(A.m11 + B.m11, A.m21 + B.m21, A.m31 + B.m31, A.m41 + B.m41,
...
inline LinearMapR4 operator-(const LinearMapR4& A) {
  return (LinearMapR4(-A.m11, -A.m21, -A.m31, -A.m41, -A.m12, -A.m22, -A.m32,
                      -A.m42, -A.m13, -A.m23, -A.m33, -A.m43, -A.m14, -A.m24,
                      -A.m34, -A.m44));
}
...
inline LinearMapR4 operator*(const LinearMapR4& A, double b) {
  return (LinearMapR4(A.m11 * b, A.m21 * b, A.m31 * b, A.m41 * b, A.m12 * b,
                      A.m22 * b, A.m32 * b, A.m42 * b, A.m13 * b, A.m23 * b,
                      A.m33 * b, A.m43 * b, A.m14 * b, A.m24 * b, A.m34 * b,
                      A.m44 * b));
}
...
{
  return (LinearMapR4(m11, m12, m13, m14, m21, m22, m23, m24, m31, m32, m33,
                      m34, m41, m42, m43, m44));
}

inline LinearMapR4& LinearMapR4::operator*=(
    const Matrix4x4& B)  // Matrix product
{
...
inline RotationMapR4::RotationMapR4(double a11, double a21, double a31,
                                    double a41, double a12, double a22,
                                    double a32, double a42, double a13,
                                    double a23, double a33, double a43,
...
{
  return (RotationMapR4(m11, m12, m13, m14, m21, m22, m23, m24, m31, m32, m33,
                        m34, m41, m42, m43, m44));
}

inline VectorR4 RotationMapR4::Invert(
    const VectorR4& u) const  // Returns solution
{
...
inline RotationMapR4& RotationMapR4::operator*=(
    const RotationMapR4& B)  // Matrix product
{
  (*this).Matrix4x4::operator*=(B);
...
inline void TimesTranspose(const VectorR4& u, const VectorR4& v,
  ...
  M.Set(v.x * u.x, v.x * u.y, v.x * u.z, v.x * u.w,  // Set by columns!
        v.y * u.x, v.y * u.y, v.y * u.z, v.y * u.w, v.z * u.x, v.z * u.y,
        v.z * u.z, v.z * u.w, v.w * u.x, v.w * u.y, v.w * u.z, v.w * u.w);
}

// Returns the solid angle between vectors u and v (not necessarily unit
// vectors)
inline double SolidAngle(const VectorR4& u, const VectorR4& v) {
