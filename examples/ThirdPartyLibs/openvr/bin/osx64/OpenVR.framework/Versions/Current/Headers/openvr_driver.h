...
struct CameraVideoStreamFrame_t {
  ...
  uint32_t m_nISPFrameTimeStamp;  // Driver provided time stamp per driver
                                  // centric time base
  ...
  double
      m_flFrameCaptureTime_ServerRelative;  // In System time within the server
  uint64_t
      m_nFrameCaptureTicks_ServerAbsolute;  // In system ticks within the server
  double m_flFrameCaptureTime_ClientRelative;  // At the client, relative to
                                               // when the frame was
                                               // exposed/captured.

  double m_flSyncMarkerError;

  TrackedDevicePose_t m_StandingTrackedDevicePose;  // Supplied by HMD layer
                                                    // when used as a tracked
                                                    // camera
...
struct DriverPose_t {
   ...
   * use angular acceleration.  A driver's angular acceleration is generally not
   * measured,
   ...
   * lagged due to the filtering required to reduce noise to an acceptable
   * level.
   */
...
class ITrackedDeviceServerDriver {
  ...
  * always be
  * called before any display or tracking methods. Memory and processor use by
  * the
  * ITrackedDeviceServerDriver object should be kept to a minimum until it is
  * activated.
  ...
  * active display
  * to another Hmd being the active display. The driver should clean whatever
  * memory
  ...
  * functionality. The driver should return NULL
  * if the requested interface or version is not supported. */
  virtual void *GetComponent(const char *pchComponentNameAndVersion) = 0;

  /** A VR Client has made this debug request of the driver. The set of valid
  * requests is entirely
  * up to the driver and the client to figure out, as is the format of the
  * response. Responses that
  * exceed the length of the supplied buffer should be truncated and null
  * terminated */
...
static const char *ITrackedDeviceServerDriver_Version =
...
class IVRDisplayComponent {
 public:
   ...
   * input UVs. UVs go from 0,0 in
          * the upper left of that eye's viewport and 1,1 in the lower right of
   * that eye's viewport. */
...
namespace vr {
// ----------------------------------------------------------------------------------------------
// Purpose: This component is used for drivers that implement direct mode
...
class IVRDriverDirectModeComponent {
 public:
  ...
  /** Specific to Oculus compositor support, textures supplied must be created
   * using this method. */
  virtual void CreateSwapTextureSet(
      uint32_t unPid, uint32_t unFormat, uint32_t unWidth, uint32_t unHeight,
      vr::SharedTextureHandle_t (*pSharedTextureHandles)[3]) {}

  /** Used to textures created using CreateSwapTextureSet.  Only one of the
   * set's handles needs to be used to destroy the entire set. */
  virtual void DestroySwapTextureSet(
      vr::SharedTextureHandle_t sharedTextureHandle) {}

  /** Used to purge all texture sets for a given process. */
  virtual void DestroyAllSwapTextureSets(uint32_t unPid) {}

  /** After Present returns, calls this to get the next index to use for
   * rendering. */
  virtual void GetNextSwapTextureSetIndex(
      vr::SharedTextureHandle_t sharedTextureHandles[2],
      uint32_t (*pIndices)[2]) {}

  /** Call once per layer to draw for this frame.  One shared texture handle per
   * eye.  Textures must be created
          * using CreateSwapTextureSet and should be alternated per frame.  Call
   * Present once all layers have been submitted. */
  virtual void SubmitLayer(vr::SharedTextureHandle_t sharedTextureHandles[2],
                           const vr::VRTextureBounds_t (&bounds)[2],
                           const vr::HmdMatrix34_t *pPose) {}

  /** Submits queued layers for display. */
  virtual void Present(vr::SharedTextureHandle_t syncTexture) {}
};

static const char *IVRDriverDirectModeComponent_Version =
    "IVRDriverDirectModeComponent_002";
...
class IVRControllerComponent {
 public:
   ...
   * will return 0. */
  virtual bool TriggerHapticPulse(uint32_t unAxisId,
                                  uint16_t usPulseDurationMicroseconds) = 0;
};

static const char *IVRControllerComponent_Version =
...
class IVRCameraComponent {
 public:
  ...
  // Camera Methods
  // ------------------------------------
  virtual bool GetCameraFrameDimensions(
      vr::ECameraVideoStreamFormat nVideoStreamFormat, uint32_t *pWidth,
      uint32_t *pHeight) = 0;
  virtual bool GetCameraFrameBufferingRequirements(
      int *pDefaultFrameQueueSize, uint32_t *pFrameBufferDataSize) = 0;
  virtual bool SetCameraFrameBuffering(int nFrameBufferCount,
                                       void **ppFrameBuffers,
                                       uint32_t nFrameBufferDataSize) = 0;
  virtual bool SetCameraVideoStreamFormat(
      vr::ECameraVideoStreamFormat nVideoStreamFormat) = 0;
  ...
  virtual void ReleaseVideoStreamFrame(
      const vr::CameraVideoStreamFrame_t *pFrameImage) = 0;
  virtual bool SetAutoExposure(bool bEnable) = 0;
  virtual bool PauseVideoStream() = 0;
  virtual bool ResumeVideoStream() = 0;
  virtual bool GetCameraDistortion(float flInputU, float flInputV,
                                   float *pflOutputU, float *pflOutputV) = 0;
  virtual bool GetCameraProjection(vr::EVRTrackedCameraFrameType eFrameType,
  ...
  virtual bool SetCameraVideoSinkCallback(
      vr::ICameraVideoSinkCallback *pCameraVideoSinkCallback) = 0;
  virtual bool GetCameraCompatibilityMode(
      vr::ECameraCompatibilityMode *pCameraCompatibilityMode) = 0;
  virtual bool SetCameraCompatibilityMode(
      vr::ECameraCompatibilityMode nCameraCompatibilityMode) = 0;
  virtual bool GetCameraFrameBounds(vr::EVRTrackedCameraFrameType eFrameType,
                                    uint32_t *pLeft, uint32_t *pTop,
                                    uint32_t *pWidth, uint32_t *pHeight) = 0;
  virtual bool GetCameraIntrinsics(vr::EVRTrackedCameraFrameType eFrameType,
                                   HmdVector2_t *pFocalLength,
                                   HmdVector2_t *pCenter) = 0;
};
...
class IVRDriverContext {
 public:
  /** Returns the requested interface. If the interface was not available it
  * will return NULL and fill
  * out the error. */
...
class IServerTrackedDeviceProvider {
 public:
  /** initializes the driver. This will be called before any other methods are
  *called.
  * If Init returns anything other than VRInitError_None the driver DLL will be
  *unloaded.
  *
  * pDriverHost will never be NULL, and will always be a pointer to a
  *IServerDriverHost interface
  *
  * pchUserDriverConfigDir - The absolute path of the directory where the driver
  *should store user
  *	config files.
  * pchDriverInstallDir - The absolute path of the root directory for the
  *driver.
  */
  ...
  /** Returns the version of the ITrackedDeviceServerDriver interface used by
   * this driver */
  ...
  /** Called when the system is entering Standby mode. The driver should switch
  * itself into whatever sort of low-power
  * state it has. */
  virtual void EnterStandby() = 0;

  /** Called when the system is leaving Standby mode. The driver should switch
  itself back to
  full operation. */
  virtual void LeaveStandby() = 0;
};

static const char *IServerTrackedDeviceProvider_Version =
...
class IVRProperties {
 public:
  ...
  virtual ETrackedPropertyError ReadPropertyBatch(
      PropertyContainerHandle_t ulContainerHandle, PropertyRead_t *pBatch,
      uint32_t unBatchEntryCount) = 0;

  /** Writes a set of properties atomically. See the PropertyWriteBatch_t struct
   * for more information. */
  virtual ETrackedPropertyError WritePropertyBatch(
      PropertyContainerHandle_t ulContainerHandle, PropertyWrite_t *pBatch,
      uint32_t unBatchEntryCount) = 0;
...
class CVRPropertyHelpers {
  ...
  * property value type does not match,
  * this function will return false. */
  ...
  /** Returns a single typed property. If the device index is not valid or the
  ...
  * characters. */
  uint32_t GetProperty(PropertyContainerHandle_t ulContainerHandle,
                       ETrackedDeviceProperty prop,
                       VR_OUT_STRING() void *pvBuffer, uint32_t unBufferSize,
  ...
  * characters. */
  ...
  /** Returns a string property as a std::string. If the device index is not
  * valid or the property is not a string type this function will
  * return an empty string. */
  std::string GetStringProperty(vr::PropertyContainerHandle_t ulContainer,
                                vr::ETrackedDeviceProperty prop,
                                vr::ETrackedPropertyError *peError = nullptr);

  /** Sets a scaler property. The new value will be returned on any subsequent
   * call to get this property in any process. */
  ETrackedPropertyError SetBoolProperty(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
      bool bNewValue);
  ETrackedPropertyError SetFloatProperty(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
      float fNewValue);
  ETrackedPropertyError SetInt32Property(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
      int32_t nNewValue);
  ETrackedPropertyError SetUint64Property(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
      uint64_t ulNewValue);

  /** Sets a string property. The new value will be returned on any subsequent
   * call to get this property in any process. */
  ETrackedPropertyError SetStringProperty(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
      const char *pchNewValue);

  /** Sets a single typed property. The new value will be returned on any
  ...
  ETrackedPropertyError SetProperty(PropertyContainerHandle_t ulContainerHandle,
                                    ETrackedDeviceProperty prop,
                                    void *pvNewValue, uint32_t unNewValueSize,
                                    PropertyTypeTag_t unTag);

  /** Sets the error return value for a property. This value will be returned on
   * all subsequent requests to get the property */
  ETrackedPropertyError SetPropertyError(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
      ETrackedPropertyError eError);

  /** Clears any value or error set for the property. */
  ETrackedPropertyError EraseProperty(
      PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop);
 ...
 private:
  ...
  T GetPropertyHelper(PropertyContainerHandle_t ulContainerHandle,
                      ETrackedDeviceProperty prop,
                      ETrackedPropertyError *pError, T bDefault,
                      PropertyTypeTag_t unTypeTag);

  IVRProperties *m_pProperties;
};

inline uint32_t CVRPropertyHelpers::GetProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    VR_OUT_STRING() void *pvBuffer, uint32_t unBufferSize,
    PropertyTypeTag_t *punTag, ETrackedPropertyError *pError) {
...
/** Sets a single typed property. The new value will be returned on any
 * subsequent call to get this property in any process. */
inline ETrackedPropertyError CVRPropertyHelpers::SetProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    void *pvNewValue, uint32_t unNewValueSize, PropertyTypeTag_t unTag) {
...
/** Returns a string property. If the device index is not valid or the property
* is not a string type this function will
* return 0. Otherwise it returns the length of the number of bytes necessary to
* hold this string including the trailing
* null. Strings will always fit in buffers of k_unMaxPropertyStringSize
* characters. */
inline uint32_t CVRPropertyHelpers::GetStringProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    VR_OUT_STRING() char *pchValue, uint32_t unBufferSize,
    ETrackedPropertyError *pError) {
  PropertyTypeTag_t unTag;
  ETrackedPropertyError error;
  uint32_t unRequiredSize = GetProperty(ulContainerHandle, prop, pchValue,
...
/** Returns a string property as a std::string. If the device index is not valid
* or the property is not a string type this function will
* return an empty string. */
inline std::string CVRPropertyHelpers::GetStringProperty(
    vr::PropertyContainerHandle_t ulContainer, vr::ETrackedDeviceProperty prop,
    vr::ETrackedPropertyError *peError) {
  char buf[1024];
  vr::ETrackedPropertyError err;
  uint32_t unRequiredBufferLen =
      GetStringProperty(ulContainer, prop, buf, sizeof(buf), &err);
  ...
  if (err == TrackedProp_Success) {
    ...
    unRequiredBufferLen = GetStringProperty(ulContainer, prop, pchBuffer,
                                            unRequiredBufferLen, &err);
    sResult = pchBuffer;
    delete[] pchBuffer;
  }
...
/** Sets a string property. The new value will be returned on any subsequent
 * call to get this property in any process. */
inline ETrackedPropertyError CVRPropertyHelpers::SetStringProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    const char *pchNewValue) {
  if (!pchNewValue) return TrackedProp_InvalidOperation;
  ...
  return SetProperty(ulContainerHandle, prop, (void *)pchNewValue,
                     (uint32_t)(pchCurr - pchNewValue) + 1,
                     k_unStringPropertyTag);
}

template <typename T>
inline T CVRPropertyHelpers::GetPropertyHelper(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    ETrackedPropertyError *pError, T bDefault, PropertyTypeTag_t unTypeTag) {
  T bValue;
  ETrackedPropertyError eError;
  PropertyTypeTag_t unReadTag;
  GetProperty(ulContainerHandle, prop, &bValue, sizeof(bValue), &unReadTag,
              &eError);
  if (unReadTag != unTypeTag && eError == TrackedProp_Success) {
    eError = TrackedProp_WrongDataType;
  };

  if (pError) *pError = eError;
...
inline bool CVRPropertyHelpers::GetBoolProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    ETrackedPropertyError *pError) {
  return GetPropertyHelper<bool>(ulContainerHandle, prop, pError, false,
                                 k_unBoolPropertyTag);
}

inline float CVRPropertyHelpers::GetFloatProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    ETrackedPropertyError *pError) {
  return GetPropertyHelper<float>(ulContainerHandle, prop, pError, 0.f,
                                  k_unFloatPropertyTag);
}

inline int32_t CVRPropertyHelpers::GetInt32Property(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    ETrackedPropertyError *pError) {
  return GetPropertyHelper<int32_t>(ulContainerHandle, prop, pError, 0,
                                    k_unInt32PropertyTag);
}

inline uint64_t CVRPropertyHelpers::GetUint64Property(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    ETrackedPropertyError *pError) {
  return GetPropertyHelper<uint64_t>(ulContainerHandle, prop, pError, 0,
...
inline ETrackedPropertyError CVRPropertyHelpers::SetBoolProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    bool bNewValue) {
  return SetProperty(ulContainerHandle, prop, &bNewValue, sizeof(bNewValue),
                     k_unBoolPropertyTag);
}
...
inline ETrackedPropertyError CVRPropertyHelpers::SetFloatProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    float fNewValue) {
  return SetProperty(ulContainerHandle, prop, &fNewValue, sizeof(fNewValue),
                     k_unFloatPropertyTag);
}
...
inline ETrackedPropertyError CVRPropertyHelpers::SetInt32Property(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    int32_t nNewValue) {
  return SetProperty(ulContainerHandle, prop, &nNewValue, sizeof(nNewValue),
                     k_unInt32PropertyTag);
}
...
inline ETrackedPropertyError CVRPropertyHelpers::SetUint64Property(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    uint64_t ulNewValue) {
  return SetProperty(ulContainerHandle, prop, &ulNewValue, sizeof(ulNewValue),
                     k_unUint64PropertyTag);
}

/** Sets the error return value for a property. This value will be returned on
 * all subsequent requests to get the property */
inline ETrackedPropertyError CVRPropertyHelpers::SetPropertyError(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop,
    ETrackedPropertyError eError) {
  PropertyWrite_t batch;
...
/** Clears any value or error set for the property. */
inline ETrackedPropertyError CVRPropertyHelpers::EraseProperty(
    PropertyContainerHandle_t ulContainerHandle, ETrackedDeviceProperty prop) {
  PropertyWrite_t batch;
...
class IVRServerDriverHost {
 public:
  /** Notifies the server that a tracked device has been added. If this function
  * returns true
  * the server will call Activate on the device. If it returns false some kind
  * of error
  * has occurred and the device will not be activated. */
  ...
  /** Notifies the server that vsync has occurred on the the display attached to
  * the device. This is
  * only permitted on devices of the HMD class. */
  ...
  virtual void TrackedDeviceAxisUpdated(
      uint32_t unWhichDevice, uint32_t unWhichAxis,
      const VRControllerAxis_t &axisState) = 0;
  ...
  virtual void ProximitySensorState(uint32_t unWhichDevice,
                                    bool bProximitySensorTriggered) = 0;
  ...
  * VREvent_t struct */
  ...
  /** Provides access to device poses for drivers.  Poses are in their "raw"
  * tracking space which is uniquely
  * defined by each driver providing poses for its devices.  It is up to clients
  * of this function to correlate
  * poses across different drivers.  Poses are indexed by their device id, and
  * their associated driver and
  * other properties can be looked up via IVRProperties. */
  virtual void GetRawTrackedDevicePoses(
      float fPredictedSecondsFromNow,
      TrackedDevicePose_t *pTrackedDevicePoseArray,
      uint32_t unTrackedDevicePoseArrayCount) = 0;

  /** Notifies the server that a tracked device's display component transforms
   * have been updated. */
  virtual void TrackedDeviceDisplayTransformUpdated(
...
class CVRHiddenAreaHelpers {
 public:
  CVRHiddenAreaHelpers(IVRProperties *pProperties)
      : m_pProperties(pProperties) {}

  /** Stores a hidden area mesh in a property */
  ETrackedPropertyError SetHiddenArea(EVREye eEye, EHiddenAreaMeshType type,
                                      HmdVector2_t *pVerts,
                                      uint32_t unVertCount);

  /** retrieves a hidden area mesh from a property. Returns the vert count read
   * out of the property.  */
  uint32_t GetHiddenArea(EVREye eEye, EHiddenAreaMeshType type,
                         HmdVector2_t *pVerts, uint32_t unVertCount,
                         ETrackedPropertyError *peError);

 private:
  ETrackedDeviceProperty GetPropertyEnum(EVREye eEye,
                                         EHiddenAreaMeshType type) {
    return (ETrackedDeviceProperty)(Prop_DisplayHiddenArea_Binary_Start +
                                    ((int)type * 2) + (int)eEye);
  }
...
inline ETrackedPropertyError CVRHiddenAreaHelpers::SetHiddenArea(
    EVREye eEye, EHiddenAreaMeshType type, HmdVector2_t *pVerts,
    uint32_t unVertCount) {
  ...
  return propHelpers.SetProperty(
      ...
      prop, pVerts, sizeof(HmdVector2_t) * unVertCount,
      k_unHiddenAreaPropertyTag);
}

inline uint32_t CVRHiddenAreaHelpers::GetHiddenArea(
    EVREye eEye, EHiddenAreaMeshType type, HmdVector2_t *pVerts,
    uint32_t unVertCount, ETrackedPropertyError *peError) {
  ETrackedDeviceProperty prop = GetPropertyEnum(eEye, type);
  CVRPropertyHelpers propHelpers(m_pProperties);
  ...
  uint32_t unBytesNeeded = propHelpers.GetProperty(
      propHelpers.TrackedDeviceToPropertyContainer(k_unTrackedDeviceIndex_Hmd),
      prop, pVerts, sizeof(HmdVector2_t) * unVertCount, &unTag, &propError);
...
// ivrwatchdoghost.h
namespace vr {
/** This interface is provided by vrclient to allow the driver to make
 * everything wake up */
class IVRWatchdogHost {
 public:
  /** Client drivers in watchdog mode should call this when they have received a
  * signal from hardware that should
...
namespace vr {
// ----------------------------------------------------------------------------------------------
// Purpose: This component is used for drivers that implement a virtual display
// (e.g. wireless).
// ----------------------------------------------------------------------------------------------
...
/** Returns the current IVRVirtualDisplay pointer or NULL the interface could
 * not be found. */
...
class COpenVRDriverContext {
 public:
  COpenVRDriverContext()
      : m_propertyHelpers(nullptr), m_hiddenAreaHelpers(nullptr) {
    Clear();
  }
  void Clear();
  ...
  IVRSettings *VRSettings() {
    if (m_pVRSettings == nullptr) {
      EVRInitError eError;
      m_pVRSettings = (IVRSettings *)VRDriverContext()->GetGenericInterface(
  ...
  IVRServerDriverHost *VRServerDriverHost() {
    if (m_pVRServerDriverHost == nullptr) {
      EVRInitError eError;
      m_pVRServerDriverHost =
          (IVRServerDriverHost *)VRDriverContext()->GetGenericInterface(
  ...
  IVRWatchdogHost *VRWatchdogHost() {
    if (m_pVRWatchdogHost == nullptr) {
      EVRInitError eError;
      m_pVRWatchdogHost =
          (IVRWatchdogHost *)VRDriverContext()->GetGenericInterface(
  ...
  IVRDriverManager *VRDriverManager() {
    if (!m_pVRDriverManager) {
      EVRInitError eError;
      m_pVRDriverManager =
          (IVRDriverManager *)VRDriverContext()->GetGenericInterface(
...
inline EVRInitError COpenVRDriverContext::InitServer() {
  Clear();
  if (!VRServerDriverHost() || !VRSettings() || !VRProperties() ||
      !VRDriverLog() || !VRDriverManager() || !VRResources())
