...
bool StringHasPrefixCaseSensitive(const std::string &sString,
...
bool StringHasSuffix(const std::string &sString, const std::string &sSuffix) {
  ...
  if (cSuffixLen > cStrLen) return false;
...
bool StringHasSuffixCaseSensitive(const std::string &sString,
...
void strcpy_safe(char *pchBuffer, size_t unBufferSizeBytes,
...
std::string StringToUpper(const std::string &sString) {
  ...
  for (std::string::const_iterator i = sString.begin(); i != sString.end();
       i++) {
...
uint32_t ReturnStdString(const std::string &sValue, char *pchBuffer,
                         uint32_t unBufferLen) {
  uint32_t unLen = (uint32_t)sValue.length() + 1;
  if (!pchBuffer || !unBufferLen) return unLen;
...
void BufferToStdString(std::string &sDest, const char *pchBuffer,
                       uint32_t unBufferLen) {
...
//-----------------------------------------------------------------------------
// Purpose: Helper for converting a numeric value to a hex digit, value should
// be 0-15.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Helper for converting a hex char value to numeric, return -1 if the
// char
//          is not a valid hex digit.
//-----------------------------------------------------------------------------
int iHexCharToInt(char cValue) {
  int32_t iValue = cValue;
  if ((uint32_t)(iValue - '0') < 10) return iValue - '0';

  iValue |= 0x20;
  if ((uint32_t)(iValue - 'a') < 6) return iValue - 'a' + 10;

  return -1;
}

//-----------------------------------------------------------------------------
// Purpose: Internal implementation of encode, works in the strict RFC manner,
// or
//          with spaces turned to + like HTML form encoding.
//-----------------------------------------------------------------------------
void V_URLEncodeInternal(char *pchDest, int nDestLen, const char *pchSource,
                         int nSourceLen, bool bUsePlusForSpace) {
  // AssertMsg( nDestLen > 3*nSourceLen, "Target buffer for V_URLEncode should
  // be 3x source length, plus one for terminating null\n" );
  ...
  for (int i = 0; i < nSourceLen; ++i) {
    ...
    // We allow only a-z, A-Z, 0-9, period, underscore, and hyphen to pass
    // through unescaped.
    // These are the characters allowed by both the original RFC 1738 and the
    // latest RFC 3986.
    // Current specs also allow '~', but that is forbidden under original RFC
    // 1738.
    if (!(pchSource[i] >= 'a' && pchSource[i] <= 'z') &&
        !(pchSource[i] >= 'A' && pchSource[i] <= 'Z') &&
        !(pchSource[i] >= '0' && pchSource[i] <= '9') && pchSource[i] != '-' &&
        pchSource[i] != '_' && pchSource[i] != '.') {
...
//-----------------------------------------------------------------------------
// Purpose: Internal implementation of decode, works in the strict RFC manner,
// or
//          with spaces turned to + like HTML form encoding.
//
//			Returns the amount of space used in the output buffer.
//-----------------------------------------------------------------------------
size_t V_URLDecodeInternal(char *pchDecodeDest, int nDecodeDestLen,
                           const char *pchEncodedSource, int nEncodedSourceLen,
                           bool bUsePlusForSpace) {
  if (nDecodeDestLen < nEncodedSourceLen) {
    // AssertMsg( false, "V_URLDecode needs a dest buffer at least as large as
    // the source" );
    return 0;
  }
  ...
  for (int i = 0; i < nEncodedSourceLen; ++i) {
    if (bUsePlusForSpace && pchEncodedSource[i] == '+') {
      ...
      // Percent signifies an encoded value, look ahead for the hex code,
      // convert to numeric, and use that
  ...
  // We may not have extra room to NULL terminate, since this can be used on raw
  // data, but if we do
...
//-----------------------------------------------------------------------------
// Purpose: Encodes a string (or binary data) from URL encoding format, see
// rfc1738 section 2.2.
//          This version of the call isn't a strict RFC implementation, but uses
//          + for space as is
//          the standard in HTML form encoding, despite it not being part of the
//          RFC.
//
//          Dest buffer should be at least as large as source buffer to
//          guarantee room for decode.
//-----------------------------------------------------------------------------
void V_URLEncode(char *pchDest, int nDestLen, const char *pchSource,
                 int nSourceLen) {
  return V_URLEncodeInternal(pchDest, nDestLen, pchSource, nSourceLen, true);
}

//-----------------------------------------------------------------------------
// Purpose: Decodes a string (or binary data) from URL encoding format, see
// rfc1738 section 2.2.
//          This version of the call isn't a strict RFC implementation, but uses
//          + for space as is
//          the standard in HTML form encoding, despite it not being part of the
...
//          Dest buffer should be at least as large as source buffer to
//          guarantee room for decode.
//			Dest buffer being the same as the source buffer (decode
//in-place) is explicitly allowed.
//-----------------------------------------------------------------------------
size_t V_URLDecode(char *pchDecodeDest, int nDecodeDestLen,
                   const char *pchEncodedSource, int nEncodedSourceLen) {
  return V_URLDecodeInternal(pchDecodeDest, nDecodeDestLen, pchEncodedSource,
                             nEncodedSourceLen, true);
}
