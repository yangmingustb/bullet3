...
// we stricmp (from WIN) but it isn't POSIX - OSX/LINUX have strcasecmp so just
// inline bridge to it
#if defined(OSX) || defined(LINUX)
#ifndef __THROW  // If __THROW is defined, these will clash with throw()
                 // versions on gcc
#include <strings.h>
...
inline errno_t wcsncpy_s(wchar_t *strDest, size_t numberOfElements,
                         const wchar_t *strSource, size_t count) {
  return wcslcpy(strDest, strSource, numberOfElements);
}

inline errno_t strncpy_s(char *strDest, size_t numberOfElements,
...
// return code.
#include "string.h"
inline void wcsncpy_s(wchar_t *strDest, size_t numberOfElements,
                      const wchar_t *strSource, size_t count) {
  wcsncpy(strDest, strSource, numberOfElements);
  strDest[numberOfElements - 1] = '\0';
}

inline void strncpy_s(char *strDest, size_t numberOfElements,
                      const char *strSource, size_t count) {
