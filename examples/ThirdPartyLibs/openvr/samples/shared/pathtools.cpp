...
std::string Path_GetExecutablePath() {
  ...
  WideCharToMultiByte(CP_UTF8, 0, pwchPath, -1, pchPath,
                      MAX_UNICODE_PATH_IN_UTF8, NULL, NULL);
  delete[] pwchPath;
...
/** Sets the path of the current working directory. Returns true if this was
 * successful. */
...
std::string Path_StripFilename(const std::string &sPath, char slash) {
  if (slash == 0) slash = Path_GetSlash();
...
std::string Path_StripExtension(const std::string &sPath) {
  for (std::string::const_reverse_iterator i = sPath.rbegin();
       i != sPath.rend(); i++) {
    if (*i == '.') {
...
bool Path_IsAbsolute(const std::string &sPath) {
  ...
  if (sPath[1] ==
      ':')  // drive letter plus slash, but must test both slash cases
  {
    if (sPath[2] == '\\' || sPath[2] == '/') return true;
...
std::string Path_MakeAbsolute(const std::string &sRelativePath,
                              const std::string &sBasePath, char slash) {
  if (slash == 0) slash = Path_GetSlash();

  if (Path_IsAbsolute(sRelativePath))
    return sRelativePath;
  else {
    if (!Path_IsAbsolute(sBasePath)) return "";

    std::string sCompacted =
        Path_Compact(Path_Join(sBasePath, sRelativePath, slash), slash);
...
std::string Path_Join(const std::string &first, const std::string &second,
                      const std::string &third, char slash) {
...
std::string Path_Join(const std::string &first, const std::string &second,
                      ...
                      char slash) {
  return Path_Join(Path_Join(Path_Join(first, second, slash), third, slash),
                   fourth, slash);
}

std::string Path_Join(const std::string &first, const std::string &second,
                      const std::string &third, const std::string &fourth,
                      const std::string &fifth, char slash) {
  return Path_Join(
      Path_Join(Path_Join(Path_Join(first, second, slash), third, slash),
                fourth, slash),
      fifth, slash);
}
...
/** Removes redundant <dir>/.. elements in the path. Returns an empty path if
* the
...
std::string Path_Compact(const std::string &sRawPath, char slash) {
  ...
  // each time we encounter .. back up until we've found the previous directory
  // name
  // then get rid of both
  std::string::size_type i = 0;
  while (i < sPath.length()) {
    if (i > 0 && sPath.length() - i >= 2 && sPath[i] == '.' &&
        sPath[i + 1] == '.' &&
      ...
      while (iDirStart > 0 && sPath[iDirStart - 1] != slash) --iDirStart;
...
// gets the path of vrclient.dll itself
  ...
  ::GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                      ...
                      reinterpret_cast<LPCTSTR>(Path_GetThisModulePath),
                      &hmodule);
  ...
  // get the addr of a function in vrclient.so and then ask the dlopen system
  // about it
  Dl_info info;
...
bool Path_IsDirectory(const std::string &sPath) {
  ...
  if (sFixedPath.empty()) return false;
...
#if defined(OSX)
  NSBundle *bundle =
      [NSBundle bundleWithPath:[NSString stringWithUTF8String:sPath.c_str()]];
...
//-----------------------------------------------------------------------------
std::string Path_FindParentSubDirectoryRecursively(
    const std::string &strStartDirectory, const std::string &strDirectoryName) {
  std::string strFoundPath = "";
  std::string strCurrentPath = Path_FixSlashes(strStartDirectory);
  if (strCurrentPath.length() == 0) return "";
...
unsigned char *Path_ReadBinaryFile(const std::string &strFilename, int *pSize) {
  ...
  // the open operation needs to be sharable, therefore use of _wfsopen instead
  // of _wfopen_s
...
uint32_t Path_ReadBinaryFile(const std::string &strFilename,
                             unsigned char *pBuffer, uint32_t unSize) {
  FILE *f;
#if defined(POSIX)
  f = fopen(strFilename.c_str(), "rb");
#else
...
bool Path_WriteBinaryFile(const std::string &strFilename, unsigned char *pData,
                          unsigned nSize) {
  FILE *f;
#if defined(POSIX)
...
bool Path_WriteStringToTextFile(const std::string &strFilename,
                                const char *pchData) {
  FILE *f;
#if defined(POSIX)
...
bool Path_WriteStringToTextFileAtomic(const std::string &strFilename,
                                      const char *pchData) {
  std::string strTmpFilename = strFilename + ".tmp";

  if (!Path_WriteStringToTextFile(strTmpFilename, pchData)) return false;
...
// Platform specific atomic file replacement
  ...
  if (!::ReplaceFileW(wsFilename.c_str(), wsTmpFilename.c_str(), nullptr, 0, 0,
                      0)) {
    // if we couldn't ReplaceFile, try a non-atomic write as a fallback
    if (!Path_WriteStringToTextFile(strFilename, pchData)) return false;
  }
#elif defined(POSIX)
  if (rename(strTmpFilename.c_str(), strFilename.c_str()) == -1) return false;
#else
...
// ----------------------------------------------------------------------------------------------------------------------------
// Purpose: Turns a path to a file on disk into a URL (or just returns the value
// if it's already a URL)
// ----------------------------------------------------------------------------------------------------------------------------
std::string Path_FilePathToUrl(const std::string &sRelativePath,
  ...
  if (!strnicmp(sRelativePath.c_str(), "http://", 7) ||
    ...
    if (sAbsolute.empty()) return sAbsolute;
    return std::string(FILE_URL_PREFIX) + sAbsolute;
  }
}

// -----------------------------------------------------------------------------------------------------
// Purpose: Strips off file:// off a URL and returns the path. For other kinds
// of URLs an empty string is returned
// -----------------------------------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------------------------------
// Purpose: Returns the root of the directory the system wants us to store user
// documents in
// -----------------------------------------------------------------------------------------------------
std::string GetUserDocumentsPath() {
  ...
  if (!SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_MYDOCUMENTS | CSIDL_FLAG_CREATE,
                                  NULL, 0, rwchPath))) {
    return "";
  }
...
#elif defined(LINUX)
  // @todo: not solved/changed as part of OSX - still not real - just removed
  // old class based steam cut and paste
