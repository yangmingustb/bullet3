...
void DragAndDrop::ControlDeleted(Gwen::Controls::Base* pControl) {
  ...
  if (LastPressedControl == pControl) LastPressedControl = NULL;

  if (HoveredControl == pControl) HoveredControl = NULL;

  if (NewHoveredControl == pControl) NewHoveredControl = NULL;
}
...
bool OnDrop(int x, int y) {
  ...
  if (DragAndDrop::HoveredControl) {
    DragAndDrop::HoveredControl->DragAndDrop_HoverLeave(
        DragAndDrop::CurrentPackage);
    bSuccess = DragAndDrop::HoveredControl->DragAndDrop_HandleDrop(
        DragAndDrop::CurrentPackage, x, y);
  }
...
bool DragAndDrop::OnMouseButton(Gwen::Controls::Base* pHoveredControl, int x,
                                int y, bool bDown) {
  if (!bDown) {
    LastPressedControl = NULL;
...
bool ShouldStartDraggingControl(int x, int y) {
  ...
  DragAndDrop::CurrentPackage = LastPressedControl->DragAndDrop_GetPackage(
  ...
  DragAndDrop::SourceControl->DragAndDrop_StartDragging(
      DragAndDrop::CurrentPackage, LastPressedPos.x, LastPressedPos.y);
...
void UpdateHoveredControl(Gwen::Controls::Base* pCtrl, int x, int y) {
  ...
  if (NewHoveredControl == DragAndDrop::SourceControl) NewHoveredControl = NULL;
  ...
  while (NewHoveredControl &&
         !NewHoveredControl->DragAndDrop_CanAcceptPackage(
             DragAndDrop::CurrentPackage)) {
  ...
  if (DragAndDrop::HoveredControl) {
    DragAndDrop::HoveredControl->DragAndDrop_HoverEnter(
        DragAndDrop::CurrentPackage, x, y);
  }
...
void DragAndDrop::OnMouseMoved(Gwen::Controls::Base* pHoveredControl, int x,
                               int y) {
  ...
  if (!CurrentPackage && !ShouldStartDraggingControl(x, y)) return;
...
void DragAndDrop::RenderOverlay(Gwen::Controls::Canvas* /*pCanvas*/,
                                Skin::Base* skin) {
  ...
  skin->GetRender()->AddRenderOffset(Gwen::Rect(
