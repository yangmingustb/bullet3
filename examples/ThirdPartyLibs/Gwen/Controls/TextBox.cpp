...
void TextBox::Render(Skin::Base* skin) {
  ...
  // Draw caret
  if (std::fmod(Gwen::Platform::GetTimeInSeconds() - m_fLastInputTime, 1.0f) >
      0.5f)
...
bool TextBox::OnKeyRight(bool bDown) {
  ...
  if (m_iCursorPos < TextLength()) m_iCursorPos++;
...
bool TextBox::HasSelection() { return m_iCursorPos != m_iCursorEnd; }
...
void TextBox::OnMouseClickLeft(int x, int y, bool bDown) {
  ...
  int iChar =
      m_Text->GetClosestCharacter(m_Text->CanvasPosToLocal(Gwen::Point(x, y)));

  if (bDown) {
    SetCursorPos(iChar);

    if (!Gwen::Input::IsShiftDown()) SetCursorEnd(iChar);
...
void TextBox::MakeCaratVisible() {
  ...
  // Or the left
  if (idealx > m_rTextPadding.left) idealx = m_rTextPadding.left;
...
void TextBox::OnEnter() { onReturnPressed.Call(this); }