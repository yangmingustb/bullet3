...
class GWEN_EXPORT Splitter : public Base {
 public:
  ...
  Splitter(Base* pParent) : BaseClass(pParent) {
    for (int i = 0; i < 2; i++) m_pPanel[i] = NULL;
  }
  ...
  void Layout(Skin::Base* skin) { LayoutVertical(skin); }
 ...
 private:
  void LayoutVertical(Skin::Base* /*skin*/) {
    ...
    if (m_pPanel[0]) {
      ...
      m_pPanel[0]->SetBounds(m.left, m.top, w - m.left - m.right,
                             (h * 0.5) - m.top - m.bottom);
    }

    if (m_pPanel[1]) {
      const Margin& m = m_pPanel[1]->GetMargin();
      m_pPanel[1]->SetBounds(m.left, m.top + (h * 0.5f), w - m.left - m.right,
