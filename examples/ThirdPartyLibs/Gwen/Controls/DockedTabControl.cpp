...
void DockedTabControl::DragAndDrop_StartDragging(
...
void DockedTabControl::DragAndDrop_EndDragging(bool bSuccess, int /*x*/,
                                               int /*y*/) {
  SetHidden(false);
...
void DockedTabControl::MoveTabsTo(DockedTabControl* pTarget) {
  ...
  for (Base::List::iterator iter = Children.begin(); iter != Children.end();
       ++iter) {
