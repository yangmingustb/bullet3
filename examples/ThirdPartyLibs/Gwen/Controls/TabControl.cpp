...
class TabControlInner : public Base {
 public:
  ...
  void Render(Skin::Base* skin) { skin->DrawTabControl(this, m_ButtonRect); }

  void UpdateCurrentButton(Gwen::Rect rct) { m_ButtonRect = rct; }
...
TabButton* TabControl::AddPage(const UnicodeString& strText,
                               Controls::Base* pPage) {
  if (!pPage) {
    pPage = new Base(this);
  } else {
...
void TabControl::PostLayout(Skin::Base* skin) {
  ...
  if (m_TabStrip->Hidden()) {
    m_InnerPanel->DynamicCastTabControlInner()->UpdateCurrentButton(
        Gwen::Rect(0, 0, 0, 0));
    ...
    rct = Gwen::Rect(p.x + 1, p.y + 1, m_pCurrentButton->Width() - 2,
                     m_pCurrentButton->Height() - 2);
    m_InnerPanel->DynamicCastTabControlInner()->UpdateCurrentButton(rct);
  }
}

void TabControl::OnLoseTab(TabButton* pButton) {
  if (m_pCurrentButton == pButton) m_pCurrentButton = NULL;
...
int TabControl::TabCount(void) { return m_TabStrip->NumChildren(); }

void TabControl::SetTabStripPosition(int iDock) {
  m_TabStrip->SetTabPosition(iDock);
}

bool TabControl::DoesAllowDrag() { return m_TabStrip->AllowsTabReorder(); }
...
void TabControl::ScrollPressLeft(Base* pFrom) { m_iScrollOffset -= 120; }

void TabControl::ScrollPressRight(Base* pFrom) { m_iScrollOffset += 120; }