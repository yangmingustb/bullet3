...
void Gwen::Anim::Cancel(Gwen::Controls::Base* control) {
  ...
  */
  ...
  if ((iAnimations = g_Animations.find(control)) != g_Animations.end()) {
    Gwen::Anim::Animation::ChildList& ChildAnimationsForControl =
        iAnimations->second;
...
Gwen::Anim::TimedAnimation::TimedAnimation(float fLength, float fDelay,
                                           float fEase) {
...
bool Gwen::Anim::TimedAnimation::Finished() { return m_bFinished; }
