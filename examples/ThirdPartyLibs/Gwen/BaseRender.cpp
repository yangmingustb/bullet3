...
Base::~Base() {
  if (GetCTT()) GetCTT()->ShutDown();
}

void Base::RenderText(Gwen::Font* pFont, Gwen::Point pos,
...
void Base::DrawPixel(int x, int y) { DrawFilledRect(Gwen::Rect(x, y, 1, 1)); }
...
const Gwen::Rect& Base::ClipRegion() const { return m_rectClipRegion; }

bool Base::ClipRegionVisible() {
  if (m_rectClipRegion.w <= 0 || m_rectClipRegion.h <= 0) return false;
...
/*
                        If they haven't defined these font functions in their
   renderer code
                        we just draw some rects where the letters would be to
   give them an idea.
                */
...
void Base::RenderText(Gwen::Font* pFont, Gwen::Point pos,
  ...
  for (float i = 0; i < text.length(); i++) {
    ...
    /*
                            This isn't important, it's just me messing around
       changing the
                            shape of the rect based on the letter.. just for
       fun.
                    */
...
Gwen::Point Base::MeasureText(Gwen::Font* pFont,
                              const Gwen::UnicodeString& text) {
  Gwen::Point p;
