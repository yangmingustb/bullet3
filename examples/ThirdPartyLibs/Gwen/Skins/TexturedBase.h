...
class TexturedBase : public Gwen::Skin::Base {
 public:
  ...
  Texturing::Bordered m_texTab_Control, m_texTab, m_texTab_Inactive,
      m_texTab_Gap, m_texTabBar;
  ...
  virtual void Init(const TextObject& TextureName) {
    ...
    m_texMenu_Panel_Border.Init(&m_Texture, 194, 97, 62, 32,
                                Margin(24, 8, 8, 8));
    ...
    m_texWindow_Inactive.Init(&m_Texture, 97, 0, 96, 96,
                              Margin(16, 32, 16, 16));
  ...
  virtual void DrawButton(Gwen::Controls::Base* control, bool bDepressed,
                          bool bHovered) {
    if (bDepressed)
  ...
  virtual void DrawMenuItem(Gwen::Controls::Base* control, bool bSubmenuOpen,
    ...
    if (bChecked)
      m_CheckMark.Draw(GetRender(),
                       Gwen::Rect(control->GetRenderBounds().x + 2,
                                  control->GetRenderBounds().y + 2, 16, 16));
  }
  ...
  virtual void DrawMenu(Gwen::Controls::Base* control, bool bPaddingDisabled) {
    if (!bPaddingDisabled) {
      return m_texMenu_Panel_Border.Draw(GetRender(),
                                         control->GetRenderBounds());
    }
  ...
  virtual void DrawRadioButton(Gwen::Controls::Base* control, bool bSelected,
                               bool bDepressed) {
    if (bSelected)
  ...
  virtual void DrawCheckBox(Gwen::Controls::Base* control, bool bSelected,
                            bool bDepressed) {
    if (bSelected)
  ...
  virtual void DrawGroupBox(Gwen::Controls::Base* control, int textStart,
    ...
    GetRender()->DrawFilledRect(
        Gwen::Rect(rect.x + 1, rect.y + 1, textStart - 3, 1));
    GetRender()->DrawFilledRect(
        Gwen::Rect(rect.x + 1 + textStart + textWidth, rect.y + 1,
                   rect.w - textStart + textWidth - 2, 1));
    GetRender()->DrawFilledRect(
        Gwen::Rect(rect.x + 1, (rect.y + rect.h) - 1, rect.x + rect.w - 2, 1));

    GetRender()->DrawFilledRect(Gwen::Rect(rect.x + 1, rect.y + 1, 1, rect.h));
    GetRender()->DrawFilledRect(
        Gwen::Rect((rect.x + rect.w) - 2, rect.y + 1, 1, rect.h - 1));

    GetRender()->SetDrawColor(m_colDarker);

    GetRender()->DrawFilledRect(
        Gwen::Rect(rect.x + 1, rect.y, textStart - 3, 1));
    GetRender()->DrawFilledRect(
        Gwen::Rect(rect.x + 1 + textStart + textWidth, rect.y,
                   rect.w - textStart - textWidth - 2, 1));
    GetRender()->DrawFilledRect(
        Gwen::Rect(rect.x + 1, (rect.y + rect.h) - 1, rect.x + rect.w - 2, 1));
    ...
    GetRender()->DrawFilledRect(
        Gwen::Rect((rect.x + rect.w) - 1, rect.y + 1, 1, rect.h - 1));
  }
  ...
  virtual void DrawTextBox(Gwen::Controls::Base* control) {
    ...
    /*
                            if ( CursorRect.w == 1 )
                            {
                                    if ( bHasFocus )
                                    {
                                            GetRender()->SetDrawColor(
       Gwen::Color( 0, 0, 0, 200 ) );
                                            GetRender()->DrawFilledRect(
       CursorRect );
                                    }
                            }
  ...
  virtual void DrawWindow(Gwen::Controls::Base* control, int topHeight,
                          bool inFocus) {
    Gwen::Rect rect = control->GetRenderBounds();

    if (inFocus)
      m_texWindow.Draw(GetRender(), control->GetRenderBounds());
    else
      m_texWindow_Inactive.Draw(GetRender(), control->GetRenderBounds());
  }
  ...
  virtual void DrawScrollBar(Gwen::Controls::Base* control, bool isHorizontal,
                             bool bDepressed) {
    Gwen::Rect rect = control->GetRenderBounds();
    if (bDepressed)
  ...
  virtual void DrawScrollBarBar(Controls::Base* control, bool bDepressed,
                                bool isHovered, bool isHorizontal) {
    // TODO: something specialized
    DrawButton(control, bDepressed, isHovered);
  }

  virtual void DrawProgressBar(Gwen::Controls::Base* control, bool isHorizontal,
                               float progress) {
    Gwen::Rect rect = control->GetRenderBounds();
    ...
    if (isHorizontal) {
      ...
      GetRender()->DrawFilledRect(
          Gwen::Rect(1, 1, rect.w * progress - 2, rect.h - 2));
      ...
      // Top half
      GetRender()->SetDrawColor(FillColour);
      GetRender()->DrawFilledRect(Gwen::Rect(
          1, 1 + (rect.h * (1 - progress)), rect.w - 2, rect.h * progress - 2));
  ...
  virtual void DrawSlider(Gwen::Controls::Base* control, bool bIsHorizontal,
  ...
  virtual void DrawScrollButton(Gwen::Controls::Base* control, int iDirection,
                                bool bDepressed) {
  ...
  virtual void DrawComboBoxButton(Gwen::Controls::Base* control,
                                  bool bDepressed) {
    m_Render->SetDrawColor(Gwen::Color(0, 0, 0, 240));
  ...
  virtual void DrawNumericUpDownButton(Gwen::Controls::Base* control,
                                       bool bDepressed, bool bUp) {
  ...
  virtual void DrawPropertyRow(Controls::Base* control, int iWidth,
                               bool bBeingEdited) {
    Gwen::Rect rect = control->GetRenderBounds();
  ...
  virtual void DrawPropertyTreeNode(Controls::Base* control, int BorderLeft,
    ...
    GetRender()->DrawFilledRect(Gwen::Rect(rect.x + BorderLeft, rect.y,
                                           rect.w - BorderLeft, BorderTop));
  }
  ...
  void DrawTreeNode(Controls::Base* ctrl, bool bOpen, bool bSelected,
    ...
    if (bSelected) {
      ...
      GetRender()->DrawFilledRect(
          Gwen::Rect(17, 0, iLabelWidth + 2, iLabelHeight - 1));
      GetRender()->SetDrawColor(Color(0, 150, 255, 200));
      GetRender()->DrawLinedRect(
          Gwen::Rect(17, 0, iLabelWidth + 2, iLabelHeight - 1));
    }
    ...
    GetRender()->DrawFilledRect(Gwen::Rect(
        14 + 8, iLabelHeight, 1, iLastBranch + iHalfWay - iLabelHeight));
  }
  ...
  void DrawColorDisplay(Controls::Base* control, Gwen::Color color) {
    ...
    if (color.a != 255) {
      ...
      GetRender()->DrawFilledRect(
          Gwen::Rect(rect.w * 0.5, rect.h * 0.5, rect.w * 0.5, rect.h * 0.5));
    }
