...
OpenGL_DebugFont::OpenGL_DebugFont(float retinaScale)
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_pFontTexture->width,
               m_pFontTexture->height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
...
void OpenGL_DebugFont::StartClip() {
  ...
  glScissor(retinaScale * rect.x * Scale(), retinaScale * rect.y * Scale(),
            retinaScale * rect.w * Scale(), retinaScale * rect.h * Scale());
...
void OpenGL_DebugFont::RenderText(Gwen::Font* pFont, Gwen::Point pos,
  ...
  for (int i = 0; i < (int)text.length(); i++) {
    ...
    float curSpacing = sGwenDebugFontSpacing[(int)ch] * m_fLetterSpacing *
                       fSize * m_fFontScale[0];
    Gwen::Rect r(pos.x + yOffset, pos.y - fSize * 0.2f,
                 (fSize * m_fFontScale[0]), fSize * m_fFontScale[1]);
...
void OpenGL_DebugFont::DrawTexturedRect(Gwen::Texture* pTexture,
                                        Gwen::Rect rect, float u1, float v1,
                                        float u2, float v2) {
