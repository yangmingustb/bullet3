...
void UpdateHoveredControl(Controls::Base* pInCanvas) {
  Controls::Base* pHovered =
      pInCanvas->GetControlAt(MousePosition.x, MousePosition.y);
  ...
  if (pHovered != Gwen::HoveredControl) {
    ...
    if (Gwen::HoveredControl) Gwen::HoveredControl->OnMouseEnter();
...
void FindKeyboardFocus(Controls::Base* pControl) {
  ...
  if (pControl->GetKeyboardInputEnabled()) {
    ...
    for (Controls::Base::List::iterator iter = pControl->Children.begin();
         iter != pControl->Children.end(); ++iter) {
      Controls::Base* pChild = *iter;
      if (pChild == Gwen::KeyboardFocus) return;
    }
...
Gwen::Point Gwen::Input::GetMousePosition() { return MousePosition; }

void Gwen::Input::OnCanvasThink(Controls::Base* pControl) {
  if (Gwen::MouseFocus && !Gwen::MouseFocus->Visible()) Gwen::MouseFocus = NULL;

  if (Gwen::KeyboardFocus) {
    bool isVisible = Gwen::KeyboardFocus->Visible();
    bool isEnabled = KeyboardFocus->GetKeyboardInputEnabled();

    if (!isVisible || !isEnabled) Gwen::KeyboardFocus = NULL;
  }
  ...
  for (int i = 0; i < Gwen::Key::Count; i++) {
    ...
    if (KeyData.KeyState[i] && fTime > KeyData.NextRepeat[i]) {
      KeyData.NextRepeat[i] =
          Gwen::Platform::GetTimeInSeconds() + KeyRepeatRate;
...
bool Gwen::Input::IsKeyDown(int iKey) { return KeyData.KeyState[iKey]; }

bool Gwen::Input::IsLeftMouseDown() { return KeyData.LeftMouseDown; }

bool Gwen::Input::IsRightMouseDown() { return KeyData.RightMouseDown; }

void Gwen::Input::OnMouseMoved(Controls::Base* pCanvas, int x, int y,
...
bool Gwen::Input::OnMouseClicked(Controls::Base* pCanvas, int iMouseButton,
                                 bool bDown) {
  // If we click on a control that isn't a menu we want to close
  // all the open menus. Menus are children of the canvas.
  if (bDown &&
      (!Gwen::HoveredControl || !Gwen::HoveredControl->IsMenuComponent())) {
    pCanvas->CloseMenus();
  }
  ...
  if (bDown && g_pntLastClickPos.x == MousePosition.x &&
      g_pntLastClickPos.y == MousePosition.y &&
      (Gwen::Platform::GetTimeInSeconds() - g_fLastClickTime[iMouseButton]) <
          DOUBLE_CLICK_SPEED) {
    bIsDoubleClick = true;
  }
  ...
  if (bDown) Gwen::HoveredControl->Touch();
  ...
  switch (iMouseButton) {
    case 0: {
      if (DragAndDrop::OnMouseButton(Gwen::HoveredControl, MousePosition.x,
                                     MousePosition.y, bDown))
        return true;

      if (bIsDoubleClick)
        Gwen::HoveredControl->OnMouseDoubleClickLeft(MousePosition.x,
                                                     MousePosition.y);
      else
        Gwen::HoveredControl->OnMouseClickLeft(MousePosition.x, MousePosition.y,
                                               bDown);
      return true;
    }
    ...
    case 1: {
      if (bIsDoubleClick)
        Gwen::HoveredControl->OnMouseDoubleClickRight(MousePosition.x,
                                                      MousePosition.y);
      else
        Gwen::HoveredControl->OnMouseClickRight(MousePosition.x,
                                                MousePosition.y, bDown);
      return true;
    }
  }
...
bool Gwen::Input::HandleAccelerator(Controls::Base* pCanvas,
                                    Gwen::UnicodeChar chr) {
  // Build the accelerator search string
  Gwen::UnicodeString accelString;
  if (Gwen::Input::IsControlDown()) accelString += L"Ctrl + ";
  if (Gwen::Input::IsShiftDown()) accelString += L"Shift + ";
  ...
  if (Gwen::KeyboardFocus &&
      Gwen::KeyboardFocus->HandleAccelerator(accelString))
    return true;

  if (Gwen::MouseFocus && Gwen::MouseFocus->HandleAccelerator(accelString))
    return true;

  if (pCanvas->HandleAccelerator(accelString)) return true;

  return false;
}

bool Gwen::Input::DoSpecialKeys(Controls::Base* pCanvas,
                                Gwen::UnicodeChar chr) {
...
bool Gwen::Input::OnKeyEvent(Controls::Base* pCanvas, int iKey, bool bDown) {
  ...
  if (bDown) {
    if (!KeyData.KeyState[iKey]) {
      ...
      KeyData.NextRepeat[iKey] =
          Gwen::Platform::GetTimeInSeconds() + KeyRepeatDelay;
