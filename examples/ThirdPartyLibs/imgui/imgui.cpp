...
// maintenance.
// If you work for a company, please consider financial support, see Readme. For
// individuals: https://www.patreon.com/imgui
...
/*
 ...
 without a label? (Yes). A primer on labels and the ID stack.
 ...
 content e.g. creating a window, opening a tree node
 ...
 dynamic UI, no construction/destructions steps, less data retention
 ...
 in your code before it is permanently removed from the public API.
   If you have a problem with a missing function/symbols, search for its name in
 the code, there will likely be a comment about it.
 ...
 system.
   It is recommended you build the .cpp files as part of your project and not as
 a library.
 ...
 marked 'Settings': at minimum you need to set io.DisplaySize
   (application resolution). Later on you will fill your keyboard mapping,
 clipboard handlers, and other advanced features but for a basic
 ...
 render data. it will call your io.RenderDrawListFn handler.
       (Even if you don't render, call Render() and ignore the callback, or call
 EndFrame() instead. Otherwhise some features will break)
 ...
 regardless of considerations of being in "update" vs "render" phases
 ...
 about choosing a font yet)
     unsigned char* pixels;
     int width, height;
     io.Fonts->GetTexDataAsRGBA32(pixels, &width, &height);
     // TODO: At this points you've got the texture data and you need to upload
 that your your graphic system:
     MyTexture* texture = MyEngine::CreateTextureFromMemoryPixels(pixels, width,
 height, TEXTURE_TYPE_RGBA)
 ...
 window"); ImGui::Text("Hello, world!"); ImGui::End();
 ...
 fragment shader sample color from 1 texture, multiply by vertex color.
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          const ImDrawVert* vtx_buffer = cmd_list->VtxBuffer.Data;  // vertex
 buffer generated by ImGui
          const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.Data;   // index
 buffer generated by ImGui
 ...
 updated.
   They tell you if ImGui intends to use your inputs. So for example, if
 'io.WantCaptureMouse' is set you would typically want to hide
   mouse inputs from the rest of your application. Read the FAQ below for more
 information about those flags.

 USING GAMEPAD/KEYBOARD NAVIGATION [BETA]

 - Ask questions and report issues at
 ...
 - Keyboard:
    - Set io.NavFlags |= ImGuiNavFlags_EnableKeyboard to enable. NewFrame() will
 automatically fill io.NavInputs[] based on your io.KeyDown[] + io.KeyMap[]
 arrays.
    - When keyboard navigation is active (io.NavActive +
 NavFlags_EnableKeyboard), the io.WantCaptureKeyboard flag will be set.
 ...
 goes false when mouse is used).
       - or query focus information with e.g. IsWindowFocused(), IsItemFocused()
 etc. functions.
      Please reach out if you think the game vs navigation input sharing could
 be improved.
 - Gamepad:
    - Set io.NavFlags |= ImGuiNavFlags_EnableGamepad to enable. Fill the
 ...
 entry of io.NavInputs[], set the following values:
         0.0f= not held. 1.0f= fully held. Pass intermediate 0.0f..1.0f values
 for analog triggers/sticks.
    - We uses a simple >0.0f test for activation testing, and won't attempt to
 test for a dead-zone.
      Your code will probably need to transform your raw inputs (such as e.g.
 remapping your 0.2..0.9 raw input range to 0.0..1.0 imgui range, maybe a power
 curve, etc.).
 ...
 a spare analog stick as a mouse-emulation fallback.
    - Consoles/Tablet/Phone users: Consider using Synergy host (on your
 computer) + uSynergy.c (in your console/tablet/phone app) to use your PC
 mouse/keyboard.
    - On a TV/console system where readability may be lower or mouse inputs may
 be awkward, you may want to set the ImGuiNavFlags_MoveMouse flag in
 io.NavFlags.
      Enabling ImGuiNavFlags_MoveMouse instructs dear imgui to move your mouse
 cursor along with navigation movements.
      When enabled, the NewFrame() function may alter 'io.MousePos' and set
 'io.WantMoveMouse' to notify you that it wants the mouse cursor to be moved.
      When that happens your back-end NEEDS to move the OS or underlying mouse
 cursor on the next frame. Some of the binding in examples/ do that.
      (If you set the ImGuiNavFlags_MoveMouse flag but don't honor
 'io.WantMoveMouse' properly, imgui will misbehave as it will see your mouse as
 ...
 uSynergy.c doesn't expose moving remote mouse cursor, you may want
       to set a boolean to ignore your other external mouse positions until the
 ...
 mouse_button=0 parameter because it is really usable in typical conditions at
 the moment.
 ...
 to share a font atlas between contexts. Otherwhise CreateContext() will create
 ...
 look them up.
                       Please note that the style system will keep evolving
 (hopefully stabilizing in Q1 2018), and so custom styles will probably subtly
 break over time. It is recommended you use the StyleColorsClassic(),
 StyleColorsDark(), StyleColorsLight() functions.
 ...
 two functions we are now obsoleting.
 ...
 or TODO.txt and see how you want/can help!
 ...
 screenshot, a mention somewhere etc.
      You may post screenshot or links in the gallery threads
 (github.com/ocornut/imgui/issues/1269). Visuals are ideal as they inspire other
 programmers.
      But even without visuals, disclosing your use of dear imgui help the
 library grow credibility, and help other teams and programmers with taking
 decisions.
 ...
 around until it hits your render function.
    Dear ImGui knows nothing about what those bits represent, it just passes
 them around. It is up to you to decide what you want the void* to carry!
 ...
 ImGui::Image(), ImGui::ImageButton(), ImDrawList::AddImage() functions.
    Dear ImGui will generate the geometry and draw calls using the ImTextureID
 that you passed and which your renderer can use.
    You may call ImGui::ShowMetricsWindow() to explore active draw lists and
 visualize/understand how the draw data is generated.
 ...
 A: Yes. A primer on labels and the ID stack...
   ...
   - Elements that are typically not clickable, such as Text() items don't need
 an ID.

   - Interactive widgets require state to be carried over multiple frames (most
 typically Dear ImGui often needs to remember what is
     the "active" widget). to do so they need a unique ID. unique ID are
 typically derived from a string label, an integer index or a pointer.

       Button("OK");          // Label = "OK",     ID = hash of "OK"
       Button("Cancel");      // Label = "Cancel", ID = hash of "Cancel"

   - ID are uniquely scoped within windows, tree nodes, etc. so no conflict can
 happen if you have two buttons called "OK"
 ...
 within string itself.
     Use "##" to pass a complement to the ID that won't be visible to the
 end-user.
     This helps solving the simple collision cases when you know which items are
 going to be created.
 ...
 bar, but windows are uniquely identified by their ID..
 ...
 formed from the concatenation of _everything_ in the ID stack!
 ...
 pointers as ID.
      e.g. when displaying a single object that may change over time (dynamic
 1-1 relationship), using a static string as ID will preserve your
 ...
 will preserve the node open/closed state differently.
 ...
 lagging by one frame. But reading those flags before calling NewFrame() is
    also generally ok, as the bool toggles fairly rarely and you don't generally
 expect to interact with either Dear ImGui or your application during
    the same frame when that transition occurs. Dear ImGui is tracking dragging
 and widget activity that may occur outside the boundary of a window,
    so 'io.WantCaptureMouse' is more accurate and correct than checking if a
 window is hovered.
 ...
 may or not be inconvenient. You might want to track which key-downs
     were for Dear ImGui, e.g. with an array of bool, and filter out the
 corresponding key-ups.)
 ...
 you want to use a backslash \ in a string literal you need to write a double
 backslash "\\":
      io.Fonts->AddFontFromFileTTF("MyDataFolder\MyFontFile.ttf",
 size_in_pixels);   // WRONG
      io.Fonts->AddFontFromFileTTF("MyDataFolder\\MyFontFile.ttf",
 ...
 (here "Hello world" contains 7 unique characters)
      builder.AddChar(0x7262);                               // Add a specific
 character
      builder.AddRanges(io.Fonts->GetGlyphRangesJapanese()); // Add one of the
 default ranges
      builder.BuildRanges(&ranges);                          // Build the final
 result (ordered ranges with all the unique characters submitted)
      io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels, NULL,
 ranges.Data);

    All your strings needs to use UTF-8 encoding. In C++11 you can encode a
 string literal in UTF-8 by using the u8"hello" syntax.
    Specifying literal in your source code using a local code page (such as
 CP-923 for Japanese or CP-1251 for Cyrillic) will NOT work!
    Otherwise you can convert yourself to UTF-8 or load text data from file
 ...
 io.AddInputCharacter(). The applications in examples/ are doing that.
    For languages using IME, on Windows you can copy the Hwnd of your
 application to io.ImeWindowHandle.
    The default implementation of io.ImeSetInputScreenPosFn() on Windows will
 set your IME position correctly.

 Q: How can I preserve my Dear ImGui context across reloading a DLL? (loss of
 the global/static variables)
 A: Create your own context 'ctx = CreateContext()' + 'SetCurrentContext(ctx)'
 and your own font atlas 'ctx->GetIO().Fonts = new ImFontAtlas()'
    so you don't rely on the default globals.

 Q: How can I use the drawing facilities without an ImGui window? (using
 ImDrawList API)
 A: - You can create a dummy window. Call Begin() with
 NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flag,
      push a ImGuiCol_WindowBg with zero alpha, then retrieve the ImDrawList*
 via GetWindowDrawList() and draw to it in any way you like.
 ...
 disappearing when I move windows around..
 A: You are probably mishandling the clipping rectangles in your render
 function.
 ...
 frame, it will keep appending to the same window.
        this is also useful to set yourself in the context of another window (to
 ...
 once a frame. You can use it to quickly add custom UI in the middle
...
// assumed to be != NULL.
// CreateContext() will automatically set this pointer if it is NULL. Change to
// a different context by calling ImGui::SetCurrentContext().
// If you use DLL hotreloading you might need to call SetCurrentContext() after
// reloading code from this file.
// ImGui functions are not thread-safe because of this pointer. If you want
// thread-safety to allow N threads to access N different contexts, you can:
...
// after reloading code from this file.
// Otherwise, you probably don't want to modify them mid-program, and if you use
...
ImGuiStyle::ImGuiStyle() {
  ...
  MouseCursorScale = 1.0f;  // Scale software rendered mouse cursor (when
...
// is done separately and is up to you.
...
bool ImGuiListClipper::Step() {
  ...
  if (StepNo == 2)  // Step 2: dummy step only required if an explicit
                    ...
                    // user still call Step(). Does nothing and switch to Step
                    // 3.
  {
...
static void SetNavIDAndMoveMouse(ImGuiID id, int nav_layer,
...
void ImGui::SetActiveID(ImGuiID id, ImGuiWindow* window) {
  ...
  if (g.ActiveIdIsJustActivated) g.ActiveIdTimer = 0.0f;
  g.ActiveId = id;
...
void ImGui::SetFocusID(ImGuiID id, ImGuiWindow* window) {
  ...
  // Assume that SetFocusID() is called in the context where its NavLayer is the
  // current layer, which is the case everywhere we call it.
  ...
  if (g.NavWindow)
    if (ImGuiWindow* focused_root_window = g.NavWindow->RootWindow)
      if (focused_root_window->WasActive &&
        ...
        // "modal popup"
        // NB: The order of those two tests is important because Modal windows
        // are also Popups.
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand) {
  ...
  const ImRect& curr = g.NavScoringRectScreen;  // Current modified source rect
                                                // (NB: we've applied Max.x =
                                                // Min.x in NavUpdate() to
                                                // inhibit the effect of having
                                                // varied item width)
  g.NavScoringCount++;

  // We perform scoring on items bounding box clipped by their parent window on
  // the other axis (clipping on our movement axis would give us equal scores
  // for all clipped items)
  if (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) {
    cand.Min.y =
        ImClamp(cand.Min.y, window->ClipRect.Min.y, window->ClipRect.Max.y);
    cand.Max.y =
        ImClamp(cand.Max.y, window->ClipRect.Min.y, window->ClipRect.Max.y);
  } else {
    cand.Min.x =
        ImClamp(cand.Min.x, window->ClipRect.Min.x, window->ClipRect.Max.x);
    cand.Max.x =
        ImClamp(cand.Max.x, window->ClipRect.Min.x, window->ClipRect.Max.x);
  }
  ...
  float dby = NavScoreItemDistInterval(
      ...
      ImLerp(curr.Min.y, curr.Max.y, 0.8f));  // Scale down on Y to keep using
                                              // box-distance for vertically
                                              // touching items
  ...
  float dist_center =
      fabsf(dcx) +
      fabsf(dcy);  // L1 metric (need this for our connectedness guarantee)
  ...
  if (ImGui::IsMouseHoveringRect(cand.Min, cand.Max)) {
    ...
    g.OverlayDrawList.AddRectFilled(
        cand.Max - ImVec2(4, 4),
                            ...
                            // C to rotate.
  {
    ...
    if (quadrant == g.NavMoveDir) {
      ImFormatString(buf, IM_ARRAYSIZE(buf), "%.0f/%.0f", dist_box,
                     dist_center);
      g.OverlayDrawList.AddRectFilled(cand.Min, cand.Max,
                                      IM_COL32(255, 0, 0, 200));
      g.OverlayDrawList.AddText(g.IO.FontDefault, 13.0f, cand.Min,
  ...
  if (quadrant == g.NavMoveDir) {
    ...
    if (dist_box == result->DistBox) {
      ...
      if (dist_center < result->DistCenter) {
        ...
        // since we the current "best" button already (so it must have a lower
        // index),
        // this is fairly easy. This rule ensures that all buttons with
        // dx==dy==0 will end up being linked in order of appearance along the x
        // axis.
  ...
  // no "real" matches
  // are found, so it only augments the graph produced by the above method using
  // extra links. (important, since it doesn't guarantee strong connectedness)
  // This is just to avoid buttons having no links in a particular direction
  // when there's a suitable neighbor. you get good graphs without this too.
  ...
  // failure. For general navigation it feels awkward.
  // Disabling it may however lead to disconnected graphs when nodes are very
  // spaced out on different axis. Perhaps consider offering this as an option?
...
static void NavRestoreLayer(int layer) {
  ...
  if (layer == 0 && g.NavWindow->NavLastIds[0] != 0)
    SetNavIDAndMoveMouse(g.NavWindow->NavLastIds[0], layer,
...
static inline void NavUpdateAnyRequestFlag() {
  ImGuiContext& g = *GImGui;
  g.NavAnyRequest =
      g.NavMoveRequest || g.NavInitRequest || IMGUI_DEBUG_NAV_SCORING;
}
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb,
  ...
  if (g.NavId == id) {
    g.NavWindow = window;  // Always refresh g.NavWindow, because some
                           // operations such as FocusItem() don't have a
                           // window.
...
// declare their minimum size requirement to ItemSize() and then use a larger
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg) {
  ...
  if (id != 0) {
    ...
    //  select a default widget
    //  (b) So that we can scroll up/down past clipped items. This adds a small
    //  O(N) cost to regular navigation requests unfortunately, but it is still
    //  limited to one window.
    //      it may not scale very well for windows with ten of thousands of
    //      item, but at least NavMoveRequest is only set on user interaction,
    //      aka maximum once a frame.
    //      We could early out with "if (is_clipped && !g.NavInitRequest) return
    //      false;" but when we wouldn't be able to reach unclipped widgets.
    //      This would work if user had explicit scrolling control (e.g. mapped
    //      on a stick)
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags) {
  ...
  // believe it is correct to NOT test for RootWindow but this leaves us unable
  // to use IsItemHovered() after EndChild() itself.
...
bool ImGui::ItemHoverable(const ImRect& bb, ImGuiID id) {
  ...
  if (g.NavDisableMouseHover ||
      !IsWindowContentHoverable(window, ImGuiHoveredFlags_Default))
    return false;
  if (window->DC.ItemFlags & ImGuiItemFlags_Disabled) return false;
...
static ImVec2 NavCalcPreferredMousePos() {
  ...
  if (!window) return g.IO.MousePos;
  ...
  return ImFloor(ImClamp(pos, visible_rect.Min,
                         visible_rect.Max));  // ImFloor() is important because
                                              // non-integer mouse position
                                              // application in back-end might
                                              // be lossy and result in
                                              // undesirable non-zero delta.
}
...
static int FindWindowIndex(ImGuiWindow* window)  // FIXME-OPT O(N)
{
  ...
  for (int i = g.Windows.Size - 1; i >= 0; i--)
    if (g.Windows[i] == window) return i;
  return -1;
}

static ImGuiWindow* FindWindowNavigable(int i_start, int i_stop,
                                        int dir)  // FIXME-OPT O(N)
{
  ImGuiContext& g = *GImGui;
  for (int i = i_start; i >= 0 && i < g.Windows.Size && i != i_stop; i += dir)
    if (ImGui::IsWindowNavFocusable(g.Windows[i])) return g.Windows[i];
  return NULL;
}
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode) {
  ...
  if (t < 0.0f && mode == ImGuiInputReadMode_Released)  // Return 1.0f when just
                                                        // released, no repeat,
                                                        // ignore analog input.
  ...
  if (mode == ImGuiInputReadMode_Repeat)
    return (float)CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime,
...
static bool IsNavInputPressedAnyOfTwo(ImGuiNavInput n1, ImGuiNavInput n2,
                                      ImGuiInputReadMode mode) {
  return (ImGui::GetNavInputAmount(n1, mode) +
          ImGui::GetNavInputAmount(n2, mode)) > 0.0f;
}
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir) {
  ...
  ImGuiWindow* window_target = FindWindowNavigable(i_current + focus_change_dir,
                                                   -INT_MAX, focus_change_dir);
  if (!window_target)
    window_target =
        FindWindowNavigable((focus_change_dir < 0) ? (g.Windows.Size - 1) : 0,
...
static void ImGui::NavUpdateWindowing() {
  ...
  bool start_windowing_with_keyboard =
      ...
      (g.IO.NavFlags & ImGuiNavFlags_EnableKeyboard);
  if (start_windowing_with_gamepad || start_windowing_with_keyboard)
    if (ImGuiWindow* window =
            g.NavWindow ? g.NavWindow : FindWindowNavigable(g.Windows.Size - 1,
                                                            -INT_MAX, -1)) {
      ...
      g.NavWindowingInputSource = start_windowing_with_keyboard
  ...
  if (g.NavWindowingTarget &&
      g.NavWindowingInputSource == ImGuiInputSource_NavGamepad) {
    ...
    g.NavWindowingHighlightAlpha =
        ImMax(g.NavWindowingHighlightAlpha,
              ImSaturate((g.NavWindowingHighlightTimer - 0.20f) / 0.05f));
    ...
    // release (until then the window was merely rendered front-most)
    if (!IsNavInputDown(ImGuiNavInput_Menu)) {
      g.NavWindowingToggleLayer &=
          (g.NavWindowingHighlightAlpha < 1.0f);  // Once button was held long
                                                  // enough we don't consider it
                                                  // a tap-to-toggle-layer press
                                                  // anymore.
  ...
  // Keyboard: Focus
  if (g.NavWindowingTarget &&
      g.NavWindowingInputSource == ImGuiInputSource_NavKeyboard) {
    ...
    g.NavWindowingHighlightAlpha = ImMax(
        g.NavWindowingHighlightAlpha,
        ImSaturate((g.NavWindowingHighlightTimer - 0.15f) / 0.04f));  // 1.0f
  ...
  if (g.NavWindowingTarget &&
    ...
    ImVec2 move_delta;
    if (g.NavWindowingInputSource == ImGuiInputSource_NavKeyboard &&
        !g.IO.KeyShift)
    ...
    if (move_delta.x != 0.0f || move_delta.y != 0.0f) {
      ...
      const float move_speed = ImFloor(NAV_MOVE_SPEED * g.IO.DeltaTime *
  ...
  if (apply_focus_window &&
      (g.NavWindow == NULL ||
       apply_focus_window != g.NavWindow->RootWindowForTabbing)) {
    g.NavDisableHighlight = false;
    g.NavDisableMouseHover = true;
  ...
  if (apply_toggle_layer && g.NavWindow) {
    ...
    g.NavDisableMouseHover = true;
    NavRestoreLayer(
        (g.NavWindow->DC.NavLayerActiveMask & (1 << 1)) ? (g.NavLayer ^ 1) : 0);
  }
}
...
static void NavScrollToBringItemIntoView(ImGuiWindow* window,
  ...
  // g.OverlayDrawList.AddRect(window->Pos + window_rect_rel.Min, window->Pos +
  // window_rect_rel.Max, IM_COL32_WHITE); // [DEBUG]
  if (window_rect_rel.Contains(item_rect_rel)) return;

  ImGuiContext& g = *GImGui;
  if (window->ScrollbarX && item_rect_rel.Min.x < window_rect_rel.Min.x) {
    window->ScrollTarget.x =
        item_rect_rel.Min.x + window->Scroll.x - g.Style.ItemSpacing.x;
    window->ScrollTargetCenterRatio.x = 0.0f;
  } else if (window->ScrollbarX &&
             item_rect_rel.Max.x >= window_rect_rel.Max.x) {
    window->ScrollTarget.x =
        item_rect_rel.Max.x + window->Scroll.x + g.Style.ItemSpacing.x;
    window->ScrollTargetCenterRatio.x = 1.0f;
  }
  if (item_rect_rel.Min.y < window_rect_rel.Min.y) {
    window->ScrollTarget.y =
        item_rect_rel.Min.y + window->Scroll.y - g.Style.ItemSpacing.y;
    window->ScrollTargetCenterRatio.y = 0.0f;
  } else if (item_rect_rel.Max.y >= window_rect_rel.Max.y) {
    window->ScrollTarget.y =
        item_rect_rel.Max.y + window->Scroll.y + g.Style.ItemSpacing.y;
    window->ScrollTargetCenterRatio.y = 1.0f;
  }

  // Estimate upcoming scroll so we can offset our relative mouse position so
  // mouse position can be applied immediately (under this block)
...
#undef NAV_MAP_KEY
  ...
  if (g.NavInitResultId != 0 &&
    ...
    if (g.NavInitRequestFromMove)
      SetNavIDAndMoveMouse(g.NavInitResultId, g.NavLayer,
  ...
  if (g.NavMoveRequest &&
    ...
    if (g.NavMoveResultOther.ID != 0 &&
        g.NavMoveResultOther.Window->ParentWindow ==
            g.NavWindow)  // Maybe entering a flattened child? In this case
                          // solve the tie using the regular scoring rules
      if ((g.NavMoveResultOther.DistBox < g.NavMoveResultLocal.DistBox) ||
          (g.NavMoveResultOther.DistBox == g.NavMoveResultLocal.DistBox &&
           g.NavMoveResultOther.DistCenter < g.NavMoveResultLocal.DistCenter))
  ...
  // Apply application mouse position movement, after we had a chance to process
  // move request result.
  if (g.NavMousePosDirty && g.NavIdIsAlive) {
    // Set mouse position given our knowledge of the nav widget position from
    // last frame
  ...
  g.IO.NavActive =
      (g.IO.NavFlags &
       (ImGuiNavFlags_EnableGamepad | ImGuiNavFlags_EnableKeyboard)) &&
  ...
  if (g.NavId != 0 && !g.NavDisableHighlight && !g.NavWindowingTarget &&
    ...
    bool activate_pressed =
        activate_down &&
        IsNavInputPressed(ImGuiNavInput_Activate, ImGuiInputReadMode_Pressed);
  ...
  // If we initiate a movement request and have no current NavId, we initiate a
  // InitDefautRequest that will be used as a fallback if the direction fails to
  // find a match
  ...
  if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) &&
      !g.NavWindowingTarget) {
    // *Fallback* manual-scroll with NavUp/NavDown when window has no navigable
    // item
    ImGuiWindow* window = g.NavWindow;
    const float scroll_speed =
        ImFloor(window->CalcFontSize() * 100 * g.IO.DeltaTime +
                0.5f);  // We need round the scrolling speed because sub-pixel
                        // scroll isn't reliably supported.
    if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll &&
        g.NavMoveRequest) {
      if (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right)
        SetWindowScrollX(
            window, ImFloor(window->Scroll.x +
                            ((g.NavMoveDir == ImGuiDir_Left) ? -1.0f : +1.0f) *
                                scroll_speed));
      if (g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down)
        SetWindowScrollY(
            window, ImFloor(window->Scroll.y +
  ...
  // When we have manually scrolled (without using navigation) and NavId becomes
  // out of bounds, we project its bounding box to the visible area to restart
  // navigation within visible items
  if (g.NavMoveRequest && g.NavMoveFromClampedRefRect && g.NavLayer == 0) {
    ...
    if (!window_rect_rel.Contains(window->NavRectRel[g.NavLayer])) {
      ...
      window_rect_rel.Expand(ImVec2(
          ...
          -ImMin(window_rect_rel.GetHeight(), pad)));  // Terrible approximation
                                                       // for the intent of
                                                       // starting navigation
                                                       // from first fully
                                                       // visible item
  ...
  // items)
  ImRect nav_rect_rel =
      (g.NavWindow && g.NavWindow->NavRectRel[g.NavLayer].IsFinite())
  ...
  IM_ASSERT(!g.NavScoringRectScreen.IsInverted());  // Ensure if we have a
                                                    // finite, non-inverted
                                                    // bounding box here will
                                                    // allows us to remove
                                                    // extraneous fabsf() calls
                                                    // in NavScoreItem().
  ...
  if (g.NavWindow) {
    ImU32 col = (g.NavWindow->HiddenFrames <= 0) ? IM_COL32(255, 0, 255, 255)
                                                 : IM_COL32(255, 0, 0, 255);
    ImVec2 p = NavCalcPreferredMousePos();
    char buf[32];
...
static void ImGui::UpdateMovingWindow() {
  ...
  if (g.MovingWindow && g.MovingWindow->MoveId == g.ActiveId &&
    ...
    // clicked on (could be a child window).
    // We track it to preserve Focus and so that ActiveIdWindow == MovingWindow
    // and ActiveId == MovingWindow->MoveId for consistency.
    KeepAliveID(g.ActiveId);
    IM_ASSERT(g.MovingWindow && g.MovingWindow->RootWindow);
    ImGuiWindow* moving_window = g.MovingWindow->RootWindow;
    if (g.IO.MouseDown[0]) {
      ImVec2 pos = g.IO.MousePos - g.ActiveIdClickOffset;
      if (moving_window->PosFloat.x != pos.x ||
          moving_window->PosFloat.y != pos.y) {
...
void ImGui::NewFrame() {
  ...
  IM_ASSERT(g.IO.DeltaTime >= 0.0f &&
            "Need a positive DeltaTime (zero is tolerated but will cause some "
            "timing issues)");
  ...
  for (int n = 0; n < ImGuiKey_COUNT; n++)
    IM_ASSERT(g.IO.KeyMap[n] >= -1 &&
              ...
              "io.KeyMap[] contains an out of bound value (need to be 0..512, "
              "or -1 for unmapped key)");

  // Do a simple check for required key mapping (we intentionally do NOT check
  // all keys to not pressure user into setting up everything, but Space is
  // required and was super recently added in 1.60 WIP)
  if (g.IO.NavFlags & ImGuiNavFlags_EnableKeyboard)
    IM_ASSERT(
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    ...
    if (g.IO.MouseClicked[i]) {
      ...
      g.IO.MouseDragMaxDistanceSqr[i] =
          ImMax(g.IO.MouseDragMaxDistanceSqr[i], ImLengthSqr(mouse_delta));
    }
  ...
  // capture/discard the inputs away from the rest of their application.
  ...
  if (g.IO.NavActive && (g.IO.NavFlags & ImGuiNavFlags_EnableKeyboard) &&
      !(g.IO.NavFlags & ImGuiNavFlags_NoCaptureKeyboard))
  ...
  if (g.HoveredWindow && !g.HoveredWindow->Collapsed &&
      (g.IO.MouseWheel != 0.0f || g.IO.MouseWheelH != 0.0f)) {
        ...
        scroll_amount = (float)(int)ImMin(
            scroll_amount, (scroll_window->ContentsRegionRect.GetHeight() +
                            scroll_window->WindowPadding.y * 2.0f) *
                               0.67f);
        SetWindowScrollY(scroll_window, scroll_window->Scroll.y -
    ...
    if (g.IO.MouseWheelH != 0.0f && scroll_allowed) {
      ...
      if (!g.IO.KeyCtrl &&
        ...
        SetWindowScrollX(window,
                         window->Scroll.x - g.IO.MouseWheelH * scroll_amount);
    }
  }
  ...
  // descending z-order
  if (g.NavWindow && !g.NavWindow->WasActive) FocusFrontMostActiveWindow(NULL);
  ...
  // something to it.
  // We don't use "Debug" to avoid colliding with user trying to create a
  // "Debug" window with custom flags.
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx,
  ...
  for (int i = 0; i != g.Windows.Size; i++) {
    ...
    if (!settings) settings = AddWindowSettings(window->Name);
...
void ImGui::Shutdown(ImGuiContext* context) {
  ...
  // NewFrame)
  if (g.IO.Fonts && g.FontAtlasOwnedByContext) IM_DELETE(g.IO.Fonts);
  ...
  if (g.LogClipboard) IM_DELETE(g.LogClipboard);
...
static void LoadIniSettingsFromMemory(const char* buf_readonly) {
  // For convenience and to make the code simpler, we'll write zero terminators
  // inside the buffer. So let's create a writable copy.
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_render_list,
                                  ImDrawList* draw_list) {
  ...
  // A) Make sure you are coarse clipping, because ImDrawList let all your
  // vertices pass. You can use the Metrics window to inspect draw list
  // contents.
  // B) If you need/want meshes with more than 64K vertices, uncomment the
  // '#define ImDrawIdx unsigned int' line in imconfig.h to set the index size
  // to 4 bytes.
  //    You'll need to handle the 4-bytes indices to your renderer. For example,
  //    the OpenGL example code detect index size at compile-time by doing:
...
static void AddWindowToDrawData(ImVector<ImDrawList*>* out_render_list,
  ...
  for (int i = 0; i < window->DC.ChildWindows.Size; i++) {
    ...
    if (child->Active &&
        child->HiddenFrames <=
            0)  // clipped children may have been marked not active
...
void ImGui::EndFrame() {
  ...
  if (g.ActiveId == 0 && g.HoveredId == 0) {
    if (!g.NavWindow ||
        !g.NavWindow->Appearing)  // Unless we just made a window/popup appear
    {
...
void ImGui::Render() {
  ...
  if (g.Style.Alpha > 0.0f) {
    ...
    if (window_to_render_front_most && window_to_render_front_most->Active &&
        window_to_render_front_most->HiddenFrames <=
            0)  // NavWindowingTarget is always temporarily displayed as the
                // front-most window
    ...
    if (g.IO.MouseDrawCursor &&
        g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &offset, &size, &uv[0],
                                          &uv[2])) {
      const ImVec2 pos = g.IO.MousePos - offset;
      const ImTextureID tex_id = g.IO.Fonts->TexID;
      const float sc = g.Style.MouseCursorScale;
      g.OverlayDrawList.PushTextureID(tex_id);
      g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(1, 0) * sc,
                                 pos + ImVec2(1, 0) * sc + size * sc, uv[2],
                                 uv[3], IM_COL32(0, 0, 0, 48));  // Shadow
      g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(2, 0) * sc,
                                 pos + ImVec2(2, 0) * sc + size * sc, uv[2],
                                 uv[3], IM_COL32(0, 0, 0, 48));  // Shadow
      g.OverlayDrawList.AddImage(tex_id, pos, pos + size * sc, uv[2], uv[3],
                                 IM_COL32(0, 0, 0, 255));  // Black border
      g.OverlayDrawList.AddImage(tex_id, pos, pos + size * sc, uv[0], uv[1],
...
// Render. If user hasn't set a callback then they may retrieve the draw data
// via GetDrawData()
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
void ImGui::CalcListClipping(int items_count, float items_height,
  ...
  if (g.NavMoveRequest &&
      g.NavMoveDir == ImGuiDir_Up)  // When performing a navigation request,
                                    // ensure we have one item extra in the
                                    // direction we are moving to
    start--;
  if (g.NavMoveRequest && g.NavMoveDir == ImGuiDir_Down) end++;
...
static ImGuiWindow* FindHoveredWindow() {
  ...
  for (int i = g.Windows.Size - 1; i >= 0; i--) {
    ...
    if (bb.Contains(g.IO.MousePos)) return window;
  }
  return NULL;
}
...
void ImGui::OpenPopupEx(ImGuiID id) {
  ...
  popup_ref.OpenPopupPos = (!g.NavDisableHighlight && g.NavDisableMouseHover)
  ...
  if (g.OpenPopupStack.Size < current_stack_size + 1) {
    ...
    // the ui
    // would become completely unusable because the popup will always be in
    // hidden-while-calculating-size state _while_ claiming focus. Which would
    // be a very confusing
    // situation for the programmer. Instead, we silently allow the popup to
    // proceed, it will keep reappearing and the programming error will be more
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window) {
  ...
  if (ref_window) {
    for (n = 0; n < g.OpenPopupStack.Size; n++) {
      ...
      // Trim the stack if popups are not direct descendant of the reference
      // window (which is often the NavWindow)
      bool has_focus = false;
      for (int m = n; m < g.OpenPopupStack.Size && !has_focus; m++)
        has_focus =
            (g.OpenPopupStack[m].Window &&
             g.OpenPopupStack[m].Window->RootWindow == ref_window->RootWindow);
      if (!has_focus) break;
    }
  }
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open,
  ...
  bool is_open = Begin(name, p_open, flags | ImGuiWindowFlags_Popup |
...
static void NavProcessMoveRequestWrapAround(ImGuiWindow* window) {
  ...
  if (g.NavWindow == window && NavMoveRequestButNoResultYet())
    if ((g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down) &&
      ...
      g.NavWindow->NavRectRel[0].Min.y = g.NavWindow->NavRectRel[0].Max.y =
          ((g.NavMoveDir == ImGuiDir_Up)
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg,
  ...
  if (!(flags & ImGuiWindowFlags_NavFlattened) &&
    ...
    ImGui::SetActiveID(id + 1, child_window);  // Steal ActiveId with a dummy id
                                               // so that key-press won't
                                               // activate child item
...
void ImGui::EndChild() {
  ...
  if (window->BeginCount > 1) {
    ...
    if (window->AutoFitChildAxises & (1 << ImGuiAxis_X))  // Arbitrary minimum
                                                          // zero-ish child size
                                                          // of 4.0f causes less
                                                          // trouble than a 0.0f
...
static ImVec2 FindBestWindowPosForPopup(
  ...
  // the mouse cursor which we want to avoid. for popups it's a small point
  // around the cursor.)
  // r_outer = the visible area rectangle, minus safe area padding. If our popup
  // size won't fit because of safe area padding we ignore it.
  ...
  // IM_COL32(255,0,0,255));
  // GImGui->OverlayDrawList.AddRect(r_outer.Min, r_outer.Max,
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size,
  ...
  // User can disable loading and saving of settings. Tooltip and child windows
  // also don't store settings.
...
static ImRect GetBorderRect(ImGuiWindow* window, int border_n,
  ...
  if (border_n == 1)
    return ImRect(rect.Max.x - thickness, rect.Min.y + perp_padding, rect.Max.x,
                  rect.Max.y - perp_padding);
  if (border_n == 2)
    return ImRect(rect.Min.x + perp_padding, rect.Max.y - thickness,
                  rect.Max.x - perp_padding, rect.Max.y);
  if (border_n == 3)
    return ImRect(rect.Min.x, rect.Min.y + perp_padding, rect.Min.x + thickness,
                  rect.Max.y - perp_padding);
  IM_ASSERT(0);
  return ImRect();
}
...
static void ImGui::UpdateManualResize(ImGuiWindow* window,
  ...
  for (int resize_grip_n = 0; resize_grip_n < resize_grip_count;
    ...
    bool hovered, held;
    ButtonBehavior(
        resize_rect, window->GetID((void*)(intptr_t)resize_grip_n), &hovered,
        &held, ImGuiButtonFlags_FlattenChildren | ImGuiButtonFlags_NoNavFocus);
    if (hovered || held)
    ...
    if (g.HoveredWindow == window && held && g.IO.MouseDoubleClicked[0] &&
      ...
      ImVec2 corner_target =
          ...
          resize_rect.GetSize() * grip.CornerPos;  // Corner of the window
                                                   ...
                                                   // corner grip
      CalcResizePosSizeFromAnyCorner(window, corner_target, grip.CornerPos,
  ...
  for (int border_n = 0; border_n < resize_border_count; border_n++) {
    ...
    ButtonBehavior(border_rect, window->GetID((void*)(intptr_t)(border_n + 4)),
  ...
  if (g.NavWindowingTarget == window) {
    ...
    if (g.NavWindowingInputSource == ImGuiInputSource_NavKeyboard &&
        g.IO.KeyShift)
      nav_resize_delta = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard,
                                             ImGuiInputReadMode_Down);
    if (g.NavWindowingInputSource == ImGuiInputSource_NavGamepad)
      nav_resize_delta = GetNavInputAmount2d(ImGuiNavDirSourceFlags_PadDPad,
                                             ImGuiInputReadMode_Down);
    if (nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f) {
      ...
      // handle sizing constraints properly, right now a constraint will make us
      // stuck.
      size_target =
          CalcSizeAfterConstraint(window, window->SizeFull + nav_resize_delta);
    }
  }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
  ...
  if (first_begin_of_the_frame) {
    const bool window_is_child_tooltip =
        (flags & ImGuiWindowFlags_ChildWindow) &&
        (flags & ImGuiWindowFlags_Tooltip);  // FIXME-WIP: Undocumented behavior
                                             // of Child+Tooltip for pinned
                                             // tooltip (#1345)

    // Initialize
    window->ParentWindow = parent_window;
    window->RootWindow = window->RootWindowForTitleBarHighlight =
        window->RootWindowForTabbing = window->RootWindowForNav = window;
    ...
    if (parent_window && !(flags & ImGuiWindowFlags_Modal) &&
        (flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_Popup)))
      window->RootWindowForTitleBarHighlight = window->RootWindowForTabbing =
          parent_window->RootWindowForTitleBarHighlight;  // Same value in
                                                          // master branch, will
                                                          // differ for docking
    ...
    if ((flags & ImGuiWindowFlags_ChildWindow) &&
        !(flags &
          (ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_Popup)) &&
    ...
    // Update contents size from last frame for auto-fitting (unless explicitly
    // specified)
    window->SizeContents = CalcSizeContents(window);

    // Hide popup/tooltip window when re-opening while we measure size (because
    // we recycle the windows)
    ...
    // Update scrollbar status (based on the Size that was effective during last
    // frame or the auto-resized Size).
    if (!window->Collapsed) {
    ...
    if (window_pos_with_pivot) {
      ...
      if (parent_menu->DC.MenuBarAppending)
        rect_to_avoid =
            ImRect(-FLT_MAX, parent_menu->Pos.y + parent_menu->TitleBarHeight(),
                   FLT_MAX, parent_menu->Pos.y + parent_menu->TitleBarHeight() +
                                parent_menu->MenuBarHeight());
      else
        rect_to_avoid =
            ImRect(parent_menu->Pos.x + horizontal_overlap, -FLT_MAX,
    ...
    if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api &&
      ...
      ImVec2 ref_pos = (!g.NavDisableHighlight && g.NavDisableMouseHover)
                           ? NavCalcPreferredMousePos()
                           : g.IO.MousePos;
      ImRect rect_to_avoid;
      if (!g.NavDisableHighlight && g.NavDisableMouseHover &&
          !(g.IO.NavFlags & ImGuiNavFlags_MoveMouse))
        rect_to_avoid = ImRect(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 16,
                               ref_pos.y + 8);
      else
        rect_to_avoid =
    ...
    const float grip_draw_size = (float)(int)ImMax(
        g.FontSize * 1.35f, window->WindowRounding + 1.0f + g.FontSize * 0.2f);
    if (!window->Collapsed)
      UpdateManualResize(window, size_auto_fit, &border_held, resize_grip_count,
                         &resize_grip_col[0]);
    ...
    if ((flags & ImGuiWindowFlags_Modal) != 0 &&
      ...
      window->DrawList->AddRectFilled(viewport_rect.Min, viewport_rect.Max,
    ...
    if (g.NavWindowingTarget == window) {
      ...
      if (!bb.Contains(viewport_rect))  // Avoid drawing if the window covers
                                        // all the viewport anyway
        window->DrawList->AddRectFilled(
            bb.Min, bb.Max, GetColorU32(ImGuiCol_NavWindowingHighlight,
    ...
    if (window->Collapsed) {
      ...
      ImU32 title_bar_col = GetColorU32(
          window->Collapsed ? ImGuiCol_TitleBgCollapsed
      ...
      if (flags & ImGuiWindowFlags_MenuBar) {
        ...
        menu_bar_rect.ClipWith(window->Rect());  // Soft clipping, in particular
                                                 // child window don't have
                                                 // minimum size covering the
                                                 // menu bar so this is useful
                                                 // for them.
        window->DrawList->AddRectFilled(
    ...
    if (g.NavWindowingTarget == window) {
      ...
      ImRect bb = window->Rect();
      bb.Expand(g.FontSize);
      if (bb.Contains(viewport_rect))  // If a window fits the entire viewport,
    ...
    // Store a backup of SizeFull which we will use next frame to decide if we
    // need scrollbars.
    ...
    if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
      ...
      if (!(flags & ImGuiWindowFlags_NoCollapse)) {
        ...
        ImRect bb(window->Pos + style.FramePadding + ImVec2(1, 1),
                  window->Pos + style.FramePadding +
                      ImVec2(g.FontSize, g.FontSize) - ImVec2(1, 1));
        ItemAdd(bb, id);  // To allow navigation
        if (ButtonBehavior(bb, id, NULL, NULL))
          window->CollapseToggleWanted = true;  // Defer collapsing to next
                                                // frame as we are too far in
                                                // the Begin() function
...
*/
    ...
    // rectangle doesn't lag by a frame
    ...
    window->InnerRect.Max.y = window->Pos.y + window->Size.y -
                              ...
                              window->WindowBorderSize;
    // window->DrawList->AddRect(window->InnerRect.Min, window->InnerRect.Max,
    // IM_COL32_WHITE);

    // After Begin() we fill the last item / hovered data using the title bar
    // data. Make that a standard behavior (to allow usage of context menus on
    // title bar only, etc.).
  ...
  // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's
  // render code produce correct result.
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_first_use,
  ...
  // Old API feature: we could pass the initial window size as a parameter. This
  // was misleading because it only had an effect if the window didn't have data
  // in the .ini file.
...
void ImGui::Scrollbar(ImGuiLayoutType direction) {
  ...
  if (horizontal)
    ...
    window_rounding_corners =
        ...
        (other_scrollbar ? 0 : ImDrawCornerFlags_BotRight);
  window->DrawList->AddRectFilled(
      bb.Min, bb.Max, GetColorU32(ImGuiCol_ScrollbarBg), window->WindowRounding,
      window_rounding_corners);
  bb.Expand(ImVec2(
  ...
  // amount visible (vs the total scrollable amount)
...
bool ImGui::IsWindowNavFocusable(ImGuiWindow* window) {
  ImGuiContext& g = *GImGui;
  return window->Active && window == window->RootWindowForTabbing &&
...
float ImGui::GetScrollMaxX() { return GetScrollMaxX(GImGui->CurrentWindow); }

float ImGui::GetScrollMaxY() { return GetScrollMaxY(GImGui->CurrentWindow); }
...
void ImGui::AlignTextToFramePadding() {
  ...
  ImGuiContext& g = *GImGui;
  window->DC.CurrentLineHeight = ImMax(window->DC.CurrentLineHeight,
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
  ...
  if (g.NavActivateDownId == id) {
    ...
    if (nav_activated_by_code || nav_activated_by_inputs || g.ActiveId == id) {
      ...
      SetActiveID(id, window);
      if (!(flags & ImGuiButtonFlags_NoNavFocus)) SetFocusID(id, window);
      g.ActiveIdAllowNavDirFlags = (1 << ImGuiDir_Left) |
                                   (1 << ImGuiDir_Right) | (1 << ImGuiDir_Up) |
                                   (1 << ImGuiDir_Down);
    }
  }
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius) {
  ...
  // Alt,Right,Validate sequence close a window.
  // (this isn't the regular behavior of buttons, but it doesn't affect the user
  // much because navigation tends to keep items visible).
...
bool ImGui::ArrowButton(ImGuiID id, ImGuiDir dir, ImVec2 padding,
  ...
  const ImRect bb(window->DC.CursorPos,
                  window->DC.CursorPos + ImVec2(g.FontSize + padding.x * 2.0f,
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  // ItemSpacing
  // (Ideally we'd want to add a flag for the user to specify if we want the hit
  // test to be done up to the right side of the content or not)
  ...
  // node when navigating one of its child.
  // For this purpose we essentially compare if g.NavIdIsAlive went from 0 to 1
  // between TreeNode() and TreePop().
  // This is currently only support 32 level deep and we are fine with (1 <<
  // Depth) overflowing into a zero.
  if (is_open && !g.NavIdIsAlive &&
      (flags & ImGuiTreeNodeFlags_NavCloseFromChild) &&
  ...
  if (display_frame) {
    ...
    RenderTriangle(frame_bb.Min + ImVec2(padding.x, text_base_offset_y),
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      ...
      RenderTriangle(frame_bb.Min + ImVec2(padding.x, g.FontSize * 0.15f +
                                                          text_base_offset_y),
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v,
  ...
  const ImU32 frame_col =
      GetColorU32((g.ActiveId == id && g.ActiveIdSource == ImGuiInputSource_Nav)
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
        ...
        clicked_t = SliderBehaviorCalcRatioFromValue(*v, v_min, v_max, power,
                                                     linear_zero_pos);
        if (decimal_precision == 0 && !is_non_linear) {
          if (fabsf(v_max - v_min) <= 100.0f ||
              IsNavInputDown(ImGuiNavInput_TweakSlow))
            delta = ((delta < 0.0f) ? -1.0f : +1.0f) /
                    (v_max -
                     v_min);  // Gamepad/keyboard tweak speeds in integer steps
          else
            delta /= 100.0f;
        } else {
          delta /=
        ...
        if ((clicked_t >= 1.0f && delta > 0.0f) ||
            (clicked_t <= 0.0f && delta < 0.0f))  // This is to avoid applying
                                                  // the saturation when already
                                                  // past the limits
          set_new_value = false;
        else
  ...
  if (g.ActiveId == id) {
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Nav) {
      adjust_delta =
          GetNavInputAmount2d(
              ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad,
              ImGuiInputReadMode_RepeatFast, 1.0f / 10.0f, 10.0f)
              .x;
      if (v_min < v_max &&
          ((v_cur >= v_max && adjust_delta > 0.0f) ||
           (v_cur <= v_min && adjust_delta < 0.0f)))  // This is to avoid
                                                      // applying the saturation
                                                      // when already past the
                                                      // limits
        adjust_delta = 0.0f;
      v_speed =
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label,
  ...
  if (values_count > 0) {
    ...
    float t0 = 0.0f;
    ImVec2 tp0 = ImVec2(
        t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale));  // Point in the
                                                               // normalized
                                                               // space of our
                                                               // target
                                                               // rectangle
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size,
  ...
  const bool focus_requested = FocusableItemRegister(
      window, id, (flags & (ImGuiInputTextFlags_CallbackCompletion |
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    const bool is_wordmove_key_down =
        io.OptMacOSXBehaviors ? io.KeyAlt : io.KeyCtrl;  // OS X style: Text
                                                         // editing cursor
                                                         // movement using Alt
                                                         // instead of Ctrl
  ...
  if (g.ActiveId == id || is_currently_scrolling) {
    ...
    bool cursor_is_visible = (!g.IO.OptCursorBlink) ||
...
bool ImGui::Combo(const char* label, int* current_item,
  ...
  if (!BeginCombo(label, preview_text, 0)) return false;
...
bool ImGui::Selectable(const char* label, bool selected,
  ...
  if (!ItemAdd(bb_with_spacing,
               (flags & ImGuiSelectableFlags_Disabled) ? 0 : id)) {
  ...
  // Hovering selectable with mouse updates NavId accordingly so navigation can
  // be resumed with gamepad/keyboard (this doesn't happen on most widgets)
  if (pressed ||
      hovered)  // && (g.IO.MouseDelta.x != 0.0f || g.IO.MouseDelta.y != 0.0f))
    if (!g.NavDisableMouseHover && g.NavWindow == window &&
        g.NavLayer == window->DC.NavLayerActiveMask) {
      g.NavDisableHighlight = true;
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected,
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    // a menu bar, which is a little misleading but may be useful
    // Note that in this situation we render neither the shortcut neither the
    // selected tick mark
    ...
    if (selected)
      RenderCheckMark(
          pos +
              ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f,
...
bool ImGui::BeginMenuBar() {
  ...
  // to the area below. However we clip with window full rect.
  // We remove 1 worth of rounding to Max.x to that text in long menus don't
...
void ImGui::EndMenuBar() {
  ...
  if (NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      // movement request for yet another frame.
      // This involve a one-frame delay which isn't very problematic in this
      // situation. We could remove it by scoring in advance for multiple window
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    // FindBestPopupWindowPos() in Begin()
    ...
    if (!enabled)
      PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
    RenderTriangle(
        pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f,
                     0.0f),
        ImGuiDir_Right);
...
void ImGui::EndMenu() {
  ...
  // menu.
  // A menu doesn't close itself because EndMenuBar() wants the catch the last
  // Left<>Right inputs.
  // However it means that with the current code, a BeginMenu() from outside
  // another menu or a menu-bar won't be closable with the Left direction.
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
// avoiding that.
// I spent a non reasonable amount of time trying to getting this right for
// ColorButton with rounding+anti-aliasing+ImGuiColorEditFlags_HalfAlphaPreview
// flag + various grid sizes and offsets, and eventually gave up... probably
...
// 'desc_id' is not called 'label' because we don't display it next to the
// button, but only in the tooltip.
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col,
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
    ...
    ImVec4 col_source =
        (flags & ImGuiColorEditFlags_AlphaPreview) ? col : col_without_alpha;
  ...
  {
    if (flags & ImGuiColorEditFlags_NoAlpha)
      SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F, &col, sizeof(float) * 3,
                         ImGuiCond_Once);
    else
      SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F, &col, sizeof(float) * 4,
                         ImGuiCond_Once);
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags) {
  ...
  if (BeginPopup("Copy")) {
    ...
    if (flags & ImGuiColorEditFlags_NoAlpha)
      ...
      ImFormatString(buf, IM_ARRAYSIZE(buf), "0x%02X%02X%02X%02X", cr, cg, cb,
                     ca);
    if (Selectable(buf)) SetClipboardText(buf);
    EndPopup();
  }
...
// floats if ImGuiColorEditFlags_NoAlpha flag is set.
// With typical options: Left-click on colored square to open color picker.
// Right-click to open option menu. CTRL-Click over input fields to edit them
...
bool ImGui::ColorEdit4(const char* label, float col[4],
  ...
  flags |=
      (g.ColorEditOptions &
       ~(ImGuiColorEditFlags__InputsMask | ImGuiColorEditFlags__DataTypeMask |
  ...
  if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) &&
      BeginDragDropTarget())  // NB: The flag test is merely an optional
                              // micro-optimization, BeginDragDropTarget() does
                              // the same test.
  {
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    for (int n = 0; n < 6; n++) {
      ...
      ShadeVertsLinearColorGradientKeepAlpha(
          ...
          draw_list->VtxBuffer.Data + vert_end_idx, gradient_p0, gradient_p1,
...
void ImGui::BeginColumns(const char* str_id, int columns_count,
  ...
  for (int n = 0; n < columns_count + 1; n++) {
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
      t = ImMin(t, PixelsToOffsetNorm(columns, (columns->MaxX - columns->MinX) -
                                                   g.Style.ColumnsMinSpacing *
                                                       (columns->Count - n)));
    column->OffsetNorm = t;

    if (n == columns_count) continue;
...
void ImGui::EndColumns() {
  ...
  if (!(columns->Flags & ImGuiColumnsFlags_GrowParentContentsSize))
    window->DC.CursorMaxPos.x = ImMax(
        ...
        columns->MaxX);  // Restore cursor max pos, as columns don't grow parent
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags) {
  ...
  if (!(flags & ImGuiDragDropFlags_SourceExtern)) {
    ...
    if (source_id == 0) {
      ...
      // identifier for interaction, such as Text() or Image(), you need to:
      // A) Read the explanation below, B) Use the
      // ImGuiDragDropFlags_SourceAllowNullID flag, C) Swallow your programmer
      // pride.
      ...
      // assigned ID, e.g. Text(), Image()
      // We build a throwaway ID based on current ID stack + relative AABB of
      // items in window.
      // THE IDENTIFIER WON'T SURVIVE ANY REPOSITIONING OF THE WIDGET, so if
      // your widget moves your dragging operation will be canceled.
      // We don't need to maintain/call ClearActiveID() as releasing the button
      // will early out this function and trigger !ActiveIdIsAlive.
      ...
      if (g.ActiveId == source_id)  // Allow the underlying widget to
                                    // display/return hovered during the mouse
                                    // release frame, else we would get a
                                    // flicker.
  ...
  if (source_drag_active) {
    ...
    if (!(flags & ImGuiDragDropFlags_SourceNoPreviewTooltip)) {
      ...
      // PushStyleVar(ImGuiStyleVar_Alpha, g.Style.Alpha * 0.60f); // This is
      // better but e.g ColorButton with checkboard has issue with transparent
      // colors :(
      SetNextWindowPos(g.IO.MousePos);
      PushStyleColor(ImGuiCol_PopupBg, GetStyleColorVec4(ImGuiCol_PopupBg) *
...
bool ImGui::SetDragDropPayload(const char* type, const void* data,
  ...
  IM_ASSERT(strlen(type) < IM_ARRAYSIZE(payload.DataType) &&
            "Payload type can be at most 12 characters long");
...
// 2) and it's faster. as this code may be very frequently called, we want to
// early out as fast as we can.
// Also note how the HoveredWindow test is positioned differently in both
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type,
  ...
  // targets conveniently without ordering constraints.
  // NB: We currently accept NULL id as target. However, overlapping targets
  // requires a unique ID to function!
  ...
  payload.Delivery =
      ...
      !IsMouseDown(g.DragDropMouseButton);  // For extern drag sources affecting
                                            // os window focus, it's easier to
                                            // just test !IsMouseDown() instead
                                            // of IsMouseReleased()
...
void ImGui::ShowMetricsWindow(bool* p_open) {
  if (ImGui::Begin("ImGui Metrics", p_open)) {
    ...
    struct Funcs {
      static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list,
        ...
        ImDrawList* overlay_draw_list =
            ImGui::GetOverlayDrawList();  // Render additional visuals into the
                                          // top-most draw list
        if (window && ImGui::IsItemHovered())
          overlay_draw_list->AddRect(window->Pos, window->Pos + window->Size,
            ...
            for (int prim = clipper.DisplayStart,
              ...
              for (int n = 0; n < 3; n++, vtx_i++) {
                ...
                buf_p += ImFormatString(
                    ...
                    (n == 0) ? "vtx" : "   ", vtx_i, v.pos.x, v.pos.y, v.uv.x,
                    v.uv.y, v.col);
              }
              ...
              if (ImGui::IsItemHovered()) {
                ...
                overlay_draw_list->Flags &=
                    ~ImDrawListFlags_AntiAliasedLines;  // Disable AA on
                                                        // triangle outlines at
                                                        // is more readable for
                                                        // very large and thin
                                                        // triangles.
      ...
      static void NodeWindow(ImGuiWindow* window, const char* label) {
        ...
        ImGui::BulletText(
            "Flags: 0x%08X (%s%s%s%s%s%s..)", flags,
        ...
        ImGui::BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x,
                          GetScrollMaxX(window), window->Scroll.y,
                          GetScrollMaxY(window));
        ImGui::BulletText("Active: %d, WriteAccessed: %d", window->Active,
    ...
    if (ImGui::TreeNode("Internal state")) {
      const char* input_source_names[] = {"None", "Mouse", "Nav", "NavGamepad",
                                          "NavKeyboard"};
      IM_ASSERT(IM_ARRAYSIZE(input_source_names) == ImGuiInputSource_Count_);
      ...
      ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec)", g.HoveredId,
                  ...
                  g.HoveredIdTimer);  // Data is "in-flight" so depending on
                                      // when the Metrics window is called we
                                      // may see current frame information or
                                      // not
      ImGui::Text("ActiveId: 0x%08X/0x%08X (%.2f sec), ActiveIdSource: %s",
      ...
      ImGui::Text("NavActive: %d, NavVisible: %d", g.IO.NavActive,
                  g.IO.NavVisible);
      ImGui::Text("NavActivateId: 0x%08X, NavInputId: 0x%08X", g.NavActivateId,
                  g.NavInputId);
      ImGui::Text("NavDisableHighlight: %d, NavDisableMouseHover: %d",
                  g.NavDisableHighlight, g.NavDisableMouseHover);
      ImGui::Text("DragDrop: %d, SourceId = 0x%08X, Payload \"%s\" (%d bytes)",
