...
// their code base:
// Don't do it! Do NOT remove this file from your project! It is useful
// reference code that you and other users will want to refer to.
// Everything in this file will be stripped out by the linker if you don't call
// ImGui::ShowDemoWindow().
// During development, you can call ImGui::ShowDemoWindow() in your code to
// learn about various features of ImGui. Have it wired in a debug menu!
// Removing this file from your project is hindering access to documentation for
// everyone in your team, likely leading you to poorer usage of the library.
// Note that you can #define IMGUI_DISABLE_DEMO_WINDOWS in imconfig.h for the
// same effect.
// If you want to link core ImGui in your final builds but not those demo
// windows, #define IMGUI_DISABLE_DEMO_WINDOWS in imconfig.h and those functions
// will be empty.
// In other situation, when you have ImGui available you probably want this to
// be available for reference and execution.
// Thank you,
// -Your beloved friend, imgui_demo.cpp (that you won't delete)

// Message to beginner C/C++ programmers. About the meaning of 'static': in this
// demo code, we frequently we use 'static' variables inside functions.
// We do this as a way to gather code and data in the same place, just to make
// the demo code faster to read, faster to write, and use less code.
// A static variable persist across calls, so it is essentially like a global
// variable but declared inside the scope of the function.
// It also happens to be a convenient way of storing simple UI related
// information as long as your function doesn't need to be reentrant or used in
// threads.
// This might be a pattern you occasionally want to use in your code, but most
// of the real data you would be editing is likely to be stored outside your
// function.
...
void ImGui::ShowDemoWindow(bool* p_open) {
  ...
  if (ImGui::CollapsingHeader("Help")) {
    ImGui::TextWrapped(
        "This window is being created by the ShowDemoWindow() function. Please "
        "refer to the code in imgui_demo.cpp for reference.\n\n");
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    if (ImGui::TreeNode("Basic")) {
...

      {
        ...
        ImGui::Combo("combo", &current_item_1,
                     "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");
        // ImGui::Combo("combo w/ array of char*", &current_item_2_idx, items,
        // IM_ARRAYSIZE(items));   // Combo using proper array. You can also
        // pass a callback to retrieve array value, no need to create/copy an
        // array just for that.

        // General BeginCombo() API, you have full control over your selection
        // data and display type
        const char* items[] = {
            "AAAA", "BBBB",    "CCCC", "DDDD",       "EEEE", "FFFF",
            "GGGG", "HHHH",    "IIII", "JJJJ",       "KKKK", "LLLLLLL",
            "MMMM", "OOOOOOO", "PPPP", "QQQQQQQQQQ", "RRR",  "SSSS"};
        static const char* current_item_2 = NULL;
        if (ImGui::BeginCombo("combo 2", current_item_2))  // The second
                                                           // parameter is the
                                                           // label previewed
                                                           // before opening the
                                                           // combo.
        {
          for (int n = 0; n < IM_ARRAYSIZE(items); n++) {
            bool is_selected =
                (current_item_2 == items[n]);  // You can store your selection
    ...
    if (ImGui::TreeNode("Images")) {
      ...
      // have access to inside the demo code.
      // Remember that ImTextureID is just storage for whatever you want it to
      // be, it is essentially a value that will be passed to the render
      // function inside the ImDrawCmd structure.
      // If you use one of the default imgui_impl_XXXX.cpp renderer, they all
      // have comments at the top of their file to specify what they expect to
      // be stored in ImTextureID.
      // (for example, the imgui_impl_dx11.cpp renderer expect a
      // 'ID3D11ShaderResourceView*' pointer. The imgui_impl_glfw_gl3.cpp
      // renderer expect a GLuint OpenGL texture identifier etc.)
      // If you decided that ImTextureID = MyEngineTexture*, then you can pass
      // your MyEngineTexture* pointers to ImGui::Image(), and gather
      // width/height through your own functions, etc.
      // Using ShowMetricsWindow() as a "debugger" to inspect the draw data that
      // are being passed to your render will help you debug issues if you are
      // confused about this.
      ...
      if (ImGui::IsItemHovered()) {
        ...
        ImGui::Image(my_tex_id, ImVec2(128, 128), uv0, uv1,
    ...
    if (ImGui::TreeNode("Selectables")) {
      ...
      // When Selectable() has been clicked is returns true and you can alter
      // selection state accordingly.
      // - The one taking "bool* p_selected" as a read-write selection
      // information (convenient in some cases)
      // The earlier is more flexible, as in real application your selection may
      // be stored in a different manner (in flags within objects, as an
      // external list, etc).
      if (ImGui::TreeNode("Basic")) {
  ...
  if (ImGui::CollapsingHeader("Layout")) {
    if (ImGui::TreeNode("Child regions")) {
      ...
      {
        ImGui::BeginChild(
            "Child1", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f, 300),
            false,
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
      ...
      for (int line = 0; line < lines; line++) {
        ...
        // real application you may be better off
        // manipulating the cursor position yourself, aka using
        // SetCursorPos/SetCursorScreenPos to position the widgets yourself. You
        // may also want to use the lower-level ImDrawList API)
  ...
  if (ImGui::CollapsingHeader("Popups & Modal windows")) {
    ...
    if (ImGui::TreeNode("Context menus")) {
      ...
      // code. This the comments inside BeginPopupContextItem() implementation.
      ...
      ImGui::Button(buf);
      if (ImGui::BeginPopupContextItem())  // When used after an item that has
                                           // an ID (here the Button), we can
                                           // skip providing an ID to
    ...
    if (ImGui::TreeNode("Modals")) {
      ...
      if (ImGui::BeginPopupModal("Stacked 1")) {
        ...
        ImGui::ColorEdit4("color", color);  // This is to test behavior of
                                            // stacked regular popups over a
                                            // modal
...
*/
  ...
  if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
    ...
    ShowHelpMarker(
        ...
        "a mouse cursor rendered via your application GPU rendering path will "
        ...
        "something).");
    ImGui::CheckboxFlags("io.NavFlags: EnableGamepad",
                         (unsigned int*)&io.NavFlags,
                         ImGuiNavFlags_EnableGamepad);
    ImGui::CheckboxFlags("io.NavFlags: EnableKeyboard",
                         (unsigned int*)&io.NavFlags,
                         ImGuiNavFlags_EnableKeyboard);
    ImGui::CheckboxFlags("io.NavFlags: MoveMouse", (unsigned int*)&io.NavFlags,
                         ImGuiNavFlags_MoveMouse);
    ImGui::SameLine();
    ShowHelpMarker(
        "Request ImGui to move your move cursor when using gamepad/keyboard "
        "navigation. NewFrame() will change io.MousePos and set the "
        "io.WantMoveMouse flag, your backend will need to apply the new mouse "
        "position.");
    ...
    if (ImGui::TreeNode("Focused & Hovered Test")) {
      ...
      if (embed_all_inside_a_child_window)
        ImGui::BeginChild("embeddingchild",
                          ImVec2(0, ImGui::GetFontSize() * 25), true);
      ...
      // Testing IsItemHovered() function (because BulletText is an item itself
      // and that would affect the output of IsItemHovered, we pass all lines in
      // a single items to shorten the code)
      ImGui::Button("ITEM");
      ImGui::BulletText(
          "IsItemHovered() = %d\n"
      ...
      ImGui::Text(
          "This is another child window for testing IsWindowHovered() flags.");
      ImGui::EndChild();

      if (embed_all_inside_a_child_window) EndChild();
    ...
    if (ImGui::TreeNode("Dragging")) {
      ...
      if (ImGui::IsItemActive()) {
        ...
        // You can request a lower or higher threshold using the second
        // parameter of IsMouseDragging() and GetMouseDragDelta()
...
// Here we use the simplified Combo() api that packs items into a single literal
// string. Useful for quick combo boxes where the choices are known locally.
...
void ImGui::ShowFontSelector(const char* label) {
  ...
  ShowHelpMarker(
      ...
      "- If you need to add/remove fonts at runtime (e.g. for DPI change), do "
      "it before calling NewFrame().");
}
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
  ...
  ShowHelpMarker(
      ...
      "are not affected. Use \"Export Colors\" below to save them somewhere.");
  ...
  if (ImGui::TreeNode("Settings")) {
    ...
    ImGui::SliderFloat("WindowBorderSize", &style.WindowBorderSize, 0.0f, 1.0f,
                       "%.0f");
    ...
    ImGui::SliderFloat("FrameBorderSize", &style.FrameBorderSize, 0.0f, 1.0f,
                       "%.0f");
    ImGui::Text("Rounding");
    ImGui::SliderFloat("WindowRounding", &style.WindowRounding, 0.0f, 14.0f,
                       "%.0f");
  ...
  if (ImGui::TreeNode("Colors")) {
    ...
    for (int i = 0; i < ImGuiCol_COUNT; i++) {
      ...
      if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
        ...
        // icon font into the main font, so instead of "Save"/"Revert" you'd use
        // icons.
        // Read the FAQ and misc/fonts/README.txt about using icon fonts. It's
  ...
  ImGui::ColorEdit3("Color", &col.x);
  {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      draw_list->AddCircle(ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f,
                           col32, 20, thickness);
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 0.0f,
...
static void ShowExampleAppLongText(bool* p_open) {
  ...
  ImGui::Combo("Test type", &test_type,
               ...
               "clipped manually\0Multiple calls to Text(), not clipped "
               "(slow)\0");
