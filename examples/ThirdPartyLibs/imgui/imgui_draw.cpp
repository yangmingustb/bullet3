...
void ImDrawList::PathRect(const ImVec2& a, const ImVec2& b, float rounding,
  ...
  rounding = ImMin(
      rounding,
      fabsf(b.x - a.x) * (((rounding_corners & ImDrawCornerFlags_Top) ==
...
void ImGui::ShadeVertsLinearAlphaGradientForLeftToRightText(
  ...
  for (ImDrawVert* vert = vert_end - 1; vert >= vert_start; vert--) {
    ...
    if (alpha_mul >= 1.0f && ++full_alpha_count > 2) return;  // Early out
...
bool ImFontAtlas::GetMouseCursorTexData(ImGuiMouseCursor cursor_type,
  ...
  if (cursor_type <= ImGuiMouseCursor_None ||
      cursor_type >= ImGuiMouseCursor_Count_)
    return false;
...
bool ImFontAtlasBuildWithStbTruetype(ImFontAtlas* atlas) {
  ...
  // they wish.
  // Width doesn't really matter much, but some API/GPU have texture size
  // limitations and increasing width can decrease height.
  ...
  memset(buf_rects, 0, total_glyphs_count *
                           sizeof(stbrp_rect));  // Unnecessary but let's clear
...
// Use the program in misc/fonts/binary_to_compressed_c.cpp to create the array
// from a TTF file.
