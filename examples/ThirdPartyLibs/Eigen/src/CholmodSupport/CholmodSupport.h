...
#define EIGEN_CHOLMOD_SPECIALIZE0(ret, name)            \
  ...
  template <>                                           \
  inline ret cm_##name<long>(cholmod_common & Common) { \
...
#define EIGEN_CHOLMOD_SPECIALIZE1(ret, name, t1, a1)             \
  ...
  template <>                                                    \
  inline ret cm_##name<long>(t1 & a1, cholmod_common & Common) { \
...
template <>
inline cholmod_dense* cm_solve<long>(int sys, cholmod_factor& L,
...
template <>
inline cholmod_sparse* cm_spsolve<long>(int sys, cholmod_factor& L,
                                        cholmod_sparse& B,
                                        cholmod_common& Common) {
...
template <>
inline int cm_factorize_p<long>(cholmod_sparse* A, double beta[2], long* fset,
                                std::size_t fsize, cholmod_factor* L,
                                cholmod_common& Common) {
...
class CholmodBase : public SparseSolverBase<Derived> {
 ...
 public:
  ...
  void _solve_impl(const MatrixBase<Rhs>& b, MatrixBase<Dest>& dest) const {
    ...
    // alignment, etc.)
    // NOTE Actually, the copy can be avoided by calling cholmod_solve2 instead
    // of cholmod_solve
    dest =
  ...
  void _solve_impl(const SparseMatrixBase<RhsDerived>& b,
    ...
    // TODO optimize this copy by swapping when possible (be careful with
    // alignment, etc.)
    // NOTE cholmod_spsolve in fact just calls the dense solver for blocks of 4
    // columns at a time (similar to Eigen's sparse solver)
 ...
 * lower, or both; compressed or non compressed.
  *
  * \warning Only double precision real and complex scalar types are supported
 * by Cholmod.
  *
