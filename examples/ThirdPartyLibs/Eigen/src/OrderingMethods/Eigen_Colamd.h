...
inline IndexType colamd_c(IndexType n_col) {
  return IndexType(((n_col) + 1) * sizeof(colamd_col<IndexType>) /
                   sizeof(IndexType));
}

template <typename IndexType>
inline IndexType colamd_r(IndexType n_row) {
  return IndexType(((n_row) + 1) * sizeof(Colamd_Row<IndexType>) /
                   sizeof(IndexType));
}
...
static IndexType init_rows_cols(IndexType n_row, IndexType n_col,
                                ...
                                colamd_col<IndexType> col[], IndexType A[],
                                IndexType p[], IndexType stats[COLAMD_STATS]);

template <typename IndexType>
static void init_scoring(IndexType n_row, IndexType n_col,
                         Colamd_Row<IndexType> Row[],
                         colamd_col<IndexType> Col[], IndexType A[],
                         IndexType head[], double knobs[COLAMD_KNOBS],
...
template <typename IndexType>
static void order_children(IndexType n_col, colamd_col<IndexType> Col[],
                           IndexType p[]);

template <typename IndexType>
static void detect_super_cols(colamd_col<IndexType> Col[], IndexType A[],
 ...
 */
template <typename IndexType>
static bool colamd(IndexType n_row, IndexType n_col, IndexType Alen,
                   IndexType *A, IndexType *p, double knobs[COLAMD_KNOBS],
...
static IndexType init_rows_cols /* returns true if OK, or false otherwise */
    (
        ...
        Colamd_Row<IndexType>
            Row[], /* of size n_row+1 */
        colamd_col<IndexType>
            Col[],     /* of size n_col+1 */
...
static void init_scoring(
  ...
  dense_row_count = numext::maxi(
      IndexType(0),
      numext::mini(IndexType(knobs[COLAMD_DENSE_ROW] * n_col), n_col));
  dense_col_count = numext::maxi(
      IndexType(0),
      numext::mini(IndexType(knobs[COLAMD_DENSE_COL] * n_row), n_row));
  COLAMD_DEBUG1(
