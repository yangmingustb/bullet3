...
struct ldlt_inplace<Lower> {
  ...
  static bool unblocked(MatrixType& mat, TranspositionType& transpositions,
    ...
    if (size <= 1) {
      ...
      if (numext::real(mat.coeff(0, 0)) > static_cast<RealScalar>(0))
    ...
    for (Index k = 0; k < size; ++k) {
      ...
      // was smaller than the cutoff value. However, since LDLT is not
      // rank-revealing
      ...
      if ((rs > 0) && pivot_is_valid) A21 /= realAkk;
...
void LDLT<_MatrixType, _UpLo>::_solve_impl(const RhsType& rhs,
  ...
  // However, LDLT is not rank revealing, and so adjusting the tolerance wrt to
  // the highest
  // diagonal element is not well justified and leads to numerical issues in
  // some cases.
