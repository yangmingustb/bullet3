...
EIGEN_STRONG_INLINE Packet8f pmadd(const Packet8f& a, const Packet8f& b,
  ...
  // common use case is to accumulate
...
EIGEN_STRONG_INLINE Packet8f ploaddup<Packet8f>(const float* from) {
  ...
  tmp = _mm256_blend_ps(
      tmp, _mm256_castps128_ps256(_mm_permute_ps(_mm256_castps256_ps128(tmp),
                                                 _MM_SHUFFLE(1, 0, 1, 0))),
      15);
...
// NOTE: for the record the following seems to be slower: return
// _mm256_i32gather_ps(from, _mm256_set1_epi32(stride), 4);
template <>
...
// FIXME: why is this ok? why isn't the simply implementation working as
// expected?
template <>
