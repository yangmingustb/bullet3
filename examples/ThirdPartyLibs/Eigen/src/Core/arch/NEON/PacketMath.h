...
// FMA is a true fused multiply-add i.e. only 1 rounding at the end, no
// intermediate rounding.
// MLA is not fused i.e. does 2 roundings.
// In addition to giving better accuracy, FMA also gives better performance here
// on a Krait (Nexus 4):
...
EIGEN_STRONG_INLINE Packet4f pmadd(const Packet4f& a, const Packet4f& b,
  ...
  // at least -mcpu=cortex-a8 and -mcpu=cortex-a7. Since the former is the
  // default on
  ...
  Packet4f r = c;
  asm volatile("vmla.f32 %q[r], %q[a], %q[b]"
               : [r] "+w"(r)
               : [a] "w"(a), [b] "w"(b)
               :);
  return r;
...
template <>
EIGEN_DEVICE_FUNC inline Packet4i pgather<int32_t, Packet4i>(
...
template <>
EIGEN_DEVICE_FUNC inline void pscatter<int32_t, Packet4i>(int32_t* to,
                                                          const Packet4i& from,
                                                          Index stride) {
...
EIGEN_DEVICE_FUNC inline void ptranspose(PacketBlock<Packet4f, 4>& kernel) {
  ...
  kernel.packet[0] =
      vcombine_f32(vget_low_f32(tmp1.val[0]), vget_low_f32(tmp2.val[0]));
  kernel.packet[1] =
      vcombine_f32(vget_high_f32(tmp1.val[0]), vget_high_f32(tmp2.val[0]));
  kernel.packet[2] =
      vcombine_f32(vget_low_f32(tmp1.val[1]), vget_low_f32(tmp2.val[1]));
  kernel.packet[3] =
      vcombine_f32(vget_high_f32(tmp1.val[1]), vget_high_f32(tmp2.val[1]));
}
...
EIGEN_DEVICE_FUNC inline void ptranspose(PacketBlock<Packet4i, 4>& kernel) {
  ...
  kernel.packet[0] =
      vcombine_s32(vget_low_s32(tmp1.val[0]), vget_low_s32(tmp2.val[0]));
  kernel.packet[1] =
      vcombine_s32(vget_high_s32(tmp1.val[0]), vget_high_s32(tmp2.val[0]));
  kernel.packet[2] =
      vcombine_s32(vget_low_s32(tmp1.val[1]), vget_low_s32(tmp2.val[1]));
  kernel.packet[3] =
      vcombine_s32(vget_high_s32(tmp1.val[1]), vget_high_s32(tmp2.val[1]));
}
...
// bug will be fixed.
// https://gist.github.com/yamaya/2924292 suggests that the 3 first digits are
// only updated with
