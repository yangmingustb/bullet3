...
// promoted given the scalar type ExprScalar of the given expression.
...
// the proper order for ExprScalar and T.
...
class compute_matrix_flags {
 ...
 public:
  ...
  // level to handle DenseCoeffsBase<>
  // and then propagate this information to the evaluator's flags.
  // However, I (Gael) think that DirectAccessBit should only matter at the
  // evaluation stage.
...
struct ref_selector {
  ...
  typedef typename conditional<bool(traits<T>::Flags& NestByRefBit), T&,
 ...
 * b+c should be
  * evaluated into the bigger product expression. The choice is between nesting
 * the expression b+c as-is, or
...
struct nested_eval {
  enum {
    ...
    CoeffReadCost = evaluator<T>::CoeffReadCost,  // NOTE What if an evaluator
                                                  // evaluate itself into a
                                                  // tempory?
    //      Then CoeffReadCost will be small (e.g., 1) but we still have to
    //      evaluate, especially if n>1.
    //      This situation is already taken care by the EvalBeforeNestingBit
    //      flag, which is turned ON
    //      for all evaluator creating a temporary. This flag is then propagated
    //      by the parent evaluators.
 ...
 * with kind B.
  * The template parameter ProductTag permits to specialize the resulting
 * storage kind wrt to
  * some compile-time properties of the product: GemmProduct, GemvProduct,
...
/** \class ScalarBinaryOpTraits
  ...
  * By default, the following generic combinations are supported:
  ...
  <tr class="alt"><td>\c NumTraits<T>::Real </td><td>\c T </td><td>\c *
  </td><td>\c T </td><td>Only if \c NumTraits<T>::IsComplex </td></tr>
  <tr            ><td>\c T </td><td>\c NumTraits<T>::Real </td><td>\c *
  </td><td>\c T </td><td>Only if \c NumTraits<T>::IsComplex </td></tr>
  </table>
  *
  * \sa CwiseBinaryOp
...
template <typename T, typename BinaryOp>
struct ScalarBinaryOpTraits<T, typename NumTraits<typename internal::enable_if<
