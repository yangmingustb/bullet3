...
InvertedPendulumPDControl::InvertedPendulumPDControl(
    struct GUIHelperInterface* helper)
    : CommonMultiBodyBase(helper), m_once(true), m_frameCount(0) {}

InvertedPendulumPDControl::~InvertedPendulumPDControl() {}

btMultiBody* createInvertedPendulumMultiBody(btMultiBodyDynamicsWorld* world,
                                             GUIHelperInterface* guiHelper,
                                             const btTransform& baseWorldTrans,
                                             bool fixedBase) {
  btVector4 colors[4] = {
  ...
  int numLinks = 2;
  bool spherical =
      false;  // set it ot false -to use 1DoF hinges instead of 3DoF sphericals
  bool canSleep = false;
  ...
  if (baseMass) {
    // btCollisionShape *shape = new btSphereShape(baseHalfExtents[0]);//
    // btBoxShape(btVector3(baseHalfExtents[0], baseHalfExtents[1],
    // baseHalfExtents[2]));
  ...
  btMultiBody* pMultiBody =
      new btMultiBody(numLinks, 0, baseInertiaDiag, fixedBase, canSleep);
  ...
  for (int i = 0; i < numLinks; ++i) {
    ...
    if (i == 0) {
      shape = new btBoxShape(btVector3(linkHalfExtents[0], linkHalfExtents[1],
                                       linkHalfExtents[2]));  //
    } else {
    ...
    if (!spherical) {
      ...
      if (i == 0) {
        ...
        btVector3 parentComToCurrentCom(
            0, -radius * 2.f, 0);  // par body's COM to cur body's COM offset
        btVector3 currentPivotToCurrentCom(
            0, -radius, 0);  // cur body's COM to cur body's PIV offset
        btVector3 parentComToCurrentPivot =
            parentComToCurrentCom - currentPivotToCurrentCom;  // par body's COM
                                                               // to cur body's
                                                               // PIV offset
  ...
  //  double friction = 1;
  {
    ...
    // float
    // quat[4]={-world_to_local[0].x(),-world_to_local[0].y(),-world_to_local[0].z(),world_to_local[0].w()};

    if (1) {
      btCollisionShape* shape = new btBoxShape(btVector3(
          baseHalfExtents[0], baseHalfExtents[1],
          baseHalfExtents[2]));  // new btSphereShape(baseHalfExtents[0]);
      ...
      world->addCollisionObject(col, collisionFilterGroup,
                                collisionFilterMask);  //, 2,1+2);
  ...
  for (int i = 0; i < pMultiBody->getNumLinks(); ++i) {
    ...
    const btScalar quat[4] = {
        -world_to_local[i + 1].x(), -world_to_local[i + 1].y(),
    ...
    if (i == 0) {
      shape = new btBoxShape(
          btVector3(linkHalfExtents[0], linkHalfExtents[1],
                    linkHalfExtents[2]));  // btSphereShape(linkHalfExtents[0]);
    } else {
      shape = new btSphereShape(radius);
    }
...
void InvertedPendulumPDControl::initPhysics() {
  ...
  m_dynamicsWorld->getDebugDrawer()->setDebugMode(
      ...
      btIDebugDraw::DBG_DrawAabb);  //+btIDebugDraw::DBG_DrawConstraintLimits);

  m_dynamicsWorld->setGravity(btVector3(0, -10, 0));
  btTransform baseWorldTrans;
  baseWorldTrans.setIdentity();
  baseWorldTrans.setOrigin(btVector3(1, 2, 3));
  m_multiBody = createInvertedPendulumMultiBody(m_dynamicsWorld, m_guiHelper,
                                                baseWorldTrans, true);
...
class CommonExampleInterface* InvertedPendulumPDControlCreateFunc(
