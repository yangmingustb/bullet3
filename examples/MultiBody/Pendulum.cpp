...
void Pendulum::initPhysics() {
  ...
  if (m_dynamicsWorld->getDebugDrawer()) {
    m_dynamicsWorld->getDebugDrawer()->setDebugMode(
        ...
        btIDebugDraw::
            DBG_DrawAabb);  //+btIDebugDraw::DBG_DrawConstraintLimits);
  }
  {
    bool floating = false;
    bool damping = false;
    ...
    pMultiBody->updateCollisionObjectWorldTransforms(world_to_local,
                                                     local_origin);
  }
}
...
void Pendulum::stepSimulation(float deltaTime) {
  ...
  btVector3 to =
      m_multiBody->getLink(0).m_collider->getWorldTransform().getOrigin();
  btVector4 color(1, 0, 0, 1);
...
TEST(BulletDynamicsTest, pendulum) {
  ...
  for (int i = 0; i < 2000; i++) {
    ...
    ASSERT_NEAR(setup->m_multiBody->getJointPos(0), sPendulumGold[index],
                0.005);
  }
  setup->exitPhysics();
  delete setup;
...
class CommonExampleInterface* TestPendulumCreateFunc(
