...
void MultiDofDemo::initPhysics() {
  ...
  if (g_firstInit) {
    m_guiHelper->getRenderInterface()->getActiveCamera()->setCameraDistance(
        btScalar(10. * scaling));
  ...
  btMultiBodyDynamicsWorld* world = new btMultiBodyDynamicsWorld(
  ...
  int numLinks = 5;
  bool spherical =
      true;  // set it ot false -to use 1DoF hinges instead of 3DoF sphericals
  ...
  btMultiBody* mbC = createFeatherstoneMultiBody_testMultiDof(
      world, numLinks, btVector3(-0.4f, 3.f, 0.f), linkHalfExtents,
      baseHalfExtents, spherical, g_floatingBase);
  // mbC->forceMultiDof();							//if !spherical, you can comment
  // this line to check the 1DoF algorithm
  ...
  if (!multibodyOnly) {
    ...
    startTransform.setOrigin(btVector3(btScalar(0.0), 0.0, btScalar(0.0)));
    ...
    if (multibodyConstraint) {
      ...
      // btMultiBodySliderConstraint* p2p = new
      // btMultiBodySliderConstraint(mbC,numLinks-1,body,pointInA,pointInB,frameInA,frameInB,jointAxis);
      btMultiBodyFixedConstraint* p2p = new btMultiBodyFixedConstraint(
          mbC, numLinks - 1, mbC, numLinks - 4, pointInA, pointInA, frameInA,
          frameInB);
...
btMultiBody* MultiDofDemo::createFeatherstoneMultiBody_testMultiDof(
...
void MultiDofDemo::addColliders_testMultiDof(btMultiBody* pMultiBody,
                                             btMultiBodyDynamicsWorld* pWorld,
                                             const btVector3& baseHalfExtents,
                                             const btVector3& linkHalfExtents) {
  ...
  {
    //	float
    //pos[4]={local_origin[0].x(),local_origin[0].y(),local_origin[0].z(),1};
  ...
  for (int k = 0; k < ARRAY_SIZE_Y; k++) {
    for (int i = 0; i < ARRAY_SIZE_X; i++) {
      for (int j = 0; j < ARRAY_SIZE_Z; j++) {
        startTransform.setOrigin(btVector3(btScalar(3.0 * i + start_x),
...
class CommonExampleInterface* MultiDofCreateFunc(
