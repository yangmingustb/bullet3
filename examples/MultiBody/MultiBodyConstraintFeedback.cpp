...
MultiBodyConstraintFeedbackSetup::MultiBodyConstraintFeedbackSetup(
    ...
    : CommonMultiBodyBase(helper), m_motor(0), m_once(true) {}
...
void MultiBodyConstraintFeedbackSetup::initPhysics() {
...

  {
    ...
    btVector3 basePosition = btVector3(-0.4f, 3.f, 0.f);
    // mbC->forceMultiDof();							//if !spherical, you can comment this line to
    // check the 1DoF algorithm
    // init the base
    ...
    if (baseMass) {
      ...
      btCollisionShape* shape = new btBoxShape(btVector3(
          baseHalfExtents[0], baseHalfExtents[1], baseHalfExtents[2]));
      shape->calculateLocalInertia(baseMass, baseInertiaDiag);
      delete shape;
    }

    btMultiBody* pMultiBody = new btMultiBody(
        numLinks, baseMass, baseInertiaDiag, !floating, canSleep);
      ...
      if (!spherical) {
        ...
        if (i == 0) {
          ...
          btVector3 parentComToCurrentCom(
              ...
              0);  // par body's COM to cur body's COM offset
          btVector3 currentPivotToCurrentCom(
              0, 0, 0);  // cur body's COM to cur body's PIV offset
          // btVector3 parentComToCurrentPivot = parentComToCurrentCom -
          // currentPivotToCurrentCom;	//par body's COM to cur body's PIV
          // offset
    ...
    for (int i = 0; i < pMultiBody->getNumLinks(); ++i) {
      ...
      if (i == 0) {
        shape = new btBoxShape(btVector3(
            linkHalfExtents[0], linkHalfExtents[1],
    ...
    m_motor =
        new btMultiBodyJointMotor(pMultiBody, link, targetVelocity, maxForce);
...
void MultiBodyConstraintFeedbackSetup::stepSimulation(float deltaTime) {
  ...
  if ((count & 0x0f) == 0) {
    ...
    for (int i = 0; i < m_jointFeedbacks.size(); i++) {
      b3Printf("F_reaction[%i] linear:%f,%f,%f, angular:%f,%f,%f", i,
...
class CommonExampleInterface* MultiBodyConstraintFeedbackCreateFunc(
