...
/// Spheres will come to a rest on a sloped plane using a constraint. Damping
/// cannot achieve the same.
/// Generally it is best to leave the rolling friction coefficient zero (or
/// close to zero).
...
void RollingFrictionDemo::initPhysics() {
  ...
  //	m_dynamicsWorld->getSolverInfo().m_singleAxisRollingFrictionThreshold =
...

  {
    ...
    btCollisionShape* groundShape =
        new btBoxShape(btVector3(btScalar(10.), btScalar(5.), btScalar(25.)));
    ...
    groundTransform.setRotation(
        btQuaternion(btVector3(0, 1, 0), SIMD_PI * 0.03));
...

  {
    ...
    btCollisionShape* groundShape = new btBoxShape(
        btVector3(btScalar(100.), btScalar(100.), btScalar(50.)));
        ...
        for (int i = 0; i < ARRAY_SIZE_X; i++) {
          for (int j = 0; j < ARRAY_SIZE_Z; j++) {
            startTransform.setOrigin(
                SCALING * btVector3(btScalar(2.0 * i + start_x),
