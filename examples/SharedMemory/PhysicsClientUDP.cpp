...
b3ThreadSupportInterface* createUDPThreadSupport(int numThreads) {
  b3PosixThreadSupport::ThreadConstructionInfo constructionInfo(
      "UDPThread", UDPThreadFunc, UDPlsMemoryFunc, UDPlsMemoryReleaseFunc,
      numThreads);
...
struct UdpNetworkedInternalData {
  ...
  bool connectUDP() {
    ...
    m_peer = enet_host_connect(m_client, &m_address, /* address to connect to */
                               2,                    /* number of channels */
        ...
        case ENET_EVENT_TYPE_RECEIVE:
          ...
          if (gVerboseNetworkMessagesClient) {
            printf(
                ...
                m_event.packet->dataLength, (char*)m_event.packet->data,
                (char*)m_event.peer->data, m_event.channelID);
          }
      ...
      switch (m_event.type) {
        ...
        case ENET_EVENT_TYPE_RECEIVE: {
          ...
          if (packetSizeInBytes == m_event.packet->dataLength) {
            SharedMemoryStatus* statPtr =
                (SharedMemoryStatus*)&m_event.packet->data[4];
        ...
        if (args->m_isConnected) {
          ...
          if (hasCommand) {
            ...
            if (args->m_clientCmd.m_type == CMD_STEP_FORWARD_SIMULATION) {
              sz = sizeof(int);
              packet = enet_packet_create(&args->m_clientCmd.m_type, sz,
                                          ENET_PACKET_FLAG_RELIABLE);
            } else {
              sz = sizeof(SharedMemoryCommand);
              packet = enet_packet_create(&args->m_clientCmd, sz,
                                          ENET_PACKET_FLAG_RELIABLE);
            }
            int res;
          ...
          if (hasNewStatus) {
            if (args->m_hasStatus) {
              ...
              b3Assert(0);
              printf(
                  "Error: received new status but previous status not "
                  "processed yet");
            } else {
              args->m_cs->lock();
...
UdpNetworkedPhysicsProcessor::UdpNetworkedPhysicsProcessor(const char* hostName,
                                                           int port) {
...
bool UdpNetworkedPhysicsProcessor::processCommand(
...
bool UdpNetworkedPhysicsProcessor::receiveStatus(
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = false;
...
void UdpNetworkedPhysicsProcessor::setGuiHelper(
