...
struct b3PluginFunctions {
  ...
  b3PluginFunctions(PFN_INIT initFunc, PFN_EXIT exitFunc,
                    PFN_EXECUTE executeCommandFunc)
      : m_initFunc(initFunc),
        m_exitFunc(exitFunc),
...
class b3PluginManager {
 ...
 public:
  ...
  int executePluginCommand(int pluginUniqueId,
                           const struct b3PluginArguments* arguments);
  void addEvents(const struct b3VRControllerEvent* vrControllerEvents,
                 int numVRControllerEvents,
  ...
  int registerStaticLinkedPlugin(const char* pluginPath,
