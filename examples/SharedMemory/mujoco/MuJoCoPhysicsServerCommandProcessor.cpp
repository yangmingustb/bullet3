...
bool MuJoCoPhysicsServerCommandProcessor::processCommand(
...
bool MuJoCoPhysicsServerCommandProcessor::processRequestInternalDataCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processSyncBodyInfoCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processSyncUserDataCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processLoadMJCFCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  if (strlen(fileName) > 0) {
    ...
    bool fileFound = (b3ResourcePath::findResourcePath(
    ...
    if (!fileFound) {
...

      {
        ...
        mjModel* mnew = 0;
        if (strlen(relativeFileName) > 4 &&
            !strcmp(relativeFileName + strlen(relativeFileName) - 4, ".mjb")) {
...
bool MuJoCoPhysicsServerCommandProcessor::processRequestBodyInfoCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  if (m_data->m_mujocoModel && sdfInfoArgs.m_bodyUniqueId >= 0 &&
      sdfInfoArgs.m_bodyUniqueId < m_data->m_mujocoModel->nbody) {
    const char* name =
        m_data->m_mujocoModel->names +
        m_data->m_mujocoModel->name_bodyadr[sdfInfoArgs.m_bodyUniqueId];
...
bool MuJoCoPhysicsServerCommandProcessor::processForwardDynamicsCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  if (m_data->m_mujocoModel) {
    ...
    if (m_data->m_numSimulationSubSteps > 0) {
      for (int i = 0; i < m_data->m_numSimulationSubSteps; i++) {
        m_data->m_mujocoModel->opt.timestep =
...
bool MuJoCoPhysicsServerCommandProcessor::processSendPhysicsParametersCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processRequestActualStateCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  if (m_data->m_mujocoModel) {
    ...
    if (bodyUniqueId >= 0 && bodyUniqueId < m_data->m_mujocoModel->nbody) {
      ...
      serverCmd.m_sendActualStateArgs.m_numLinks =
          0;  // todo body->m_multiBody->getNumLinks();
      ...
      // do we don't use this conditional "if (!mb->hasFixedBase())"
      {
        ...
        int type = (m_data->m_mujocoModel->jnt_type +
                    m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[rootLink];
        // assume mjJNT_FREE?
        int qposAdr =
            (m_data->m_mujocoModel->jnt_qposadr +
             m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[rootLink];
        ...
        serverCmd.m_sendActualStateArgs.m_actualStateQdot[0] =
            cvel[3];  // mb->getBaseVel()[0];
        serverCmd.m_sendActualStateArgs.m_actualStateQdot[1] =
            cvel[4];  // mb->getBaseVel()[1];
        serverCmd.m_sendActualStateArgs.m_actualStateQdot[2] =
            cvel[5];  // mb->getBaseVel()[2];

        // base angular velocity (in world space, carthesian)
        serverCmd.m_sendActualStateArgs.m_actualStateQdot[3] =
            cvel[0];  // mb->getBaseOmega()[0];
        serverCmd.m_sendActualStateArgs.m_actualStateQdot[4] =
            cvel[1];  // mb->getBaseOmega()[1];
        serverCmd.m_sendActualStateArgs.m_actualStateQdot[5] =
            cvel[2];                 // mb->getBaseOmega()[2];
      ...
      for (int l = 0; l < numLinks; l++) {
        int type = (m_data->m_mujocoModel->jnt_type +
                    m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[l];
// int
// type=(m_data->m_mujocoModel->jnt_type+m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[l];
...
bool MuJoCoPhysicsServerCommandProcessor::processResetSimulationCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::receiveStatus(
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  return false;
}
