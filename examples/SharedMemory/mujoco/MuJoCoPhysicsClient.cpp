...
MuJoCoPhysicsClient::MuJoCoPhysicsClient(
    PhysicsCommandProcessorInterface* physSdk, bool passSdkOwnership) {
...
bool MuJoCoPhysicsClient::processDebugLines(
...
bool MuJoCoPhysicsClient::processVisualShapeData(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
  const SharedMemoryStatus& serverCmd = m_data->m_serverStatus;
...
bool MuJoCoPhysicsClient::processOverlappingObjects(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
...
bool MuJoCoPhysicsClient::processContactPointData(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
...
bool MuJoCoPhysicsClient::processCamera(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
...
void MuJoCoPhysicsClient::processBodyJointInfo(
...
void MuJoCoPhysicsClient::processAddUserData(
    const struct SharedMemoryStatus& serverCmd) {
  const b3UserDataGlobalIdentifier userDataGlobalId =
      serverCmd.m_userDataResponseArgs.m_userDataGlobalId;
  BodyJointInfoCache2** bodyJointsPtr =
      m_data->m_bodyJointMap[userDataGlobalId.m_bodyUniqueId];
  if (bodyJointsPtr && *bodyJointsPtr) {
    MuJoCoUserDataCache* userDataCachePtr =
        (*bodyJointsPtr)->m_jointToUserDataMap[userDataGlobalId.m_linkIndex];
    if (!userDataCachePtr) {
      MuJoCoUserDataCache cache;
      (*bodyJointsPtr)
          ->m_jointToUserDataMap.insert(userDataGlobalId.m_linkIndex, cache);
    }
    userDataCachePtr =
        (*bodyJointsPtr)->m_jointToUserDataMap[userDataGlobalId.m_linkIndex];
    ...
    SharedMemoryUserData* userDataPtr =
        userDataCachePtr->m_userDataMap[userDataGlobalId.m_userDataId];
    if (userDataPtr) {
      // Only replace the value.
      (userDataPtr)
          ->replaceValue(dataStream,
                         serverCmd.m_userDataResponseArgs.m_valueLength,
                         userDataValue.m_type);
    } else {
      // Add a new user data entry.
      (userDataCachePtr)
          ->m_userDataMap.insert(
              userDataGlobalId.m_userDataId,
              SharedMemoryUserData(serverCmd.m_userDataResponseArgs.m_key));
      userDataPtr =
          (userDataCachePtr)->m_userDataMap[userDataGlobalId.m_userDataId];
      userDataPtr->replaceValue(dataStream,
                                serverCmd.m_userDataResponseArgs.m_valueLength,
                                userDataValue.m_type);
      (userDataCachePtr)
          ->m_keyToUserDataIdMap.insert(serverCmd.m_userDataResponseArgs.m_key,
                                        userDataGlobalId.m_userDataId);
    }
  }
}

void MuJoCoPhysicsClient::postProcessStatus(
    const struct SharedMemoryStatus& serverCmd) {
  switch (serverCmd.m_type) {
    ...
    case CMD_SYNC_USER_DATA_COMPLETED: {
      ...
      b3UserDataGlobalIdentifier* identifiers =
          new b3UserDataGlobalIdentifier[numIdentifiers];
      memcpy(identifiers, &m_data->m_bulletStreamDataServerToClient[0],
             numIdentifiers * sizeof(b3UserDataGlobalIdentifier));
    ...
    case CMD_REMOVE_USER_DATA_COMPLETED: {
      const b3UserDataGlobalIdentifier userDataGlobalId =
          serverCmd.m_removeUserDataResponseArgs;
      BodyJointInfoCache2** bodyJointsPtr =
          m_data->m_bodyJointMap[userDataGlobalId.m_bodyUniqueId];
      if (bodyJointsPtr && *bodyJointsPtr) {
        MuJoCoUserDataCache* userDataCachePtr =
            (*bodyJointsPtr)
                ->m_jointToUserDataMap[userDataGlobalId.m_linkIndex];
        if (userDataCachePtr) {
          SharedMemoryUserData* userDataPtr =
              (userDataCachePtr)->m_userDataMap[userDataGlobalId.m_userDataId];
          if (userDataPtr) {
            (userDataCachePtr)
                ->m_keyToUserDataIdMap.remove((userDataPtr)->m_key.c_str());
            (userDataCachePtr)
                ->m_userDataMap.remove(userDataGlobalId.m_userDataId);
          }
        }
      }
      break;
...
bool MuJoCoPhysicsClient::submitClientCommand(
...
int MuJoCoPhysicsClient::getUserConstraintInfo(
...
bool MuJoCoPhysicsClient::getBodyInfo(int bodyUniqueId,
...
bool MuJoCoPhysicsClient::getJointInfo(int bodyIndex, int jointIndex,
...
void MuJoCoPhysicsClient::uploadBulletFileToSharedMemory(const char* data,
                                                         int len) {
...
void MuJoCoPhysicsClient::uploadRaysToSharedMemory(
...
void MuJoCoPhysicsClient::getCachedContactPointInformation(
...
void MuJoCoPhysicsClient::getCachedOverlappingObjects(
...
void MuJoCoPhysicsClient::getCachedVisualShapeInformation(
...
void MuJoCoPhysicsClient::getCachedCollisionShapeInformation(
...
void MuJoCoPhysicsClient::getCachedVREvents(
...
void MuJoCoPhysicsClient::getCachedKeyboardEvents(
...
void MuJoCoPhysicsClient::getCachedMouseEvents(
...
void MuJoCoPhysicsClient::getCachedRaycastHits(
...
void MuJoCoPhysicsClient::getCachedMassMatrix(int dofCountCheck,
                                              double* massMatrix) {
...
bool MuJoCoPhysicsClient::getCachedUserData(
    int bodyUniqueId, int linkIndex, int userDataId,
  ...
  MuJoCoUserDataCache* userDataCachePtr =
      (*bodyJointsPtr)->m_jointToUserDataMap[linkIndex];
  if (!userDataCachePtr) {
    return false;
  }
  SharedMemoryUserData* userDataPtr =
      (userDataCachePtr)->m_userDataMap[userDataId];
  if (!userDataPtr) {
    return false;
  }
...
int MuJoCoPhysicsClient::getCachedUserDataId(int bodyUniqueId, int linkIndex,
...
void MuJoCoPhysicsClient::getUserDataInfo(int bodyUniqueId, int linkIndex,
                                          int userDataIndex,
                                          const char** keyOut,
  ...
  MuJoCoUserDataCache* userDataCachePtr =
      (*bodyJointsPtr)->m_jointToUserDataMap[linkIndex];
  if (!userDataCachePtr ||
      userDataIndex >= (userDataCachePtr)->m_userDataMap.size()) {
    *keyOut = 0;
    *userDataIdOut = -1;
    return;
  }
  *userDataIdOut =
      (userDataCachePtr)->m_userDataMap.getKeyAtIndex(userDataIndex).getUid1();
  SharedMemoryUserData* userDataPtr =
      (userDataCachePtr)->m_userDataMap.getAtIndex(userDataIndex);
