...
/// increase the SHARED_MEMORY_MAGIC_NUMBER whenever incompatible changes are
/// made in the structures
...
enum EnumSharedMemoryClientCommand {
  ...
  CMD_CREATE_SENSOR,  /// enable or disable joint feedback for force/torque
                      /// sensors
  CMD_INIT_POSE,
  CMD_SEND_PHYSICS_SIMULATION_PARAMETERS,
  CMD_SEND_DESIRED_STATE,  // todo: reconsider naming, for example
                           // SET_JOINT_CONTROL_VARIABLE?
...
enum EnumSharedMemoryServerStatus {
  ...
  // CMD_CLIENT_COMMAND_COMPLETED is a generic 'completed' status that doesn't
  // need special handling on the client
  CMD_CLIENT_COMMAND_COMPLETED,
  // the server will skip unknown command and report a status
...
struct b3BodyInfo {
  ...
  char m_bodyName[1024];  // for btRigidBody, it does not have a base, but can
                          // still have a body name from urdf
};
...
struct b3JointSensorState {
  ...
  double
      m_jointForceTorque[6]; /* note to roboticists: this is NOT the motor
                                torque/force, but the spatial reaction force
                                vector at joint */
...
struct b3JointSensorState2 {
  ...
  double
      m_jointReactionForceTorque[6]; /* note to roboticists: this is NOT the
                                        motor torque/force, but the spatial
                                        reaction force vector at joint */
...
struct b3VRControllerEvent {
  int m_controllerId;  // valid for VR_CONTROLLER_MOVE_EVENT and
                       // VR_CONTROLLER_BUTTON_EVENT
  int m_deviceType;
  int m_numMoveEvents;
  int m_numButtonEvents;

  float m_pos[4];  // valid for VR_CONTROLLER_MOVE_EVENT and
                   // VR_CONTROLLER_BUTTON_EVENT
  float m_orn[4];  // valid for VR_CONTROLLER_MOVE_EVENT and
                   // VR_CONTROLLER_BUTTON_EVENT

  float m_analogAxis;  // valid if VR_CONTROLLER_MOVE_EVENT
  float m_auxAnalogAxis[MAX_VR_ANALOG_AXIS * 2];  // store x,y per axis, only
                                                  // valid if
                                                  // VR_CONTROLLER_MOVE_EVENT
  int m_buttons[MAX_VR_BUTTONS];  // valid if VR_CONTROLLER_BUTTON_EVENT, see
                                  // b3VRButtonInfo
};
...
struct b3ContactPointData {
  // todo: expose some contact flags, such as telling which fields below are
  // valid
  int m_contactFlags;
  int m_bodyUniqueIdA;
  int m_bodyUniqueIdB;
  int m_linkIndexA;
  int m_linkIndexB;
  double m_positionOnAInWS[3];  // contact point location on object A, in world
                                // space coordinates
  double m_positionOnBInWS[3];  // contact point location on object A, in world
                                // space coordinates
  double m_contactNormalOnBInWS[3];  // the separating contact normal, pointing
                                     // from object B towards object A
  double m_contactDistance;  // negative number is penetration, positive is
                             // distance.
...
struct b3VisualShapeData {
  ...
  // todo: add more data if necessary (material color etc, although material can
  // be in asset file .obj file)
...
struct b3CollisionShapeData {
  ...
  double m_dimensions[3];  // meaning depends on m_visualGeometryType GEOM_BOX:
                           // extents, GEOM_SPHERE: radius,
                           // GEOM_CAPSULE+GEOM_CYLINDER:length, radius,
                           // GEOM_MESH: mesh scale
...
/// b3LinkState provides extra information such as the Cartesian world
/// coordinates
/// center of mass (COM) of the link, relative to the world reference frame.
/// Orientation is a quaternion x,y,z,w
/// Note: to compute the URDF link frame (which equals the joint frame at joint
/// position 0)
...
struct b3LinkState {
  ...
  double m_worldLinearVelocity
      [3];  // only valid when ACTUAL_STATE_COMPUTE_LINKVELOCITY is set
            // (b3RequestActualStateCommandComputeLinkVelocity)
  double m_worldAngularVelocity
      [3];  // only valid when ACTUAL_STATE_COMPUTE_LINKVELOCITY is set
            // (b3RequestActualStateCommandComputeLinkVelocity)

  double m_worldAABBMin[3];  // world space bounding minium and maximum box
                             // corners.
...
enum eURDF_Flags {
  URDF_USE_INERTIA_FROM_FILE =
      2,                        // sync with URDFJointTypes.h 'ConvertURDFFlags'
...
enum eStateLoggingFlags {
  ...
  STATE_LOG_JOINT_TORQUES =
      STATE_LOG_JOINT_MOTOR_TORQUES + STATE_LOG_JOINT_USER_TORQUES,
};
...
struct b3ForwardDynamicsAnalyticsArgs {
  ...
  struct b3ForwardDynamicsAnalyticsIslandData
      m_islandData[MAX_ISLANDS_ANALYTICS];
};
