...
*/
  ...
  bool checkData() {
    ...
    int maxLen =
        4 + sizeof(SharedMemoryStatus) + SHARED_MEMORY_MAX_STREAM_CHUNK_SIZE;
...
TcpNetworkedPhysicsProcessor::TcpNetworkedPhysicsProcessor(const char* hostName,
                                                           int port) {
...
bool TcpNetworkedPhysicsProcessor::processCommand(
...
bool TcpNetworkedPhysicsProcessor::receiveStatus(
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
...
void TcpNetworkedPhysicsProcessor::setGuiHelper(
