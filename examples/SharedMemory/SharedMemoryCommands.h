...
// this is a very experimental draft of commands. We will iterate on this API
...
/// InitPoseArgs is mainly to initialize (teleport) the robot in a particular
/// position
/// No motors or controls are needed to initialize the pose. It is similar to
/// moving a robot to a starting place, while it is switched off. It is only
/// called
/// at the start of a robot control session. All velocities and control forces
/// are cleared to zero.
struct InitPoseArgs {
  int m_bodyUniqueId;
...
enum EnumRequestPixelDataUpdateFlags {
  ...
  // don't exceed (1<<15), because this enum is shared with EnumRenderer in
...
struct RequestRaycastIntersections {
  ...
  // streaming ray data stored in shared memory streaming part. (size
  // m_numStreamingRays )
};
...
/// Controlling a robot involves sending the desired state to its joint motor
/// controllers.
...
struct SendDesiredStateArgs {
  ...
  double m_Kp[MAX_DEGREE_OF_FREEDOM];  // indexed by degree of freedom, 6 for
                                       // base, and then the dofs for each link
  double m_Kd[MAX_DEGREE_OF_FREEDOM];  // indexed by degree of freedom, 6 for
                                       // base, and then the dofs for each link
  double m_rhsClamp[MAX_DEGREE_OF_FREEDOM];

  int m_hasDesiredStateFlags[MAX_DEGREE_OF_FREEDOM];

  // desired state is only written by the client, read-only access by server is
  // expected

  // m_desiredStateQ is indexed by position variables,
  // starting with 3 base position variables, 4 base orientation variables
  // (quaternion), then link position variables
  double m_desiredStateQ[MAX_DEGREE_OF_FREEDOM];

  // m_desiredStateQdot is index by velocity degrees of freedom, 3 linear and 3
  // angular variables for the base and then link velocity variables
  double m_desiredStateQdot[MAX_DEGREE_OF_FREEDOM];

  // m_desiredStateForceTorque is either the actual applied force/torque (in
  // CONTROL_MODE_TORQUE) or
  // or the maximum applied force/torque for the PD/motor/constraint to reach
  // the desired velocity in CONTROL_MODE_VELOCITY and
  // CONTROL_MODE_POSITION_VELOCITY_PD mode
...
struct SendActualStateSharedMemoryStorage {
  // actual state is only written by the server, read-only access by client is
  // expected
  ...
  double m_linkWorldVelocities[6 * MAX_NUM_LINKS];  // linear velocity and
...
struct CreateSensorArgs {
  ...
  /// todo: clean up the duplication, make sure no-one else is using those
  /// members directly (use C-API header instead)
...
struct CalculateInverseKinematicsArgs {
  ...
  double
      m_targetOrientation[4];  // orientation represented as quaternion, x,y,z,w
...
struct StateLoggingRequest {
  ...
  int m_numBodyUniqueIds;  ////only if STATE_LOGGING_FILTER_OBJECT_UNIQUE_ID
                           ///flag is set
  int m_bodyUniqueIds[MAX_SDF_BODIES];
  ...
  int m_bodyUniqueIdA;  // only if STATE_LOGGING_FILTER_BODY_UNIQUE_ID_A flag is
                        // set
  int m_bodyUniqueIdB;  // only if STATE_LOGGING_FILTER_BODY_UNIQUE_ID_B flag is
                        // set
...
struct SyncUserDataRequestArgs {
  // The number of bodies for which we'd like to sync the user data of. When 0,
  // all bodies are synced.
...
struct SharedMemoryCommand {
  ...
  // for example m_updateFlags = SIM_PARAM_UPDATE_DELTA_TIME |
  // SIM_PARAM_UPDATE_NUM_SOLVER_ITERATIONS;
  int m_updateFlags;
  ...
  union {
    ...
    struct ConfigureOpenGLVisualizerRequest
        m_configureOpenGLVisualizerArguments;
