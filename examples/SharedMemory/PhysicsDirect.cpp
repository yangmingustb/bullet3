...
struct PhysicsDirectInternalData {
  ...
  PhysicsDirectInternalData()
    ...
    memset(m_bulletStreamDataServerToClient, 0,
           sizeof(m_bulletStreamDataServerToClient));
  }
};
...
const SharedMemoryStatus* PhysicsDirect::processServerStatus() {
  if (!m_data->m_hasStatus) {
    m_data->m_hasStatus = m_data->m_commandProcessor->receiveStatus(
        m_data->m_serverStatus, &m_data->m_bulletStreamDataServerToClient[0],
        SHARED_MEMORY_MAX_STREAM_CHUNK_SIZE);
  }
...
bool PhysicsDirect::processDebugLines(
    const struct SharedMemoryCommand& orgCommand) {
  ...
  do {
    ...
    if (hasStatus) {
      ...
      float* linesFrom = (float*)&m_data->m_bulletStreamDataServerToClient[0];
      float* linesTo = (float*)(&m_data->m_bulletStreamDataServerToClient[0] +
...
bool PhysicsDirect::processVisualShapeData(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
  const SharedMemoryStatus& serverCmd = m_data->m_serverStatus;
  ...
  do {
    ...
    if (hasStatus) {
      ...
      b3VisualShapeData* shapeData =
          (b3VisualShapeData*)&m_data->m_bulletStreamDataServerToClient[0];
  ...
  } while (serverCmd.m_sendVisualShapeArgs.m_numRemainingVisualShapes > 0 &&
           serverCmd.m_sendVisualShapeArgs.m_numVisualShapesCopied);
  ...
  do {
    ...
    if (hasStatus) {
      ...
      b3OverlappingObject* objects =
          (b3OverlappingObject*)&m_data->m_bulletStreamDataServerToClient[0];
...
bool PhysicsDirect::processContactPointData(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
  ...
  do {
    ...
    if (hasStatus) {
      ...
      b3ContactPointData* contactData =
          (b3ContactPointData*)&m_data->m_bulletStreamDataServerToClient[0];
  ...
  } while (serverCmd.m_sendContactPointArgs.m_numRemainingContactPoints > 0 &&
           serverCmd.m_sendContactPointArgs.m_numContactPointsCopied);

  return m_data->m_hasStatus;
}

bool PhysicsDirect::processCamera(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
  ...
  do {
    ...
    if (hasStatus) {
      ...
      float* depthBuffer = (float*)&(
          m_data->m_bulletStreamDataServerToClient
      ...
      for (int i = 0; i < serverCmd.m_sendPixelDataArguments.m_numPixelsCopied;
           i++) {
      ...
      for (int i = 0; i < serverCmd.m_sendPixelDataArguments.m_numPixelsCopied;
           i++) {
        m_data->m_cachedSegmentationMask
            [i + serverCmd.m_sendPixelDataArguments.m_startingPixelIndex] =
...
bool PhysicsDirect::processMeshData(
    const struct SharedMemoryCommand& orgCommand) {
  SharedMemoryCommand command = orgCommand;
  ...
  do {
    ...
    m_data->m_hasStatus = hasStatus;
    if (hasStatus) {
      btAssert(m_data->m_serverStatus.m_type ==
      ...
      int numTotalPixels = args.m_startingVertex + args.m_numVerticesCopied +
                           args.m_numVerticesRemaining;
      ...
      for (int i = 0; i < args.m_numVerticesCopied; i++) {
        m_data->m_cachedVertexPositions[i + args.m_startingVertex].x =
            verticesReceived[i].x();
        m_data->m_cachedVertexPositions[i + args.m_startingVertex].y =
            verticesReceived[i].y();
        m_data->m_cachedVertexPositions[i + args.m_startingVertex].z =
            verticesReceived[i].z();
        m_data->m_cachedVertexPositions[i + args.m_startingVertex].w =
            verticesReceived[i].w();
      }
  ...
  } while (serverCmd.m_sendMeshDataArgs.m_numVerticesRemaining > 0 &&
           serverCmd.m_sendMeshDataArgs.m_numVerticesCopied);

  return m_data->m_hasStatus;
}

void PhysicsDirect::processBodyJointInfo(int bodyUniqueId,
  ...
  bParse::btBulletFile bf(&m_data->m_bulletStreamDataServerToClient[0],
...
void PhysicsDirect::postProcessStatus(
    const struct SharedMemoryStatus& serverCmd) {
  switch (serverCmd.m_type) {
    case CMD_REQUEST_RAY_CAST_INTERSECTIONS_COMPLETED: {
      ...
      b3RayHitInfo* rayHits =
          (b3RayHitInfo*)m_data->m_bulletStreamDataServerToClient;
    ...
    case CMD_REQUEST_VR_EVENTS_DATA_COMPLETED: {
      ...
      m_data->m_cachedVREvents.resize(
          serverCmd.m_sendVREvents.m_numVRControllerEvents);
      for (int i = 0; i < serverCmd.m_sendVREvents.m_numVRControllerEvents;
           i++) {
        m_data->m_cachedVREvents[i] =
    ...
    case CMD_SDF_LOADING_COMPLETED: {
      ...
      for (int i = 0; i < numConstraints; i++) {
        ...
        m_data->m_tmpInfoRequestCommand.m_updateFlags =
            USER_CONSTRAINT_REQUEST_INFO;
        m_data->m_tmpInfoRequestCommand.m_userConstraintArguments
            .m_userConstraintUniqueId = constraintUid;

        bool hasStatus = m_data->m_commandProcessor->processCommand(
            m_data->m_tmpInfoRequestCommand, m_data->m_tmpInfoStatus,
            &m_data->m_bulletStreamDataServerToClient[0],
            SHARED_MEMORY_MAX_STREAM_CHUNK_SIZE);
        ...
        if (hasStatus) {
          int cid = m_data->m_tmpInfoStatus.m_userConstraintResultArgs
                        .m_userConstraintUniqueId;
          m_data->m_userConstraintInfoMap.insert(
              cid, m_data->m_tmpInfoStatus.m_userConstraintResultArgs);
        }
      }
      ...
      for (int i = 0; i < numBodies; i++) {
        ...
        m_data->m_tmpInfoRequestCommand.m_sdfRequestInfoArgs.m_bodyUniqueId =
            bodyUniqueId;
    ...
    case CMD_ACTUAL_STATE_UPDATE_COMPLETED: {
      SendActualStateSharedMemoryStorage* serverState =
          (SendActualStateSharedMemoryStorage*)&m_data
              ->m_bulletStreamDataServerToClient[0];
      m_data->m_cachedState = *serverState;
      m_data->m_serverStatus.m_sendActualStateArgs.m_stateDetails =
          &m_data->m_cachedState;
      break;
    }
    ...
    case CMD_COLLISION_SHAPE_INFO_COMPLETED: {
      ...
      b3CollisionShapeData* shapeData =
          (b3CollisionShapeData*)&m_data->m_bulletStreamDataServerToClient[0];
    ...
    case CMD_LOAD_SOFT_BODY_COMPLETED: {
      int bodyUniqueId =
          serverCmd.m_loadSoftBodyResultArguments.m_objectUniqueId;
      BodyJointInfoCache2* bodyJoints = new BodyJointInfoCache2;
      m_data->m_bodyJointMap.insert(bodyUniqueId, bodyJoints);
      bodyJoints->m_bodyName = serverCmd.m_dataStreamArguments.m_bodyName;
    ...
    case CMD_SYNC_USER_DATA_COMPLETED: {
      ...
      memcpy(identifiers, &m_data->m_bulletStreamDataServerToClient[0],
             numIdentifiers * sizeof(int));

      for (int i = 0; i < numIdentifiers; i++) {
        m_data->m_tmpInfoRequestCommand.m_type = CMD_REQUEST_USER_DATA;
        m_data->m_tmpInfoRequestCommand.m_userDataRequestArgs.m_userDataId =
            identifiers[i];
...
bool PhysicsDirect::submitClientCommand(
...
int PhysicsDirect::getUserConstraintInfo(int constraintUniqueId,
...
bool PhysicsDirect::getBodyInfo(int bodyUniqueId,
...
bool PhysicsDirect::getJointInfo(int bodyIndex, int jointIndex,
...
void PhysicsDirect::uploadRaysToSharedMemory(
  ...
  if (newNumRays < MAX_RAY_INTERSECTION_BATCH_SIZE_STREAMING) {
    for (int i = 0; i < numRays; i++) {
      b3RayData* rayDataStream =
          (b3RayData*)m_data->m_bulletStreamDataServerToClient;
...
void PhysicsDirect::getCachedCameraImage(b3CameraImageData* cameraData) {
  if (cameraData) {
    ...
    cameraData->m_segmentationMaskValues =
        m_data->m_cachedSegmentationMask.size()
            ? &m_data->m_cachedSegmentationMask[0]
            : 0;
  }
}
...
void PhysicsDirect::getCachedContactPointInformation(
...
void PhysicsDirect::getCachedOverlappingObjects(
...
void PhysicsDirect::getCachedVisualShapeInformation(
...
void PhysicsDirect::getCachedCollisionShapeInformation(
...
void PhysicsDirect::getCachedKeyboardEvents(
...
void PhysicsDirect::getCachedMouseEvents(
...
void PhysicsDirect::getCachedRaycastHits(
...
double PhysicsDirect::getTimeOut() const { return m_data->m_timeOutInSeconds; }

bool PhysicsDirect::getCachedUserData(int userDataId,
...
int PhysicsDirect::getCachedUserDataId(int bodyUniqueId, int linkIndex,
...
void PhysicsDirect::getUserDataInfo(int bodyUniqueId, int userDataIndex,
