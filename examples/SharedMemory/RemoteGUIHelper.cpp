...
struct RemoteGUIHelperInternalData {
  ...
  RemoteGUIHelperInternalData() : m_waitingForServer(false), m_testBlock1(0) {
#ifdef _WIN32
  ...
  bool submitClientCommand(const GraphicsSharedMemoryCommand& command) {
    /// at the moment we allow a maximum of 1 outstanding command, so we check
    /// for this
    // once the server processed the command and returns a status, we clear the
    // flag
  ...
  const GraphicsSharedMemoryStatus* processServerStatus() {
    ...
    if (m_testBlock1->m_numServerCommands >
      ...
      const GraphicsSharedMemoryStatus& serverCmd =
          m_testBlock1->m_serverCommands[0];

      m_lastServerStatus = serverCmd;

      //       EnumSharedMemoryServerStatus s =
      //       (EnumSharedMemoryServerStatus)serverCmd.m_type;
      // consume the command
      ...
      m_testBlock1->m_numProcessedServerCommands++;
      // we don't have more than 1 command outstanding (in total, either server
      // or client)
  ...
  bool connect() {
    ...
    m_testBlock1 =
        (GraphicsSharedMemoryBlock*)m_sharedMemory->allocateSharedMemory(
            m_sharedMemoryKey, GRAPHICS_SHARED_MEMORY_SIZE, allowCreation);

    if (m_testBlock1) {
      if (m_testBlock1->m_magicId != GRAPHICS_SHARED_MEMORY_MAGIC_NUMBER) {
        b3Error(
            "Error connecting to shared memory: please start server before "
            "client\n");
...
RemoteGUIHelper::~RemoteGUIHelper() { delete m_data; }
...
void RemoteGUIHelper::createRigidBodyGraphicsObject(btRigidBody* body,
...
bool RemoteGUIHelper::getCameraInfo(int* width, int* height,
                                    ...
                                    float camForward[3], float hor[3],
                                    float vert[3], float* yaw, float* pitch,
                                    float* camDist, float camTarget[3]) const {
  GraphicsSharedMemoryCommand* cmd = m_data->getAvailableSharedMemoryCommand();
  if (cmd) {
    cmd->m_updateFlags = 0;
...
void RemoteGUIHelper::createCollisionObjectGraphicsObject(
  ...
  if (body->getUserIndex() < 0) {
    ...
    if (graphicsShapeId >= 0) {
      ...
      float colorRGBA[4] = {(float)color[0], (float)color[1], (float)color[2],
                            (float)color[3]};
      float pos[4] = {(float)startTransform.getOrigin()[0],
                      (float)startTransform.getOrigin()[1],
                      (float)startTransform.getOrigin()[2],
                      (float)startTransform.getOrigin()[3]};
      float orn[4] = {(float)startTransform.getRotation()[0],
                      (float)startTransform.getRotation()[1],
                      (float)startTransform.getRotation()[2],
                      (float)startTransform.getRotation()[3]};
      int graphicsInstanceId = registerGraphicsInstance(
          graphicsShapeId, pos, orn, colorRGBA, localScaling);
      body->setUserIndex(graphicsInstanceId);
    }
  }
}

void RemoteGUIHelper::createCollisionShapeGraphicsObject(
    btCollisionShape* collisionShape) {
  printf("createCollisionShapeGraphicsObject\n");
}

void RemoteGUIHelper::syncPhysicsToGraphics(
    const btDiscreteDynamicsWorld* rbWorld) {}

void RemoteGUIHelper::syncPhysicsToGraphics2(
    const btDiscreteDynamicsWorld* rbWorld) {
...
void RemoteGUIHelper::syncPhysicsToGraphics2(const GUISyncPosition* positions,
                                             int numPositions) {
  GraphicsSharedMemoryCommand* cmd = m_data->getAvailableSharedMemoryCommand();
  if (cmd) {
    uploadData((unsigned char*)positions,
               numPositions * sizeof(GUISyncPosition), 0);
    cmd->m_updateFlags = 0;
...
void RemoteGUIHelper::createPhysicsDebugDrawer(
    btDiscreteDynamicsWorld* rbWorld) {}

int RemoteGUIHelper::uploadData(const unsigned char* data, int sizeInBytes,
                                int slot) {
  ...
  while (remainingBytes) {
    ...
    GraphicsSharedMemoryCommand* cmd =
        m_data->getAvailableSharedMemoryCommand();
    if (cmd) {
...
int RemoteGUIHelper::registerTexture(const unsigned char* texels, int width,
                                     int height) {
  int textureId = -1;
...
int RemoteGUIHelper::registerGraphicsShape(const float* vertices,
...
int RemoteGUIHelper::registerGraphicsInstance(int shapeIndex,
  ...
  if (cmd) {
    ...
    if (status->m_type == GFX_CMD_REGISTER_GRAPHICS_INSTANCE_COMPLETED) {
      graphicsInstanceId =
          status->m_registerGraphicsInstanceStatus.m_graphicsInstanceId;
    }
  }
...
void RemoteGUIHelper::changeRGBAColor(int instanceUid,
...
Common2dCanvasInterface* RemoteGUIHelper::get2dCanvasInterface() { return 0; }

CommonParameterInterface* RemoteGUIHelper::getParameterInterface() { return 0; }

CommonRenderInterface* RemoteGUIHelper::getRenderInterface() { return 0; }
...
void RemoteGUIHelper::resetCamera(float camDist, float yaw, float pitch,
                                  float camPosX, float camPosY, float camPosZ) {
}

void RemoteGUIHelper::copyCameraImageData(
    ...
    int segmentationMaskBufferSizeInPixels, int startPixelIndex, int width,
    int height, int* numPixelsCopied)

{
  if (numPixelsCopied) *numPixelsCopied = 0;
}

void RemoteGUIHelper::setProjectiveTextureMatrices(
    const float viewMatrix[16], const float projectionMatrix[16]) {}
...
void RemoteGUIHelper::drawText3D(const char* txt, float posX, float posZY,
                                 float posZ, float size) {}

void RemoteGUIHelper::drawText3D(const char* txt, float position[3],
                                 float orientation[4], float color[4],
                                 float size, int optionFlag) {}

int RemoteGUIHelper::addUserDebugLine(const double debugLineFromXYZ[3],
