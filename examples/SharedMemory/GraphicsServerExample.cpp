...
class GraphicsServerExample : public CommonExampleInterface {
 ...
 public:
  GraphicsServerExample(GUIHelperInterface* guiHelper)
      : m_guiHelper(guiHelper), m_x(0), m_y(0), m_z(0) {
  ...
  virtual ~GraphicsServerExample() { disconnectSharedMemory(); }

  virtual void initPhysics() {}
  virtual void exitPhysics() {}

  GraphicsSharedMemoryStatus& createServerStatus(int statusType,
                                                 int sequenceNumber,
                                                 int timeStamp,
                                                 int blockIndex) {
    GraphicsSharedMemoryStatus& serverCmd =
        m_testBlocks[blockIndex]->m_serverCommands[0];
  ...
  bool processCommand(const struct GraphicsSharedMemoryCommand& clientCmd,
                      struct GraphicsSharedMemoryStatus& serverStatusOut,
    ...
    switch (clientCmd.m_type) {
      ...
      case GFX_CMD_SET_VISUALIZER_FLAG: {
        if (clientCmd.m_visualizerFlagCommand.m_visualizerFlag !=
            COV_ENABLE_RENDERING) {
          // printf("clientCmd.m_visualizerFlag.m_visualizerFlag: %d,
          // clientCmd.m_visualizerFlag.m_enable %d\n",
          //	clientCmd.m_visualizerFlagCommand.m_visualizerFlag,
          //clientCmd.m_visualizerFlagCommand.m_enable);

          this->m_guiHelper->setVisualizerFlag(
              clientCmd.m_visualizerFlagCommand.m_visualizerFlag,
              clientCmd.m_visualizerFlagCommand.m_enable);
        }
        break;
      ...
      case GFX_CMD_UPLOAD_DATA: {
        // printf("uploadData command: curSize=%d, offset=%d, slot=%d",
        // clientCmd.m_uploadDataCommand.m_numBytes,
        // clientCmd.m_uploadDataCommand.m_dataOffset,
        // clientCmd.m_uploadDataCommand.m_dataSlot);
      ...
      case GFX_CMD_REGISTER_TEXTURE: {
        ...
        if (sizeData) {
          ...
          int textureId = this->m_guiHelper->registerTexture(
              texels, clientCmd.m_registerTextureCommand.m_width,
              clientCmd.m_registerTextureCommand.m_height);
      ...
      case GFX_CMD_REGISTER_GRAPHICS_SHAPE: {
        ...
        int numVertices =
            clientCmd.m_registerGraphicsShapeCommand.m_numVertices;
        int numIndices = clientCmd.m_registerGraphicsShapeCommand.m_numIndices;
        int primitiveType =
            clientCmd.m_registerGraphicsShapeCommand.m_primitiveType;
        int textureId = clientCmd.m_registerGraphicsShapeCommand.m_textureId;
        int shapeId = this->m_guiHelper->registerGraphicsShape(
            vertices, numVertices, indices, numIndices, primitiveType,
            textureId);
      ...
      case GFX_CMD_REGISTER_GRAPHICS_INSTANCE: {
        int graphicsInstanceId = m_guiHelper->registerGraphicsInstance(
            clientCmd.m_registerGraphicsInstanceCommand.m_shapeIndex,
        ...
        serverStatusOut.m_registerGraphicsInstanceStatus.m_graphicsInstanceId =
            graphicsInstanceId;
      ...
      case GFX_CMD_SYNCHRONIZE_TRANSFORMS: {
        ...
        for (int i = 0; i < clientCmd.m_syncTransformsCommand.m_numPositions;
             i++) {
          m_app->m_renderer->writeSingleInstanceTransformToCPU(
              positions[i].m_pos, positions[i].m_orn,
              positions[i].m_graphicsInstanceId);
        }
        break;
      }
      ...
      case GFX_CMD_REMOVE_SINGLE_GRAPHICS_INSTANCE: {
        m_app->m_renderer->removeGraphicsInstance(
            clientCmd.m_removeGraphicsInstanceCommand.m_graphicsUid);
        break;
      }
      case GFX_CMD_CHANGE_RGBA_COLOR: {
        m_guiHelper->changeRGBAColor(
            clientCmd.m_changeRGBAColorCommand.m_graphicsUid,
            clientCmd.m_changeRGBAColorCommand.m_rgbaColor);
        break;
      }
      ...
      default: { printf("unsupported command:%d\n", clientCmd.m_type); }
    }
    return true;
  }
  ...
  void processClientCommands() {
    ...
    for (int block = 0; block < MAX_GRAPHICS_SHARED_MEMORY_BLOCKS; block++) {
      if (m_areConnected[block] && m_testBlocks[block]) {
        ...
        if (m_testBlocks[block]->m_numClientCommands >
            m_testBlocks[block]->m_numProcessedClientCommands) {
          // BT_PROFILE("processClientCommand");

          // until we implement a proper ring buffer, we assume always maximum
          // of 1 outstanding commands
          btAssert(m_testBlocks[block]->m_numClientCommands ==
                   m_testBlocks[block]->m_numProcessedClientCommands + 1);

          const GraphicsSharedMemoryCommand& clientCmd =
              m_testBlocks[block]->m_clientCommands[0];

          m_testBlocks[block]->m_numProcessedClientCommands++;
          // todo, timeStamp
          int timeStamp = 0;
          GraphicsSharedMemoryStatus& serverStatusOut =
              createServerStatus(GFX_CMD_CLIENT_COMMAND_FAILED,
                                 clientCmd.m_sequenceNumber, timeStamp, block);
          bool hasStatus =
              processCommand(clientCmd, serverStatusOut,
                             &m_testBlocks[block]->m_bulletStreamData[0],
                             GRAPHICS_SHARED_MEMORY_MAX_STREAM_CHUNK_SIZE);
          if (hasStatus) {
  ...
  bool connectSharedMemory(struct GUIHelperInterface* guiHelper,
                           int sharedMemoryKey);
  void disconnectSharedMemory(bool deInitializeSharedMemory = true);
};

bool GraphicsServerExample::connectSharedMemory(
    struct GUIHelperInterface* guiHelper, int sharedMemoryKey) {
    ...
    do {
      m_testBlocks[block] =
          (GraphicsSharedMemoryBlock*)m_sharedMemory->allocateSharedMemory(
              m_sharedMemoryKey + block, GRAPHICS_SHARED_MEMORY_SIZE,
              allowCreation);
      if (m_testBlocks[block]) {
        ...
        if (m_testBlocks[block]->m_magicId !=
            GRAPHICS_SHARED_MEMORY_MAGIC_NUMBER) {
          ...
          m_sharedMemory->releaseSharedMemory(m_sharedMemoryKey + block,
...
void GraphicsServerExample::disconnectSharedMemory(
    bool deInitializeSharedMemory) {
...
CommonExampleInterface* GraphicsServerCreateFuncBullet(
