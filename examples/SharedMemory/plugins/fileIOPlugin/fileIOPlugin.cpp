...
struct InMemoryFileIO : public CommonFileIOInterface {
  ...
  InMemoryFileIO() : CommonFileIOInterface(eInMemoryFileIO, 0) {
    m_numAllocs = 0;
    m_numFrees = 0;
  ...
  void freeBuffer(char* buffer) { delete[] buffer; }
    ...
    if (fileHandle >= 0 && fileHandle < B3_FILEIO_MAX_FILES) {
      ...
      if (f.m_file) {
        ...
        if (f.m_curPos + numBytes <= f.m_file->m_fileSize) {
          ...
          if (numBytes != 1) {
            printf(
                "InMemoryFileIO::fileRead Attempt to read beyond end of "
                "file\n");
          }
        }
      }
  ...
  virtual int fileWrite(int fileHandle, const char* sourceBuffer,
                        int numBytes) {
    return 0;
  }
  ...
  virtual bool findResourcePath(const char* fileName, char* resourcePathOut,
                                int resourcePathMaxNumBytes) {
    InMemoryFile* f = getInMemoryFile(fileName);
  ...
  virtual char* readLine(int fileHandle, char* destBuffer, int numBytes) {
    ...
    if (numRead < numBytes) {
      ...
      if (endOfFile == 0) {
        printf(
            "InMemoryFileIO::readLine readLine warning: numRead=%d, "
            "numBytes=%d\n",
            numRead, numBytes);
      }
    }
  ...
  virtual void enableFileCaching(bool enable) { (void)enable; }
};
...
struct WrapperFileIO : public CommonFileIOInterface {
            ...
            if (childHandle >= 0) {
              if (m_enableFileCaching) {
                ...
                if (fileSize) {
                  ...
                  if (buffer) {
                    int readBytes =
                        childFileIO->fileRead(childHandle, buffer, fileSize);
                    if (readBytes != fileSize) {
                      if (readBytes < fileSize) {
                        fileSize = readBytes;
                      } else {
                        printf(
                            "WrapperFileIO error: reading more bytes (%d) then "
                            "reported file size (%d) of file %s.\n",
                ...
                // potentially register a zero byte file, or files that only can
                // be read partially
  ...
  virtual bool findResourcePath(const char* fileName, char* resourcePathOut,
                                int resourcePathMaxNumBytes) {
    if (m_cachedFiles.findResourcePath(fileName, resourcePathOut,
                                       resourcePathMaxNumBytes))
      return true;

    bool found = false;
    for (int i = 0; i < B3_MAX_FILEIO_INTERFACES; i++) {
      if (m_availableFileIOInterfaces[i]) {
        found = m_availableFileIOInterfaces[i]->findResourcePath(
            fileName, resourcePathOut, resourcePathMaxNumBytes);
      }
      if (found) break;
    }
    return found;
...
struct FileIOClass {
  ...
  FileIOClass() : m_testData(42), m_fileIO() {}
  virtual ~FileIOClass() {}
};
...
B3_SHARED_API int executePluginCommand_fileIOPlugin(
        ...
        for (int i = 0; i < B3_MAX_FILEIO_INTERFACES; i++) {
          ...
          if (fileIO) {
            if (fileIO->m_fileIOType == fileIOType) {
              if (fileIO->m_pathPrefix &&
                  strcmp(fileIO->m_pathPrefix, arguments->m_text) == 0) {
                result = i;
                alreadyExists = true;
                break;
              }
...
#else
              result = obj->m_fileIO.addFileIOInterface(
                  new b3BulletDefaultFileIO(ePosixFileIO, arguments->m_text));
#endif
              break;
            }
            case eZipFileIO: {
              ...
              if (arguments->m_text[0]) {
                result = obj->m_fileIO.addFileIOInterface(new ZipFileIO(
                    eZipFileIO, arguments->m_text, &obj->m_fileIO));
              }
#else
...
#ifdef B3_USE_CNS_FILEIO
              result = obj->m_fileIO.addFileIOInterface(
                  new CNSFileIO(eCNSFileIO, arguments->m_text));
...
B3_SHARED_API struct CommonFileIOInterface* getFileIOFunc_fileIOPlugin(
    struct b3PluginContext* context) {
  FileIOClass* obj = (FileIOClass*)context->m_userPointer;
  return &obj->m_fileIO;
}
