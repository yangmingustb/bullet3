...
// plugin =
// p.loadPlugin("e:/develop/bullet3/bin/pybullet_vrSyncPlugin_vs2010_x64_release.dll")
...
B3_SHARED_API int preTickPluginCallback_vrSyncPlugin(
        ...
        for (int n = 0; n < context->m_numVRControllerEvents; n++) {
          ...
          if (event.m_controllerId == obj->m_controllerId) {
            if (obj->m_constraintId >= 0) {
              ...
              if (b3GetUserConstraintInfo(context->m_physClient,
                                          obj->m_constraintId,
                                          &constraintInfo)) {
                // this is basically equivalent to doing this in
                // Python/pybullet:
                // p.changeConstraint(pr2_cid, e[POSITION], e[ORIENTATION],
                // maxForce=...)
                ...
                double pos[4] = {event.m_pos[0], event.m_pos[1], event.m_pos[2],
                                 1};
                b3InitChangeUserConstraintSetPivotInB(commandHandle, pos);
                double orn[4] = {event.m_orn[0], event.m_orn[1], event.m_orn[2],
                                 event.m_orn[3]};
                b3InitChangeUserConstraintSetFrameInB(commandHandle, orn);
                b3InitChangeUserConstraintSetMaxForce(commandHandle,
                                                      obj->m_maxForce);
                b3SharedMemoryStatusHandle statusHandle =
                    b3SubmitClientCommandAndWaitStatus(context->m_physClient,
                                                       commandHandle);
              }
            }
            // apply the analogue button to close the constraint, using a gear
            // constraint with position target
            if (obj->m_constraintId2 >= 0) {
              ...
              if (b3GetUserConstraintInfo(context->m_physClient,
                ...
                // p.changeConstraint(c,gearRatio=1, erp=...,
                // relativePositionTarget=relPosTarget, maxForce=...)
            ...
            // use the pr2_gripper motors to keep the gripper centered/symmetric
            ...
            if (obj->m_gripperId >= 0) {
              ...
              // targetPosition=b, force=3)

              // printf("obj->m_gripperId=%d\n", obj->m_gripperId);
              {
                b3SharedMemoryCommandHandle cmd_handle =
                    b3RequestActualStateCommandInit(context->m_physClient,
                                                    obj->m_gripperId);
                ...
                if (status_type == CMD_ACTUAL_STATE_UPDATE_COMPLETED) {
                  ...
                  if (b3GetJointState(context->m_physClient, status_handle, 2,
                                      &sensorState)) {
                    b3SharedMemoryCommandHandle commandHandle;
                    ...
                    if (1) {
                      b3JointInfo info;
                      b3GetJointInfo(context->m_physClient, obj->m_gripperId, 0,
                                     &info);
                      commandHandle = b3JointControlCommandInit2(
                          context->m_physClient, obj->m_gripperId,
                          CONTROL_MODE_POSITION_VELOCITY_PD);
                      double kp = .1;
                      double targetVelocity = 0;
                      double kd = .6;
                      b3JointControlSetDesiredPosition(
                          commandHandle, info.m_qIndex, targetPosition);
                      b3JointControlSetKp(commandHandle, info.m_uIndex, kp);
                      b3JointControlSetDesiredVelocity(
                          commandHandle, info.m_uIndex, targetVelocity);
                      b3JointControlSetKd(commandHandle, info.m_uIndex, kd);
                      b3JointControlSetMaximumForce(
                          commandHandle, info.m_uIndex, obj->m_maxForce2);
                      b3SubmitClientCommandAndWaitStatus(context->m_physClient,
                                                         cmd_handle);
                    }
                  } else {
...
B3_SHARED_API int executePluginCommand_vrSyncPlugin(
    struct b3PluginContext* context,
  ...
  if (arguments->m_numInts >= 4 && arguments->m_numFloats >= 2) {
    ...
    b3SharedMemoryCommandHandle command =
        b3InitSyncBodyInfoCommand(context->m_physClient);
    b3SharedMemoryStatusHandle statusHandle =
        b3SubmitClientCommandAndWaitStatus(context->m_physClient, command);
