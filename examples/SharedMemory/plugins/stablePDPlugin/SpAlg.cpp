...
cSpAlg::tSpVec cSpAlg::ConvertCoordM(const tSpVec& m0, const tVector& origin0,
...
Eigen::MatrixXd cSpAlg::ApplyTransM(const tSpTrans& X,
...
Eigen::MatrixXd cSpAlg::ApplyTransF(const tSpTrans& X,
                                    const Eigen::MatrixXd& sm) {
  assert(sm.rows() == gSpVecSize);
  Eigen::MatrixXd result(gSpVecSize, sm.cols());
  for (int i = 0; i < sm.cols(); ++i) {
...
Eigen::MatrixXd cSpAlg::ApplyInvTransM(const tSpTrans& X,
                                       const Eigen::MatrixXd& sm) {
  assert(sm.rows() == gSpVecSize);
...
Eigen::MatrixXd cSpAlg::ApplyInvTransF(const tSpTrans& X,
                                       const Eigen::MatrixXd& sm) {
  assert(sm.rows() == gSpVecSize);
  Eigen::MatrixXd result(sm.rows(), sm.cols());
  for (int i = 0; i < sm.cols(); ++i) {
