...
bool btExtractJointBodyFromTempLinks(btAlignedObjectArray<TempLink>& links,
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    if (links[j].m_collider) {
      const btCollisionShape* collisionShape =
          links[j].m_collider->getCollisionShape();
      if (collisionShape->isCompound()) {
        const btCompoundShape* compound =
            (const btCompoundShape*)collisionShape;
    ...
    // tQuaternion body_to_parent_body = parent_to_parent_body * this_to_parent
    // * body_to_this;
    ...
    btVector3 body_attach_pt2 =
        quatRotate(links[j].m_this_to_body1.inverse(), body_attach_pt1);
    ...
    body_to_this1.getEulerZYX(bodyAttachThetaZ, bodyAttachThetaY,
                              bodyAttachThetaX);
    ...
    jointAttachPointMy =
        quatRotate(linkToParentBody.inverse(), jointAttachPointMy);
    ...
    btQuaternion recoverthis2parent = parent2parent_body2.inverse() *
                                      combined2 *
                                      this2bodyA;  // body2this.inverse();
...
void btExtractJointBodyFromBullet(const btMultiBody* bulletMB,
    ...
    switch (bulletMB->getBaseCollider()->getCollisionShape()->getShapeType()) {
      ...
      case COMPOUND_SHAPE_PROXYTYPE: {
        btCompoundShape* compound =
            (btCompoundShape*)bulletMB->getBaseCollider()->getCollisionShape();
  ...
  if (numBaseShapes) {
    ...
    links[0].m_jointType = (bulletMB->hasFixedBase())
                               ? cKinTree::eJointTypeFixed
                               : cKinTree::eJointTypeNone;
  ...
  for (int j = 0; j < bulletMB->getNumLinks(); ++j) {
    ...
    if ((baseLink) == 0 && j == 0)  // for 'root' either use fixed or none
    {
      ...
      links[j].m_jointType = (bulletMB->hasFixedBase())
                                 ? cKinTree::eJointTypeFixed
                                 : cKinTree::eJointTypeNone;
      ...
      links[j].m_zeroRotParentToThis =
          bulletMB->getLink(j).m_zeroRotParentToThis;
      ...
      links[j + baseLink].m_zeroRotParentToThis =
          bulletMB->getLink(j).m_zeroRotParentToThis;
