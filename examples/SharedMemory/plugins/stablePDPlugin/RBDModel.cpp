...
void cRBDModel::Init(const Eigen::MatrixXd& joint_mat,
                     const Eigen::MatrixXd& body_defs, const tVector& gravity) {
  ...
  mChildParentMatArr =
      Eigen::MatrixXd::Zero(num_joints * trans_mat.rows(), trans_mat.cols());
  mSpWorldJointTransArr = Eigen::MatrixXd::Zero(
      num_joints * cSpAlg::gSVTransRows, cSpAlg::gSVTransCols);
...
void cRBDModel::Update(const Eigen::VectorXd& pose,
...
void cRBDModel::SetGravity(const tVector& gravity) { mGravity = gravity; }
...
const Eigen::Block<const Eigen::MatrixXd> cRBDModel::GetJointSubspace(
    int j) const {
  assert(j >= 0 && j < GetNumJoints());
...
void cRBDModel::InitJointSubspaceArr() {
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    int dim = cKinTree::GetParamSize(mJointMat, j);
    int r = static_cast<int>(mJointSubspaceArr.rows());
    mJointSubspaceArr.block(0, offset, r, dim) =
        cRBDUtil::BuildJointSubspace(mJointMat, mPose, j);
  }
}
...
void cRBDModel::UpdateChildParentMatArr() {
  int num_joints = GetNumJoints();
  for (int j = 0; j < num_joints; ++j) {
    tMatrix child_parent_trans =
        cKinTree::ChildParentTrans(mJointMat, mPose, j);
