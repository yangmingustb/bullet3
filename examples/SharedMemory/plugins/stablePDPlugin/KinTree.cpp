...
const std::string gJointDescKeys[cKinTree::eJointDescMax] = {
    "Type",         "Parent",        "AttachX",      "AttachY",  "AttachZ",
    "AttachThetaX", "AttachThetaY",  "AttachThetaZ", "LimLow0",  "LimLow1",
    "LimLow2",      "LimHigh0",      "LimHigh1",     "LimHigh2", "TorqueLim",
    "ForceLim",     "IsEndEffector", "DiffWeight",   "Offset"};
...
const std::string gDrawShapeDescKeys[cKinTree::eDrawShapeParamMax] = {
    "Shape",   "ParentJoint",  "AttachX",      "AttachY",
    "AttachZ", "AttachThetaX", "AttachThetaY", "AttachThetaZ",
    "Param0",  "Param1",       "Param2",       "ColorR",
    "ColorG",  "ColorB",       "ColorA",       "MeshID"};
...
void cKinTree::FindChildren(const Eigen::MatrixXd& joint_desc, int joint_id,
                            Eigen::VectorXi& out_children) {
...
#ifdef USE_JSON
bool cKinTree::LoadBodyDefs(const std::string& char_file,
...
bool cKinTree::ParseBodyDef(const Json::Value& root,
                            cKinTree::tBodyDef& out_def) {
  std::string shape_str =
      root.get(gBodyDescKeys[eBodyParamShape], "").asString();
  cShape::eShape shape;
...
bool cKinTree::LoadDrawShapeDefs(const std::string& char_file,
...
bool cKinTree::ParseDrawShapeDef(const Json::Value& root,
                                 tDrawShapeDef& out_def) {
  std::string shape_str =
      root.get(gDrawShapeDescKeys[eDrawShapeShape], "").asString();
  cShape::eShape shape;
  bool succ = cShape::ParseShape(shape_str, shape);
  if (succ) {
...
bool cKinTree::ParseJoint(const Json::Value& root, tJointDesc& out_joint_desc) {
  ...
  if (type_json.isNull()) {
    ...
    out_joint_desc[eJointDescType] =
        static_cast<double>(static_cast<int>(joint_type));
  }
...
std::string cKinTree::BuildJointJson(int id, const tJointDesc& joint_desc) {
  ...
  for (int i = 0; i < eJointDescMax; ++i) {
    ...
    if (i == eJointDescType) {
      ...
      if (!std::isfinite(val) &&
          (i == eJointDescTorqueLim || i == eJointDescForceLim)) {
        continue;
      }
...
tVector cKinTree::CalcBodyPartPos(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::MatrixXd& body_defs,
...
tVector cKinTree::CalcBodyPartVel(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::MatrixXd& body_defs,
                                  const Eigen::VectorXd& state,
                                  const Eigen::VectorXd& vel, int part_id) {
  tVector attach_pt = cKinTree::GetBodyAttachPt(body_defs, part_id);
  return CalcWorldVel(joint_mat, state, vel, part_id, attach_pt);
}

cShape::eShape cKinTree::GetBodyShape(const Eigen::MatrixXd& body_defs,
                                      int part_id) {
  cShape::eShape shape = static_cast<cShape::eShape>(
      static_cast<int>(body_defs(part_id, cKinTree::eBodyParamShape)));
  return shape;
}

tVector cKinTree::GetBodyAttachPt(const Eigen::MatrixXd& body_defs,
                                  int part_id) {
  const cKinTree::tBodyDef& def = body_defs.row(part_id);
  tVector attach_pt = tVector(def(eBodyParamAttachX), def(eBodyParamAttachY),
                              def(eBodyParamAttachZ), 0);
  return attach_pt;
}

tVector cKinTree::GetBodyAttachTheta(const Eigen::MatrixXd& body_defs,
                                     int part_id) {
...
void cKinTree::GetBodyRotation(const Eigen::MatrixXd& body_defs, int part_id,
...
bool cKinTree::GetBodyEnableFallContact(const Eigen::MatrixXd& body_defs,
                                        int part_id) {
  double fall_val = body_defs(part_id, eBodyParamEnableFallContact);
  return fall_val != 0;
}

void cKinTree::SetBodyEnableFallContact(int part_id, bool enable,
                                        Eigen::MatrixXd& out_body_defs) {
...
tVector cKinTree::GetBodySize(const Eigen::MatrixXd& body_defs, int part_id) {
  ...
  tVector size =
      tVector(def(eBodyParam0), def(eBodyParam1), def(eBodyParam2), 0);
  return size;
}

tVector cKinTree::GetBodyColor(const Eigen::MatrixXd& body_defs, int part_id) {
  const tBodyDef& def = body_defs.row(part_id);
  tVector col = tVector(def(eBodyColorR), def(eBodyColorG), def(eBodyColorB),
                        def(eBodyColorA));
  return col;
}
...
tVector cKinTree::GetBodyLocalCoM(const Eigen::MatrixXd& body_defs,
                                  int part_id) {
  cShape::eShape shape = GetBodyShape(body_defs, part_id);
...
tVector cKinTree::GetDrawShapeAttachPt(const tDrawShapeDef& shape) {
  return tVector(shape[eDrawShapeAttachX], shape[eDrawShapeAttachY],
                 shape[cKinTree::eDrawShapeAttachZ], 0);
}

tVector cKinTree::GetDrawShapeAttachTheta(const tDrawShapeDef& shape) {
  return tVector(shape[eDrawShapeAttachThetaX], shape[eDrawShapeAttachThetaY],
                 shape[eDrawShapeAttachThetaZ], 0);
}

void cKinTree::GetDrawShapeRotation(const tDrawShapeDef& shape,
                                    tVector& out_axis, double& out_theta) {
...
tVector cKinTree::GetDrawShapeColor(const tDrawShapeDef& shape) {
  return tVector(shape[eDrawShapeColorR], shape[eDrawShapeColorG],
                 shape[eDrawShapeColorB], shape[cKinTree::eDrawShapeColorA]);
}
...
void cKinTree::CalcBodyPartRotation(const Eigen::MatrixXd& joint_mat,
                                    const Eigen::MatrixXd& body_defs,
                                    const Eigen::VectorXd& state, int part_id,
                                    tVector& out_axis, double& out_theta) {
...
tVector cKinTree::LocalToWorldPos(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::VectorXd& state, int parent_id,
...
tQuaternion cKinTree::CalcJointWorldRot(const Eigen::MatrixXd& joint_mat,
                                        const Eigen::VectorXd& state,
                                        int joint_id) {
...
void cKinTree::CalcJointWorldTheta(const Eigen::MatrixXd& joint_mat,
                                   const Eigen::VectorXd& state, int joint_id,
                                   tVector& out_axis, double& out_theta) {
  tMatrix mat = JointWorldTrans(joint_mat, state, joint_id);
  cMathUtil::RotMatToAxisAngle(mat, out_axis, out_theta);
}

tVector cKinTree::CalcJointWorldVel(const Eigen::MatrixXd& joint_mat,
                                    const Eigen::VectorXd& state,
                                    const Eigen::VectorXd& vel, int joint_id) {
  return CalcWorldVel(joint_mat, state, vel, joint_id, tVector::Zero());
}

tVector cKinTree::CalcWorldVel(const Eigen::MatrixXd& joint_mat,
                               const Eigen::VectorXd& state,
                               const Eigen::VectorXd& vel, int parent_id,
                               const tVector& attach_pt) {
  cSpAlg::tSpVec sv = cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
  tVector pos =
      cKinTree::LocalToWorldPos(joint_mat, state, parent_id, attach_pt);
  cSpAlg::tSpTrans world_to_pt = cSpAlg::BuildTrans(pos);
  sv = cSpAlg::ApplyTransM(world_to_pt, sv);
  return cSpAlg::GetV(sv);
}

tVector cKinTree::CalcJointWorldAngularVel(const Eigen::MatrixXd& joint_mat,
                                           const Eigen::VectorXd& state,
                                           const Eigen::VectorXd& vel,
                                           int joint_id) {
...
tVector cKinTree::CalcWorldAngularVel(const Eigen::MatrixXd& joint_mat,
                                      const Eigen::VectorXd& state,
                                      const Eigen::VectorXd& vel, int parent_id,
                                      const tVector& attach_pt) {
  cSpAlg::tSpVec sv = cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
...
int cKinTree::GetNumDof(const Eigen::MatrixXd& joint_mat) {
  int num_joints = GetNumJoints(joint_mat);
  int num_dof = cKinTree::GetParamOffset(joint_mat, num_joints - 1) +
                cKinTree::GetParamSize(joint_mat, num_joints - 1);
  return num_dof;
}

void cKinTree::ApplyStep(const Eigen::MatrixXd& joint_mat,
...
Eigen::VectorXi cKinTree::FindJointChain(const Eigen::MatrixXd& joint_mat,
                                         int joint_beg, int joint_end) {
...
bool cKinTree::IsAncestor(const Eigen::MatrixXd& joint_mat, int child_joint,
                          int ancestor_joint, int& out_len) {
...
double cKinTree::CalcChainLength(const Eigen::MatrixXd& joint_mat,
                                 const Eigen::VectorXi& chain) {
  double len = 0;
...
void cKinTree::CalcAABB(const Eigen::MatrixXd& joint_desc,
                        const Eigen::VectorXd& state, tVector& out_min,
                        tVector& out_max) {
...
void cKinTree::GetJointParams(const Eigen::MatrixXd& joint_mat,
                              const Eigen::VectorXd& state, int j,
...
void cKinTree::SetJointParams(const Eigen::MatrixXd& joint_mat, int j,
...
cKinTree::eJointType cKinTree::GetJointType(const Eigen::MatrixXd& joint_mat,
                                            int joint_id) {
  eJointType type = static_cast<eJointType>(
      static_cast<int>(joint_mat(joint_id, cKinTree::eJointDescType)));
  return type;
}
...
double cKinTree::GetTorqueLimit(const Eigen::MatrixXd& joint_mat,
                                int joint_id) {
...
tVector cKinTree::GetJointLimLow(const Eigen::MatrixXd& joint_mat,
                                 int joint_id) {
...
tVector cKinTree::GetJointLimHigh(const Eigen::MatrixXd& joint_mat,
                                  int joint_id) {
...
double cKinTree::GetJointDiffWeight(const Eigen::MatrixXd& joint_mat,
                                    int joint_id) {
  return joint_mat(joint_id, cKinTree::eJointDescDiffWeight);
}

double cKinTree::CalcLinkLength(const Eigen::MatrixXd& joint_mat,
                                int joint_id) {
...
tVector cKinTree::GetAttachPt(const Eigen::MatrixXd& joint_mat, int joint_id) {
  tVector attach_pt =
      tVector(joint_mat(joint_id, cKinTree::eJointDescAttachX),
              joint_mat(joint_id, cKinTree::eJointDescAttachY),
              joint_mat(joint_id, cKinTree::eJointDescAttachZ), 0);
  return attach_pt;
}

tVector cKinTree::GetAttachTheta(const Eigen::MatrixXd& joint_mat,
                                 int joint_id) {
  tVector attach_theta =
      tVector(joint_mat(joint_id, cKinTree::eJointDescAttachThetaX),
              joint_mat(joint_id, cKinTree::eJointDescAttachThetaY),
              joint_mat(joint_id, cKinTree::eJointDescAttachThetaZ), 0);
  return attach_theta;
}

void cKinTree::CalcMaxSubChainLengths(const Eigen::MatrixXd& joint_mat,
                                      Eigen::VectorXd& out_lengths) {
...
void cKinTree::CalcSubTreeMasses(const Eigen::MatrixXd& joint_mat,
                                 const Eigen::MatrixXd& body_defs,
...
bool cKinTree::ParseJointType(const std::string& type_str,
                              eJointType& out_joint_type) {
...
tMatrix cKinTree::BuildAttachTrans(const Eigen::MatrixXd& joint_mat,
                                   int joint_id) {
  // child to parent
  tVector attach_pt = GetAttachPt(joint_mat, joint_id);
...
tMatrix cKinTree::ChildParentTrans(const Eigen::MatrixXd& joint_mat,
                                   const Eigen::VectorXd& state, int joint_id) {
  tMatrix mat;
...
tMatrix cKinTree::ParentChildTrans(const Eigen::MatrixXd& joint_mat,
                                   const Eigen::VectorXd& state, int joint_id) {
...
tMatrix cKinTree::JointWorldTrans(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::VectorXd& state, int joint_id) {
...
tMatrix cKinTree::WorldJointTrans(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::VectorXd& state, int joint_id) {
...
tMatrix cKinTree::BodyWorldTrans(const Eigen::MatrixXd& joint_mat,
                                 const Eigen::MatrixXd& body_defs,
                                 const Eigen::VectorXd& state, int part_id) {
...
tMatrix cKinTree::BodyJointTrans(const Eigen::MatrixXd& body_defs,
                                 int part_id) {
...
cKinTree::tJointDesc cKinTree::BuildJointDesc(eJointType joint_type,
                                              int parent_id,
                                              const tVector& attach_pt) {
...
void cKinTree::BuildDefaultPose(const Eigen::MatrixXd& joint_mat,
                                Eigen::VectorXd& out_pose) {
...
void cKinTree::BuildDefaultVel(const Eigen::MatrixXd& joint_mat,
                               Eigen::VectorXd& out_vel) {
  int num_dof = GetNumDof(joint_mat);
...
void cKinTree::CalcPoseDiff(const Eigen::MatrixXd& joint_mat,
...
tVector cKinTree::CalcRootPosDiff(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::VectorXd& pose0,
                                  const Eigen::VectorXd& pose1) {
...
tQuaternion cKinTree::CalcRootRotDiff(const Eigen::MatrixXd& joint_mat,
                                      const Eigen::VectorXd& pose0,
                                      const Eigen::VectorXd& pose1) {
...
double cKinTree::CalcPoseErr(const Eigen::MatrixXd& joint_mat, int joint_id,
                             const Eigen::VectorXd& pose0,
                             const Eigen::VectorXd& pose1) {
  bool is_root = IsRoot(joint_mat, joint_id);
  double err = 0;
...
double cKinTree::CalcRootPosErr(const Eigen::MatrixXd& joint_mat,
                                const Eigen::VectorXd& pose0,
                                const Eigen::VectorXd& pose1) {
...
double cKinTree::CalcRootRotErr(const Eigen::MatrixXd& joint_mat,
                                const Eigen::VectorXd& pose0,
                                const Eigen::VectorXd& pose1) {
...
void cKinTree::CalcVelDiff(const Eigen::MatrixXd& joint_mat,
...
tVector cKinTree::CalcRootVelDiff(const Eigen::MatrixXd& joint_mat,
                                  const Eigen::VectorXd& pose0,
                                  const Eigen::VectorXd& pose1) {
...
tVector cKinTree::CalcRootAngVelDiff(const Eigen::MatrixXd& joint_mat,
                                     const Eigen::VectorXd& pose0,
                                     const Eigen::VectorXd& pose1) {
...
double cKinTree::CalcVelErr(const Eigen::MatrixXd& joint_mat, int joint_id,
                            const Eigen::VectorXd& vel0,
                            const Eigen::VectorXd& vel1) {
  bool is_root = IsRoot(joint_mat, joint_id);
  double err = 0;
...
double cKinTree::CalcRootVelErr(const Eigen::MatrixXd& joint_mat,
                                const Eigen::VectorXd& vel0,
                                const Eigen::VectorXd& vel1) {
...
double cKinTree::CalcRootAngVelErr(const Eigen::MatrixXd& joint_mat,
                                   const Eigen::VectorXd& vel0,
                                   const Eigen::VectorXd& vel1) {
...
void cKinTree::CalcJointPoseDiff(const Eigen::MatrixXd& joint_mat, int joint_id,
  ...
  if (is_root) {
    ...
    switch (joint_type) {
      case eJointTypeSpherical: {
        tQuaternion q0 =
            cMathUtil::VecToQuat(pose0.segment(param_offset, param_size));
        tQuaternion q1 =
            cMathUtil::VecToQuat(pose1.segment(param_offset, param_size));
...
void cKinTree::CalcJointVelDiff(const Eigen::MatrixXd& joint_mat, int joint_id,
  ...
  int param_offset = GetParamOffset(joint_mat, joint_id);
  int param_size = GetParamSize(joint_mat, joint_id);
  out_diff = vel1.segment(param_offset, param_size) -
             vel0.segment(param_offset, param_size);
}

void cKinTree::CalcVel(const Eigen::MatrixXd& joint_mat,
                       const Eigen::VectorXd& pose0,
                       const Eigen::VectorXd& pose1, double dt,
                       Eigen::VectorXd& out_vel) {
  ...
  tQuaternion root_rot0 = GetRootRot(joint_mat, pose0);
  tQuaternion root_rot1 = GetRootRot(joint_mat, pose1);
  tVector root_rot_vel =
      cMathUtil::CalcQuaternionVelRel(root_rot0, root_rot1, dt);
    ...
    switch (joint_type) {
      case eJointTypeSpherical: {
        ...
        out_vel.segment(param_offset, param_size) =
            rot_vel.segment(0, param_size);
        break;
      }
      default:
        out_vel.segment(param_offset, param_size) =
            (pose1.segment(param_offset, param_size) -
             pose0.segment(param_offset, param_size)) /
            dt;
        break;
    }
...
void cKinTree::LerpPoses(const Eigen::MatrixXd& joint_mat,
                         const Eigen::VectorXd& pose0,
                         const Eigen::VectorXd& pose1, double lerp,
                         Eigen::VectorXd& out_pose) {
  int num_joints = GetNumJoints(joint_mat);
  int root_id = GetRoot(joint_mat);
  int root_offset = GetParamOffset(joint_mat, root_id);
  ...
  for (int j = 1; j < num_joints; ++j) {
    ...
    if (joint_type == eJointTypeSpherical) {
      ...
      out_pose.segment(offset, size) =
          cMathUtil::QuatToVec(rot_lerp).segment(0, size);
    } else {
      out_pose.segment(offset, size) =
          (1 - lerp) * pose0.segment(offset, size) +
          lerp * pose1.segment(offset, size);
    }
  }
}

void cKinTree::VelToPoseDiff(const Eigen::MatrixXd& joint_mat,
...
double cKinTree::CalcHeading(const Eigen::MatrixXd& joint_mat,
  ...
  printf("root_rot=%f,%f,%f,%f\n", root_rot.x(), root_rot.y(), root_rot.z(),
         root_rot.w());
...
tQuaternion cKinTree::CalcHeadingRot(const Eigen::MatrixXd& joint_mat,
                                     const Eigen::VectorXd& pose) {
  double heading = CalcHeading(joint_mat, pose);
  return cMathUtil::AxisAngleToQuaternion(tVector(0, 1, 0, 0), heading);
}

tMatrix cKinTree::BuildHeadingTrans(const Eigen::MatrixXd& joint_mat,
                                    const Eigen::VectorXd& pose) {
  double heading = CalcHeading(joint_mat, pose);
...
tMatrix cKinTree::BuildOriginTrans(const Eigen::MatrixXd& joint_mat,
                                   const Eigen::VectorXd& pose) {
  // world to origin transform
  // origin is the point right under the root of the character on the xz plane
  // with x-axis
...
void cKinTree::NormalizePoseHeading(const Eigen::MatrixXd& joint_mat,
                                    Eigen::VectorXd& out_pose) {
...
void cKinTree::NormalizePoseHeading(const Eigen::MatrixXd& joint_mat,
  ...
  tQuaternion heading_q =
      cMathUtil::AxisAngleToQuaternion(tVector(0, 1, 0, 0), -heading);
...
tMatrix cKinTree::ChildParentTransRoot(const Eigen::MatrixXd& joint_mat,
                                       const Eigen::VectorXd& state,
                                       int joint_id) {
...
tMatrix cKinTree::ChildParentTransRevolute(const Eigen::MatrixXd& joint_mat,
                                           const Eigen::VectorXd& state,
                                           int joint_id) {
...
tMatrix cKinTree::ChildParentTransPlanar(const Eigen::MatrixXd& joint_mat,
                                         const Eigen::VectorXd& state,
                                         int joint_id) {
  int param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
...
tMatrix cKinTree::ChildParentTransFixed(const Eigen::MatrixXd& joint_mat,
                                        const Eigen::VectorXd& state,
                                        int joint_id) {
  tMatrix A = BuildAttachTrans(joint_mat, joint_id);
  tMatrix mat = A;
...
tMatrix cKinTree::ChildParentTransPrismatic(const Eigen::MatrixXd& joint_mat,
                                            const Eigen::VectorXd& state,
                                            int joint_id) {
  int param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
  tVector offset = tVector::Zero();
...
tMatrix cKinTree::ChildParentTransSpherical(const Eigen::MatrixXd& joint_mat,
                                            const Eigen::VectorXd& state,
                                            int joint_id) {
  int param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
