...
#include "math.h"
void cRBDUtil::SolveInvDyna(const cRBDModel& model, const Eigen::VectorXd& acc,
  ...
  for (int j = 0; j < num_joints; ++j) {
    if (cKinTree::IsValidBody(body_defs, j)) {
      ...
      cSpAlg::tSpVec curr_vel =
          cSpAlg::ApplyTransM(parent_child_trans, vel_p) + vj;
      cSpAlg::tSpVec curr_acc = cSpAlg::ApplyTransM(parent_child_trans, acc_p) +
                                Sddq + cj + cSpAlg::CrossM(curr_vel, vj);
      cSpAlg::tSpVec curr_f =
          I * curr_acc + cSpAlg::CrossF(curr_vel, I * curr_vel);
...
void cRBDUtil::SolveForDyna(const cRBDModel& model, const Eigen::VectorXd& tau,
...
void cRBDUtil::SolveForDyna(const cRBDModel& model, const Eigen::VectorXd& tau,
                            const Eigen::VectorXd& total_force,
                            Eigen::VectorXd& out_acc) {
  Eigen::VectorXd C;
  Eigen::MatrixXd H;
...
void cRBDUtil::BuildMassMat(const cRBDModel& model,
                            Eigen::MatrixXd& out_mass_mat) {
...
void cRBDUtil::BuildMassMat(const cRBDModel& model,
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    cSpAlg::tSpTrans child_parent_trans = model.GetSpChildParentTrans(j);
    cSpAlg::tSpMat child_parent_mat =
        cSpAlg::BuildSpatialMatF(child_parent_trans);
    cSpAlg::tSpMat parent_child_mat =
        cSpAlg::BuildSpatialMatM(cSpAlg::InvTrans(child_parent_trans));
  ...
  for (int j = num_joints - 1; j >= 0; --j) {
    if (cKinTree::IsValidBody(body_defs, j)) {
      Eigen::Block<Eigen::MatrixXd, -1, -1, false> curr_I =
          Is.block(j * svs, 0, svs, svs);
      int parent_id = cKinTree::GetParent(joint_mat, j);
      if (cKinTree::HasParent(joint_mat, j)) {
        cSpAlg::tSpTrans child_parent_trans = model.GetSpChildParentTrans(j);
        cSpAlg::tSpMat child_parent_mat =
            child_parent_mats_F.block(j * svs, 0, svs, svs);
        cSpAlg::tSpMat parent_child_mat =
            parent_child_mats_M.block(j * svs, 0, svs, svs);
        Is.block(parent_id * svs, 0, svs, svs) +=
            child_parent_mat * curr_I * parent_child_mat;
      }
      ...
      if (dim > 0) {
        ...
        while (cKinTree::HasParent(joint_mat, curr_id)) {
          cSpAlg::tSpMat child_parent_mat =
              child_parent_mats_F.block(curr_id * svs, 0, svs, svs);
          ...
          if (curr_dim > 0) {
            const Eigen::Block<const Eigen::MatrixXd> curr_S =
                model.GetJointSubspace(curr_id);
            H.block(offset, curr_offset, dim, curr_dim) =
                F.transpose() * curr_S;
            H.block(curr_offset, offset, curr_dim, dim) =
                H.block(offset, curr_offset, dim, curr_dim).transpose();
          }
        }
      }
    }
...
void cRBDUtil::BuildEndEffectorJacobian(const cRBDModel& model, int joint_id,
  ...
  while (curr_id != cKinTree::gInvalidJointID) {
    ...
    out_J.block(0, offset, cSpAlg::gSpVecSize, size) =
        cSpAlg::ApplyTransM(curr_trans, S);
...
void cRBDUtil::BuildEndEffectorJacobian(const Eigen::MatrixXd& joint_mat,
                                        const Eigen::VectorXd& pose,
                                        int joint_id, Eigen::MatrixXd& out_J) {
  // jacobian in world coordinates
  int num_dofs = cKinTree::GetNumDof(joint_mat);
  out_J = Eigen::MatrixXd::Zero(cSpAlg::gSpVecSize, num_dofs);
  ...
  while (curr_id != cKinTree::gInvalidJointID) {
    ...
    int parent_id = cKinTree::GetParent(joint_mat, curr_id);
    cSpAlg::tSpTrans parent_child_trans =
        BuildParentChildTransform(joint_mat, pose, curr_id);
    curr_trans = cSpAlg::CompTrans(curr_trans, parent_child_trans);
    curr_id = parent_id;
  }
...
Eigen::MatrixXd cRBDUtil::MultJacobianEndEff(const Eigen::MatrixXd& joint_mat,
  ...
  while (curr_id != cKinTree::gInvalidJointID) {
    ...
    const Eigen::Block<const Eigen::MatrixXd> curr_J =
        J.block(0, offset, cSpAlg::gSpVecSize, size);
...
void cRBDUtil::BuildJacobian(const cRBDModel& model, Eigen::MatrixXd& out_J) {
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    out_J.block(0, offset, cSpAlg::gSpVecSize, size) =
        cSpAlg::ApplyInvTransM(world_joint_trans, S);
  }
}

Eigen::MatrixXd cRBDUtil::ExtractEndEffJacobian(
    const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& J, int joint_id) {
  int curr_id = joint_id;
...
void cRBDUtil::BuildCOMJacobian(const cRBDModel& model,
                                Eigen::MatrixXd& out_J) {
...
void cRBDUtil::BuildCOMJacobian(const cRBDModel& model,
  ...
  for (int j = num_joints - 1; j >= 0; --j) {
    if (cKinTree::IsValidBody(body_defs, j)) {
      ...
      cSpAlg::tSpTrans body_world_trans =
          cSpAlg::CompTrans(cSpAlg::InvTrans(world_child_trans),
                            cSpAlg::MatToTrans(body_joint_mat));
      ...
      while (curr_id != cKinTree::gInvalidJointID) {
        ...
        const Eigen::MatrixXd& J_block =
            J.block(0, offset, cSpAlg::gSpVecSize, size);
        Eigen::Block<Eigen::MatrixXd, -1, -1, false> J_com_block =
            out_J.block(0, offset, cSpAlg::gSpVecSize, size);
...
void cRBDUtil::BuildJacobianDot(const cRBDModel& model,
                                Eigen::MatrixXd& out_J_dot) {
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    int offset = cKinTree::GetParamOffset(joint_mat, j);
    int size = cKinTree::GetParamSize(joint_mat, j);
    out_J_dot.block(0, offset, cSpAlg::gSpVecSize, size) =
        cSpAlg::CrossMs(parent_Sq, S);
  }
}
...
cSpAlg::tSpVec cRBDUtil::BuildCOMVelProdAccAux(const cRBDModel& model,
...
cSpAlg::tSpVec cRBDUtil::CalcVelProdAcc(const cRBDModel& model,
                                        const Eigen::MatrixXd& Jd,
                                        int joint_id) {
  const Eigen::MatrixXd& joint_mat = model.GetJointMat();
  const Eigen::VectorXd& pose = model.GetPose();
  ...
  while (curr_id != cKinTree::gInvalidJointID) {
    ...
    const Eigen::Block<const Eigen::MatrixXd> curr_Jd =
        Jd.block(0, offset, cSpAlg::gSpVecSize, size);
...
cSpAlg::tSpVec cRBDUtil::CalcJointWorldVel(const cRBDModel& model,
                                           int joint_id) {
  cSpAlg::tSpVec joint_vel = CalcJointWorldVel(
      model.GetJointMat(), model.GetPose(), model.GetVel(), joint_id);
  return joint_vel;
}

cSpAlg::tSpVec cRBDUtil::CalcJointWorldVel(const Eigen::MatrixXd& joint_mat,
...
cSpAlg::tSpVec cRBDUtil::CalcWorldVel(const cRBDModel& model, int joint_id) {
  return CalcWorldVel(model.GetJointMat(), model.GetPose(), model.GetVel(),
                      joint_id);
}

cSpAlg::tSpVec cRBDUtil::CalcWorldVel(const Eigen::MatrixXd& joint_mat,
                                      const Eigen::VectorXd& pose,
                                      const Eigen::VectorXd& vel,
                                      int joint_id) {
  Eigen::MatrixXd J;
...
tVector cRBDUtil::CalcCoMVel(const Eigen::MatrixXd& joint_mat,
...
void cRBDUtil::CalcCoM(const cRBDModel& model, tVector& out_com,
                       tVector& out_vel) {
  const Eigen::MatrixXd& joint_mat = model.GetJointMat();
  const Eigen::MatrixXd& body_defs = model.GetBodyDefs();
  const Eigen::VectorXd& pose = model.GetPose();
  ...
  for (int j = 0; j < num_joints; ++j) {
    if (cKinTree::IsValidBody(body_defs, j)) {
      ...
      tMatrix joint_world_mat =
          cSpAlg::TransToMat(cSpAlg::InvTrans(world_joint_trans));
      tMatrix body_joint_mat = cKinTree::BodyJointTrans(body_defs, j);
...
void cRBDUtil::CalcCoM(const Eigen::MatrixXd& joint_mat,
                       const Eigen::MatrixXd& body_defs,
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertia(const Eigen::MatrixXd& body_defs,
                                            int part_id) {
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaBox(const Eigen::MatrixXd& body_defs,
                                               int part_id) {
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaCapsule(
    const Eigen::MatrixXd& body_defs, int part_id) {
  const cKinTree::tBodyDef& def = body_defs.row(part_id);
  double mass = cKinTree::GetBodyMass(body_defs, part_id);
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaSphere(
    const Eigen::MatrixXd& body_defs, int part_id) {
  const cKinTree::tBodyDef& def = body_defs.row(part_id);
  double mass = cKinTree::GetBodyMass(body_defs, part_id);
  double r = 0.5 * def(cKinTree::eBodyParam0);
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaCylinder(
    const Eigen::MatrixXd& body_defs, int part_id) {
  const cKinTree::tBodyDef& def = body_defs.row(part_id);
  double mass = cKinTree::GetBodyMass(body_defs, part_id);
  double r = 0.5 * def(cKinTree::eBodyParam0);
...
cSpAlg::tSpMat cRBDUtil::BuildInertiaSpatialMat(
    const Eigen::MatrixXd& body_defs, int part_id) {
  ...
  cSpAlg::tSpMat Io = cSpAlg::BuildSpatialMatF(X) * Ic *
                      cSpAlg::BuildSpatialMatM(cSpAlg::InvTrans(X));
  return Io;
}

void cRBDUtil::CalcWorldJointTransforms(const cRBDModel& model,
                                        Eigen::MatrixXd& out_trans_arr) {
  const Eigen::MatrixXd& joint_mat = model.GetJointMat();
  const Eigen::VectorXd& pose = model.GetPose();
  ...
  for (int j = 0; j < num_joints; ++j) {
    ...
    cSpAlg::tSpTrans world_child_trans =
        cSpAlg::CompTrans(parent_child_trans, world_parent_trans);
    out_trans_arr.block(row_idx, 0, cSpAlg::gSVTransRows,
                        cSpAlg::gSVTransCols) = world_child_trans;
  }
}

cSpAlg::tSpTrans cRBDUtil::BuildParentChildTransform(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
  cSpAlg::tSpTrans trans = BuildChildParentTransform(joint_mat, pose, j);
  trans = cSpAlg::InvTrans(trans);
  return trans;
}

cSpAlg::tSpTrans cRBDUtil::BuildChildParentTransform(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspace(const Eigen::MatrixXd& joint_mat,
                                             const Eigen::VectorXd& pose,
                                             int j) {
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspaceRoot(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose) {
  ...
  S.block(0, pos_dim, 3, rot_dim - 1) =
      E.block(0, 0, 3, rot_dim - 1).transpose();
  return S;
}

Eigen::MatrixXd cRBDUtil::BuildJointSubspaceRevolute(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspacePrismatic(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspacePlanar(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspaceFixed(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspaceSpherical(
    const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, int j) {
...
cSpAlg::tSpVec cRBDUtil::BuildCj(const Eigen::MatrixXd& joint_mat,
                                 const Eigen::VectorXd& q,
...
cSpAlg::tSpVec cRBDUtil::BuildCjRoot(const Eigen::MatrixXd& joint_mat,
                                     const Eigen::VectorXd& q,
                                     const Eigen::VectorXd& q_dot, int j) {
...
cSpAlg::tSpVec cRBDUtil::BuildCjRevolute(const Eigen::MatrixXd& joint_mat,
                                         const Eigen::VectorXd& q_dot, int j) {
  return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjPrismatic(const Eigen::MatrixXd& joint_mat,
                                          const Eigen::VectorXd& q_dot, int j) {
  return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjPlanar(const Eigen::MatrixXd& joint_mat,
                                       const Eigen::VectorXd& q_dot, int j) {
  return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjFixed(const Eigen::MatrixXd& joint_mat,
                                      const Eigen::VectorXd& q_dot, int j) {
  return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjSpherical(const Eigen::MatrixXd& joint_mat,
                                          const Eigen::VectorXd& q_dot, int j) {
  return cSpAlg::tSpVec::Zero();
}

void cRBDUtil::BuildBiasForce(const cRBDModel& model,
                              Eigen::VectorXd& out_bias_force) {
  Eigen::VectorXd acc = Eigen::VectorXd::Zero(model.GetNumDof());
  SolveInvDyna(model, acc, out_bias_force);
}

void cRBDUtil::CalcGravityForce(const cRBDModel& model,
                                Eigen::VectorXd& out_g_force) {
  const Eigen::MatrixXd& joint_mat = model.GetJointMat();
  const Eigen::MatrixXd& body_defs = model.GetBodyDefs();
  const tVector& gravity = model.GetGravity();
  const Eigen::VectorXd& pose = model.GetPose();
