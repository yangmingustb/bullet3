...
tMatrix cMathUtil::RotateMat(const tVector& axis, double theta) {
  ...
  mat << c + x * x * (1 - c), x * y * (1 - c) - z * s, x * z * (1 - c) + y * s,
      0, y * x * (1 - c) + z * s, c + y * y * (1 - c), y * z * (1 - c) - x * s,
      0, z * x * (1 - c) - y * s, z * y * (1 - c) + x * s, c + z * z * (1 - c),
      0, 0, 0, 0, 1;
...
tVector cMathUtil::RotMatToEuler(const tMatrix& mat) {
  ...
  euler[1] = std::atan2(
      -mat(2, 0), std::sqrt(mat(2, 1) * mat(2, 1) + mat(2, 2) * mat(2, 2)));
...
tQuaternion cMathUtil::AxisAngleToQuaternion(const tVector& axis,
                                             double theta) {
...
tMatrix cMathUtil::BuildQuaternionDiffMat(const tQuaternion& q) {
  tMatrix mat;
  mat << -0.5 * q.x(), -0.5 * q.y(), -0.5 * q.z(), 0, 0.5 * q.w(), -0.5 * q.z(),
      0.5 * q.y(), 0, 0.5 * q.z(), 0.5 * q.w(), -0.5 * q.x(), 0, -0.5 * q.y(),
      0.5 * q.x(), 0.5 * q.w(), 0;
  return mat;
}
