...
struct btHashVisual {
  ...
  bool equals(const btHashVisual& other) const {
    if ((m_vis.m_geometry.m_type == URDF_GEOM_MESH) &&
      ...
      bool sameVis = m_vis.m_geometry.m_meshFileName ==
                     other.m_vis.m_geometry.m_meshFileName;
      bool sameLocalFrame =
          m_vis.m_linkLocalFrame == other.m_vis.m_linkLocalFrame;
...
struct EGLRendererVisualShapeConverterInternalData {
  ...
  EGLRendererVisualShapeConverterInternalData()
    ...
    m_instancingRenderer =
        new GLInstancingRenderer(maxNumObjectCapacity, maxShapeCapacityInBytes);
    b3Assert(glGetError() == GL_NO_ERROR);
...
static void SimpleWheelCallback(float deltax, float deltay) {
  ...
  if (gWindow && gWindow->m_data->m_instancingRenderer) {
    class GLInstancingRenderer* renderer =
        gWindow->m_data->m_instancingRenderer;
    b3Vector3 cameraTargetPosition, cameraPosition,
        cameraUp = b3MakeVector3(0, 0, 0);
    ...
    if (!m_leftMouseButton) {
      ...
      if (deltay < 0 || cameraDistance > 1) {
        ...
        cameraTargetPosition +=
            fwd * deltay * wheelMultiplier;  // todo: expose it in the GUI?
      }
    } else {
    ...
    camera->setCameraTargetPosition(cameraTargetPosition[0],
...
void EGLRendererVisualShapeConverter::mouseButtonCallback(int button, int state,
                                                          float x, float y) {
  if (button == 0) m_data->m_leftMouseButton = (state == 1);
  if (button == 1) m_data->m_middleMouseButton = (state == 1);

  if (button == 2) m_data->m_rightMouseButton = (state == 1);
...
void EGLRendererVisualShapeConverter::mouseMoveCallback(float x, float y) {
  ...
  if (isAltPressed || isControlPressed) {
    ...
    b3Vector3 cameraPosition = b3MakeVector3(
        b3Scalar(camPos[0]), b3Scalar(camPos[1]), b3Scalar(camPos[2]));

    b3Vector3 cameraTargetPosition = b3MakeVector3(
        b3Scalar(targPos[0]), b3Scalar(targPos[1]), b3Scalar(targPos[2]));
...
void EGLRendererVisualShapeConverter::setLightDirection(float x, float y,
                                                        float z) {
...
void EGLRendererVisualShapeConverter::setLightSpecularCoeff(
    float specularCoeff) {
...
static void convertURDFToVisualShape2(
    const UrdfShape* visual, const char* urdfPathPrefix,
    const btTransform& visualTransform,
    btAlignedObjectArray<GLInstanceVertex>& verticesOut,
    btAlignedObjectArray<int>& indicesOut,
    btAlignedObjectArray<MyTexture3>& texturesOut,
    b3VisualShapeData& visualShapeOut, struct CommonFileIOInterface* fileIO,
    int flags) {
  ...
  memset(visualShapeOut.m_meshAssetFileName, 0,
         sizeof(visualShapeOut.m_meshAssetFileName));
#if 0
  ...
  switch (visual->m_geometry.m_type) {
    ...
    case URDF_GEOM_CAPSULE: {
      ...
      if (visual->m_geometry.m_hasFromTo) {
        ...
        for (int i = 0; i < numSteps; i++) {
          {
            btVector3 vert =
                p1 + ax1 * rad * btSin(SIMD_2_PI * (float(i) / numSteps)) +
                ax2 * rad * btCos(SIMD_2_PI * (float(i) / numSteps));
            vertices.push_back(vert);
          }
          {
            btVector3 vert =
                p2 + ax1 * rad * btSin(SIMD_2_PI * (float(i) / numSteps)) +
                ax2 * rad * btCos(SIMD_2_PI * (float(i) / numSteps));
            vertices.push_back(vert);
          }
        }
        ...
        rad = visual->m_geometry.m_capsuleRadius;
        for (int i = 0; i < numSteps; i++) {
          btVector3 vert(rad * btSin(SIMD_2_PI * (float(i) / numSteps)),
                         rad * btCos(SIMD_2_PI * (float(i) / numSteps)),
                         len / 2.);
          vertices.push_back(vert);
          vert[2] = -len / 2.;
          vertices.push_back(vert);
      ...
      btConvexHullShape* cylZShape = new btConvexHullShape(
          &vertices[0].x(), vertices.size(), sizeof(btVector3));
      // btCapsuleShape* cylZShape = new
      // btCapsuleShape(rad,len);//btConvexHullShape(&vertices[0].x(),
    ...
    case URDF_GEOM_BOX: {
      ...
      // btConvexShape* boxShape = new
      // btConeShapeX(extents[2]*0.5,extents[0]*0.5);
    ...
    case URDF_GEOM_MESH: {
      strncpy(visualShapeOut.m_meshAssetFileName,
      ...
      switch (visual->m_geometry.m_meshFileType) {
        case UrdfGeometry::FILE_OBJ: {
          ...
          if (b3ImportMeshUtility::loadAndRegisterMeshFromFileInternal(
                  visual->m_geometry.m_meshFileName, meshData, fileIO)) {
        ...
        case UrdfGeometry::FILE_COLLADA: {
          ...
          LoadMeshFromCollada(visual->m_geometry.m_meshFileName.c_str(),
                              visualShapes, visualShapeInstances, upAxisTrans,
                              unitMeterScaling, upAxis, fileIO);
          ...
          for (int i = 0; i < visualShapeInstances.size(); i++) {
            ...
            GLInstanceGraphicsShape* gfxShape =
                &visualShapes[instance->m_shapeIndex];
            ...
            for (int k = 0; k < additionalIndices; k++) {
              glmesh->m_indices->at(curNumIndices + k) =
                  gfxShape->m_indices->at(k) + baseIndex;
            }
            ...
            unitMeterScalingMat.setPureScaling(btVector3(
                unitMeterScaling, unitMeterScaling, unitMeterScaling));
            btMatrix4x4 worldMat =
                unitMeterScalingMat * upAxisMat * instance->m_worldTransform;
            ...
            for (int v = 0; v < verts.size(); v++) {
              btVector3 pos(verts[v].xyzw[0], verts[v].xyzw[1],
                            verts[v].xyzw[2]);
              pos = worldMat * pos;
        ...
        default:
          // should never get here (findExistingMeshFile returns false if it
          // doesn't recognize extension)
          btAssert(0);
      }
      ...
      if (glmesh && glmesh->m_vertices && (glmesh->m_numvertices > 0)) {
        ...
        for (int i = 0; i < glmesh->m_vertices->size(); i++) {
          glmesh->m_vertices->at(i).xyzw[0] *=
              visual->m_geometry.m_meshScale[0];
          glmesh->m_vertices->at(i).xyzw[1] *=
              visual->m_geometry.m_meshScale[1];
          glmesh->m_vertices->at(i).xyzw[2] *=
              visual->m_geometry.m_meshScale[2];
        }
      } else {
...
int EGLRendererVisualShapeConverter::convertVisualShapes(
  ...
  btAssert(linkPtr);  // TODO: remove if (not doing it now, because diff will be
                      // 50+ lines)
  if (linkPtr) {
    bool useVisual;
    int cnt = 0;
    ...
    if (linkPtr->m_visualArray.size() > 0) {
      ...
      // We have to see something, take collision shape. Useful for MuJoCo xml,
      // where there are no explicit visual shapes.
      useVisual = false;
    ...
    for (int v1 = 0; v1 < cnt; v1++) {
      ...
      int colorIndex = linkIndex;  // colObj?
      ...
      float rgbaColor[4] = {(float)color[0], (float)color[1], (float)color[2],
                            (float)color[3]};
      // if
      // (colObj->getCollisionShape()->getShapeType()==STATIC_PLANE_PROXYTYPE)
      //{
      //	color.setValue(1,1,1,1);
      //}
      if (model) {
        if (useVisual) {
          btHashString matName(
              linkPtr->m_visualArray[v1].m_materialName.c_str());
          UrdfMaterial* const* matPtr = model->m_materials[matName];
          if (matPtr) {
            for (int i = 0; i < 4; i++) {
              rgbaColor[i] = (*matPtr)->m_matColor.m_rgbaColor[i];
            }
            // printf("UrdfMaterial %s, rgba =
            ...
            if (vis && vis->m_geometry.m_hasLocalMaterial) {
              for (int i = 0; i < 4; i++) {
                rgbaColor[i] =
                    vis->m_geometry.m_localMaterial.m_matColor.m_rgbaColor[i];
              }
            }
          }
        }
      ...
      EGLRendererObjectArray** visualsPtr =
          m_data->m_swRenderInstances[orgGraphicsUniqueId];
      if (visualsPtr == 0) {
        m_data->m_swRenderInstances.insert(orgGraphicsUniqueId,
                                           new EGLRendererObjectArray);
      }
      ...
      visualShape.m_localVisualFrame[3] =
          vis->m_linkLocalFrame.getRotation()[0];
      visualShape.m_localVisualFrame[4] =
          vis->m_linkLocalFrame.getRotation()[1];
      visualShape.m_localVisualFrame[5] =
          vis->m_linkLocalFrame.getRotation()[2];
      visualShape.m_localVisualFrame[6] =
          vis->m_linkLocalFrame.getRotation()[3];
      ...
      btHashVisual tmp;
      {
        ...
        if (bla) {
          ...
          convertURDFToVisualShape2(vis, pathPrefix, tr, vertices, indices,
                                    textures, visualShape, fileIO,
                                    m_data->m_flags);
        }
      }
        ...
        if (vertices.size() && indices.size()) {
          ...
          if (textures.size()) {
            ...
            if (bla) {
              ...
              textureIndex = m_data->m_instancingRenderer->registerTexture(
...

      {
        ...
        if (shapeIndex < 0) {
          shapeIndex = m_data->m_instancingRenderer->registerShape(
        ...
        int graphicsIndex =
            m_data->m_instancingRenderer->registerGraphicsInstance(
                shapeIndex, &visualShape.m_localVisualFrame[0],
                &visualShape.m_localVisualFrame[3], &visualShape.m_rgbaColor[0],
                scaling);
  ...
  if (start >= 0) {
    for (int i = start; i < m_data->m_visualShapes.size(); i++) {
      if (m_data->m_visualShapes[i].m_objectUniqueId == bodyUniqueId) {
        ...
        // storage of each visual shape for a given body unique id assumed to be
        // contiguous
        break;
      }
    }
...
int EGLRendererVisualShapeConverter::getVisualShapesData(
    int bodyUniqueId, int shapeIndex, struct b3VisualShapeData* shapeData) {
  int start = -1;
  // find first one, then count how many
  for (int i = 0; i < m_data->m_visualShapes.size(); i++) {
    if (m_data->m_visualShapes[i].m_objectUniqueId == bodyUniqueId) {
...
void EGLRendererVisualShapeConverter::changeRGBAColor(
    int bodyUniqueId, int linkIndex, int shapeIndex,
    const double rgbaColor[4]) {
  // int start = -1;
  for (int i = 0; i < m_data->m_visualShapes.size(); i++) {
    if (m_data->m_visualShapes[i].m_objectUniqueId == bodyUniqueId &&
        m_data->m_visualShapes[i].m_linkIndex == linkIndex) {
  ...
  for (int i = 0; i < m_data->m_swRenderInstances.size(); i++) {
    ...
    if (ptrptr && *ptrptr) {
      float rgba[4] = {(float)rgbaColor[0], (float)rgbaColor[1],
                       (float)rgbaColor[2], (float)rgbaColor[3]};
      EGLRendererObjectArray* visuals = *ptrptr;
      if ((bodyUniqueId == visuals->m_objectUniqueId) &&
          (linkIndex == visuals->m_linkIndex)) {
      }
    }
  }
}
...
void EGLRendererVisualShapeConverter::resetCamera(float camDist, float yaw,
                                                  float pitch, float camPosX,
                                                  float camPosY,
                                                  float camPosZ) {
  ...
  m_data->m_camera.setAspectRatio((float)m_data->m_swWidth /
                                  (float)m_data->m_swHeight);
  m_data->m_camera.update();
}
...
void EGLRendererVisualShapeConverter::render() {
  // mode the the actual render code inside 'copyImageData' since we need to
  // know the width/height
}

void EGLRendererVisualShapeConverter::render(const float viewMat[16],
  ...
  // don't disableVRCamera, see issue
  // https://github.com/bulletphysics/bullet3/issues/2390
  // todo: check out why
  //
  //	m_data->m_camera.disableVRCamera();

  // cout<<viewMat[4*0 + 0]<<" "<<viewMat[4*0+1]<<" "<<viewMat[4*0+2]<<"
  // "<<viewMat[4*0+3] << endl;
  // cout<<viewMat[4*1 + 0]<<" "<<viewMat[4*1+1]<<" "<<viewMat[4*1+2]<<"
  // "<<viewMat[4*1+3] << endl;
  // cout<<viewMat[4*2 + 0]<<" "<<viewMat[4*2+1]<<" "<<viewMat[4*2+2]<<"
  // "<<viewMat[4*2+3] << endl;
  // cout<<viewMat[4*3 + 0]<<" "<<viewMat[4*3+1]<<" "<<viewMat[4*3+2]<<"
  // "<<viewMat[4*3+3] << endl;
}

void EGLRendererVisualShapeConverter::getWidthAndHeight(int& width,
                                                        int& height) {
...
void EGLRendererVisualShapeConverter::setWidthAndHeight(int width, int height) {
  ...
  m_data->m_segmentationMaskBuffer.resize(m_data->m_swWidth *
                                          m_data->m_swHeight);
  m_data->m_rgbColorBuffer = TGAImage(width, height, TGAImage::RGB);
}

void EGLRendererVisualShapeConverter::setProjectiveTextureMatrices(
    const float viewMatrix[16], const float projectionMatrix[16]) {
  m_data->m_instancingRenderer->setProjectiveTextureMatrices(viewMatrix,
                                                             projectionMatrix);
}

void EGLRendererVisualShapeConverter::setProjectiveTexture(
    bool useProjectiveTexture) {
...
void EGLRendererVisualShapeConverter::copyCameraImageDataGL(
    ...
    int segmentationMaskSizeInPixels, int startPixelIndex, int* widthPtr,
    int* heightPtr, int* numPixelsCopied) {
  if (numPixelsCopied) *numPixelsCopied = 0;

  int destinationWidth = *widthPtr;
  int destinationHeight = *heightPtr;
  int sourceWidth = btMin(
      destinationWidth,
      (int)(m_data->m_window->getWidth() * m_data->m_window->getRetinaScale()));
  int sourceHeight =
      btMin(destinationHeight, (int)(m_data->m_window->getHeight() *
                                     m_data->m_window->getRetinaScale()));
  ...
  if (1) {
    if (startPixelIndex == 0) {
      ...
      glViewport(0, 0, sourceWidth * m_data->m_window->getRetinaScale(),
                 sourceHeight * m_data->m_window->getRetinaScale());
      B3_PROFILE("m_instancingRenderer render");
      m_data->m_instancingRenderer->writeTransforms();
      if (m_data->m_hasLightDirection) {
        m_data->m_instancingRenderer->setLightPosition(
            m_data->m_lightDirection);
      }
      ...
      m_data->m_instancingRenderer->drawLine(b3MakeVector3(0, 0, 0),
                                             b3MakeVector3(1, 0, 0),
                                             b3MakeVector3(1, 0, 0), 3);
      m_data->m_instancingRenderer->drawLine(b3MakeVector3(0, 0, 0),
                                             b3MakeVector3(0, 1, 0),
                                             b3MakeVector3(0, 1, 0), 3);
      m_data->m_instancingRenderer->drawLine(b3MakeVector3(0, 0, 0),
                                             b3MakeVector3(0, 0, 1),
                                             b3MakeVector3(0, 0, 1), 3);
      ...
      if (pixelsRGBA || depthBuffer) {
        {
          ...
          m_data->m_sourceRgbaPixelBuffer.resize(sourceWidth * sourceHeight *
                                                 numBytesPerPixel);
          m_data->m_sourceDepthBuffer.resize(sourceWidth * sourceHeight);
          {
            ...
            b3Assert((sourceWidth * sourceHeight * 4) ==
                     rgbaBufferSizeInPixels);
            // glClear(GL_COLOR_BUFFER_BIT);
            // b3Warning("EGL\n");
            if ((sourceWidth * sourceHeight * 4) ==
                rgbaBufferSizeInPixels)  // remove this if
            {
              glReadPixels(0, 0, sourceWidth, sourceHeight, GL_RGBA,
                           GL_UNSIGNED_BYTE,
                           &(m_data->m_sourceRgbaPixelBuffer[0]));
              int glstat;
              glstat = glGetError();
              b3Assert(glstat == GL_NO_ERROR);
            }
            if ((sourceWidth * sourceHeight) == depthBufferSizeInPixels) {
              glReadPixels(0, 0, sourceWidth, sourceHeight, GL_DEPTH_COMPONENT,
                           GL_FLOAT, &(m_data->m_sourceDepthBuffer[0]));
              int glstat;
              glstat = glGetError();
              b3Assert(glstat == GL_NO_ERROR);
            }
        ...
        m_data->m_rgbaPixelBuffer1.resize((*widthPtr) * (*heightPtr) *
                                          numBytesPerPixel);
        ...
        // rescale and flip
        {
          ...
          for (int j = 0; j < *heightPtr; j++) {
            for (int i = 0; i < *widthPtr; i++) {
              int xIndex =
                  int(float(i) * (float(sourceWidth) / float(*widthPtr)));
              int yIndex = int(float(*heightPtr - 1 - j) *
                               (float(sourceHeight) / float(*heightPtr)));
              btClamp(xIndex, 0, sourceWidth);
              btClamp(yIndex, 0, sourceHeight);
              int bytesPerPixel = 4;  // RGBA

              int sourcePixelIndex =
                  (xIndex + yIndex * sourceWidth) * bytesPerPixel;
              int sourceDepthIndex = xIndex + yIndex * sourceWidth;
#define COPY4PIXELS 1
#ifdef COPY4PIXELS
              int* dst =
                  (int*)&m_data
                      ->m_rgbaPixelBuffer1[(i + j * (*widthPtr)) * 4 + 0];
              int* src =
                  (int*)&m_data->m_sourceRgbaPixelBuffer[sourcePixelIndex + 0];
              *dst = *src;

#else
              m_data->m_rgbaPixelBuffer1[(i + j * widthPtr) * 4 + 0] =
                  sourceRgbaPixelBuffer[sourcePixelIndex + 0];
              m_data->m_rgbaPixelBuffer1[(i + j * widthPtr) * 4 + 1] =
                  sourceRgbaPixelBuffer[sourcePixelIndex + 1];
              m_data->m_rgbaPixelBuffer1[(i + j * widthPtr) * 4 + 2] =
                  sourceRgbaPixelBuffer[sourcePixelIndex + 2];
              m_data->m_rgbaPixelBuffer1[(i + j * widthPtr) * 4 + 3] = 255;
#endif
              if (depthBuffer) {
                m_data->m_depthBuffer1[i + j * (*widthPtr)] =
                    m_data->m_sourceDepthBuffer[sourceDepthIndex];
              }
            }
          }
        }
      ...
      if (segmentationMaskBuffer) {
        {
          ...
          m_data->m_instancingRenderer->renderSceneInternal(
...

        {
          ...
          // copy the image into our local cache
          m_data->m_segmentationMaskSourceRgbaPixelBuffer.resize(
              sourceWidth * sourceHeight * numBytesPerPixel);
          m_data->m_segmentationMaskSourceDepthBuffer.resize(sourceWidth *
                                                             sourceHeight);
          {
            BT_PROFILE("getScreenPixels");
            {
              glReadPixels(0, 0, sourceWidth, sourceHeight, GL_DEPTH_COMPONENT,
                           GL_FLOAT,
                           &(m_data->m_segmentationMaskSourceDepthBuffer[0]));
              int glstat;
              glstat = glGetError();
              b3Assert(glstat == GL_NO_ERROR);
            }
            {
              glReadPixels(
                  0, 0, sourceWidth, sourceHeight, GL_RGBA, GL_UNSIGNED_BYTE,
                  &(m_data->m_segmentationMaskSourceRgbaPixelBuffer[0]));
              int glstat;
              glstat = glGetError();
              b3Assert(glstat == GL_NO_ERROR);
            }
        ...
        m_data->m_segmentationMaskBuffer.resize(
          ...
          for (int j = 0; j < destinationHeight; j++) {
            for (int i = 0; i < destinationWidth; i++) {
              int xIndex = int(float(i) *
                               (float(sourceWidth) / float(destinationWidth)));
              int yIndex =
                  int(float(destinationHeight - 1 - j) *
              ...
              if (segmentationMaskBuffer) {
                float depth =
                    m_data
                        ->m_segmentationMaskSourceDepthBuffer[sourceDepthIndex];
                if (depth < 1) {
                  int segMask =
                      m_data->m_segmentationMaskSourceRgbaPixelBuffer
                          [sourcePixelIndex + 0] +
                      256 * (m_data->m_segmentationMaskSourceRgbaPixelBuffer
                                 [sourcePixelIndex + 1]) +
                      256 * 256 *
                          (m_data->m_segmentationMaskSourceRgbaPixelBuffer
                               [sourcePixelIndex + 2]);
                  m_data->m_segmentationMaskBuffer[i + j * destinationWidth] =
                      segMask;
                } else {
                  m_data->m_segmentationMaskBuffer[i + j * destinationWidth] =
                      -1;
      ...
      glViewport(
          0, 0,
          m_data->m_window->getWidth() * m_data->m_window->getRetinaScale(),
          m_data->m_window->getHeight() * m_data->m_window->getRetinaScale());
    }
    if (pixelsRGBA) {
      BT_PROFILE("copy rgba pixels");
      for (int i = 0; i < numRequestedPixels * numBytesPerPixel; i++) {
        pixelsRGBA[i] =
            m_data->m_rgbaPixelBuffer1[i + startPixelIndex * numBytesPerPixel];
      }
    }
    if (depthBuffer) {
    ...
    if (segmentationMaskBuffer) {
      ...
      for (int i = 0; i < numRequestedPixels; i++) {
        int graphicsIndexSegMask =
            m_data->m_segmentationMaskBuffer[i + startPixelIndex];
        int segMask = -1;
        if (graphicsIndexSegMask >= 0 &&
            graphicsIndexSegMask <
                m_data->m_graphicsIndexToSegmentationMask.size()) {
          segMask =
              m_data->m_graphicsIndexToSegmentationMask[graphicsIndexSegMask];
        }
        if ((m_data->m_flags & ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX) ==
            0) {
          if (segMask >= 0) {
            segMask &= ((1 << 24) - 1);
          }
...
void EGLRendererVisualShapeConverter::copyCameraImageData(
    unsigned char* pixelsRGBA, int rgbaBufferSizeInPixels, float* depthBuffer,
    int depthBufferSizeInPixels, int* segmentationMaskBuffer,
    int segmentationMaskSizeInPixels, int startPixelIndex, int* widthPtr,
    int* heightPtr, int* numPixelsCopied) {
  B3_PROFILE("copyCameraImageDataGL");
  copyCameraImageDataGL(pixelsRGBA, rgbaBufferSizeInPixels, depthBuffer,
                        depthBufferSizeInPixels, segmentationMaskBuffer,
                        segmentationMaskSizeInPixels, startPixelIndex, widthPtr,
                        heightPtr, numPixelsCopied);
}

void EGLRendererVisualShapeConverter::removeVisualShape(
    int collisionObjectUniqueId) {
  EGLRendererObjectArray** ptrptr =
      m_data->m_swRenderInstances[collisionObjectUniqueId];
  if (ptrptr && *ptrptr) {
    EGLRendererObjectArray* ptr = *ptrptr;
    if (ptr) {
      for (int i = 0; i < ptr->m_graphicsInstanceIds.size(); i++) {
        m_data->m_instancingRenderer->removeGraphicsInstance(
            ptr->m_graphicsInstanceIds[i]);
      }
    }
    delete ptr;
...
void EGLRendererVisualShapeConverter::changeShapeTexture(int objectUniqueId,
                                                         int jointIndex,
                                                         int shapeIndex,
                                                         int textureUniqueId) {
...
int EGLRendererVisualShapeConverter::registerTexture(unsigned char* texels,
...
int EGLRendererVisualShapeConverter::loadTextureFile(
    const char* filename, struct CommonFileIOInterface* fileIO) {
  ...
  if (fileIO) {
    ...
    if (buffer.size()) {
      image = stbi_load_from_memory((const unsigned char*)&buffer[0],
...
void EGLRendererVisualShapeConverter::syncTransform(
    int collisionObjectUniqueId, const btTransform& worldTransform,
    const btVector3& localScaling) {
  EGLRendererObjectArray** renderObjPtr =
      m_data->m_swRenderInstances[collisionObjectUniqueId];
  if (renderObjPtr) {
    ...
    for (int i = 0; i < renderObj->m_graphicsInstanceIds.size(); i++) {
      ...
      if (graphicsInstanceId >= 0) {
        ...
        m_data->m_instancingRenderer->writeSingleInstanceTransformToCPU(
            pos, orn, graphicsInstanceId);
      }
    }
  }
}

bool EGLRendererVisualShapeConverter::getCameraInfo(
    int* width, int* height, float viewMatrix[16], float projectionMatrix[16],
    float camUp[3], float camForward[3], float hor[3], float vert[3],
    float* yaw, float* pitch, float* camDist, float cameraTarget[3]) const {
  if (m_data->m_instancingRenderer &&
      m_data->m_instancingRenderer->getActiveCamera()) {
    *width = m_data->m_window->getWidth() * m_data->m_window->getRetinaScale();
    *height =
        m_data->m_window->getHeight() * m_data->m_window->getRetinaScale();
    m_data->m_instancingRenderer->getActiveCamera()->getCameraViewMatrix(
        viewMatrix);
    m_data->m_instancingRenderer->getActiveCamera()->getCameraProjectionMatrix(
        projectionMatrix);
    m_data->m_instancingRenderer->getActiveCamera()->getCameraUpVector(camUp);
    m_data->m_instancingRenderer->getActiveCamera()->getCameraForwardVector(
        camForward);
    ...
    m_data->m_instancingRenderer->getActiveCamera()->getCameraTargetPosition(
        camTarget);
    ...
    *camDist =
        m_data->m_instancingRenderer->getActiveCamera()->getCameraDistance();
