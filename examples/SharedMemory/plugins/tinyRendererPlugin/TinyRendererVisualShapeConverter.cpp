...
void TinyRendererVisualShapeConverter::setLightDirection(float x, float y,
                                                         float z) {
  m_data->m_lightDirection.setValue(x, y, z);
  m_data->m_hasLightDirection = true;
}

void TinyRendererVisualShapeConverter::setLightColor(float x, float y,
                                                     float z) {
...
static void convertURDFToVisualShape(
    ...
    btAlignedObjectArray<MyTexture2>& texturesOut,
  ...
  switch (visual->m_geometry.m_type) {
    ...
    case URDF_GEOM_MESH: {
        ...
        case UrdfGeometry::MEMORY_VERTICES: {
          ...
          for (int i = 0; i < visual->m_geometry.m_vertices.size(); i++) {
            glmesh->m_vertices->at(i).xyzw[0] =
                visual->m_geometry.m_vertices[i].x();
            glmesh->m_vertices->at(i).xyzw[1] =
                visual->m_geometry.m_vertices[i].y();
            glmesh->m_vertices->at(i).xyzw[2] =
                visual->m_geometry.m_vertices[i].z();
            glmesh->m_vertices->at(i).xyzw[3] = 1;
            btVector3 normal(visual->m_geometry.m_vertices[i]);
            if (visual->m_geometry.m_normals.size() ==
                visual->m_geometry.m_vertices.size()) {
            ...
            if (visual->m_geometry.m_uvs.size() ==
                visual->m_geometry.m_vertices.size()) {
      ...
      if (glmesh && glmesh->m_vertices && (glmesh->m_numvertices > 0)) {
        ...
        if (visual->m_geometry.m_meshFileType !=
            UrdfGeometry::MEMORY_VERTICES) {
...

{
  ...
  if (linkPtr) {
    ...
    for (int v1 = 0; v1 < cnt; v1++) {
      ...
      TinyRendererObjectArray** visualsPtr =
          m_data->m_swRenderInstances[uniqueId];
      if (visualsPtr == 0) {
        m_data->m_swRenderInstances.insert(uniqueId,
                                           new TinyRendererObjectArray);
      }
...

      {
        ...
        convertURDFToVisualShape(
            vis, pathPrefix, localInertiaFrame.inverse() * childTrans, vertices,
            indices, textures, visualShape, fileIO, m_data->m_flags);
      }
      ...
      if (vertices.size() && indices.size()) {
        TinyRenderObjectData* tinyObj = new TinyRenderObjectData(
            m_data->m_rgbColorBuffer, m_data->m_depthBuffer,
            &m_data->m_shadowBuffer, &m_data->m_segmentationMaskBuffer,
...

        {
          ...
          tinyObj->registerMeshShape(&vertices[0].xyzw[0], vertices.size(),
                                     &indices[0], indices.size(), rgbaColor,
                                     textureImage1, textureWidth,
                                     textureHeight);
        }
      ...
      if (!shapes) {
        m_data->m_visualShapesMap.insert(
            visualShape.m_objectUniqueId,
...
int TinyRendererVisualShapeConverter::addVisualShape(
  ...
  if (b3ImportMeshUtility::loadAndRegisterMeshFromFileInternal(
    ...
    // meshData.m_gfxShape is allocated by a helper function used to create
    // visualShape,
...
int TinyRendererVisualShapeConverter::getVisualShapesData(
...
void TinyRendererVisualShapeConverter::changeRGBAColor(
  ...
  for (int i = 0; i < m_data->m_swRenderInstances.size(); i++) {
    TinyRendererObjectArray** ptrptr =
        m_data->m_swRenderInstances.getAtIndex(i);
    if (ptrptr && *ptrptr) {
...
void TinyRendererVisualShapeConverter::resetCamera(float camDist, float yaw,
...
void TinyRendererVisualShapeConverter::render(const float viewMat[16],
  ...
  if (m_data->m_hasShadow) {
    for (int n = 0; n < m_data->m_swRenderInstances.size(); n++) {
      TinyRendererObjectArray** visualArrayPtr =
          m_data->m_swRenderInstances.getAtIndex(n);
      if (0 == visualArrayPtr) continue;  // can this ever happen?
  ...
  // flip z-buffer and segmentation Buffer
  {
    ...
    for (int j = 0; j < half; j++) {
      ...
      for (int i = 0; i < m_data->m_swWidth; i++) {
        ...
        btSwap(m_data->m_segmentationMaskBuffer[l1 + i],
               m_data->m_segmentationMaskBuffer[l2 + i]);
      }
    }
  }
}

void TinyRendererVisualShapeConverter::getWidthAndHeight(int& width,
                                                         int& height) {
  width = m_data->m_swWidth;
  height = m_data->m_swHeight;
}

void TinyRendererVisualShapeConverter::setWidthAndHeight(int width,
                                                         int height) {
...
void TinyRendererVisualShapeConverter::copyCameraImageData(
  ...
  if (numRequestedPixels) {
    for (int i = 0; i < numRequestedPixels; i++) {
      if (depthBuffer) {
        ...
        // TinyRenderer returns clip coordinates, transform to eye coordinates
        // first
        float z_c = -m_data->m_depthBuffer[i + startPixelIndex];
        // float distance = (farPlane - nearPlane) / (farPlane + nearPlane) *
        // (z_c + 2. * farPlane * nearPlane / (farPlane - nearPlane));
        ...
        depthBuffer[i] =
            farPlane * (nearPlane + z_c) /
            (2. * farPlane * nearPlane + farPlane * z_c - nearPlane * z_c);
      }
      ...
      if (pixelsRGBA) {
        pixelsRGBA[i * numBytesPerPixel] =
            m_data->m_rgbColorBuffer.buffer()[(i + startPixelIndex) * 3 + 0];
        pixelsRGBA[i * numBytesPerPixel + 1] =
            m_data->m_rgbColorBuffer.buffer()[(i + startPixelIndex) * 3 + 1];
        pixelsRGBA[i * numBytesPerPixel + 2] =
            m_data->m_rgbColorBuffer.buffer()[(i + startPixelIndex) * 3 + 2];
...
void TinyRendererVisualShapeConverter::removeVisualShape(
...
void TinyRendererVisualShapeConverter::changeShapeTexture(int objectUniqueId,
                                                          int jointIndex,
                                                          int shapeIndex,
                                                          int textureUniqueId) {
  ...
  if (textureUniqueId >= -1 && textureUniqueId < m_data->m_textures.size()) {
    for (int n = 0; n < m_data->m_swRenderInstances.size(); n++) {
      ...
      if (visualArray->m_objectUniqueId == objectUniqueId &&
          visualArray->m_linkIndex == jointIndex) {
        for (int v = 0; v < visualArray->m_renderObjects.size(); v++) {
          TinyRenderObjectData* renderObj = visualArray->m_renderObjects[v];

          if ((shapeIndex < 0) || (shapeIndex == v)) {
            if (textureUniqueId >= 0) {
              renderObj->m_model->setDiffuseTextureFromData(
                  m_data->m_textures[textureUniqueId].textureData1,
                  m_data->m_textures[textureUniqueId].m_width,
                  m_data->m_textures[textureUniqueId].m_height);
            } else {
...
int TinyRendererVisualShapeConverter::loadTextureFile(
