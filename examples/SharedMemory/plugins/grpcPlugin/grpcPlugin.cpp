...
class ServerImpl final {
 public:
  ...
  void Init(PhysicsCommandProcessorInterface* comProc,
    ...
    std::cout << "grpcPlugin Bullet Physics GRPC server listening on "
              << hostNamePort << std::endl;
 ...
 private:
  ...
  class CallData {
   public:
    ...
    // with the gRPC runtime.
    CallData(PyBulletAPI::AsyncService* service, ServerCompletionQueue* cq,
             PhysicsCommandProcessorInterface* comProc)
        : service_(service),
          cq_(cq),
          responder_(&ctx_),
          status_(CREATE),
    ...
    CallStatus Proceed() {
      if (status_ == CREATE) {
        ...
        if (m_command.has_checkversioncommand()) {
          ...
          m_status.mutable_checkversionstatus()->set_serverversion(
              SHARED_MEMORY_MAGIC_NUMBER);
        } else {
          cmdPtr = convertGRPCToBulletCommand(m_command, cmd);

          if (cmdPtr) {
            bool hasStatus = m_comProc->processCommand(
                *cmdPtr, serverStatus, &buffer[0], buffer.size());
            ...
            while ((!hasStatus) &&
                   ((curTimeSeconds - startTimeSeconds) < timeOutInSeconds)) {
              hasStatus = m_comProc->receiveStatus(serverStatus, &buffer[0],
                                                   buffer.size());
            ...
            if (hasStatus) {
              ...
              convertStatusToGRPC(serverStatus, &buffer[0], buffer.size(),
                                  m_status);
            }
          }
   ...
   private:
    ...
    PhysicsCommandProcessorInterface*
        m_comProc;  // physics server command processor
  };
  ...
  void GatherRequests() {
    ...
    while (!m_requestThreadCancelled) {
      ...
      grpc::CompletionQueue::NextStatus nextStatus =
          cq_->AsyncNext(&tag, &ok, gpr_now(GPR_CLOCK_MONOTONIC));
...
struct grpcMyClass {
  ...
  grpcMyClass()
      : m_testData(42), m_grpcInitialized(false), m_grpcTerminated(false) {}
  virtual ~grpcMyClass() {}
};
...
B3_SHARED_API int preTickPluginCallback_grpcPlugin(
    struct b3PluginContext* context) {
  // process grpc server messages
  return 0;
}

B3_SHARED_API int processClientCommands_grpcPlugin(
    struct b3PluginContext* context) {
...
B3_SHARED_API int postTickPluginCallback_grpcPlugin(
    struct b3PluginContext* context) {
  grpcMyClass* obj = (grpcMyClass*)context->m_userPointer;
  obj->m_testData++;
  return 0;
...
B3_SHARED_API int executePluginCommand_grpcPlugin(
    struct b3PluginContext* context,
  ...
  if (strlen(arguments->m_text)) {
    if (!obj->m_grpcInitialized && context->m_rpcCommandProcessorInterface) {
      obj->m_grpcServer.Init(context->m_rpcCommandProcessorInterface,
                             arguments->m_text);
    }
