...
struct MyPDControlContainer {
  ...
  MyPDControlContainer() : m_testData(42) {}
...
B3_SHARED_API int preTickPluginCallback_pdControlPlugin(
  ...
  for (int i = 0; i < obj->m_controllers.size(); i++) {
    ...
    if (obj->m_api.getJointState(pdControl.m_objectUniqueId,
                                 pdControl.m_linkIndex, &actualState)) {
      if (pdControl.m_maxForce > 0) {
        ...
        btScalar force =
            pdControl.m_kp * positionError + pdControl.m_kd * velocityError;
        ...
        obj->m_api.setJointMotorControl(pdControl.m_objectUniqueId,
                                        pdControl.m_linkIndex, args);
      }
    }
  }
...
B3_SHARED_API int executePluginCommand_pdControlPlugin(
  ...
  if (arguments->m_numInts != 3) return -1;

  switch (arguments->m_ints[0]) {
    case eSetPDControl: {
      if (arguments->m_numFloats < 5) return -1;
      ...
      int foundIndex = -1;
      for (int i = 0; i < obj->m_controllers.size(); i++) {
        if (obj->m_controllers[i].m_objectUniqueId ==
                controller.m_objectUniqueId &&
            obj->m_controllers[i].m_linkIndex == controller.m_linkIndex) {
