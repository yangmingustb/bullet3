...
struct PhysicsLoopBackInternalData {
  ...
  PhysicsLoopBackInternalData()
      : m_commandProcessor(0), m_physicsClient(0), m_physicsServer(0) {}
};

struct Bullet2CommandProcessorCreation2
...
bool PhysicsLoopBack::submitClientCommand(
    const struct SharedMemoryCommand& command) {
...
bool PhysicsLoopBack::getBodyInfo(int bodyUniqueId,
                                  struct b3BodyInfo& info) const {
...
bool PhysicsLoopBack::getJointInfo(int bodyIndex, int jointIndex,
...
int PhysicsLoopBack::getUserConstraintInfo(
    int constraintUniqueId, struct b3UserConstraint& info) const {
  return m_data->m_physicsClient->getUserConstraintInfo(constraintUniqueId,
                                                        info);
}
...
void PhysicsLoopBack::uploadBulletFileToSharedMemory(const char* data,
                                                     int len) {
  m_data->m_physicsClient->uploadBulletFileToSharedMemory(data, len);
}

void PhysicsLoopBack::uploadRaysToSharedMemory(
    struct SharedMemoryCommand& command, const double* rayFromWorldArray,
    const double* rayToWorldArray, int numRays) {
  m_data->m_physicsClient->uploadRaysToSharedMemory(command, rayFromWorldArray,
                                                    rayToWorldArray, numRays);
}
...
void PhysicsLoopBack::getCachedCameraImage(
    struct b3CameraImageData* cameraData) {
...
void PhysicsLoopBack::getCachedContactPointInformation(
    struct b3ContactInformation* contactPointData) {
  return m_data->m_physicsClient->getCachedContactPointInformation(
      contactPointData);
}

void PhysicsLoopBack::getCachedVisualShapeInformation(
    struct b3VisualShapeInformation* visualShapesInfo) {
  return m_data->m_physicsClient->getCachedVisualShapeInformation(
      visualShapesInfo);
}

void PhysicsLoopBack::getCachedCollisionShapeInformation(
    struct b3CollisionShapeInformation* collisionShapesInfo) {
  return m_data->m_physicsClient->getCachedCollisionShapeInformation(
      collisionShapesInfo);
}
...
bool PhysicsLoopBack::getCachedUserData(
    int userDataId, struct b3UserDataValue& valueOut) const {
  return m_data->m_physicsClient->getCachedUserData(userDataId, valueOut);
}

int PhysicsLoopBack::getCachedUserDataId(int bodyUniqueId, int linkIndex,
                                         int visualShapeIndex,
                                         const char* key) const {
  return m_data->m_physicsClient->getCachedUserDataId(bodyUniqueId, linkIndex,
                                                      visualShapeIndex, key);
}
...
void PhysicsLoopBack::getUserDataInfo(int bodyUniqueId, int userDataIndex,
                                      const char** keyOut, int* userDataIdOut,
                                      int* linkIndexOut,
                                      int* visualShapeIndexOut) const {
  m_data->m_physicsClient->getUserDataInfo(bodyUniqueId, userDataIndex, keyOut,
                                           userDataIdOut, linkIndexOut,
                                           visualShapeIndexOut);
}
