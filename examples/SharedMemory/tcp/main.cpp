    ...
    while (!exitRequested) {
      ...
      if ((pClient = socket.Accept()) != NULL) {
        ...
        while (1) {
          ...
          int maxLen = 4 + sizeof(SharedMemoryStatus) +
          ...
          if (pClient->Receive(maxLen)) {
            ...
            if (bytesReceived.size() >= 4) {
              ...
              if (cmdPtr) {
                ...
                bool hasStatus = sm->processCommand(*cmdPtr, serverStatus,
                                                    &buffer[0], buffer.size());

                double startTimeSeconds = clock.getTimeInSeconds();
                double curTimeSeconds = clock.getTimeInSeconds();

                while ((!hasStatus) && ((curTimeSeconds - startTimeSeconds) <
                                        timeOutInSeconds)) {
                  hasStatus = sm->receiveStatus(serverStatus, &buffer[0],
                                                buffer.size());
                ...
                if (hasStatus) {
                  ...
                  if (cmdPtr->m_type == CMD_STEP_FORWARD_SIMULATION) {
                    ...
                    if (cmdPtr->m_type == CMD_REQUEST_VR_EVENTS_DATA) {
                      int headerSize = 16 + 5 * sizeof(int) +
                                       sizeof(smUint64_t) + sizeof(char*) +
                                       sizeof(b3VRControllerEvent) *
                                           serverStatus.m_sendVREvents
                                               .m_numVRControllerEvents;
                      ...
                      // create packetData with [int packetSizeInBytes, status,
                      // streamBytes)
                      packetData.resize(4 + sizeof(SharedMemoryStatus) +
                                        serverStatus.m_numDataStreamBytes);
                      int sz = packetData.size();
                      int curPos = 0;
                      ...
                      for (int i = 0; i < serverStatus.m_numDataStreamBytes;
                           i++) {
