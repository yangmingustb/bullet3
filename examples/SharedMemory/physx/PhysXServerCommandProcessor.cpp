...
class CustomProfilerCallback : public physx::PxProfilerCallback {
 public:
  ...
  virtual void* zoneStart(const char* eventName, bool detached,
                          uint64_t contextId) {
    b3EnterProfileZone(eventName);
    return 0;
  }

  virtual void zoneEnd(void* profilerData, const char* eventName, bool detached,
                       uint64_t contextId) {
    b3LeaveProfileZone();
  }
};
...
struct PhysXServerCommandProcessorInternalData
  ...
  void onContactModify(physx::PxContactModifyPair* const pairs,
                       physx::PxU32 count) {
    ...
    for (physx::PxU32 i = 0; i < nbPairs; i++) {
      ...
      if (contactCount) {
        ...
        for (physx::PxU32 j = 0; j < contactCount; j++) {
          ...
          MyPhysXUserData* udA =
              (MyPhysXUserData*)pairHeader.actors[0]->userData;
          MyPhysXUserData* udB =
              (MyPhysXUserData*)pairHeader.actors[1]->userData;
          ...
          srcPt.m_positionOnAInWS[0] =
              contact.position.x + contact.separation * contact.normal.x;
          srcPt.m_positionOnAInWS[1] =
              contact.position.y + contact.separation * contact.normal.y;
          srcPt.m_positionOnAInWS[2] =
              contact.position.z + contact.separation * contact.normal.z;
          srcPt.m_positionOnBInWS[0] =
              contact.position.x - contact.separation * contact.normal.x;
          srcPt.m_positionOnBInWS[1] =
              contact.position.y - contact.separation * contact.normal.y;
          srcPt.m_positionOnBInWS[2] =
              contact.position.z - contact.separation * contact.normal.z;
          ...
          // compute friction direction from impulse projected in contact plane
          // using contact normal.
          physx::PxVec3 fric =
              contact.impulse - contact.normal * srcPt.m_normalForce;
          ...
          // std::cout << "Contact: bw " << pairHeader.actors[0]->getName() << "
          // and " << pairHeader.actors[1]->getName() << " | " <<
          // contacts[j].position.x << "," << contacts[j].position.y << ","
...
PhysXServerCommandProcessor::PhysXServerCommandProcessor(int argc,
                                                         char* argv[]) {
  m_data = new PhysXServerCommandProcessorInternalData(this, argc, argv);
}

PhysXServerCommandProcessor::~PhysXServerCommandProcessor() { delete m_data; }

physx::PxFilterFlags MyPhysXFilter(physx::PxFilterObjectAttributes attributes0,
...
bool PhysXServerCommandProcessor::connect() {
  ...
  int result = 0;
  {
    m_data->m_foundation = PxCreateFoundation(
        PX_PHYSICS_VERSION, m_data->m_allocator, m_data->m_errorCallback);
    // This call should be performed after PVD is initialized, otherwise it will
    // have no effect.
    PxSetProfilerCallback(&gCustomProfilerCallback);

    m_data->m_physics =
        PxCreatePhysics(PX_PHYSICS_VERSION, *m_data->m_foundation,
                        physx::PxTolerancesScale(), true, 0);
    m_data->m_cooking =
        PxCreateCooking(PX_PHYSICS_VERSION, *m_data->m_foundation,
    ...
    m_data->m_dispatcher =
        physx::PxDefaultCpuDispatcherCreate(numCores == 0 ? 0 : numCores - 1);
    ...
    m_data->m_commandLineArgs.GetCmdLineArgument("enableContactCallback",
                                                 enableContactCallback);
    ...
    // PxRigidStatic* groundPlane = PxCreatePlane(*gPhysics, PxPlane(0, 1, 0,
    // 0), *gMaterial);
...
bool PhysXServerCommandProcessor::processCustomCommand(
...
bool PhysXServerCommandProcessor::processStateLoggingCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
...
bool PhysXServerCommandProcessor::processInitPoseCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  if (body && body->mArticulation) {
    ...
    body->mArticulation->copyInternalStateToCache(
        *c, physx::PxArticulationCache::ePOSITION |
                physx::PxArticulationCache::
                    eVELOCITY);  // physx::PxArticulationCache::eALL);
    physx::PxArticulationLink* physxLinks[64];
    physx::PxU32 bufferSize = 64;
    physx::PxU32 startIndex = 0;
    int numLinks2 =
        body->mArticulation->getLinks(physxLinks, bufferSize, startIndex);
    ...
    if (numLinks2 > 0) {
      ...
      if (clientCmd.m_updateFlags & INIT_POSE_HAS_BASE_LINEAR_VELOCITY) {
        physxLinks[0]->setLinearVelocity(
            physx::PxVec3(baseLinVel[0], baseLinVel[1], baseLinVel[2]));
      }

      if (clientCmd.m_updateFlags & INIT_POSE_HAS_BASE_ANGULAR_VELOCITY) {
        physxLinks[0]->setAngularVelocity(
            physx::PxVec3(baseAngVel[0], baseAngVel[1], baseAngVel[2]));
      }
    ...
    if (clientCmd.m_updateFlags & INIT_POSE_HAS_JOINT_STATE) {
      ...
      for (int i = 1; i < numLinks2; i++) {
        ...
        if (hasPosVar) {
          if (posVarCount == 1) {
            c->jointPosition[dofStarts[physxCacheLinkIndex]] =
    ...
    body->mArticulation->applyCache(*c,
                                    physx::PxArticulationCache::ePOSITION |
                                        physx::PxArticulationCache::eVELOCITY);
...
bool PhysXServerCommandProcessor::processSendDesiredStateCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
          ...
          for (int link = 1; link < numLinks2; link++) {
            ...
            if (dofs == 1) {
              physx::PxArticulationJointReducedCoordinate* joint =
                  static_cast<physx::PxArticulationJointReducedCoordinate*>(
                      physxLinks[link]->getInboundJoint());
              ...
              if ((clientCmd.m_sendDesiredStateCommandArgument
                ...
                joint->setDriveVelocity(physx::PxArticulationAxis::eTWIST,
                                        desiredVelocity);
                ...
                if ((clientCmd.m_sendDesiredStateCommandArgument
                  ...
                  forceLimit = clientCmd.m_sendDesiredStateCommandArgument
                                   .m_desiredStateForceTorque[dofIndex];
                }
                joint->setDrive(physx::PxArticulationAxis::eTWIST, stiffness,
                                damping, forceLimit);
              }
            }
            dofIndex += dofs;
          ...
          for (int link = 1; link < numLinks2; link++) {
            ...
            if (dofs == 1) {
              ...
              if ((clientCmd.m_sendDesiredStateCommandArgument
                ...
                joint->setDriveTarget(physx::PxArticulationAxis::eTWIST,
                                      desiredPosition);
                joint->setDrive(physx::PxArticulationAxis::eTWIST, stiffness,
                                damping, forceLimit, isAcceleration);
              }
            }
...
bool PhysXServerCommandProcessor::processChangeDynamicsInfoCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool PhysXServerCommandProcessor::
  ...
  serverCmd.m_simulationParameterResultArgs.m_allowedCcdPenetration =
      0;  // m_data->m_dynamicsWorld->getDispatchInfo().m_allowedCcdPenetration;
  serverCmd.m_simulationParameterResultArgs.m_collisionFilterMode =
      0;  // m_data->m_broadphaseCollisionFilterCallback->m_filterMode;
  ...
  serverCmd.m_simulationParameterResultArgs.m_numSimulationSubSteps =
      0;  // m_data->m_numSimulationSubSteps;
  serverCmd.m_simulationParameterResultArgs.m_numSolverIterations =
      0;  // m_data->m_dynamicsWorld->getSolverInfo().m_numIterations;
  serverCmd.m_simulationParameterResultArgs.m_restitutionVelocityThreshold =
      0;  // m_data->m_dynamicsWorld->getSolverInfo().m_restitutionVelocityThreshold;

  serverCmd.m_simulationParameterResultArgs.m_solverResidualThreshold =
      0;  // m_data->m_dynamicsWorld->getSolverInfo().m_leastSquaresResidualThreshold;
  serverCmd.m_simulationParameterResultArgs.m_splitImpulsePenetrationThreshold =
      0;  // m_data->m_dynamicsWorld->getSolverInfo().m_splitImpulsePenetrationThreshold;
  serverCmd.m_simulationParameterResultArgs.m_useRealTimeSimulation =
      0;  // m_data->m_useRealTimeSimulation;
  serverCmd.m_simulationParameterResultArgs.m_useSplitImpulse =
      0;  // m_data->m_dynamicsWorld->getSolverInfo().m_splitImpulse;

  return hasStatus;
}

bool PhysXServerCommandProcessor::processRequestContactpointInformationCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  SharedMemoryStatus& serverCmd = serverStatusOut;
  ...
  int numContactPointBatch =
      btMin(int(m_data->m_contactPoints.size()), contactPointStorage);
  ...
  serverCmd.m_sendContactPointArgs.m_startingContactPointIndex =
      startContactPointIndex;
  serverCmd.m_sendContactPointArgs.m_numRemainingContactPoints =
      m_data->m_contactPoints.size() - startContactPointIndex -
      serverCmd.m_sendContactPointArgs.m_numContactPointsCopied;
...
bool PhysXServerCommandProcessor::processCreateCollisionShapeCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  return false;
...
bool PhysXServerCommandProcessor::processSetAdditionalSearchPathCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool PhysXServerCommandProcessor::processUserDebugDrawCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  ...
  if (clientCmd.m_updateFlags & USER_DEBUG_READ_PARAMETER) {
    double* valPtr = m_data->m_userDebugParameters[clientCmd.m_userDebugDrawArgs
                                                       .m_itemUniqueId];
    if (valPtr) {
  ...
  if (clientCmd.m_updateFlags & USER_DEBUG_REMOVE_ONE_ITEM) {
    m_data->m_userDebugParameters.remove(
...
bool PhysXServerCommandProcessor::processCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
...
bool PhysXServerCommandProcessor::processRequestCameraImageCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  int oldWidth;
  int oldHeight;
  m_data->m_pluginManager.getRenderInterface()->getWidthAndHeight(oldWidth,
                                                                  oldHeight);
      ...
      if (m_data->m_pluginManager.getRenderInterface()) {
        if (clientCmd.m_requestPixelDataArguments.m_startPixelIndex == 0) {
          ...
          if ((clientCmd.m_updateFlags &
            ...
            b3OpenGLVisualizerCameraInfo tmpCamResult;
            bool result =
                m_data->m_pluginManager.getRenderInterface()->getCameraInfo(
                    &tmpCamResult.m_width, &tmpCamResult.m_height,
                    tmpCamResult.m_viewMatrix, tmpCamResult.m_projectionMatrix,
                    tmpCamResult.m_camUp, tmpCamResult.m_camForward,
                    tmpCamResult.m_horizontal, tmpCamResult.m_vertical,
  ...
  m_data->m_pluginManager.getRenderInterface()->setWidthAndHeight(oldWidth,
                                                                  oldHeight);
...
bool PhysXServerCommandProcessor::processRequestInternalDataCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool PhysXServerCommandProcessor::processSyncUserDataCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
struct MyPhysXURDFImporter : public PhysXURDFImporter {
  ...
  MyPhysXURDFImporter(struct CommonFileIOInterface* fileIO,
                      double globalScaling, int flags,
                      b3PluginManager& pluginManager)
      : PhysXURDFImporter(fileIO, globalScaling, flags),
        m_pluginManager(pluginManager) {}

  int convertLinkVisualShapes3(int linkIndex, const char* pathPrefix,
    ...
    if (m_pluginManager.getRenderInterface()) {
      int graphicsUniqueId =
          m_pluginManager.getRenderInterface()->convertVisualShapes(
              linkIndex, pathPrefix, localInertiaFrame, linkPtr, model,
              collisionObjectUniqueId, bodyUniqueId, fileIO);
...
bool PhysXServerCommandProcessor::processLoadURDFCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  ...
  MyPhysXURDFImporter u2p(&fileIO, globalScaling, urdfArgs.m_urdfFlags,
  ...
  if (loadOk) {
    for (int m = 0; m < u2p.getNumModels(); m++) {
      ...
      InternalPhysXBodyHandle* bodyHandle =
      ...
      u2p.setBodyUniqueId(bodyUniqueId);
      {
        ...
        // u2p.getMassAndInertia2((urdfLinkIndex, mass, localInertiaDiagonal,
        // bodyHandle->m_rootLocalInertialFrame, flags);
      }

      physx::PxBase* baseObj =
          URDF2PhysX(m_data->m_foundation, m_data->m_physics, m_data->m_cooking,
                     m_data->m_scene, u2p, urdfArgs.m_urdfFlags,
                     u2p.getPathPrefix(), rootTrans, &fileIO, useMultiBody);
      ...
      physx::PxRigidDynamic* rigidDynamic =
          baseObj->is<physx::PxRigidDynamic>();
      ...
      if (rigidStatic || rigidDynamic || articulation) {
        serverStatusOut.m_type = CMD_URDF_LOADING_COMPLETED;
        serverStatusOut.m_dataStreamArguments.m_bodyUniqueId = bodyUniqueId;
        sprintf(serverStatusOut.m_dataStreamArguments.m_bodyName, "%s",
      ...
      if (articulation) {
        ...
        ser.registerNameForPointer(bodyHandle->m_bodyName.c_str(),
                                   bodyHandle->m_bodyName.c_str());
        {
          char* name =
              (char*)ser.findNameForPointer(bodyHandle->m_bodyName.c_str());
        ...
        if (mbd->m_numLinks) {
          ...
          physx::PxU32 startIndex = 0;
          int numLinks2 =
              articulation->getLinks(physxLinks, bufferSize, startIndex);
          ...
          for (int j = 0; j < numElem; j++, memPtr++) {
            ...
            memPtr->m_parentIndex = i > 0
                                        ? physxLinks[i]
                                              ->getInboundJoint()
                                              ->getParentArticulationLink()
                                              .getLinkIndex()
                                        : -1;
            memPtr->m_jointDamping =
                0;  // todophysxLinks[i]->getLinearDamping();//??
            ...
            memPtr->m_jointMaxVelocity =
                0;  // todogetLink(i).m_jointMaxVelocity;
            ...
            {
              char* name =
                  (char*)ser.findNameForPointer(physxLinks[i]->getName());
            ...
            memPtr->m_linkCollider =
                (btCollisionObjectData*)ser.getUniquePointer(0);
          }
          ser.finalizeChunk(chunk, btMultiBodyLinkDataName, BT_ARRAY_CODE,
                            (void*)articulation);
        }
        mbd->m_links = mbd->m_numLinks
                           ? (btMultiBodyLinkData*)ser.getUniquePointer(
                                 (void*)articulation)
                           : 0;
...
bool PhysXServerCommandProcessor::processRequestBodyInfoCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  int streamSizeInBytes =
      0;  // createBodyInfoStream(sdfInfoArgs.m_bodyUniqueId,
          // bufferServerToClient, bufferSizeInBytes);
...
bool PhysXServerCommandProcessor::processForwardDynamicsCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  }
  {
    ...
    if (m_data->m_pluginManager.getRenderInterface()) {
      ...
      for (int i = 0; i < usedHandles.size(); i++) {
        InternalPhysXBodyHandle* bodyHandle =
            m_data->m_bodyHandles.getHandle(usedHandles[i]);

        physx::PxRigidActor* actor = 0;
        if (bodyHandle->m_rigidDynamic) actor = bodyHandle->m_rigidDynamic;
        if (bodyHandle->m_rigidStatic) actor = bodyHandle->m_rigidStatic;
        ...
        if (actor) {
          ...
          m_data->m_pluginManager.getRenderInterface()->syncTransform(
              ud->m_graphicsUniqueId, tr, localScaling);
        }
        ...
        if (bodyHandle->mArticulation) {
          ...
          physx::PxU32 bufferSize = 64;
          physx::PxU32 startIndex = 0;
          int numLinks2 = bodyHandle->mArticulation->getLinks(
              physxLinks, bufferSize, startIndex);
...

    {
      ...
      int width = 1024;
      int height = 768;
      m_data->m_pluginManager.getRenderInterface()->getWidthAndHeight(width,
                                                                      height);
      int numPixelsCopied = 0;
...
bool PhysXServerCommandProcessor::processSendPhysicsParametersCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool PhysXServerCommandProcessor::processRequestActualStateCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
  ...
  if (bodyHandle->mArticulation) {
...

    {
      ...
      serverCmd.m_sendActualStateArgs.m_numLinks =
          bodyHandle->mArticulation->getNbLinks() - 1;  // skip base!
      ...
      bool computeForwardKinematics =
          ((clientCmd.m_updateFlags &
            ACTUAL_STATE_COMPUTE_FORWARD_KINEMATICS) != 0);
      ...
      bodyHandle->mArticulation->copyInternalStateToCache(
          *c, physx::PxArticulationCache::ePOSITION |
                  physx::PxArticulationCache::
                      eVELOCITY);  // physx::PxArticulationCache::eALL);
      ...
      for (int l = 1; l < numLinks2; l++) {
        ...
        for (int d = 0; d < dofs; d++) {
          serverCmd.m_sendActualStateArgs
              .m_actualStateQ[totalDegreeOfFreedomQ++] =
              c->jointPosition[dofStarts[llIndex + d]];
          serverCmd.m_sendActualStateArgs
              .m_actualStateQ[totalDegreeOfFreedomU++] =
              c->jointVelocity[dofStarts[llIndex + d]];
        }
      }
...
bool PhysXServerCommandProcessor::processResetSimulationCommand(
    const struct SharedMemoryCommand& clientCmd,
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  bool hasStatus = true;
...
bool PhysXServerCommandProcessor::receiveStatus(
    struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient,
    int bufferSizeInBytes) {
  return false;
}
