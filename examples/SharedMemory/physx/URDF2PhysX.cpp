...
struct URDF2PhysXCachedData {
  ...
  int getMbIndexFromUrdfIndex(int urdfIndex) const {
    if (urdfIndex == -2) return -2;
    return m_urdfLinkIndices2BulletLinkIndices[urdfIndex];
  }

  void registerMultiBody(int urdfLinkIndex,
                         class physx::PxArticulationLink* body,
                         const btTransform& worldTransform, btScalar mass,
  ...
  void registerRigidBody(int urdfLinkIndex,
                         class physx::PxArticulationLink* body,
                         const btTransform& worldTransform, btScalar mass,
                         const btVector3& localInertiaDiagonal,
...
void ComputeParentIndices(const URDFImporterInterface& u2b,
...
void ComputeTotalNumberOfJoints(const URDFImporterInterface& u2b,
                                URDF2PhysXCachedData& cache, int linkIndex) {
  btAlignedObjectArray<int> childIndices;
...
void InitURDF2BulletCache(const URDFImporterInterface& u2b,
                          URDF2PhysXCachedData& cache, int flags) {
...
static btVector4 colors4[4] = {
    btVector4(1, 0, 0, 1), btVector4(0, 1, 0, 1), btVector4(0, 1, 1, 1),
    btVector4(1, 1, 0, 1),
};
...
static physx::PxConvexMesh* createPhysXConvex(
    physx::PxU32 numVerts, const physx::PxVec3* verts,
  ...
  // If the gaussMapLimit is chosen higher than the number of output vertices,
  // no gauss map is added to the convex mesh data (here 256).
  // If the gaussMapLimit is chosen lower than the number of output vertices, a
  // gauss map is added to the convex mesh data (here 16).
  ...
  // We provide points only, therefore the PxConvexFlag::eCOMPUTE_CONVEX flag
  // must be specified
  ...
  physx::PxConvexMesh* convex = creation.m_cooking->createConvexMesh(
      desc, creation.m_physics->getPhysicsInsertionCallback());
  PX_ASSERT(convex);

  return convex;
}

int convertLinkPhysXShapes(const URDFImporterInterface& u2b,
                           URDF2PhysXCachedData& cache,
                           ArticulationCreationInterface& creation,
                           int urdfLinkIndex, const char* pathPrefix,
                           const btTransform& localInertiaFrame,
                           physx::PxArticulationReducedCoordinate* articulation,
                           int mbLinkIndex, physx::PxRigidActor* linkPtr) {
  int numShapes = 0;
  ...
  cache.m_linkMaterials.push_back(physx::PxVec3(contactInfo.m_lateralFriction,
                                                contactInfo.m_lateralFriction,
                                                contactInfo.m_restitution));
  physx::PxMaterial* material = creation.m_physics->createMaterial(
      contactInfo.m_lateralFriction, contactInfo.m_lateralFriction,
      contactInfo.m_restitution);

  const UrdfLink* link = u2b.getUrdfLink(
      urdfLinkIndex);  //.convertLinkCollisionShapes
  ...
  if (linkPtr) {
    for (int v = 0; v < link->m_collisionArray.size(); v++) {
      ...
      tr.p = physx::PxVec3(localTrans.getOrigin()[0], localTrans.getOrigin()[1],
                           localTrans.getOrigin()[2]);
      tr.q = physx::PxQuat(
          localTrans.getRotation()[0], localTrans.getRotation()[1],
          localTrans.getRotation()[2], localTrans.getRotation()[3]);
      ...
      switch (col.m_geometry.m_type) {
        case URDF_GEOM_PLANE: {
          ...
          shape = physx::PxRigidActorExt::createExclusiveShape(
              *linkPtr, physx::PxPlaneGeometry(), *material);

          btTransform diffTr;
          diffTr.setIdentity();
          diffTr.setRotation(diffQuat);
          btTransform localTrans =
              localInertiaFrame.inverse() * childTrans * diffTr;
          physx::PxTransform tr;
          tr.p = physx::PxVec3(localTrans.getOrigin()[0],
                               localTrans.getOrigin()[1],
                               localTrans.getOrigin()[2]);
          tr.q = physx::PxQuat(
              localTrans.getRotation()[0], localTrans.getRotation()[1],
              localTrans.getRotation()[2], localTrans.getRotation()[3]);
        ...
        case URDF_GEOM_CAPSULE: {
          ...
          // static PxShape* createExclusiveShape(PxRigidActor& actor, const
          // PxGeometry& geometry, PxMaterial*const* materials, PxU16
          // materialCount,
          ...
          // shape = PxGetPhysics().createShape(physx::PxCapsuleGeometry(radius,
          // 0.5*height), &material, 1, false, shapeFlags);

          shape = physx::PxRigidActorExt::createExclusiveShape(
              *linkPtr, physx::PxCapsuleGeometry(radius, 0.5 * height),
              *material);
          ...
          btTransform localTrans =
              localInertiaFrame.inverse() * childTrans * x2z;
          physx::PxTransform tr;
          tr.p = physx::PxVec3(localTrans.getOrigin()[0],
                               localTrans.getOrigin()[1],
                               localTrans.getOrigin()[2]);
          ...
          numShapes++;
          // btCapsuleShapeZ* capsuleShape = new btCapsuleShapeZ(radius,
          // height);
          // shape = capsuleShape;
        ...
        case URDF_GEOM_CYLINDER: {
          ...
          cache.m_geomDimensions.push_back(
              physx::PxVec3(cylRadius, cylHalfLength, 0));
          ...
          // else
          {
            ...
            for (int i = 0; i < numSteps; i++) {
              physx::PxVec3 vert(
                  cylRadius * btSin(SIMD_2_PI * (float(i) / numSteps)),
            ...
            physx::PxConvexMesh* convexMesh =
                createPhysXConvex(vertices.size(), &vertices[0], creation);

            shape = physx::PxRigidActorExt::createExclusiveShape(
                *linkPtr, physx::PxConvexMeshGeometry(convexMesh), *material);
        ...
        case URDF_GEOM_BOX: {
          ...
          shape = physx::PxRigidActorExt::createExclusiveShape(
              *linkPtr, physx::PxBoxGeometry(extents[0] * 0.5, extents[1] * 0.5,
                                             extents[2] * 0.5),
              *material);
          cache.m_geomDimensions.push_back(physx::PxVec3(
              extents[0] * 0.5, extents[1] * 0.5, extents[2] * 0.5));
          shape->setLocalPose(tr);
          cache.m_geomLocalPoses.push_back(tr);
          numShapes++;
          break;
        ...
        case URDF_GEOM_SPHERE: {
          ...
          shape = physx::PxRigidActorExt::createExclusiveShape(
              *linkPtr, physx::PxSphereGeometry(radius), *material);
        ...
        case URDF_GEOM_MESH: {
          ...
          switch (collision->m_geometry.m_meshFileType) {
            case UrdfGeometry::FILE_OBJ: {
              ...
              pathPrefix[0] = 0;
              if (creation.m_fileIO->findResourcePath(
                      collision->m_geometry.m_meshFileName.c_str(),
                      relativeFileName, 1024)) {
                b3FileUtils::extractPath(relativeFileName, pathPrefix, 1024);
              }
              glmesh =
                  LoadMeshFromObj(collision->m_geometry.m_meshFileName.c_str(),
                                  pathPrefix, creation.m_fileIO);

              break;
            }
            case UrdfGeometry::FILE_STL: {
              glmesh =
                  LoadMeshFromSTL(collision->m_geometry.m_meshFileName.c_str(),
                                  creation.m_fileIO);
              break;
            }
            default: {}
          ...
          if (glmesh && glmesh->m_numvertices) {
            for (int i = 0; i < glmesh->m_numvertices; i++) {
              physx::PxVec3 vert(glmesh->m_vertices->at(i).xyzw[0] *
                                     collision->m_geometry.m_meshScale[0],
                                 glmesh->m_vertices->at(i).xyzw[1] *
                                     collision->m_geometry.m_meshScale[1],
                                 glmesh->m_vertices->at(i).xyzw[2] *
                                     collision->m_geometry.m_meshScale[2]);
              vertices.push_back(vert);
            }
        ...
        default: { printf("unknown physx shape\n"); }
      }
...
btTransform ConvertURDF2PhysXInternal(
    const PhysXURDFImporter& u2b, ArticulationCreationInterface& creation,
    URDF2PhysXCachedData& cache, int urdfLinkIndex,
    const btTransform& parentTransformInWorldSpace, bool createActiculation,
    const char* pathPrefix, int flags,
    UrdfVisualShapeCache2* cachedLinkGraphicsShapesIn,
    UrdfVisualShapeCache2* cachedLinkGraphicsShapesOut, bool recursive) {
  ...
  if (urdfParentIndex == -2) {
    ...
    u2b.getMassAndInertia2(urdfParentIndex, parentMass,
                           parentLocalInertiaDiagonal, parentLocalInertialFrame,
                           flags);
  }
  ...
  bool hasParentJoint = u2b.getJointInfo2(
  ...
  switch (jointType) {
    ...
    case URDFRevoluteJoint: {
      ...
      // btQuaternion axis2ReferenceRot(btVector3(0, 1, 0), SIMD_HALF_PI);// =
      // shortestArcQuat(refAxis, btVector3(axis[0], axis[1], axis[2]));
      btQuaternion axis2ReferenceRot =
          shortestArcQuat(refAxis, btVector3(axis[0], axis[1], axis[2]));
  ...
  if (flags & CUF_USE_SDF) {
    parent2joint =
        parentTransformInWorldSpace.inverse() * linkTransformInWorldSpace;
  } else {
  ...
  int graphicsIndex;
  {
    ...
    graphicsIndex = u2b.convertLinkVisualShapes3(
        urdfLinkIndex, pathPrefix, localInertialFrame,
        u2b.getUrdfLink(urdfLinkIndex), u2b.getUrdfModel(), -1,
        u2b.getBodyUniqueId(), creation.m_fileIO);
  ...
  if (1) {
    ...
    */
    ...
    btTransform inertialFrameInWorldSpace =
    ...
    physx::PxTransform tr;
    tr.p = physx::PxVec3(linkTransformInWorldSpace.getOrigin().x(),
                         linkTransformInWorldSpace.getOrigin().y(),
                         linkTransformInWorldSpace.getOrigin().z());
    tr.q = physx::PxQuat(linkTransformInWorldSpace.getRotation().x(),
                         linkTransformInWorldSpace.getRotation().y(),
                         linkTransformInWorldSpace.getRotation().z(),
                         linkTransformInWorldSpace.getRotation().w());
    ...
    if (!createActiculation) {
      ...
      if (cache.m_articulation == 0) {
        cache.m_articulation =
            creation.m_physics->createArticulationReducedCoordinate();

        if (isFixedBase) {
          cache.m_articulation->setArticulationFlags(
              physx::PxArticulationFlag::eFIX_BASE);
        }

        physx::PxArticulationLink* base =
            cache.m_articulation->createLink(NULL, tr);

        linkPtr = base;
        rbLinkPtr = base;

        // physx::PxRigidActorExt::createExclusiveShape(*base,
        // PxBoxGeometry(0.5f, 0.25f, 1.5f), *gMaterial);
        ...
        cache.m_jointTypes.push_back(
            physx::PxArticulationJointType::eUNDEFINED);
...
// cache.m_articulation->setSeparationTolerance(0.001f);
        ...
        cache.registerMultiBody(urdfLinkIndex, base, inertialFrameInWorldSpace,
                                mass, localInertiaDiagonal, localInertialFrame);
      } else {
        physx::PxArticulationLink* parentLink =
            cache.getPhysxLinkFromLink(urdfParentIndex);

        physx::PxArticulationLink* childLink =
            cache.m_articulation->createLink(parentLink, tr);
        linkPtr = childLink;
        rbLinkPtr = childLink;

        physx::PxArticulationJointReducedCoordinate* joint =
            static_cast<physx::PxArticulationJointReducedCoordinate*>(
                childLink->getInboundJoint());
        ...
        switch (jointType) {
          ...
          case URDFSphericalJoint: {
            ...
            joint->setMotion(physx::PxArticulationAxis::eTWIST,
                             physx::PxArticulationMotion::eFREE);
            joint->setMotion(physx::PxArticulationAxis::eSWING2,
                             physx::PxArticulationMotion::eFREE);
            joint->setMotion(physx::PxArticulationAxis::eSWING1,
                             physx::PxArticulationMotion::eFREE);
            break;
          }
          case URDFContinuousJoint:
          ...
          case URDFPrismaticJoint: {
            ...
            joint->setMotion(physx::PxArticulationAxis::eX,
                             physx::PxArticulationMotion::eFREE);
            break;
          }
          default: {
        ...
        physx::PxTransform parentPose(
            physx::PxVec3(offsetInA.getOrigin()[0], offsetInA.getOrigin()[1],
                          offsetInA.getOrigin()[2]),
            physx::PxQuat(
                offsetInA.getRotation()[0], offsetInA.getRotation()[1],
                offsetInA.getRotation()[2], offsetInA.getRotation()[3]));

        physx::PxTransform childPose(
            physx::PxVec3(offsetInB.getOrigin()[0], offsetInB.getOrigin()[1],
                          offsetInB.getOrigin()[2]),
            physx::PxQuat(
                offsetInB.getRotation()[0], offsetInB.getRotation()[1],
                offsetInB.getRotation()[2], offsetInB.getRotation()[3]));
        ...
        cache.registerMultiBody(urdfLinkIndex, childLink,
                                inertialFrameInWorldSpace, mass,
                                localInertiaDiagonal, localInertialFrame);
      }
    }
    ...
    convertLinkPhysXShapes(u2b, cache, creation, urdfLinkIndex, pathPrefix,
                           localInertialFrame, cache.m_articulation,
                           mbLinkIndex, linkPtr);
    ...
    if (hasParentJoint) {
      ...
      btQuaternion parentRotToThis =
          offsetInB.getRotation() * offsetInA.inverse().getRotation();
  ...
  if (recursive) {
    for (int i = 0; i < numChildren; i++) {
      ...
      ConvertURDF2PhysXInternal(u2b, creation, cache, urdfChildLinkIndex,
                                linkTransformInWorldSpace, createActiculation,
                                pathPrefix, flags, cachedLinkGraphicsShapesIn,
                                cachedLinkGraphicsShapesOut, recursive);
    }
  }
  return linkTransformInWorldSpace;
}

physx::PxBase* URDF2PhysX(physx::PxFoundation* foundation,
                          physx::PxPhysics* physics, physx::PxCooking* cooking,
                          physx::PxScene* scene, class PhysXURDFImporter& u2p,
                          int flags, const char* pathPrefix,
                          const btTransform& rootTransformInWorldSpace,
                          struct CommonFileIOInterface* fileIO,
                          bool createActiculation) {
  URDF2PhysXCachedData cache;
  ...
  if (recursive) {
    ConvertURDF2PhysXInternal(u2p, creation, cache, urdfLinkIndex,
                              rootTransformInWorldSpace, createActiculation,
                              pathPrefix, flags, &cachedLinkGraphicsShapes,
                              &cachedLinkGraphicsShapesOut, recursive);
  ...
  if (cache.m_articulation) {
    ...
    for (int i = 0; i < cache.m_linkTransWorldSpace.size(); i++) {
      printf("PxTransform(PxVec3(%f,%f,%f), PxQuat(%f,%f,%f,%f),\n",
             cache.m_linkTransWorldSpace[i].p.x,
             cache.m_linkTransWorldSpace[i].p.y,
             cache.m_linkTransWorldSpace[i].p.z,
             cache.m_linkTransWorldSpace[i].q.x,
             cache.m_linkTransWorldSpace[i].q.y,
             cache.m_linkTransWorldSpace[i].q.z,
             cache.m_linkTransWorldSpace[i].q.w);
    }
    printf("m_parentLocalPoses\n");
    for (int i = 0; i < cache.m_parentLocalPoses.size(); i++) {
      printf("PxTransform(PxVec3(%f,%f,%f), PxQuat(%f,%f,%f,%f),\n",
             cache.m_parentLocalPoses[i].p.x, cache.m_parentLocalPoses[i].p.y,
             cache.m_parentLocalPoses[i].p.z, cache.m_parentLocalPoses[i].q.x,
             cache.m_parentLocalPoses[i].q.y, cache.m_parentLocalPoses[i].q.z,
             cache.m_parentLocalPoses[i].q.w);
    }

    printf("m_childLocalPoses\n");
    for (int i = 0; i < cache.m_childLocalPoses.size(); i++) {
      printf("PxTransform(PxVec3(%f,%f,%f), PxQuat(%f,%f,%f,%f),\n",
             cache.m_childLocalPoses[i].p.x, cache.m_childLocalPoses[i].p.y,
             cache.m_childLocalPoses[i].p.z, cache.m_childLocalPoses[i].q.x,
             cache.m_childLocalPoses[i].q.y, cache.m_childLocalPoses[i].q.z,
             cache.m_childLocalPoses[i].q.w);
    }

    printf("m_geomDimensions\n");
    for (int i = 0; i < cache.m_geomDimensions.size(); i++) {
      printf("PxVec3(%f,%f,%f),\n", cache.m_geomDimensions[i].x,
             cache.m_geomDimensions[i].y, cache.m_geomDimensions[i].z);
    }

    printf("m_geomLocalPoses\n");
    for (int i = 0; i < cache.m_geomLocalPoses.size(); i++) {
      printf("PxTransform(PxVec3(%f,%f,%f), PxQuat(%f,%f,%f,%f),\n",
             cache.m_geomLocalPoses[i].p.x, cache.m_geomLocalPoses[i].p.y,
             cache.m_geomLocalPoses[i].p.z, cache.m_geomLocalPoses[i].q.x,
             cache.m_geomLocalPoses[i].q.y, cache.m_geomLocalPoses[i].q.z,
             cache.m_geomLocalPoses[i].q.w);
    }

    printf("m_linkMaterials\n");
    for (int i = 0; i < cache.m_linkMaterials.size(); i++) {
      printf("PxVec3(%f,%f,%f),\n", cache.m_linkMaterials[i].x,
             cache.m_linkMaterials[i].y, cache.m_linkMaterials[i].z);
    }
#endif  // DEBUG_ARTICULATIONS

    // see also https://github.com/NVIDIAGameWorks/PhysX/issues/43
    if ((flags & CUF_USE_SELF_COLLISION) == 0) {
      physx::PxU32 nbActors = cache.m_articulation->getNbLinks();
      ;  // Max number of actors expected in the aggregate
      bool selfCollisions = false;
      physx::PxAggregate* aggregate =
          physics->createAggregate(nbActors, selfCollisions);
