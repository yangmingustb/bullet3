...
/// now it become more general we need to move out the shared memory specifics
/// away
/// for example naming [disconnectSharedMemory -> disconnect] [ move
/// setSharedMemoryKey to shared memory specific subclass ]
...
class PhysicsLoopBack : public PhysicsClient {
 ...
 public:
  ...
  virtual bool getJointInfo(int bodyIndex, int jointIndex,
  ...
  virtual bool getCachedUserData(int userDataId,
                                 struct b3UserDataValue& valueOut) const;
  virtual int getCachedUserDataId(int bodyUniqueId, int linkIndex,
                                  int visualShapeIndex, const char* key) const;
