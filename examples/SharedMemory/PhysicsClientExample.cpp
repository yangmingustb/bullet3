...
class PhysicsClientExample : public SharedMemoryCommon {
 protected:
  ...
  // this m_physicsServer is only used when option eCLIENTEXAMPLE_SERVER is
  // enabled
 ...
 public:
  ...
  virtual bool wantsTermination() { return m_wantsTermination; }

  virtual bool isConnected() { return (m_physicsClientHandle != 0); }
    ...
    if (1) {
      ...
      if (points.size() && indices.size()) {
        m_guiHelper->getRenderInterface()->drawLines(
            &points[0].m_floats[0], color, points.size(),
            sizeof(btVector3FloatData), &indices[0], indices.size(), lineWidth);
      }
    } else {
      for (int i = 0; i < numLines; i++) {
        m_guiHelper->getRenderInterface()->drawLine(
  ...
  void prepareControlCommand(b3SharedMemoryCommandHandle commandHandle) {
    for (int i = 0; i < m_numMotors; i++) {
      ...
      static int serial = 0;
      serial++;
      //  b3Printf("# motors = %d, cmd[%d] qIndex = %d, uIndex = %d, targetPos =
      //  %f", m_numMotors, serial, qIndex,uIndex,targetPos);
  ...
  virtual void setSharedMemoryKey(int key) { m_sharedMemoryKey = key; }
};
  ...
  switch (commandId) {
    case CMD_LOAD_URDF: {
      b3SharedMemoryCommandHandle commandHandle =
          b3LoadUrdfCommandInit(m_physicsClientHandle, "kuka_iiwa/model.urdf");
      // setting the initial position, orientation and other arguments are
      // optional
      double startPosX = 0;
      static double startPosY = 0;
      double startPosZ = 0;
      b3LoadUrdfCommandSetStartPosition(commandHandle, startPosX, startPosY,
                                        startPosZ);
      startPosY += 2.f;
...
#ifdef BT_DEBUG
      b3SharedMemoryCommandHandle commandHandle =
          b3LoadSdfCommandInit(m_physicsClientHandle, "two_cubes.sdf");
#else
      b3SharedMemoryCommandHandle commandHandle = b3LoadSdfCommandInit(
          m_physicsClientHandle,
          "kitchens/1.sdf");  // two_cubes.sdf");//kitchens/1.sdf");//kuka_iiwa/model.sdf");
#endif
      b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
      break;
    }
    case CMD_REQUEST_CAMERA_IMAGE_DATA: {
      ...
      b3SharedMemoryCommandHandle commandHandle =
          b3InitRequestCameraImage(m_physicsClientHandle);
      // b3RequestCameraImageSelectRenderer(commandHandle,ER_BULLET_HARDWARE_OPENGL);

      float viewMatrix[16];
      float projectionMatrix[16];
      m_guiHelper->getRenderInterface()
          ->getActiveCamera()
          ->getCameraProjectionMatrix(projectionMatrix);
      m_guiHelper->getRenderInterface()->getActiveCamera()->getCameraViewMatrix(
          viewMatrix);

      b3RequestCameraImageSetCameraMatrices(commandHandle, viewMatrix,
                                            projectionMatrix);
      b3RequestCameraImageSetPixelResolution(commandHandle, camVisualizerWidth,
                                             camVisualizerHeight);
      float lightPos[3];
    ...
    case CMD_CREATE_BOX_COLLISION_SHAPE: {
      b3SharedMemoryCommandHandle commandHandle =
          b3CreateBoxShapeCommandInit(m_physicsClientHandle);
    ...
    case CMD_CREATE_RIGID_BODY: {
      ...
      b3CreateBoxCommandSetCollisionShapeType(commandHandle,
                                              COLLISION_SHAPE_TYPE_CYLINDER_Y);
      b3CreateBoxCommandSetColorRGBA(commandHandle, 1, 1, 0, 1);
      double radius = 0.2;
      double halfHeight = 0.5;
      b3CreateBoxCommandSetHalfExtents(commandHandle, radius, halfHeight,
                                       radius);
      b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
      break;
    }
    ...
    case CMD_REQUEST_ACTUAL_STATE: {
      if (m_selectedBody >= 0) {
        b3SharedMemoryCommandHandle commandHandle =
            b3RequestActualStateCommandInit(m_physicsClientHandle,
                                            m_selectedBody);
        b3SharedMemoryStatusHandle statusHandle =
            b3SubmitClientCommandAndWaitStatus(m_physicsClientHandle,
                                               commandHandle);
        b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
    ...
    case CMD_INIT_POSE: {
      if (m_selectedBody >= 0) {
        b3SharedMemoryCommandHandle commandHandle =
            b3CreatePoseCommandInit(m_physicsClientHandle, m_selectedBody);
        static int toggle = 0;
        double pos[3] = {0, 0, 0};
        pos[toggle] = 2;
        toggle++;
        ...
        b3CreatePoseCommandSetBaseOrientation(commandHandle, orn[0], orn[1],
                                              orn[2], orn[3]);
        b3CreatePoseCommandSetBasePosition(commandHandle, pos[0], pos[1],
                                           pos[2]);
        int numJoints = b3GetNumJoints(m_physicsClientHandle, m_selectedBody);
        ...
        for (int i = 0; i < numJoints; i++) {
          ...
          if ((info.m_jointType == 0) ||
              (info.m_jointType == 1))  // revolute or prismatic
          {
            b3CreatePoseCommandSetJointPosition(m_physicsClientHandle,
                                                commandHandle, i, jointPos);
          }
        }
    ...
    case CMD_STEP_FORWARD_SIMULATION: {
      b3SharedMemoryCommandHandle commandHandle =
          b3InitStepSimulationCommand(m_physicsClientHandle);
      b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
      break;
    }
    ...
    case CMD_REQUEST_DEBUG_LINES: {
      b3SharedMemoryCommandHandle commandHandle =
          b3InitRequestDebugLinesCommand(m_physicsClientHandle,
    ...
    case CMD_SEND_DESIRED_STATE: {
      if (m_selectedBody >= 0) {
        // b3SharedMemoryCommandHandle command = b3JointControlCommandInit(
        // m_physicsClientHandle, m_selectedBody, CONTROL_MODE_VELOCITY);
        b3SharedMemoryCommandHandle command =
            b3JointControlCommandInit2(m_physicsClientHandle, m_selectedBody,
    ...
    case CMD_RESET_SIMULATION: {
      b3SharedMemoryCommandHandle commandHandle =
          b3InitResetSimulationCommand(m_physicsClientHandle);
      b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
      break;
    }
    ...
    case CMD_CUSTOM_SET_GRAVITY: {
      b3SharedMemoryCommandHandle commandHandle =
          b3InitPhysicsParamCommand(m_physicsClientHandle);
      ...
      if (m_selectedBody >= 0) {
        ...
        if (numJoints) {
          ...
          for (int i = 0; i < numJoints; i++) {
            ...
            b3Printf("Desired joint acceleration[%d]=%f", i,
                     jointAccelerations[i]);
          }
          b3SharedMemoryCommandHandle commandHandle =
              b3CalculateInverseDynamicsCommandInit(
                  m_physicsClientHandle, m_selectedBody, &jointPositionsQ[0],
                  &jointVelocitiesQdot[0], &jointAccelerations[0]);
          b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
        }
      }
      break;
    ...
    case CMD_REQUEST_CONTACT_POINT_INFORMATION: {
      b3SharedMemoryCommandHandle commandHandle =
          b3InitRequestContactPointInformation(m_physicsClientHandle);
    ...
    case CMD_SAVE_WORLD: {
      b3SharedMemoryCommandHandle commandHandle =
          b3SaveWorldCommandInit(m_physicsClientHandle, "saveWorld.py");
      b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
      break;
    }
    case CMD_REQUEST_VISUAL_SHAPE_INFO: {
      if (m_selectedBody >= 0) {
        ...
        b3SharedMemoryCommandHandle commandHandle =
            b3InitRequestVisualShapeInformation(m_physicsClientHandle,
                                                m_selectedBody);
        b3SubmitClientCommand(m_physicsClientHandle, commandHandle);
      }
      break;
    ...
    case CMD_UPDATE_VISUAL_SHAPE: {
      ...
      b3SharedMemoryCommandHandle commandHandle = b3InitUpdateVisualShape2(
          m_physicsClientHandle, objectUniqueId, linkIndex, shapeIndex);
...
struct Bullet2CommandProcessorCreation3
    : public CommandProcessorCreationInterface {
...
void PhysicsClientExample::createButton(const char* name, int buttonId,
                                        bool isTrigger) {
...
void PhysicsClientExample::createButtons() {
  ...
  if (m_guiHelper && m_guiHelper->getParameterInterface()) {
    ...
    createButton("Get Visual Shape Info", CMD_REQUEST_VISUAL_SHAPE_INFO,
                 isTrigger);
    ...
    createButton("Create Box Collider", CMD_CREATE_BOX_COLLISION_SHAPE,
                 isTrigger);
    ...
    createButton("Compute Inverse Dynamics", CMD_CALCULATE_INVERSE_DYNAMICS,
                 isTrigger);
    createButton("Get Contact Point Info",
                 CMD_REQUEST_CONTACT_POINT_INFORMATION, isTrigger);

    if (m_bodyUniqueIds.size()) {
      if (m_selectedBody < 0) m_selectedBody = 0;
    ...
    if (m_physicsClientHandle && m_selectedBody >= 0) {
      ...
      for (int i = 0; i < numJoints; i++) {
        b3JointInfo info;
        b3GetJointInfo(m_physicsClientHandle, m_selectedBody, i, &info);
        // b3Printf("Joint %s at q-index %d and u-index
...

    {
      ...
      SliderParams sliderSpecularCoeff("specular coefficient",
                                       &m_specularCoeff);
...

  {
    ...
    if (m_canvas) {
      m_canvasRGBIndex = m_canvas->createCanvas("Synthetic Camera RGB data",
                                                camVisualizerWidth,
                                                camVisualizerHeight, 8, 55);
      m_canvasDepthIndex = m_canvas->createCanvas(
          "Synthetic Camera Depth data", camVisualizerWidth,
          camVisualizerHeight, 8, 75 + camVisualizerHeight);
      m_canvasSegMaskIndex = m_canvas->createCanvas(
          "Synthetic Camera Segmentation Mask", camVisualizerWidth,
          camVisualizerHeight, 8, 95 + camVisualizerHeight * 2);
    ...
    if (hasStatus) {
      ...
      if (statusType == CMD_CAMERA_IMAGE_COMPLETED) {
        ...
        if (m_canvas) {
          ...
          for (int i = 0; i < camVisualizerWidth; i++) {
            for (int j = 0; j < camVisualizerHeight; j++) {
              int xIndex = int(float(i) * (float(imageData.m_pixelWidth) /
                                           float(camVisualizerWidth)));
              int yIndex = int(float(j) * (float(imageData.m_pixelHeight) /
                                           float(camVisualizerHeight)));
              btClamp(xIndex, 0, imageData.m_pixelWidth);
              btClamp(yIndex, 0, imageData.m_pixelHeight);

              if (m_canvasDepthIndex >= 0) {
                int depthPixelIndex =
                    (xIndex + yIndex * imageData.m_pixelWidth);
          ...
          for (int i = 0; i < camVisualizerWidth; i++) {
            for (int j = 0; j < camVisualizerHeight; j++) {
              ...
              if (m_canvasRGBIndex >= 0) {
                int rgbPixelIndex =
                    (xIndex + yIndex * imageData.m_pixelWidth) * bytesPerPixel;
              ...
              if (m_canvasDepthIndex >= 0) {
                ...
                if (depthValue > -1e20) {
                  ...
                  if (maxDepthValue != minDepthValue) {
                    rgb = (depthValue - minDepthValue) *
                          (255. / (btFabs(maxDepthValue - minDepthValue)));
                    if (rgb < 0 || rgb > 255) {
                      // printf("rgb=%d\n",rgb);
                    }
                  }
              ...
              if (m_canvasSegMaskIndex >= 0 &&
                  (0 != imageData.m_segmentationMaskValues)) {
                int segmentationMaskPixelIndex =
                    (xIndex + yIndex * imageData.m_pixelWidth);
                int segmentationMask =
                    imageData
                        .m_segmentationMaskValues[segmentationMaskPixelIndex];
                btVector4 palette[4] = {
      ...
      if (statusType == CMD_CALCULATED_INVERSE_DYNAMICS_COMPLETED) {
        ...
        b3GetStatusInverseDynamicsJointForces(status, &bodyUniqueId, &dofCount,
                                              0);

        btAlignedObjectArray<double> jointForces;
        if (dofCount) {
          jointForces.resize(dofCount);
          b3GetStatusInverseDynamicsJointForces(status, 0, 0, &jointForces[0]);
      ...
      if (statusType == CMD_SDF_LOADING_COMPLETED) {
        ...
        int numBodies =
            b3GetStatusBodyIndices(status, bodyIndicesOut, bodyCapacity);
        if (numBodies > bodyCapacity) {
          b3Warning(
              "loadSDF number of bodies (%d) exceeds the internal body "
              "capacity (%d)",
              numBodies, bodyCapacity);
        } else {
          for (int i = 0; i < numBodies; i++) {
            ...
            /*                        int numJoints =
b3GetNumJoints(m_physicsClientHandle,bodyUniqueId);
...
*/
  ...
  if (b3CanSubmitCommand(m_physicsClientHandle)) {
    if (m_userCommandRequests.size()) {
      ...
      // a manual 'pop_front', we don't use 'remove' because it will re-order
      // the commands
      ...
      // for the CMD_RESET_SIMULATION we need to do something special: clear the
      // GUI sliders
...
class CommonExampleInterface* PhysicsClientCreateFunc(
    struct CommonExampleOptions& options) {
  PhysicsClientExample* example =
      new PhysicsClientExample(options.m_guiHelper, options.m_option);
