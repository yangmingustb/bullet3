...
static int gyroflags[4] = {0,  // none, no gyroscopic term
                           BT_ENABLE_GYROSCOPIC_FORCE_EXPLICIT,
                           BT_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_WORLD,
                           BT_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_BODY};

static const char* gyroNames[4] = {"No Gyroscopic", "Explicit",
                                   "Implicit (World)", "Implicit (Body)"};
...
void GyroscopicSetup::initPhysics() {
  ...
  btVector3 positions[4] = {
      btVector3(-10, 8, 4), btVector3(-5, 8, 4), btVector3(0, 8, 4),
      btVector3(5, 8, 4),
  };
...

  {
    ...
    // btBoxShape(btVector3(btScalar(50.),btScalar(50.),btScalar(0.5)));
...
void GyroscopicSetup::physicsDebugDraw(int debugFlags) {
  ...
  for (int i = 0; i < m_dynamicsWorld->getNumCollisionObjects(); i++) {
    btRigidBody* body =
        btRigidBody::upcast(m_dynamicsWorld->getCollisionObjectArray()[i]);
...
class CommonExampleInterface* GyroscopicCreateFunc(
