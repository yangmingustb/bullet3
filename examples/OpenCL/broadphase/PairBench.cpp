// those header files need to be at the top, because of conflict __global and
// STL
...
// we use an offset, just for testing to make sure there is no assumption in the
// broadphase that 'index' starts at 0
...
PairBench::PairBench(GUIHelperInterface* helper) : CommonOpenCLBase(helper) {
  m_data = new PairBenchInternalData;

  m_data->m_validationBroadphase = 0;
}
PairBench::~PairBench() { delete m_data; }
...
void PairBench::initPhysics() {
  ...
  if (m_clData->m_clContext) {
    cl_int err;
    cl_program pairBenchProg = b3OpenCLUtils::compileCLProgramFromString(
        m_clData->m_clContext, m_clData->m_clDevice, pairsKernelsCL, &err, "",
        PAIRS_CL_PROGRAM_PATH);
    int errNum = 0;
    m_data->m_moveObjectsKernel = b3OpenCLUtils::compileCLKernelFromString(
        m_clData->m_clContext, m_clData->m_clDevice, pairsKernelsCL,
        "moveObjectsKernel", &errNum, pairBenchProg);
    m_data->m_sineWaveKernel = b3OpenCLUtils::compileCLKernelFromString(
        m_clData->m_clContext, m_clData->m_clDevice, pairsKernelsCL,
        "sineWaveKernel", &errNum, pairBenchProg);
    m_data->m_colorPairsKernel = b3OpenCLUtils::compileCLKernelFromString(
        m_clData->m_clContext, m_clData->m_clDevice, pairsKernelsCL,
        "colorPairsKernel2", &errNum, pairBenchProg);
    m_data->m_updateAabbSimple = b3OpenCLUtils::compileCLKernelFromString(
        m_clData->m_clContext, m_clData->m_clDevice, pairsKernelsCL,
        "updateAabbSimple", &errNum, pairBenchProg);

    // Method for validating the overlapping pairs requires that the
    // reference broadphase does not maintain internal state aside from AABB
    // data.
    ...
    // and then calling b3GpuBroadphaseInterface::calculateOverlappingPairs()
    // should
    // always produce the same result regardless of the current state of the
    // broadphase.
    m_data->m_validationBroadphase =
        b3GpuParallelLinearBvhBroadphase::CreateFunc(
...
void PairBench::createBroadphase(int arraySizeX, int arraySizeY,
                                 int arraySizeZ) {
  m_data->m_broadphaseGPU =
      (allBroadphases[curSelectedBroadphase].m_createFunc)(
          m_clData->m_clContext, m_clData->m_clDevice, m_clData->m_clQueue);
  ...
  int shapeId = m_guiHelper->getRenderInterface()->registerShape(
      &cube_vertices[0], numVertices, cube_indices, numIndices);
  int group = 1;
  int mask = 1;
  ...
  if (gPairBenchFileName) {
    ...
    const char* prefix[] = {"./data/", "../data/", "../../data/",
                            "../../../data/", "../../../../data/"};
    int prefixIndex = -1;
    {
      ...
      for (int i = 0; i < numPrefixes; i++) {
        ...
        if (f) {
          ...
          while (actualReadBytes < size) {
            ...
            actualReadBytes +=
                fread(&buf[actualReadBytes], 1, numPlannedRead, f);
          }
          ...
          for (oriptr = buf; (patloc = strstr(oriptr, pattern));
            ...
            if (patloc) {
              *patloc = 0;
              ...
              b3Vector4 scaling =
                  b3MakeVector4(0.5 * extents.x, 0.5 * extents.y,
                                0.5 * extents.z, 1);  // b3MakeVector4(1,1,1,1);
              ...
              if (l > 500) {
                ...
                m_data->m_broadphaseGPU->createLargeProxy(aabbMin, aabbMax,
                                                          index, group, mask);
              } else {
                ...
                id =
                    m_guiHelper->getRenderInterface()->registerGraphicsInstance(
                        shapeId, position, orn, color, scaling);
                m_data->m_broadphaseGPU->createProxy(aabbMin, aabbMax, index,
                                                     group, mask);
                index++;
              }
    ...
    for (int i = 0; i < arraySizeX; i++) {
      for (int j = 0; j < arraySizeY; j++) {
        for (int k = 0; k < arraySizeZ; k++) {
          ...
          int id;
          id = m_guiHelper->getRenderInterface()->registerGraphicsInstance(
              shapeId, position, orn, color, scaling);
          ...
          if (large) {
            m_data->m_broadphaseGPU->createLargeProxy(aabbMin, aabbMax, index,
                                                      group, mask);
          } else {
            m_data->m_broadphaseGPU->createProxy(aabbMin, aabbMax, index, group,
                                                 mask);
          }
          index++;
        }
...
void PairBench::stepSimulation(float deltaTime) {
  ...
  GLInstanceRendererInternalData* internalData =
      m_guiHelper->getRenderInterface()->getInternalData();

  if (internalData == 0) return;
  ...
  if (numObjects) {
    ...
    char* hostPtr = 0;
    {
      ...
      hostPtr = (char*)glMapBufferRange(
          GL_ARRAY_BUFFER, internalData->m_maxShapeCapacityInBytes,
          arraySizeInBytes,
          GL_MAP_WRITE_BIT |
              GL_MAP_READ_BIT);  // GL_READ_WRITE);//GL_WRITE_ONLY
    }
    ...
    if (m_data->m_instancePosOrnColor &&
        m_data->m_instancePosOrnColor->size() != 3 * numObjects) {
      delete m_data->m_instancePosOrnColor;
      m_data->m_instancePosOrnColor = 0;
    }
    if (!m_data->m_instancePosOrnColor) {
      m_data->m_instancePosOrnColor = new b3OpenCLArray<b3Vector4>(
          m_clData->m_clContext, m_clData->m_clQueue);
      m_data->m_instancePosOrnColor->resize(3 * numObjects);
      m_data->m_instancePosOrnColor->copyFromHostPointer(positions,
                                                         3 * numObjects, 0);
      m_data->m_bodyTimes =
          new b3OpenCLArray<float>(m_clData->m_clContext, m_clData->m_clQueue);
    ...
    if (!gPairBenchFileName) {
      if (1) {
        if (1) {
          ...
          b3LauncherCL launcher(m_clData->m_clQueue,
                                m_data->m_moveObjectsKernel,
                                "m_moveObjectsKernel");
          launcher.setBuffer(m_data->m_instancePosOrnColor->getBufferCL());
          launcher.setConst(numObjects);
          launcher.launch1D(numObjects);
        ...
        for (int nodeId = 0; nodeId < numObjects; nodeId++) {
          {
            ...
            b3SapAabb orgAabb =
                m_data->m_broadphaseGPU->getAllAabbsCPU()[nodeId];
            b3Vector3 halfExtents =
                0.5f * (orgAabb.m_maxVec - orgAabb.m_minVec);
            int orgNodeIndex = orgAabb.m_minIndices[3];
            int orgBroadphaseIndex = orgAabb.m_signedMaxIndices[3];

            m_data->m_broadphaseGPU->getAllAabbsCPU()[nodeId].m_minVec =
                position - halfExtents;
            m_data->m_broadphaseGPU->getAllAabbsCPU()[nodeId].m_minIndices[3] =
                orgNodeIndex;
            m_data->m_broadphaseGPU->getAllAabbsCPU()[nodeId].m_maxVec =
                position + halfExtents;
            m_data->m_broadphaseGPU->getAllAabbsCPU()[nodeId]
                .m_signedMaxIndices[3] = orgBroadphaseIndex;
          }
        }
        m_data->m_broadphaseGPU->writeAabbsToGpu();
      }
  ...
  const bool VALIDATE_BROADPHASE =
      false;  // Check that overlapping pairs of 2 broadphases are the same
  if (numObjects && VALIDATE_BROADPHASE) {
...

    {
      ...
      // m_data->m_validationBroadphase->getAllAabbsCPU() =
      // m_data->m_broadphaseGPU->getAllAabbsCPU();

      m_data->m_validationBroadphase->getAllAabbsGPU().copyFromOpenCLArray(
          m_data->m_broadphaseGPU->getAllAabbsGPU());
      m_data->m_validationBroadphase->getSmallAabbIndicesGPU()
          .copyFromOpenCLArray(
              m_data->m_broadphaseGPU->getSmallAabbIndicesGPU());
      m_data->m_validationBroadphase->getLargeAabbIndicesGPU()
          .copyFromOpenCLArray(
              m_data->m_broadphaseGPU->getLargeAabbIndicesGPU());
    ...
    m_data->m_broadphaseGPU->getOverlappingPairsGPU().copyToHost(
        overlappingPairs);
    m_data->m_validationBroadphase->getOverlappingPairsGPU().copyToHost(
        overlappingPairsReference);
    ...
    // Compare
    {
      ...
      if (numPairs == numPairsReference) {
        for (int i = 0; i < numPairsReference; ++i) {
          ...
          if (pairA.x != pairB.x || pairA.y != pairB.y || pairA.z != pairB.z ||
              pairA.w != pairB.w) {
            b3Error(
                "Error: one or more overlappingPairs differs from "
                "reference.\n");
            success = false;
            break;
          }
  ...
  if (m_data->m_gui)
  {
          ...
          char msg[1024];
          sprintf(msg,"#objects = %d, #overlapping pairs = %d, time = %f ms",
  allAabbs,numOverlap,time );
          //printf("msg=%s\n",msg);
  ...
  */
  ...
  if (numObjects) {
    ...
    if (numObjects) {
      m_data->m_instancePosOrnColor->copyToHostPointer(positions,
                                                       3 * numObjects, 0);
    }
...
class CommonExampleInterface* PairBenchOpenCLCreateFunc(
