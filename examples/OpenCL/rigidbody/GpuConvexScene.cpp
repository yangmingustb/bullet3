...
class GpuConvexScene : public GpuRigidBodyDemo {
 ...
 public:
  ...
  virtual const char* getName() { return "Tetrahedra"; }
  ...
  virtual int createDynamicsObjects2(const float* vertices, int numVertices,
...
class GpuConvexPlaneScene : public GpuConvexScene {
 public:
  GpuConvexPlaneScene(GUIHelperInterface* helper) : GpuConvexScene(helper) {}
  virtual ~GpuConvexPlaneScene() {}
  virtual const char* getName() { return "ConvexOnPlane"; }
...
class GpuBoxPlaneScene : public GpuConvexPlaneScene {
 public:
  ...
  virtual const char* getName() { return "BoxBox"; }
...
class GpuTetraScene : public GpuConvexScene {
 ...
 public:
  virtual const char* getName() { return "TetraBreakable"; }

  virtual int createDynamicsObjects();
};

b3Vector4 colors[4] = {
    b3MakeVector4(1, 0, 0, 1), b3MakeVector4(0, 1, 0, 1),
    b3MakeVector4(0, 1, 1, 1), b3MakeVector4(1, 1, 0, 1),
};

void GpuConvexScene::setupScene() {
  m_raycaster = new b3GpuRaycast(m_clData->m_clContext, m_clData->m_clDevice,
                                 m_clData->m_clQueue);
  ...
  float camPos[4] = {0, 0, 0,
                     0};  // ci.arraySizeX,ci.arraySizeY/2,ci.arraySizeZ,0};
  // float camPos[4]={1,12.5,1.5,0};

  m_guiHelper->getRenderInterface()->getActiveCamera()->setCameraTargetPosition(
...
int GpuConvexScene::createDynamicsObjects() {
  ...
  return
  createDynamicsObjects2(ci,barrel_vertices,numVertices,barrel_indices,numIndices);
  */

  int numVertices = sizeof(tetra_vertices) / strideInBytes;
  int numIndices = sizeof(tetra_indices) / sizeof(int);
  return createDynamicsObjects2(tetra_vertices, numVertices, tetra_indices,
                                numIndices);
}
...
int GpuBoxPlaneScene::createDynamicsObjects() {
  ...
  return createDynamicsObjects2(cube_vertices_textured, numVertices,
                                cube_indices, numIndices);
}

int GpuConvexScene::createDynamicsObjects2(const float* vertices,
  ...
  if (0) {
    ...
    if (image) {
      textureIndex =
          m_instancingRenderer->registerTexture(image, width, height);
    }
  }

  int shapeId = m_guiHelper->getRenderInterface()->registerShape(
      &vertices[0], numVertices, indices, numIndices, B3_GL_TRIANGLES,
      textureIndex);
  // int group=1;
  // int mask=1;
  int index = 0;
    ...
    for (int i = 0; i < gGpuArraySizeX; i++) {
      ...
      for (int j = 0; j < gGpuArraySizeY; j++) {
        for (int k = 0; k < gGpuArraySizeZ; k++) {
          // int colIndex =
          // m_data->m_np->registerConvexHullShape(&vertices[0],strideInBytes,numVertices,
          // scaling);
          if (!gUseInstancedCollisionShapes)
            colIndex = m_data->m_np->registerConvexHullShape(utilPtr);
          ...
          curColor &= 3;
          //					b3Vector4
          //scaling=b3MakeVector4(1,1,1,1);
          int id;
          id = m_guiHelper->getRenderInterface()->registerGraphicsInstance(
              shapeId, position, orn, color, scaling);
          int pid;
          pid = m_data->m_rigidBodyPipeline->registerPhysicsInstance(
              mass, position, orn, colIndex, index, false);

          if (prevBody >= 0) {
            // b3Point2PointConstraint* p2p = new
            // b3Point2PointConstraint(pid,prevBody,b3Vector3(0,-1.1,0),b3Vector3(0,1.1,0));
...
void GpuConvexScene::createStaticEnvironment() {
  ...
  int numIndices = sizeof(cube_indices) / sizeof(int);
  // int shapeId =
  // ci.m_instancingRenderer->registerShape(&cube_vertices[0],numVertices,cube_indices,numIndices);
...

  {
    ...
    int colIndex = m_data->m_np->registerConvexHullShape(
        &cube_vertices[0], strideInBytes, numVertices, scaling);
    ...
    int id;
    id = m_instancingRenderer->registerGraphicsInstance(shapeId, position, orn,
                                                        color, scaling);
    int pid;
    pid = m_data->m_rigidBodyPipeline->registerPhysicsInstance(
        0.f, position, orn, colIndex, index, false);
  }
}
...
ci.m_instancingRenderer->registerShape(&cube_vertices[0],numVertices,cube_indices,numIndices);
...

        {
                ...
                //int colIndex =
m_data->m_np->registerConvexHullShape(&cube_vertices[0],strideInBytes,numVertices,
scaling);
                b3Vector3 normal=b3MakeVector3(0,1,0);
                float constant=0.f;
                int colIndex =
m_data->m_np->registerPlaneShape(normal,constant);//>registerConvexHullShape(&cube_vertices[0],strideInBytes,numVertices,
scaling);
                ...
                int id =
ci.m_instancingRenderer->registerGraphicsInstance(shapeId,position,orn,color,scaling);
                int pid =
m_data->m_rigidBodyPipeline->registerPhysicsInstance(0.f,position,orn,colIndex,index,false);
...
static float mytetra_vertices[] = {
    -1.f, 0, -1.f, 0.5f, 0, 1, 0, 0, 0, -1.f, 0, 1.f,  0.5f, 0, 1, 0, 1, 0,
    1.f,  0, 1.f,  0.5f, 0, 1, 0, 1, 1, 1.f,  0, -1.f, 0.5f, 0, 1, 0, 0, 1};

static int mytetra_indices[] = {0, 1, 2, 3, 1, 2, 3, 2, 0, 3, 0, 1};

/* Create from TetGen .ele, .face, .node data */
void GpuTetraScene::createFromTetGenData(const char* ele, const char* node) {
  b3Scalar scaling(10);
  ...
  if (ele && ele[0]) {
    ...
    for (int i = 0; i < ntetra; ++i) {
      ...
      int shapeId = m_instancingRenderer->registerShape(
          &mytetra_vertices[0], numVertices, mytetra_indices, numIndices);
      //	int group=1;
      //	int mask=1;

      {
        ...
        int colIndex = m_data->m_np->registerConvexHullShape(
            &mytetra_vertices[0], strideInBytes, numVertices, scaling);
        ...
        int id;
        id = m_instancingRenderer->registerGraphicsInstance(
            shapeId, position, orn, color, scaling);
        int pid;
        pid = m_data->m_rigidBodyPipeline->registerPhysicsInstance(
            1.f, position, orn, colIndex, 0, false);
  ...
  for (int p = 0; p < allPairs.size(); p++) {
    ...
    b3Quaternion relTargetAB =
        frameInA.getRotation() * frameInB.getRotation().inverse();
    ...
    if (useGPU) {
      int cid;
      cid = m_data->m_rigidBodyPipeline->createFixedConstraint(
          bodyIndexA, bodyIndexB, pivotInA, pivotInB, relTargetAB,
          breakingThreshold);
    } else {
      b3FixedConstraint* c =
          new b3FixedConstraint(bodyIndexA, bodyIndexB, frameInA, frameInB);
...
class CommonExampleInterface* OpenCLBoxBoxCreateFunc(
