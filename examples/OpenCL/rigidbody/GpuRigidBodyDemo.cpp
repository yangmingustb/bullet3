...
static const char* s_rigidBodyKernelString = MSTRINGIFY(
    ...
    __kernel void copyTransformsToVBOKernel(__global Body* gBodies,
                                            __global float4* posOrnColor,
                                            const int numNodes) {
...
GpuRigidBodyDemo::GpuRigidBodyDemo(GUIHelperInterface* helper)
    : CommonOpenCLBase(helper), m_instancingRenderer(0), m_window(0) {
...
void GpuRigidBodyDemo::resetCamera() {
  ...
  m_data->m_guiHelper->resetCamera(dist, yaw, pitch, targetPos[0], targetPos[1],
                                   targetPos[2]);
}

GpuRigidBodyDemo::~GpuRigidBodyDemo() { delete m_data; }
...
void GpuRigidBodyDemo::initPhysics() {
  ...
  if (m_clData->m_clContext) {
    ...
    cl_program rbProg = 0;
    m_data->m_copyTransformsToVBOKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clData->m_clContext, m_clData->m_clDevice,
            s_rigidBodyKernelString, "copyTransformsToVBOKernel", &errNum,
            rbProg);

    m_data->m_config.m_maxConvexBodies =
        b3Max(m_data->m_config.m_maxConvexBodies,
              gGpuArraySizeX * gGpuArraySizeY * gGpuArraySizeZ + 10);
    ...
    m_data->m_config.m_maxBroadphasePairs =
        maxPairsPerBody * m_data->m_config.m_maxConvexBodies;
    m_data->m_config.m_maxContactCapacity =
        m_data->m_config.m_maxBroadphasePairs;

    b3GpuNarrowPhase* np =
        new b3GpuNarrowPhase(m_clData->m_clContext, m_clData->m_clDevice,
                             m_clData->m_clQueue, m_data->m_config);
    b3GpuBroadphaseInterface* bp = 0;

    if (useUniformGrid) {
      bp = new b3GpuGridBroadphase(m_clData->m_clContext, m_clData->m_clDevice,
                                   m_clData->m_clQueue);
    } else {
      bp = new b3GpuSapBroadphase(m_clData->m_clContext, m_clData->m_clDevice,
                                  m_clData->m_clQueue);
    }
    m_data->m_np = np;
    m_data->m_bp = bp;
    m_data->m_broadphaseDbvt =
        new b3DynamicBvhBroadphase(m_data->m_config.m_maxConvexBodies);

    m_data->m_rigidBodyPipeline = new b3GpuRigidBodyPipeline(
        m_clData->m_clContext, m_clData->m_clDevice, m_clData->m_clQueue, np,
        bp, m_data->m_broadphaseDbvt, m_data->m_config);
...
void GpuRigidBodyDemo::stepSimulation(float deltaTime) {
  if (!m_instancingRenderer) return;
  ...
  if (animate && numObjects) {
    ...
    if (!m_data->m_instancePosOrnColor) {
      ...
      positions = (b3Vector4*)glMapBufferRange(
          GL_ARRAY_BUFFER, m_instancingRenderer->getMaxShapeCapacity(),
          arraySizeInBytes, GL_MAP_READ_BIT);  // GL_READ_WRITE);//GL_WRITE_ONLY
      GLint err = glGetError();
      assert(err == GL_NO_ERROR);
  ...
  if (numObjects) {
    if (convertOnCpu) {
      ...
      b3LauncherCL launch(m_clData->m_clQueue,
                          m_data->m_copyTransformsToVBOKernel,
                          "m_copyTransformsToVBOKernel");
  ...
  if (animate && numObjects) {
    ...
    positions = (b3Vector4*)glMapBufferRange(
        GL_ARRAY_BUFFER, m_instancingRenderer->getMaxShapeCapacity(),
        arraySizeInBytes, GL_MAP_WRITE_BIT);  // GL_READ_WRITE);//GL_WRITE_ONLY
    err = glGetError();
    assert(err == GL_NO_ERROR);
    m_data->m_instancePosOrnColor->copyToHostPointer(positions, 3 * numObjects,
                                                     0);
    glUnmapBuffer(GL_ARRAY_BUFFER);
    err = glGetError();
    assert(err == GL_NO_ERROR);
...
b3Vector3 GpuRigidBodyDemo::getRayTo(int x, int y) {
  if (!m_instancingRenderer) return b3MakeVector3(0, 0, 0);
...
unsigned char* GpuRigidBodyDemo::loadImage(const char* fileName, int& width,
                                           int& height, int& n) {
...
bool GpuRigidBodyDemo::mouseMoveCallback(float x, float y) {
  if (!m_instancingRenderer) return false;

  if (m_data->m_altPressed != 0 || m_data->m_controlPressed != 0) return false;

  if (m_data->m_pickBody >= 0 && m_data->m_pickConstraint >= 0) {
    m_data->m_rigidBodyPipeline->removeConstraintByUid(
        m_data->m_pickConstraint);
    b3Vector3 newRayTo = getRayTo(x, y);
    b3Vector3 rayFrom;
    //	b3Vector3 oldPivotInB = m_data->m_pickPivotInB;
    b3Vector3 newPivotB;
    m_guiHelper->getRenderInterface()->getActiveCamera()->getCameraPosition(
        rayFrom);
    ...
    m_data->m_pickConstraint =
        m_data->m_rigidBodyPipeline->createPoint2PointConstraint(
            m_data->m_pickBody, m_data->m_pickFixedBody, m_data->m_pickPivotInA,
            m_data->m_pickPivotInB, 1e30);
    m_data->m_rigidBodyPipeline->writeAllInstancesToGpu();
    return true;
  }
  return false;
}
bool GpuRigidBodyDemo::mouseButtonCallback(int button, int state, float x,
                                           float y) {
  if (!m_instancingRenderer) return false;

  if (state == 1) {
    if (button == 0 &&
        (m_data->m_altPressed == 0 && m_data->m_controlPressed == 0)) {
      ...
      if (hitResults[0].m_hitFraction < 1.f) {
        ...
        if (m_data->m_np->getBodiesCpu()[hitBodyA].m_invMass) {
          ...
          // pivotInA
          b3Vector3 pivotInB;
          pivotInB.setInterpolate3(ray.m_from, ray.m_to,
          ...
          if (m_data->m_pickFixedBody < 0) {
            ...
            int fixedSphere = m_data->m_np->registerConvexHullShape(
                0, 0, 0, 0);  //>registerSphereShape(0.1);
            m_data->m_pickFixedBody =
                m_data->m_rigidBodyPipeline->registerPhysicsInstance(
                    0, pos, orn, fixedSphere, 0, false);
            m_data->m_rigidBodyPipeline->writeAllInstancesToGpu();
            ...
            if (m_data->m_pickGraphicsShapeIndex < 0) {
              ...
              m_data->m_pickGraphicsShapeIndex =
                  m_guiHelper->getRenderInterface()->registerShape(
                      &point_sphere_vertices[0], numVertices,
                      point_sphere_indices, numIndices, B3_GL_POINTS);

              float color[4] = {1, 0, 0, 1};
              float scaling[4] = {1, 1, 1, 1};

              m_data->m_pickGraphicsShapeInstance =
                  m_guiHelper->getRenderInterface()->registerGraphicsInstance(
                      m_data->m_pickGraphicsShapeIndex, pivotInB, orn, color,
                      scaling);
              m_guiHelper->getRenderInterface()->writeTransforms();
              delete m_data->m_instancePosOrnColor;
              ...
              m_guiHelper->getRenderInterface()
                  ->writeSingleInstanceTransformToCPU(
                      pivotInB, orn, m_data->m_pickGraphicsShapeInstance);
              if (this->m_instancingRenderer)
                m_instancingRenderer->writeSingleInstanceTransformToGPU(
                    pivotInB, orn, m_data->m_pickGraphicsShapeInstance);
              m_data->m_np->setObjectTransformCpu(pos, orn,
                                                  m_data->m_pickFixedBody);
            }
          }
          pivotInB.w = 0.f;
          ...
          m_data->m_pickConstraint =
              m_data->m_rigidBodyPipeline->createPoint2PointConstraint(
                  hitBodyA, m_data->m_pickFixedBody, pivotInA, pivotInB,
                  1e30);  // hitResults[0].m_hitResult0
          m_data->m_rigidBodyPipeline->writeAllInstancesToGpu();
