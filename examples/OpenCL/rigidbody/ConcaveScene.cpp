...
GLInstanceGraphicsShape* createGraphicsShapeFromWavefrontObj(
    std::vector<tinyobj::shape_t>& shapes) {
  b3AlignedObjectArray<GLInstanceVertex>* vertices =
      new b3AlignedObjectArray<GLInstanceVertex>;
  {
    ...
    for (int s = 0; s < shapes.size(); s++) {
      ...
      for (int f = 0; f < faceCount; f += 3) {
        ...
        if (1) {
          ...
          vtx0.uv[0] =
              0.5f;  // shape.mesh.positions[shape.mesh.indices[f]*3+2];?
          vtx0.uv[1] = 0.5f;

          GLInstanceVertex vtx1;
          vtx1.xyzw[0] =
              shape.mesh.positions[shape.mesh.indices[f + 1] * 3 + 0];
          vtx1.xyzw[1] =
          ...
          vtx2.xyzw[0] =
              shape.mesh.positions[shape.mesh.indices[f + 2] * 3 + 0];
          vtx2.xyzw[1] =
          ...
          b3Vector3 v0 =
              b3MakeVector3(vtx0.xyzw[0], vtx0.xyzw[1], vtx0.xyzw[2]);
          b3Vector3 v1 =
              b3MakeVector3(vtx1.xyzw[0], vtx1.xyzw[1], vtx1.xyzw[2]);
          b3Vector3 v2 =
              b3MakeVector3(vtx2.xyzw[0], vtx2.xyzw[1], vtx2.xyzw[2]);
...
void ConcaveScene::createConcaveMesh(const ConstructionInfo& ci,
                                     const char* fileName,
                                     const b3Vector3& shift,
                                     const b3Vector3& scaling) {
...

  {
    std::vector<tinyobj::shape_t> shapes;
    std::string err =
        tinyobj::LoadObj(shapes, relativeFileName, prefix[prefixIndex]);

    GLInstanceGraphicsShape* shape =
        createGraphicsShapeFromWavefrontObj(shapes);

    b3AlignedObjectArray<b3Vector3> verts;
    for (int i = 0; i < shape->m_numvertices; i++) {
      for (int j = 0; j < 3; j++) shape->m_vertices->at(i).xyzw[j] += shift[j];
    ...
    int colIndex = m_data->m_np->registerConcaveMesh(&verts, shape->m_indices,
                                                     b3MakeVector3(1, 1, 1));

    {
      ...
      int shapeId = ci.m_instancingRenderer->registerShape(
          &shape->m_vertices->at(0).xyzw[0], shape->m_numvertices,
          &shape->m_indices->at(0), shape->m_numIndices);
...

      {
        ...
        int id = ci.m_instancingRenderer->registerGraphicsInstance(
            shapeId, position, orn, color, scaling);
        int pid = m_data->m_rigidBodyPipeline->registerPhysicsInstance(
...
void ConcaveScene::setupScene(const ConstructionInfo& ci) {
  ...
  if (ci.m_gui) ci.m_gui->setStatusBarMessage(msg, true);
}
...
void ConcaveScene::createDynamicObjects(const ConstructionInfo& ci) {
  ...
  if (1) {
    int curColor = 0;
    b3Vector4 colors[4] = {
        b3MakeVector4(1, 1, 1, 1), b3MakeVector4(1, 1, 0.3, 1),
        b3MakeVector4(0.3, 1, 1, 1), b3MakeVector4(0.3, 0.3, 1, 1),
    };
    ...
    for (int i = 0; i < ci.arraySizeX; i++) {
      for (int j = 0; j < ci.arraySizeY; j++) {
        for (int k = 0; k < ci.arraySizeZ; k++) {
          ...
          // b3Vector3
          // position(-2*ci.gapX+i*ci.gapX,25+j*ci.gapY,-2*ci.gapZ+k*ci.gapZ);
          b3Vector3 position = b3MakeVector3(
              -(ci.arraySizeX / 2) * CONCAVE_GAPX + i * CONCAVE_GAPX,
              23 + j * CONCAVE_GAPY,
  ...
  if (shapes.size() > 0) {
...

    {
      ...
      //			int i=4;
      {
        ...
        int childColIndex = m_data->m_np->registerConvexHullShape(
            &tmpVertices[0].xyzw[0], strideInBytes, numVertices, scaling);
    ...
    // ci.m_instancingRenderer->registerShape(&cube_vertices[0],numVertices,cube_indices,numIndices);
    int shapeId = ci.m_instancingRenderer->registerShape(
        &vertexArray[0].xyzw[0], vertexArray.size(), &indexArray[0],
        indexArray.size());

    b3Vector4 colors[4] = {
        b3MakeVector4(1, 0, 0, 1), b3MakeVector4(0, 1, 0, 1),
        b3MakeVector4(0, 0, 1, 1), b3MakeVector4(0, 1, 1, 1),
    };
    ...
    for (int i = 0; i < 1; i++)  // ci.arraySizeX;i++)
    {
      for (int j = 0; j < 4; j++) {
        ...
        int k = 0;
        {
          ...
          b3Vector3 position = b3MakeVector3(
              i * 10 * ci.gapX, 10 + j * 10 * ci.gapY, k * 10 * ci.gapZ);
...
void ConcaveCompoundScene::createDynamicObjects(const ConstructionInfo& ci) {
...

  {
    ...
    b3Vector3 childPositions[3] = {b3MakeVector3(0, -2, 0),
                                   b3MakeVector3(0, 0, 0),
                                   b3MakeVector3(0, 0, 2)};
    ...
    for (int i = 0; i < numChildShapes; i++) {
      ...
      for (int v = 0; v < numVertices; v++) {
        ...
        b3Vector3 vertPos =
            b3MakeVector3(vert.xyzw[0], vert.xyzw[1], vert.xyzw[2]);
        b3Vector3 newPos = tr * vertPos;
        vert.xyzw[0] = newPos[0];
        vert.xyzw[1] = newPos[1];
        vert.xyzw[2] = newPos[2];
  ...
  for (int i = 0; i < ci.arraySizeX; i++) {
    for (int j = 0; j < ci.arraySizeY; j++) {
...

      {
        ...
        b3Vector3 position =
            b3MakeVector3((-ci.arraySizeX / 2 + i) * ci.gapX, 50 + j * ci.gapY,
                          (-ci.arraySizeZ / 2 + k) * ci.gapZ);
        // b3Quaternion orn(0,0,0,1);
...
void ConcaveSphereScene::createDynamicObjects(const ConstructionInfo& ci) {
  ...
  // m_data->m_np->registerConvexHullShape(&cube_vertices[0],strideInBytes,numVertices,
  // scaling);
  int colIndex = m_data->m_np->registerSphereShape(
      radius);  //>registerConvexHullShape(&cube_vertices[0],strideInBytes,numVertices,
                //scaling);
  ...
  for (int i = 0; i < ci.arraySizeX; i++) {
    for (int j = 0; j < ci.arraySizeY; j++) {
      for (int k = 0; k < ci.arraySizeZ; k++) {
        ...
        b3Vector3 position =
            b3MakeVector3(-(ci.arraySizeX / 2) * 8 + i * 8, 50 + j * 8,
                          -(ci.arraySizeZ / 2) * 8 + k * 8);

        // b3Vector3
        // position(0,-41,0);//0,0,0);//i*radius*3,-41+j*radius*3,k*radius*3);
        ...
        int id = ci.m_instancingRenderer->registerGraphicsInstance(
            prevGraphicsShapeIndex, position, orn, color, scaling);
        int pid = m_data->m_rigidBodyPipeline->registerPhysicsInstance(
            mass, position, orn, colIndex, index, false);
