...
enum eTerrainModel {
  ...
  eImageFile =
      3,  // terrain from png/jpg files, asset from
...
static btVector3 getUpVector(int upAxis, btScalar regularValue,
                             btScalar upValue) {
...
static btScalar convertToFloat(const byte_t* p, PHY_ScalarType type) {
  btAssert(p);
...
static btScalar getGridHeight(byte_t* grid, int i, int j, PHY_ScalarType type) {
  btAssert(grid);
...
static void convertFromFloat(byte_t* p, btScalar value, PHY_ScalarType type) {
  btAssert(p && "null");
...
static void setRadial(byte_t* grid, int bytesPerElement, PHY_ScalarType type,
                      btScalar phase = 0.0) {
  btAssert(grid);
...
static float randomHeight(int step) {
  return (0.33 * s_gridSpacing * s_gridSize * step *
          (rand() - (0.5 * RAND_MAX))) /
         (1.0 * RAND_MAX * s_gridSize);
}
...
static void setFractal(byte_t* grid, int bytesPerElement, PHY_ScalarType type,
                       int step) {
  btAssert(grid);
  btAssert(bytesPerElement > 0);
  btAssert(step > 0);
  ...
  if (s_gridSize - 1 == step) {
    ...
    convertFromFloat(grid + (step * s_gridSize + step) * bytesPerElement, base,
                     type);
  }
  ...
  btScalar c10 =
      convertToFloat(grid + (step * s_gridSize) * bytesPerElement, type);
  btScalar c11 =
      convertToFloat(grid + (step * s_gridSize + step) * bytesPerElement, type);

  // set top middle
  updateHeight(grid + newStep * bytesPerElement,
               0.5 * (c00 + c01) + randomHeight(step), type);

  // set left middle
  updateHeight(grid + (newStep * s_gridSize) * bytesPerElement,
               0.5 * (c00 + c10) + randomHeight(step), type);

  // set right middle
  updateHeight(grid + (newStep * s_gridSize + step) * bytesPerElement,
               0.5 * (c01 + c11) + randomHeight(step), type);

  // set bottom middle
  updateHeight(grid + (step * s_gridSize + newStep) * bytesPerElement,
               0.5 * (c10 + c11) + randomHeight(step), type);

  // set middle
  updateHeight(grid + (newStep * s_gridSize + newStep) * bytesPerElement,
               0.25 * (c00 + c01 + c10 + c11) + randomHeight(step), type);
  ...
  setFractal(grid + (newStep * s_gridSize) * bytesPerElement, bytesPerElement,
             type, newStep);
  setFractal(grid + ((newStep * s_gridSize) + newStep) * bytesPerElement,
             bytesPerElement, type, newStep);
}

#define MYLINELENGTH 16 * 32768

static byte_t* getRawHeightfieldData(eTerrainModel model, PHY_ScalarType type,
                                     btScalar& minHeight, btScalar& maxHeight) {
  if (model == eImageFile) {
    b3BulletDefaultFileIO fileIO;
    char relativeFileName[1024];
    int found =
        fileIO.findFile("heightmaps/wm_height_out.png", relativeFileName, 1024);
...
void setHeightfieldTypeComboBoxCallback(int combobox, const char* item,
                                        void* userPointer) {
...
HeightfieldExample::~HeightfieldExample(void) { clearWorld(); }
...
class btRaycastBar3 {
 public:
  ...
  btRaycastBar3(btScalar ray_length, btScalar z, btScalar max_y,
                struct GUIHelperInterface* guiHelper, int upAxisIndex) {
  ...
  struct CastRaysLoopBody : public btIParallelForBody {
    ...
    CastRaysLoopBody(btCollisionWorld* cw, btRaycastBar3* rb)
...
void HeightfieldExample::stepSimulation(float deltaTime) {
  ...
  if (m_rawHeightfieldData && m_isDynamic && eRadial == m_model &&
    ...
    if (gfxVertices.size() && indices.size()) {
      m_guiHelper->getRenderInterface()->updateShape(
          m_heightfieldShape->getUserIndex(), &gfxVertices[0].xyzw[0]);
    }
  }
...
void HeightfieldExample::resetPhysics(void) {
  ...
  m_heightfieldShape = new btHeightfieldTerrainShape(
      s_gridSize, s_gridSize, m_rawHeightfieldData, s_gridHeightScale,
  ...
  if (m_model == eImageFile) {
    b3BulletDefaultFileIO fileIO;
    char relativeFileName[1024];
    int found = fileIO.findFile("heightmaps/gimp_overlay_out.png",
  ...
  if (m_upAxis == 2) m_heightfieldShape->setFlipTriangleWinding(true);
...
CommonExampleInterface* HeightfieldExampleCreateFunc(
