...
struct b3BulletDefaultFileIO : public CommonFileIOInterface {
  static bool FileIOPluginFindFile(void* userPtr, const char* orgFileName,
                                   char* relativeFileName,
                                   int maxRelativeFileNameMaxLen) {
    b3BulletDefaultFileIO* fileIo = (b3BulletDefaultFileIO*)userPtr;
    return fileIo->findFile(orgFileName, relativeFileName,
                            maxRelativeFileNameMaxLen);
  }
  ...
  b3BulletDefaultFileIO(int fileIOType = 0, const char* pathPrefix = 0)
      : CommonFileIOInterface(fileIOType, m_prefix), m_numFileHandles(0) {
    m_prefix[0] = 0;
    if (pathPrefix) {
  ...
  virtual bool findResourcePath(const char* fileName, char* relativeFileName,
                                int relativeFileNameSizeInBytes) {
    return b3ResourcePath::findResourcePath(
               fileName, relativeFileName, relativeFileNameSizeInBytes,
               b3BulletDefaultFileIO::FileIOPluginFindFile, this) > 0;
  }

  virtual bool findFile(const char* orgFileName, char* relativeFileName,
                        int maxRelativeFileNameMaxLen) {
    FILE* f = 0;
    ...
    // printf("Trying various directories, relative to current working
    // directory\n");
    const char* prefix[] = {m_prefix,           "./",          "./data/",
                            "../data/",         "../../data/", "../../../data/",
                            "../../../../data/"};
...
#ifdef _MSC_VER
      sprintf_s(relativeFileName, maxRelativeFileNameMaxLen, "%s%s", prefix[i],
                orgFileName);
#else
  ...
  virtual int getFileSize(int fileHandle) {
    ...
    if (fileHandle >= 0 && fileHandle < B3_FILEIO_MAX_FILES) {
      FILE* f = m_fileHandles[fileHandle];
      if (f) {
        if (fseek(f, 0, SEEK_END) || (size = ftell(f)) == EOF ||
            fseek(f, 0, SEEK_SET)) {
