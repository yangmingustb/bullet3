...
struct btTimings {
  btTimings() : m_numTimings(0), m_activeBuffer(0) {}
  void flush() {
    for (int i = 0; i < m_numTimings; i++) {
      ...
      unsigned long long int startTime =
          m_timings[m_activeBuffer][i].m_usStartTime;
...
void MyEnterProfileZoneFunc(const char* msg) {
  ...
  if (threadId < 0 || threadId >= BT_QUICKPROF_MAX_THREAD_COUNT) return;
...
void b3ChromeUtilsEnableProfiling() { gProfileDisabled = false; }
