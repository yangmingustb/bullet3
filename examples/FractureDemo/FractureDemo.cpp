...
/// It assumes a btCompoundShaps (where the childshapes are the pre-fractured
/// pieces)
/// The btFractureBody is a class derived from btRigidBody, dealing with the
/// collision impacts.
 ...
 public:
  ...
  virtual void stepSimulation(float deltaTime) {
...

    {
      ...
      // right now: brute force remove all graphics objects, and re-create them
      // every frame
...
void FractureDemo::initPhysics() {
  ...
  btFractureDynamicsWorld* fractureWorld = new btFractureDynamicsWorld(
      m_dispatcher, m_broadphase, m_solver, m_collisionConfiguration);
  m_dynamicsWorld = fractureWorld;
  m_guiHelper->createPhysicsDebugDrawer(m_dynamicsWorld);

  // m_splitImpulse removes the penetration resolution from the applied impulse,
  // otherwise objects might fracture due to deep penetrations.
...

  {
    ...
    btCollisionShape* colShape =
        new btBoxShape(btVector3(SCALING * 1, SCALING * 1, SCALING * 1));
...
class CommonExampleInterface* FractureDemoCreateFunc(
