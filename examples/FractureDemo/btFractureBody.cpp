      ...
      for (int j = i + 1; j < compound->getNumChildShapes(); j++) {
        struct MyContactResultCallback
          ...
          virtual btScalar addSingleResult(
              ...
              int index1) {
            if (cp.getDistance() <= m_margin) m_connected = true;
            return 1.f;
          }
        };
...
btCompoundShape* btFractureBody::shiftTransformDistributeMass(
    btCompoundShape* boxCompound, btScalar mass, btTransform& shift) {
...
btCompoundShape* btFractureBody::shiftTransform(btCompoundShape* boxCompound,
                                                btScalar* masses,
                                                btTransform& shift,
                                                btVector3& principalInertia) {
  btTransform principal;

  boxCompound->calculatePrincipalAxisTransform(masses, principal,
                                               principalInertia);

/// create a new compound with world transform/center of mass properly aligned
...
// rbInfo(mass,myMotionState,newCompound,principalInertia);
  ...
  for (int i = 0; i < boxCompound->getNumChildShapes(); i++) {
    ...
    /// updateChildTransform is really slow, because it re-calculates the AABB
    /// each time. todo: add option to disable this update
    boxCompound->updateChildTransform(i, newChildTransform);
  }
  bool isDynamic = (mass != 0.f);
  btVector3 localInertia(0, 0, 0);
  if (isDynamic) boxCompound->calculateLocalInertia(mass, localInertia);
  ...
  for (int i = 0; i < boxCompound->getNumChildShapes(); i++) {
    ...
    /// updateChildTransform is really slow, because it re-calculates the AABB
    /// each time. todo: add option to disable this update
    newBoxCompound->addChildShape(newChildTransform,
                                  boxCompound->getChildShape(i));
  }
