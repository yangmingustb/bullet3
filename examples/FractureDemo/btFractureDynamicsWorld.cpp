...
// Set the correct object offset in Collision Object Array
  ...
  for (startIslandIndex = 0; startIslandIndex < numElem;
    ...
    for (endIslandIndex = startIslandIndex + 1;
         ...
         (unionFind.getElement(endIslandIndex).m_id == islandId);
         endIslandIndex++) {
    }
    ...
    for (idx = startIslandIndex; idx < endIslandIndex; idx++) {
      ...
      if (!otherObject || !otherObject->getInvMass()) continue;
      numObjects++;
    }

    /// Then for each island that contains at least two objects and one fracture
    /// object
    if (fractureObjectIndex >= 0 && numObjects > 1) {
      btFractureBody* fracObj =
          (btFractureBody*)getCollisionObjectArray()[fractureObjectIndex];

      /// glueing objects means creating a new compound and removing the old
      /// objects
      ...
      oldImpulses.push_back(fracObj->getLinearVelocity() / 1. /
                            fracObj->getInvMass());
      ...
      if (fracObj->getCollisionShape()->isCompound()) {
        ...
        btCompoundShape* oldCompound =
            (btCompoundShape*)fracObj->getCollisionShape();
        for (int c = 0; c < oldCompound->getNumChildShapes(); c++) {
          compound->addChildShape(oldCompound->getChildTransform(c),
                                  oldCompound->getChildShape(c));
      ...
      for (idx = startIslandIndex; idx < endIslandIndex; idx++) {
        ...
        if (i == fractureObjectIndex) continue;

        btCollisionObject* otherCollider = getCollisionObjectArray()[i];

        btRigidBody* otherObject = btRigidBody::upcast(otherCollider);
        // don't glue/merge with static objects right now, otherwise everything
        // gets stuck to the ground
        /// todo: expose this as a callback
        if (!otherObject || !otherObject->getInvMass()) continue;

        oldImpulses.push_back(otherObject->getLinearVelocity() *
                              (1.f / otherObject->getInvMass()));
      ...
      btTransform shift;
      shift.setIdentity();
      btCompoundShape* newCompound =
          btFractureBody::shiftTransformDistributeMass(compound, totalMass,
                                                       shift);
      ...
      btFractureBody* newBody =
          new btFractureBody(totalMass, 0, newCompound, localInertia,
                             &massArray[0], numChildren, this);
      ...
      for (int i = 0; i < oldImpulses.size(); i++) {
        btVector3 rel_pos =
            oldCenterOfMassesWS[i] - newBody->getCenterOfMassPosition();
  ...
  // otherwise the island tags would not match the world collision object array
  // indices anymore
  while (removedObjects.size()) {
    btCollisionObject* otherCollider =
        removedObjects[removedObjects.size() - 1];
...
void btFractureDynamicsWorld::solveConstraints(
...
btFractureBody* btFractureDynamicsWorld::addNewBody(
    const btTransform& oldTransform, btScalar* masses,
    btCompoundShape* oldCompound) {
  int i;
  ...
  btVector3 localInertia;
  btCompoundShape* newCompound =
      btFractureBody::shiftTransform(oldCompound, masses, shift, localInertia);
  btScalar totalMass = 0;
  for (i = 0; i < newCompound->getNumChildShapes(); i++) totalMass += masses[i];
  // newCompound->calculateLocalInertia(totalMass,localInertia);

  btFractureBody* newBody =
      new btFractureBody(totalMass, 0, newCompound, localInertia, masses,
                         newCompound->getNumChildShapes(), this);
  newBody->recomputeConnectivity(this);
...
void btFractureDynamicsWorld::breakDisconnectedParts(btFractureBody* fracObj) {
  if (!fracObj->getCollisionShape()->isCompound()) return;
...
// Set the correct object offset in Collision Object Array
  ...
  for (startIslandIndex = 0; startIslandIndex < numElem;
    ...
    for (idx = startIslandIndex; idx < endIslandIndex; idx++) {
      ...
      newCompound->addChildShape(compound->getChildTransform(i),
...
void btFractureDynamicsWorld::fractureCallback() {
  ...
  for (int i = 0; i < numManifolds; i++) {
    ...
    // some threshold otherwise resting contact would break objects after a
    // while
    if (totalImpact < 40.f) continue;

    //		printf("strong impact\n");

    //@todo: add better logic to decide what parts to fracture
    // For example use the idea from the SIGGRAPH talk about the fracture in the
    // movie 2012:
    //
    // Breaking thresholds can be stored as connectivity information between
    // child shapes in the fracture object
    //
    // You can calculate some "impact value" by simulating all the individual
    // child shapes
    // as rigid bodies, without constraints, running it in a separate simulation
    // world
    //(or by running the constraint solver without actually modifying the
    //dynamics world)
    // Then measure some "impact value" using the offset and applied impulse for
    // each child shape
    // weaken the connections based on this "impact value" and only break
    // if this impact value exceeds the breaking threshold.
    // you can propagate the weakening and breaking of connections using the
    ...
    if (f0 == f1 == m_fractureBodies.size()) continue;
    ...
    if (f0 < m_fractureBodies.size()) {
      ...
      if (pi < 0) {
        ...
        btAssert(sFracturePairs[pi].m_contactManifolds.findLinearSearch(
                     manifold) == sFracturePairs[pi].m_contactManifolds.size());
    ...
    if (f1 < m_fractureBodies.size()) {
      int j = f1;
      {
        ...
        if (pi < 0) {
          ...
          btAssert(sFracturePairs[pi].m_contactManifolds.findLinearSearch(
                       manifold) ==
                   sFracturePairs[pi].m_contactManifolds.size());
          sFracturePairs[pi].m_contactManifolds.push_back(manifold);
        }
      }
    }
...

  {
    ...
    for (int i = 0; i < sFracturePairs.size(); i++) {
      ...
      if (sFracturePairs[i].m_fracObj->getCollisionShape()->isCompound()) {
        ...
        tr.setIdentity();
        btCompoundShape* oldCompound =
            (btCompoundShape*)sFracturePairs[i].m_fracObj->getCollisionShape();
        if (oldCompound->getNumChildShapes() > 1) {
          ...
          for (int j = 0; j < sFracturePairs[i].m_contactManifolds.size();
               j++) {
            btPersistentManifold* manifold =
            ...
            for (int k = 0; k < manifold->getNumContacts(); k++) {
              ...
              if (manifold->getBody0() == sFracturePairs[i].m_fracObj) {
                for (int f = 0;
                     f < sFracturePairs[i].m_fracObj->m_connections.size();
                     f++) {
                  btConnection& connection =
                      sFracturePairs[i].m_fracObj->m_connections[f];
