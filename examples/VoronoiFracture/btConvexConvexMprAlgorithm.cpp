...
btVector3 btBulletShapeSupportFunc(const void* shapeAptr, const btVector3& dir,
                                   bool includeMargin) {
...
//
void btConvexConvexMprAlgorithm::processCollision(
    const btCollisionObjectWrapper* body0Wrap,
    const btCollisionObjectWrapper* body1Wrap,
  ...
  if (gUseMprCollisionFunction) {
    ...
    if (res == 0) {
      ...
      if (distInfo.m_distance <= 0) {
        resultOut->addContactPoint(distInfo.m_normalBtoA, distInfo.m_pointOnB,
                                   distInfo.m_distance);
      }
      // ASSERT_EQ(0,result);
      ...
      // btVector3 computedA =
      // distInfo.m_pointOnB+distInfo.m_distance*distInfo.m_normalBtoA;
    ...
    btGjkPairDetector gjkPairDetector(min0, min1, &vs,
                                      &epa);  // m_simplexSolver,m_pdSolver);
...
btScalar btConvexConvexMprAlgorithm::calculateTimeOfImpact(
