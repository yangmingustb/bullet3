...
Voronoi fracture and shatter code and demo copyright (c) 2011 Alain Ducharme
  - Reset scene (press spacebar) to generate new random voronoi shattered
cuboids
  - Check console for total time required to: compute and mesh all 3D shards,
calculate volumes and centers of mass and create rigid bodies
  - Modify VORONOIPOINTS define below to change number of potential voronoi
shards
  - Note that demo's visual cracks between voronoi shards are NOT present in the
internally generated voronoi mesh!
*/
...
class VoronoiFractureDemo : public CommonRigidBodyBase {
 ...
 public:
  ...
  virtual ~VoronoiFractureDemo() { btAssert(m_dynamicsWorld == 0); }
  ...
  void voronoiBBShatter(const btAlignedObjectArray<btVector3>& points,
                        const btVector3& bbmin, const btVector3& bbmax,
                        const btQuaternion& bbq, const btVector3& bbt,
                        btScalar matDensity);
  void voronoiConvexHullShatter(const btAlignedObjectArray<btVector3>& points,
        ...
        if (body0->checkCollideWithOverride(body1)) {
          {
            ...
            btVector3 contactPosWorld =
                manifold->getContactPoint(minIndex).m_positionWorldOnA;
...
void VoronoiFractureDemo::getVerticesInsidePlanes(
    const btAlignedObjectArray<btVector3>& planes,
    btAlignedObjectArray<btVector3>& verticesOut,
    std::set<int>& planeIndicesOut) {
...
struct pointCmp {
  bool operator()(const btVector3& p1, const btVector3& p2) const {
    ...
    bool result0 = v1 < v2;
    // bool result1 = ((btScalar)(p1-curVoronoiPoint).length2()) <
    // ((btScalar)(p2-curVoronoiPoint).length2());
    // apparently result0 is not always result1, because extended precision used
    // in registered is different from precision when values are stored in
    // memory
    return result0;
  }
};

void VoronoiFractureDemo::voronoiBBShatter(
    const btAlignedObjectArray<btVector3>& points, const btVector3& bbmin,
    const btVector3& bbmax, const btQuaternion& bbq, const btVector3& bbt,
    btScalar matDensity) {
  ...
  for (i = 0; i < numpoints; i++) {
    ...
    for (j = 1; j < numpoints; j++) {
      ...
      if (numplaneIndices != planes.size()) {
        ...
        for (k = 0; k < numplaneIndices; k++) {
          if (k != *planeIndicesIter) planes[k] = planes[*planeIndicesIter];
          planeIndicesIter++;
        }
    ...
    convexHC->compute(&vertices[0].getX(), sizeof(btVector3), vertices.size(),
                      CONVEX_MARGIN, 0.0);

    // At this point we have a complete 3D voronoi shard mesh contained in
    // convexHC
    ...
    for (j = 0; j < numFaces; j++) {
      const btConvexHullComputer::Edge* edge =
          &convexHC->edges[convexHC->faces[j]];
      v0 = edge->getSourceVertex();
      v1 = edge->getTargetVertex();
      edge = edge->getNextEdgeOfFace();
      v2 = edge->getTargetVertex();
      while (v2 != v0) {
        // Counter-clockwise triangulated voronoi shard mesh faces (v0-v1-v2)
        // and edges here...
        btScalar vol = convexHC->vertices[v0].triple(convexHC->vertices[v1],
                                                     convexHC->vertices[v2]);
        volume += vol;
        com += vol * (convexHC->vertices[v0] + convexHC->vertices[v1] +
                      convexHC->vertices[v2]);
        edge = edge->getNextEdgeOfFace();
        v1 = v2;
        v2 = edge->getTargetVertex();
    ...
    // Note:
    // At this point convex hulls contained in convexHC should be accurate (line
    // up flush with other pieces, no cracks),
    // ...however Bullet Physics rigid bodies demo visualizations appear to
    // produce some visible cracks.
    // Use the mesh in convexHC for visual display or to perform boolean
    // operations with.

    // Create Bullet Physics rigid body shards
    btCollisionShape* shardShape = new btConvexHullShape(
        &(convexHC->vertices[0].getX()), convexHC->vertices.size());
    shardShape->setMargin(CONVEX_MARGIN);  // for this demo; note convexHC has
                                           // optional margin parameter for this
    m_collisionShapes.push_back(shardShape);
    btTransform shardTransform;
    shardTransform.setIdentity();
    shardTransform.setOrigin(curVoronoiPoint +
                             com);  // Shard's adjusted location
    btDefaultMotionState* shardMotionState =
        new btDefaultMotionState(shardTransform);
    btScalar shardMass(volume * matDensity);
    btVector3 shardInertia(0., 0., 0.);
    shardShape->calculateLocalInertia(shardMass, shardInertia);
    btRigidBody::btRigidBodyConstructionInfo shardRBInfo(
        shardMass, shardMotionState, shardShape, shardInertia);
...
void VoronoiFractureDemo::voronoiConvexHullShatter(
    const btAlignedObjectArray<btVector3>& points,
    const btAlignedObjectArray<btVector3>& verts, const btQuaternion& bbq,
    const btVector3& bbt, btScalar matDensity) {
  // points define voronoi cells in world space (avoid duplicates)
  ...
  // Using convexHullComputer faster than getPlaneEquationsFromVertices for
  // large meshes...
  ...
  for (i = 0; i < numFaces; i++) {
    const btConvexHullComputer::Edge* edge =
        &convexHC->edges[convexHC->faces[i]];
    v0 = edge->getSourceVertex();
    v1 = edge->getTargetVertex();
    edge = edge->getNextEdgeOfFace();
    v2 = edge->getTargetVertex();
    plane = (convexHC->vertices[v1] - convexHC->vertices[v0])
                .cross(convexHC->vertices[v2] - convexHC->vertices[v0])
                .normalize();
...
void VoronoiFractureDemo::initPhysics() {
  ...
  if (useMpr) {
    ...
    btConvexConvexMprAlgorithm::CreateFunc* cf =
        new btConvexConvexMprAlgorithm::CreateFunc;
    m_dispatcher->registerCollisionCreateFunc(CONVEX_HULL_SHAPE_PROXYTYPE,
                                              CONVEX_HULL_SHAPE_PROXYTYPE, cf);
    m_dispatcher->registerCollisionCreateFunc(CONVEX_HULL_SHAPE_PROXYTYPE,
                                              BOX_SHAPE_PROXYTYPE, cf);
  ...
  //btStaticPlaneShape(btVector3(0,1,0),50);
  ...
  // We can also use DemoApplication::localCreateRigidBody, but for clarity it
  // is provided here:
  {
    btScalar mass(0.);
  ...
  {
    btCollisionShape* groundShape =
        new btBoxShape(btVector3(btScalar(10.), btScalar(8.), btScalar(1.)));
    btScalar mass(0.);
  ...
  btVector3 bbmax(btScalar(rand() / btScalar(RAND_MAX)) * 12. + 0.5,
                  btScalar(rand() / btScalar(RAND_MAX)) * 1. + 0.5,
                  btScalar(rand() / btScalar(RAND_MAX)) * 1. + 0.5);
  btVector3 bbmin = -bbmax;
  // Place it 10 units above ground
  btVector3 bbt(0, 15, 0);
  // Use an arbitrary material density for shards (should be consitent/relative
  // with/to rest of RBs in world)
  btScalar matDensity = 1;
  // Using random rotation
  btQuaternion bbq(btScalar(rand() / btScalar(RAND_MAX)) * 2. - 1.,
                   btScalar(rand() / btScalar(RAND_MAX)) * 2. - 1.,
                   btScalar(rand() / btScalar(RAND_MAX)) * 2. - 1.,
                   btScalar(rand() / btScalar(RAND_MAX)) * 2. - 1.);
  bbq.normalize();
  ...
  for (int i = 0; i < VORONOIPOINTS; i++) {
    ...
    point =
        quatRotate(bbq,
                   btVector3(btScalar(rand() / btScalar(RAND_MAX)) * diff.x() -
                                 diff.x() / 2.,
                             btScalar(rand() / btScalar(RAND_MAX)) * diff.y() -
                                 diff.y() / 2.,
                             btScalar(rand() / btScalar(RAND_MAX)) * diff.z() -
                                 diff.z() / 2.)) +
        bbt;
...
CommonExampleInterface* VoronoiFractureCreateFunc(
