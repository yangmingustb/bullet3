...
struct CommonExampleOptions {
  ...
  // Those are optional, some examples will use them others don't. Each example
  // should work with them being 0.
  int m_option;
...
class CommonExampleInterface {
 public:
  typedef class CommonExampleInterface*(CreateFunc)(
  ...
  virtual void physicsDebugDraw(
      int debugFlags) = 0;  // for now we reuse the flags in
                            // Bullet/src/LinearMath/btIDebugDraw.h
  // reset camera is only called when switching demo. this way you can restart
  // (initPhysics) and watch in a specific location easier
  ...
  virtual void vrControllerMoveCallback(int controllerId, float pos[4],
                                        float orientation[4], float analogAxis,
