...
float clamp(float a, const float minval, const float maxval) {
...
void BoxLimits(const float3 *verts, int verts_count, float3 &bmin,
               float3 &bmax) {
...
void BoxLimits(const float4 *verts, int verts_count, float3 &bmin,
               float3 &bmax) {
  bmin = float3(FLT_MAX, FLT_MAX, FLT_MAX);
  bmax = float3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
  for (int i = 0; i < verts_count; i++) {
...
int overlap(const float3 &bmina, const float3 &bmaxa, const float3 &bminb,
            const float3 &bmaxb) {
  for (int j = 0; j < 3; j++) {
...
float3x3 outerprod(const float3 &a, const float3 &b) {
  return float3x3(a.x * b, a.y * b,
                  a.z * b);  // a is a column vector b is a row vector
}
...
float3 PlaneLineIntersection(const float3 &normal, const float dist,
...
float DistanceBetweenLines(const float3 &ustart, const float3 &udir,
  ...
  if (upoint) {
    ...
    *upoint = PlaneLineIntersection(normal, -dot(normal, vstart), ustart,
                                    ustart + udir);
  }
  if (vpoint) {
    float3 normal = normalize(cross(udir, cp));
    *vpoint = PlaneLineIntersection(normal, -dot(normal, ustart), vstart,
                                    vstart + vdir);
  }
  return dist;
}
...
int countpolyhit = 0;
int HitCheckPoly(const float3 *vert, const int n, const float3 &v0,
                 const float3 &v1, float3 *impact, float3 *normal) {
  countpolyhit++;
  int i;
  float3 nrml(0, 0, 0);
  for (i = 0; i < n; i++) {
...
int SolveQuadratic(float a, float b, float c, float *ta,
                   float *tb)  // if true returns roots ta,tb where ta<=tb
{
  assert(ta);
  assert(tb);
...
int HitCheckRaySphere(const float3 &sphereposition, float radius,
                      const float3 &_v0, const float3 &_v1, float3 *impact,
                      float3 *normal) {
  assert(impact);
  assert(normal);
  float3 dv = _v1 - _v0;
  float3 v0 = _v0 - sphereposition;  // solve in coord system of the sphere
  if (radius <= 0.0f || _v0 == _v1)
    return 0;  // only true if point moves from outside to inside sphere.
  float a = dot(dv, dv);
...
int HitCheckRayCylinder(const float3 &p0, const float3 &p1, float radius,
                        const float3 &_v0, const float3 &_v1, float3 *impact,
                        float3 *normal) {
  assert(impact);
  assert(normal);
  ...
  if (v0.z < 0.0f)
    v0 = PlaneLineIntersection(float3(0, 0, 1.0f), 0, v0,
                               v1);  // crop to cylinder range
  ...
  if (ta >= 0.0f && ta <= 1.0f && (ta <= tb || tb <= 0.0f)) {
    ...
    *normal = (float3(v0.x, v0.y, 0.0f) + float3(dv.x, dv.y, 0) * ta) / radius *
              Transpose(m);
    return 1;
  }
  if (tb >= 0.0f && tb <= 1.0f) {
    assert(tb <= ta || ta <= 0.0f);  // tb must be better than ta
    *impact = (v0 + dv * tb) * Transpose(m) +
              p0;  // compute intersection in original space
    *normal = (float3(v0.x, v0.y, 0.0f) + float3(dv.x, dv.y, 0) * tb) / radius *
              Transpose(m);
    return 1;
  }
  return 0;
...
int HitCheckSweptSphereTri(const float3 &p0, const float3 &p1, const float3 &p2,
                           float radius, const float3 &v0, const float3 &_v1,
                           float3 *impact, float3 *normal) {
  float3 unused;
  if (!normal) normal = &unused;
  float3 v1 =
      _v1;  // so we can update v1 after each sub intersection test if necessary
  int hit = 0;
  float3 cp = cross(p1 - p0, p2 - p0);
  if (dot(cp, v1 - v0) >= 0.0f)
    return 0;  // coming from behind and/or moving away
...
float3 PlanesIntersection(const Plane *planes, int planes_count,
                          const float3 &seed) {
  int i;
...
Plane Transform(const Plane &p, const float3 &translation,
...
int Clip(const float3 &plane_normal, float plane_dist, const float3 *verts_in,
         int count_in, float3 *verts_out) {
  // clips a polygon specified by the non-indexed vertex list verts_in.
  // verts_out must be preallocated with a size >= count+1
  assert(verts_out);
  int n = 0;
  int prev_status =
      (dot(plane_normal, verts_in[count_in - 1]) + plane_dist > 0);
  for (int i = 0; i < count_in; i++) {
    int status = (dot(plane_normal, verts_in[i]) + plane_dist > 0);
    if (status != prev_status) {
      verts_out[n++] = PlaneLineIntersection(
          plane_normal, plane_dist, verts_in[(i == 0) ? count_in - 1 : i - 1],
          verts_in[i]);
    }
    if (status == 0)  // under
    {
  ...
  assert(n <=
         count_in +
             1);  // remove if intention to use this routine on convex polygons
  return n;
}

int ClipPolyPoly(const float3 &normal, const float3 *clipper, int clipper_count,
                 const float3 *verts_in, int in_count, float3 *scratch) {
  ...
  // if the arrays are the same &verts_in==&scratch the routine should still
  // work anyways.
  ...
  for (i = 0; i < clipper_count; i++) {
    ...
    n = normalize(n);
    count = Clip(n, -dot(clipper[i], n),
                 (i == 0) ? verts_in : (i % 2) ? scratch : scratch + bsize,
                 count, (i % 2) ? scratch + bsize : scratch);
    assert(count < bsize);
  }
  if (clipper_count % 2)
    memcpy(scratch, scratch + bsize, count * sizeof(float3));
  return count;
}
...
float3x3 Inertia(const float3 *vertices, const int3 *tris, const int count,
  ...
  // defaults to [0,0,0] if unsupplied
  // assume mass==1.0  you can multiply by mass later.
  // for improved accuracy the next 3 variables, the determinant d, and its
  // calculation should be changed to double
  ...
  for (int i = 0; i < count; i++)  // for each triangle
  {
    ...
    float d = Determinant(A);  // vol of tiny parallelapiped= d * dr * ds * dt
                               // (the 3 partials of my tetral triple integral
                               // equasion)
  ...
  return float3x3(diag.y + diag.z, -offd.z, -offd.y, -offd.z, diag.x + diag.z,
                  -offd.x, -offd.y, -offd.x, diag.x + diag.y);
}

float3x3 ShapeInertiaContrib(const float3 &cor, const float3 &position,
                             const Quaternion &orientation,
                             const float3 &shape_com,
                             const float3x3 &shape_inertia, float shape_mass) {
  // transforms 3x3 inertia tensor from local reference frame to a more global
  // one.
  // essentially returns the contribution of a subshape to the inertia of a
  // larger rigid body
  // typical usage:
  ...
  // Another application is when an object is attached to something (nail-joint)
  // that is static, in which
  // one easy way to implement this is to lock the center of rotation and adjust
  // the inertia accordingly.
  // position & orientation - is the current pose or transform of the shape.
  // Obviously position, orientation and cor are all described wrt the same
  // reference frame.
  // shape_com and shape_inertia are the center of mass and the inertia of the
  // shape in the local coordinate system of that shape.
  // To clarify, if a shape happened to be located somewhere else then position
  // or orientation would be different, but
  ...
  return Transpose(R) * shape_inertia * R +
         (Identity * dot(r, r) - outerprod(r, r)) * shape_mass;
}
...
Quaternion Diagonalizer(const float3x3 &A) {
  ...
  // As per 'row' convention if float3x3 Q = q.getmatrix(); then v*Q =
  // q*v*conj(q)
  ...
  for (i = 0; i < maxsteps; i++) {
    ...
    float t =
        sgn / (thet + ((thet < 1.E6f) ? sqrtf(squared(thet) + 1.0f)
    ...
    jr[k] *= -1.0f;   // since our quat-to-matrix convention was for v*M instead
                      // of M*v
...
float3 Diagonal(const float3x3 &M) { return float3(M[0][0], M[1][1], M[2][2]); }
