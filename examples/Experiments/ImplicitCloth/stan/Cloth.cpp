...
//-----------------------------------------------------------------------------------------------
//
// The remainder of this file shows how to use the spring network class with
...
Cloth::Cloth(const char *_name, int _n)
    : SpringNetwork(_n), color(0, 0.5f, 1.0f) {
  cloths.Add(this);
}
Cloth::~Cloth() { cloths.Remove(this); }
...
float cloth_showvert =
    0.025f;  // size of box to put around current vert selected, 0 turns off
...
Cloth *ClothCreate(int w, int h, float size) {
  ...
  for (i = 0; i < h; i++)
    for (j = 0; j < w; j++) {
      cloth->X[i * w + j] =
          (float3(-0.5f, -0.5f, 0) +
           float3((float)j / (w - 1.0f), 1.0f - (float)i / (h - 1.0f), 0)) *
          size;
    }
  for (i = 0; i < h; i++)
    for (j = 0; j < w; j++) {
      if (i < h - 1)
        cloth->CreateSpring(SPRING_STRUCT, i * w + j,
                            (i + 1) * w + j);  // structural
      if (j < w - 1)
        cloth->CreateSpring(SPRING_STRUCT, i * w + j,
                            i * w + (j + 1));  // structural
      if (j < w - 1 && i < h - 1)
        cloth->CreateSpring(SPRING_SHEAR, i * w + j,
                            (i + 1) * w + (j + 1));  // shear
      if (j > 0 && i < h - 1)
        cloth->CreateSpring(SPRING_SHEAR, i * w + j,
                            (i + 1) * w + (j - 1));  // shear
      if (i < h - 2)
        cloth->CreateSpring(SPRING_BEND, i * w + j,
                            (i + 2) * w + j);  // benders
      if (j < w - 2)
        cloth->CreateSpring(SPRING_BEND, i * w + j,
                            i * w + (j + 2));  // benders
...
static void ClothDrawSprings(Cloth *cloth)
{
       ...
       for(int i=0;i<cloth->springs.count;i++)
       {
               ...
               extern void Line(const float3 &,const float3 &,const float3
&color_rgb);
