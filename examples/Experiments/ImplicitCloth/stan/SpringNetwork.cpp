...
// Although its free software, I'll gaurantee and support this software as much
// as is reasonable.
// However, if you choose to use any of this code, then you agree that
// I assume no financial liability should the software not meet your
// expectations.
// But do feel free to send any feedback.
//
// The core backward integration functionality has all been extracted into the
// SpringNetwork class.
// This makes it easy for you if you just want to look at or use the math and
// the algorithms.
// The remainder of the code builds a cloth system with basic render support,
// I/O, and manipulators,
// so its possible to make use of the technology within a 3D application.
// This code is separated from the SpringNetwork class in order to avoid pushing
// a particular style
...
//
// Instead of having unique Hooke force and damping coefficients on each spring,
// the SpringNetwork
// code uses a spring 'type' that indexes a short list of shared named
// coefficients.
...
inline float3x3 dfdx_spring(const float3 &dir, float length, float rest,
                            float k) {
  // dir is unit length direction, rest is spring's restlength, k is spring
  // constant.
  return ((I - outerprod(dir, dir)) * Min(1.0f, rest / length) - I) * -k;
}
inline float3x3 dfdx_damp(const float3 &dir, float length, const float3 &vel,
                          float rest, float damping) {
  // inner spring damping   vel is the relative velocity  of the endpoints.
  return (I - outerprod(dir, dir)) *
         (-damping * -(dot(dir, vel) / Max(length, rest)));
}
...
SpringNetwork::SpringNetwork(int _n)
    : X(_n), V(_n), F(_n), dV(_n), A(_n), dFdX(_n), dFdV(_n) {
...
SpringNetwork::Spring &SpringNetwork::AddBlocks(Spring &s) {
  ...
  // Note the indices (s.iab,s.iba) are also stored with spring to avoid looking
  // them up each time a spring is applied
  // All 3 matrices A,dFdX, and dFdV are contstructed identically so the block
  // array layout will be the same for each.
...
void SpringNetwork::PreSolveSpring(const SpringNetwork::Spring &s) {
  // Adds this spring's contribution into force vector F and force derivitves
  // dFdX and dFdV
  // One optimization would be premultiply dfdx by dt*dt and F and dFdV by dt
  // right here in this function.
  // However, for educational purposes we wont do that now and intead just
  ...
  float3 f =
      dir * ((k * (length - s.restlen)) +
             spring_damp * dot(vel, dir));  // spring force + damping force
  F[s.a] += f;
  F[s.b] -= f;
  float3x3 dfdx = dfdx_spring(dir, length, s.restlen, k) +
                  dfdx_damp(dir, length, vel, s.restlen, spring_damp);
...
void SpringNetwork::Simulate(float dt) {
  ...
  UpdateLimits();
  awake = (dot(V, V) < cloth_sleepthreshold) ? awake - 1 : awake =
                                                               cloth_sleepcount;
}
