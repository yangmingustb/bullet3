...
class SoftDemo : public CommonRigidBodyBase {
 public:
  btAlignedObjectArray<btSoftSoftCollisionAlgorithm*>
      m_SoftSoftCollisionAlgorithms;

  btAlignedObjectArray<btSoftRididCollisionAlgorithm*>
      m_SoftRigidCollisionAlgorithms;
 ...
 public:
  ...
  {}
  virtual ~SoftDemo() { btAssert(m_dynamicsWorld == 0); }

  // virtual void clientMoveAndDisplay();

  // virtual void displayCallback();

  void createStack(btCollisionShape* boxShape, float halfCubeSize, int size,
                   float zPos);
  ...
  GUIHelperInterface* getGUIHelper() { return m_guiHelper; }
  ...
  virtual void renderScene() {
    ...
    for (int i = 0; i < softWorld->getSoftBodyArray().size(); i++) {
      ...
      // if (softWorld->getDebugDrawer() &&
      // !(softWorld->getDebugDrawer()->getDebugMode() &
      // (btIDebugDraw::DBG_DrawWireframe)))
      {
        btSoftBodyHelpers::DrawFrame(psb, softWorld->getDebugDrawer());
        btSoftBodyHelpers::Draw(psb, softWorld->getDebugDrawer(),
...
#else
// const int gNumObjects = 1;//try this in release mode: 3000. never go above
// 16384, unless you increate maxNumObjects  value in DemoApplication.cp
#endif
...
//
void SoftDemo::createStack(btCollisionShape* boxShape, float halfCubeSize,
                           int size, float zPos) {
  btTransform trans;
  trans.setIdentity();
...
void pickingPreTickCallback(btDynamicsWorld* world, btScalar timeStep) {
  ...
  if (softDemo->m_drag) {
    ...
    float rf[3];
    softDemo->getGUIHelper()
        ->getRenderInterface()
        ->getActiveCamera()
        ->getCameraPosition(rf);
    float target[3];
    softDemo->getGUIHelper()
        ->getRenderInterface()
        ->getActiveCamera()
        ->getCameraTargetPosition(target);
...
static inline btScalar UnitRand() { return (rand() / (btScalar)RAND_MAX); }

static inline btScalar SignedUnitRand() { return (UnitRand() * 2 - 1); }

static inline btVector3 Vector3Rand() {
  const btVector3 p =
      btVector3(SignedUnitRand(), SignedUnitRand(), SignedUnitRand());
  return (p.normalized());
}
...
// Big plate
//
static btRigidBody* Ctor_BigPlate(SoftDemo* pdemo, btScalar mass = 15,
                                  btScalar height = 4) {
  btTransform startTransform;
  startTransform.setIdentity();
  startTransform.setOrigin(btVector3(0, height, 0.5));
  btRigidBody* body = pdemo->createRigidBody(
      mass, startTransform, new btBoxShape(btVector3(5, 1, 5)));
  body->setFriction(1);
  return (body);
}
...
// Linear stair
//
static void Ctor_LinearStair(SoftDemo* pdemo, const btVector3& org,
                             const btVector3& sizes, btScalar angle,
                             int count) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    startTransform.setOrigin(
        org + btVector3(sizes.x() * i * 2, sizes.y() * i * 2, 0));
...
// Softbox
//
static btSoftBody* Ctor_SoftBox(SoftDemo* pdemo, const btVector3& p,
                                const btVector3& s) {
  ...
  const btVector3 c[] = {
      ...
      p + h * btVector3(-1, +1, +1), p + h * btVector3(+1, +1, +1)};
  btSoftBody* psb =
      btSoftBodyHelpers::CreateFromConvexHull(pdemo->m_softBodyWorldInfo, c, 8);
...
// SoftBoulder
//
static btSoftBody* Ctor_SoftBoulder(SoftDemo* pdemo, const btVector3& p,
                                    const btVector3& s, int np, int id) {
  btAlignedObjectArray<btVector3> pts;
  if (id) srand(id);
  for (int i = 0; i < np; ++i) {
    pts.push_back(Vector3Rand() * s + p);
  }
  btSoftBody* psb = btSoftBodyHelpers::CreateFromConvexHull(
      pdemo->m_softBodyWorldInfo, &pts[0], pts.size());
  psb->generateBendingConstraints(2);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
...
//#define TRACEDEMO { pdemo->demoname=__FUNCTION__+5;printf("Launching demo: "
//__FUNCTION__ "\r\n"); }
...
static void Init_Ropes(SoftDemo* pdemo) {
  ...
  for (int i = 0; i < n; ++i) {
    btSoftBody* psb = btSoftBodyHelpers::CreateRope(
        pdemo->m_softBodyWorldInfo, btVector3(-10, 0, i * 0.25),
        btVector3(10, 0, i * 0.25), 16, 1 + 2);
...
static void Init_RopeAttach(SoftDemo* pdemo) {
  ...
  struct Functors {
    static btSoftBody* CtorRope(SoftDemo* pdemo, const btVector3& p) {
      btSoftBody* psb = btSoftBodyHelpers::CreateRope(
          pdemo->m_softBodyWorldInfo, p, p + btVector3(10, 0, 0), 8, 1);
      psb->setTotalMass(50);
      pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
      return (psb);
    }
  ...
  btRigidBody* body = pdemo->createRigidBody(
      50, startTransform, new btBoxShape(btVector3(2, 6, 2)));
...
static void Init_ClothAttach(SoftDemo* pdemo) {
  ...
  btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
      pdemo->m_softBodyWorldInfo, btVector3(-s, h, -s), btVector3(+s, h, -s),
      btVector3(-s, h, +s), btVector3(+s, h, +s), r, r, 4 + 8, true);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
  ...
  btRigidBody* body = pdemo->createRigidBody(
      20, startTransform, new btBoxShape(btVector3(s, 1, 3)));
...
static void Init_Impact(SoftDemo* pdemo) {
  // TRACEDEMO
  btSoftBody* psb = btSoftBodyHelpers::CreateRope(pdemo->m_softBodyWorldInfo,
                                                  btVector3(0, 0, 0),
                                                  btVector3(0, -1, 0), 0, 1);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
  psb->m_cfg.kCHR = 0.5;
...
static void Init_CapsuleCollision(SoftDemo* pdemo) {
  ...
  if (g_openCLSIMDSolver) g_openCLSIMDSolver->optimize(emptyArray);
  ...
  //	btRigidBody*
  //body=pdemo->createRigidBody(20,startTransform,capsuleShape);
  ...
  btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
      pdemo->m_softBodyWorldInfo, btVector3(-s, h, -s), btVector3(+s, h, -s),
      btVector3(-s, h, +s), btVector3(+s, h, +s), r, r, fixed, true);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
...
static void Init_Collide(SoftDemo* pdemo) {
  // TRACEDEMO
  struct Functor {
    static btSoftBody* Create(SoftDemo* pdemo, const btVector3& x,
                              const btVector3& a) {
      btSoftBody* psb = btSoftBodyHelpers::CreateFromTriMesh(
          pdemo->m_softBodyWorldInfo, gVertices, &gIndices[0][0],
          NUM_TRIANGLES);
      psb->generateBendingConstraints(2);
  ...
  for (int i = 0; i < 3; ++i) {
    Functor::Create(
        pdemo, btVector3(3 * i, 2, 0),
        btVector3(SIMD_PI / 2 * (1 - (i & 1)), SIMD_PI / 2 * (i & 1), 0));
  }
  pdemo->m_cutting = true;
}
...
static void Init_Collide2(SoftDemo* pdemo) {
  ...
  struct Functor {
    static btSoftBody* Create(SoftDemo* pdemo, const btVector3& x,
                              const btVector3& a) {
      btSoftBody* psb = btSoftBodyHelpers::CreateFromTriMesh(
          pdemo->m_softBodyWorldInfo, gVerticesBunny, &gIndicesBunny[0][0],
          BUNNY_NUM_TRIANGLES);
  ...
  for (int i = 0; i < 3; ++i) {
    Functor::Create(pdemo, btVector3(0, -1 + 5 * i, 0),
                    btVector3(0, SIMD_PI / 2 * (i & 1), 0));
  }
  pdemo->m_cutting = true;
}
...
static void Init_Collide3(SoftDemo* pdemo) {
  // TRACEDEMO
  {
    const btScalar s = 8;
    btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
        pdemo->m_softBodyWorldInfo, btVector3(-s, 0, -s), btVector3(+s, 0, -s),
        btVector3(-s, 0, +s), btVector3(+s, 0, +s), 15, 15, 1 + 2 + 4 + 8,
        true);
  ...
  }
  {
    ...
    btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
        pdemo->m_softBodyWorldInfo, btVector3(-s, 0, -s) + o,
        btVector3(+s, 0, -s) + o, btVector3(-s, 0, +s) + o,
        btVector3(+s, 0, +s) + o, 7, 7, 0, true);
    btSoftBody::Material* pm = psb->appendMaterial();
    pm->m_kLST = 0.1;
    pm->m_flags -= btSoftBody::fMaterial::DebugDraw;
    psb->generateBendingConstraints(2, pm);
...
static void Init_Aero(SoftDemo* pdemo) {
  ...
  for (int i = 0; i < count; ++i) {
    btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
        pdemo->m_softBodyWorldInfo, btVector3(-s, h, -s), btVector3(+s, h, -s),
        btVector3(-s, h, +s), btVector3(+s, h, +s), segments, segments, 0,
        true);
    btSoftBody::Material* pm = psb->appendMaterial();
    pm->m_flags -= btSoftBody::fMaterial::DebugDraw;
    psb->generateBendingConstraints(2, pm);
...
static void Init_Aero2(SoftDemo* pdemo) {
  ...
  for (int i = 0; i < count; ++i) {
    btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
        pdemo->m_softBodyWorldInfo, btVector3(-s, 0, -s * 3),
        btVector3(+s, 0, -s * 3), btVector3(-s, 0, +s), btVector3(+s, 0, +s),
...
static void Init_Pressure(SoftDemo* pdemo) {
  // TRACEDEMO
  btSoftBody* psb = btSoftBodyHelpers::CreateEllipsoid(
      pdemo->m_softBodyWorldInfo, btVector3(35, 25, 0), btVector3(1, 1, 1) * 3,
      512);
...
static void Init_Sticks(SoftDemo* pdemo) {
  ...
  for (int y = 0; y < n; ++y) {
    for (int x = 0; x < n; ++x) {
      ...
      btSoftBody* psb = btSoftBodyHelpers::CreateRope(
          pdemo->m_softBodyWorldInfo, org, org + btVector3(hg * 0.001, hg, 0),
          sg, 1);
      psb->m_cfg.kDP = 0.005;
      psb->m_cfg.kCHR = 0.1;
      for (int i = 0; i < 3; ++i) {
...
static void Init_Bending(SoftDemo* pdemo) {
  // TRACEDEMO
  const btScalar s = 4;
  const btVector3 x[] = {btVector3(-s, 0, -s), btVector3(+s, 0, -s),
...
static void Init_Cloth(SoftDemo* pdemo) {
  ...
  btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
      pdemo->m_softBodyWorldInfo, btVector3(-s, 0, -s), btVector3(+s, 0, -s),
      btVector3(-s, 0, +s), btVector3(+s, 0, +s), 31, 31,
      //		31,31,
      1 + 2 + 4 + 8, true);
...
static void Init_Torus(SoftDemo* pdemo) {
  // TRACEDEMO
  btSoftBody* psb = btSoftBodyHelpers::CreateFromTriMesh(
      pdemo->m_softBodyWorldInfo, gVertices, &gIndices[0][0], NUM_TRIANGLES);
  psb->generateBendingConstraints(2);
  psb->m_cfg.piterations = 2;
  psb->randomizeConstraints();
  btMatrix3x3 m;
...
static void Init_Cutting1(SoftDemo* pdemo) {
  ...
  btSoftBody* psb =
      btSoftBodyHelpers::CreatePatch(pdemo->m_softBodyWorldInfo, p[0], p[1],
                                     p[2], p[3], r, r, 1 + 2 + 4 + 8, true);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
...
#if 1
  shape->addChildShape(btTransform(btQuaternion(0, 0, 0)),
                       new btBoxShape(btVector3(5, 1, 6)));
  shape->addChildShape(btTransform(btQuaternion(0, 0, SIMD_HALF_PI)),
                       new btBoxShape(btVector3(5, 1, 6)));
#else
  shape->addChildShape(btTransform(btQuaternion(0, 0, 0)),
                       new btCylinderShapeZ(btVector3(5, 1, 7)));
  shape->addChildShape(btTransform(btQuaternion(0, 0, SIMD_HALF_PI)),
                       new btBoxShape(btVector3(4, 1, 8)));
#endif
  btRigidBody* body = pdemo->createRigidBody(10, startTransform, shape);
  body->setFriction(1);
  btDynamicsWorld* world = pdemo->getDynamicsWorld();
  btHingeConstraint* hinge =
      new btHingeConstraint(*body, btTransform::getIdentity());
  if (speed != 0) hinge->enableAngularMotor(true, speed, 3);
  world->addConstraint(hinge);
}

//
static btSoftBody* Ctor_ClusterBunny(SoftDemo* pdemo, const btVector3& x,
                                     const btVector3& a) {
  btSoftBody* psb = btSoftBodyHelpers::CreateFromTriMesh(
      pdemo->m_softBodyWorldInfo, gVerticesBunny, &gIndicesBunny[0][0],
      BUNNY_NUM_TRIANGLES);
...
//
static btSoftBody* Ctor_ClusterTorus(SoftDemo* pdemo, const btVector3& x,
                                     const btVector3& a,
                                     const btVector3& s = btVector3(2, 2, 2)) {
  btSoftBody* psb = btSoftBodyHelpers::CreateFromTriMesh(
      pdemo->m_softBodyWorldInfo, gVertices, &gIndices[0][0], NUM_TRIANGLES);
  btSoftBody::Material* pm = psb->appendMaterial();
  pm->m_kLST = 1;
...
static void Init_ClusterDeform(SoftDemo* pdemo) {
  btSoftBody* psb = Ctor_ClusterTorus(pdemo, btVector3(0, 0, 0),
                                      btVector3(SIMD_PI / 2, 0, SIMD_HALF_PI));
...
static void Init_ClusterCollide1(SoftDemo* pdemo) {
  ...
  btSoftBody* psb = btSoftBodyHelpers::CreatePatch(
      pdemo->m_softBodyWorldInfo, btVector3(-s, 0, -s), btVector3(+s, 0, -s),
      btVector3(-s, 0, +s), btVector3(+s, 0, +s), 17, 17,  // 9,9,//31,31,
      1 + 2 + 4 + 8, true);
  btSoftBody::Material* pm = psb->appendMaterial();
  pm->m_kLST = 0.4;
  pm->m_flags -= btSoftBody::fMaterial::DebugDraw;
  ...
  /// pass zero in generateClusters to create  cluster for each tetrahedron or
  /// triangle
...
static void Init_ClusterCombine(SoftDemo* pdemo) {
  ...
  btSoftBody* psb0 = Ctor_ClusterTorus(
      pdemo, btVector3(0, 8, 0), btVector3(SIMD_PI / 2, 0, SIMD_HALF_PI), sz);
  btSoftBody* psb1 = Ctor_ClusterTorus(
      pdemo, btVector3(0, 8, 10), btVector3(SIMD_PI / 2, 0, SIMD_HALF_PI), sz);
...
static void Init_ClusterCar(SoftDemo* pdemo) {
  ...
  btSoftBody* pa =
      Ctor_ClusterBunny(pdemo, btVector3(0, 0, 0), btVector3(0, 0, 0));
  btSoftBody* pfl = Ctor_ClusterTorus(
      pdemo, wheels[0], btVector3(0, 0, SIMD_HALF_PI), btVector3(2, 4, 2));
  btSoftBody* pfr = Ctor_ClusterTorus(
      pdemo, wheels[1], btVector3(0, 0, SIMD_HALF_PI), btVector3(2, 4, 2));
  btSoftBody* prl = Ctor_ClusterTorus(
      pdemo, wheels[2], btVector3(0, 0, SIMD_HALF_PI), btVector3(2, 5, 2));
  btSoftBody* prr = Ctor_ClusterTorus(
      pdemo, wheels[3], btVector3(0, 0, SIMD_HALF_PI), btVector3(2, 5, 2));

  pfl->m_cfg.kDF = pfr->m_cfg.kDF = prl->m_cfg.kDF = prr->m_cfg.kDF = 1;
  ...
  pfl->m_cfg.piterations = pfr->m_cfg.piterations = prl->m_cfg.piterations =
      prr->m_cfg.piterations = 1;
  pfl->m_clusters[0]->m_matching = pfr->m_clusters[0]->m_matching =
      prl->m_clusters[0]->m_matching = prr->m_clusters[0]->m_matching = 0.05;
  pfl->m_clusters[0]->m_ndamping = pfr->m_clusters[0]->m_ndamping =
      prl->m_clusters[0]->m_ndamping = prr->m_clusters[0]->m_ndamping = 0.05;
...
static void Init_ClusterRobot(SoftDemo* pdemo) {
  struct Functor {
    static btSoftBody* CreateBall(SoftDemo* pdemo, const btVector3& pos) {
      btSoftBody* psb = btSoftBodyHelpers::CreateEllipsoid(
          pdemo->m_softBodyWorldInfo, pos, btVector3(1, 1, 1) * 3, 512);
      psb->m_materials[0]->m_kLST = 0.45;
      psb->m_cfg.kVC = 20;
      psb->setTotalMass(50, true);
      psb->setPose(true, false);
  ...
  btSoftBody* psb2 =
      Functor::CreateBall(pdemo, base + btVector3(0, 0, +8 * btSqrt(2)));
  const btVector3 ctr =
      (psb0->clusterCom(0) + psb1->clusterCom(0) + psb2->clusterCom(0)) / 3;
  btCylinderShape* pshp = new btCylinderShape(btVector3(8, 1, 8));
  btRigidBody* prb = pdemo->createRigidBody(
      50, btTransform(btQuaternion(0, 0, 0), ctr + btVector3(0, 5, 0)), pshp);
  ...
  btRigidBody* pgrn;
  pgrn = pdemo->createRigidBody(
      0, btTransform(btQuaternion(0, -SIMD_HALF_PI / 2, 0), btVector3(0, 0, 0)),
      pbox);
...
static void Init_ClusterStackSoft(SoftDemo* pdemo) {
  for (int i = 0; i < 10; ++i) {
    btSoftBody* psb = Ctor_ClusterTorus(pdemo, btVector3(0, -9 + 8.25 * i, 0),
                                        btVector3(0, 0, 0));
    psb->m_cfg.kDF = 1;
  }
}
...
static void Init_ClusterStackMixed(SoftDemo* pdemo) {
  for (int i = 0; i < 10; ++i) {
    if ((i + 1) & 1) {
      ...
      btSoftBody* psb = Ctor_ClusterTorus(pdemo, btVector3(0, -9 + 4.25 * i, 0),
                                          btVector3(0, 0, 0));
      psb->m_cfg.kDF = 1;
    }
  }
...
static void Init_TetraBunny(SoftDemo* pdemo) {
  btSoftBody* psb = btSoftBodyHelpers::CreateFromTetGenData(
      pdemo->m_softBodyWorldInfo, TetraBunny::getElements(), 0,
      TetraBunny::getNodes(), false, true, true);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
...
static void Init_TetraCube(SoftDemo* pdemo) {
  btSoftBody* psb = btSoftBodyHelpers::CreateFromTetGenData(
      pdemo->m_softBodyWorldInfo, TetraCube::getElements(), 0,
      TetraCube::getNodes(), false, true, true);
  pdemo->getSoftDynamicsWorld()->addSoftBody(psb);
...
void SoftDemo::setDrawClusters(bool drawClusters) {
  if (drawClusters) {
    getSoftDynamicsWorld()->setDrawFlags(
        getSoftDynamicsWorld()->getDrawFlags() | fDrawFlags::Clusters);
  } else {
    getSoftDynamicsWorld()->setDrawFlags(
        getSoftDynamicsWorld()->getDrawFlags() & (~fDrawFlags::Clusters));
  }
...
void SoftDemo::initPhysics() {
  ...
  btCollisionShape* groundShape = 0;
  {
    ...
    btTriangleIndexVertexArray* indexVertexArrays =
        new btTriangleIndexVertexArray(
            totalTriangles, gGroundIndices, indexStride, totalVerts,
  ...
  btCollisionShape* groundBox =
      new btBoxShape(btVector3(100, CUBE_HALF_EXTENTS, 100));
  m_collisionShapes.push_back(groundBox);

  btCompoundShape* cylinderCompound = new btCompoundShape;
  btCollisionShape* cylinderShape = new btCylinderShape(
      btVector3(CUBE_HALF_EXTENTS, CUBE_HALF_EXTENTS, CUBE_HALF_EXTENTS));
  btTransform localTransform;
  localTransform.setIdentity();
  cylinderCompound->addChildShape(localTransform, cylinderShape);
  ...
  /// register some softbody collision algorithms on top of the default
  ...
  if (g_openCLSIMDSolver) delete g_openCLSIMDSolver;
  if (g_softBodyOutput) delete g_softBodyOutput;

  if (1) {
    g_openCLSIMDSolver =
        new btOpenCLSoftBodySolverSIMDAware(g_cqCommandQue, g_cxMainContext);
    //	g_openCLSIMDSolver = new btOpenCLSoftBodySolver( g_cqCommandQue,
    //g_cxMainContext);
    g_openCLSIMDSolver->setCLFunctions(
        new CachingCLFunctions(g_cqCommandQue, g_cxMainContext));
  }
  ...
  btDiscreteDynamicsWorld* world =
      new btSoftRigidDynamicsWorld(m_dispatcher, m_broadphase, m_solver,
                                   m_collisionConfiguration, softBodySolver);
  ...
  if (current_demo < 0) current_demo = lastDemo;
  if (current_demo > lastDemo) current_demo = 0;
...
class CommonExampleInterface* SoftDemoCreateFunc(
