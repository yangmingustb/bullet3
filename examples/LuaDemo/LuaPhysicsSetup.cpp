...
LuaPhysicsSetup::~LuaPhysicsSetup() { sLuaDemo = 0; }
...
static int gDeleteDynamicsWorld(lua_State* L) { return 0; }

ATTRIBUTE_ALIGNED16(struct)
CustomRigidBodyData { int m_graphicsInstanceIndex; };
...
static int gCreateCubeShape(lua_State* L) {
  ...
  if (argc == 4) {
    ...
    halfExtents =
        btVector3(lua_tonumber(L, 2), lua_tonumber(L, 3), lua_tonumber(L, 4));
    btCollisionShape* colShape = new btBoxShape(halfExtents);

    lua_pushlightuserdata(L, colShape);
    return 1;
  } else {
    std::cerr << "Error: invalid number of arguments to createCubeShape, "
                 "expected 4 (world,halfExtentsX,halfExtentsY,halfExtentsX) "
                 "but got "
              << argc;
  }
  return 0;
...
static int gCreateSphereShape(lua_State* L) {
  ...
  if (argc == 2) {
    ...
    std::cerr << "Error: invalid number of arguments to createSphereShape, "
                 "expected 2 (world,radius) but got "
              << argc;
  }
  return 0;
}
...
static int gLoadMultiBodyFromUrdf(lua_State* L) {
  ...
  if (argc == 4) {
    ...
    btDiscreteDynamicsWorld* world =
        (btDiscreteDynamicsWorld*)lua_touserdata(L, 1);
    ...
    if (loadOk) {
      ...
      //			printf("urdf root link index =
      //%d\n",rootLinkIndex);
      MyMultiBodyCreator creation(sLuaDemo->m_guiHelper);
      bool m_useMultiBody = true;
      ConvertURDF2Bullet(u2b, creation, tr, sLuaDemo->m_dynamicsWorld,
...
static int gCreateRigidBody(lua_State* L) {
  ...
  if (argc == 5) {
    ...
    if (!lua_isuserdata(L, 2)) {
      std::cerr << "error: second argument to b3CreateRigidbody should be "
                   "collision shape";
      return 0;
    }
    ...
    std::cerr << "Error: invalid number of arguments to createRigidBody, "
                 "expected 5 (world,shape,mass,pos,orn) but got "
              << argc;
  }
  return 0;
}
...
static int gSetBodyPosition(lua_State* L) {
  ...
  if (argc == 3) {
    ...
    std::cerr << "error: setBodyPosition expects 6 arguments like "
                 "setBodyPosition(world,body,0,1,0)";
  }
  return 0;
}
...
static int gSetBodyOrientation(lua_State* L) {
  ...
  if (argc == 3) {
    ...
    std::cerr << "error: setBodyOrientation expects 3 arguments like "
...
void LuaPhysicsSetup::exitPhysics() { CommonMultiBodyBase::exitPhysics(); }

class CommonExampleInterface* LuaDemoCreateFunc(
