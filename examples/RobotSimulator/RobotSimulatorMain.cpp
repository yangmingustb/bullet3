...
int main(int argc, char* argv[]) {
  ...
  while (sim->canSubmitCommand()) {
    ...
    if (keyEvents.m_numKeyboardEvents) {
      ...
      // m_keyState is a flag combination of eButtonIsDown,eButtonTriggered,
      // eButtonReleased
      for (int i = 0; i < keyEvents.m_numKeyboardEvents; i++) {
        ...
        if (e.m_keyCode == '0') {
          if (e.m_keyState & eButtonTriggered) {
            if (vidLogId < 0) {
              vidLogId =
                  sim->startStateLogging(STATE_LOGGING_VIDEO_MP4, "video.mp4");
            } else {
        ...
        if (e.m_keyCode == 'm') {
          if (minitaurLogId < 0 && e.m_keyState & eButtonTriggered) {
            minitaurLogId =
                sim->startStateLogging(STATE_LOGGING_MINITAUR, "simlog.bin");
          }
