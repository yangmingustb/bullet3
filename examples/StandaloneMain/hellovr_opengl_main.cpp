...
class CMainApplication {
 ...
 private:  // OpenGL bookkeeping
  ...
  int m_iSceneVolumeInit;  // if you want something other than the default
                           // 20x20x20
...
CMainApplication::CMainApplication(int argc, char *argv[])
  ...
  for (int i = 1; i < argc; i++) {
    if (!stricmp(argv[i], "-gldebug")) {
      ...
      g_bPrintf = false;
    } else if (!stricmp(argv[i], "-cubevolume") && (argc > i + 1) &&
               (*argv[i + 1] != '-')) {
...
void MyKeyboardCallback(int key, int state) {
  ...
  if (prevKeyboardCallback) prevKeyboardCallback(key, state);
}
...
bool CMainApplication::BInit() {
  ...
  */
  ...
  m_app =
      new SimpleOpenGL3App("SimpleOpenGL3App", m_nWindowWidth, m_nWindowHeight,
                           true, maxNumObjectCapacity, maxShapeCapacityInBytes);
  ...
  */
  ...
  std::string strWindowTitle =
      "hellovr_bullet - " + m_strDriver + " " + m_strDisplay;
...
bool CMainApplication::HandleInput() {
  ...
  for (vr::TrackedDeviceIndex_t unDevice = 0;
    ...
    if (m_pHMD->GetControllerState(unDevice, &state,
      ...
      if (m_pHMD->GetTrackedDeviceClass(unDevice) ==
        ...
        for (int button = 0; button < vr::k_EButton_Max; button++) {
          ...
          if (isTrigger) {
            ...
            if ((sPrevStates[unDevice].ulButtonPressed & trigger) == 0) {
              //							printf("Device
              //PRESSED: %d, button %d\n", unDevice, button);
              sExample->vrControllerButtonCallback(unDevice, button, 1, pos,
                                                   orn);
            } else {
              //							printf("Device
              //MOVED: %d\n", unDevice);
              sExample->vrControllerMoveCallback(unDevice, pos, orn,
                                                 state.rAxis[1].x, allAxis);
            }
          } else {
            if (m_pHMD->GetTrackedDeviceClass(unDevice) ==
                vr::TrackedDeviceClass_Controller) {
              ...
              if ((sPrevStates[unDevice].ulButtonPressed & trigger) != 0) {
                ...
                sExample->vrControllerButtonCallback(unDevice, button, 0, pos,
                                                     orn);
              } else {
                sExample->vrControllerMoveCallback(unDevice, pos, orn,
                                                   state.rAxis[1].x, allAxis);
      ...
      //			m_rbShowTrackedDevice[ unDevice ] =
...
void CMainApplication::RenderFrame() {
  ...
  if (m_pHMD) {
    ...
    if (!gDisableDesktopGL) {
      if (gDisplayDistortion) {
        ...
        // See
        // https://twitter.com/id_aa_carmack/status/268488838425481217?lang=en
        ...
        glBlitFramebuffer(0, 0, m_nRenderWidth, m_nRenderHeight, 0, 0,
                          m_nRenderWidth, m_nRenderHeight, GL_COLOR_BUFFER_BIT,
                          GL_LINEAR);
    ...
    vr::Texture_t leftEyeTexture = {(void *)leftEyeDesc.m_nResolveTextureId,
                                    vr::TextureType_OpenGL,
                                    vr::ColorSpace_Gamma};
    vr::VRCompositor()->Submit(vr::Eye_Left, &leftEyeTexture);
    vr::Texture_t rightEyeTexture = {(void *)rightEyeDesc.m_nResolveTextureId,
                                     vr::TextureType_OpenGL,
                                     vr::ColorSpace_Gamma};
  ...
  if (m_bVblank && m_bGlFinishHack) {
    ...
    // issues. This glFinish()
    // appears to clear that up. Temporary fix while I try to get nvidia to
  ...
  // Clear
  {
    ...
    // complete, not just the submission
    // of the command. So, we do a clear here right here so the glFinish will
    // wait fully for the swap.
  ...
  if (m_iTrackedControllerCount != m_iTrackedControllerCount_Last ||
      m_iValidPoseCount != m_iValidPoseCount_Last) {
    ...
    b3Printf("PoseCount:%d(%s) Controllers:%d\n", m_iValidPoseCount,
             m_strPoseClasses.c_str(), m_iTrackedControllerCount);
  }
...
//-----------------------------------------------------------------------------
GLuint CMainApplication::CompileGLShader(const char *pchShaderName,
                                         const char *pchVertexShader,
                                         const char *pchFragmentShader) {
  ...
  glDeleteShader(
      nSceneVertexShader);  // the program hangs onto this once it's attached
  ...
  glDeleteShader(
      nSceneFragmentShader);  // the program hangs onto this once it's attached
...
bool CMainApplication::CreateAllShaders() {
  ...
  m_nControllerMatrixLocation =
      glGetUniformLocation(m_unControllerTransformProgramID, "matrix");
  ...
  m_nRenderModelMatrixLocation =
      glGetUniformLocation(m_unRenderModelProgramID, "matrix");
  ...
  m_unLensProgramID = CompileGLShader(
      ...
      "void main()\n"
      "{\n"
      "	float fBoundsCheck = ( (dot( vec2( lessThan( v2UVgreen.xy, "
      "vec2(0.05, 0.05)) ), vec2(1.0, 1.0))+dot( vec2( greaterThan( "
      "v2UVgreen.xy, vec2( 0.95, 0.95)) ), vec2(1.0, 1.0))) );\n"
      "	if( fBoundsCheck > 1.0 )\n"
...
bool CMainApplication::SetupTexturemaps() {
  ...
  std::string strFullPath =
      Path_MakeAbsolute("../cube_texture.png", sExecutableDirectory);
...
void CMainApplication::SetupScene() {
  ...
  for (int z = 0; z < m_iSceneVolumeDepth; z++) {
    ...
    mat =
        mat *
        Matrix4().translate(0, -((float)m_iSceneVolumeHeight) * m_fScaleSpacing,
                            m_fScaleSpacing);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainApplication::AddCubeVertex(float fl0, float fl1, float fl2, float fl3,
                                     float fl4, std::vector<float> &vertdata) {
...
void CMainApplication::DrawControllers() {
  ...
  for (vr::TrackedDeviceIndex_t unTrackedDevice =
    ...
    if (m_pHMD->GetTrackedDeviceClass(unTrackedDevice) !=
        vr::TrackedDeviceClass_Controller)
      continue;
...
void CMainApplication::SetupDistortion() {
  ...
  glBufferData(GL_ARRAY_BUFFER, vVerts.size() * sizeof(VertexDataLens),
               &vVerts[0], GL_STATIC_DRAW);

  glGenBuffers(1, &m_glIDIndexBuffer);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_glIDIndexBuffer);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, vIndices.size() * sizeof(GLushort),
...
void CMainApplication::RenderStereoTargets() {
  ...
  glClearColor(0.15f, 0.15f, 0.18f,
               1.0f);  // nice background color, but not black
  ...
  // Left Eye
  {
    ...
    float dist = 1;
    m_app->m_instancingRenderer->getActiveCamera()->setCameraTargetPosition(
        mat[12] - dist * mat[8], mat[13] - dist * mat[9],
        mat[14] - dist * mat[10]);
    m_app->m_instancingRenderer->getActiveCamera()->setCameraUpVector(
        mat[0], mat[1], mat[2]);
    m_app->m_instancingRenderer->getActiveCamera()->setVRCamera(
        viewMatLeft.get(), m_mat4ProjectionLeft.get());
  ...
  m_app->m_instancingRenderer->setRenderFrameBuffer(
      (unsigned int)leftEyeDesc.m_nRenderFramebufferId);
...

  {
    ...
    m_app->m_instancingRenderer->getActiveCamera()->setVRCamera(
        viewMatRight.get(), m_mat4ProjectionRight.get());
    m_app->m_instancingRenderer->updateCamera(m_app->getUpAxis());
    m_app->m_instancingRenderer->getActiveCamera()->setVRCamera(
        viewMatRight.get(), m_mat4ProjectionRight.get());
  }
  ...
  m_app->m_instancingRenderer->setRenderFrameBuffer(
      (unsigned int)rightEyeDesc.m_nRenderFramebufferId);
...
void CMainApplication::RenderScene(vr::Hmd_Eye nEye) {
  ...
  if (gEnableVRRenderControllers) {
    ...
    for (uint32_t unTrackedDevice = 0;
         unTrackedDevice < vr::k_unMaxTrackedDeviceCount; unTrackedDevice++) {
      if (!m_rTrackedDeviceToRenderModel[unTrackedDevice] ||
          !m_rbShowTrackedDevice[unTrackedDevice])
        continue;

      const vr::TrackedDevicePose_t &pose =
          m_rTrackedDevicePose[unTrackedDevice];
      if (!pose.bPoseIsValid) continue;

      if (bIsInputCapturedByAnotherProcess &&
          m_pHMD->GetTrackedDeviceClass(unTrackedDevice) ==
              vr::TrackedDeviceClass_Controller)
        continue;
...
void CMainApplication::UpdateHMDMatrixPose() {
  ...
  }
  {
    ...
    if (m_rTrackedDevicePose[vr::k_unTrackedDeviceIndex_Hmd].bPoseIsValid) {
      m_mat4HMDPose =
          m_rmat4DevicePose[vr::k_unTrackedDeviceIndex_Hmd].invert();
    }
  }
}
...
CGLRenderModel *CMainApplication::FindOrLoadRenderModel(
  ...
  if (!pRenderModel) {
    ...
    while (1) {
      error = vr::VRRenderModels()->LoadRenderModel_Async(pchRenderModelName,
                                                          &pModel);
    ...
    while (1) {
      error = vr::VRRenderModels()->LoadTexture_Async(pModel->diffuseTextureId,
                                                      &pTexture);
      if (error != vr::VRRenderModelError_Loading) break;
...
void CMainApplication::SetupRenderModelForTrackedDevice(
  ...
  std::string sRenderModelName = GetTrackedDeviceString(
      m_pHMD, unTrackedDeviceIndex, vr::Prop_RenderModelName_String);
  CGLRenderModel *pRenderModel =
      FindOrLoadRenderModel(sRenderModelName.c_str());
  if (!pRenderModel) {
    std::string sTrackingSystemName = GetTrackedDeviceString(
        m_pHMD, unTrackedDeviceIndex, vr::Prop_TrackingSystemName_String);
    b3Printf("Unable to load render model for tracked device %d (%s.%s)",
             unTrackedDeviceIndex, sTrackingSystemName.c_str(),
...
void CMainApplication::SetupRenderModels() {
  memset(m_rTrackedDeviceToRenderModel, 0,
...
int main(int argc, char *argv[]) {
  ...
  args.GetCmdLineArgument("max_shape_capacity_in_bytes",
  ...
  if (sExample) {
    // until we have a proper VR gui, always assume we want the hard-coded
    // default robot assets
    char *newargv[2];
