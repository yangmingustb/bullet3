...
static btVector4 sMyColors[4] = {
    btVector4(0.3, 0.3, 1, 1), btVector4(0.6, 0.6, 1, 1), btVector4(0, 1, 0, 1),
    btVector4(0, 1, 1, 1),
    // btVector4(1,1,0,1),
};
...
struct TinyRendererGUIHelper : public GUIHelperInterface {
  ...
  const TGAImage& getFrameBuffer() const { return m_rgbColorBuffer; }
  ...
  virtual int registerGraphicsShape(const float* vertices, int numvertices,
    ...
    TinyRenderObjectData* swObj =
        new TinyRenderObjectData(m_rgbColorBuffer, m_depthBuffer);
  ...
  virtual void resetCamera(float camDist, float pitch, float yaw, float camPosX,
