 ...
 public:
  ...
  virtual void initPhysics() {
    ...
    /// the endeffector tries to track it using Inverse Kinematics
    {
      ...
      m_targetSphereInstance = m_app->m_renderer->registerGraphicsInstance(
          sphereId, m_targetPos, orn, color, scaling);
    }
...
*/
  ...
  virtual void stepSimulation(float deltaTime) {
    ...
    m_time += dt;
    m_targetPos.setValue(0.4 - 0.4 * b3Cos(m_time), 0,
                         0.8 + 0.4 * b3Cos(m_time));
  ...
  virtual void renderScene() {
    ...
    m_app->m_renderer->writeSingleInstanceTransformToCPU(
        m_targetPos, orn, m_targetSphereInstance);
    m_app->m_renderer->writeTransforms();
...
class CommonExampleInterface* KukaGraspExampleCreateFunc(
