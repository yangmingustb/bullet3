...
class R2D2GraspExample : public CommonExampleInterface {
 ...
 public:
    ...
    if ((m_options & eROBOTIC_LEARN_GRASP) != 0) {
      {
        ...
        if (m_r2d2Index >= 0) {
          ...
          for (int i = 0; i < 4; i++) {
            ...
            m_robotSim.setJointMotorControl(m_r2d2Index, wheelJointIndices[i],
                                            controlArgs);
          }
        }
      }
  ...
  virtual void stepSimulation(float deltaTime) { m_robotSim.stepSimulation(); }
...
class CommonExampleInterface* R2D2GraspExampleCreateFunc(
