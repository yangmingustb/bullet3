...
class GripperGraspExample : public CommonExampleInterface {
 ...
 public:
  ...
  virtual void initPhysics() {
    ...
    m_robotSim.configureDebugVisualizer(COV_ENABLE_SEGMENTATION_MARK_PREVIEW,
                                        0);
...
*/
          ...
          for (int i = 0; i < 3; i++) {
            ...
            controlArgs.m_kd = 1.;
            m_robotSim.setJointMotorControl(m_gripperIndex,
                                            fingerJointIndices[i], controlArgs);
          }
        }
      }
  ...
  virtual void exitPhysics() { m_robotSim.disconnect(); }
  virtual void stepSimulation(float deltaTime) {
    if ((m_options & eGRIPPER_GRASP) != 0) {
      if ((m_gripperIndex >= 0)) {
        int fingerJointIndices[3] = {0, 1, 3};
        double fingerTargetVelocities[3] = {sGripperVerticalVelocity,
        ...
        for (int i = 0; i < 3; i++) {
          ...
          controlArgs.m_maxTorqueValue = maxTorqueValues[i];
          controlArgs.m_kd = 1.;
          m_robotSim.setJointMotorControl(m_gripperIndex, fingerJointIndices[i],
                                          controlArgs);
        }
      }
    }
    ...
    if ((m_options & eSOFTBODY_MULTIBODY_COUPLING) != 0) {
      ...
      if (numJoints == 7) {
        ...
        m_robotSim.getLinkState(0, 6, computeVelocity, computeForwardKinematics,
                                &linkState);

        m_worldPos.setValue(linkState.m_worldLinkFramePosition[0],
                            linkState.m_worldLinkFramePosition[1],
                            linkState.m_worldLinkFramePosition[2]);
        m_worldOri.setValue(linkState.m_worldLinkFrameOrientation[0],
        ...
        ikargs.m_flags |=
            B3_HAS_IK_TARGET_ORIENTATION /* + B3_HAS_NULL_SPACE_VELOCITY*/;
...
class CommonExampleInterface* GripperGraspExampleCreateFunc(
