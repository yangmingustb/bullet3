...
GLint att[] = {GLX_RGBA, GLX_DEPTH_SIZE,   24,  GLX_RED_SIZE,
               ...
               8,        GLX_ALPHA_SIZE,   8,   GLX_STENCIL_SIZE,
...
struct InternalData2 {
  ...
  InternalData2()
    ...
    if (missingFunc) {
      fprintf(stderr,
              "Error: missing func XGetKeyboardMapping in %s, exiting!\n",
              X11_LIBRARY);
      exit(EXIT_FAILURE);
    }
    ...
    if (missingFunc) {
      fprintf(stderr,
              "Error: missing func XGetWindowAttributes in %s, exiting!\n",
              X11_LIBRARY);
      exit(EXIT_FAILURE);
    }
...
X11OpenGLWindow::X11OpenGLWindow()
    : m_OpenGLInitialized(false), m_requestedExit(false) {
...
void X11OpenGLWindow::enableOpenGL() {
  if (forceOpenGL3) {
    ...
    if (!isExtensionSupported(glxExts, "GLX_ARB_create_context") ||
      ...
      ctx = glXCreateNewContext(m_data->m_dpy, m_data->m_bestFbc, GLX_RGBA_TYPE,
                                0, True);
    }
...
*/
      ...
      if (!ctxErrorOccurred && ctx)
        ...
        // return the newest context version compatible with OpenGL versions
        // less
        // than version 3.0.
    ...
    m_data->m_glc =
        glXCreateContext(m_data->m_dpy, m_data->m_vi, NULL, GL_TRUE);
...
void X11OpenGLWindow::createWindow(const b3gWindowConstructionInfo& ci) {
  ...
  if (forceOpenGL3) {
    ...
    if (!glXQueryVersion(m_data->m_dpy, &glxMajor, &glxMinor) ||
    ...
    GLXFBConfig* fbc = glXChooseFBConfig(
        m_data->m_dpy, DefaultScreen(m_data->m_dpy), visual_attribs, &fbcount);
    if (!fbc) {
...
/// don't use highest samples, it is really slow on some NVIDIA Quadro cards
    ...
    for (i = 0; i < fbcount; ++i) {
      ...
      if (vi) {
        ...
        glXGetFBConfigAttrib(m_data->m_dpy, fbc[i], GLX_SAMPLE_BUFFERS,
                             &samp_buf);
    ...
    m_data->m_swa.colormap = m_data->m_cmap = MyXCreateColormap(
        m_data->m_dpy, RootWindow(m_data->m_dpy, m_data->m_vi->screen),
    ...
    // m_data->m_win = m_data->m_x11_XCreateWindow(m_data->m_dpy,
    // m_data->m_root, 0, 0, ci.m_width, ci.m_height, 0, m_data->m_vi->depth,
    // InputOutput, m_data->m_vi->visual, CWColormap | CWEventMask,
    // &m_data->m_swa);
        ...
        if (m_data->m_keyboardCallback) {
          ...
          if (MyXEventsQueued(m_data->m_dpy, QueuedAfterReading)) {
            ...
            if (nev.type == KeyPress &&
                nev.xkey.time == m_data->m_xev.xkey.time &&
...
#endif
          int state = 0;
          if (!is_retriggered) (*m_data->m_keyboardCallback)(keycode, state);
        }
...
float X11OpenGLWindow::getTimeInSeconds() { return 0.f; }

bool X11OpenGLWindow::requestedExit() const { return m_requestedExit; }

void X11OpenGLWindow::setRequestExit() { m_requestedExit = true; }
...
void X11OpenGLWindow::setMouseButtonCallback(
...
int X11OpenGLWindow::getWidth() const {
  if (m_data) return m_data->m_glWidth;
  return 0;
}
int X11OpenGLWindow::getHeight() const {
  if (m_data) return m_data->m_glHeight;
  return 0;
}
