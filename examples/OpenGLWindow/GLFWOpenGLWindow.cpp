...
void GLFWOpenGLWindow::mouseButtonCallbackInternal(int button, int state) {
  if (getMouseButtonCallback() && m_data->m_hasCursorPos) {
    getMouseButtonCallback()(button, state, m_data->m_cursorXPos,
                             m_data->m_cursorYPos);
  }
}
...
void GLFWOpenGLWindow::createDefaultWindow(int width, int height,
                                           const char* title) {
...
float GLFWOpenGLWindow::getTimeInSeconds() { return 0.f; }
...
void GLFWOpenGLWindow::setMouseButtonCallback(
...
void GLFWOpenGLWindow::setResizeCallback(b3ResizeCallback resizeCallback) {
  ...
  getResizeCallback()(m_data->m_width / getRetinaScale(),
                      m_data->m_height / getRetinaScale());
}
...
void GLFWOpenGLWindow::setKeyboardCallback(
