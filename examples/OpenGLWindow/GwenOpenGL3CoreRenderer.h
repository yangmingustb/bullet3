...
class GwenOpenGL3CoreRenderer : public Gwen::Renderer::Base {
 ...
 public:
  GwenOpenGL3CoreRenderer(GLPrimitiveRenderer* primRender, sth_stash* font,
                          float screenWidth, float screenHeight,
                          float retinaScale, MyTextureLoader* loader = 0)
  ...
  virtual void End() { glDisable(GL_BLEND); }

  virtual void StartClip() {
    if (m_useTrueTypeFont) sth_flush_draw(m_font);
    ...
    glScissor(
        m_retinaScale * rect.x * Scale(), m_retinaScale * rect.y * Scale(),
        m_retinaScale * rect.w * Scale(), m_retinaScale * rect.h * Scale());
  ...
  virtual void DrawFilledRect(Gwen::Rect rect) {
    ...
    m_primitiveRenderer->drawRect(rect.x, rect.y + m_yOffset, rect.x + rect.w,
                                  rect.y + rect.h + m_yOffset, m_currentColor);

    //		m_primitiveRenderer->drawTexturedRect2a(rect.x,
    //rect.y+m_yOffset,
    //		rect.x+rect.w, rect.y+rect.h+m_yOffset, m_currentColor,0,0,1,1);
    //		m_yOffset+=rect.h+10;
  }

  void RenderText(Gwen::Font* pFont, Gwen::Point rasterPos,
    ...
    if (m_useTrueTypeFont) {
      ...
      Translate(r);
      sth_draw_text(m_font, 1, m_fontScaling, r.x, r.y + yoffset, unicodeText,
                    &dx, m_screenWidth, m_screenHeight, measureOnly,
                    m_retinaScale);
    } else {
      // float width = 0.f;
      int pos = 0;
      ...
      while (unicodeText[pos]) {
        ...
        m_primitiveRenderer->drawTexturedRect2(
            rect.x, rect.y + m_yOffset, rect.x + rect.w,
            rect.y + rect.h + m_yOffset, m_currentColor,
            m_currentFont->m_CharU0[c], m_currentFont->m_CharV0[c],
            m_currentFont->m_CharU1[c], m_currentFont->m_CharV1[c]);
  ...
  Gwen::Point MeasureText(Gwen::Font* pFont, const Gwen::UnicodeString& text) {
    ...
    float dx = 0;
    if (m_useTrueTypeFont) {
      sth_draw_text(m_font, 1, m_fontScaling, xpos, ypos, unicodeText, &dx,
                    m_screenWidth, m_screenHeight, measureOnly);
      ...
      while (unicodeText[pos]) {
        width +=
            m_currentFont->m_CharWidth[(int)unicodeText[pos]] + extraSpacing;
        pos++;
      }
      Gwen::Point pt;
  ...
  virtual void LoadTexture(Gwen::Texture* pTexture) {
    if (m_textureLoader) m_textureLoader->LoadTexture(pTexture);
  }
  virtual void FreeTexture(Gwen::Texture* pTexture) {
    if (m_textureLoader) m_textureLoader->FreeTexture(pTexture);
  }

  virtual void DrawTexturedRect(Gwen::Texture* pTexture, Gwen::Rect rect,
    ...
    // m_primitiveRenderer->drawRect(rect.x, rect.y+m_yOffset, rect.x+rect.w,
    // rect.y+rect.h+m_yOffset, eraseColor);
...
*/
    ...
    m_primitiveRenderer->drawTexturedRect(
        rect.x, rect.y + m_yOffset, rect.x + rect.w,
        rect.y + rect.h + m_yOffset, color, 0 + add, 0, 1 + add, 1, true);
