...
int getSpecialKeyFromVirtualKeycode(int virtualKeyCode) {
  ...
  switch (virtualKeyCode) {
    ...
    default: {
      // keycode = MapVirtualKey( virtualKeyCode, MAPVK_VK_TO_CHAR ) &
      // 0x0000FFFF;
    }
  };
...
bool Win32Window::isModifierKeyPressed(int key) {
  ...
  switch (key) {
    case B3G_ALT: {
      isPressed =
          ((sData->m_internalKeyModifierFlags & INTERNAL_ALT_MODIFIER) != 0);
      break;
    };
    case B3G_SHIFT: {
      isPressed =
          ((sData->m_internalKeyModifierFlags & INTERNAL_SHIFT_MODIFIER) != 0);
      break;
    };
    case B3G_CONTROL: {
      isPressed = ((sData->m_internalKeyModifierFlags &
                    INTERNAL_CONTROL_MODIFIER) != 0);
      break;
    };
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  switch (message) {
    ...
    case WM_RBUTTONUP: {
      ...
      if (sData && sData->m_mouseButtonCallback)
        (*sData->m_mouseButtonCallback)(2, 0, sData->m_mouseXpos,
                                        sData->m_mouseYpos);
    ...
    case WM_RBUTTONDOWN: {
      ...
      if (sData && sData->m_mouseButtonCallback)
        (*sData->m_mouseButtonCallback)(2, 1, sData->m_mouseXpos,
                                        sData->m_mouseYpos);
    ...
    case WM_SIZE:  // Size Action Has Taken Place
      ...
      switch (wParam)  // Evaluate Size Action
      {
        ...
        case SIZE_RESTORED:   // Was Window Restored?
          ...
          sData->m_fullWindowHeight =
              wr.bottom - wr.top;  // LOWORD (lParam) HIWORD (lParam);
          sData->m_openglViewportWidth = clientRect.right;
          sData->m_openglViewportHeight = clientRect.bottom;

          if (sData->m_resizeCallback) {
            glViewport(0, 0, sData->m_openglViewportWidth,
                       sData->m_openglViewportHeight);
            (*sData->m_resizeCallback)(sData->m_openglViewportWidth,
                                       sData->m_openglViewportHeight);
          }
...
void Win32Window::setWindowTitle(const char* titleChar) {
  ...
  SendMessageTimeoutA(m_data->m_hWnd, WM_SETTEXT, 0,
                      reinterpret_cast<LPARAM>(titleChar), SMTO_ABORTIFHUNG,
                      2000, &dwResult);
#else
  DWORD dwResult;
...
void Win32Window::createWindow(const b3gWindowConstructionInfo& ci) {
  ...
  if (!windowHandle) {
    ...
    wcex.hIcon = LoadIcon(
        NULL, IDI_APPLICATION);  //(HICON)LoadImage(hInstance, "bullet_ico.ico",
                                 //IMAGE_ICON, 0,0,
    ...
    //		wcex.hIcon = (HICON)LoadImage(hInstance, "bullet.ico", IMAGE_ICON,
    //0,0, LR_LOADFROMFILE);
    ...
    if (!fullscreen)
      style = WS_SYSMENU | WS_BORDER | WS_CAPTION | WS_CLIPCHILDREN |
              WS_CLIPSIBLINGS | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_SIZEBOX;
    ...
    int windowLeft =
        (GetSystemMetrics(SM_CXSCREEN) - m_data->m_fullWindowWidth) / 2;
    int windowTop =
        (GetSystemMetrics(SM_CYSCREEN) - m_data->m_fullWindowHeight) / 2;
    ...
    m_data->m_hWnd =
        CreateWindow(ClassName, emptyString, style, windowLeft, windowTop,
                     m_data->m_fullWindowWidth, m_data->m_fullWindowHeight,
    ...
    MoveWindow(m_data->m_hWnd, windowLeft, windowTop, m_data->m_fullWindowWidth,
               m_data->m_fullWindowHeight, TRUE);

    GetClientRect(m_data->m_hWnd, &clientRect);
    int w = clientRect.right - clientRect.left;
    int h = clientRect.bottom - clientRect.top;
    //		printf("actual client OpenGL viewport width / height = %d,
    //%d\n",w,h);
  ...
  if (fullscreen) {
    ...
    if (res != DISP_CHANGE_SUCCESSFUL) {  // try again without forcing display
                                          // frequency
...
void Win32Window::switchFullScreen(bool fullscreen, int width, int height,
  ...
  if (fullscreen) {
    ...
    MoveWindow(m_data->m_hWnd, 0, 0, m_data->m_fullWindowWidth,
               m_data->m_fullWindowHeight, TRUE);
    ...
    DWORD style = WS_SYSMENU | WS_BORDER | WS_CAPTION | WS_CLIPCHILDREN |
    ...
    //|SWP_NOACTIVATE | SWP_NOCOPYBITS | SWP_NOOWNERZORDER | SWP_NOREPOSITION |
    //SWP_NOZORDER);
  }
}
...
float Win32Window::getTimeInSeconds() { return 0.f; }

void Win32Window::setDebugMessage(int x, int y, const char* message) {}

void Win32Window::setRequestExit() { m_data->m_quit = true; }
bool Win32Window::requestedExit() const { return m_data->m_quit; }
...
void Win32Window::setResizeCallback(b3ResizeCallback resizeCallback) {
  ...
  if (m_data->m_resizeCallback)
    (*m_data->m_resizeCallback)(m_data->m_openglViewportWidth,
                                m_data->m_openglViewportHeight);
}
