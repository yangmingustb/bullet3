...
/// Not Invented Here link reminder
...
static btVector4 sColors[4] = {
    btVector4(1, 0.7, 0.7, 1), btVector4(1, 1, 0.7, 1),
    btVector4(0.7, 1, 0.7, 1), btVector4(0.7, 1, 1, 1),
};

void myNearCallback(plCollisionSdkHandle sdkHandle,
                    plCollisionWorldHandle worldHandle, void* userData,
                    plCollisionObjectHandle objA,
                    plCollisionObjectHandle objB) {
  numNearCallbacks++;
  ...
  if (remainingCapacity > 0) {
    ...
    int numNewPoints = plCollide(sdkHandle, worldHandle, objA, objB, pointPtr,
                                 remainingCapacity);
...
class CollisionTutorialBullet2 : public CommonExampleInterface {
 ...
 public:
    ...
    switch (m_tutorialIndex) {
      ...
      case TUT_SPHERE_PLANE_BULLET2: {
        ...
        if (m_collisionSdkHandle) {
          ...
          m_collisionWorldHandle =
              plCreateCollisionWorld(m_collisionSdkHandle, maxNumObjsCapacity,
          ...
          // create objects, do query etc
          {
            ...
            void* userPointer = 0;
            {
              for (int j = 0; j < sNumCompounds; j++) {
                plCollisionShapeHandle compoundShape = plCreateCompoundShape(
                    m_collisionSdkHandle, m_collisionWorldHandle);
                ...
                for (int i = 0; i < sNumSpheres; i++) {
                  ...
                  plCollisionShapeHandle childShape = plCreateSphereShape(
                      m_collisionSdkHandle, m_collisionWorldHandle, radius);
                  plAddChildShape(m_collisionSdkHandle, m_collisionWorldHandle,
                                  compoundShape, childShape, childPos,
                                  childOrn);
...

                {
                  ...
                  plCollisionObjectHandle colObjHandle =
                      plCreateCollisionObject(
                          m_collisionSdkHandle, m_collisionWorldHandle,
                          userPointer, -1, compoundShape, pos, orn);
                  if (m_tutorialIndex == TUT_SPHERE_PLANE_BULLET2) {
                    btCollisionObject* colObj =
                        (btCollisionObject*)colObjHandle;
                    btVector4 color = sColors[j & 3];
                    m_guiHelper->createCollisionObjectGraphicsObject(colObj,
                                                                     color);
                    colliders.push_back(colObjHandle);
                    plAddCollisionObject(m_collisionSdkHandle,
                                         m_collisionWorldHandle, colObjHandle);
                  }
                }
              }
            }
          ...
          {
            plCollisionShapeHandle colShape = plCreatePlaneShape(
                m_collisionSdkHandle, m_collisionWorldHandle, 0, 1, 0, -3.5);
            btVector3 pos(0, 0, 0);
            btQuaternion orn(0, 0, 0, 1);
            void* userPointer = 0;
            plCollisionObjectHandle colObj = plCreateCollisionObject(
                m_collisionSdkHandle, m_collisionWorldHandle, userPointer, 0,
                colShape, pos, orn);
            colliders.push_back(colObj);
            plAddCollisionObject(m_collisionSdkHandle, m_collisionWorldHandle,
                                 colObj);
          }

          int numContacts =
              plCollide(m_collisionSdkHandle, m_collisionWorldHandle,
                        colliders[0], colliders[1], pointsOut, sPointCapacity);
          printf("numContacts = %d\n", numContacts);
          void* myUserPtr = 0;

          plWorldCollide(m_collisionSdkHandle, m_collisionWorldHandle,
                         myNearCallback, myUserPtr);
        ...
        /*
        m_timeSeriesCanvas0 = new
        TimeSeriesCanvas(m_app->m_2dCanvasInterface,512,256,"Constant
        Velocity");
  ...
  virtual void stepSimulation(float deltaTime) {
    ...
    //	m_app->m_renderer->writeSingleInstanceTransformToCPU(m_bodies[i]->m_worldPose.m_position,
...
class CommonExampleInterface* CollisionTutorialBullet2CreateFunc(
