...
// the current simulation speeds to choose from (the slider will snap to those
// using a custom form of snapping)
...
static double speeds[] = {SimulationSpeeds::PAUSE,
                          SimulationSpeeds::QUARTER_SPEED,
                          SimulationSpeeds::HALF_SPEED,
                          SimulationSpeeds::NORMAL_SPEED,
                          SimulationSpeeds::DOUBLE_SPEED,
                          SimulationSpeeds::QUADRUPLE_SPEED,
                          SimulationSpeeds::DECUPLE_SPEED,
                          SimulationSpeeds::CENTUPLE_SPEED,
                          SimulationSpeeds::QUINCENTUPLE_SPEED,
                          SimulationSpeeds::MILLITUPLE_SPEED};

static btScalar gSolverIterations =
    10;  // default number of solver iterations for the iterative solvers
...
static int gMaxSpeed =
    SimulationSpeeds::MAX_SPEED;  // the maximum simulation speed

static bool gMaximumSpeed = false;  // the demo does not try to achieve maximum
                                    // stepping speed by default

static bool gInterpolate =
    false;  // the demo does not use any bullet interpolated physics substeps

static bool useSplitImpulse = true;  // split impulse fixes issues with
                                     // restitution in Baumgarte stabilization
// http://bulletphysics.org/Bullet/phpBB3/viewtopic.php?f=9&t=7117&p=24631&hilit=Baumgarte#p24631
// disabling continuous collision detection can also fix issues with
// restitution, though CCD is disabled by default an only kicks in at higher
// speeds
// set CCD speed threshold and testing sphere radius per rigidbody
// (rb->setCCDSpeedThreshold())
...
static SolverEnumType SOLVER_TYPE =
    SEQUENTIALIMPULSESOLVER;  // You can switch the solver here

// TODO:s===
// TODO: Give specific explanations about solver values

/**
 * Step size of the bullet physics simulator (solverAccuracy). Accuracy versus
 * speed.
 */
...
// static btScalar gPhysicsStepsPerSecond = 120.0f; // Double steps for more
// accuracy
...
static double fixedPhysicsStepSizeSec =
    1.0f / gPhysicsStepsPerSecond;  // steps size in seconds
static double fixedPhysicsStepSizeMilli =
    1000.0f / gPhysicsStepsPerSecond;  // step size in milliseconds

static btScalar gApplicationFrequency =
    60.0f;  // number of internal application ticks per second
...
inline void twxChangePhysicsStepsPerSecond(
    float physicsStepsPerSecond,
    void*) {  // function to change simulation physics steps per second
...
inline void twxChangeERPCFM(
    float notUsed, void*) {  // function to change ERP/CFM appropriately
  gChangeErpCfm = true;
}

inline void changeSolver(int comboboxId, const char* item,
                         void* userPointer) {  // function to change the solver
  for (int i = 0; i < NUM_SOLVERS; i++) {
    if (strcmp(solverTypes[i], item) == 0) {  // if the strings are equal
      SOLVER_TYPE = ((SolverEnumType)i);
      b3Printf(
          "=%s=\n Reset the simulation by double clicking it in the menu list.",
          item);
      return;
    }
  }
...
inline void twxChangeSolverIterations(
    float notUsed, void* userPtr) {  // change the solver iterations
}

inline void clampToCustomSpeedNotches(
    float speed, void*) {  // function to clamp to custom speed notches
  double minSpeed = 0;
  double minSpeedDist = SimulationSpeeds::MAX_SPEED;
  for (int i = 0; i < SimulationSpeeds::NUM_SPEEDS; i++) {
    double speedDist = (speeds[i] - speed >= 0)
                           ? speeds[i] - speed
                           : speed - speeds[i];  // float absolute
...
inline void switchInterpolated(
    int buttonId, bool buttonState,
    void* userPointer) {  // toggle if interpolation steps are taken
  gInterpolate = !gInterpolate;
  //	b3Printf("Interpolate substeps %s", gInterpolate?"on":"off");
}

inline void switchHeadless(
    int buttonId, bool buttonState,
    void* userPointer) {  // toggle if the demo should run headless
  gIsHeadless = !gIsHeadless;
  //	b3Printf("Run headless %s", gIsHeadless?"on":"off");
}

inline void switchMaximumSpeed(
    int buttonId, bool buttonState,
    void* userPointer) {  // toggle it the demo should run as fast as possible
  //	b3Printf("Run maximum speed %s", gMaximumSpeed?"on":"off");
}

inline void setApplicationTick(float frequency,
                               void*) {  // set internal application tick
  gApplicationTick = 1000.0f / frequency;
}

/**
 * @link: Gaffer on Games - Fix your timestep:
...
struct NN3DWalkersTimeWarpBase : public CommonRigidBodyBase {
  NN3DWalkersTimeWarpBase(struct GUIHelperInterface* helper)
    ...
    mApplicationStart = mLoopTimer
                            .getTimeMilliseconds(); /**!< Initialize when the
                                                       application started
                                                       running */
    mInputClock = mApplicationStart; /**!< Initialize the last time the input
                                        was updated */
    mPreviousModelIteration = mApplicationStart;
    mThisModelIteration = mApplicationStart;
    mApplicationRuntime =
        mThisModelIteration -
        mApplicationStart; /**!< Initialize the application runtime */

    // sub frame time initializations
    mGraphicsStart =
        mApplicationStart; /** !< Initialize the last graphics start */
    ...
    mPhysicsStepStart =
        mApplicationStart; /**!< Initialize the physics step start */
    mPhysicsStepEnd =
        mApplicationStart; /**!< Initialize the physics step end */
    ...
    speedUpPrintTimeStamp =
        mLoopTimer
            .getTimeSeconds();  // timer to print the speed up periodically
    mLoopTimer.reset();
  }
  ...
  void initPhysics() {  // initialize the demo
    ...
    setupBasicParamInterface();  // setup adjustable sliders and buttons for
                                 // parameters
  ...
  void setupBasicParamInterface() {  // setup the adjustable sliders and button
                                     // for parameters

    {  // create a slider to adjust the simulation speed
      // Force increase the simulation speed to run the simulation with the same
      // accuracy but a higher speed
      SliderParams slider("Simulation speed", &gSimulationSpeed);
    ...
    {  // create a button to switch to headless simulation
      // This turns off the graphics update and therefore results in more time
    ...
    {  // create a button to switch to maximum speed simulation (fully
       // deterministic)
    ...
    {  // create a button to switch bullet to perform interpolated substeps to
       // speed up simulation
      // generally, interpolated steps are a good speed-up and should only be
      // avoided if higher accuracy is needed (research purposes etc.)
  ...
  void setupAdvancedParamInterface() {
    ...
    {  // create a slider to adjust the number of internal application ticks
      // The set application tick should contain enough time to perform a full
      // cycle of model update (physics and input)
      // and view update (graphics) with average application load. The graphics
      // and input update determine the remaining time
      // for the physics update
      SliderParams slider("Application Ticks", &gApplicationFrequency);
      slider.m_minVal = gMinSpeed;
      slider.m_maxVal = gMaxSpeed;
    ...
    {  // create a slider to adjust the number of physics steps per second
      // The default number of steps is at 60, which is appropriate for most
      // general simulations
      // For simulations with higher complexity or if you experience undesired
      // behavior, try increasing the number of steps per second
      // Alternatively, try increasing the number of solver iterations if you
      // experience jittering constraints due to non-converging solutions
    ...
    {  // create a slider to adjust the number of solver iterations to converge
       // to a solution
      // more complex simulations might need a higher number of iterations to
      // converge, it also
      // depends on the type of solver.
      SliderParams slider("Solver interations", &gSolverIterations);
      slider.m_minVal = 0;
      slider.m_maxVal = 1000;
      slider.m_callback = twxChangePhysicsStepsPerSecond;
    ...
    {  // create a slider to adjust CFM damper c constant
      SliderParams slider("Global CFM singularity avoidance",
                          &gCFMSingularityAvoidance);
      slider.m_minVal = 0;
      slider.m_maxVal = 10;
      slider.m_callback = twxChangeERPCFM;
      slider.m_clampToNotches = false;
  ...
  void createEmptyDynamicsWorld() {  // create an empty dynamics worlds
                                     // according to the chosen settings via
                                     // statics (top section of code)
    ...
    if (1) {
      ...
      if (SOLVER_TYPE == DANZIGSOLVER || SOLVER_TYPE == GAUSSSEIDELSOLVER) {
        m_dynamicsWorld->getSolverInfo().m_minimumSolverBatchSize =
            1;  // for mlcp solver it is better to have a small A matrix
      } else {
        m_dynamicsWorld->getSolverInfo().m_minimumSolverBatchSize =
            128;  // for direct solver, it is better to solve multiple objects
                  // together, small batches have high overhead
      }

      m_dynamicsWorld->getDispatchInfo().m_useContinuous =
          true;  // set continuous collision
    } else {
      // use btMultiBodyDynamicsWorld for Featherstone btMultiBody support
      m_dynamicsWorld = new btMultiBodyDynamicsWorld(
          m_dispatcher, m_broadphase, (btMultiBodyConstraintSolver*)m_solver,
          m_collisionConfiguration);
    }

    changeERPCFM();  // set appropriate ERP/CFM values according to the string
                     // and damper properties of the constraint

    if (useSplitImpulse) {  // If you experience strong repulsion forces in your
                            // constraints, it might help to enable the split
                            // impulse feature
      m_dynamicsWorld->getSolverInfo().m_splitImpulse =
          1;  // enable split impulse feature
      ...
      //			BulletUtils::getERP(fixedPhysicsStepSizeSec, 10,
      //1);
      //			b3Printf("Using split impulse feature with
      //ERP/TurnERP: (%f,%f)",
      //				m_dynamicsWorld->getSolverInfo().m_erp2,
      //				m_dynamicsWorld->getSolverInfo().m_splitImpulseTurnErp);
    }
    ...
    m_dynamicsWorld->getSolverInfo().m_numIterations =
        gSolverIterations;  // set the number of solver iterations for iteration
                            // based solvers

    m_dynamicsWorld->setGravity(
        btVector3(0, -9.81f, 0));  // set gravity to -9.81
  }

  btScalar calculatePerformedSpeedup() {  // calculate performed speedup
    // we calculate the performed speed up
    btScalar speedUp =
        ((double)performedTime * 1000.0) /
        ((double)(mLoopTimer.getTimeMilliseconds() - performanceTimestamp));
  ...
  void stepSimulation(float deltaTime) {  // customly step the simulation
    do {
      ...
      //#############
      // model update - here you perform updates of your model, be it the
      // physics model, the game or simulation state or anything not related to
      // graphics and input
      ...
      if (mLoopTimer.getTimeSeconds() - speedUpPrintTimeStamp > 1) {
        ...
        // double speedUp =
        // ((double)performedTime*1000.0)/((double)(mLoopTimer.getTimeMilliseconds()-performanceTimestamp));
        //				b3Printf("Avg Effective speedup:
        //%f",speedUp);
        performedTime = 0;
        performanceTimestamp = mLoopTimer.getTimeMilliseconds();
      ...
      mFrameTime = mThisModelIteration -
                   mPreviousModelIteration; /**!< Calculate the frame time (in
                                               Milliseconds) */
      ...
      mApplicationRuntime =
          mThisModelIteration -
          mApplicationStart; /**!< Update main frame timer (in Milliseconds) */

      mModelStart =
          mLoopTimer.getTimeMilliseconds(); /**!< Begin with the model update
                                               (in Milliseconds)*/
      mLastGraphicsTick =
          mModelStart -
          mGraphicsStart; /**!< Update graphics timer (in Milliseconds) */

      if (gMaximumSpeed /** If maximum speed is enabled*/) {
        performMaxStep();
      } else { /**!< This mode tries to progress as much time as it is expected
                  from the game loop*/
        performSpeedStep();
      }

      mInputStart =
          mLoopTimer.getTimeMilliseconds(); /**!< Start the input update */
      mLastModelTick =
          mInputStart -
          mModelStart; /**!< Calculate the time the model update took */
      ...
      if (mInputDt >= gApplicationTick) {
        ...
        //	         mInputHandler.injectInput(); /**!< Inject input into
        //handlers */
        //	         mInputHandler.update(mInputClock); /**!< update
        //elements that work on the current input state */
      }

      mGraphicsStart =
          mLoopTimer.getTimeMilliseconds(); /**!< Start the graphics update */
      mLastInputTick =
          mGraphicsStart -
          mInputStart; /**!< Calculate the time the input injection took */

      //#############
      // Graphics update - Here you perform the representation of your model,
      // meaning graphics rendering according to what your game or simulation
      // model describes
      // In the example browser, there is a separate method called renderScene()
      // for this

      // Uncomment this for some detailed output about the application ticks
      //	b3Printf(
      //		"Physics time: %u milliseconds / Graphics time: %u
      //milliseconds / Input time: %u milliseconds / Total time passed: %u
      //milliseconds",
      //		mLastModelTick, mLastGraphicsTick, mLastInputTick,
      //mApplicationRuntime);

    } while (mLoopTimer.getTimeMilliseconds() - fpsTimeStamp <
             fpsStep);  // escape the loop if it is time to render
    // Unfortunately, the input is not included in the loop, therefore the input
    // update frequency is equal to the fps
  ...
  void changePhysicsStepsPerSecond(
      float physicsStepsPerSecond) {  // change the simulation accuracy
  ...
  void changeERPCFM() {  // Change ERP/CFM appropriately to the timestep and the
                         // ERP/CFM parameters above
    if (m_dynamicsWorld) {
      m_dynamicsWorld->getSolverInfo()
          .m_erp = b3ERPCFMHelper::getERP(  // set the error reduction parameter
      ...
      m_dynamicsWorld->getSolverInfo()
          .m_globalCfm = b3ERPCFMHelper::getCFM(  // set the constraint force
                                                  // mixing according to the
                                                  // time step
          gCFMSingularityAvoidance,  // singularity avoidance (if you experience
                                     // unsolvable constraints, increase this
                                     // value
  ...
  void changeSolverIterations(
      int iterations) {  // change the number of iterations
  ...
  void performTrueSteps(
      btScalar timeStep) {  // physics stepping without interpolated substeps
    int subSteps = floor((timeStep / fixedPhysicsStepSizeSec) +
                         0.5); /**!< Calculate the number of full normal time
                                  steps we can take */

    for (int i = 0; i < subSteps;
         i++) { /**!< Perform the number of substeps to reach the timestep*/
      if (timeStep && m_dynamicsWorld) {
        // since we want to perform all proper steps, we perform no interpolated
        // substeps
        int subSteps = 1;

        m_dynamicsWorld->stepSimulation(btScalar(timeStep), btScalar(subSteps),
                                        btScalar(fixedPhysicsStepSizeSec));
      }
    }
  }

  void performInterpolatedSteps(
      btScalar timeStep) {  // physics stepping with interpolated substeps
    int subSteps = 1 +
                   floor((timeStep / fixedPhysicsStepSizeSec) +
                         0.5); /**!< Calculate the number of full normal time
                                  steps we can take, plus 1 for safety of not
                                  losing time */
    if (timeStep && m_dynamicsWorld) {
      m_dynamicsWorld->stepSimulation(
          btScalar(timeStep), btScalar(subSteps),
          btScalar(fixedPhysicsStepSizeSec)); /**!< Perform the number of
                                                 substeps to reach the
                                                 timestep*/
    }
  }
  ...
  void performMaxStep() {  // perform as many steps as possible
    if (gApplicationTick >=
        mLastGraphicsTick + mLastInputTick) {  // if the remaining time for
                                               // graphics is going to be
                                               // positive
      mPhysicsTick = gApplicationTick /**!< calculate the remaining time for
                                         physics (in Milliseconds) */
                     - mLastGraphicsTick - mLastInputTick;
    } else {
      mPhysicsTick = 0;  // no time for physics left / The internal application
                         // step is too high
    }
    ...
    if (mPhysicsTick > 0) {  // with positive physics tick we perform as many
                             // update steps until the time for it is used up

      mPhysicsStepStart =
          mLoopTimer
              .getTimeMilliseconds(); /**!< The physics updates start (in
                                         Milliseconds)*/
      mPhysicsStepEnd = mPhysicsStepStart;

      while (mPhysicsTick >
             mPhysicsStepEnd - mPhysicsStepStart) { /**!< Update the physics
                                                       until we run out of time
                                                       (in Milliseconds) */
        //			b3Printf("Physics passed: %u", mPhysicsStepEnd -
        //mPhysicsStepStart);
        double timeStep =
            fixedPhysicsStepSizeSec; /**!< update the world (in Seconds) */
        ...
        mPhysicsStepEnd = mLoopTimer
                              .getTimeMilliseconds(); /**!< Update the last
                                                         physics step end to
                                                         stop updating in time
                                                         (in Milliseconds) */
      }
    }
  }

  void performSpeedStep() {  // force-perform the number of steps needed to
                             // achieve a certain speed (safe to too high
                             // speeds, meaning the application will lose time,
                             // not the physics)
    if (mFrameTime > gApplicationTick) { /** cap frametime to make the
                                            application lose time, not the
                                            physics (in Milliseconds) */
      mFrameTime = gApplicationTick;     // This prevents the physics time
                                         // accumulator to sum up too much time
    }  // The simulation therefore gets slower, but still performs all requested
       // physics steps

    mModelAccumulator += mFrameTime; /**!< Accumulate the time the physics
                                        simulation has to perform in order to
                                        stay in real-time (in Milliseconds) */
    //	b3Printf("Model time accumulator: %u", mModelAccumulator);

    int steps =
        floor(mModelAccumulator / fixedPhysicsStepSizeMilli); /**!< Calculate
                                                                 the number of
                                                                 time steps we
                                                                 can take */
    ...
    if (steps > 0) { /**!< Update if we can take at least one step */
      ...
      double timeStep =
          gSimulationSpeed * steps *
          fixedPhysicsStepSizeSec; /**!< update the universe (in Seconds) */
      ...
      performedTime +=
          timeStep;  // sum up the performed time for measuring the speed up
      mModelAccumulator -=
          steps *
          fixedPhysicsStepSizeMilli; /**!< Remove the time performed by the
                                        physics simulation from the accumulator,
                                        the remaining time carries over to the
                                        next cycle  (in Milliseconds) */
    }
  }

  void renderScene() {   // render the scene
    if (!gIsHeadless) {  // while the simulation is not running headlessly,
                         // render to screen
  ...
  unsigned long int mApplicationStart; /**!< The time the application was
                                          started (absolute, in Milliseconds) */
  unsigned long int
      mPreviousModelIteration; /**!< The previous model iteration timestamp
                                  (absolute, in Milliseconds) */
  unsigned long int mThisModelIteration; /**!< This model iteration timestamp
                                            (absolute, in Milliseconds) */

  //# loop durations
  long int mModelAccumulator;   /**!< The time to forward the model in this loop
                                   iteration (relative, in Milliseconds) */
  unsigned long int mFrameTime; /**!< The time to render a frame (relative, in
                                   Milliseconds) */
  ...
  long int mLastGraphicsTick; /*!< The time it took the graphics rendering last
                                 time (relative, in Milliseconds) */
  unsigned long int mGraphicsStart;

  long int
      mLastInputTick; /**!< The time it took the input to process last time
                         (relative, in Milliseconds) */
...
This includes the bullet physics update */
  unsigned long int
      mModelStart; /**!< The timestamp the model started updating last
                      (absolute, in Milliseconds)*/

  long int mPhysicsTick; /**!< The time remaining in the loop to update the
                            physics (relative, in Milliseconds)*/
  unsigned long int mPhysicsStepStart; /**!< The physics start timestamp
                                          (absolute, in Milliseconds) */
  unsigned long int
      mPhysicsStepEnd; /**!< The last physics step end (absolute, in
                          Milliseconds) */
