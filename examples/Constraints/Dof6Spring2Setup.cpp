...
struct Dof6Spring2SetupInternalData {
  ...
  Dof6Spring2SetupInternalData() : mDt(1. / 60.), frameID(0) {}
};
...
void Dof6Spring2Setup::initPhysics() {
  ...
  // Setup a big ground box
  {
    btCollisionShape* groundShape =
        new btBoxShape(btVector3(btScalar(200.), btScalar(5.), btScalar(200.)));
...
#define CREATE_GROUND_COLLISION_OBJECT 1
  ...
  /////////// the box should oscillate left-to-right forever
  {
    ...
    localB.setIdentity();
    constraint =
        new CONSTRAINT_TYPE(*staticBody, *m_data->m_TranslateSpringBody, localA,
                            localB EXTRAPARAMS);
  ...
  /////////// box should swing (rotate) left-to-right forever
  {
    ...
    constraint = new CONSTRAINT_TYPE(*staticBody, *m_data->m_RotateSpringBody,
                                     localA, localB EXTRAPARAMS);
  ...
  /////////// box with bouncing constraint, translation is bounced at the
  ///positive x limit, but not at the negative limit
  /////////// bouncing can not be set independently at low and high limits, so
  ///two constraints will be created: one that defines the low (non bouncing)
  ///limit, and one that defines the high (bouncing) limit
  /////////// the box should move to the left (as an impulse will be applied to
  ///it periodically) until it reaches its limit, then bounce back
  {
    mass = 1.0;
    shape = new btBoxShape(btVector3(0.5, 0.5, 0.5));
    shape->calculateLocalInertia(mass, localInertia);
    ...
    localB.setIdentity();
    constraint =
        new CONSTRAINT_TYPE(*staticBody, *m_data->m_BouncingTranslateBody,
                            localA, localB EXTRAPARAMS);
...
#else  // bounce is named restitution in 6dofspring, but not implemented for
       // translational limit motor, so the following line has no effect
  ...
  /////////// the box should rotate around the y axis
  {
    ...
    localB.setIdentity();
    constraint = new CONSTRAINT_TYPE(*staticBody, *m_data->m_MotorBody, localA,
                                     localB EXTRAPARAMS);
    constraint->setLimit(0, 0, 0);
    constraint->setLimit(1, 0, 0);
    constraint->setLimit(2, 0, 0);
  ...
  /////////// the box should rotate around the y axis until it reaches its
  ///target
  /////////// the target will be negated periodically
  {
    ...
    localB.setIdentity();
    constraint = new CONSTRAINT_TYPE(*staticBody, *m_data->m_ServoMotorBody,
                                     localA, localB EXTRAPARAMS);
    constraint->setLimit(0, 0, 0);
    constraint->setLimit(1, 0, 0);
    constraint->setLimit(2, 0, 0);
...
// servo motor is not implemented in 6dofspring constraint
  ...
  ////////// chain of boxes linked together with fully limited rotational and
  ///translational constraints
  ////////// the chain will be pulled to the left and to the right periodically.
  ///They should strictly stick together.
  {
    ...
    for (int i = 0; i < bodycount; ++i) {
      ...
      if (prevBody != 0) {
        ...
        CONSTRAINT_TYPE* constraint =
            new CONSTRAINT_TYPE(*prevBody, *body, localA, localB EXTRAPARAMS);
        ...
        if (i < bodycount - 1) {
          ...
          localB.setIdentity();
          CONSTRAINT_TYPE* constraintZY = new CONSTRAINT_TYPE(
              *staticBody, *body, localA, localB EXTRAPARAMS);
        ...
        m_data->m_ChainLeftConstraint =
            new CONSTRAINT_TYPE(*staticBody, *body, localA, localB EXTRAPARAMS);
        ...
        for (int a = 0; a < 6; ++a) {
          ...
          m_data->m_ChainLeftConstraint->setParam(BT_CONSTRAINT_STOP_CFM, 0.0,
                                                  a);
        }
    ...
    localB.setIdentity();
    localB.setOrigin(btVector3(0, 0, 0));
    m_data->m_ChainRightConstraint = new CONSTRAINT_TYPE(
        *staticBody, *m_data->m_ChainRightBody, localA, localB EXTRAPARAMS);
...
/////// servo motor: flip its target periodically
  ...
  if (servoNextFrame < 0) {
    m_data->m_ServoMotorConstraint->getRotationalLimitMotor(2)->m_servoTarget *=
        -1;
    servoNextFrame = 3.0;
  }
...
class CommonExampleInterface* Dof6Spring2CreateFunc(
