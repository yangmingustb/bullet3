...
/// AllConstraintDemo shows how to create a constraint, like Hinge or
/// btGenericD6constraint
...
void AllConstraintDemo::initPhysics() {
  ...
  // btBoxShape(btVector3(btScalar(50.),btScalar(40.),btScalar(50.)));
  btCollisionShape* groundShape =
      new btStaticPlaneShape(btVector3(0, 1, 0), 40);
...
#define RATIO L_2 / L_1
...

  {
    ...
    btRigidBody* body = new btRigidBody(ci);  // 1,0,cyl0,localInertia);
    body->setLinearFactor(btVector3(0, 0, 0));
    btHingeConstraint* hinge = new btHingeConstraint(*body, btVector3(0, 0, 0),
                                                     btVector3(0, 1, 0), true);
  ...
  btGearConstraint* gear =
      new btGearConstraint(*bodyA, *bodyB, axisA, axisB, RATIO);
  ...
  // point to point constraint (ball socket)
  {
    ...
    btVector3 pivotInA(CUBE_HALF_EXTENTS, -CUBE_HALF_EXTENTS,
                       -CUBE_HALF_EXTENTS);
    btVector3 axisInA(0, 0, 1);

    //	btVector3 pivotInB = body1 ?
    //body1->getCenterOfMassTransform().inverse()(body0->getCenterOfMassTransform()(pivotInA))
    //: pivotInA;
    //		btVector3 axisInB = body1?
    //			(body1->getCenterOfMassTransform().getBasis().inverse()*(body1->getCenterOfMassTransform().getBasis()
    //* axisInA)) :
...
#define P2P
#ifdef P2P
    btTypedConstraint* p2p = new btPoint2PointConstraint(*body0, pivotInA);
    // btTypedConstraint* p2p = new
    // btPoint2PointConstraint(*body0,*body1,pivotInA,pivotInB);
    // btTypedConstraint* hinge = new
    // btHingeConstraint(*body0,*body1,pivotInA,pivotInB,axisInA,axisInB);
    m_dynamicsWorld->addConstraint(p2p);
    p2p->setDbgDrawSize(btScalar(5.f));
#else
    btHingeConstraint* hinge = new btHingeConstraint(*body0, pivotInA, axisInA);

    // use zero targetVelocity and a small maxMotorImpulse to simulate joint
    // friction
...
#if ENABLE_ALL_DEMOS
  {
    ...
    btSliderConstraint* spSlider1 =
        new btSliderConstraint(*pRbA1, *pRbB1, frameInA, frameInB, true);
    //	spSlider1 = new btSliderConstraint(*pRbA1, *pRbB1, frameInA, frameInB,
    //false);
  ...
  // create a slider, using the generic D6 constraint
  {
    ...
    //		bool useLinearReferenceFrameA = false;//use fixed frame B for linear
    //llimits
    bool useLinearReferenceFrameA =
        true;  // use fixed frame A for linear llimits
    spSlider6Dof = new btGeneric6DofConstraint(
        *fixedBody1, *d6body0, frameInA, frameInB, useLinearReferenceFrameA);
    spSlider6Dof->setLinearLowerLimit(lowerSliderLimit);
    spSlider6Dof->setLinearUpperLimit(hiSliderLimit);

    // range should be small, otherwise singularities will 'explode' the
    // constraint
  ...
  {  // create a door using hinge constraint attached to the world
    ...
    const btVector3 btPivotA(10.f + 2.1f, -2.0f,
                             0.0f);  // right next to the door slightly outside
    ...
    //		spDoorHinge->setLimit( -SIMD_PI*0.8f, SIMD_PI, 0.9f, 0.3f,
    //0.0f);
    //		spDoorHinge->setLimit( -SIMD_PI*0.8f, SIMD_PI, 0.9f, 0.01f, 0.0f); //
    //"sticky limits"
  ...
  {  // create a generic 6DOF constraint
    ...
    btGeneric6DofConstraint* pGen6DOF =
        new btGeneric6DofConstraint(*pBodyA, *pBodyB, frameInA, frameInB, true);
    //		btGeneric6DofConstraint* pGen6DOF = new
    //btGeneric6DofConstraint(*pBodyA, *pBodyB, frameInA, frameInB, false);
    ...
    //		pGen6DOF->getTranslationalLimitMotor()->m_targetVelocity[0] =
    //5.0f;
    //		pGen6DOF->getTranslationalLimitMotor()->m_maxMotorForce[0] =
    //6.0f;

    //		pGen6DOF->setAngularLowerLimit(btVector3(0., SIMD_HALF_PI*0.9,
    //0.));
    //		pGen6DOF->setAngularUpperLimit(btVector3(0., -SIMD_HALF_PI*0.9,
    //0.));
    //		pGen6DOF->setAngularLowerLimit(btVector3(0., 0.,
    //-SIMD_HALF_PI));
    //		pGen6DOF->setAngularUpperLimit(btVector3(0., 0., SIMD_HALF_PI));

    pGen6DOF->setAngularLowerLimit(
        btVector3(-SIMD_HALF_PI * 0.5f, -0.75, -SIMD_HALF_PI * 0.8f));
    pGen6DOF->setAngularUpperLimit(
        btVector3(SIMD_HALF_PI * 0.5f, 0.75, SIMD_HALF_PI * 0.8f));
    //		pGen6DOF->setAngularLowerLimit(btVector3(0.f, -0.75, SIMD_HALF_PI *
    //0.8f));
    //		pGen6DOF->setAngularUpperLimit(btVector3(0.f, 0.75, -SIMD_HALF_PI *
    //0.8f));
    //		pGen6DOF->setAngularLowerLimit(btVector3(0.f, -SIMD_HALF_PI * 0.8f,
    //SIMD_HALF_PI * 1.98f));
    //		pGen6DOF->setAngularUpperLimit(btVector3(0.f, SIMD_HALF_PI * 0.8f,
    //-SIMD_HALF_PI * 1.98f));
  ...
  {  // create a ConeTwist constraint
    ...
    //		m_ctc->setLimit(btScalar(SIMD_PI_4), btScalar(SIMD_PI_4),
    //btScalar(SIMD_PI) * 0.8f);
    //		m_ctc->setLimit(btScalar(SIMD_PI_4*0.6f), btScalar(SIMD_PI_4),
    //btScalar(SIMD_PI) * 0.8f, 1.0f); // soft limit == hard limit
    m_ctc->setLimit(btScalar(SIMD_PI_4 * 0.6f), btScalar(SIMD_PI_4),
                    btScalar(SIMD_PI) * 0.8f, 0.5f);
...
#endif
#if ENABLE_ALL_DEMOS
  {  // Hinge connected to the world, with motor (to hinge motor with new and
     // old constraint solver)
    btTransform tr;
    tr.setIdentity();
    ...
    btHingeConstraint* pHinge =
        new btHingeConstraint(*pBody, btPivotA, btAxisA);
    //		pHinge->enableAngularMotor(true, -1.0, 0.165); // use for the old
    //solver
    pHinge->enableAngularMotor(true, -1.0f,
                               1.65f);  // use for the new SIMD solver
...
#if ENABLE_ALL_DEMOS
  {
    ...
    btUniversalConstraint* pUniv = new btUniversalConstraint(
        *pBodyA, *pBodyB, anchor, parentAxis, childAxis);
  ...
  {  // create a generic 6DOF constraint with springs
    ...
    btGeneric6DofSpringConstraint* pGen6DOFSpring =
        new btGeneric6DofSpringConstraint(*pBodyA, *pBodyB, frameInA, frameInB,
                                          true);
...
#if ENABLE_ALL_DEMOS
  {
    ...
    spHingeDynAB =
        new btHingeConstraint(*pBodyA, *pBodyB, pivotA, pivotB, axisA, axisB);
  ...
  {  // 6DOF connected to the world, with motor
    ...
    frameB.setIdentity();
    btGeneric6DofConstraint* pGen6Dof =
        new btGeneric6DofConstraint(*pBody, frameB, false);
...
class CommonExampleInterface* AllConstraintCreateFunc(
