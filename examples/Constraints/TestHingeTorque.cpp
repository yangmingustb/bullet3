...
int collisionFilterMask =
    int(btBroadphaseProxy::AllFilter ^ (btBroadphaseProxy::CharacterFilter));
...
TestHingeTorque::TestHingeTorque(struct GUIHelperInterface* helper)
    : CommonRigidBodyBase(helper), m_once(true) {}
...
void TestHingeTorque::stepSimulation(float deltaTime) {
  if (0)  // m_once)
  {
    ...
    btHingeConstraint* hinge =
        (btHingeConstraint*)m_dynamicsWorld->getConstraint(0);

    btRigidBody& bodyA = hinge->getRigidBodyA();
    btTransform trA = bodyA.getWorldTransform();
    btVector3 hingeAxisInWorld =
        trA.getBasis() * hinge->getFrameOffsetA().getBasis().getColumn(2);
  ...
  if ((count & 0x0f) == 0) {
    btRigidBody* base =
        btRigidBody::upcast(m_dynamicsWorld->getCollisionObjectArray()[0]);
    ...
    btRigidBody* child =
        btRigidBody::upcast(m_dynamicsWorld->getCollisionObjectArray()[1]);
    ...
    for (int i = 0; i < m_jointFeedback.size(); i++) {
      b3Printf(
          "Applied force at the COM/Inertial frame B[%d]:(%f,%f,%f), torque "
          "B:(%f,%f,%f)\n",
          i,
...
void TestHingeTorque::initPhysics() {
  ...
  {  // create a door using hinge constraint attached to the world
    ...
    base->setDamping(0, 0);
    m_dynamicsWorld->addRigidBody(base, collisionFilterGroup,
                                  collisionFilterMask);
    ...
    for (int i = 0; i < numLinks; i++) {
      ...
      linkTrans.setOrigin(basePosition -
                          btVector3(0, linkHalfExtents[1] * 2.f * (i + 1), 0));
      ...
      m_dynamicsWorld->addRigidBody(linkBody, collisionFilterGroup,
                                    collisionFilterMask);
      ...
      if (i == 0) {
        ...
        btHingeConstraint* hinge =
            new btHingeConstraint(*prevBody, *linkBody, pivotInA, pivotInB,
        ...
        btGeneric6DofSpring2Constraint* fixed =
            new btGeneric6DofSpring2Constraint(*prevBody, *linkBody, pivotInA,
                                               pivotInB);
