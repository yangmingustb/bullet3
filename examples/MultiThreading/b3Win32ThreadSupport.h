...
class b3Win32ThreadSupport : public b3ThreadSupportInterface {
 ...
 public:
  ...
  /// start the spus (can be called at the beginning of each frame, to make sure
  /// that the right SPU program is loaded)
  virtual void startThreads();

  /// tell the task scheduler we are done with the SPU tasks
  virtual void stopThreads();

  virtual void setNumTasks(int numTasks) { m_maxNumTasks = numTasks; }

  virtual int getNumTasks() const { return m_maxNumTasks; }
