...
// b3PosixThreadSupport helps to initialize/shutdown libspe2, start/stop SPU
// tasks and communication
class b3PosixThreadSupport : public b3ThreadSupportInterface {
 public:
  typedef enum sStatus { STATUS_BUSY, STATUS_READY, STATUS_FINISHED } Status;
 ...
 public:
  ...
  virtual int getNumTasks() const { return m_activeThreadStatus.size(); }

  /// non-blocking test if a task is completed. First implement all versions,
  /// and then enable this API
  virtual bool isTaskCompleted(int* puiArgument0, int* puiArgument1,
