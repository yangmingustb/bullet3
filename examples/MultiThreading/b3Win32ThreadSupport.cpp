...
///@todo: each worker should be linked to a single core, using
///SetThreadIdealProcessor.

/// b3Win32ThreadSupport helps to initialize/shutdown libspe2, start/stop SPU
/// tasks and communication
/// Setup and initialize SPU/CELL/Libspe2
b3Win32ThreadSupport::b3Win32ThreadSupport(
    const Win32ThreadConstructionInfo& threadConstructionInfo) {
  m_maxNumTasks = threadConstructionInfo.m_numThreads;
  startThreads(threadConstructionInfo);
}

/// cleanup/shutdown Libspe2
b3Win32ThreadSupport::~b3Win32ThreadSupport() { stopThreads(); }

#include <stdio.h>

DWORD WINAPI Thread_no_1(LPVOID lpParam) {
  b3Win32ThreadSupport::b3ThreadStatus* status =
      (b3Win32ThreadSupport::b3ThreadStatus*)lpParam;
...
void b3Win32ThreadSupport::runTask(int uiCommand, void* uiArgument0,
                                   int taskId) {
  ///	gMidphaseSPU.sendRequest(CMD_GATHER_AND_PROCESS_PAIRLIST, (void*)
  ///&taskDesc);

  /// we should spawn an SPU task here, and in 'waitForResponse' it should wait
  /// for response of the (one of) the first tasks that finished
...
void b3Win32ThreadSupport::waitForResponse(int* puiArgument0,
                                           int* puiArgument1) {
  /// We should wait for (one of) the first tasks to finish (or other SPU
  /// messages), and report its response

  /// A possible response can be 'yes, SPU handled it', or 'no, please do a PPU
  /// fallback'
  ...
  DWORD res = WaitForMultipleObjects(m_completeHandles.size(),
                                     &m_completeHandles[0], FALSE, INFINITE);
...
/// check for messages from SPUs
bool b3Win32ThreadSupport::isTaskCompleted(int* puiArgument0, int* puiArgument1,
...
#ifndef SINGLE_THREADED
  DWORD res =
      WaitForMultipleObjects(m_completeHandles.size(), &m_completeHandles[0],
                             FALSE, timeOutInMilliseconds);
...
void b3Win32ThreadSupport::startThreads(
  ...
  for (int i = 0; i < threadConstructionInfo.m_numThreads; i++) {
    ...
    sprintf(threadStatus.m_eventStartHandleName, "es%.8s%d%d",
            threadConstructionInfo.m_uniqueName, uniqueId, i);
    threadStatus.m_eventStartHandle =
        CreateEventA(0, false, false, threadStatus.m_eventStartHandleName);

    sprintf(threadStatus.m_eventCompletetHandleName, "ec%.8s%d%d",
            threadConstructionInfo.m_uniqueName, uniqueId, i);
    threadStatus.m_eventCompletetHandle =
        CreateEventA(0, false, false, threadStatus.m_eventCompletetHandleName);
    ...
    // SetThreadAffinityMask(handle, 1 << 1); // this is what it was doing
    // originally, a complete disaster for threading performance!
    // SetThreadAffinityMask(handle, 1 << i); // I'm guessing this was the
    // intention, but is still bad for performance due to one of the threads
    //  sometimes unable to execute because it wants to be on the same processor
    //  as the main thread (my guess)
    ...
    threadStatus.m_lsMemoryReleaseFunc =
        threadConstructionInfo.m_lsMemoryReleaseFunc;
...
class b3Win32Barrier : public b3Barrier {
 ...
 public:
  ...
  void sync() {
    ...
    // PFX_PRINTF("enter taskId %d count %d stage %d phase %d mEnableCounter
    ...
    // PFX_PRINTF("leave taskId %d count %d stage %d phase %d mEnableCounter
    // %d\n",taskId,mCounter,debug&0xff,debug>>16,mEnableCounter);
...
class b3Win32CriticalSection : public b3CriticalSection {
 ...
 public:
  b3Win32CriticalSection() { InitializeCriticalSection(&mCriticalSection); }

  ~b3Win32CriticalSection() { DeleteCriticalSection(&mCriticalSection); }
...
b3Barrier* b3Win32ThreadSupport::createBarrier() {
  unsigned char* mem =
      (unsigned char*)b3AlignedAlloc(sizeof(b3Win32Barrier), 16);
...
b3CriticalSection* b3Win32ThreadSupport::createCriticalSection() {
  unsigned char* mem =
      (unsigned char*)b3AlignedAlloc(sizeof(b3Win32CriticalSection), 16);
...
void b3Win32ThreadSupport::deleteCriticalSection(
    b3CriticalSection* criticalSection) {
