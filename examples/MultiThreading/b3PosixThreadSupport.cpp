...
b3PosixThreadSupport::b3PosixThreadSupport(
    ThreadConstructionInfo& threadConstructionInfo) {
  startThreads(threadConstructionInfo);
}

// cleanup/shutdown Libspe2
b3PosixThreadSupport::~b3PosixThreadSupport() { stopThreads(); }
...
static void* threadFunction(void* argument) {
  b3PosixThreadSupport::b3ThreadStatus* status =
      (b3PosixThreadSupport::b3ThreadStatus*)argument;
...
void b3PosixThreadSupport::runTask(int uiCommand, void* uiArgument0,
                                   int taskId) {
  ///	gMidphaseSPU.sendRequest(CMD_GATHER_AND_PROCESS_PAIRLIST, (int)
  ///&taskDesc);
...
/// non-blocking test if a task is completed. First implement all versions, and
/// then enable this API
bool b3PosixThreadSupport::isTaskCompleted(int* puiArgument0, int* puiArgument1,
...
void b3PosixThreadSupport::waitForResponse(int* puiArgument0,
                                           int* puiArgument1) {
...
void b3PosixThreadSupport::startThreads(
    ThreadConstructionInfo& threadConstructionInfo) {
  ...
  for (int i = 0; i < threadConstructionInfo.m_numThreads; i++) {
    ...
    checkPThreadFunction(pthread_create(&spuStatus.thread, NULL,
                                        &threadFunction, (void*)&spuStatus));
    ...
    spuStatus.m_lsMemoryReleaseFunc =
...
class b3PosixCriticalSection : public b3CriticalSection {
 ...
 public:
  b3PosixCriticalSection() { pthread_mutex_init(&m_mutex, NULL); }
  virtual ~b3PosixCriticalSection() { pthread_mutex_destroy(&m_mutex); }
...
class b3PosixBarrier : public b3Barrier {
 ...
 public:
  b3PosixBarrier() : m_numThreads(0) {}
  virtual ~b3PosixBarrier() { pthread_barrier_destroy(&m_barr); }
  ...
  virtual int getMaxCount() { return m_numThreads; }
};
#else
...
void b3PosixThreadSupport::deleteBarrier(b3Barrier* barrier) { delete barrier; }
