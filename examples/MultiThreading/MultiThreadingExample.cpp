...
struct SampleJob1 : public SampleJobInterface {
  ...
  SampleJob1(int jobId) : m_fakeWork(0), m_jobId(jobId) {}
  virtual ~SampleJob1() {}

  virtual void executeJob(int threadIndex) {
    printf("start SampleJob1 %d using threadIndex %d\n", m_jobId, threadIndex);
    // do some fake work
    for (int i = 0; i < 1000000; i++) m_fakeWork = b3Scalar(1.21) * m_fakeWork;
  ...
  virtual void initPhysics() {
    ...
    for (int i = 0; i < m_threadSupport->getNumTasks(); i++) {
      SampleThreadLocalStorage* storage =
          (SampleThreadLocalStorage*)m_threadSupport->getThreadLocalMemory(i);
      b3Assert(storage);
      storage->threadId = i;
    }
...
class CommonExampleInterface* MultiThreadingExampleCreateFunc(
