...
/// Under Windows it uses Win32 Threads. On Mac and Linux it uses pthreads. On
/// PlayStation 3 Cell SPU it uses SPURS.
...
b3ThreadSupportInterface* createThreadSupport(int numThreads) {
  b3PosixThreadSupport::ThreadConstructionInfo constructionInfo(
      ...
      SamplelsMemoryReleaseFunc, numThreads);
...
b3ThreadSupportInterface* createThreadSupport(int numThreads) {
  b3Win32ThreadSupport::Win32ThreadConstructionInfo threadConstructionInfo(
      "testThreads", SampleThreadFunc, SamplelsMemoryFunc, numThreads);
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < threadSupport->getNumTasks(); i++) {
    SampleThreadLocalStorage* storage =
        (SampleThreadLocalStorage*)threadSupport->getThreadLocalMemory(i);
    b3Assert(storage);
    storage->threadId = i;
  }
