...
MyDebugDrawer : public btIDebugDraw {
 ...
 public:
  ...
  virtual DefaultColors getDefaultColors() const { return m_ourColors; }
  /// the default implementation for setDefaultColors has no effect. A derived
  /// class can implement it and store the colors.
  virtual void setDefaultColors(const DefaultColors& colors) {
    m_ourColors = colors;
  }

  virtual void drawLine(const btVector3& from1, const btVector3& to1,
    ...
    if (m_currentLineColor != color1 ||
        m_linePoints.size() >= BT_LINE_BATCH_SIZE) {
      flushLines();
  ...
  virtual void flushLines() {
    ...
    if (sz) {
      ...
      debugColor[3] = 1.f;
      m_glApp->m_renderer->drawLines(
          &m_linePoints[0].x, debugColor, m_linePoints.size(),
          sizeof(MyDebugVec3), &m_lineIndices[0], m_lineIndices.size(), 1);
      m_linePoints.clear();
      m_lineIndices.clear();
    }
  }
...
struct MyHashShape {
  ...
  bool equals(const MyHashShape& other) const {
    ...
    return sameShapeType && sameSphere0 && sameSphere1 && sameHalfExtents &&
           sameRadius0 && sameRadius1 && sameTransform && sameUpAxis &&
           sameHalfHeight;
  }
  // to our success
...
const struct CommonRenderInterface* OpenGLGuiHelper::getRenderInterface()
    const {
  return m_data->m_glApp->m_renderer;
}

void OpenGLGuiHelper::createRigidBodyGraphicsObject(btRigidBody* body,
 ...
 public:
  ...
  virtual void processTriangle(btVector3* tris, int partId, int triangleIndex) {
    for (int k = 0; k < 3; k++) {
      ...
      v.uv[0] =
          (1. - ((v.xyzw[0] - m_aabbMin[0]) / (m_aabbMax[0] - m_aabbMin[0]))) *
          m_textureScaling;
      v.uv[1] =
          (1. - (v.xyzw[1] - m_aabbMin[1]) / (m_aabbMax[1] - m_aabbMin[1])) *
          m_textureScaling;
...
void OpenGLGuiHelper::createCollisionObjectGraphicsObject(
    btCollisionObject* body, const btVector3& color) {
  if (body->getUserIndex() < 0) {
    ...
    if (graphicsShapeId >= 0) {
      ...
      int graphicsInstanceId =
          m_data->m_glApp->m_renderer->registerGraphicsInstance(
              graphicsShapeId, startTransform.getOrigin(),
              startTransform.getRotation(), color, localScaling);
      body->setUserIndex(graphicsInstanceId);
    }
  }
}

int OpenGLGuiHelper::registerTexture(const unsigned char* texels, int width,
                                     int height) {
  int textureId =
      m_data->m_glApp->m_renderer->registerTexture(texels, width, height);
  return textureId;
}
...
void OpenGLGuiHelper::changeTexture(int textureUniqueId,
                                    const unsigned char* rgbTexels, int width,
                                    int height) {
  bool flipPixelsY = true;
  m_data->m_glApp->m_renderer->updateTexture(textureUniqueId, rgbTexels,
                                             flipPixelsY);
}

int OpenGLGuiHelper::registerGraphicsShape(const float* vertices,
                                           int numvertices, const int* indices,
                                           int numIndices, int primitiveType,
                                           int textureId) {
  if (textureId == -2) {
  ...
  int shapeId = m_data->m_glApp->m_renderer->registerShape(
      vertices, numvertices, indices, numIndices, primitiveType, textureId);
  return shapeId;
}

int OpenGLGuiHelper::registerGraphicsInstance(int shapeIndex,
                                              const float* position,
                                              const float* quaternion,
                                              const float* color,
                                              const float* scaling) {
  return m_data->m_glApp->m_renderer->registerGraphicsInstance(
      shapeIndex, position, quaternion, color, scaling);
}
...
void OpenGLGuiHelper::changeInstanceFlags(int instanceUid, int flags) {
  if (instanceUid >= 0) {
    ...
    m_data->m_glApp->m_renderer->writeSingleInstanceFlagsToCPU(flags,
                                                               instanceUid);
  }
}
void OpenGLGuiHelper::changeRGBAColor(int instanceUid,
                                      const double rgbaColor[4]) {
  if (instanceUid >= 0) {
    m_data->m_glApp->m_renderer->writeSingleInstanceColorToCPU(rgbaColor,
                                                               instanceUid);
  };
}
void OpenGLGuiHelper::changeSpecularColor(int instanceUid,
                                          const double specularColor[3]) {
  if (instanceUid >= 0) {
    m_data->m_glApp->m_renderer->writeSingleInstanceSpecularColorToCPU(
        specularColor, instanceUid);
  };
}
...
void OpenGLGuiHelper::createCollisionShapeGraphicsObject(
    btCollisionShape* collisionShape) {
  // already has a graphics object?
  if (collisionShape->getUserIndex() >= 0) return;
  ...
  if (collisionShape->getShapeType() == BOX_SHAPE_PROXYTYPE) {
    ...
    if (graphicsShapeIndexPtr) {
      ...
      graphicsShapeIndex = registerGraphicsShape(
          &transformedVertices[0], numVertices, cube_indices, numIndices,
          B3_GL_TRIANGLES, m_data->m_checkedTextureGrey);
  ...
  if (collisionShape->getShapeType() == TERRAIN_SHAPE_PROXYTYPE) {
    const btHeightfieldTerrainShape* heightField =
        static_cast<const btHeightfieldTerrainShape*>(collisionShape);
    ...
    if (gfxVertices.size() && indices.size()) {
      ...
      int shapeId = m_data->m_glApp->m_renderer->registerShape(
          &gfxVertices[0].xyzw[0], gfxVertices.size(), &indices[0],
          indices.size(), 1, userImage);
  ...
  if (collisionShape->getShapeType() == SOFTBODY_SHAPE_PROXYTYPE) {
    ...
    if (gfxVertices.size() && indices.size()) {
      int shapeId = registerGraphicsShape(
          &gfxVertices[0].xyzw[0], gfxVertices.size(), &indices[0],
          indices.size(), B3_GL_TRIANGLES, m_data->m_checkedTexture);
    ...
    if (ms->getSphereCount() == 2) {
      ...
      if (graphicsShapeIndexPtr) {
        ...
        for (int i = 0; i < numVertices; i++) {
          ...
          transformedVertices[i * 9 + 1] = trVer[1];
          transformedVertices[i * 9 + 2] = trVer[2];
          transformedVertices[i * 9 + 3] =
              textured_detailed_sphere_vertices[i * 9 + 3];
          transformedVertices[i * 9 + 4] =
              textured_detailed_sphere_vertices[i * 9 + 4];
          transformedVertices[i * 9 + 5] =
              textured_detailed_sphere_vertices[i * 9 + 5];
          transformedVertices[i * 9 + 6] =
              textured_detailed_sphere_vertices[i * 9 + 6];
          transformedVertices[i * 9 + 7] =
              textured_detailed_sphere_vertices[i * 9 + 7];
          transformedVertices[i * 9 + 8] =
              textured_detailed_sphere_vertices[i * 9 + 8];
        }
  ...
  if (collisionShape->getShapeType() == SPHERE_SHAPE_PROXYTYPE) {
    ...
    if (graphicsShapeIndexPtr) {
      ...
      int numIndices = sizeof(textured_detailed_sphere_indices) / sizeof(int);
      graphicsShapeIndex =
          registerGraphicsShape(&transformedVertices[0], numVertices,
  ...
  if (collisionShape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE) {
    ...
    if (compound->getNumChildShapes() == 1) {
      if (compound->getChildShape(0)->getShapeType() ==
        ...
        if (graphicsShapeIndexPtr) {
          ...
          for (int i = 0; i < numVertices; i++) {
            ...
            btVector3 trVer =
                compound->getChildTransform(0) * (radiusScale * vert);
            transformedVertices[i * 9 + 0] = trVer[0];
            transformedVertices[i * 9 + 1] = trVer[1];
            transformedVertices[i * 9 + 2] = trVer[2];
            transformedVertices[i * 9 + 3] =
          ...
          int numIndices =
              sizeof(textured_detailed_sphere_indices) / sizeof(int);
          graphicsShapeIndex = registerGraphicsShape(
              &transformedVertices[0], numVertices,
              textured_detailed_sphere_indices, numIndices, B3_GL_TRIANGLES,
              m_data->m_checkedTextureGrey);
      ...
      if (compound->getChildShape(0)->getShapeType() ==
        ...
        btCapsuleShape* sphereShape =
            (btCapsuleShape*)compound->getChildShape(0);
      ...
      if (compound->getChildShape(0)->getShapeType() ==
        ...
        btMultiSphereShape* ms =
            (btMultiSphereShape*)compound->getChildShape(0);
        if (ms->getSphereCount() == 2) {
          btAlignedObjectArray<float> transformedVertices;
          int numVertices =
              sizeof(textured_detailed_sphere_vertices) / strideInBytes;
  ...
  if (collisionShape->getShapeType() == STATIC_PLANE_PROXYTYPE) {
    const btStaticPlaneShape* staticPlaneShape =
        static_cast<const btStaticPlaneShape*>(collisionShape);
  ...
  // todo: create some textured objects for popular objects, like plane, cube,
  // sphere, capsule

  {
    btAlignedObjectArray<btVector3> vertexPositions;
    btAlignedObjectArray<btVector3> vertexNormals;
    CollisionShape2TriangleMesh(collisionShape, startTrans, vertexPositions,
                                vertexNormals, indices);
  ...
  if (gfxVertices.size() && indices.size()) {
    int shapeId =
        registerGraphicsShape(&gfxVertices[0].xyzw[0], gfxVertices.size(),
                              &indices[0], indices.size(), B3_GL_TRIANGLES, -1);
    collisionShape->setUserIndex(shapeId);
  }
}
void OpenGLGuiHelper::syncPhysicsToGraphics(
    const btDiscreteDynamicsWorld* rbWorld) {
  // in VR mode, we skip the synchronization for the second eye
  if (m_data->m_vrMode && m_data->m_vrSkipShadowPass == 1) return;
  ...
  int numCollisionObjects = rbWorld->getNumCollisionObjects();
  {
    ...
    for (int i = 0; i < numCollisionObjects; i++) {
      ...
      if (collisionShape->getShapeType() == SOFTBODY_SHAPE_PROXYTYPE &&
          collisionShape->getUserIndex() >= 0) {
        btAlignedObjectArray<GLInstanceVertex> gfxVertices;
        btAlignedObjectArray<int> indices;
        computeSoftBodyVertices(collisionShape, gfxVertices, indices);
        m_data->m_glApp->m_renderer->updateShape(collisionShape->getUserIndex(),
      ...
      if (index >= 0) {
        m_data->m_glApp->m_renderer->writeSingleInstanceTransformToCPU(pos, orn,
                                                                       index);
      }
    }
  }
...
void OpenGLGuiHelper::render(const btDiscreteDynamicsWorld* rbWorld) {
  if (m_data->m_vrMode) {
    ...
    if (m_data->m_vrSkipShadowPass >= 1) {
      m_data->m_glApp->m_renderer->renderSceneInternal(
...
void OpenGLGuiHelper::createPhysicsDebugDrawer(
...
void OpenGLGuiHelper::setUpAxis(int axis) { m_data->m_glApp->setUpAxis(axis); }

void OpenGLGuiHelper::setVisualizerFlagCallback(
    VisualizerFlagCallback callback) {
...
void OpenGLGuiHelper::resetCamera(float camDist, float yaw, float pitch,
  ...
  if (getRenderInterface() && getRenderInterface()->getActiveCamera()) {
    ...
    getRenderInterface()->getActiveCamera()->setCameraTargetPosition(
        camPosX, camPosY, camPosZ);
  }
}

bool OpenGLGuiHelper::getCameraInfo(
  ...
  if (getRenderInterface() && getRenderInterface()->getActiveCamera()) {
    ...
    *height = m_data->m_glApp->m_window->getHeight();
    ...
    getRenderInterface()->getActiveCamera()->getCameraProjectionMatrix(
        projectionMatrix);
...
void OpenGLGuiHelper::setProjectiveTextureMatrices(
    const float viewMatrix[16], const float projectionMatrix[16]) {
  m_data->m_glApp->m_renderer->setProjectiveTextureMatrices(viewMatrix,
                                                            projectionMatrix);
}
...
void OpenGLGuiHelper::copyCameraImageData(
    const float viewMatrix[16], const float projectionMatrix[16],
  ...
  int sourceWidth = btMin(destinationWidth,
                          (int)(m_data->m_glApp->m_window->getWidth() *
                                m_data->m_glApp->m_window->getRetinaScale()));
  int sourceHeight = btMin(destinationHeight,
                           (int)(m_data->m_glApp->m_window->getHeight() *
                                 m_data->m_glApp->m_window->getRetinaScale()));
  ...
  if (numRequestedPixels) {
    if (startPixelIndex == 0) {
      ...
      getRenderInterface()->getActiveCamera()->setVRCamera(viewMatrix,
                                                           projectionMatrix);
      {
...

      {
        ...
        sourceRgbaPixelBuffer.resize(sourceWidth * sourceHeight *
                                     numBytesPerPixel);
        sourceDepthBuffer.resize(sourceWidth * sourceHeight);
        {
          BT_PROFILE("getScreenPixels");
          m_data->m_glApp->getScreenPixels(
              &(sourceRgbaPixelBuffer[0]), sourceRgbaPixelBuffer.size(),
              &sourceDepthBuffer[0], sizeof(float) * sourceDepthBuffer.size());
        }

        m_data->m_rgbaPixelBuffer1.resize(destinationWidth * destinationHeight *
                                          numBytesPerPixel);
...
#define COPY4PIXELS 1
              ...
              int* dst =
                  (int*)&m_data
                      ->m_rgbaPixelBuffer1[(i + j * destinationWidth) * 4 + 0];
              int* src = (int*)&sourceRgbaPixelBuffer[sourcePixelIndex + 0];
              *dst = *src;

#else
              m_data->m_rgbaPixelBuffer1[(i + j * destinationWidth) * 4 + 0] =
                  sourceRgbaPixelBuffer[sourcePixelIndex + 0];
              m_data->m_rgbaPixelBuffer1[(i + j * destinationWidth) * 4 + 1] =
                  sourceRgbaPixelBuffer[sourcePixelIndex + 1];
              m_data->m_rgbaPixelBuffer1[(i + j * destinationWidth) * 4 + 2] =
                  sourceRgbaPixelBuffer[sourcePixelIndex + 2];
              m_data->m_rgbaPixelBuffer1[(i + j * destinationWidth) * 4 + 3] =
                  255;
#endif
              if (depthBuffer) {
                m_data->m_depthBuffer1[i + j * destinationWidth] =
                    sourceDepthBuffer[sourceDepthIndex];
              }
            }
          }
        }
      ...
      if (segmentationMaskBuffer) {
        {
          ...
          m_data->m_glApp->setViewport(sourceWidth, sourceHeight);
          BT_PROFILE("renderScene");
          getRenderInterface()->renderSceneInternal(
...

        {
          ...
          sourceDepthBuffer.resize(sourceWidth * sourceHeight);
          {
            ...
            m_data->m_glApp->getScreenPixels(
                &(sourceRgbaPixelBuffer[0]), sourceRgbaPixelBuffer.size(),
                &sourceDepthBuffer[0],
                sizeof(float) * sourceDepthBuffer.size());
          }
          ...
          // rescale and flip
          {
            ...
            for (int j = 0; j < destinationHeight; j++) {
              for (int i = 0; i < destinationWidth; i++) {
                int xIndex = int(
                    float(i) * (float(sourceWidth) / float(destinationWidth)));
                int yIndex =
                    int(float(destinationHeight - 1 - j) *
                        (float(sourceHeight) / float(destinationHeight)));
                btClamp(xIndex, 0, sourceWidth);
                ...
                if (segmentationMaskBuffer) {
                  ...
                  if (depth < 1) {
                    int segMask =
                        sourceRgbaPixelBuffer[sourcePixelIndex + 0] +
                        256 * (sourceRgbaPixelBuffer[sourcePixelIndex + 1]) +
                        256 * 256 *
                            (sourceRgbaPixelBuffer[sourcePixelIndex + 2]);
    ...
    if (segmentationMaskBuffer) {
      ...
      for (int i = 0; i < numRequestedPixels; i++) {
        segmentationMaskBuffer[i] =
            m_data->m_segmentationMaskBuffer[i + startPixelIndex];
      }
    }
    if (numPixelsCopied) *numPixelsCopied = numRequestedPixels;
  }
...
bool shapePointerCompareFunc(const btCollisionObject* colA,
                             const btCollisionObject* colB) {
...
void OpenGLGuiHelper::autogenerateGraphicsObjects(
    btDiscreteDynamicsWorld* rbWorld) {
  // sort the collision objects based on collision shape, the gfx library
  // requires instances that re-use a shape to be added after eachother
...
void OpenGLGuiHelper::drawText3D(const char* txt, float position[3],
                                 float orientation[4], float color[4],
                                 float size, int optionFlags) {
  B3_PROFILE("OpenGLGuiHelper::drawText3D");

  btAssert(m_data->m_glApp);
  m_data->m_glApp->drawText3D(txt, position, orientation, color, size,
                              optionFlags);
}

void OpenGLGuiHelper::drawText3D(const char* txt, float posX, float posY,
...
void OpenGLGuiHelper::computeSoftBodyVertices(
    btCollisionShape* collisionShape,
    btAlignedObjectArray<GLInstanceVertex>& gfxVertices,
    btAlignedObjectArray<int>& indices) {
  if (collisionShape->getUserPointer() == 0) return;
