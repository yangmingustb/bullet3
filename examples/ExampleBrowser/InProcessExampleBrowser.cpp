...
static b3ThreadSupportInterface* createExampleBrowserThreadSupport(
    int numThreads) {
  b3PosixThreadSupport::ThreadConstructionInfo constructionInfo(
      "testThreads", ExampleBrowserThreadFunc, ExampleBrowserMemoryFunc,
      ExampleBrowserMemoryReleaseFunc, numThreads);
...
static ExampleEntryPhysicsServer gDefaultExamplesPhysicsServer[] = {
    ...
    ExampleEntryPhysicsServer(1, "Physics Server",
                              "Create a physics server that communicates with "
                              "a physics client over shared memory",
                              PhysicsServerCreateFuncBullet2),
    ExampleEntryPhysicsServer(
        1, "Physics Server (RTC)",
        "Create a physics server that communicates with a physics client over "
        "shared memory. At each update, the Physics Server will continue "
        "calling 'stepSimulation' based on the real-time clock (RTC).",
...
ExampleEntriesPhysicsServer::~ExampleEntriesPhysicsServer() { delete m_data; }
...
void ExampleEntriesPhysicsServer::initExampleEntries() {
  ...
  int numDefaultEntries =
      sizeof(gDefaultExamplesPhysicsServer) / sizeof(ExampleEntryPhysicsServer);
  for (int i = 0; i < numDefaultEntries; i++) {
    m_data->m_allExamples.push_back(gDefaultExamplesPhysicsServer[i]);
  }
}

void ExampleEntriesPhysicsServer::registerExampleEntry(
    int menuLevel, const char* name, const char* description,
...
ExampleEntriesPhysicsServer::getExampleCreateFunc(int index) {
...
struct ExampleBrowserArgs {
  ExampleBrowserArgs() : m_fakeWork(1), m_argc(0) {}
...
void ExampleBrowserThreadFunc(void* userPtr, void* lsMemory) {
  ...
  ExampleBrowserThreadLocalStorage* localStorage =
      (ExampleBrowserThreadLocalStorage*)lsMemory;
  ...
  if (init) {
    ...
    do {
      ...
      float deltaTimeInSeconds = clock.getTimeMicroseconds() / 1000000.f;
      {
        ...
        !exampleBrowser->requestedExit() &&
        (args->m_cs->getSharedParam(0) != eRequestTerminateExampleBrowser));
  } else {
    args->m_cs->lock();
...
btInProcessExampleBrowserInternalData* btCreateInProcessExampleBrowser(
    int argc, char** argv2, bool useInProcessMemory) {
  btInProcessExampleBrowserInternalData* data =
      new btInProcessExampleBrowserInternalData;
  ...
  for (i = 0; i < data->m_threadSupport->getNumTasks(); i++) {
    ExampleBrowserThreadLocalStorage* storage =
        (ExampleBrowserThreadLocalStorage*)
            data->m_threadSupport->getThreadLocalMemory(i);
    b3Assert(storage);
    storage->threadId = i;
  ...
  for (i = 0; i < numThreads; i++) {
    data->m_threadSupport->runTask(B3_THREAD_SCHEDULE_TASK,
                                   (void*)&data->m_args, i);
  }

  while (data->m_args.m_cs->getSharedParam(0) ==
         eExampleBrowserIsUnInitialized) {
    b3Clock::usleep(1000);
  }
...
SharedMemoryInterface* btGetSharedMemoryInterface(
    btInProcessExampleBrowserInternalData* data) {
...
btInProcessExampleBrowserMainThreadInternalData*
btCreateInProcessExampleBrowserMainThread(int argc, char** argv,
                                          bool useInProcessMemory) {
  btInProcessExampleBrowserMainThreadInternalData* data =
      new btInProcessExampleBrowserMainThreadInternalData;
...
bool btIsExampleBrowserMainThreadTerminated(
    btInProcessExampleBrowserMainThreadInternalData* data) {
  return data->m_exampleBrowser->requestedExit();
}

void btUpdateInProcessExampleBrowserMainThread(
    btInProcessExampleBrowserMainThreadInternalData* data) {
...
void btShutDownExampleBrowserMainThread(
    btInProcessExampleBrowserMainThreadInternalData* data) {
...
class SharedMemoryInterface* btGetSharedMemoryInterfaceMainThread(
    btInProcessExampleBrowserMainThreadInternalData* data) {
  return data->m_sharedMem;
}
