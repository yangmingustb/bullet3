...
void GL_ShapeDrawer::drawOpenGL(btScalar* m, const btCollisionShape* shape,
  ...
  if (shape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE) {
    ...
    if (!(debugMode & btIDebugDraw::DBG_DrawWireframe)) {
      ...
      /// the benefit of 'default' is that it approximates the actual collision
      /// shape including collision margin
      // int
      ...
      switch (shapetype) {
        case SPHERE_SHAPE_PROXYTYPE: {
          ...
          float radius = sphereShape->getMargin();  // radius doesn't include
            ...
            if (poly) {
              ...
              for (i = 0; i < poly->m_faces.size(); i++) {
                ...
                if (numVerts > 2) {
                  ...
                  for (int v = 0; v < poly->m_faces[i].m_indices.size() - 2;
                       v++) {
                    btVector3 v2 =
...
//
void GL_ShapeDrawer::drawShadow(btScalar* m, const btVector3& extrusion,
  ...
  if (shape->getShapeType() == UNIFORM_SCALING_SHAPE_PROXYTYPE) {
    ...
    btScalar tmpScaling[4][4] = {{scalingFactor, 0, 0, 0},
                                 ...
                                 {0, 0, scalingFactor, 0},
                                 {0, 0, 0, 1}};
    drawShadow((btScalar*)tmpScaling, extrusion, convexShape, worldBoundsMin,
               worldBoundsMax);
    glPopMatrix();
    return;
    ...
    if (shape->isConvex()) {
      ...
      for (int i = 0; i < sc->m_edges.size(); ++i) {
        ...
        if ((d * btDot(sc->m_edges[i].n[1], extrusion)) < 0) {
          ...
          const btVector3& b =
              hull->getVertexPointer()[sc->m_edges[i].v[1 - q]];
          glVertex3f(a[0], a[1], a[2]);
          glVertex3f(b[0], b[1], b[2]);
          glVertex3f(b[0] + extrusion[0], b[1] + extrusion[1],
                     b[2] + extrusion[2]);
          glVertex3f(a[0] + extrusion[0], a[1] + extrusion[1],
                     a[2] + extrusion[2]);
        }
      }
      glEnd();
...
void GL_ShapeDrawer::drawSceneInternal(
    const btDiscreteDynamicsWorld* dynamicsWorld, int pass, int cameraUpAxis) {
  ...
  for (int i = 0; i < numObjects; i++) {
    ...
    switch (pass) {
      case 0:
        drawOpenGL(m, colObj->getCollisionShape(), wireColor, debugMode,
                   aabbMin, aabbMax);
        break;
      case 1:
        drawShadow(m, m_sundirection * rot, colObj->getCollisionShape(),
                   aabbMin, aabbMax);
        break;
      case 2:
        drawOpenGL(m, colObj->getCollisionShape(), wireColor * btScalar(0.3), 0,
                   aabbMin, aabbMax);
        break;
    }
  }
...
// this GL_ShapeDrawer will be removed, in the meanwhile directly access this
// global 'useShadoMaps'
extern bool useShadowMap;
void GL_ShapeDrawer::drawScene(const btDiscreteDynamicsWorld* dynamicsWorld,
                               bool useShadows1, int cameraUpAxis) {
