...
class MyGraphWindow : public Gwen::Controls::WindowControl {
 ...
 public:
  ...
  MyGraphWindow(const MyGraphInput& input)
      : Gwen::Controls::WindowControl(input.m_data->pCanvas), m_menuItems(0) {
    Gwen::UnicodeString str = Gwen::Utility::StringToUnicode(input.m_name);
    SetTitle(str);

    SetPos(input.m_xPos, input.m_yPos);
    SetSize(12 + input.m_width + 2 * input.m_borderWidth,
            30 + input.m_height + 2 * input.m_borderWidth);

    m_imgPanel = new Gwen::Controls::ImagePanel(this);
    if (input.m_texName) {
      Gwen::UnicodeString texName =
          Gwen::Utility::StringToUnicode(input.m_texName);
  ...
  virtual ~MyGraphWindow() { delete m_imgPanel; }
};
...
class MyMenuItems2 : public Gwen::Controls::Base {
 ...
 public:
  ...
  MyMenuItems2(MyGraphWindow* graphWindow)
      : Gwen::Controls::Base(0), m_graphWindow(graphWindow), m_item(0) {}

  void MenuItemSelect(Gwen::Controls::Base* pControl) {
    if (m_graphWindow->Hidden()) {
      m_graphWindow->SetHidden(false);
      //@TODO(erwincoumans) setCheck/SetCheckable drawing is broken, need to see
      //what's wrong
      //			if (m_item)
...
MyGraphWindow* setupTextureWindow(const MyGraphInput& input) {
  ...
  Gwen::UnicodeString str = Gwen::Utility::StringToUnicode(input.m_name);
  menuItems->m_item = input.m_data->m_viewMenu->GetMenu()->AddItem(
      str, menuItems,
      (Gwen::Event::Handler::Function)&MyMenuItems2::MenuItemSelect);
