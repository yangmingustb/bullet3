...
struct MyButtonEventHandler : public Gwen::Event::Handler {
  ...
  MyButtonEventHandler(Gwen::Controls::Button* buttonControl,
                       ButtonParamChangedCallback callback, int buttonId,
                       void* userPointer)
...
struct MySliderEventHandler : public Gwen::Event::Handler {
  ...
  MySliderEventHandler(const char* varName, Gwen::Controls::TextBox* label,
                       Gwen::Controls::Slider* pSlider, T* target,
                       SliderParamChangedCallback callback, void* userPtr)
      : m_callback(callback),
  ...
  void SliderMoved(Gwen::Controls::Base* pControl) {
    ...
    // printf("value = %f\n", pSlider->GetValue());//UnitPrint( Utility::Format(
    // L"Slider Value: %.2f", pSlider->GetValue() ) );
...
void GwenParameterInterface::setSliderValue(int sliderIndex,
                                            double sliderValue) {
...
void GwenParameterInterface::registerButtonParameter(ButtonParams& params) {
  Gwen::Controls::Button* button =
      new Gwen::Controls::Button(m_gwenInternalData->m_demoPage->GetPage());
  MyButtonEventHandler* handler = new MyButtonEventHandler(
      button, params.m_callback, params.m_buttonId, params.m_userPointer);
...
struct MyComboBoxHander2 : public Gwen::Event::Handler {
  ...
  MyComboBoxHander2(GwenInternalData* data, int buttonId,
                    ComboBoxCallback callback, void* userPointer)
      : m_data(data),
        m_buttonId(buttonId),
        m_callback(callback),
        m_userPointer(userPointer) {}
  ...
  void onSelect(Gwen::Controls::Base* pControl) {
    ...
    if (m_callback) (*m_callback)(m_buttonId, str.c_str(), m_userPointer);
  }
};

void GwenParameterInterface::registerComboBox(ComboBoxParams& params) {
  Gwen::Controls::ComboBox* combobox =
      new Gwen::Controls::ComboBox(m_gwenInternalData->m_demoPage->GetPage());
  m_paramInternalData->m_comboBoxes.push_back(combobox);
  MyComboBoxHander2* handler =
      new MyComboBoxHander2(m_gwenInternalData, params.m_comboboxId,
                            params.m_callback, params.m_userPointer);
  ...
  for (int i = 0; i < params.m_numItems; i++) {
    Gwen::Controls::MenuItem* item =
        combobox->AddItem(Gwen::Utility::StringToUnicode(params.m_items[i]));
    if (i == params.m_startItem) combobox->OnItemSelected(item);
  }
}

void GwenParameterInterface::registerSliderFloatParameter(
    SliderParams& params) {
  Gwen::Controls::TextBox* label =
      new Gwen::Controls::TextBox(m_gwenInternalData->m_demoPage->GetPage());
  ...
  Gwen::Controls::HorizontalSlider* pSlider =
      new Gwen::Controls::HorizontalSlider(
          m_gwenInternalData->m_demoPage->GetPage());
  ...
  if (params.m_clampToIntegers) {
    ...
    pSlider->SetNotchCount(
        16);  // float(params.m_maxVal-params.m_minVal)/100.f);
  ...
  MySliderEventHandler<btScalar>* handler = new MySliderEventHandler<btScalar>(
      labelName, label, pSlider, params.m_paramValuePointer, params.m_callback,
      params.m_userPointer);
  handler->m_showValue = params.m_showValues;
  m_paramInternalData->m_sliderEventHandlers.push_back(handler);

  pSlider->onValueChanged.Add(handler,
                              &MySliderEventHandler<btScalar>::SliderMoved);
...
void GwenParameterInterface::syncParameters() {
  for (int i = 0; i < m_paramInternalData->m_sliderEventHandlers.size(); i++) {
    MySliderEventHandler<btScalar>* handler =
        m_paramInternalData->m_sliderEventHandlers[i];
...
void GwenParameterInterface::removeAllParameters() {
  ...
  m_gwenInternalData->m_curYposition =
      this->m_paramInternalData->m_savedYposition;
