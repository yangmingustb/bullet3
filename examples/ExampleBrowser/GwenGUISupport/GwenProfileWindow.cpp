...
class MyProfileWindow : public Gwen::Controls::WindowControl {
 ...
 protected:
  ...
  void OnCheckChangedStiffnessWarping(Gwen::Controls::Base* pControl) {
    //	Gwen::Controls::CheckBox* labeled = (Gwen::Controls::CheckBox*
    //)pControl;
 ...
 public:
  ...
  MyProfileWindow(Gwen::Controls::Base* pParent)
      : Gwen::Controls::WindowControl(pParent), profIter(0) {
...

    {
      ...
      // pNode->AddNode( L"Brown" )->AddNode( L"Node Two Inside" )->AddNode(
      // L"Eyes" )->AddNode( L"Brown" );
      ...
      m_ctrl->SetBounds(this->GetInnerBounds().x, this->GetInnerBounds().y,
                        this->GetInnerBounds().w, this->GetInnerBounds().h);
    }
  }
  ...
  float dumpRecursive(CProfileIterator* profileIterator,
                      Gwen::Controls::TreeNode* parentNode) {
    profileIterator->First();
    if (profileIterator->Is_Done()) return 0.f;
    ...
    if (0 == frames_since_reset) return 0.f;
    ...
    for (i = 0; !profileIterator->Is_Done(); i++, profileIterator->Next()) {
      ...
      double fraction = parent_time > SIMD_EPSILON
      ...
      txt = Gwen::Utility::Format(
          L"%s (%.2f %%) :: %.3f ms / frame (%d calls)", uname.c_str(),
          fraction, (current_total_time / (double)frames_since_reset),
          profileIterator->Get_Current_Total_Calls());

#else
      txt = Gwen::Utility::Format(
          L"%s (%.2f %%) :: %.3f ms / frame (%d calls)", name.c_str(), fraction,
          (current_total_time / (double)frames_since_reset),
          profileIterator->Get_Current_Total_Calls());

#endif

      Gwen::Controls::TreeNode* childNode =
          (Gwen::Controls::TreeNode*)profileIterator->Get_Current_UserPointer();
      if (!childNode) {
  ...
  void UpdateText(CProfileIterator* profileIterator, bool idle) {
    ...
    // Gwen::UnicodeString txt = Gwen::Utility::Format( L"FEM Settings  %i fps",
    // test );
    {
...
#ifdef _WIN32
      Gwen::UnicodeString uname = Gwen::Utility::StringToUnicode(name);
      Gwen::UnicodeString txt = Gwen::Utility::Format(
          L"Profiling: %s total time: %.3f ms, unaccounted %.3f %% :: %.3f ms",
          uname.c_str(), parent_time,
...
MyProfileWindow* setupProfileWindow(GwenInternalData* data) {
  ...
  data->m_viewMenu->GetMenu()->AddItem(
      L"Profiler", menuItems,
      (Gwen::Event::Handler::Function)&MyMenuItems3::MenuItemSelect);
