...
struct MyTestMenuBar : public Gwen::Controls::MenuStrip {
  ...
  MyTestMenuBar(Gwen::Controls::Base* pParent)
    ...
    //		Gwen::Controls::MenuStrip* menu = new Gwen::Controls::MenuStrip(
    //pParent );
    {
      ...
      m_fileMenu->GetMenu()->AddItem(
          L"Open", m_menuItems,
          (Gwen::Event::Handler::Function)&MyMenuItems::fileOpen);
      m_fileMenu->GetMenu()->AddItem(
          L"Quit", m_menuItems,
          (Gwen::Event::Handler::Function)&MyMenuItems::myQuitApp);
      m_viewMenu = AddItem(L"View");
    }
  }
  virtual ~MyTestMenuBar() { delete m_menuItems; }
};
...
void GwenUserInterface::setExampleDescription(const char* message) {
  // Gwen apparently doesn't have text/word wrap, so do rudimentary brute-force
  // implementation here.
  ...
  for (int endPos = 0; endPos <= wrapLen; endPos++) {
    ...
    Gwen::Point pt = m_data->pRenderer->MeasureText(
        m_data->pCanvas->GetSkin()->GetDefaultFont(), sub);

    if (pt.x <= fixedWidth) {
      lastFit = sub;

      if (message[endPos] == ' ' || message[endPos] == '.' ||
          message[endPos] == ',') {
        hasSpace = true;
        lastFitSpace = sub;
        spacePos = endPos;
...
void GwenUserInterface::init(int width, int height,
  ...
  m_data->pRenderer =
      renderer;  // new
                 // GwenOpenGL3CoreRenderer(m_data->m_primRenderer,stash,width,height,retinaScale);
  ...
  Gwen::Controls::StatusBar* bar =
      new Gwen::Controls::StatusBar(m_data->pCanvas);
  m_data->m_bar = bar;
  ...
  // Gwen::KeyboardFocus
  /*Gwen::Controls::GroupBox* box = new
  Gwen::Controls::GroupBox(m_data->pCanvas);
  box->SetText("text");
  box->SetName("name");
  box->SetHeight(500);
  */
  Gwen::Controls::ScrollControl* windowRight =
      new Gwen::Controls::ScrollControl(m_data->pCanvas);
  ...
  /*Gwen::Controls::WindowControl* windowBottom = new
  Gwen::Controls::WindowControl(m_data->pCanvas);
  windowBottom->SetHeight(100);
  windowBottom->Dock(Gwen::Pos::Bottom);
  windowBottom->SetTitle("bottom");
  */
  //	Gwen::Controls::Property::Text* prop = new
  //Gwen::Controls::Property::Text(m_data->pCanvas);
  // prop->Dock(Gwen::Pos::Bottom);
  /*Gwen::Controls::SplitterBar* split = new
  Gwen::Controls::SplitterBar(m_data->pCanvas);
  ...
  */
  ...
  Gwen::Controls::ScrollControl* windowLeft =
      new Gwen::Controls::ScrollControl(m_data->pCanvas);
  ...
  Gwen::Controls::TabControl* explorerTab =
      new Gwen::Controls::TabControl(windowLeft);
  ...
  // Gwen::Controls::HSVColorPicker* color = new
  // Gwen::Controls::HSVColorPicker(shapes->GetPage());
  Gwen::Controls::ColorPicker* color =
      new Gwen::Controls::ColorPicker(shapes->GetPage());
  color->SetKeyboardInputEnabled(true);

  Gwen::Controls::TreeControl* ctrl =
      new Gwen::Controls::TreeControl(m_data->m_explorerPage->GetPage());
  m_data->m_explorerTreeCtrl = ctrl;
  ctrl->SetKeyboardInputEnabled(true);
  ctrl->Focus();
  ctrl->SetBounds(2, 10, 236, 300);

  m_data->m_exampleInfoGroupBox =
      new Gwen::Controls::Label(m_data->m_explorerPage->GetPage());
  ...
  m_data->m_exampleInfoTextOutput =
      new Gwen::Controls::ListBox(m_data->m_explorerPage->GetPage());
...
void GwenUserInterface::setToggleButtonCallback(
    b3ToggleButtonCallback callback) {
...
void GwenUserInterface::registerToggleButton2(int buttonId, const char* name) {
  ...
  Gwen::Controls::Button* but =
      new Gwen::Controls::Button(m_data->m_demoPage->GetPage());
...
void GwenUserInterface::registerComboBox2(int comboboxId, int numItems,
                                          const char** items, int startItem) {
  Gwen::Controls::ComboBox* combobox =
      new Gwen::Controls::ComboBox(m_data->m_demoPage->GetPage());
  ...
  for (int i = 0; i < numItems; i++) {
    ...
    if (i == startItem) combobox->OnItemSelected(item);
  }
...
bool GwenUserInterface::mouseMoveCallback(float x, float y) {
  ...
  if (m_data->pCanvas) {
    ...
    handled = m_data->pCanvas->InputMouseMoved(x, y, m_lastmousepos[0],
                                               m_lastmousepos[1]);
  }
  return handled;
}
...
bool GwenUserInterface::mouseButtonCallback(int button, int state, float x,
                                            float y) {
  bool handled = false;
  if (m_data->pCanvas) {
