...
// quick test for file import, @todo(erwincoumans) make it more general and add
// other file formats
  ...
  if (gEnableDefaultKeyboardShortcuts) {
    ...
    if (key == 'd' && state) {
      ...
      gDisableDeactivation =
          ((gDebugDrawFlags & btIDebugDraw::DBG_NoDeactivation) != 0);
    }
    if (key == 'j' && state) {
...
static void MyMouseMoveCallback(float x, float y) {
  bool handled = false;
  if (sCurrentDemo) handled = sCurrentDemo->mouseMoveCallback(x, y);
  if (renderGui) {
    if (!handled && gui2) handled = gui2->mouseMoveCallback(x, y);
  }
  if (!handled) {
    if (prevMouseMoveCallback) prevMouseMoveCallback(x, y);
  }
}
...
static void MyMouseButtonCallback(int button, int state, float x, float y) {
  ...
  if (!handled) {
    if (prevMouseButtonCallback) prevMouseButtonCallback(button, state, x, y);
  }
...
void OpenGLExampleBrowser::registerFileImporter(
...
void openFileDemo(const char* filename) {
  ...
  s_guiHelper->setVisualizerFlagCallback(
      OpenGLExampleBrowserVisualizerFlagCallback);
...
void selectDemo(int demoIndex) {
  ...
  CommonExampleInterface::CreateFunc* func =
      gAllExamples->getExampleCreateFunc(demoIndex);
  if (func) {
    ...
    if (sCurrentDemo) {
      ...
      if (gui2) {
        gui2->setExampleDescription(
            gAllExamples->getExampleDescription(demoIndex));
      }
...
static void loadCurrentSettings(const char* startFileName,
                                b3CommandLineArgs& args) {
  // int currentEntry= 0;
...
// in case of multi-threading, don't submit messages while the GUI is rendering
// (causing crashes)
...
struct MyMenuItemHander : public Gwen::Event::Handler {
  ...
  MyMenuItemHander(int buttonId) : m_buttonId(buttonId) {}
  ...
  void onButtonA(Gwen::Controls::Base* pControl) {
    ...
    Gwen::UnicodeString la =
        node->GetButton()
            ->GetText();  // node->GetButton()->GetName();// GetText();
  ...
  void onButtonB(Gwen::Controls::Base* pControl) {
    ...
    Gwen::UnicodeString la =
        label->GetText();  // node->GetButton()->GetName();// GetText();
    Gwen::String laa = Gwen::Utility::UnicodeToString(la);
    // const char* ha = laa.c_str();
  ...
  void onButtonC(Gwen::Controls::Base* pControl) {
    /*Gwen::Controls::Label* label = (Gwen::Controls::Label*) pControl;
    Gwen::UnicodeString la = label->GetText();// node->GetButton()->GetName();//
    GetText();
    Gwen::String laa = Gwen::Utility::UnicodeToString(la);
    const char* ha = laa.c_str();
  ...
  void onButtonD(Gwen::Controls::Base* pControl) {
    /*		Gwen::Controls::Label* label = (Gwen::Controls::Label*)
    pControl;
    Gwen::UnicodeString la = label->GetText();// node->GetButton()->GetName();//
    GetText();
    Gwen::String laa = Gwen::Utility::UnicodeToString(la);
  ...
  void onButtonE(Gwen::Controls::Base* pControl) {
    ...
    gui2->setExampleDescription(
        gAllExamples->getExampleDescription(sCurrentHightlighted));
  }
...
void quitCallback() { s_window->setRequestExit(); }
...
struct QuickCanvas : public Common2dCanvasInterface {
  ...
  QuickCanvas(GL3TexLoader* myTexLoader)
      : m_myTexLoader(myTexLoader), m_curNumGraphWindows(0) {
  ...
  virtual int createCanvas(const char* canvasName, int width, int height,
    ...
    if (m_curNumGraphWindows < MAX_GRAPH_WINDOWS) {
      ...
      if (slot >= MAX_GRAPH_WINDOWS) return 0;  // don't crash
  ...
  virtual void setPixel(int canvasId, int x, int y, unsigned char red,
  ...
  virtual void getPixel(int canvasId, int x, int y, unsigned char& red,
                        unsigned char& green, unsigned char& blue,
                        unsigned char& alpha) {
    btAssert(canvasId >= 0);
    btAssert(canvasId < m_curNumGraphWindows);
...
bool OpenGLExampleBrowser::init(int argc, char* argv[]) {
  ...
  /// If you have a high-end desktop GPU such as AMD 7970 or better, or NVIDIA
  /// GTX 680 with up-to-date drivers
  /// you could give it a try
  /// Note that several old OpenCL physics examples still have to be ported over
  /// to this new Example Browser
  ...
  if (sUseOpenGL2) {
    char title[1024];
    sprintf(title, "%s using limited OpenGL2 fallback %s %s", appTitle,
            glContext, optMode);
...
#ifndef NO_OPENGL3
  if (gVideoFileName) simpleApp->dumpFramesToVideo(gVideoFileName);
#endif
...

  {
    ...
    if (sUseOpenGL2) {
      m_internalData->m_gwenRenderer =
          new Gwen::Renderer::OpenGL_DebugFont(s_window->getRetinaScale());
    }
#ifndef NO_OPENGL3
    else {
      sth_stash* fontstash = simpleApp->getFontStash();
      m_internalData->m_gwenRenderer = new GwenOpenGL3CoreRenderer(
          simpleApp->m_primRenderer, fontstash, width, height,
          s_window->getRetinaScale(), myTexLoader);
    }
#endif
    ...
    gui2->init(width, height, m_internalData->m_gwenRenderer,
  ...
  if (gui2) {
    ...
    Gwen::Controls::TreeControl* tree =
        gui2->getInternalData()->m_explorerTreeCtrl;
...
// gui->getInternalData()->pRenderer->setTextureLoader(myTexLoader);
    ...
    s_parameterInterface = s_app->m_parameterInterface =
        new GwenParameterInterface(gui2->getInternalData());
    ...
    tree->onReturnKeyDown.Add(m_internalData->m_handler2,
                              &MyMenuItemHander::onButtonD);
    int firstAvailableDemoIndex = -1;
    Gwen::Controls::TreeNode* firstNode = 0;

    for (int d = 0; d < numDemos; d++) {
      //		sprintf(nodeText, "Node %d", i);
      Gwen::UnicodeString nodeUText =
          Gwen::Utility::StringToUnicode(gAllExamples->getExampleName(d));
      if (gAllExamples->getExampleCreateFunc(
              d))  // was test for gAllExamples[d].m_menuLevel==1
      {
        ...
        pNode->GetButton()->onDoubleClick.Add(handler,
                                              &MyMenuItemHander::onButtonB);
...
#endif
        //			pNode->onKeyReturn.Add(handler,
        //&MyMenuItemHander::onButtonD);
        //			pNode->GetButton()->onKeyboardReturn.Add(handler,
        //&MyMenuItemHander::onButtonD);
        //		pNode->onNamePress.Add(handler,
        //&MyMenuItemHander::onButtonD);
        //			pNode->onKeyboardPressed.Add(handler,
        //&MyMenuItemHander::onButtonD);
        //			pNode->OnKeyPress
      } else {
...
bool OpenGLExampleBrowser::requestedExit() { return s_window->requestedExit(); }
...
void OpenGLExampleBrowser::update(float deltaTime) {
  ...
  if (sCurrentDemo) {
    ...
    if (renderVisualGeometry &&
        ((gDebugDrawFlags & btIDebugDraw::DBG_DrawWireframe) == 0)) {
      if (visualWireframe) {
  ...
  {
    if (gui2 && s_guiHelper && s_guiHelper->getRenderInterface() &&
        s_guiHelper->getRenderInterface()->getActiveCamera()) {
      B3_PROFILE("setStatusBarMessage");
      char msg[1024];
      float camDist = s_guiHelper->getRenderInterface()
                          ->getActiveCamera()
                          ->getCameraDistance();
      float pitch = s_guiHelper->getRenderInterface()
                        ->getActiveCamera()
                        ->getCameraPitch();
      float yaw =
          s_guiHelper->getRenderInterface()->getActiveCamera()->getCameraYaw();
      float camTarget[3];
      float camPos[3];
      s_guiHelper->getRenderInterface()->getActiveCamera()->getCameraPosition(
          camPos);
      s_guiHelper->getRenderInterface()
          ->getActiveCamera()
          ->getCameraTargetPosition(camTarget);
      sprintf(
          msg,
          "camTargetPos=%2.2f,%2.2f,%2.2f, dist=%2.2f, pitch=%2.2f, yaw=%2.2f",
          camTarget[0], camTarget[1], camTarget[2], camDist, pitch, yaw);
  ...
  if (renderGui) {
...

    {
      ...
      if (sUseOpenGL2) {
        saveOpenGLState(
            s_instancingRenderer->getScreenWidth() * s_window->getRetinaScale(),
            s_instancingRenderer->getScreenHeight() *
                s_window->getRetinaScale());
      }

      if (m_internalData->m_gui) {
        gBlockGuiMessages = true;
        m_internalData->m_gui->draw(s_instancingRenderer->getScreenWidth(),
                                    s_instancingRenderer->getScreenHeight());
...
void OpenGLExampleBrowser::setSharedMemoryInterface(
    class SharedMemoryInterface* sharedMem) {
