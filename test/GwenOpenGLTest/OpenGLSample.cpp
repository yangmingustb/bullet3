...
void MyMouseMoveCallback(float x, float y) {
  ...
  if (pCanvas) {
    ...
    bool handled =
        pCanvas->InputMouseMoved(x, y, m_lastmousepos[0], m_lastmousepos[1]);
  }
}
...
sth_stash* initFont(GLPrimitiveRenderer* primRenderer) {
  ...
  OpenGL2RenderCallbacks* renderCallbacks =
      new OpenGL2RenderCallbacks(primRenderer);

  stash =
      sth_create(512, 512, renderCallbacks);  // 256,256);//,1024);//512,512);
  err = glGetError();
  ...
  const char* fontPaths[] = {"./", "../../bin/", "../bin/", "bin/"};
  ...
  for (int i = 0; i < numPaths; i++) {
    ...
    sprintf(
        ...
        "DroidSerif-Regular.ttf");  // OpenSans.ttf");//"DroidSerif-Regular.ttf");
...
int main() {
  ...
  if (!(sscanf((const char*)glGetString(GL_VERSION), "%d.%d", &majorGlVersion,
               &minorGlVersion) == 2)) {
    printf(
        "Exit: Error cannot extract OpenGL version from GL_VERSION string\n");
    exit(0);
  }
  char title[1024];
...
// we need glewExperimental on Linux
    ...
    gwenRenderer = new GwenOpenGL3CoreRenderer(primRenderer, font, sWidth,
                                               sHeight, retinaScale);
  } else
#endif
  {
...
//
//		Gwen::Renderer::OpenGL_DebugFont * pRenderer = new
//Gwen::Renderer::OpenGL_DebugFont();
