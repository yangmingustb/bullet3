...
/// You can provide your own support function in combination with the template
/// functions
...
btVector3 MyBulletShapeSupportFunc(const void* shapeAptr, const btVector3& dir,
                                   bool includeMargin) {
...
struct ConvexWrap {
  ...
  inline btScalar getMargin() const { return m_convex->getMargin(); }
  inline btVector3 getObjectCenterInWorld() const {
    return m_worldTrans.getOrigin();
  }
  inline const btTransform& getWorldTransform() const { return m_worldTrans; }
...
inline int btComputeGjkEpaSphereSphereCollision(
    const btSphereSphereCollisionDescription& input, btDistanceInfo* distInfo,
    SphereSphereTestMethod method) {
  /// for spheres it is best to use a 'point' and set the margin to the radius
  /// (which is what btSphereShape does)
  ...
  /// also compare with
  /// https://code.google.com/p/bullet/source/browse/branches/PhysicsEffects/src/base_level/collision/pfx_gjk_solver.cpp
  switch (method) {
    case SSTM_GJKEPA_RADIUS_NOT_FULL_MARGIN:
    case SSTM_GJKEPA: {
    ...
    case SSTM_GJKMPR: {
      ...
      if (res == 0) {
        ...
        //                if (res==0)
        //                {
        //                    printf("use MPR results in distance
        //                    %f\n",distInfo->m_distance);
        //                }
      }
      break;
...
void testSphereSphereDistance(SphereSphereTestMethod method,
                              btScalar abs_error) {
  {
  ...
  for (int rb = 1; rb < 5; rb++)
    for (int z = -5; z < 5; z++) {
      for (int j = 1; j < 5; j++) {
        for (int i = -5; i < 5; i++) {
          if (i != z)  // skip co-centric spheres for now (todo(erwincoumans)
                       // fix this)
          {
            btSphereSphereCollisionDescription ssd;
            ssd.m_sphereTransformA.setIdentity();
            ...
            switch (method) {
              ...
              case SSTM_GJKEPA:
              case SSTM_GJKEPA_RADIUS_NOT_FULL_MARGIN: {
                result = btComputeGjkEpaSphereSphereCollision(ssd, &distInfo,
                                                              method);
                break;
              }
              default: {
...
#endif
            ASSERT_EQ(0, result);
            ASSERT_NEAR(btFabs(btScalar(i - z)) - btScalar(j) - ssd.m_radiusB,
                        distInfo.m_distance, abs_error);
            btVector3 computedA = distInfo.m_pointOnB +
                                  distInfo.m_distance * distInfo.m_normalBtoA;
