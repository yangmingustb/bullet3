...
void loadBufferData() {
  ...
  Vertex vertexData[4] = {{vec4(-0.5, -0.5, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
                           vec2(0.0078125, 0.015625)},
                          {vec4(-0.5, 0.5, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
                           vec2(0.101562, 0.015625)},
                          {vec4(0.5, 0.5, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
                           vec2(0.101562, 0.105469)},
                          {vec4(0.5, -0.5, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
                           vec2(0.0078125, 0.105469)}};

  Vertex vertexData2[4] = {
      {vec4(0, 0.901042, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
       vec2(0.0078125, 0.015625)},
      {vec4(0.0234375, 0.901042, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
       vec2(0.101562, 0.015625)},
      {vec4(0.0234375, 0.871094, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
       vec2(0.101562, 0.105469)},
      {vec4(0., 0.871094, 0.0, 1.0), vec4(1.0, 1.0, 1.0, 1.0),
       vec2(0.0078125, 0.105469)}};
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * sizeof(int), indexData,
               GL_STATIC_DRAW);
  ...
  glVertexAttribPointer(sData.m_positionAttribute, 4, GL_FLOAT, GL_FALSE,
                        sizeof(Vertex), (const GLvoid*)0);
  glVertexAttribPointer(sData.m_colourAttribute, 4, GL_FLOAT, GL_FALSE,
                        sizeof(Vertex), (const GLvoid*)sizeof(vec4));
  glVertexAttribPointer(sData.m_textureAttribute, 2, GL_FLOAT, GL_FALSE,
                        sizeof(Vertex),
                        (const GLvoid*)(sizeof(vec4) + sizeof(vec4)));
  err = glGetError();
  b3Assert(err == GL_NO_ERROR);
}
...
static const char* fragmentShader =
    ...
    "//	vec4 texcolor = "
    "vec4(texcolorred.x,texcolorred.x,texcolorred.x,texcolorred.x);\n"
...
void loadShader() {
  ...
  sData.m_positionAttribute =
      glGetAttribLocation(sData.m_shaderProg, "position");
...
void Usage() {
  printf(
      "\nprogram.exe [--pause_simulation=<0 or 1>] "
      "[--load_bulletfile=test.bullet] [--enable_interop=<0 or 1>] "
      "[--enable_gpusap=<0 or 1>] [--enable_convexheightfield=<0 or 1>] "
      "[--enable_static=<0 or 1>] [--x_dim=<int>] [--y_dim=<num>] "
      "[--z_dim=<int>] [--x_gap=<float>] [--y_gap=<float>] "
      "[--z_gap=<float>]\n");
};
...
int main(int argc, char* argv[]) {
  ...
  SimpleOpenGL2RenderCallbacks* renderCallbacks =
      new SimpleOpenGL2RenderCallbacks(&sData);
...
// Load the first truetype font from memory (just because we can).
  ...
  sprintf(
      fullFontFileName, "%s%s", fontPath,
      "DroidSerif-Regular.ttf");  // cour.ttf");//times.ttf");//DroidSerif-Regular.ttf");
  ...
  while (!window->requestedExit()) {
    ...
    if (1) {
      ...
      // sth_draw_text(stash, droidRegular,12.f, dx, dy-50, "How does this
      // OpenGL True Type font look? ", &dx,width,height);
      int spacing = 512;
      if (1)
        for (int i = 20; i <= 110; i += 12) {
          ...
          sprintf(
              ...
              i);
          sth_draw_text(stash, droidRegular, i, 10, dy - spacing, txt, &dx,
                        width, height);
          spacing -= i;
        }
      ...
      if (0)
        for (int i = 0; i < 1; i++) {
          dx = sx;
          if (once != 1) {
            // need to save this file as UTF-8 without signature, codepage
            // 650001 in Visual Studio
            err = glGetError();
            b3Assert(err == GL_NO_ERROR);

            // sth_draw_text(stash, droidJapanese,16.f, dx, dy-36, (const char*)
            // "\xE7\xA7\x81\xE3\x81\xAF\xE3\x82\xAC\xE3\x83\xA9\xE3\x82\xB9\xE3\x82\x92\xE9\xA3\x9F\xE3\x81\xB9\xE3\x82\x89\xE3\x82\x8C\xE3\x81\xBE\xE3\x81\x99\xE3\x80\x82",&dx,
            //              width,height);//はabcdefghijlkmnopqrstuvwxyz
            //              ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()_-+=?/\][{}.,<>`~@#$%^",
            //              &dx);
            //				sth_draw_text(stash, droidJapanese,32.f, dx, dy, (const
            //char*)
            //"私はガラスを食べられます。それは私を傷つけません。",&dx);//はabcdefghijlkmnopqrstuvwxyz
            //ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()_-+=?/\][{}.,<>`~@#$%^",
            //&dx);
            ...
            dx = 0;
            sth_draw_text(stash, droidRegular, 14.f, dx, dy - 80,
                          "How does this OpenGL True Type font look? ", &dx,
                          width, height);
            dx = 0;
            dy -= 30;

            // sth_draw_text(stash, droidRegular,16.f, dx, dy-80, "Profile How
            // does this OpenGL True Type font look? ", &dx,width,height);
            dx = 0;
            dy -= 30;

            sth_draw_text(stash, droidRegular, 16.f, dx, dy - 80,
                          "ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890", &dx, width,
                          height);
            dx = 0;
            dy -= 30;
            sth_draw_text(stash, droidRegular, 16.f, dx, dy - 80,
                          "!@#$%^abcdefghijklmnopqrstuvwxyz", &dx, width,
                          height);

            dx = 0;
            //	sth_draw_text(stash, droidRegular,16.f, dx, dy-42, "aph OpenGL
            //Profile aCABCabdabcdefghijlkmnopqrstuvwxyz
            //ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^", &dx,width,height);
            // sth_draw_text(stash, droidRegular,16.f, dx, dy-42, "aph OpenGL
            // Profile aCABCabdabcdefghijlkmnopqrstuvwxyz
            // ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^", &dx,width,height);
            ...
            sth_draw_texture(stash, droidRegular, 16.f, 0, 0, width, height,
                             "a", &dx);
            err = glGetError();
            b3Assert(err == GL_NO_ERROR);
