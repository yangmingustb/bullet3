// todo: turn this into a gtest, comparing maximal and reduced coordinates
// contact behavior etc
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < NUM_SIM; i++) {
    ...
    // Can also use
    // eCONNECT_DIRECT,eCONNECT_SHARED_MEMORY,eCONNECT_UDP,eCONNECT_TCP, for
    // example:
    // sim->connect(eCONNECT_UDP, "localhost", 1234);
    sim->configureDebugVisualizer(COV_ENABLE_GUI, 0);
    //	sim->configureDebugVisualizer( COV_ENABLE_SHADOWS,
    //0);//COV_ENABLE_WIREFRAME
    // sim->setTimeOut(3);
    ...
    args.m_useMultiBody = i == 0 ? false : true;  // true : false;//false :
                                                  // true;
    ...
    double distance = 1.5;
    double yaw = 50;
    sim->resetDebugVisualizerCamera(distance, yaw, 20,
                                    b3MakeVector3(0, 0, 0.1));
