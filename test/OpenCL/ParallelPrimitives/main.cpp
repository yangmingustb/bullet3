...
#define TEST_REPORT(testName)                                \
  ...
  if (g_testFailed)                                          \
    ...
    g_nPassed++;
#define NEXTMULTIPLEOF(num, alignment) \
  (((num) / (alignment) + (((num) % (alignment) == 0) ? 0 : 1)) * (alignment))
...
void initCL(int preferredDeviceIndex, int preferredPlatformIndex) {
  ...
  // void* glDC = 0;
  int ciErrNum = 0;
  // bound search and radix sort only work on GPU right now (assume 32 or 64
  // width workgroup without barriers)

  cl_device_type deviceType = CL_DEVICE_TYPE_ALL;

  g_context = b3OpenCLUtils::createContextFromType(deviceType, &ciErrNum, 0, 0,
...
__inline void seedRandom(int seed) { srand(seed); }
...
struct b3SortDataCompare {
  inline bool operator()(const b3SortData& first,
                         const b3SortData& second) const {
    return (first.m_key < second.m_key) ||
           (first.m_key == second.m_key && first.m_value < second.m_value);
  }
};
...
void boundSearchTest() {
  ...
  b3BoundSearchCL* search =
      new b3BoundSearchCL(g_context, g_device, g_queue, maxSize);
  ...
  for (int iter = 0; iter < NUM_TESTS; iter++) {
    ...
    search->execute(srcCL, size, upperCL, bucketSize,
                    b3BoundSearchCL::BOUND_UPPER);
    search->execute(srcCL, size, lowerCL, bucketSize,
                    b3BoundSearchCL::BOUND_LOWER);

    search->executeHost(srcHost, size, upperHostCompare, bucketSize,
                        b3BoundSearchCL::BOUND_UPPER);
    search->executeHost(srcHost, size, lowerHostCompare, bucketSize,
                        b3BoundSearchCL::BOUND_LOWER);
...
void prefixScanTest() {
  ...
  b3PrefixScanCL* scan =
      new b3PrefixScanCL(g_context, g_device, g_queue, maxSize);
  ...
  for (int iter = 0; iter < NUM_TESTS; iter++) {
    ...
    for (int i = 0; i < size; i++) buf0Host[i] = 1;
    ...
    for (int i = 0; i < size; i++) TEST_ASSERT(buf1Host[i] == buf0Host[i]);
  }
...
bool radixSortTest() {
  ...
  b3RadixSort32CL* sort =
      new b3RadixSort32CL(g_context, g_device, g_queue, maxSize);
  ...
  for (int iter = 0; iter < NUM_TESTS; iter++) {
    ...
    for (int i = 0; i < size; i++) {
      TEST_ASSERT(buf0Host[i].m_value == buf1Host[i].m_value &&
                  buf0Host[i].m_key == buf1Host[i].m_key);
    }
  }
