...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < numPlatforms; i++) {
    ...
    g_cxMainContext = b3OpenCLUtils::createContextFromPlatform(
    ...
    for (int j = 0; j < numDevices; j++) {
      ...
      g_cqCommandQue =
          clCreateCommandQueue(g_cxMainContext, device, 0, &ciErrNum);
  ...
  void* glDC = 0;
  b3Printf(
      "Initialize OpenCL using b3OpenCLUtils::createContextFromType for "
      "CL_DEVICE_TYPE_GPU\n");
  g_cxMainContext =
      b3OpenCLUtils::createContextFromType(deviceType, &ciErrNum, glCtx, glDC);
