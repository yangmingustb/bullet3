...
static const char* s_testKernelString = MSTRINGIFY(
    ...
    struct MyTest { int bla; };

    typedef float4 b3Float4; typedef struct b3Contact4Data b3Contact4Data_t;
    struct b3Contact4Data {
      b3Float4 m_worldPos[4];
      b3Float4 m_localPosA[4];
      b3Float4 m_localPosB[4];
    ...
    inline int b3Contact4Data_getNumPoints(
        const struct b3Contact4Data* contact) {
      return (int)contact->m_worldNormal.w;
    };
    inline void b3Contact4Data_setNumPoints(struct b3Contact4Data* contact,
                                            int numPoints) {
    ...
    __kernel void testKernel(__global int* testData,
                             __global b3Contact4Data_t* contactData,
                             my_counter32_t numElements) {
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < numPlatforms; i++) {
    ...
    cl_context context = b3OpenCLUtils::createContextFromPlatform(
        platform, deviceType, &ciErrNum);
    if (context) {
      ...
      for (int j = 0; j < numDevices; j++) {
        ...
        cl_kernel testKernel = b3OpenCLUtils::compileCLKernelFromString(
            context, dev, s_testKernelString, "testKernel", &errNum,
            pairBenchProg);
        if (testKernel) {
