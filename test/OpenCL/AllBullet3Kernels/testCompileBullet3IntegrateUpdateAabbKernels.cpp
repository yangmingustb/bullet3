...
TEST_F(testCompileBullet3IntegrateUpdateAabbKernels, integrateKernelCL) {
  ...
  cl_program prog = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, integrateKernelCL, &errNum, "", 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, integrateKernelCL, "integrateTransformsKernel",
        &errNum, prog);
    ASSERT_EQ(CL_SUCCESS, errNum);
    ASSERT_FALSE(k == 0);
    clReleaseKernel(k);
...
TEST_F(testCompileBullet3IntegrateUpdateAabbKernels, updateAabbsKernelCL) {
  cl_int errNum = 0;
  cl_program prog = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, updateAabbsKernelCL, &errNum, "", 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, updateAabbsKernelCL, "initializeGpuAabbsFull",
        &errNum, prog);
    ASSERT_EQ(CL_SUCCESS, errNum);
    ASSERT_FALSE(k == 0);
    clReleaseKernel(k);
  ...
  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, updateAabbsKernelCL,
