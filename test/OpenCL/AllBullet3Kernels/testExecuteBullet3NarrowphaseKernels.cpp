        ...
        if (result == UNZ_OK) {
          ...
          if (result != UNZ_OK) {
            ...
            if (result == UNZ_OK) {
              ...
              result =
                  unzReadCurrentFile(zipfile, buffer, info.uncompressed_size);
              if (result < 0) {
                free(buffer);
                buffer = 0;
              } else {
...
void testLauncher(const char* fileName2, b3LauncherCL& launcher,
                  cl_context ctx) {
  int sizeInBytes = 0;
...

  {
    ...
    if (1) {
      const char* fileNames[] = {
          "mprPenetrationKernel60.bin", "mprPenetrationKernel61.bin",
          "mprPenetrationKernel70.bin", "mprPenetrationKernel128.bin"};
