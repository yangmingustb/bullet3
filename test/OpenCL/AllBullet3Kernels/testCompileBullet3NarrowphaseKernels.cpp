...
TEST_F(CompileBullet3NarrowphaseKernels, satKernelsCL) {
  ...
  cl_program satProg = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, satKernelsCL, &errNum, flags, 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel m_findSeparatingAxisKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clContext, m_clDevice, satKernelsCL, "findSeparatingAxisKernel",
            &errNum, satProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
...

  {
    cl_kernel m_findSeparatingAxisVertexFaceKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clContext, m_clDevice, satKernelsCL,
...

  {
    cl_kernel m_findCompoundPairsKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clContext, m_clDevice, satKernelsCL, "findCompoundPairsKernel",
            &errNum, satProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
...

  {
    cl_kernel m_processCompoundPairsKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clContext, m_clDevice, satKernelsCL, "processCompoundPairsKernel",
            &errNum, satProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
...
TEST_F(CompileBullet3NarrowphaseKernels, satConcaveKernelsCL) {
  ...
  cl_program satConcaveProg = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, satConcaveKernelsCL, &errNum, flags, 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel m_findConcaveSeparatingAxisVertexFaceKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clContext, m_clDevice, satConcaveKernelsCL,
...
TEST_F(CompileBullet3NarrowphaseKernels, satClipKernelsCL) {
  ...
  cl_program satClipContactsProg = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, satClipKernelsCL, &errNum, flags, 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel m_clipHullHullKernel = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, satClipKernelsCL, "clipHullHullKernel",
...

  {
    cl_kernel m_clipCompoundsHullHullKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            m_clContext, m_clDevice, satClipKernelsCL,
            "clipCompoundsHullHullKernel", &errNum, satClipContactsProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(m_clipCompoundsHullHullKernel);
  }

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, satClipKernelsCL, "findClippingFacesKernel",
        &errNum, satClipContactsProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, satClipKernelsCL,
        "clipFacesAndFindContactsKernel", &errNum, satClipContactsProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, satClipKernelsCL,
        "clipHullHullConcaveConvexKernel", &errNum, satClipContactsProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }
  ...
  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, satClipKernelsCL, "newContactReductionKernel",
        &errNum, satClipContactsProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }
...
TEST_F(CompileBullet3NarrowphaseKernels, bvhTraversalKernels) {
  ...
  cl_program bvhTraversalProg = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, bvhTraversalKernelCL, &errNum, "", 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, bvhTraversalKernelCL, "bvhTraversalKernel",
        &errNum, bvhTraversalProg, "");
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }
...
TEST_F(CompileBullet3NarrowphaseKernels, primitiveContactsKernelsCL) {
  ...
  cl_program primitiveContactsProg = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, primitiveContactsKernelsCL, &errNum, "", 0,
      true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, primitiveContactsKernelsCL,
...
TEST_F(CompileBullet3NarrowphaseKernels, mprKernelsCL) {
  ...
  cl_program mprProg = b3OpenCLUtils::compileCLProgramFromString(
      m_clContext, m_clDevice, mprKernelsCL, &errNum, flags, 0, true);
  ASSERT_EQ(CL_SUCCESS, errNum);

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        m_clContext, m_clDevice, mprKernelsCL, "mprPenetrationKernel", &errNum,
        mprProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }

  {
    cl_kernel k = b3OpenCLUtils::compileCLKernelFromString(
        ...
        "findSeparatingAxisUnitSphereKernel", &errNum, mprProg);
    ASSERT_EQ(CL_SUCCESS, errNum);
    clReleaseKernel(k);
  }
