 ...
 * AUTHORS' REQUEST:
 *
 * 		If you use|reference|benchmark this code, please cite our
 *Technical
 * 		Report
...
/// Note that b3OpenCLUtils might still use the preference of using a platform
/// vendor that matches the SDK vendor used to build the application.
...
/**
 * Keys-only sorting.  Uses the GPU to sort the specified vector of elements for
 * the given
 ...
 */
template <typename K>
void TimedSort(unsigned int num_elements, K *h_keys, unsigned int iterations) {
  ...
  {
    // printf("Key-values, %d iterations, %d elements", iterations,
    // num_elements);
...
/**
 * Key-value sorting.  Uses the GPU to sort the specified vector of elements for
 * the given
 * number of iterations, displaying runtime information.
 *
 * @param[in] 		num_elements
...
void TimedSort(unsigned int num_elements, K *h_keys, V *h_values,
 ...
 * to the GPU for the given number of iterations, displaying runtime
 * information.
 *
 * @param[in] 		iterations
 * 		Number of times to invoke the GPU sorting primitive
 ...
 */
template <typename K, typename V>
void TestSort(unsigned int iterations, int num_elements, bool keys_only) {
  // Allocate the sorting problem on the host and fill the keys with random
  // bytes
  ...
  for (unsigned int i = 0; i < num_elements; ++i) {
    ...
    if (!keys_only) h_values[i] = h_keys[i];  // 0xffffffffu-i;
...
void Usage() {
  printf(
      "\ntest_large_problem_sorting [--device=<device index>] [--v] "
      "[--i=<num-iterations>] [--n=<num-elements>] [--key-values] "
      "[--deviceId=<int>] [--platformId=<int>]\n");
  printf("\n");
  printf("\t--v\tDisplays sorted results to the console.\n");
  printf("\n");
  printf("\t--i\tPerforms the sorting operation <num-iterations> times\n");
  printf(
      "\t\t\ton the device. Re-copies original input each time. Default = 1\n");
  printf("\n");
  printf("\t--n\tThe number of elements to comprise the sample problem\n");
  printf("\t\t\tDefault = 512\n");
  printf("\n");
  printf(
      "\t--key-values\tSpecifies that keys are accommodated by value "
      "pairings\n");
  printf("\n");
}
...
template <>
void b3CommandLineArgs::GetCmdLineArgument<char *>(const char *arg_name,
                                                   char *&val) {
  using namespace std;
  map<string, string>::iterator itr;
  if ((itr = pairs.find(arg_name)) != pairs.end()) {
...
void myprintf(const char *msg) { (void *)msg; }
int main(int argc, char **argv) {
  ...
  //	g_cxMainContext =
  //b3OpenCLUtils_createContextFromType(CL_DEVICE_TYPE_ALL, &ciErrNum, 0,
  //0,gPreferredDeviceId,gPreferredPlatformId,&platformId);
  g_cxMainContext = b3OpenCLUtils_createContextFromType(
      CL_DEVICE_TYPE_GPU, &ciErrNum, 0, 0, gPreferredDeviceId,
      gPreferredPlatformId, &platformId);
  // g_cxMainContext = b3OpenCLUtils_createContextFromType(CL_DEVICE_TYPE_CPU,
  // &ciErrNum, 0, 0,gPreferredDeviceId,gPreferredPlatformId,&platformId);
  ...
  g_cqCommandQueue =
      clCreateCommandQueue(g_cxMainContext, g_device, 0, &ciErrNum);
  oclCHECKERROR(ciErrNum, CL_SUCCESS);
  ...
  unsigned int num_elements =
      8 * 1024 *
      1024;  // 4*1024*1024;//4*1024*1024;//257;//8*524288;//2048;//512;//524288;
  ...
  TestSort<unsigned int, unsigned int>(iterations, num_elements, keys_only);
}
