...
SIMD_FORCE_INLINE bool Intersect_ref(btDbvtAabbMm& a, btDbvtAabbMm& b) {
  return (
      (a.tMins().x() <= b.tMaxs().x()) && (a.tMaxs().x() >= b.tMins().x()) &&
      (a.tMins().y() <= b.tMaxs().y()) && (a.tMaxs().y() >= b.tMins().y()) &&
      (a.tMins().z() <= b.tMaxs().z()) && (a.tMaxs().z() >= b.tMins().z()));
}

SIMD_FORCE_INLINE btScalar Proximity_ref(btDbvtAabbMm& a, btDbvtAabbMm& b) {
  const btVector3 d = (a.tMins() + a.tMaxs()) - (b.tMins() + b.tMaxs());
  return (btFabs(d.x()) + btFabs(d.y()) + btFabs(d.z()));
}

SIMD_FORCE_INLINE int Select_ref(btDbvtAabbMm& o, btDbvtAabbMm& a,
                                 btDbvtAabbMm& b) {
  return (Proximity_ref(o, a) < Proximity_ref(o, b) ? 0 : 1);
}

SIMD_FORCE_INLINE void Merge_ref(btDbvtAabbMm& a, btDbvtAabbMm& b,
                                 btDbvtAabbMm& r) {
  //
  // Changing '3' into '4' to compare with the vector code which changes all 4
  // floats.
  // Erwin: don't do this because the 4th component is ignore and not computed
  // on non-vector code (there is no NEON version and scalar is just 3
  // components)
  //
  for (int i = 0; i < 3; ++i) {
...
int Test_btDbvt(void) {
  ...
  for (i = 0; i < DATA_SIZE; i++) {
    ...
    if (Intersect_Test_Res[i] != Intersect_Ref_Res[i]) {
      ...
      printf(
          "a_mx_f[0] = %.3f, a_mx_f[1] = %.3f, a_mx_f[2] = %.3f, a_mx_f[3] = "
          "%.3f\n",
          a[i].tMaxs().m_floats[0], a[i].tMaxs().m_floats[1],
          a[i].tMaxs().m_floats[2], a[i].tMaxs().m_floats[3]);
      printf(
          "a_mi_f[0] = %.3f, a_mi_f[1] = %.3f, a_mi_f[2] = %.3f, a_mi_f[3] = "
          "%.3f\n",
          a[i].tMins().m_floats[0], a[i].tMins().m_floats[1],
          a[i].tMins().m_floats[2], a[i].tMins().m_floats[3]);
      printf(
          "b_mx_f[0] = %.3f, b_mx_f[1] = %.3f, b_mx_f[2] = %.3f, b_mx_f[3] = "
          "%.3f\n",
          b[i].tMaxs().m_floats[0], b[i].tMaxs().m_floats[1],
          b[i].tMaxs().m_floats[2], b[i].tMaxs().m_floats[3]);
      printf(
          "b_mi_f[0] = %.3f, b_mi_f[1] = %.3f, b_mi_f[2] = %.3f, b_mi_f[3] = "
          "%.3f\n",
          b[i].tMins().m_floats[0], b[i].tMins().m_floats[1],
          b[i].tMins().m_floats[2], b[i].tMins().m_floats[3]);

      printf(
          "a_mx_f_ref[0] = %.3f, a_mx_f_ref[1] = %.3f, a_mx_f_ref[2] = %.3f, "
          "a_mx_f_ref[3] = %.3f\n",
          a_ref[i].tMaxs().m_floats[0], a_ref[i].tMaxs().m_floats[1],
          a_ref[i].tMaxs().m_floats[2], a_ref[i].tMaxs().m_floats[3]);
      printf(
          "a_mi_f_ref[0] = %.3f, a_mi_f_ref[1] = %.3f, a_mi_f_ref[2] = %.3f, "
          "a_mi_f_ref[3] = %.3f\n",
          a_ref[i].tMins().m_floats[0], a_ref[i].tMins().m_floats[1],
          a_ref[i].tMins().m_floats[2], a_ref[i].tMins().m_floats[3]);
      printf(
          "b_mx_f_ref[0] = %.3f, b_mx_f_ref[1] = %.3f, b_mx_f_ref[2] = %.3f, "
          "b_mx_f_ref[3] = %.3f\n",
          b_ref[i].tMaxs().m_floats[0], b_ref[i].tMaxs().m_floats[1],
          b_ref[i].tMaxs().m_floats[2], b_ref[i].tMaxs().m_floats[3]);
      printf(
          "b_mi_f_ref[0] = %.3f, b_mi_f_ref[1] = %.3f, b_mi_f_ref[2] = %.3f, "
          "b_mi_f_ref[3] = %.3f\n",
          b_ref[i].tMins().m_floats[0], b_ref[i].tMins().m_floats[1],
          b_ref[i].tMins().m_floats[2], b_ref[i].tMins().m_floats[3]);
    }
  }
#endif
...
*/
  for (i = 0; i < DATA_SIZE; i++) {
    ...
    if ((fabs(c[i].tMaxs().m_floats[0] - c_ref[i].tMaxs().m_floats[0]) >
         0.001) ||
        (fabs(c[i].tMaxs().m_floats[1] - c_ref[i].tMaxs().m_floats[1]) >
         0.001) ||
        (fabs(c[i].tMaxs().m_floats[2] - c_ref[i].tMaxs().m_floats[2]) >
         0.001) ||
        // (fabs(c[i].tMaxs().m_floats[3] - c_ref[i].tMaxs().m_floats[3]) >
        // 0.001) ||
        (fabs(c[i].tMins().m_floats[0] - c_ref[i].tMins().m_floats[0]) >
         0.001) ||
        (fabs(c[i].tMins().m_floats[1] - c_ref[i].tMins().m_floats[1]) >
         0.001) ||
        (fabs(c[i].tMins().m_floats[2] - c_ref[i].tMins().m_floats[2]) > 0.001)
        //|| (fabs(c[i].tMins().m_floats[3] - c_ref[i].tMins().m_floats[3]) >
        //0.001)
        )

    // if((c[i].tMaxs().m_floats[0] != c_ref[i].tMaxs().m_floats[0]) ||
    // (c[i].tMaxs().m_floats[1] != c_ref[i].tMaxs().m_floats[1]) ||
    // (c[i].tMaxs().m_floats[2] != c_ref[i].tMaxs().m_floats[2]) ||
    // (c[i].tMaxs().m_floats[3] != c_ref[i].tMaxs().m_floats[3]) ||
    // (c[i].tMins().m_floats[0] != c_ref[i].tMins().m_floats[0]) ||
    // (c[i].tMins().m_floats[1] != c_ref[i].tMins().m_floats[1]) ||
    // (c[i].tMins().m_floats[2] != c_ref[i].tMins().m_floats[2]) ||
    // (c[i].tMins().m_floats[3] != c_ref[i].tMins().m_floats[3]))
    {
      printf("Merge fail at %d with test = %d, ref = %d\n", i,
             Select_Test_Res[i], Select_Ref_Res[i]);
      ...
      printf(
          ...
          b[i].tMins().m_floats[2], b[i].tMins().m_floats[3]);
      printf(
          "c_mx_f[0] = %.3f, c_mx_f[1] = %.3f, c_mx_f[2] = %.3f, c_mx_f[3] = "
          "%.3f\n",
          c[i].tMaxs().m_floats[0], c[i].tMaxs().m_floats[1],
          c[i].tMaxs().m_floats[2], c[i].tMaxs().m_floats[3]);
      printf(
          "c_mi_f[0] = %.3f, c_mi_f[1] = %.3f, c_mi_f[2] = %.3f, c_mi_f[3] = "
          "%.3f\n",
          c[i].tMins().m_floats[0], c[i].tMins().m_floats[1],
          c[i].tMins().m_floats[2], c[i].tMins().m_floats[3]);
      ...
      printf(
          ...
          b_ref[i].tMins().m_floats[2], b_ref[i].tMins().m_floats[3]);
      printf(
          "c_mx_f_ref[0] = %.3f, c_mx_f_ref[1] = %.3f, c_mx_f_ref[2] = %.3f, "
          "c_mx_f_ref[3] = %.3f\n",
          c_ref[i].tMaxs().m_floats[0], c_ref[i].tMaxs().m_floats[1],
          c_ref[i].tMaxs().m_floats[2], c_ref[i].tMaxs().m_floats[3]);
      printf(
          "c_mi_f_ref[0] = %.3f, c_mi_f_ref[1] = %.3f, c_mi_f_ref[2] = %.3f, "
          "c_mi_f_ref[3] = %.3f\n",
          c_ref[i].tMins().m_floats[0], c_ref[i].tMins().m_floats[1],
          c_ref[i].tMins().m_floats[2], c_ref[i].tMins().m_floats[3]);
      return 1;
    }
  }
