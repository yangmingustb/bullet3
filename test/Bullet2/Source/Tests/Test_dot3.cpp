...
static btVector3 dot3_ref(const btVector3 &, const btVector3 &,
                          const btVector3 &, const btVector3 &);
static btVector3 dot3_ref(const btVector3 &v, const btVector3 &v1,
...
SIMD_FORCE_INLINE int operator!=(const btVector3 &s, const btVector3 &v)
{
    ...
    uint32x4_t test = vandq_u32( vceqq_f32( s.mVec128, v.mVec128 ),
(uint32x4_t){-1,-1,-1,0});
...
#define DATA_SIZE 1024
  ...
  for (k = 0; k < DATA_SIZE; k++) {
    ...
    if (correct != test) {
      vlog(
          "Error (%ld) - dot3 result error! *{%a, %a, %a, %a} != {%a, %a, %a, "
          "%a} \n",
          k, correct.x(), correct.y(), correct.z(), correct.w(), test.x(),
          test.y(), test.z(), test.w());
...

  {
    ...
    for (j = 0; j < NUM_CYCLES; j++) {
      ...
      for (k = 0; k + 4 <= LOOPCOUNT; k += 4) {
        ...
        res_arr[k32] = dot3_ref(vec3_arr[k32], vec3_arr1[k32], vec3_arr2[k32],
                                vec3_arr3[k32]);
        k32++;
        res_arr[k32] = dot3_ref(vec3_arr[k32], vec3_arr1[k32], vec3_arr2[k32],
                                vec3_arr3[k32]);
...

  {
    ...
    for (j = 0; j < NUM_CYCLES; j++) {
      ...
      for (k = 0; k + 4 <= LOOPCOUNT; k += 4) {
        size_t k32 = (k & (DATA_SIZE - 1));
        res_arr[k32] =
            vec3_arr[k32].dot3(vec3_arr1[k32], vec3_arr2[k32], vec3_arr3[k32]);
        k32++;
        res_arr[k32] =
            vec3_arr[k32].dot3(vec3_arr1[k32], vec3_arr2[k32], vec3_arr3[k32]);
        k32++;
