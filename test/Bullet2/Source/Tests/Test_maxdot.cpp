...
static long maxdot_ref(const btSimdFloat4 *vertices, float *vec, size_t count,
                       float *dotResult);
...
int Test_maxdot(void) {
  ...
  btSimdFloat4 *data =
      (btSimdFloat4 *)GuardCalloc(1, MAX_SIZE * sizeof(btSimdFloat4), NULL);
  float *fp = (float *)data;
  long correct, test;
  ...
  for (size = 1; size <= MAX_SIZE; size++) {
    ...
    for (position = 0; position < size; position++) {
      ...
      correctDot = BT_NAN;
      testDot = BT_NAN;
      correct = maxdot_ref((btSimdFloat4 *)in, (float *)&localScaling, size,
                           &correctDot);
      ...
      if (correct != test) {
        vlog("Error @ %ld: index misreported! *%ld vs %ld  (*%f, %f)\n", size,
             correct, test,
             fp[0] * in[4 * correct] + fp[1] * in[4 * correct + 1] +
                 fp[2] * in[4 * correct + 2],
             fp[0] * in[4 * test] + fp[1] * in[4 * test + 1] +
                 fp[2] * in[4 * test + 2]);
        return 1;
      }
      if (test != position) {
        vlog(
            "Biggest not found where it is supposed to be: *%ld vs %ld (*%f, "
            "%f)\n",
            position, test, fp[0] * in[4 * test] + fp[1] * in[4 * test + 1] +
                                fp[2] * in[4 * test + 2],
            fp[0] * in[4 * position] + fp[1] * in[4 * position + 1] +
                fp[2] * in[4 * position + 2]);
        return 1;
      }
      ...
      if (correctDot != testDot) {
        ...
        if (relativeError > 1e-6) {
          ...
          if (maxRelativeError < relativeError) {
            ...
            sprintf(
                errStr,
                "Warning @ %ld: dotpr misreported! *%f vs %f    (*%f, %f)\n",
  ...
  for (size = 1; size <= 32; size++)
    vlog("%5lu\t%10.2f\t%10.2f\n", size,
         TicksToCycles(scalarTimes[size]) / LOOPCOUNT,
         TicksToCycles(vectorTimes[size]) / LOOPCOUNT);
  size_t index = 33;
  for (size = 64; size <= MAX_SIZE; size *= 2) {
    vlog("%5lu\t%10.2f\t%10.2f\n", size,
         TicksToCycles(scalarTimes[index]) / LOOPCOUNT,
         TicksToCycles(vectorTimes[index]) / LOOPCOUNT);
    index++;
  }
