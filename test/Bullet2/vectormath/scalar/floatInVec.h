...
//
inline const floatInVec select(floatInVec vec0, floatInVec vec1,
...
namespace Vectormath {
inline floatInVec::floatInVec(boolInVec vec) { mData = float(vec.getAsBool()); }

inline floatInVec::floatInVec(float scalar) { mData = scalar; }

inline float floatInVec::getAsFloat() const { return mData; }

#ifndef _VECTORMATH_NO_SCALAR_CAST
inline floatInVec::operator float() const { return getAsFloat(); }
#endif
...
inline floatInVec& floatInVec::operator=(floatInVec vec) {
  mData = vec.mData;
  return *this;
}
...
inline const floatInVec operator/(floatInVec num, floatInVec den) {
