...
inline const Vector3 Quat::getXYZ() const { return Vector3(mX, mY, mZ); }
...
inline float Quat::getX() const { return mX; }
...
inline float Quat::getY() const { return mY; }
...
inline float Quat::getZ() const { return mZ; }
...
inline float Quat::getW() const { return mW; }
...
inline float Quat::getElem(int idx) const { return *(&mX + idx); }

inline float &Quat::operator[](int idx) { return *(&mX + idx); }

inline float Quat::operator[](int idx) const { return *(&mX + idx); }

inline const Quat Quat::operator+(const Quat &quat) const {
  return Quat((mX + quat.mX), (mY + quat.mY), (mZ + quat.mZ), (mW + quat.mW));
}
...
inline const Quat Quat::operator*(float scalar) const {
  return Quat((mX * scalar), (mY * scalar), (mZ * scalar), (mW * scalar));
}
...
inline const Quat Quat::operator-() const { return Quat(-mX, -mY, -mZ, -mW); }
