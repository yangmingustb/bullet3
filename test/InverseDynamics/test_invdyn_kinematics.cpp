// Test of kinematic consistency: check if finite differences of velocities,
// accelerations
// match positions
...
idScalar calculateNorm(vec3& v) {
  return BT_ID_SQRT(BT_ID_POW(v(0), 2) + BT_ID_POW(v(1), 2) +
                    BT_ID_POW(v(2), 2));
}
...
class DiffFD {
 public:
  ...
  void printCurrent() {
    if (m_valid_fd) {
      // note: m_old_true_diff already equals m_true_diff here, so values are
      // not aligned.
      //      (but error calculation takes this into account)
      printf("%s time: %e fd: %e %e %e true: %e %e %e\n", m_name.c_str(),
             m_time, m_diff_fd(0), m_diff_fd(1), m_diff_fd(2),
             m_old_true_diff(0), m_old_true_diff(1), m_old_true_diff(2));
    }
  }
...
class VecDiffFD {
 public:
  VecDiffFD(std::string name, int dim, idScalar dt)
      : m_name(name), m_fd(dim), m_dt(dt) {
  ...
  void update(int i, ValueType& val, DiffType& true_diff) {
    m_fd[i].update(val, true_diff);
  }
...
// calculate maximum difference between finite difference and analytical
// differentiation
int calculateDifferentiationError(const MultiBodyTreeCreator& creator,
                                  idScalar deltaT, idScalar endTime,
  ...
  // set gravity to zero, so nothing is added to accelerations in forward
  // kinematics
  vec3 gravity_zero;
  gravity_zero(0) = 0;
  gravity_zero(1) = 0;
  ...
  VecDiffFD<vec3, vec3> fd_acc("linear-acceleration", tree->numBodies(),
                               deltaT);
  VecDiffFD<mat33, vec3> fd_omg("angular-velocity", tree->numBodies(), deltaT);
  VecDiffFD<vec3, vec3> fd_omgd("angular-acceleration", tree->numBodies(),
                                deltaT);
  ...
  for (idScalar t = 0.0; t < endTime; t += deltaT) {
    for (int body = 0; body < tree->numBodies(); body++) {
      ...
      dot_q(body) = kAmplitude * 2.0 * BT_ID_PI * kFrequency *
                    cos(t * 2.0 * BT_ID_PI * kFrequency);
      ddot_q(body) = -kAmplitude * pow(2.0 * BT_ID_PI * kFrequency, 2) *
                     sin(t * 2.0 * BT_ID_PI * kFrequency);
    }
  ...
  *max_angular_acceleration_error =
      fd_omgd.getMaxError() / fd_omgd.getMaxValue();
...
// CAVEAT:these values are hand-tuned to work for the specific trajectory
// defined above.
  ...
  // test serial chain
  calculateDifferentiationError(
      coil_creator, kDeltaT, kDuration, &max_linear_velocity_error,
  ...
  // test branched tree
  calculateDifferentiationError(
      dill_creator, kDeltaT, kDuration, &max_linear_velocity_error,
      &max_angular_velocity_error, &max_linear_acceleration_error,
      &max_angular_acceleration_error);
  ...
  calculateDifferentiationError(
      simple_creator, kDeltaT, kDuration, &max_linear_velocity_error,
      &max_angular_velocity_error, &max_linear_acceleration_error,
      &max_angular_acceleration_error);
...
TEST(InvDynKinematicsDifferentiation, errorOrder) {
  ...
  // test serial chain
  calculateDifferentiationError(
      coil_creator, kDeltaTs[0], kDuration, &max_linear_velocity_error[0],
      &max_angular_velocity_error[0], &max_linear_acceleration_error[0],
      &max_angular_acceleration_error[0]);

  calculateDifferentiationError(
      coil_creator, kDeltaTs[1], kDuration, &max_linear_velocity_error[1],
      &max_angular_velocity_error[1], &max_linear_acceleration_error[1],
