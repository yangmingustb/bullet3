...
///      - compare input accelerations to inverse dynamics to output from
///      forward dynamics
...
TEST(InvDynCompare, bulletUrdfR2D2) {
  ...
  for (int loop = 0; loop < kNLoops; loop++) {
    ...
    // call inverse dynamics once, to get global position & velocity of root
    // body
    // (fixed, so q, u, dot_u arbitrary)
    EXPECT_EQ(id_tree->calculateInverseDynamics(q, u, dot_u, &joint_forces), 0);

    EXPECT_EQ(
        compareInverseAndForwardDynamics(q, u, dot_u, gravity, FLAGS_verbose,
                                         btmb, id_tree, &pos_error, &acc_error),
        0);
