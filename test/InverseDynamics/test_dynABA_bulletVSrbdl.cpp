/// Then - run forward dynamics on random input data (q, u, dot_u) to get forces
/// with rbdl
///      - run forward dynamics on random input data (q, u, dot_u) to get forces
///      with bullet
...
int compareABAForwardDynamicsBulletAndRbdl(
    btAlignedObjectArray<btScalar> &q, btAlignedObjectArray<btScalar> &u,
    btAlignedObjectArray<btScalar> &joint_forces,
    btAlignedObjectArray<btScalar> &dot_u, btVector3 &gravity, bool verbose,
...
#define RETURN_ON_FAILURE(x)                   \
  ...
  if (q_index != q.size()) {
    bt_id_error_message(
        "error in number of dofs for btMultibody and MultiBodyTree\n");
    return -1;
  }
  ...
  // this triggers switch between using either appliedConstraintForce or
  // appliedForce
  ...
  for (int link = 0; link < btmb->getNumLinks(); link++) {
    ...
    if (verbose) {
      printf("link %d, applying gravity %f %f %f\n", link,
             gravity[0] * btmb->getLinkMass(link),
             gravity[1] * btmb->getLinkMass(link),
  ...
  btmb->computeAccelerationsArticulatedBodyAlgorithmMultiDof(
      dt, scratch_r, scratch_v, scratch_m, isConstraintPass, false, false);

  // read generalized accelerations back from btMultiBody
  // the mapping from scratch variables to accelerations is taken from the
  // implementation
  ...
  *acc_error = 0;
  ...
  if (true == btmb->hasFixedBase()) {
    for (int i = 0; i < btmb->getNumDofs(); i++) {
      if (verbose) {
        printf("bt:ddot_q[%d]= %.20f, id:ddot_q= %.20f, diff= %.20f\n", i,
               joint_accel[i], dot_u[i + dot_u_offset],
               joint_accel[i] - dot_u[i]);
      }
    ...
    // com happens to coincide with link origin here. If that changes, we need
    // to calculate
    // ddot_com
    ...
    for (int i = 0; i < 3; i++) {
      if (verbose) {
        printf("bt::base_dot_omega(%d)= %e dot_u[%d]= %e, diff= %e\n", i,
               ((btScalar *)base_dot_omega)[i], i, dot_u[i],
               ((btScalar *)base_dot_omega)[i] - dot_u[i]);
      }
    ...
    for (int i = 0; i < 3; i++) {
      if (verbose) {
        printf("bt::base_ddot_com(%d)= %e dot_u[%d]= %e, diff= %e\n", i,
               ((btScalar *)base_ddot_com)[i], i, dot_u[i + 3],
               ((btScalar *)base_ddot_com)[i] - dot_u[i + 3]);
      }
    ...
    for (int i = 0; i < btmb->getNumDofs(); i++) {
      if (verbose) {
        printf("bt:ddot_q[%d]= %.10f, id:ddot_q= %e, diff= %e\n", i,
               joint_accel[i], dot_u[i + 6], joint_accel[i] - dot_u[i + 6]);
      }
...
/// this test loads the a urdf model with fixed, floating, prismatic and
/// rotational joints,
/// converts in to an inverse dynamics model and compares forward to inverse
/// dynamics for
/// random input
TEST(DynCompareBulletAndRbdl, UrdfPantilt) {
  ...
  ASSERT_TRUE(
      b3ResourcePath::findResourcePath(kUrdfFile, relativeFileName, 1024, 0));
  ...
  if (!RigidBodyDynamics::Addons::URDFReadFromFile(relativeFileName, &model,
                                                   floatbase, verbose)) {
  ...
  for (int loop = 0; loop < kNLoops; loop++) {
    ...
    EXPECT_EQ(compareABAForwardDynamicsBulletAndRbdl(q, u, joint_forces, dot_u,
                                                     gravity, FLAGS_verbose,
                                                     btmb, &acc_error),
              0);
