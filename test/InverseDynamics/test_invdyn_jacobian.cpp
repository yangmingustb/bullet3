...
void calculateDotJacUError(const MultiBodyTreeCreator& creator,
                           const int nloops, double* max_error) {
  // tree1 is used as reference to compute dot(Jacobian)*u from
  // acceleration(dot(u)=0)
  ptr<MultiBodyTree> tree1(CreateMultiBodyTree(creator));
  ASSERT_TRUE(0x0 != tree1);
  CloneTreeCreator clone(tree1.get());
  // tree2 is used to compute dot(Jacobian)*u using the calculateJacobian
  // function
...
void calculateJacobianError(const MultiBodyTreeCreator& creator,
                            const int nloops, double* max_error) {
  // tree1 is used as reference to compute the Jacobian from velocities with
  // unit u vectors.
  ptr<MultiBodyTree> tree1(CreateMultiBodyTree(creator));
  ASSERT_TRUE(0x0 != tree1);
...
void calculateVelocityJacobianError(const MultiBodyTreeCreator& creator,
                                    const int nloops, double* max_error) {
...
// test nonlinear terms: dot(Jacobian)*u (linear and angular acceleration for
// dot_u ==0)
// from Jacobian calculation method and pseudo-numerically using via the
// kinematics method.
